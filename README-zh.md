...
- **标签：** `ai-agents`, `memory`, `multi-model`
- **推荐理由：** 自进化多模型代理，带记忆能力。
...
- **标签：** `agent-collaboration`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 企业级多代理工作台，统一���持多种编码引擎
...
- **标签：** `vision`, `multimodal`, `dsh-plugin`
- **推荐理由：** 首个视觉插件，让纯文本代理具备图像理解能力
...
- **推荐理由：** 一键切换Claude Code、DeepSeek等多款AI编程工具。
...
- **标签：** `agent-runtime`, `ai-ide`, `dsh-plugin`
- **推荐理由：** 高性能Rust Agent运行时，搭配精美桌面IDE。
...
- **标签：** `agent`, `agent-skill`, `deepseek-harness`, `dsh-plugin`, `skill`
- **推荐理由：** 开发DSH插件的Agent技能，涵盖插件标准与打包流程
...
- **标签：** `plugin-dev`, `harness-tools`, `credentials`
- **推荐理由：** 构建和管理DSH插件、槽位和凭证的必备工具
...
- **推荐理由：** 审计上下文Token成本，检测指令冲突，含Web UI面板。
...
- **标签：** `agent-framework`, `mcp`, `dsh-plugin`, `database`
- **推荐理由：** 42个技能和5个CLI的工具包，覆盖AI智能体开发全流程。
...
- **推荐理由：** 用于构建和验证DSH/Cordis插件的可复用技能。
...
- **推荐理由：** 便携技能，用于DSH插件开发与审计，含bundle适配器。
...
- **推荐理由：** DSH插件开发实战指南，覆盖cordis、tsconfig等踩坑记录。
...
- **推荐理由：** 快速优雅的Neovim插件管理器，适配DSH开发流程。
...
- **标签：** `desktop`, `dsh-plugin`, `desktop-app`
...
- **标签：** `dsh-web`, `plugin-aggregation`, `dsh-plugin`
- **推荐理由：** DSH生态Web端插件聚合分发平台
...
- **标签：** `electron`, `desktop`, `ssh-client`
- **推荐理由：** 功能完整的Electron桌面客户端，支持SSH和远程访问。
...
- **推荐理由：** 复刻锤子便签，一键Docker部署，支持导出公众号格式。
...
- **标签：** `taskboard`, `agent`, `sqlite`
- **推荐理由：** 本地SQLite任务板，Agent认领审查，原生Web界面无iframe。
...
- **⭐ 星标：** 58
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄控制台（浏览器面板管理官�� repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
- **标签：** `console`, `deepseek-harness`, `dsh`, `dsh-plugin`, `plugin-management`, `ui`
- **推荐理由：** 浏览器面板管理插件仓库，0 patch轻量基础设施工具
...
- **标签：** `deepseek-harness`, `desktop-app`, `dsh`, `dsh-plugin`, `electron`, `typescript`
...
- **标签：** `ui`, `mcp`, `capability-management`
...
- **标签：** `usage-tracking`, `ui`, `dsh-plugin`
- **推荐理由：** 常驻底部信息栏，显示模型、余额和峰谷价格倒计时。
...
- **推荐理由：** Tauri 2桌面应用，整合数据库、SSH、Docker与AI。
...
- **推荐理由：** Electron桌面壳封装DSH Web，集成侧边栏与ModLens。
...
- **标签：** `ui`, `theme`, `customization`, `webui`
- **推荐理由：** 自定义网页背景，支持图片上传和预设主题
...
- **标签：** `memory-management`, `self-evolving`, `dsh-plugin`
...
- **标签：** `memory`, `rag`, `ai-agents`
- **推荐理由：** 为AI提供无限记忆存储解决方案
...
- **标签：** `embeddings`, `vector-search`, `skill-md`
- **推荐理由：** 从SKILL.md构建可检索的Skill知识库，支持语义搜索。
...
- **标签：** `memory`, `long-term`, `rag`
- **推荐理由：** 持久化Agent长期记忆，带回忆工具和可检查设置页。
...
- **标签：** `knowledge-base`, `documentation`, `plugin-dev`
- **推荐理由：** 官方插件开发文档知识库，支持中英文
...
- **标签：** `evaluation`, `rag-evaluation`, `dsh-plugin`, `agent-evaluation`
- **推荐理由：** 基于证据的评估框架，支持提示词、RAG和智能体评测。
...
- **标签：** `agent-skills`, `ai-agent`, `deepseek-harness`, `dsh-plugin`
...
- **标签：** `coding`, `agent-skills`, `architecture`
- **推荐理由：** 架构感知的AI编码，证据验证防漂移
...
- **标签：** `workflow`, `conversations`, `productivity`
- **推荐理由：** 将AI对话转化为可复用工作流
...
- **推荐理由：** 全栈自动化数学建模竞赛技能，支持GitOps流水线。
...
- **标签：** `plugin-discovery`, `skills`
- **推荐理由：** 在DSH内直接搜索安装验证GitHub插件。
...
- **标签：** `skills`, `infrastructure`, `agent`
- **推荐理由：** AI基础设施开发专用技能集合，调试优化高性能系统。
...
- **标签：** `workflow`, `agent`, `orchestration`
- **推荐理由：** Agent工作流编排，简化DSH生态中的复杂流程自动化。
...
- **推荐理由：** 移植Superpowers技能：TDD、调试、规划，提升编码质量
...
- **推荐理由：** Superpowers技能移植到Reasonix，支持结构化编码流程
...
- **推荐理由：** AI导师技能：扫描架构、生成学习路径、自动判题和Code Review
...
- **标签：** `agent-plugins`, `agent-skills`, `deep-research`, `research-tools`, `skill-md`
- **推荐理由：** 88个跨平台智能体技能，覆盖研究、营销和业务流程
...
- **标签：** `agent-skills`, `ai-coding`, `deepseek-harness`, `spec-driven-development`, `vibe-coding`
- **推荐理由：** 仓库级规格驱动开发技能，支持spec编程工作流
...
- **标签：** `agent-handoff`, `session-memory`, `workflow`
- **推荐理由：** 跨平台会话记忆，实现无缝Agent交接
...
- **推荐理由：** 可离线验证的版本锁定Swift Cycle治理技能
...
- **标签：** `agent`, `skills`, `dsh-plugin`, `wayfinder`
- **推荐理由：** 游戏化任务系统，通过任务栏自动推进项目进度。
...
- **标签：** `agent-skill`, `engineering`, `fact-finding`
- **推荐理由：** 便携式工程纪律Agent技能，编码高质量决策框架。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，适合游戏工作流。
...
- **标签：** `agent-preset`, `product-management`, `workflows`
- **推荐理由：** 虚拟产品经理团队协作，模拟完整开发流程
...
- **标签：** `agent-skills`, `diagrams`, `dsh-plugin`
- **推荐理由：** 从文本生成精美架构图和工作流图，支持多种图表类型
...
- **标签：** `design-as-code`, `vector-design`, `dsh-plugin`
- **推荐理由：** 首个开源AI矢量设计工具，支持Agent协作与设计即代码
...
- **标签：** `design`, `ppt`, `openpencil`
- **推荐理由：** 对话内预览编辑OpenPencil演示文档，设计辅助利器。
...
- **标签：** `agent-skills`, `claude-code`, `diagram`, `dsh-plugin`, `legal`, `svg`, `timeline`
- **推荐理由：** 法律诉讼可视化工具，生成时间轴和案件结构图
...
- **标签：** `kline`, `visualization`, `finance`
- **推荐理由：** 原生侧栏交互式K线分析，支持技术指标
...
- **标签：** `design-skills`, `ui`, `vibe-coding`, `dsh-plugin`
- **推荐理由：** 6种设计风格，帮助AI生成网站摆脱模板化外观。
...
- **标签：** `agent-skills`, `image-generation`, `vision`, `dsh-plugin`
- **推荐理由：** 免费图片读取与生成技能，支持Vision自动转录。
...
- **标签：** `ai-director`, `ai-video`, `dsh-plugin`, `storyboard`
- **推荐理由：** AI视频/图像/音频技能集成，支持可配置视觉与模型工具。
...
- **标签：** `bilibili`, `xiaohongshu`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 跨平台AI内容发现Agent，覆盖B站、小红书等8大平台。
...
- **推荐理由：** Playwright驱动浏览器自动化插件，DSH生态网页操控利器。
...
- **标签：** `preset`, `harness-engineering`, `dsh-plugin`
- **推荐理由：** 两阶段配置方案：精简引导到完整标准工具链
...
- **标签：** `vfs`, `virtual-filesystem`, `dsh-plugin`
- **推荐理由：** 统一虚拟文件系统，为AI代理提供一致的文件操作
...
- **标签：** `desktop`, `tauri`, `deepseek`
- **推荐理由：** 仅5MB免安装桌面客户端，开箱即用
...
- **推荐理由：** 跨预设会话迁移，固定模式保持状态，支持source-model意图。
...
- **推荐理由：** 崩溃恢复插件，撤销配置变更，密钥安全快照一键SAFE MODE。
...
- **推荐理由：** 对话内构建运行Android模拟器或USB设备，驱动完整交互。
...
- **标签：** `awesome-deepseek-harness`, `awesome-dsh-plugin`, `awesome-list`, `deepseek-harness`, `dsh`
...
- **标签：** `containers`, `ui`, `deployment`
- **推荐理由：** 容器化隔离运行多版本DSH，内置WebView
...
- **标签：** `backup`, `config`, `sync`, `webdav`
- **推荐理由：** 全面备份/恢复/同步DSH配置、插件和MCP
...
- **标签：** `skills`, `manager`, `utilities`
- **推荐理由：** 专门用于管理DSH技能生命周期的插件
...
- **标签：** `plugin-conflict`, `utilities`, `infrastructure`
- **推荐理由：** 自动检测和解决插件版本冲突问题
...
- **⭐ 星标：** 3
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞前检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库���用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `plugin-check`, `utility`, `agent-tools`
- **推荐理由：** 插件库起飞前检查，提升调用可预期性
...
- **标签：** `awesome-list`, `ecosystem`, `plugins`
- **推荐理由：** DSH插件生态系统的精选清单
...
- **标签：** `awesome-list`, `catalog`, `ecosystem`
- **推荐理由：** 生态系统全景概览与工具目录
...
- **推荐理由：** 对话内多源搜番，卡片展示Bangumi评分与磁力，便捷实用。
...
- **标签：** `awesome-list`, `discovery`, `plugins`
- **推荐理由：** 收录14类280+个DSH插件的精选目录
...
- **标签：** `token-usage`, `cost-tracking`, `data-visualization`
...
- **标签：** `awesome-list`, `dsh-plugin`, `2origin`
- **推荐理由：** 插件雷达与2Origin实验室，适合生态概览
...
- **标签：** `recommender`, `mcp`, `dsh-plugin`
- **推荐理由：** AI-curated、每小时更新的插件推荐，12个分类
...
- **标签：** `curated-list`, `dsh-plugin`, `ui`
- **推荐理由：** 涵盖模型、工具、UI和Agent循环的插件指南
...
- **推荐理由：** DeepSeek 专用破甲插件「无限二代」dsh-infinite-gen-2 
...
- **推荐理由：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶
...
- **推荐理由：** Open-source alternative to Claude Cowork
...
- **推荐理由：** ccteam turns the coding agents you alrea
...
- **⭐ 星标：** 277
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，��你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 G
...
- **推荐理由：** Use ChatGPT (Codex), Claude, and Grok (X
...
- **推荐理由：** AI skills大全 mcp ai知识库 Agent 全维度 AI 资源百科，
...
- **标签：** `plugin-marketplace`, `plugins`
- **推荐理由：** DeepSeek Harness插件市场与集合中心。
...
- **标签：** `marketplace`, `plugin`, `browser`
- **推荐理由：** 一键浏览安装更新DSH插件市场全部插件，生态聚合工具。
...
- **推荐理由：** 社区插件市场整合进DSH网页GUI，浏览安装一键搞定。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** DSH内测收官合影墙，支持GitHub OAuth零权限登录。
...
- **标签：** `migration`, `import`, `claude-code`, `codex`
- **推荐理由：** 一键迁移Claude Code/Codex/OpenCode配置到DSH生态。
...
- **标签：** `awesome-list`, `quality-check`, `2026`
- **推荐理由：** 2026年经质量筛选的DSH插件精选列表。
...
- **标签：** `registry`, `awesome-list`, `discovery`
...
- **标签：** `registry`, `awesome-list`, `verified`
- **推荐理由：** 经过验证的插件目录和权限安装指南
...
- **标签：** `registry`, `awesome-list`, `collection`
- **推荐理由：** 实用DeepSeek Harness插件合集目录
...
- **标签：** `portal`, `static-site`, `discovery`, `web`
...
- **标签：** `registry`, `awesome-list`, `resources`
- **推荐理由：** 精选插件、工具、技能资源总览
...
- **标签：** `registry`, `bundle`, `plugins`
- **推荐理由：** 各类DSH插件和Bundle列表
...
- **标签：** `registry`, `automation`, `awesome-list`, `bilingual`
...
- **标签：** `marketplace`, `dsh-plugin`, `dsh-plugin-market`
- **推荐理由：** 类CodeX的DSH插件市场，便于浏览和安装
