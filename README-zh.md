...
- **标签：** `routing`, `harness-tool`, `reasoning-mode`
- **推荐理由：** 运行时注入器+任务感知推理路由，优化DSH体验。
...
- **标签：** `multi-agent`, `enterprise`, `agent-workbench`
- **推荐理由：** 企业级统一多Agent工作台，支持Codex/DSH/OpenCode。
...
- **标签：** `vfs`, `virtual-terminal`, `agent-sandbox`
- **推荐理由：** 虚拟终端配合VFS，为AI提供隔离沙箱环境
...
- **标签：** `vision`, `toolkit`, `ocr`
- **推荐理由：** 为纯文本模型提供视觉工具包：图像问答与OCR。
...
- **推荐理由：** 支持RPC、子Agent、hashline编辑和MCP的编码智能体
...
- **推荐理由：** 一键配置多AI Agent的MCP和Skills，节省时间
...
- **标签：** `sync`, `codex`, `migration`
- **推荐理由：** Codex与DSH双向同步桥梁，无缝迁移
...
- **标签：** `migration`, `claude-code`, `dsh`
- **推荐理由：** 从Claude Code/Codex迁移到DSH的向导工具
...
- **标签：** `launcher`, `orchestration`, `dsh`
- **推荐理由：** 统一编排Runtime、Provider、Plugin和MCP
...
- **标签：** `agent-skill`, `development`
- **推荐理由：** 标准化 DSH 插件开发工作流
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** 类Claude Code的TUI界面，含实时流式思考和回滚
...
- **标签：** `desktop`, `agent-runtime`, `ai-ide`
- **推荐理由：** Rust高性能Agent运行时，搭配精美桌面客户端。
...
- **推荐理由：** 免安装Windows桌面GUI客户端，支持SSH和移动远程访问
...
- **标签：** `desktop-app`, `electron`, `mcp`
- **推荐理由：** 本地优先AI桌面工作区，支持项目、文件和MCP集成
...
- **推荐理由：** 本地优先多模型桌面应用，替代Claude Cowork
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核，UI简洁
...
- **推荐理由：** 原生移动客户端，在手机上享受DSH工作区与Agent轨迹
...
- **推荐理由：** 跨平台桌面客户端，CodePilot风格体验，含插件套件
...
- **标签：** `notes`, `ui`, `dsh-plugin`
- **推荐理由：** 开源锤子便签风格笔记，支持Docker部署。
...
- **标签：** `ui`, `market`, `deepseek-harness`
- **推荐理由：** Web GUI社区插件市场，一键安装
...
- **标签：** `ui`, `capability`, `dsh`
- **推荐理由：** 统一能力菜单，支持三级暴露策略
...
- **标签：** `workbench`, `typescript`, `agent-platform`, `plugin-system`
- **推荐理由：** 基于DSH的插件化Web工作台，全局技能管理与工作区集成。
...
- **标签：** `desktop`, `devops`
- **推荐理由：** 一体化 DevOps 桌面：SSH、Docker、AI
...
- **标签：** `desktop`, `electron`
- **推荐理由：** Electron 桌面封装 DSH Web + 侧边栏 + ModLens
...
- **推荐理由：** 本地优先的Markdown原生记忆层，跨应用自我进化。
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** 基于Milvus的统一持久记忆层，支持多Agent共享。
...
- **标签：** `skill-reranking`, `vector-search`, `agent-memory`
- **推荐理由：** 将分散的SKILL.md文件转为可检索的Agent技能库
...
- **推荐理由：** 基于证据的Prompt、RAG和Agent评估框架
...
- **推荐理由：** 跨设备Agent工作空间，看板式任务管理支持API访问。
...
- **标签：** `skills`, `workflow-automation`, `routing`
- **推荐理由：** 智能技能路由，在SkillsBench上节省29% token
...
- **标签：** `cognitive-enhancement`, `reasoning`, `inference-control`
- **推荐理由：** 推理时控制套件，支持深度推理、验证和恢复
...
- **标签：** `video`, `skill`, `ai-generation`
- **推荐理由：** 一键生成Vox风格拼贴解说视频的Agent技能。
...
- **标签：** `agent-teams`, `workflow`, `dsh`
- **推荐理由：** 支持DSH内多Agent团队协作能力。
...
- **标签：** `writing`, `skill`, `novel`
- **推荐理由：** 支持本地与云端的AI小说创作全流程工作流。
...
- **标签：** `agent-observability`, `video-analysis`, `OCR`
- **推荐理由：** 为AI提供视听感知能力，输出可验证的带时间戳证据
...
- **标签：** `creative-writing`, `novel-writing`, `workflow`
- **推荐理由：** 内置小说短剧游戏视频解说四个工作台，创作工作流完整
...
- **推荐理由：** DSH内嵌实时iOS模拟器，22个Agent工具控制真机
...
- **标签：** `agent-skills`, `workflow`, `gitops`
- **推荐理由：** 全栈数学建模自动化助手，支持国赛美赛。
...
- **标签：** `skills`, `ai-infra`, `deepseek-harness`
...
- **标签：** `skills`, `agent-learning`, `dsh`
- **推荐理由：** 将成功会话转为可复用智能体技能
...
- **推荐理由：** 适配TDD、调试、规划等Superpowers技能
...
- **推荐理由：** AI编程导师，扫描项目架构，阶梯式教学，自动判题与代码审查。
...
- **标签：** `agent-skills`, `mcp`, `web-search`, `multi-agent`
- **推荐理由：** Web搜索、研究、金融与集成发现技能，覆盖多Agent平台。
...
- **⭐ 星标：** 71
- **语言：** JavaScript
- **描述：** Matt Pocock 技能面板（DSH Deck）：安装即自带 25 个工程与效率技能（mattpocock/skills v1.2.3：triage、grilling、wayfinder、handoff 等），右侧面板直接调用，无需手动装技能。当前主力支持 GitHub issue；Markdown 本地文件为预览版；GitLab 暂不在支持范围。希望它能帮你把想法���快变成现实。
- **标签：** `dsh-plugin`, `skills`, `github-issues`, `wayfinder`
- **推荐理由：** 内置25个工程效率技能，右侧面板直调，开箱即用。
...
- **标签：** `legal`, `visualization`, `svg`, `timeline`, `agent-skill`
- **推荐理由：** 法律诉讼可视化工具集：时间轴、大事记、庭审对抗图。
...
- **标签：** `gongwen`, `docx`, `dsh-plugin`, `skill`
- **推荐理由：** 党政机关公文全流程处理，格式校验、内容优化、模板生成。
...
- **推荐理由：** 插件开发知识库以Skill形式安装，附官方文档与Cordis入门。
...
- **标签：** `fengshui`, `iching`, `dsh-plugin`
- **推荐理由：** 融合八字风水等中华传统智慧的确定性引擎。
...
- **标签：** `ocr`, `vision`, `dsh-plugin`
- **推荐理由：** 让纯文本模型通过OCR读取解析图片。
...
- **标签：** `deepseek-harness`, `guard`, `quality-gate`
- **推荐理由：** 工程纪律守卫，检验需求、测试实现、证明交付。
...
- **标签：** `qa`, `test-automation`, `dsh-plugin`
- **推荐理由：** 11个QA技能框架，让AI像资深测试工程师。
...
- **推荐理由：** WorkBuddy风格技能选择器，快速切换DSH技能
...
- **标签：** `kline`, `finance`, `dsh-plugin`
- **推荐理由：** 交互式K线分析，支持技术指标和基本面分析
...
- **推荐理由：** 基于Tushare的财经数据库，自动生成MCP接口。
...
- **标签：** `mcp`, `api-documentation`, `chinese-api`, `dsh-plugin`
- **推荐理由：** 搜索20个中国平台65,600+API文档，零配置支持Skill与MCP。
...
- **标签：** `mcp`, `cloud`, `huaweicloud`, `sdk`, `terraform`
- **推荐理由：** 华为云官方工具包，提供Skill、MCP工具与云沙箱支持。
...
- **标签：** `ai-video`, `directorx`, `dsh-plugin`
- **推荐理由：** AI视频/图片/音频技能，支持分镜脚本。
...
- **标签：** `content-discovery`, `cross-platform`, `ai-agent`
- **推荐理由：** 跨平台内容发现Agent，支持B站/小红书/抖音等8+平台
...
- **推荐理由：** 持久身份浏览器，支持指纹伪装和Playwright自动化
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号在DSH内聊天。
...
- **标签：** `mcp-server`, `dsh-plugin`, `resume-builder`
- **推荐理由：** 隐私优先的开源简历构建器
...
- **标签：** `image`, `dsh-plugin`, `electron`
- **推荐理由：** 支持60+平台的终极图片上传工具
...
- **标签：** `model-switch`, `install`
- **推荐理由：** 一键安装并切换Claude Code/Codex/DSH等9大平台
...
- **标签：** `utilities`, `ecosystem`, `plugin`
- **推荐理由：** 开源DSH生态雷达，发现并验证两万余插件。
...
- **标签：** `context`, `utilities`, `dsh`
- **推荐理由：** 提供上下文仪表盘、侧边栏及管理命令的工具。
...
- **标签：** `shop`, `marketplace`, `plugin`
- **推荐理由：** 全面插件商城，每日更新，全网来源。
...
- **推荐理由：** 集成Command Code模型，支持实时目录和按计划选模型
...
- **标签：** `config`, `backup`, `dsh`
- **推荐理由：** 备份恢复DSH配置、插件和MCP
...
- **标签：** `plugin-management`, `ui`, `console`, `dsh-plugin`
- **推荐理由：** 薄控制台管理官方仓库插件，零patch，附开发引导。
...
- **标签：** `enhancement`, `terminal`
- **推荐理由：** 任务通知、LLM 跟踪、快捷键增强
...
- **标签：** `cost`, `visualization`
- **推荐理由：** 实时 Token 用量、成本图表、API 余额
...
- **⭐ 星标：** 7
- **语言：** TypeScript
- **描述：** 致力于解决各插件之间也与版本之间可能存在的冲突问题并提���解决方案
- **标签：** `debug`, `conflict-resolution`
- **推荐理由：** 自动检测并解决插件冲突
...
- **标签：** `widget`, `floating`, `theme`
- **推荐理由：** 可爱鲸鱼浮动组件，实时追踪DeepSeek账户余额
...
- **标签：** `theme`, `animation`, `ocean`
- **推荐理由：** Three.js驱动的实时海洋动态主题，支持玻璃效果调节
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗��遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `dsh-plugin`, `webui`, `theme`
- **推荐理由：** 支持自定义上传和预设主题美化DSH网页界面。
...
- **标签：** `documentation`, `deepwiki`, `analysis`
- **推荐理由：** 从代码生成精确技术文档和AI就绪上下文
...
- **标签：** `awesome-list`, `encyclopedia`, `ai-resources`
- **推荐理由：** 全面AI资源百科，覆盖Agent、RAG、MLOps等全维度
...
- **标签：** `academic-search`, `mcp`, `agent-skills`, `research-workflow`
...
- **标签：** `agent-skills`, `deep-research`, `marketing`
- **推荐理由：** 88个研究、营销、社交智能Agent技能。
...
- **标签：** `research`, `industry`, `deepseek-harness`
- **推荐理由：** 行业公司研究方法论与数据追踪
...
- **标签：** `patent`, `research`, `rag`, `multi-agent`
- **推荐理由：** 专利侵权分析系统，自动生成竞品报告，可被任意Agent调用。
...
- **标签：** `tutorial`, `learning`, `harness-engineering`
- **推荐理由：** 从零开始学习Harness工程的基础教程。
...
- **标签：** `awesome-list`, `documentation`, `handbook`
- **推荐理由：** 全面的DSH手册，涵盖运行时、MCP等。
...
- **标签：** `marketplace`, `community`, `deepseek-harness`
- **推荐理由：** 社区维护自动同步的插件市场
...
- **标签：** `awesome`, `resources`, `dsh`
- **推荐理由：** 全面的DSH资源清单：插件、工具和技能
...
- **标签：** `dsh-plugins`, `repository`, `plugin-verify`
- **推荐理由：** 带验证功能的 DSH 插件仓库
...
- **标签：** `awesome-list`, `archived`, `redirect`
- **推荐理由：** 已归档，已重定向至新仓库
...
- **标签：** `plugin-directory`, `curated-list`, `typescript`
- **推荐理由：** 覆盖模型、工具、UI的智能体注册表
