...
- [通信与社交](communication-social-zh) (4)
...
- **标签：** `routing`, `mcp`, `harness-tools`
- **推荐理由：** 任务感知路由与注入工具包，支持推理模式
...
- **推荐理由：** 首个面向纯文本编码代理的视觉插件，支持OCR和结构化JSON输出
...
- **推荐理由：** 全球首个虚拟终端，支持虚拟文件系统和终端的沙箱环境
...
- **标签：** `agent-runtime`, `desktop`, `rust`, `coding-agent`
- **推荐理由：** 高性能Rust运行时配合精美桌面应用的AI编码代理平台
...
- **标签：** `agent-skills`, `computer-vision`, `dsh-plugin`, `ocr`
- **推荐理由：** 一行安装视觉工具箱，支持图片问答与长截图OCR。
...
- **标签：** `coding-agent`, `cli`, `mcp`, `rpc`
- **推荐理由：** 终端编码代理，支持RPC、子代理和MCP。
...
- **推荐理由：** 内置iOS模拟器，提供22个Agent移动自动化工具
...
- **推荐理由：** Superpowers技能移植到Reasonix编程框架
...
- **标签：** `coding`, `mentor`, `review`
- **推荐理由：** AI编程导师，指导架构生成阶梯式章节并Code Review
...
- **标签：** `skills`, `plugin-management`
- **推荐理由：** 统一加载与管理本地Agent Skills
...
- **标签：** `plugin-dev`, `coding-agents`
- **推荐理由：** 快速构建与验证DSH插件的Agent Skill
...
- **标签：** `plugin-dev`, `codex`
- **推荐理由：** 可复用的DSH插件构建与验证Codex Skill
...
- **推荐理由：** 审计上下文token成本，检测指令链与工具schema重复冲突
...
- **标签：** `game-development`, `godot-engine`, `dsh-plugin`
...
- **标签：** `sidebar`, `ui`, `desktop-extension`, `dsh`
- **推荐理由：** 可扩展侧边栏底座，内置文件编辑器、终端、Git和子代理页面
...
- **标签：** `taskboard`, `ui`, `coding-agent`, `skills`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH���面集成
...
- **推荐理由：** Claude Code风格TUI，鲸鱼主题，实时状态与流式思考显示
...
- **标签：** `widget`, `ui`, `floating`, `dsh`
- **推荐理由：** 可爱的浮动小鲸鱼插件，监控DeepSeek账户余额并带动画效果
...
- **标签：** `deepseek-harness`, `desktop`, `dsh-plugin`, `tauri`
- **推荐理由：** 轻量Tauri桌面客户端，零配置跨平台支持。
...
- **标签：** `agentic-workflows`, `dsh-plugin`, `electron`, `desktop-app`
- **推荐理由：** 本地优先的Electron桌面工作台，支持项目与Office文件。
...
- **推荐理由：** 开源Windows桌面客户端，支持插件、SSH和远程访问。
...
- **标签：** `desktop`, `tauri`, `local-first`, `multi-model`
- **推荐理由：** 本地优先多模型桌面应用，支持自进化技能。
...
- **推荐理由：** CodePilot风格桌面客户端，跨平台Electron应用
...
- **标签：** `mobile`, `ios`, `native`
- **推荐理由：** 原生iOS客户端，通过WebSocket连接工作区和会话
...
- **推荐理由：** 开源锤子便签，支持Docker部署和Skill调用
...
- **标签：** `mcp`, `skills`, `webui`
- **推荐理由：** Web界面统一管理skill和MCP工具，便捷高效
...
- **标签：** `ui`, `mcp`, `capability`
- **推荐理由：** 统一菜单管理工具暴露级别和执行模式
...
- **标签：** `desktop`, `ui`, `plugin-management`
- **推荐理由：** Electron桌面控制台，统一管理平台插件与MCP
...
- **推荐理由：** 底部信息栏显示提供商、余额、定价及倒计时，持久化展示
...
- **推荐理由：** DSH编写器内WorkBuddy风格技能选择器，一键插入/skill-name命令
...
- **推荐理由：** 面向临床和实验科学家的生物信息学桌面agent，支持离线和技能商店
...
- **推荐理由：** DevOps一体化桌面，集成DB/SSH/Docker/SFTP
...
- **推荐理由：** 腾讯开源LLM知识平台，支持RAG和自维护Wiki。
...
- **标签：** `memory`, `agentic-rag`, `continual-learning`
- **推荐理由：** 状态记忆库，支持持续学习与状态保持
...
- **标签：** `memory`, `rag`, `agent-memory`, `knowledge`
- **推荐理由：** 专业代理记忆管理套件，支持记忆存储与智能提炼功能
...
- **标签：** `agent`, `memory`, `dsh-plugin`, `long-term-memory`
- **推荐理由：** 个人AI记忆中枢，为所有代理提供共享持久上下文。
...
- **标签：** `agent-memory`, `dsh-plugin`, `embeddings`, `semantic-search`
- **推荐理由：** 将分散的技能文件转为可检索语料库，支持语义搜索。
...
- **标签：** `evaluation`, `rag-evaluation`, `dsh-plugin`
- **推荐理由：** 基于证据的提示词与RAG评估可观测性框架
...
- **标签：** `skills`, `skills-store`, `agent-skills`
- **推荐理由：** 最全技能商店，自动抓取并分类安装技能包
...
- **标签：** `agent-control-plane`, `workflow`, `loopx`
- **推荐理由：** 长程Agent控制平面，支持持久化多Agent工作流
...
- **标签：** `preset`, `skills`, `workflow`, `deepseek-harness`
- **推荐理由：** 两阶段预设，最小引导配合完整标准工具，适合DSH快速部署
...
- **标签：** `skills`, `workflow`, `orchestration`, `agent-skills`
- **推荐理由：** 智能技能路由与工作流编排，令牌消耗降低近30%
...
- **标签：** `cognitive-enhancement`, `agent-skills`, `j-space`, `workflow`
...
- **标签：** `ai-video`, `dsh-plugin`, `ffmpeg`, `agent-skill`
- **推荐理由：** 端到端自动化Vox风格视频生成技能，集成ffmpeg管线。
...
- **标签：** `dsh-plugin`, `research`, `web-search`, `claude-code`
...
- **标签：** `task-management`, `agent`, `native`
- **推荐理由：** 原生SQLite任务板，支持Agent认领审查工作流，无需iframe
...
- **标签：** `competition`, `modeling`, `skill`
- **推荐理由：** 全栈数学建模竞赛Skill，支持GitOps流水线和代码自证
...
- **标签：** `acceptance-review`, `plan-mode`, `prompt-engineering`
- **推荐理由：** 支持Agent可组合自进化与人工审查改进
...
- **推荐理由：** 基于Tushare的多Agent金融分析系统，支持Skill拓展
...
- **推荐理由：** 为纯文本LLM添加多模态视觉能力，用于截图UI分析
...
- **标签：** `patent`, `legal-tech`, `skills`
- **推荐理由：** 专利侵权分析系统，支持多agent调用
...
- **标签：** `spec-driven`, `skills`, `coding`
- **推荐理由：** 仓库原生规格驱动开发Agent Skill
...
- **推荐理由：** 官方DSH插件开发知识库，中英双语，一键加载为agent技能
...
- **推荐理由：** 多agent协作框架，独立模型+技能+MCP工具，共享工作空间
...
- **推荐理由：** 八字风水奇门等中华传统智慧融合引擎，本地确定性计算+可视化面板
...
- **推荐理由：** 改编自Codex的代码简化技能，可作为DSH agent技能使用
...
- **推荐理由：** 11个QA测试技能+共享知识库+类型决策矩阵，适用于AI Agent
...
- **推荐理由：** 跨平台agent交接技能，保留仓库级连续记忆与任务状态
...
- **标签：** `agent-skills`, `ai-roleplay`, `dsh-plugin`
- **推荐理由：** NPC互动叙事技能，含记忆/立场/YAML存档
...
- **标签：** `skill`, `dsh-plugin`
- **推荐理由：** 腾讯官方SkillHub Open API文档与插件
...
- **标签：** `dsh-plugin`, `skill`, `plugin-dev`
- **推荐理由：** 指导AI从0到1开发、构建、安装DSH插件
...
- **标签：** `video`, `remotion`, `agent-skills`
...
- **推荐理由：** DSH插件开发技能，支持Claude Code/Codex
...
- **标签：** `diagrams`, `mermaid`, `architecture-as-code`
- **推荐理由：** 从代码生成精美架构图和时序图的Agent技能。
...
- **标签：** `ai-design`, `deepseek-harness`, `dsh-plugin`, `visual-editor`
- **推荐理由：** 原生设计系统，含AI生成、可视化编辑与PPT工作室。
...
- **推荐理由：** AI视频/图像/音频技能集成DirectorX，支持可配置视觉媒体模型
...
- **标签：** `content-discovery`, `browser`, `cross-platform`, `automation`
- **推荐理由：** 跨平台AI内容发现代理，支持B站、YouTube、Reddit等多个���台
...
- **推荐理由：** 让AI代理使用真实登录浏览器，支持CLI与扩展集成。
...
- **标签：** `utility`, `model-switch`, `multi-agent`, `infrastructure`
- **推荐理由：** 一键切换Claude Code、Codex、DeepSeek、Kimi等多种编码代理
...
- **推荐理由：** OAuth接入ChatGPT/Claude/Grok订阅作LLM提供者。
...
- **推荐理由：** 每日自动抓取GitHub，精选真实插件并剔除蹭标签项目
...
- **推荐理由：** 会话和每日成本追踪，支持预算提醒和Token用量历史
...
- **标签：** `learning`, `marketplace`, `education`
...
- **推荐理由：** 精选插件、技能、MCP服务器和UI列表，含可视化功能
...
- **推荐理由：** 插件市场含11000+插件，支持搜索、排行和公共API
...
- **标签：** `remote-access`, `dsh-plugin`, `security`
...
- **标签：** `container`, `multi-version`, `dsh-plugin`
- **推荐理由：** 容器隔离运行多个DSH版本，内嵌WebView，一键导入插件技能
...
- **标签：** `theme`, `pets`, `pixel-art`, `mascot`
- **推荐理由：** 多AI编码工具的动态宠物主题展示库
...
- **标签：** `theme`, `webgpu`, `ocean`, `chrome-extension`
- **推荐理由：** WebGPU海洋主题扩展，Three.js驱动视觉皮肤。
...
- **标签：** `ai`, `rag`, `skills`, `tutorials`
- **推荐理由：** 全面的AI资源库，含教程、提示词与知识百科
...
- **标签：** `tutorial`, `harness-engineering`, `agent`
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `ecosystem-radar`, `plugin-registry`
- **推荐理由：** 可自部署的插件生态雷达，自动发现并CI验证15900+候选。
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugin`, `plugins`
- **推荐理由：** DSH生态插件、工具与基础设施的精选列表。
...
- **推荐理由：** 涵盖CLI、Web UI和代理功能的全面插件生态列表。
...
- **推荐理由：** 科研全周期工作台：LaTeX编译、arXiv管理、GPU服务器编排
...
- **推荐理由：** 交互式K线分析插件，支持多市场数据、技术指标及新闻基本面分析
...
- **推荐理由：** 基于证据的DSH插件雷达和2Origin插件实验室。
...
- **标签：** `awesome-list`, `mcp`
- **推荐理由：** AI策划的190+ DSH插件每小时更新列表，12个分类。
...
- **标签：** `plugin-market`, `curated-list`, `daily-update`
- **推荐理由：** 每日更新插件商城，经审核，资源丰富。
...
- **推荐理由：** 收录1819+插件的DSH目录，支持中英文搜索筛选
...
- **标签：** `handbook`, `agent-runtime`, `awesome-list`
- **推荐理由：** 173+资源的DSH综合手册，覆盖运行时与插件
...
- **标签：** `2026`, `quality`, `curated`
- **推荐理由：** 2026年DSH插件质量检查列表
...
- **标签：** `star-ranked`, `community`, `verified`
- **推荐理由：** 星级排序的社区DSH插件
...
- **推荐理由：** DeepSeek Harness版的类CodeX插件市场。
