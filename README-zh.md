...
- **标签：** `dsh`, `harness`, `agent`
- **推荐理由：** 从零开始的DSH框架工程入门教程
...
- **标签：** `multi-agent`, `dsh-plugin`, `workspace`
- **推荐理由：** 企业级多智能体工作台，跨框架统一工作空间
...
- **推荐理由：** 首款视觉插件，支持OCR与图像理解，��能纯文本编码代理
...
- **推荐理由：** 全球首个虚拟终端，支持VFS和虚拟终端的AI代理工具
...
- **推荐理由：** 高性能Rust代理运行时，搭配精美桌面应用进行智能编码
...
- **标签：** `code-review`, `lint`, `ai-audit`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，提供衰减诊断与自动修复
...
- **推荐理由：** 编码Agent，支持RPC、子Agent、哈希编辑和MCP
...
- **推荐理由：** iOS模拟器自动化，22个Agent工具助力移动开发
...
- **标签：** `catalog`, `plugin-discovery`
- **推荐理由：** DSH插件总目录，支持搜索、筛选和排序，一站式发现工具。
...
- **推荐理由：** 打通Pi与DSH生态，Pi扩展可无缝作为DSH插件运行。
...
- **推荐理由：** 精选DSH插件合集列表，便于发现和了解社区优质插件。
...
- **标签：** `coding`, `reasonix`, `superpowers`, `skills`
...
- **标签：** `plugin-dev`, `skills`, `tooling`, `development`
- **推荐理由：** 开发DSH插件的Agent Skill标准指南
...
- **标签：** `plugin-dev`, `build`, `tooling`
- **推荐理由：** 构建和管理DSH插件的Agent技能
...
- **标签：** `plugin-dev`, `validation`, `creator`
...
- **标签：** `knowledge-base`, `docs`, `plugin-dev`, `guide`
- **推荐理由：** 插件开发知识库和官方文档归档
...
- **标签：** `godot`, `godot-engine`, `gdscript`
- **推荐理由：** Godot 4.x全栈游戏开发技能，支持GDScript编码与AI辅助开发
...
- **标签：** `plugin-development`, `dsh-plugins`, `skill`
- **推荐理由：** 指导AI Agent从零开发DSH插件，覆盖构建、安装与测试全流程
...
- **标签：** `dsh-plugin`, `dsh-plugin-market`, `awesome-dsh-plugin`
- **推荐理由：** DSH插件市场，类似CodeX的插件发现中心
...
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/Git/对话等功能
...
- **标签：** `taskboard`, `ui`, `dsh-plugin`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DeepSeek Harness
...
- **标签：** `tui`, `ui`, `dsh-plugin`
- **推荐理由：** Claude Code风格TUI，含鲸鱼顶栏/流式思考/回滚等功能
...
- **推荐理由：** 萌趣鲸鱼浮窗插件，实时追踪DeepSeek余额，支持拖拽吸附
...
- **推荐理由：** 5MB零配置桌面端，支持Windows/macOS/Linux，开箱即用
...
- **标签：** `desktop`, `mcp`, `local-first`
- **推荐理由：** Local-first AI desktop workspace with DSH sessions, MCP, web research, and Office automation
...
- **标签：** `gui`, `interactive`, `components`, `charts`
- **推荐理由：** 在助手回复中内联渲染交互UI组件，支持图表表单 quiz
...
- **标签：** `desktop`, `windows`, `electron`, `ssh`
- **推荐理由：** 开源Windows桌面客户端，内置Codex、插件、SSH和移动端远程访问
...
- **标签：** `desktop`, `local-first`, `multi-model`, `privacy`
- **推荐理由：** 本地优先多模型AI桌面应用，自进化技能，隐私优先
...
- **标签：** `canvas`, `workspace`, `conversation`, `branching`
- **推荐理由：** 基于画布的非线性对话工作区，支持分支会话管理
...
- **标签：** `office`, `document`, `spreadsheet`
- **推荐理由：** 为DSH带来电子表格、文档、幻灯片等办公功能
...
- **标签：** `ios`, `mobile`, `ui-extension`
- **推荐理由：** 原生iOS客户端，支持远程实时查看Agent执行轨迹。
...
- **推荐理由：** 开源锤子便签复刻版，支持Docker私有化部署和技能调用。
...
- **标签：** `ui`, `menu`, `capability`, `controls`
...
- **推荐理由：** Electron桌面控制台，管理DSH插件、MCP和技能
...
- **标签：** `plugin-management`, `ui`, `console`, `repository`
- **推荐理由：** 插件注册表控制台和官方开发引导技能
...
- **推荐理由：** 面向临床和实验室的独立生信桌面应用，支持一键安装和离线模式
...
- **推荐理由：** 集成数据库/SSH/SFTP/Docker/AI助手的运维桌面中心，功能全面
...
- **标签：** `ui`, `webui`, `button`, `compact`
- **推荐理由：** DSH Web上下文面板一键折叠按钮，精简界面
...
- **标签：** `memory`, `agentic-rag`, `llm`
- **推荐理由：** 带Agentic RAG能力的有状态Agent记忆库
...
- **标签：** `memory`, `rag`, `agent-memory`, `dsh-plugin`
- **推荐理由：** 专为代理设计的记忆管理套件，支持记忆范围和精��
...
- **推荐理由：** 持久化统一记忆层，基于Markdown和Milvus构建
...
- **标签：** `context`, `management`, `rAG`
- **推荐理由：** DSH最佳上下文管理插件，带仪表板和浏览器视图的全功能控制
...
- **标签：** `skill-corpus`, `retrieval`, `agent-memory`, `embeddings`
- **推荐理由：** 将SKILL.md文件转化为可检索的agent技能库，含评估工���
...
- **标签：** `memory`, `rag`, `knowledge`, `consolidation`
- **推荐理由：** 主动关联记忆，三层自动巩固机制
...
- **推荐理由：** 支持提示词/RAG/技能/Agent的多评测框架，含多裁判集成评估
...
- **标签：** `meta-skill`, `knowledge-distillation`, `dsh-plugin`
- **推荐理由：** 提炼专家思维为可复用智能体技能。
...
- **标签：** `agent-skills`, `workflow`, `mcp`
- **推荐理由：** 统一多设备智能体工作空间，支持任务看板编排
...
- **标签：** `agent-os`, `llm-orchestration`, `mcp`
- **推荐理由：** 自进化Agent OS，带分阶段评估与预算进化循环
...
- **标签：** `loop-engineering`, `agent-ops`, `workflow`
- **推荐理由：** 多引擎长期Agent控制面，支持持久治理工作流
...
- **标签：** `skills`, `workflow`, `agent-framework`, `dsh-plugin`
- **推荐理由：** 智能技能路由与工作流编排，奖励提升21pp，Token减少30%
...
- **推荐理由：** 基于Anthropic J-space���局工作空间研究的认知增强技能
...
- **标签：** `video`, `agent-skill`, `generative-ai`
- **推荐理由：** 输入一个主题自动生成Vox风格拼贴视频，全流程自动化
...
- **标签：** `agent-teams`, `multi-agent`, `workflow`
- **推荐理由：** 在DeepSeek Harness中实现多智能体团队协作工作流
...
- **标签：** `multimodal`, `perception`, `computer-vision`, `audio`
- **推荐理由：** 本地优先的多模态感知栈，支持视频音频屏幕理解
...
- **标签：** `task-management`, `native`, `sqlite`, `workflow`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核，无iframe
...
- **推荐理由：** 专注小说创作和短剧制作的DSH插件，内置Drama Skills引擎
...
- **标签：** `skill`, `math-modeling`, `workflow`
- **推荐理由：** 全栈自动数学建模技能，支持GitOps流水线
...
- **标签：** `agent-evolution`, `workflow`, `review`
- **推荐理由：** 可组合任务控制，支持人类审查的自演进Agent
...
- **标签：** `skills`, `architecture`, `agent`
- **推荐理由：** 将经验转化为可复用执行流程，支持多AI平台。
...
- **推荐理由：** 自动从GitHub搜索、安装并验证DSH插件，省时高效。
...
- **推荐理由：** 面向AI基础设施开发的专用技能，支持代码编写和调试。
...
- **推荐理由：** 87个逆向工程技能，适用于CTF和渗透测试安全研究。
...
- **标签：** `skills`, `agent-skills`, `automation`, `learning`
...
- **标签：** `superpowers`, `skills`, `tdd`, `coding`
- **推荐理由：** TDD/调试/规划/协作等Superpowers技能
...
- **标签：** `skills`, `mentor`, `coding`, `education`
- **推荐理由：** AI编程导师技能：架构扫描/代码评审/阶梯式教学
...
- **标签：** `skills`, `agent`, `productivity`, `github`
- **推荐理由：** 25个即用工程与效率技能，支持GitHub issue
...
- **⭐ 星标：** 41
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向��文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `workflow`, `formatting`
...
- **标签：** `spec`, `development`, `workflow`, `coding`
- **推荐理由：** 仓库原生Spec驱动开发技能
...
- **标签：** `skillhub`, `open-api`, `tencent`
- **推荐理由：** Tencent SkillHub官方API文档与示例，便于集成腾讯技能到DSH
...
- **推荐理由：** OAuth绑定ChatGPT订阅账号，在DSH内使用其模型
...
- **标签：** `architecture-diagram`, `mermaid-alternative`, `dsh-plugin`
- **推荐理由：** 代码生成精美架构图和流程图。
...
- **标签：** `mascot`, `visual`, `developer-tools`
- **推荐理由：** 为编码Agent工具添加个性动画宠物的扩展
...
- **标签：** `vision`, `computer-vision`, `OCR`
- **推荐理由：** 为纯文本模型设计的视觉工具箱，支持OCR、多图问答和截图还原
...
- **推荐理由：** AI-generated design system with visual editor, template marketplace, and PPT studio built natively
...
- **标签：** `visualization`, `legal`, `diagram`, `timeline`
...
- **推荐理由：** 集成AI视频/图像/音频生成与编辑技能，支持创意工作流
...
- **标签：** `design-skills`, `design-system`, `ui`
- **推荐理由：** 内置暗SaaS/苹果极简/新拟态等6种设计风格，提升网页输出质量
...
- **标签：** `media-skills`, `image-generation`, `vision`
- **推荐理由：** 支持图像读取与生成的视觉技能，可粘贴图片进行AI分析
...
- **推荐理由：** Convert chat history into shareable knowledge cards
...
- **标签：** `content-discovery`, `cross-platform`, `ai-agent`, `dsh-plugin`
- **推荐理由：** 本地优先跨平台内容发现代理，覆盖B站/YouTube/知乎等平台
...
- **推荐理由：** 让AI代理直接操作已登录的真实浏览器，无需中断工作流
...
- **标签：** `web-search`, `free`, `bridge`, `no-api-key`
- **推荐理由：** 免费网络搜索插件，无需注册或API密钥，桥接无网访问模型
...
- **推荐理由：** DingTalk bot turns chats into tool-capable agents
...
- **标签：** `electron`, `image-uploader`, `dsh-plugin`
...
- **标签：** `utility`, `model-switch`, `dsh-plugin`
- **推荐理由：** 一键多代理工具切换器，支持Claude Code/Codex/DeepSeek等
...
- **标签：** `ecosystem`, `radar`, `infrastructure`
- **推荐理由：** 自部署插件生态雷达，发现15900+候选并支持k8s实测管线
...
- **标签：** `oauth`, `subscriptions`, `provider`, `integration`
...
- **标签：** `cost`, `billing`, `metering`
- **推荐理由：** 追踪会话/日费用、预算和Token使用记录
...
- **标签：** `remote-access`, `p2p`, `security`
- **推荐理由：** 端到端加密P2P方案，支持多端远程访问操作Harness。
...
- **标签：** `upgrade`, `maintenance`, `utility`
- **推荐理由：** 随DSH版本升级自动更新插件的skill
...
- **标签：** `config`, `backup`, `migration`, `sync`
- **推荐理由：** 支持WebDAV同步的完整配置备份还原工具
...
- **推荐理由：** 快速优雅的Neovim插件管理器（Lua编写），适合Neovim用户
...
- **推荐理由：** 实时追踪Token用量和API成本，数据可视化清晰
...
- **标签：** `finance`, `stock`, `quant`, `curated-list`
- **推荐理由：** 精选股票/金融/量化插件合集，双语支持
...
- **⭐ 星标：** 6
- **语言：** TypeScript
- **描述：** 致力于解决各插件之间也与版本之间��能存在的冲突问题并提供解决方案
- **标签：** `troubleshooting`, `compatibility`, `utilities`
- **推荐理由：** 诊断并解决插件间的版本兼容冲突问题
...
- **标签：** `awesome-list`, `directory`, `comprehensive`
- **推荐理由：** 千余仓库全量清单，按星标排序一目了然
...
- **标签：** `awesome-list`, `guide`, `navigation`
- **推荐理由：** 插件产品说明书，按场景导航选型
...
- **标签：** `awesome-list`, `automated`, `directory`
- **推荐理由：** GitHub Actions每日自动爬取，双语插件目录
...
- **标签：** `portal`, `static-site`, `web`, `directory`
- **推荐理由：** 静态Web门户，支持发现、筛选和安装插件
...
- **推荐理由：** 适用于DeepSeek Harness的鲸鱼娘系列皮肤
...
- **标签：** `theme`, `glass-morphism`, `desktop`, `customization`
- **推荐理由：** 高自由度磨砂玻璃主题，支持模糊度、背景壁纸等自定义调节
...
- **标签：** `webui`, `background`, `customization`, `themes`
- **推荐理由：** 支持自定义背景和预设氛围，所见即所得
...
- **⭐ 星标：** 1,756
- **语言：** Python
- **描述：** last30days-cn 是一个 AI Agent 技能（Skill），能够自动搜索中国互联网 8 大主流平台最近 30 天的内容，综合分析���生成有据可查的研究报告。
- **标签：** `research`, `web-search`, `china`
- **推荐理由：** 自动搜索中文8大平台近30天内容，生成有据可查的研究报告
...
- **标签：** `security`, `redteam`, `auditing`, `vulnerability`
- **推荐理由：** AI安全攻防平台，审计幻觉包和代码安全隐患
...
- **标签：** `awesome-list`, `guide`, `reference`
- **推荐理由：** DSH终极指南，含快速入门、资源与精选插件
...
- **标签：** `learning`, `marketplace`, `reference`
...
- **标签：** `awesome-list`, `catalog`, `reference`
- **推荐理由：** 精选DSH插件、技能、MCP和服务编排列表
...
- **标签：** `catalog`, `directory`, `plugin-store`
- **推荐理由：** 插���目录含11000+条目，支持搜索、排名和API
...
- **推荐理由：** 跨库学术检索，支持引用核验和图谱生成，科研人员必备。
...
- **推荐理由：** 本地财经数据库+多Agent分析，支持盯盘推送，实用性强。
...
- **标签：** `research`, `marketing`, `workflows`
- **推荐理由：** 88个开源研究营销技能，覆盖深度调研和商业工作流。
...
- **标签：** `research`, `industry`, `analysis`, `report`
- **推荐理由：** 行业研究领域包，含政策解读与新闻追踪技能
...
- **推荐理由：** 社区整理的DSH插件目录，适合探索生态系统中的可用插件
...
- **推荐理由：** 精选插件列表带网站展示，是探索DSH生态的良好参考
...
- **标签：** `awesome-list`, `ecosystem`, `mcp`
- **推荐理由：** 全面的DSH awesome列表，涵盖插件/工具/技能/MCP，生态概览丰富
...
- **推荐理由：** 公共GitHub插件目录，含安装命令，便于开发者快速查找插件
...
- **标签：** `plugin-registry`, `curated-list`, `skills`
- **推荐理由：** 经过垃圾过滤的官方风格插件注册表，数据公开可靠，适合查插件
...
- **标签：** `awesome-list`, `mcp`, `ai-agents`
- **推荐理由：** 190+ plugins, 12 categories, hourly AI updates
...
- **标签：** `armor-breaking`, `deepseek`, `specialized`
- **推荐理由：** DeepSeek专属破甲插件，稳定化破甲提升生成能力
...
- **标签：** `awesome-list`, `curated`, `directory`
- **推荐理由：** 来自dsh-external/hub的全面精选插件工具目录
...
- **推荐理由：** DeepSeek Harness插件生态的全面精选awesome列表
...
- **标签：** `encyclopedia`, `knowledge-base`, `resources`, `learning`
- **推荐理由：** 一站式AI百科，覆盖技能MCP知识库多模态和学习路线
...
- **标签：** `directory`, `discovery`, `curated`, `weekly`
- **推荐理由：** 每日自动精选插件发现，场景化分类和使用指南
...
- **推荐理由：** 多源搜番，支持Bangumi评分和磁力链接，ACG爱好者福音。
...
- **标签：** `awesome-list`, `plugin-catalog`, `curated`
- **推荐理由：** 官方自动维护的插件精选目录
...
- **标签：** `marketplace`, `plugin-market`, `web-ui`
- **推荐理由：** Web GUI一键浏览安装卸载社区插件
...
- **标签：** `marketplace`, `catalog`, `community`
- **推荐理由：** 社区维护、每4小时自动同步的插件聚合目录
...
- **标签：** `directory`, `awesome-list`, `community`
- **推荐理由：** 收录280+插件的精选目录，覆盖14类
...
- **标签：** `agent-skills`, `ai-hedge-fund`, `backtesting`, `deepseek`, `deepseek-harness`, `dsh`, `fintech`, `investment-research`, `llm-agents`, `mcp`
- **推荐理由：** Alpha Desk — a deepseek-harness (dsh) skill pack that turns an agent session int
...
- **推荐理由：** DSH Skills Manager — 在 DeepSeek Harness 中统一加载并安全管理本机 Agent Skills · Load and saf
...
- **推荐理由：** Bottom Info Bar — an information bar plugin for DeepSeek Harness: provider/model
...
- **推荐理由：** 让 AI 像资深测试工程师一样工作：面向 AI Agent 的测试工程 Skill 框架——10 Skills + 共享知识库 + 类型决策矩阵（Claude 
...
- **推荐理由：** 面向 DeepSeek Harness Web GUI 的正式插件包：在设置页的「Web UI 插件」分组中新增一张「技能与 MCP��卡片，用于在浏览器里管理
...
- **标签：** `awesome-list`, `dsh-plugin`, `dsh-ecosystem`
- **推荐理由：** DSH插件精选目录索引，非功能插件
...
- **推荐理由：** Evidence-led bilingual plugin directory, reviewed
...
- **推荐理由：** Plugin radar + 2Origin lab, evidence-backed
...
- **推荐理由：** 80+ verified DSH plugins organized by functionality
...
- **推荐理由：** Moved to new org, legacy list reference only
...
- **标签：** `awesome-list`, `dsh-plugin`
- **推荐理由：** YAML-driven auto-updating bilingual plugin list
...
- **描述：** DSH插件
- **标签：** `dsh-plugin`
- **推荐理由：** Small DSH plugin collection, limited scope
...
- **标签：** `awesome-list`, `dsh-plugin`
- **推荐理由：** Evidence-aware shortlist of useful DSH plugins
...
- **标签：** `plugin-directory`, `registry`, `deepseek-harness`, `curated-list`
- **推荐理由：** DeepSeek Harness插件目录，便于发现和安装社区插件
