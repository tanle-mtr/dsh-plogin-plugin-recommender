...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 228,682 | Core DSH ecosystem. Everything is a plugin. |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 96,878 | Open-source Claude Design alternative, local-first |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 72,764 | Multi-agent swarms and autonomous workflows. |
...
- **标签：** `ai-agent`, `multi-agent`, `skills`, `memory`
...
- **标签：** `agent-harness`, `dsh-plugin`, `no-code`, `mcp`
- **推荐理由：** 跨设备统一Agent工作区，看板任务跟踪
...
- **标签：** `agent-collaboration`, `dsh-plugin`, `multi-agent-workspace`
- **推荐理由：** 企业级多引擎工作台，支持Codex/DSH/Opencode
...
- **标签：** `agent-os`, `dsh-plugin`, `mcp`, `loop-engineering`
- **推荐理由：** 自我进化Agent OS，评估门控的演进循环
...
- **标签：** `multi-agent`, `cli`, `tool-switcher`
- **推荐理由：** 一键安装切换8+编码Agent平台的统一入口
...
- **标签：** `agent-runtime`, `ai-coding`, `ai-ide`, `agentic`
- **推荐理由：** 高性能Rust智能体运行时，搭配精美桌面IDE体验。
...
- **推荐理由：** 免费无注册的网页搜索桥接工具，为无原生联网的模型提供搜索能力。
...
- **标签：** `coding-agent`, `mcp`, `rpc`, `go`, `tui`
- **推荐理由：** Go语言编写的编码代理，支持RPC子代理和完整MCP协议。
...
- **标签：** `agent-tools`, `cordis`, `pre-flight`
- **推荐理由：** 插件库起飞前检查单，自动预检并注入适用插件。
...
- **标签：** `desktop`, `cordis-plugin`, `dsh-plugin`
...
- **标签：** `ai-studio`, `browser-extension`, `chatgpt`, `claude-ai`, `dsh-plugin`
- **推荐理由：** 多平台AI聊天增强套件，支持提示词管理
...
- **标签：** `widget`, `floating`, `balance-tracker`
- **推荐理由：** 漂浮鲸鱼小部件，实时监控DSH账户余额
...
- **推荐理由：** 轻量5MB Tauri桌面应用，预置插件零配置开箱即用。
...
- **推荐理由：** Windows桌面GUI客户端，内置SSH、插件和技能系统，开箱即用。
...
- **推荐理由：** 本地优先的桌面工作空间，支持DSH会话、项目和Office文档。
...
- **标签：** `genui`, `ui-components`, `charts`, `dsh-ui`
- **推荐理由：** 交互UI组件库，支持图表、表单等在回复中内联渲染。
...
- **标签：** `canvas`, `non-linear`, `workspace`, `session-explorer`
- **推荐理由：** 可视化非线性会话画布，支持分支式对话工作区探索。
...
- **标签：** `desktop-app`, `local-first`, `multi-model`, `tauri`, `privacy`
- **推荐理由：** 本地优先的多模型桌面Agent应用，隐私优先，支持自进化技能。
...
- **标签：** `office`, `desktop`, `ui-extension`
- **推荐理由：** 将电子表格、文档、幻灯片引入DSH办公环境。
...
- **标签：** `mobile`, `ios`, `android`
- **推荐理由：** 原生iOS/Android客户端，支持WebSocket实时Agent跟踪。
...
- **推荐理由：** 跨平台桌面客户端，类CodePilot体验的DSH界面。
...
- **推荐理由：** 开源锤子便签，支持Docker部署、Skill调用与导出。
...
- **标签：** `marketplace`, `ui`, `plugin`
- **推荐理由：** DSH网页端一键安装社区插件市场
...
- **标签：** `agent-platform`, `skills`, `workbench`
- **推荐理由：** 插件化Web工作平台
...
- **标签：** `bioinformatics`, `desktop-app`, `single-cell`, `tauri`
- **推荐理由：** 临床医生的生物信息学桌面应用
...
- **标签：** `info-bar`, `usage-tracking`, `pricing`
- **推荐理由：** 实时显示余额和计费的底部信息栏
...
- **标签：** `skill-picker`, `ui`, `workbuddy`
- **推荐理由：** Composer中的快速技能选择器
...
- **推荐理由：** 集成DB、SSH、Docker和AI的一体化DevOps桌面工具。
...
- **标签：** `dsh-plugin`, `webui`, `compact`
- **推荐理由：** 一键收起上下文面板，提升DSH Web UI使用体验。
...
- **标签：** `agent-memory`, `dsh-plugin`, `memory`, `rag`
...
- **标签：** `agent-memory`, `dsh-plugin`, `memory`, `token-savings`
...
- **标签：** `agent-memory`, `dsh-plugin`, `context-engineering`
- **推荐理由：** 支持有状态Agent对话的持久化记忆库
...
- **推荐理由：** 基于Milvus的统一持久记忆层，适用于所有AI智能体。
...
- **标签：** `memory`, `long-term-memory`, `agent`, `mcp`
- **推荐理由：** 个人AI记忆中枢，为所有智能体提供共享持久上下文。
...
- **标签：** `agent-skills`, `dsh-plugin`, `skills`
- **推荐理由：** 全面技能商店，自动同步GitHub项目
...
- **标签：** `agent-control-plane`, `dsh-plugin`, `workflow-automation`
...
- **标签：** `skills`, `workflow`, `agent-orchestration`
...
- **标签：** `cognition`, `inference-control`, `reasoning`
...
- **标签：** `video-generation`, `agent-skill`, `ffmpeg`, `tts`
- **推荐理由：** 自动生成Vox风格拼贴解说视频的AI智能体技能。
...
- **标签：** `creative-writing`, `novel`, `rag`, `long-form`
- **推荐理由：** 完整小说创作工作流，含世界观构建和编辑支持。
...
- **推荐理由：** 将SKILL.md文件转化为可检索的代理技能语料库，支持评估工具。
...
- **标签：** `agent-skills`, `math-modeling`, `competition`
- **推荐理由：** 全流程数学建模竞赛Skill，支持GitOps流水线与代码自证。
...
- **标签：** `agent-review`, `self-evolving`, `workflow`
- **推荐理由：** 可组合任务控制，实现人类审核的Agent自演进工作流。
...
- **推荐理由：** 为Claude Code添加多模态视觉，支持截图与UI分析。
...
- **推荐理由：** 87个逆向工程与安全渗透Skill文件，适合安全研究。
...
- **标签：** `skills`, `ai-infra`
- **推荐理由：** 专为AI基础设施开发的Claude Code技能集合
...
- **标签：** `skill-generator`, `agent-skills`
- **推荐理由：** 将成功会话自动转换为可复用技能
...
- **推荐理由：** Superpowers技能适配Reasonix编码平台
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `agent-skill`
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`, `web-search`
- **推荐理由：** You.com搜索与集成发现技能
...
- **标签：** `dsh-plugin`, `skills`, `wayfinder`
- **推荐理由：** 25个预置工程技能，右侧面板即用
...
- **标签：** `patent-analysis`, `multi-agent`, `skill`
- **推荐理由：** 专利侵权分析，生成竞品报告
...
- **⭐ 星标：** 55
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版��版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `deepseek-harness`, `gongwen`, `docx`
- **推荐理由：** 国标公文全流程处理技能
...
- **标签：** `multi-agent`, `persistent`, `session-memory`
- **推荐理由：** 持久化多智能体团队记忆系统
...
- **标签：** `agent-skill`, `plugin-dev`, `skill`
...
- **标签：** `diagram-as-code`, `mermaid`, `sequence-diagram`
- **推荐理由：** 代码驱动架构图与工作流可视化。
...
- **标签：** `image-prompts`, `workflow-automation`, `skills`
- **推荐理由：** 530+图像提示词模板库，含可复用技能。
...
- **标签：** `dsh-plugin`, `ui`, `vibedesign`, `flutter`
...
- **推荐理由：** 为纯文本模型打造强大视觉工具箱，支持OCR和UI还原。
...
- **推荐理由：** 跨平台AI内容发现Agent，覆盖B站/YouTube/Reddit
...
- **推荐理由：** 持久身份浏览器，支持指纹伪装、无限本地配置和安卓模拟器。
...
- **推荐理由：** OAuth绑定ChatGPT账户，在DSH内使用ChatGPT模型。
...
- **标签：** `resume-builder`, `typescript`, `self-hosted`
- **推荐理由：** 隐私友好的免费开源简历生成器。
...
- **标签：** `image`, `electron`, `cloudflare-r2`, `s3`
...
- **标签：** `cordis`, `dsh-plugin`, `dsh-routing-suite`
- **推荐理由：** 任务感知路由注入器，智能切换推理模式
...
- **标签：** `context-management`, `dashboard`, `dsh`
- **推荐理由：** 上下文管理面板，支持侧边栏和浏览器集成。
...
- **标签：** `awesome-list`, `plugin-registry`, `ecosystem`
- **推荐理由：** DSH社区精选插件注册表和生态资源列表。
...
- **标签：** `plugin-marketplace`, `ecosystem`, `dsh`
- **推荐理由：** 每日更新的全面插件市场，含审核机制。
...
- **标签：** `security`, `redteam`, `vulnerability`, `aisecurity`
...
- **标签：** `encyclopedia`, `resource`, `ai-knowledge`, `awesome-list`
- **推荐理由：** 覆盖LLM、Agent、RAG、MLOps的全方位AI资源百科导航库。
...
- **标签：** `llm-provider`, `oauth`, `infrastructure`
- **推荐理由：** 通过OAuth使用ChatGPT/Claude/Grok订阅作为DSH模型提供商。
...
- **标签：** `task-management`, `taskboard`, `native-ui`
- **推荐理由：** 原生SQLite任务板，支持Agent认领审核与Web界面。
...
- **推荐理由：** 为DSH添加Command Code模型接入与计划感知选型。
...
- **推荐理由：** 全功能Agent开发手册，覆盖运行时、插件与沙箱评估。
...
- **推荐理由：** P2P加密多端远程访问DeepSeek Harness，安全低延迟。
...
- **标签：** `backup`, `config-manager`
- **推荐理由：** 完整备份恢复DSH配置与插件同步工具
...
- **标签：** `capability`, `management`
- **推荐理由：** 统一能力管理插件，按需暴露减少上下文占用
...
- **标签：** `bundle`, `terminal`, `keybindings`, `trace`
- **推荐理由：** 含任务通知、LLM追踪和终端的插件集合。
...
- **标签：** `token-usage`, `cost`, `data-visualization`, `api`
- **推荐理由：** 实时Token用量追踪和费用估算面板。
...
- **标签：** `tts`, `xiaomi`, `audio`, `speech`
- **推荐理由：** 接入小米MiMo免费TTS，支持自定义声音。
...
- **标签：** `doctor`, `conflict-resolution`, `utility`
- **推荐理由：** 诊断并解决插件间的版本冲突问题。
...
- **推荐理由：** 类CodeX插件市场，为DSH提供集中化插件分发中心。
...
- **标签：** `theme`, `glassmorphism`, `transparent-ui`, `customization`
- **推荐理由：** 磨砂玻璃质感主题，支持调节模糊度、透明度和背景样式。
...
- **标签：** `skin`, `animated`, `ocean`, `threejs`, `chrome-extension`
- **推荐理由：** 实时海洋动态主题，支持可调海浪、日落效果和玻璃透明度。
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜��不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `deepseek-harness`, `dsh-plugin`, `webui`
- **推荐理由：** 支持自定义背景图和预设氛围主题，个性化DSH Web界面。
...
- **标签：** `ai-guide`, `vibe-coding`, `llm`, `mcp`
- **推荐理由：** AI资源大全，含零基础教程与案例库。
...
- **标签：** `documentation`, `code-analysis`, `context`
- **推荐理由：** 从代码生成精准技术文档，AI-ready知识输出
...
- **标签：** `research`, `web-search`, `china`, `agent-skill`
- **推荐理由：** 自动搜索中文8大平台近30天内容并生成研究报告。
...
- **标签：** `research`, `latex`, `arxiv`, `gpu-cluster`, `paper-writing`
- **推荐理由：** 一站式科研平台：LaTeX论文编写、arXiv管理、实验追踪和GPU远程调度。
...
- **标签：** `finance`, `stock`, `database`
- **推荐理由：** 基于Tushare的财经数据库，支持多Agent分析与IM盯盘。
...
- **标签：** `research`, `industry`
- **推荐理由：** 行业研究与公司调研方法论技能包
...
- **标签：** `awesome-list`, `ai-curated`, `coding-agents`
- **推荐理由：** 190+插件分12类，AI每小时更新推荐，覆盖全面。
...
- **语言：** Unknown
- **描述：** DSHA精选插件合集
- **标签：** `awesome-list`, `curated`, `dsha-plugins`
- **推荐理由：** DSH精选插件合集，专注于高质量插件发现。
...
- **标签：** `low-code`, `crm`, `erp`, `internal-tools`
- **推荐理由：** AI驱动的无代码/低代码业务系统平台。
...
- **标签：** `agent`, `dsh-plugin`, `harness-engineering`
...
- **推荐理由：** 中英文双语精选DSH插件、工具和扩展目录，持续更新。
...
- **⭐ 星标：** 242
- **语言：** JavaScript
- **描述：** DeepSeek Harness plugin store, marketplace and hub — 11,000+ dsh plugins with search, rankings, install commands and a free public API. DeepSeek Harness 插件市场 / 插件商店：自动收集与格式校验，免费搜��� API。deepseek1024.com
- **标签：** `awesome-list`, `plugin-directory`, `marketplace`
- **推荐理由：** 收录11,000+插件目录，支持搜索、评分与免费API。
...
- **推荐理由：** 1958个仓库的DSH插件总目录，支持中英文搜索与分类。
...
- **推荐理由：** Unlock DeepSeek Harness plugin-native core capabilities, unifying Runtime, Provi
...
- **推荐理由：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harnes
...
- **推荐理由：** Awesome DeepSeek Harness (dsh) — curated awesome list of plugins, tools, skills 
...
- **标签：** `awesome-list`, `catalog`, `github`
- **推荐理由：** 按Star排序的千余个DSH工具完整清单。
...
- **标签：** `dsh-repository-plugin`, `dsh-plugin-verify`
- **推荐理由：** 提供插件验证工具和生态分析功能。
...
- **标签：** `moved`, `awesome-list`
- **推荐理由：** 已迁移至新仓库，建议访问更新地址。
