...
- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 16 个插件
- [UI 与桌面扩展](#ui-与桌面扩展) — 21 个插件
- [记忆与知识（RAG）](#记忆与知识（rag）) — 23 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 48 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (16)
- [UI 与桌面扩展](ui-desktop-extensions-zh) (21)
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (23)
- [Agent 技能与工作流](agent-skills-workflows-zh) (48)
...
- **标签：** `cli`, `coding-agent`, `deepseek`
- **推荐理由：** 终端原生DeepSeek编码Agent，持久稳定。
...
- **标签：** `agent-skills`, `ai-coding`
- **推荐理由：** 让编码AI感知架构，含漂移检测与证据验证
...
- **标签：** `security`, `code-audit`, `redteam`
- **推荐理由：** AI安全攻防平台，审计代码幻觉与漏洞。
...
- **标签：** `agent`, `orchestration`, `coding`
- **推荐理由：** 多编码Agent协作编排，统一调度。
...
- **标签：** `dsh-plugin`, `image-reading`, `ocr`
- **推荐理由：** 让纯文本模型通过像素识别读取图片内容。
...
- **推荐理由：** 官方DSH插件开发知识库，集成agent技能，适合插件开发者。
...
- **推荐理由：** OpenAI Codex与DSH双向桥梁，支持聊天同步与MCP镜像。
...
- **推荐理由：** 上下文注入审计工具，可视化分析指令链与Token成本。
...
- **推荐理由：** DSH插件开发与审计便携技能，含bundle适配器
...
- **推荐理由：** DSH插件开发踩坑指南：cordis/tsconfig/zstd等
...
- **标签：** `agent-skills`, `best-practices`, `engineering`
- **推荐理由：** 从DSH提炼的工程纪律：证据驱动开发实践
...
- **标签：** `dsh-web`, `web-ui`, `plugin-hub`
- **推荐理由：** DSH Web插件聚合生态包，支持创意工坊的浏览器分发。
...
- **推荐理由：** 可扩展侧边栏底座，支持文件编辑/终端/Git等页面。
...
- **推荐理由：** 高性能Rust Agent运行时，搭配精美桌面应用。
...
- **标签：** `desktop`, `agent`, `privacy`, `tauri`
- **推荐理由：** 本地优先的多模型桌面AI代理，隐私友好。
...
- **标签：** `notes`, `dsh-plugin`, `smartisan`
- **推荐理由：** 锤子便签复刻版，支持Docker私有化部署和图片导出。
...
- **推荐理由：** Electron桌面控制台，统一管理Harness和插件。
...
- **标签：** `deepseek-harness`, `dsh-plugins`, `desktop-app`
- **推荐理由：** 多版本隔离容器管理，含嵌入式WebView界面。
...
- **标签：** `capability-menu`, `mcp`, `ui`, `dsh-plugin`
- **推荐理由：** 统一管理MCP工具暴露级别与执行模式的能力菜单。
...
- **推荐理由：** 集成数据库、SSH、Docker与AI的一体化桌面。
...
- **标签：** `agent-memory`, `long-term-memory`, `rag`, `markdown`
- **推荐理由：** 本地优先、Markdown原生的可移植记忆层，支持跨应用自我进化。
...
- **标签：** `long-term-memory`, `hybrid-retrieval`, `token-savings`
- **推荐理由：** 自进化记忆系统，混合检索，节省35% Token且支持技能复用。
...
- **推荐理由：** 基于Markdown和Milvus的持久统一记忆层。
...
- **标签：** `agent-memory`, `long-term-memory`, `mcp`, `dsh-plugin`
- **推荐理由：** 持久化长期记忆插件，支持召回工具和设置页。
...
- **推荐理由：** 插件搜索发现工具，支持在Agent内直接查找和浏览DSH插件。
...
- **推荐理由：** DSH插件全量清单，1000+仓库按star排序的 Awesome 列表。
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin`, `market`
- **推荐理由：** 类似CodeX的DSH插件市场，支持插件浏览与发现。
...
- **标签：** `ai-agents`, `awesome-list`, `coding-agents`, `deepseek-harness`
- **推荐理由：** AI持续维护的190+插件推荐清单，每小时自动更新。
...
- **推荐理由：** GitHub Actions每日自动爬取的插件中英目录。
...
- **标签：** `ai-agents`, `awesome`, `dsh-plugins`, `plugins`
- **推荐理由：** 70+经过验证的DSH插件，按功能分类整理。
...
- **标签：** `agent-harness`, `coding-agents`, `deepseek-harness-plugin`, `registry`
- **推荐理由：** 覆盖模型、工具、沙箱、UI和Agent循环的精选插件目录。
...
- **标签：** `industry`, `dsh-plugin`, `awesome-list`
- **推荐理由：** 覆盖计算、金融、法律等行业垂直精选DSH插件目录。
...
- **推荐理由：** 统一管理和追踪所有Agent与工作台，支持跨设备任务看板。
...
- **标签：** `agent-os`, `self-evolving`, `llm-orchestration`
- **推荐理由：** Agent自我进化OS，通过持续评估循环让Agent越来越聪明。
...
- **标签：** `enterprise`, `multi-agent`, `visual-editor`
- **推荐理由：** 企业级多引擎工作区，支持Agent团队协作和可视化编辑。
...
- **推荐理由：** 基于J-space全局工作空间理论的AI认知增强技能集。
...
- **标签：** `agent-skills`, `workflow-automation`, `skill-orchestration`
- **推荐理由：** 智能路由技能，自动编排插件工作流。
...
- **标签：** `search`, `web-fetch`, `skill`
- **推荐理由：** 免费免注册网页搜索桥接，全模型通用。
...
- **标签：** `creative-writing`, `drama-skills`
- **推荐理由：** 小说写作与短剧创作技能插件
...
- **推荐理由：** SQLite任务看板，支持代理认领审核和原生Web UI。
...
- **标签：** `orchestration`, `subagent`, `mcp`, `dsh-plugin`
- **推荐理由：** 从Claude Code/Codex调度DSH代理，支持子代理编排。
...
- **标签：** `skills`, `superpowers`, `dsh-plugin`
- **推荐理由：** TDD、调试、规划和协作技能，适配DSH。
...
- **标签：** `agent-skills`, `legal-tech`, `visualization`
- **推荐理由：** 专门的法律时间轴和案件图表可视化工具。
...
- **推荐理由：** 跨平台Agent会话记忆继承技能，支持任务交接与状态恢复。
...
- **标签：** `chinese-traditional-wisdom`, `agent-skill`, `fengshui`
...
- **标签：** `dsh-plugin`, `agent-skills`, `skill-picker`
- **推荐理由：** WorkBuddy风格技能选择器，便捷插入官方技能命令。
...
- **标签：** `agent-team`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 多Agent团队协作插件，支持独立模型、技能与共享工作区。
...
- **推荐理由：** 可离线验证的Swift Cycle治理技能，版本锁定安全可靠。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skill`
- **推荐理由：** OAuth零权限登录合影墙，含DSH Skill包装
...
- **推荐理由：** 42个技能5个CLI的AI Agent资产包管理器
...
- **⭐ 星标：** 14
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增�� 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `ai-docs`
- **推荐理由：** 符合国标GB/T 9704的中文公文全流程处理
...
- **推荐理由：** DirectorX集成AI视频/图像/音频生成技能
...
- **⭐ 星标：** 13
- **语言：** Python
- **描述：** 中文电影与剧集长剧本写作 skill
- **标签：** `screenwriting`, `ai-writing`, `chinese`
- **推荐理由：** 中文电影与电视剧长剧本写作技能
...
- **⭐ 星标：** 11
- **语言：** JavaScript
- **描述：** DSH execution-discipline plugin suite: 11 plugins + 6 skills, zero dependencies, 426 tests. 让 DeepSeek Harness 变聪明的插件套件。
- **标签：** `agent-skills`, `execution`, `discipline`
- **推荐理由：** 11个插件+6个技能提升DSH执行纪律。
...
- **标签：** `agent`, `workflow`, `product-team`
- **推荐理由：** 虚拟产品团队Agent预设，模拟完整工作流
...
- **推荐理由：** 为纯文本Agent提供视觉能力，图片转结构化JSON证据。
...
- **标签：** `agent-skills`, `ui-generator`, `figma-alternative`
- **推荐理由：** 开源设计工具，AI Agent的可视化设计首选。
...
- **标签：** `architecture-diagram`, `agent-skills`, `mermaid-alternative`
- **推荐理由：** 生成精美可验证的架构图和流程图，适合系统设计与可视化。
...
- **标签：** `vector-design`, `agent-team`, `design-as-code`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持Agent团队协作，提示词直接生成UI。
...
- **标签：** `video-generation`, `ffmpeg`, `explainer-video`
- **推荐理由：** 一键生成Vox风格纸���动画解说视频技能。
...
- **推荐理由：** 开源跨平台内容发现Agent，支持B站、抖音、小红书等。
...
- **推荐理由：** Tabbit浏览器插件，基于Playwright的网页自动化。
...
- **标签：** `ai-agent`, `dingtalk`, `chat-bot`, `deepseek-harness`
- **推荐理由：** 钉钉IM通道，让聊天对话具备Agent工具调用能力。
...
- **标签：** `no-code`, `low-code`, `ai-agent`
- **推荐理由：** AI驱动的低代码业务系统快速构建平台。
...
- **标签：** `routing`, `injector`, `infrastructure`
- **推荐理由：** 提供运行时注入器和任务感知路由，优化Agent推理分发。
...
- **推荐理由：** 两阶段预设，先极简引导后完整标准工具配置，开发效率高。
...
- **标签：** `virtual-filesystem`, `vfs`, `abstraction`
- **推荐理由：** 统一的虚拟文件系统抽象，跨主流AI Agent框架通用。
...
- **标签：** `claude-code`, `deepseek-harness`, `tool-switch`
- **推荐理由：** 一键切换多种AI编程CLI工具的实用插件。
...
- **标签：** `marketplace`, `plugin`, `dsh`
- **推荐理由：** DSH内置可视化插件市场，一键安装插件。
...
- **标签：** `compatibility-layer`, `migration`
- **推荐理由：** 打通Pi与DSH生态兼容层
...
- **标签：** `marketplace`, `plugin-marketplace`, `dsh-plugin`
...
- **标签：** `context-migration`, `session-migration`, `dsh-plugin`
...
- **标签：** `backup`, `crash-recovery`, `undo`, `dsh-plugin`
...
- **标签：** `plugin-market`, `ui`, `web`, `dsh-plugin`
...
- **⭐ 星标：** 57
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官��插件开发引导
- **标签：** `console`, `plugin-management`, `ui`
- **推荐理由：** 必备的插件管理控制台，含浏览器面板和开发引导。
...
- **标签：** `config-manager`, `backup`, `mcp`, `dsh-plugin`
- **推荐理由：** DSH配置完整备份恢复工具，支持WebDAV同步与MCP迁移。
...
- **标签：** `token-tracking`, `cost`, `visualization`
- **推荐理由：** DSH实时token用量图表与成本估算。
...
- **标签：** `diagnostic`, `utility`, `conflict`
- **推荐理由：** 诊断并解决D SH插件冲突问题
...
- **标签：** `mascot`, `pixel-art`, `animation`
- **推荐理由：** 像素风动画宠物画廊，为你的编码Agent增添个性化陪伴。
...
- **标签：** `chrome-extension`, `theme`, `threejs`
- **推荐理由：** WebGPU海洋主题视觉皮肤
...
- **标签：** `research`, `web-search`, `report-generation`
- **推荐理由：** 自动搜索8大中文平台近30天内容并生成研究报告。
...
- **标签：** `awesome-list`, `curation`, `discovery`
- **推荐理由：** 每日自动精选插件，场景化分类排行。
...
- **标签：** `encyclopedia`, `learning`, `resource`
- **推荐理由：** AI资源百科，覆盖Agent、RAG、MLOps��学习路线。
...
- **标签：** `education`, `reference`, `marketplace`
...
- **⭐ 星标：** 55
- **语言：** TypeScript
- **描述：** 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接���索 20 个中国开放平台的 65,600+ 篇 API 文档；零配置，支持 Skill 与 DSH 原生插件。
- **标签：** `ai-agents`, `api-documentation`, `chinese-api`
- **推荐理由：** 在编码代理内直接搜索6.5万篇中国平台API文档。
...
- **推荐理由：** 交互式K线分析插件，支持多市场行情、技术指标与基本面分析。
...
- **标签：** `evaluation-framework`, `rag-evaluation`, `prompt-testing`
- **推荐理由：** 证据驱动的提示词、RAG和技能评估框架
...
- **标签：** `radar`, `research`, `evidence`
- **推荐理由：** 数据驱动的D SH插件雷达与评测实验室
...
- **推荐理由：** 全面AI资源大全与Vibe Coding零基础教程。
...
- **标签：** `toy`, `web-simulation`, `mobile-friendly`
- **推荐理由：** 零依赖单文件模拟中国传统竹蜻蜓玩具。
...
- **标签：** `plugin-marketplace`, `dsh-plugins`
- **推荐理由：** DSH插件市场目录平台
...
- **标签：** `awesome-list`, `plugins`, `deepseek-harness`
- **推荐理由：** 精选280+插件目录，覆盖14大类，是探索DSH生态的最佳起点。
...
- **标签：** `portal`, `web`, `discovery`
- **推荐理由：** 静态网页门户，方便发现和安装插件
...
- **标签：** `registry`, `archived`
- **推荐理由：** 已迁移，请访问新的awesome-dsh-plugin仓库
