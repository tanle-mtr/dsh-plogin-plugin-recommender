...
- [UI 与桌面扩展](ui-desktop-extensions-zh) (24)
...
- **标签：** `routing`, `injector`, `workflow`
- **推荐理由：** 运行时注入器和任务感知路由套件。
...
- **标签：** `agent-os`, `self-evolving`, `evaluation`
- **推荐理由：** 自进化Agent OS，带面试门禁和预算控制。
...
- **标签：** `terminal`, `sandbox`, `virtual-filesystem`
- **推荐理由：** 为AI代理提供沙盒虚拟终端执行环境
...
- **标签：** `installer`, `model-switch`, `multi-agent`
- **推荐理由：** 一键安装并切换Claude Code、Codex、DeepSeek等多种编码代理
...
- **标签：** `documentation`, `generation`, `code-analysis`
- **推荐理由：** 从代码生成精准技术文档和AI就绪上下文
...
- **标签：** `agent-teams`, `collaboration`, `orchestration`
- **推荐理由：** 支持多Agent协作编排，提升DSH团队工作流效率
...
- **推荐理由：** 为纯文本模型提供强大视觉能力，OCR+UI还原一体化
...
- **标签：** `coding-agent`, `rpc`, `mcp`, `terminal`
- **推荐理由：** 支持RPC子Agent、哈希行编辑和MCP的编码智能体。
...
- **标签：** `ios`, `mobile-automation`, `coding-agent`
- **推荐理由：** 在DSH对话中集成iOS模拟器与iPhone自动化能力
...
- **标签：** `coding`, `superpowers`, `skills`
- **推荐理由：** TDD/调试/规划技能，适配DSH编码场景
...
- **推荐理由：** Lua编写的Neovim插件管理器，快速简洁，适合高效开发环境配置。
...
- **标签：** `web-ui`, `plugin-aggregation`, `ecosystem`
- **推荐理由：** 基于Web的DSH插件聚合生态系统。
...
- **推荐理由：** 开放式侧边栏底座，支持文件编辑、终端、对话等多页面扩展
...
- **标签：** `task-panel`, `ui`, `embedded`
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex和DSH界面
...
- **标签：** `tui`, `interface`, `theme`
- **推荐理由：** 鲸鱼主题TUI界面，支持流式思考与双击Esc回滚
...
- **推荐理由：** 可爱浮动鲸鱼小部件，实时显示DeepSeek账户余额
...
- **标签：** `agent-runtime`, `desktop`, `ai-coding`
- **推荐理由：** Rust高性能运行时，桌面体验优秀，适合AI编码工作流
...
- **推荐理由：** 仅5MB安装包，零环境配置，开箱即用的跨平台桌面版
...
- **推荐理由：** 本地优先的Electron桌面工作空间，集成会话、插件和办公功能。
...
- **标签：** `desktop-app`, `windows`, `gui`, `ssh-client`
- **推荐理由：** 零配置Windows桌面GUI，支持Codex、插件、SSH和移动端远程访问。
...
- **标签：** `desktop-app`, `multi-model`, `privacy`, `local-first`
- **推荐理由：** 本地优先多模型桌面应用，支持自进化技能与隐私保护。
...
- **标签：** `office`, `spreadsheet`, `desktop-extension`
- **推荐理由：** 将完整办公套件（表格、文档、幻灯片）带入DSH
...
- **标签：** `mobile`, `ios`, `native-client`
- **推荐理由：** 原生iOS客户端，同步工作区、会话与Agent执行轨迹
...
- **标签：** `desktop`, `electron`, `ui-extension`
- **推荐理由：** 跨平台Electron桌面客户端，灵感来自CodePilot
...
- **标签：** `notes`, `ui`, `docker`
- **推荐理由：** 锤子便签开源版，Docker部署，支持skill调用和公众号导出
...
- **推荐理由：** 一站式DevOps桌面应用，集成SSH、Docker、数据库和AI助手。
...
- **推荐理由：** 基于Electron的桌面客户端，集成多个DSH工具和扩展。
...
- **标签：** `ui`, `button`, `webui`, `dsh-plugin`
- **推荐理由：** DSH Web上下文计面板一键紧凑按钮，改善界面交互体验。
...
- **标签：** `agent-memory`, `local-first`, `markdown`
- **推荐理由：** 本地优先的Markdown原生Agent记忆层。
...
- **标签：** `memory-os`, `hybrid-retrieval`, `token-optimization`
- **推荐理由：** 自演化记忆系统，混合检索节省大量Token。
...
- **标签：** `memory-library`, `stateful-agents`, `continual-learning`
...
- **标签：** `agent-memory`, `skill-corpus`, `embeddings`, `vector-search`
- **推荐理由：** 将零散技能文件转化为可检索的Agent知识库，带评估工具。
...
- **标签：** `knowledge-base`, `ai-pedia`, `rag`, `multi-model`
- **推荐理由：** 涵盖大模型、Agent、RAG、多模态的全方位AI知识库。
...
- **标签：** `agent-workspace`, `task-tracking`, `multi-device`
- **推荐理由：** 跨设备统一Agent工作区，支持任务看板。
...
- **标签：** `skills-store`, `marketplace`, `agent-skills`
...
- **标签：** `long-horizon`, `control-plane`, `workflow-automation`
- **推荐理由：** 长周期Agent控制平面，支持跨Harness工作流。
...
- **标签：** `cognition`, `skills`, `research-based`
...
- **标签：** `video`, `agent-skill`, `automation`
- **推荐理由：** 一键生成Vox风格解说视频，端到端自动化流程
...
- **⭐ 星标：** 840
- **语言：** TypeScript
- **描述：** AI 小说创作软件：把灵感、角色、世界观、大纲、章节写作、审稿和修稿组织成可控流程；提供 Windows/macOS 桌面版、Ollama 与 DSH 插件预览。AI Novel Writing Software: Organizes inspirations, characters, worldbuilding, outlines, chapter drafting, review, and revision into a controllable workflow. Features desktop apps for Windows/macOS, Ollama integration, and a DeepSeek Harness (DSH) plugin preview.
- **标签：** `writing`, `novel`, `creative`
- **推荐理由：** 完整AI小说创作流程，含世界观构建与审稿修稿功能
...
- **标签：** `canvas`, `branching`, `workspace`, `conversation`
- **推荐理由：** 可视化非线性对话画布，支持分支会话和工作空间探索。
...
- **标签：** `computer-vision`, `multimodal`, `ocr`, `agent-skill`
- **推荐理由：** 赋予Agent视觉听觉，视频音频屏幕转为可检索证据。
...
- **⭐ 星标：** 343
- **语言：** Python
- **描述：** DeepSeek 写网文/小说的工作流插件：DeepSeek Harness 社区插件，内置小说、短剧、游戏、视频解说四个工作台 | Community DeepSeek Harness plugin with novel, short-drama, game and video-recap workbenches. Not affiliated with DeepSeek.
- **标签：** `novel-writing`, `creative-writing`, `agent-skills`
- **推荐理由：** 小说、短剧、游戏、视频解说专属创作工作流
...
- **标签：** `task-management`, `agent-workflow`, `sqlite`
- **推荐理由：** SQLite驱动的原生任务看板，支持Agent认领与审核流程
...
- **标签：** `plan-mode`, `agent-review`, `self-evolving`
- **推荐理由：** 可组合审查模式，支持Agent自我进化
...
- **⭐ 星标：** 170
- **语言：** Python
- **描述：** 为 Claude Code 赋能多模态视觉能力，适配 纯文本 LLM 底座，用于截图 / UI / 图表分析；搭配 browser-harness 可做前端布局自动化��查。
...
- **标签：** `skills`, `research`, `marketing-automation`
- **推荐理由：** 88个开源技能，覆盖研究、营销和商业工作流
...
- **标签：** `ai-infra`, `skills`, `development`
- **推荐理由：** AI基础设施开发专用技能，支持高性能系统编写与调试
...
- **标签：** `skill-generation`, `agent-learning`
...
- **标签：** `fintech`, `backtesting`, `mcp`
- **推荐理由：** 合规优先的AI投资桌，支持多策略回测
...
- **标签：** `bazi`, `fengshui`, `wisdom`
- **推荐理由：** 融合八字/风水/紫微等的本地确定性引擎技能包
...
- **标签：** `roleplay`, `narrative`, `interactive`
- **推荐理由：** NPC有记忆立场的互动叙事角色扮演技能
...
- **标签：** `math-modeling`, `multi-agent`, `skill`
- **推荐理由：** 5角色多Agent数学建模，含33模型库和论文模板。
...
- **标签：** `skillhub`, `api`, `open-api`
- **推荐理由：** 官方SkillHub Open API文档及DSH插件集成。
...
- **⭐ 星标：** 24
- **语言：** TypeScript
- **描述：** dsh-plugin-developer — DeepSeek Harness 插件开发 Skill  > 指导 AI Agent �� 0 到 1 开发、构建、安装、测试 DeepSeek Harness（dsh）插件。基于 **dsh 0.1.1-rc.2** 与 dsh-openmaic 项目的完整实战经验，并内置一个**已通过 web 界面实测**的天气插件作为整包参考案例。  ## 这是什么  dsh 采用"无特权内核、万物皆插件"的设计：模型适配器、工具注册表、Agent 循环、网页界面都是插件
- **标签：** `plugin-dev`, `skill`, `tutorial`
...
- **推荐理由：** 电影感视频Agent，含152镜头配方和Remotion模板。
...
- **标签：** `plugin-dev`, `agent-skill`, `standards`
...
- **⭐ 星标：** 474
- **语言：** TypeScript
- **描述：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without native web access. Free, no signup, no API key. Ask the web or X, get structured JSON evidence. | 🥇 全网最强的 DeepSeek Harness 免费联网搜索插件，免费免注册免 API key。为不能联网的��型补上搜索，问网页或 X，拿回结构化 JSON 证据（搜索、抓取、引用）。
- **标签：** `web-search`, `free`, `mcp`, `harness-tool`
- **推荐理由：** 最强免费网页搜索桥接插件，无API、无需注册。
...
- **标签：** `provider`, `llm`, `command-code`
- **推荐理由：** Command Code模型接入，支持智能选模与推理优化
...
- **推荐理由：** 开源AI设计代理，Claude Design替代方案
...
- **标签：** `design-tool`, `vector`, `agentic-design`
...
- **推荐理由：** 首个视觉插件，为纯文本编码代理提供OCR图像识别能力
...
- **推荐理由：** 内置AI设计系统，支持可视化编辑、模板市场与PPT
...
- **标签：** `genui`, `ui-components`, `charts`, `interactive`
- **推荐理由：** 在回复中生成图表、表单、测验等交互式内联UI组件。
...
- **标签：** `kline`, `visualization`, `financial-analysis`
- **推荐理由：** 交互式K线分析，支持侧栏可视化多市场行情。
...
- **推荐理由：** 实时Token用量图表和费用估算，帮助监控API成本支出。
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任���服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `visual`, `theme`, `background`, `webui`
- **推荐理由：** 自定义DSH Web背景图片及氛围预设，提升视觉体验。
...
- **标签：** `knowledge-cards`, `awesome-dsh-plugin`
- **推荐理由：** 将聊天记录转为精美知识卡片分享
...
- **标签：** `marketplace`, `installation`, `discovery`
- **推荐理由：** 内置插件市场，支持浏览和一键安装DSH插件
...
- **标签：** `plugin`, `ecosystem`, `infrastructure`
- **推荐理由：** 自部署插件生态雷达，K8s级实测管线自动索引
...
- **标签：** `security`, `cybersecurity`, `redteam`, `auditing`
- **推荐理由：** AI安全攻防平台，审计幻觉包、漏洞和安全漏洞。
...
- **标签：** `subscriptions`, `oauth`, `llm-provider`
- **推荐理由：** 无需API密钥即可使用ChatGPT/Claude/Grok订阅作为LLM后端
...
- **标签：** `catalog`, `marketplace`, `plugin-directory`
- **推荐理由：** 插件市场，收录11000+插件并提供免费API
...
- **推荐理由：** 打通Pi与DSH生态，Pi扩展可直接作为DSH插件运行
...
- **标签：** `context`, `audit`, `tooling`
- **推荐理由：** 上下文审计，检测重复和冲突指令
...
- **标签：** `container`, `management`, `versioning`
- **推荐理由：** 容器隔离管理多版本 DSH
...
- **标签：** `launcher`, `infrastructure`, `cordis`
- **推荐理由：** 统一编排Runtime、插件、技能、MCP能力。
...
- **标签：** `utility`, `plugin-conflict`, `deepseek-harness`
- **推荐理由：** 诊断插件冲突并提供解决方案，对稳定DSH环境非常重要。
...
- **标签：** `mascot`, `animated`, `cli-theme`
- **推荐理由：** 公开动画宠物画廊，个性化Agent CLI界面。
...
- **推荐理由：** 鲸鱼娘主题皮肤，美化DeepSeek Harness桌面界面
...
- **标签：** `theme`, `glassmorphism`, `customization`, `ui`
- **推荐理由：** 高自由度磨砂玻璃主题，可调模糊度和自定义壁纸。
...
- **标签：** `animated`, `ocean`, `theme`, `threejs`
- **推荐理由：** 实时动态海洋主题，可调海浪、日落和玻璃透明度效果。
...
- **标签：** `ai-guide`, `tutorials`, `resources`
...
- **推荐理由：** 自动搜索中国8大主流平台近30天内容并生成研究报告
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU编排、实验追踪
...
- **标签：** `math-modeling`, `competition`, `agent-skill`
...
- **标签：** `academic-search`, `mcp`, `citation-graph`
- **推荐理由：** 多源学术检索，支持引文图谱和MeSH主题词
...
- **标签：** `finance`, `stock`, `mcp`
- **推荐理由：** 基于Tushare的本地财经库，支持多Agent盯盘推送到IM
...
- **标签：** `dsh-plugin-market`, `awesome-dsh-plugin`
...
- **标签：** `awesome-list`, `plugin`, `community`
- **推荐理由：** DSH插件生态精选清单，社区维护的实用资源库
...
- **推荐理由：** 每日自动抓取GitHub插件并按场景分类的精选题单
...
- **标签：** `awesome-list`, `mcp`, `orchestrator`
- **推荐理由：** 收录插件、技能、MCP服务器、补丁与编排工具的精选列表
...
- **⭐ 星标：** 51
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内��优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `ai-docs`, `deepseek-harness`, `deepseek-harness-plugin`, `docx`, `dsh-plugin`, `gongwen`, `wps-office`
- **推荐理由：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内��优化（Word 
...
- **推荐理由：** DSH插件生态发现的公开目录，适合新手快速了解插件资源。
...
- **推荐理由：** 经过垃圾过滤的开放数据注册表，是DSH插件发现的可靠来源。
...
- **推荐理由：** 经验证的插件目录，提供安全加载指导和权限-aware安装建议。
...
- **标签：** `awesome-list`, `curated-list`, `deepseek-harness`
- **推荐理由：** 2026年精选插件清单，含质量检查，保持发现内容更新。
...
- **推荐理由：** 专注股票/金融/量化领域的垂直插件列表，适合特定场景使用。
...
- **标签：** `tts`, `xiaomi`, `audio`, `plugin`
- **推荐理由：** 接入小米MiMo免费TTS服务，支持自定义和浏览器内置语音。
...
- **标签：** `awesome-list`, `dsh-plugin`, `deepseek-harness`, `bilingual`
- **推荐理由：** 基于证据的双语DSH插件目录，经不可变提交审查
...
- **标签：** `awesome-list`, `dsh-plugin`, `auto-updating`, `bilingual`
- **推荐理由：** YAML驱动的双语自动更新插件目录，README自动生成
