...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 197,377 | Core DSH framework enabling everything as a plugin |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,686 | Local-first design tool turning coding agents into |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,449 | Meta-harness for multi-agent swarms and autonomous |
...
- [记忆与知识（RAG）](#记忆与知识（rag）) — 19 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 40 个插件
...
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (19)
- [Agent 技能与工作流](agent-skills-workflows-zh) (40)
...
- **推荐理由：** DeepSeek Harness核心框架，一切皆插件。
...
- **标签：** `agentic-framework`, `mcp-server`, `dsh-plugin`
- **推荐理由：** 多智能体群集元框架，支持自主工作流编排。
...
- **标签：** `ai-coding`, `dsh`, `dsh-plugin`
- **推荐理由：** 终端AI编程Agent，深度优化DeepSeek缓存。
...
- **标签：** `agent-os`, `self-improving`
- **推荐理由：** 自进化Agent系统，越用越聪明。
...
- **标签：** `code-review`, `lint`, `agent-skill`
- **推荐理由：** 基于经典工程书籍的AI代码审查与架构诊断
...
- **标签：** `security`, `coding-agent`
- **推荐理由：** 审计 AI 生成代码中的幻觉包和安全漏洞
...
- **标签：** `agent-skill`, `deepseek-harness`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 按官方标准开发DSH插件的Agent Skill。
...
- **标签：** `deepseek-harness`, `plugin-development`
- **推荐理由：** 构建配置DSH插件，支持槽位和凭证管理。
...
- **标签：** `capability-management`, `mcp`, `skills`
- **推荐理由：** 统一管理MCP工具和技能的暴露级别。
...
- **标签：** `skills`, `manager`, `installation`
- **推荐理由：** 集中管理DSH技能，支持一键安装更新。
...
- **标签：** `plugin-development`, `deepseek-harness`
- **推荐理由：** 快速构建和验证DSH/Cordis插件的可复用技能。
...
- **标签：** `package-manager`, `agent-framework`, `cli`
...
- **推荐理由：** DSH插件开发踩坑实录，涵盖cordis、tsconfig、Windows junction等
...
- **推荐理由：** DSH插件开发全流程Agent技能，脚手架+测试一体化
...
- **标签：** `agent-tools`, `ai-agents`
- **推荐理由：** Agent任务前自动预检插件库并注入提示
...
- **标签：** `agent-skills`, `ai-design`, `dsh-plugin`
- **推荐理由：** 本地优先的设计插件，让编程Agent变身设计师。
...
- **标签：** `agent-skills`, `dsh-plugin`, `mcp-server`
- **推荐理由：** 隐私优先的简历生成器，完全可定制开源。
...
- **标签：** `dsh`, `dsh-plugin`, `desktop`
- **推荐理由：** DSH插件生态的现代化桌面端客户端。
...
- **标签：** `browser-extension`, `dsh`, `dsh-plugin`
- **推荐理由：** 浏览器增强套件，提升多平台AI使用体验。
...
- **标签：** `ui`, `sidebar`, `extension`
- **推荐理由：** 开放侧边栏，支持三方扩展，内置多面板
...
- **标签：** `desktop`, `tauri`
- **推荐理由：** 仅5MB安装器，零环境配置，预置插件的跨平台桌面版。
...
- **标签：** `desktop-app`, `agent`
- **推荐理由：** 本地优先多模型 AI 代理桌面应用，注重隐私
...
- **标签：** `desktop-app`, `ui`
- **推荐理由：** CodePilot 风格原生桌面客户端，支持插件生态
...
- **推荐理由：** 开源Windows桌面GUI，支持SSH和移动端访问
...
- **标签：** `deepseek-harness`, `desktop-app`, `dsh`, `electron`, `typescript`
- **推荐理由：** Electron桌面控制台，多标签管理Harness与插件。
...
- **推荐理由：** 持久化底部状态栏，显示Provider、余额和计价。
...
- **标签：** `desktop`, `tauri`, `devops`, `all-in-one`
- **推荐理由：** Tauri桌面应用整合数据库、SSH、Docker面板与AI助手
...
- **推荐理由：** Electron桌面封装DSH Web UI，集成侧边栏与ModLens
...
- **标签：** `knowledge`, `resource`
- **推荐理由：** 全维度 AI 知识库百科，涵盖技能与 RAG
...
- **标签：** `memory`, `long-term`, `mcp`, `recall`
- **推荐理由：** 持久化可检查的Agent长期记忆，带召回工具。
...
- **标签：** `knowledge-base`, `documentation`, `cordis`
- **推荐理由：** 官方DSH文档中英文知识库，可按需调用。
...
- **标签：** `context`, `deepseek-harness`, `ui`
- **推荐理由：** 审计上下文token成本，检测重复冲突指令。
...
- **标签：** `awesome`, `verified`, `dsh-plugins`
- **推荐理由：** 70+验证插件按功能组织，易于导航。
...
- **标签：** `curated-list`, `registry`, `skills`
- **推荐理由：** 覆盖模型、沙盒、UI和agent循环的精选插件。
...
- **标签：** `curated-list`, `evidence-aware`
- **推荐理由：** 基于证据的实用DSH插件精简列表。
...
- **标签：** `ai-agent`, `low-code`, `dsh-plugin`
...
- **标签：** `agent-workspaces`, `orchestration`
- **推荐理由：** 跨平台统一Agent工作区与任务看板。
...
- **标签：** `agent-workspace`, `enterprise`
- **推荐理由：** 企业级多Agent工作台，支持多框架。
...
- **标签：** `agent-skills`, `workflow`, `orchestration`
- **推荐理由：** 智能编排Harness工作流中的本地技能路由
...
- **标签：** `armor-breaking`, `deepseek-harness`
- **推荐理由：** 稳定破甲提示词，解锁 DeepSeek 全部能力
...
- **标签：** `anime`, `bangumi`, `search`, `magnet`
- **推荐理由：** 多源搜番，Bangumi评分卡片，支持复制磁力链接。
...
- **标签：** `notes`, `smartisan`, `docker`, `export`
- **推荐理由：** 复刻锤子便签，支持Docker部署、skill调用和图片导出。
...
- **标签：** `task-management`, `sqlite`, `webui`
- **推荐理由：** SQLite任务看板，支持Agent认领审核，原生Web UI。
...
- **推荐理由：** 提供TDD、调试、规划等技能，来自superpowers生态。
...
- **推荐理由：** 将superpowers技能移植到Reasonix编码工具链。
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`, `deep-research`, `marketing-automation`
- **推荐理由：** 88个开源研究、营销与商业工作流技能。
...
- **标签：** `agent-skills`, `ai-coding`, `deepseek-harness`, `spec-driven-development`
- **推荐理由：** 仓库级规格驱动开发技能，适配DSH工程模式。
...
- **标签：** `bazi`, `fengshui`, `iching`, `tcm`, `traditional`
- **推荐理由：** 八字风水易经中医本地确定性引擎技能包。
...
- **标签：** `session-memory`, `handoff`, `continuity`
- **推荐理由：** 保持会话上下文，实现跨会话无缝交接。
...
- **标签：** `skill-picker`, `composer`, `ux`
- **推荐理由：** 为DSHComposer提供WorkBuddy风格技能选择器。
...
- **标签：** `governance`, `swift-cycle`, `offline`
- **推荐理由：** Swift Cycle治理技能，支持版本锁定和离线验证。
...
- **标签：** `agent`, `skills`, `workflow`
- **推荐理由：** 游戏化任务系统，迷雾地图引导Agent工作流。
...
- **标签：** `canvas`, `agent-skills`, `creative`
- **推荐理由：** 无限画布+创意技能，为AI工作流提供强大可视化能力
...
- **标签：** `agent-skill`, `engineering`, `best-practices`
- **推荐理由：** 将工程纪律封装为可复用Agent Skill，提升决策质量
...
- **标签：** `godot`, `game-dev`, `agent-skill`
- **推荐理由：** Godot 4.x全栈开发技能插件，运行时自动注册
...
- **⭐ 星标：** 286
- **语言：** TypeScript
- **描述：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without native web access. Free, no signup, no API key. Ask the web or X, get structured JSON evidence. | 🥇 全网最强的 DeepSeek Harness 免费联网搜索插件，免费免注册免 API key。为不能联网的模型补上搜索，问网页或 X，拿回结构化 JSON ���据（搜索、抓取、引用）。
- **标签：** `web-search`, `mcp`
- **推荐理由：** 免费网页搜索桥接，适配无原生联网模型
...
- **标签：** `ai-agents`, `alipay`, `api-documentation`, `chinese-api`, `deepseek-harness`
...
- **推荐理由：** 设置页粘贴JSON即连MCP，热加载免重启，技能管理便捷
...
- **标签：** `ai-image-generation`, `dsh-plugin`, `skills`
- **推荐理由：** 工业级GPT-Image2提示词库，530+模板案例。
...
- **标签：** `diagrams-as-code`, `agent-skills`
- **推荐理由：** 可验证的架构与流程图生成工具，代码即图表。
...
- **标签：** `video`, `generative-ai`, `motion-graphics`
- **推荐理由：** 单主题自动生成Vox风格拼贴解说视频
...
- **标签：** `agent-skills`, `claude-code`, `diagram`, `legal`, `svg`, `visualization`
- **推荐理由：** 从案件材料自动生成诉讼时间轴与可视化图表。
...
- **标签：** `browser`, `content-discovery`, `privacy-first`
- **推荐理由：** 本地私有的跨平台内容发现Agent，支持多平台
...
- **推荐理由：** AI代理可直接操作已登录的真实浏览器，无需中断工作。
...
- **标签：** `mobile-automation`, `mcp`
- **推荐理由：** iOS 模拟器+22 项自动化代理工具
...
- **标签：** `browser`, `playwright`, `automation`, `web`
...
- **⭐ 星标：** 17
- **语言：** HTML
- **描述：** DSH 内测收官合影墙：GitHub OAuth 零��限登录 + 冻结白名单校验的拍立得合影站（含 DSH Skill 包装）
- **标签：** `social`, `community`, `oauth`
- **推荐理由：** DSH内测收官合影墙，GitHub OAuth零门槛登录。
...
- **标签：** `dsh-plugin`, `electron`, `image`
- **推荐理由：** 终��图片上传工具，支持60+图床服务。
...
- **标签：** `routing`, `infrastructure`
- **推荐理由：** 任务感知路由套件，优化Agent推理模式。
...
- **标签：** `vfs`, `virtual-filesystem`
- **推荐理由：** 统一虚拟文件系统，跨会话持久存储。
...
- **标签：** `utility`, `model-switch`, `multi-agent`
- **推荐理由：** 一键切换多个AI编码工具的集成插件
...
- **标签：** `marketplace`, `utility`, `plugin`
- **推荐理由：** DSH可视化插件市场，支持浏览和一键安装
...
- **标签：** `plugin-marketplace`, `harness`, `deepseek-v4`
- **推荐理由：** DeepSeek v4 Pro的DSH插件市场集成
...
- **标签：** `marketplace`, `plugin-manager`, `install`
- **推荐理由：** 一键浏览、安装和更新DSH插件市场所有插件。
...
- **标签：** `migration`, `session`, `context`
- **推荐理由：** 跨预设会话迁移，保留状态和未解决图片。
...
- **标签：** `backup`, `crash-recovery`, `undo`, `snapshot`
- **推荐理由：** 崩溃恢复，撤销配置/插件变更，支持一键安全模式。
...
- **标签：** `console`, `deepseek-harness`, `dsh`, `dsh-plugin`, `plugin-management`
- **推荐理由：** 插件管理控制台与官方开发脚手架工具。
...
- **标签：** `backup`, `config`, `webdav`, `sync`
- **推荐理由：** 通过WebDAV实现DSH配置全量备份恢复与同步。
...
- **标签：** `containers`, `ui`, `local-management`
- **推荐理由：** 本地隔离运行多版本DSH，嵌入WebView界面。
...
- **标签：** `migration`, `utility`, `multi-agent`
- **推荐理由：** 一键迁移Claude Code/Codex配置至DSH，保留技能与MCP
...
- **推荐理由：** 实时Token用量图表与费用估算，API预算管理必备
...
- **标签：** `pets`, `personalization`
- **推荐理由：** 为Coding Agent添加个性化动画宠物。
...
- **标签：** `theme`, `skin`, `whale`
- **推荐理由：** DeepSeek Harness鲸鱼娘系列皮肤主题
...
- **标签：** `theme`, `color-scheme`, `customization`
- **推荐理由：** 34款opencode经典配色一键替换，重启不丢。
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细��图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `webui`, `plugin-market`
- **推荐理由：** Web背景个性化，支持上传和预设主题
...
- **标签：** `ai`, `rag`, `dsh-plugin`
- **推荐理由：** 程序员鱼皮的AI资源大全与零基础教程。
...
- **标签：** `resource`, `documentation`
- **推荐理由：** 学习 DSH 原理、插件市场浏览与最佳实践
...
- **标签：** `awesome-list`, `mcp`, `deepseek-harness`
- **推荐理由：** DSH插件、技能、MCP服务器的精选目录
...
- **标签：** `kline`, `finance`, `technical-analysis`
- **推荐理由：** 交互式K线分析，支持技术指标和市场数据侧边栏。
...
- **推荐理由：** 全面的DeepSeek Harness资源精选列表
...
- **标签：** `dsh-bundle`, `plugin-verify`
- **推荐理由：** DeepSeek Harness插件生态系统注册表
...
- **标签：** `plugin-registry`, `2origin`
- **推荐理由：** 证据驱动的插件雷达和2Origin插件实验室
...
- **标签：** `ai-agents`, `coding-agents`, `recommender`
- **推荐理由：** AI每小时更新12类插件推荐，全面精准。
...
- **标签：** `industry`, `directory`, `deepseek-harness`
- **推荐理由：** 面向计算机、金融、法律、医疗等行业精选。
...
- **标签：** `awesome-list`, `plugin-curation`
- **推荐理由：** 精选DSH插件合集，一站式发现好插件。
...
- **标签：** `toy`, `fun`, `single-file`
- **推荐理由：** 零依赖传统竹知了玩具的网页模拟版
...
- **标签：** `awesome-list`, `plugin-registry`
- **推荐理由：** 收录9000+插件的精选榜单与索引，方便发现优质插件。
...
- **标签：** `agents`, `awesome`, `awesome-list`, `deepseek`, `dsh-plugin`
- **推荐理由：** DSH生态精选插件与资源目录索引。
...
- **标签：** `awesome-deepseek-harness`, `awesome-dsh-plugin`, `awesome-list`, `deepseek-harness`
- **推荐理由：** 社区维护的DSH插件自动同步聚合平台。
