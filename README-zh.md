...
- **标签：** `dsh-web`, `dsh-plugin`, `cordis`, `deepseek-harness`
- **推荐理由：** 基于Web的插件聚合生态系统，Creative Workshop分发
...
- **标签：** `mascot`, `pixel-art`, `sprites`, `dsh-plugin`, `developer-tools`
- **推荐理由：** 动画宠物画廊，为Coding Agent界面增添趣味
...
- **标签：** `sidebar`, `extension`, `deepseek-harness`
- **推荐理由：** 开放侧边栏底座，支持文件编辑、终端、Git等页面。
...
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏、流式思考、实时TPS。
...
- **推荐理由：** 可爱小鲸鱼浮窗，实时显示DeepSeek账户余额与动画
...
- **标签：** `desktop`, `electron`, `ai-workbench`
- **推荐理由：** 本地优先的Electron桌面工作区，整合会话和插件
...
- **标签：** `ui`, `gen-ui`, `dsh-plugin`
- **推荐理由：** 在DSH回复中内联渲染图表、表单、测验等交互组件
...
- **推荐理由：** 本地优先多模态桌面客户端，支持自进化技能与隐私保护
...
- **标签：** `desktop`, `windows`, `electron`, `ssh-client`
- **推荐理由：** 开源Windows桌面客户端，含SSH、Codex与移动端远程访问
...
- **推荐理由：** 跨平台桌面客户端，支持macOS/Windows/Linux的DSH插件套件
...
- **推荐理由：** DSH Web GUI社区插件市场，一键浏览和安装插件，扩展生态便捷易用。
...
- **推荐理由：** Electron桌面管理控制台，多标签页统一管理系统、插件和MCP配置。
...
- **⭐ 星标：** 12
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运维每天要用到的工具收进同一个窗口:数���库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `docker`, `ssh`, `devops`
- **推荐理由：** 集成SSH/SFTP/Docker/数据库/AI沙箱的DevOps桌面
...
- **推荐理由：** LLM知识平台，将文档转为可查询RAG与自维护Wiki。
...
- **标签：** `agent-memory`, `long-term-memory`, `rag`, `mcp`, `skills`
- **推荐理由：** 可移植的用户自有记忆层，Markdown原生自进化
...
- **标签：** `long-term-memory`, `rag`, `mcp`, `self-evolving`, `token-savings`
- **推荐理由：** 超持久混合检索记忆，显著节省Token消耗
...
- **推荐理由：** 将分散技能文件转为可检索的知识库，支持语义搜索与评估
...
- **标签：** `harness`, `dsh-plugin`, `agent`, `llm`
- **推荐理由：** 从零到一掌握Harness工程开发的入门教程
...
- **标签：** `agent-skills`, `dsh-plugin`, `skill`, `coding`, `automation`
- **推荐理由：** 最全技能商店，自动抓取GitHub技能按分类整理
...
- **推荐理由：** 智能技能路由和流程编排，SkillsBench奖励提升21.12pp。
...
- **标签：** `cognitive-enhancement`, `j-space`, `agent-skills`
...
- **推荐理由：** 让DeepSeek Harness支持多智能体团队协作
...
- **标签：** `task-management`, `sqlite`, `agent-workflow`
- **推荐理由：** SQLite驱动的本地任务看板，支持Agent认领与审核工作流
...
- **推荐理由：** 全流程自动数学建模竞赛技能，支持GitOps流水线
...
- **标签：** `self-evolve`, `agent-review`, `prompt-engineering`
- **推荐理由：** 可组合任务控制，实现Agent自检阅与自我进化
...
- **推荐理由：** 为Claude Code添加多模态视觉，支持UI和图表分析。
...
- **推荐理由：** 源自superpowers的TDD、调试和规划技能。
...
- **标签：** `reverse`, `pentest`, `security`, `ctf`
- **推荐理由：** 87个逆向工程和渗透测试技能，用于安全研究。
...
- **推荐理由：** 88个可复用技能，支持研究、营销和商业自动化，兼容多个AI编程框架。
...
- **标签：** `agent-skills`, `education`, `coding-agent`
- **推荐理由：** AI编程导师技能，自动生成学习章节、指导手写代码并提供代码审查。
...
- **标签：** `skill-generator`, `agent-skills`, `automation`
- **推荐理由：** 自动将成功的DSH会话转换为可复用技能，本地优先的技能学习工作流。
...
- **标签：** `skills`, `deck`, `wayfinder`, `deepseek-harness`
- **推荐理由：** 将 Matt Pocock 技能套件集成到 DSH，含 wayfinder 和效率工具。
...
- **推荐理由：** 便携式 Agent 技能，遵循 DSH 工程模式进行规范驱动开发。
...
- **标签：** `chinese-traditional-wisdom`, `bazi`, `fengshui`, `deepseek-harness`
- **推荐理由：** 整合中华传统智慧（八字、风水等）的 AI Agent 技能包。
...
- **标签：** `gongwen`, `document-processing`, `deepseek-harness`
- **推荐理由：** 符合国家标准的完整中文公文处理工具，支持格式检查和优化。
...
- **标签：** `skills`, `dsh`, `agent`
- **推荐理由：** 统一管理DSH本地Agent Skills，安全可靠
...
- **标签：** `skill-picker`, `composer`, `workflow`
- **推荐理由：** WorkBuddy风格技能选择器，集成于DSH
...
- **标签：** `handoff`, `session-memory`, `agent`
- **推荐理由：** 跨平台Agent会话连续性记忆，支持交接
...
- **推荐理由：** 直接搜索20个中国开放平台65600+篇API文档，零配置跨框架通用。
...
- **标签：** `ai-agents`, `chatgpt`, `deepseek-harness`, `oauth`, `subscription`
...
- **推荐理由：** 开源设计系统替代方案，AI驱动的UI原型本地优先工具。
...
- **标签：** `vibedesign`, `mcp`, `skill`, `flutter`, `react`, `ui`
...
- **标签：** `video`, `generative-ai`, `ffmpeg`, `agent-skills`
- **推荐理由：** 一键自动生成Vox风格解说视频，端到端自动化
...
- **标签：** `design`, `ppt`, `ai-design`
- **推荐理由：** AI生成设计系统，支持可视化编辑、模板和PPT制作
...
- **推荐理由：** 法律诉讼可视化工具，重绘案件图并生成时间轴，直接用于诉讼材料。
...
- **推荐理由：** 为纯文本模型提供像素转文本的图像读取，含 OCR 和视觉工具。
...
- **标签：** `awesome-dsh-plugin`, `knowledge-cards`, `ui`
- **推荐理由：** 将聊天记录转换为精美的可分享知识卡片。
...
- **标签：** `ios`, `mobile-automation`, `ui-automation`, `mcp`
- **推荐理由：** 内置iOS模拟器，22个Agent工具用于移动端UI自动化测试
...
- **推荐理由：** Tabbit浏览器集成，基于Playwright的强大浏览器自动化能力。
...
- **标签：** `low-code`, `internal-tools`, `dsh-plugin`
- **推荐理由：** AI+低代码平台，快速构建企业内部业务系统。
...
- **推荐理由：** 一键安装并在Claude Code/Codex/DeepSeek等多平台切换模型。
...
- **标签：** `armor-breaking`, `utility`, `dsh-plugin`
- **推荐理由：** DeepSeek专用破甲插件，稳定增强模型能力
...
- **标签：** `security`, `redteam`, `dsh-bundle`
- **推荐理由：** AI安全攻防平台，审计幻觉包与漏洞，保护AI生成代码
...
- **推荐理由：** 用ChatGPT/Claude/Grok订阅作为DSH LLM提供商，OAuth免API密钥
...
- **推荐理由：** 追踪会话成本、预算和token用量，支持OpenCode额度管理
...
- **⭐ 星标：** 180
- **语言：** TypeScript
- **描述：** Bridge the Pi and DeepSeek Harness ecosystems: one Pi Host ABI runs unmodified Pi extensions as native DSH plugins. 打��� Pi 与 DSH 生态。
- **标签：** `compatibility`, `migration`, `bridge`
- **推荐理由：** 通过ABR兼容层桥接Pi生态与DSH，无缝迁移插件
...
- **标签：** `mcp`, `configuration`, `tool-management`
- **推荐理由：** 统一管理MCP工具和技能的暴露级别与执行模式，安全可控。
...
- **推荐理由：** 完整的DSH配置备份恢复工具，支持WebDAV同步插件和MCP配置。
...
- **⭐ 星标：** 35
- **语言：** TypeScript
- **描述：** 帮助插件��动随着dsh版本升级的skill
- **标签：** `upgrade`, `plugin-management`, `deepseek-harness`
- **推荐理由：** 帮助插件随 DSH 版本更新平滑升级的技能。
...
- **标签：** `dshbox`, `container`, `deepseek-harness`
- **推荐理由：** 本地管理工具，可在隔离容器中运行多个 DSH 版本。
...
- **标签：** `context-audit`, `token-management`, `deepseek-harness`
- **推荐理由：** 上下文注入审计插件，统计 token 成本并检测重复和冲突。
...
- **标签：** `data-visualization`, `cost`, `token-tracking`
- **推荐理由：** 实时Token用量追踪、费用估算和API余额可视化
...
- **标签：** `ai-agent`, `deepseek`, `chatgpt`, `claude`, `gemini`, `rag`, `skills`
...
- **推荐理由：** 自动搜索8大中文平台，生成有据可查的研究报告的AI技能
...
- **推荐理由：** 含11000+插件的市场，提供免费API和搜索排序
...
- **推荐理由：** 智能搜索、安装并验证GitHub上的DSH插件技能
...
- **推荐理由：** 跨PubMed、arXiv、CrossRef的综合学术检索，支持引文图谱。
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU SSH、实验追踪。
...
- **推荐理由：** 专利侵权分析系统，输入专利号自动生成竞品侵权分析报告。
...
- **标签：** `industry-research`, `cordis`, `deepseek-harness`
- **推荐理由：** 全面研究行业公司，含方法论、产业链映射和政策追踪。
...
- **推荐理由：** 证据驱动的多维度评估框架，支持提示词/RAG/技能评测
...
- **标签：** `ai-agents`, `coding-agents`, `deepseek-harness`, `mcp`, `plugin`
- **推荐理由：** AI整理的190+插件列表，覆盖12个分类，每小时更新。
...
- **推荐理由：** DSH生态插件资源大全，中英双语 curated 列表
...
- **标签：** `ai-knowledge`, `awesome-list`, `mcp`
- **推荐理由：** AI全维度资源百科，涵盖Agent、RAG、MCP等一站式导航
...
- **推荐理由：** DSH生态精选插件和资源列表，发现和管理插件的必备指南。
...
- **推荐理由：** 自动同步的插件社区目录，每4小时更新，方便发现和安装优质插件。
...
- **推荐理由：** 精选 280+ 个 DSH 插件目录，分 14 类并提供直达链接。
...
- **标签：** `awesome-list`, `mcp`, `resources`
- **推荐理由：** 全面的DSH插件、工具、技能和MCP资源精选列表
...
- **推荐理由：** A curated, evidence-led directory of Dee
...
- **推荐理由：** Curated list of DeepSeek Harness (DSH) p
...
- **推荐理由：** 有证据支持的DSH插件雷达与2Origin插件实验室。
...
- **推荐理由：** 自动更新的精选插件列表，YAML数据驱动双语README。
...
- **标签：** `coding-agents`, `deepseek-harness`, `llm`, `skills`, `ui`
- **推荐理由：** 涵盖模型、工具、沙箱、UI和Agent循环的精选插件。
