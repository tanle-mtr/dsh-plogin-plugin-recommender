...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 198,442 | Core DSH framework — everything is a plugin |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,838 | Local-first design tool powered by AI agents |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,492 | Multi-agent swarms with MCP and skill coordination |
...
- **标签：** `ai-agent`, `multi-model`, `skills`
- **推荐理由：** 多模型AI助手，支持记忆与自进化
...
- **标签：** `preset`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 两阶段启动配置，Project2评分98/99
...
- **标签：** `claude-code`, `deepseek-harness`, `multi-agent`
- **推荐理由：** 一键切换Claude、DeepSeek等8款编程Agent模型。
...
- **标签：** `agent-runtime`, `desktop`, `code-agent`
- **推荐理由：** Rust高性能Agent运行时，搭配精美桌面GUI应用。
...
- **标签：** `code-review`, `lint`, `agent-skills`
- **推荐理由：** 基于经典工程书籍的AI代码审查，支持自动修复。
...
- **标签：** `coding`, `architecture`, `agent`
- **推荐理由：** 让编码代理具备架构感知与证据驱动验证
...
- **标签：** `vision`, `ocr`, `coding`
- **推荐理由：** 强大视觉工具箱，为纯文本模型赋能图像识别
...
- **标签：** `dsh-plugin`, `ui-components`, `interactive`
- **推荐理由：** 在DSH对话中内联渲染交互式UI组件
...
- **标签：** `agent`, `coding-agents`, `teamwork`
- **推荐理由：** 将多个编码代理整合为统一协作团队
...
- **标签：** `plugin-dev`, `agent-skills`
- **推荐理由：** 用于开发DSH插件的Agent技能与标准
...
- **推荐理由：** Codex与DSH双向同步聊天、技能和MCP工具的桥��。
...
- **标签：** `context`, `ui`, `dsh-plugin`
- **推荐理由：** 审计指令链并检测Token浪费，带Web UI面板。
...
- **标签：** `ui-generator`, `figma-alternative`, `design-systems`
- **推荐理由：** 本地优先的AI设计工具
...
- **标签：** `low-code`, `no-code`, `ai-agent`
- **推荐理由：** AI驱动的零代码业务系统构建平台
...
- **标签：** `dsh-web`, `web-ui`, `dsh-plugin`
- **推荐理由：** DSH Web插件聚���生态分发中心
...
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/Git/对话面板。
...
- **标签：** `desktop`, `companion`, `pets`
- **推荐理由：** 可爱桌面陪伴平台，含动画宠物与插件SDK
...
- **标签：** `desktop`, `workspace`, `local`
- **推荐理由：** 本地优先的DSH桌面工作空间，集成办公能力
...
- **标签：** `desktop-app`, `electron`, `windows`
- **推荐理由：** 功能丰富的Windows桌面GUI，支持SSH和远端访问
...
- **标签：** `ui`, `utils`, `dsh-plugin`
- **推荐理由：** 实时显示提供商、余额与峰谷定价。
...
- **标签：** `webui`, `theme`, `personalization`, `dsh-plugin`
- **推荐理由：** 自定义DSH网页背景，支持图片与预设主题
...
- **标签：** `video-generation`, `skill`, `ffmpeg`
- **推荐理由：** 端到端Vox风格拼贴动画视频生成Agent技能。
...
- **标签：** `workflow`, `productivity`, `prompt`
- **推荐理由：** 将AI对话转化为可组织复用的跨平台工作流
...
- **推荐理由：** 破甲提示词插件，稳定DeepSeek Agent能力
...
- **标签：** `agent-skills`, `math-modeling`
- **推荐理由：** 全栈式自动数学建模竞赛Skill，支持GitOps流水线
...
- **标签：** `skills`, `agent-framework`
- **推荐理由：** 将经验转化为跨多AI Agent的可重复执行流程
...
- **推荐理由：** 原生SQLite任务板，支持Agent认领/审核和清晰Web界面。
...
- **推荐理由：** 面向AI基础设施开发的专用Agent技能，支持编写和优化。
...
- **标签：** `agent-review`, `plan-mode`, `self-evolving`
- **推荐理由：** 可组合任务控制和人工审查的Agent自进化模式。
...
- **标签：** `skills`, `tdd`, `collaboration`
- **推荐理由：** 从obra/superpowers适配的TDD、调试、规划协作技能。
...
- **标签：** `agent-skill`, `mentor`, `code-review`
- **推荐理由：** AI编程导师，指导项目架构与代码审查
...
- **标签：** `agent-skills`, `spec-programming`, `vibe-coding`
- **推荐理由：** 仓库级规范驱动开发，提升编码效率与一致性。
...
- **标签：** `agent-skills`, `plugin-management`
- **推荐理由：** 简化插件槽位、远程端点和凭据管理。
...
- **标签：** `skill-picker`, `productivity`, `ui-helper`
- **推荐理由：** 类WorkBuddy风格技能选择器，集成至DSH编辑器。
...
- **标签：** `code-simplification`, `refactoring`, `agent-skill`
- **推荐理由：** 受DSH启发的代码简化技能，帮助重构更整洁代码库。
...
- **标签：** `multi-agent`, `collaboration`, `team-workflow`
...
- **标签：** `skills`, `agent`, `dsh-plugin`
- **推荐理由：** 游戏化任务系统，迷雾地图推进任务进度。
...
- **标签：** `agent-framework`, `dsh-plugin`
- **推荐理由：** 42技能+5工具的AI代理资产管理包。
...
- **标签：** `agent-tools`, `ai-agents`, `llm-agents`, `dsh-plugin`
- **推荐理由：** Agent任务预检插件库，自动注入Top-K相关插件
...
- **推荐理由：** DSH内Web界面管理技能和MCP服务器的面板工具。
...
- **推荐理由：** 在DSH中直接搜索20个中国平台65000+ API文档
...
- **标签：** `mcp`, `research`, `zhihu`, `dsh-plugin`
- **推荐理由：** 知乎搜索与知道提问的MCP服务器。
...
- **标签：** `image-generation`, `deepseek-harness`
- **推荐理由：** 在DSH聊天中直接生成AI图像，支持多模态模型
...
- **标签：** `photo-editing`, `image-compositing`
- **推荐理由：** AI驱动的摄影到编辑技能，严格保真
...
- **标签：** `design-skills`, `visual`, `dsh-plugin`
- **推荐理由：** 6种风格防止AI味，保持设计美感。
...
- **标签：** `image-generation`, `vision`, `dsh-plugin`
- **推荐理由：** 粘贴图片自动识别与DeepSeek图像生成。
...
- **标签：** `video`, `image`, `ai-director`, `dsh-plugin`
- **推荐理由：** AI视频/图像/音频生成技能包，支持分镜。
...
- **标签：** `bilibili`, `youtube`, `content-discovery`
- **推荐理由：** 本地优先的跨平台内容发现Agent，覆盖8大主流平台。
...
- **标签：** `web-search`, `free`, `dsh-plugin`
- **推荐理由：** 免费Web搜索桥接插件，无需注册或API密钥
...
- **标签：** `social`, `oauth`, `dsh-plugin`
- **推荐理由：** GitHub OAuth零权限合影墙，社区纪念。
...
- **标签：** `widget`, `balance`, `utility`
- **推荐理由：** 可爱浮动鲸鱼挂件，实时监控DeepSeek余额
...
- **标签：** `oauth`, `subscription`, `dsh-plugin`
- **推荐理由：** 用订阅账号替代API密钥作为DSH的LLM提供者
...
- **推荐理由：** 开源锤子便签，支持Docker部署、skill调用及公众号导出。
...
- **标签：** `marketplace`, `plugin-manager`, `browse`
- **推荐理由：** 一键浏览、安装和更新GitHub上的所有DSH插件。
...
- **推荐理由：** 跨预设会话迁移，保留状态和源模型意图的固定方案对接。
...
- **推荐理由：** 社区插件市场浏览，在DSH Web GUI中一键安装卸载。
...
- **标签：** `capability-menu`, `mcp-management`
...
- **标签：** `backup`, `config-manager`, `migration`, `sync`
- **推荐理由：** 完整的DSH配置备份、恢复和跨环境迁移工具。
...
- **标签：** `container`, `isolation`, `plugin-management`
- **推荐理由：** 多版本隔离运行，内置UI，一键导入插件技能。
...
- **标签：** `skills-manager`, `organization`, `utility`
- **推荐理由：** 专门用于高效管理和组织DSH技能的插件工具。
...
- **标签：** `dsh-plugin`, `deepseek-harness`, `conflict-resolution`
...
- **标签：** `ai-resources`, `rag`, `prompt-engineering`
- **推荐理由：** 全面AI资源库与提示词教程合集
...
- **标签：** `research`, `web-search`, `skills`
- **推荐理由：** 自动搜索8大中文平台，生成有据可查的研究简报。
...
- **⭐ 星标：** 289
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快���看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `discovery`, `dsh-plugin`
- **推荐理由：** 每日精选插件发现，支持场景化分类与排行
...
- **标签：** `awesome-list`, `ai-pedia`, `resources`
- **推荐理由：** 覆盖技能、MCP、RAG的全方位AI资源百科
...
- **标签：** `deep-research`, `market-research`, `social-listening`
- **推荐理由：** 88个代理技能，涵盖研究与社交智能
...
- **标签：** `kline`, `financial`, `chart-analysis`, `market-data`
- **推荐理由：** 交互式K线分析插件，支持技术指标和多元行情数据。
...
- **标签：** `evaluation`, `rag`, `dsh-plugin`
- **推荐理由：** 提示词、RAG、技能的证据驱动评估框架。
...
- **推荐理由：** DeepSeek Harness综合指南和资源精选合集
...
- **标签：** `awesome-list`, `mcp`, `dsh-plugin`
- **推荐理由：** DSH插件、技能、MCP服务器精选目录
...
- **标签：** `awesome-list`, `plugin-management`
- **推荐理由：** 官方自动生成的DeepSeek Harness插件目录
...
- **推荐理由：** Portable Agent Skill for developing and auditing DeepSeek Harness plugins, with 
...
- **推荐理由：** Engineering discipline distilled from DeepSeek Harness — fact-finding, durable d
...
- **标签：** `agent-migration`, `claude-code`, `codex`, `configuration-migration`, `deepseek-harness`, `developer-tools`, `dsh`, `dsh-plugin`, `import`, `jsonc`, `mcp`, `migration`, `opencode`, `skills`
- **推荐理由：** Migrate Claude Code setup into DeepSeek Harness. Import skills, commands, agents
...
- **推荐理由：** A curated list of plugins for DeepSeek Harness (dsh) — 精选 DeepSeek Harness 插件列表
...
- **推荐理由：** Awesome DSH Plugins: a public GitHub directory for DeepSeek Harness plugins, DSH
...
- **标签：** `finance`, `quant`, `stock`, `dsh-plugin`
- **推荐理由：** 精选股票/量化插件目录，含24项认证条目
...
- **推荐理由：** 全面覆盖插件、工具、技能与资源的Awesome列表
...
- **推荐理由：** 实用好用的DeepSeek Harness插件集合
...
- **标签：** `plugin-directory`, `awesome-list`, `dsh-plugin`
- **推荐理由：** 按场景组织的插件说明书与导航指南
...
- **标签：** `plugin-portal`, `static-site`, `web`, `dsh-plugin`
...
- **标签：** `dsh-plugin`, `dsh-ecosystem`, `awesome-list`
- **推荐理由：** DSH插件包与生态系统资源合集
...
- **标签：** `dsh-plugin`, `awesome-list`, `ai-agent`, `plugin-registry`
- **推荐理由：** 证据驱动的插件雷达与2Origin插件实验室
...
- **标签：** `awesome-list`, `dsh-plugin`, `ai-agents`, `deepseek`
- **推荐理由：** 双语认证插件目录，不可变提交审核
...
- **推荐理由：** DingTalk IM channel for DeepSeek Harness: Stream-mode bot that turns each chat i
...
- **推荐理由：** DSH plugins directory: 70+ verified DeepSeek Harness plugins, organized by what 
...
- **推荐理由：** Moved → github.com/awesome-dsh-plugin/awesome-dsh-plugin
...
- **推荐理由：** A curated, evidence-aware shortlist of useful DeepSeek Harness plugins
