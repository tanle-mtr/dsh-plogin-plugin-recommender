...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 210,292 | Core harness platform with plugin-first architectu |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 93,733 | Open-source Figma alternative with AI design gener |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 70,299 | Multi-agent swarm system with autonomous workflow  |
...
- **标签：** `routing`, `agent`, `dsh-plugin`, `cordis`
- **推荐理由：** 任务感知推理模式路由套件，提升Agent效率。
...
- **标签：** `loopx`, `agent-control-plane`, `workflow-automation`, `dsh-plugin`
- **推荐理由：** 长程Agent控制平面，支持多引擎持久化工作流。
...
- **推荐理由：** 首个视觉插件，为纯文本编码Agent提供图像转JSON能力。
...
- **标签：** `agent-runtime`, `rust`, `desktop`, `vibe-coding`
...
- **标签：** `code-review`, `quality`, `static-analysis`
- **推荐理由：** 基于经典工程书籍的智能代码审查与修复
...
- **推荐理由：** 强大的编码Agent，支持RPC、子Agent和MCP集成。
...
- **标签：** `context-migration`, `session`, `dsh-plugin`
- **推荐理由：** 跨预设会话迁移工具，保持状态与意图。
...
- **标签：** `plugin-dev`, `agent-skill`, `scaffold`
- **推荐理由：** 兼容Claude Code/Codex的插件开发标准技能
...
- **标签：** `spec`, `coding`, `agent-skill`
- **推荐理由：** 仓库原生的规格驱动开发Agent技能
...
- **标签：** `guide`, `knowledge-base`, `scaffold`
- **推荐理由：** 可安装的插件开发知识库含官方文档存档
...
- **标签：** `creator`, `codex`, `plugin`
- **推荐理由：** 可复用的Codex插件构建与验证技能
...
- **标签：** `agent-framework`, `skills`, `mcp`, `dev-tools`
- **推荐理由：** 42个技能+5个CLIs，AI Agent工程一站式包管理器
...
- **标签：** `mascot`, `pixel-art`, `cli`, `developer-tools`
- **推荐理由：** 编程Agent动画宠物画廊，装饰你的开发终端。
...
- **⭐ 星标：** 1,647
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数���滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `widget`, `desktop`, `ui`
- **推荐理由：** 萌系鲸鱼浮动组件实时显示账户余额
...
- **推荐理由：** 功能完整的Electron桌面工作空间集成DSH会话
...
- **标签：** `ui`, `marketplace`, `plugin-management`
- **推荐理由：** 嵌入DSH Web GUI的社区插件市场，一键安装
...
- **推荐理由：** Electron桌面控制台，多标签管理插件和MCP
...
- **标签：** `dashboard`, `skills-deck`, `deck`
- **推荐理由：** 将Matt Pocock技能套接入DSH控制面板
...
- **标签：** `ui`, `web`, `usage-tracking`, `dsh-plugin`
- **推荐理由：** 底部信息栏展示提供商、模型、余额和定价
...
- **推荐理由：** 数据库、SSH、SFTP、Docker与AI沙箱一体化桌面。
...
- **标签：** `dsh-plugin-market`, `ui`, `deepseek-harness`
- **推荐理由：** 为DSH Web面板添加一键紧凑按钮。
...
- **标签：** `agent-memory`, `rag`, `memory-management`, `mcp`
...
- **标签：** `memory`, `rag`, `long-term-memory`, `skills`
...
- **标签：** `memory`, `context-engineering`, `mcp`, `agentic-rag`
- **推荐理由：** 构建有状态Agent的内存库，支持上下文工程。
...
- **标签：** `agent-workspace`, `orchestration`, `no-code`, `mcp`
- **推荐理由：** 多设备统一的Agent工作空间，支持任务看板。
...
- **标签：** `skills`, `marketplace`, `dsh-plugin`, `workflow`
- **推荐理由：** 最全技能商店，自动抓取整理上万个Skills项目。
...
- **标签：** `agent-os`, `loop-engineering`, `llm-orchestration`, `skills`
- **推荐理由：** 自进化Agent操作系统，循环工程驱动自主成长。
...
- **标签：** `agent-collaboration`, `multi-agent`, `workspace`, `dsh-plugin`
- **推荐理由：** 企业级��引擎Agent工作台，支持可视化编辑。
...
- **推荐理由：** 全栈数学建模技能，支持GitOps流水线与代码自证。
...
- **标签：** `self-evolving`, `review`, `composable`
- **推荐理由：** 实现可组合、可审查的Agent工作流与自我进化。
...
- **标签：** `plugin-discovery`, `automation`, `github`
- **推荐理由：** 自动化从GitHub搜索、安装和验证DSH插件。
...
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销和业务流程
...
- **标签：** `skills`, `automation`, `skill-generator`
...
- **⭐ 星标：** 58
- **语言：** Python
- **描述：** 专利侵权分析系统 —— ���入专利公开号，产出竞品侵权分析报告；同时打包成 skill，可被任意 agent（dsh, codex, claudecode 等） 调用。
- **标签：** `skills`, `patent`, `legal-tech`, `rag`
- **推荐理由：** 专利侵权分析技能，输入专利号生成竞品报告
...
- **标签：** `multi-agent`, `team`, `workflow`
- **推荐理由：** 多Agent团队协作共享工作区技能
...
- **标签：** `skills-manager`, `security`, `load`
- **推荐理由：** 统一加载安全管理本地Agent技能
...
- **推荐理由：** 类似WorkBuddy的技能选择器，便于快速调用技能
...
- **标签：** `agent-handoff`, `skills`, `session-memory`
- **推荐理由：** 实现跨Agent会话连续性，保存项目上下文记忆
...
- **标签：** `qa`, `testing`, `skills`, `automation`
- **推荐理由：** 10个测试技能框架，助力AI驱动的质量保证
...
- **标签：** `godot`, `game-dev`, `gdscript`, `skills`
...
- **标签：** `agent-evaluation`, `evaluation-framework`, `dsh-plugin`
- **推荐理由：** 面向提示词、RAG、技能和Agent的量化评估框架。
...
- **推荐理由：** Swift Cycle治理技能适配DSH，可离线验证。
...
- **标签：** `skill`, `dsh-plugins`, `developer-tools`
- **推荐理由：** 指导AI Agent从零开发、构建和测试DSH插件。
...
- **标签：** `api`, `mcp`, `chinese-platforms`
- **推荐理由：** 直接在IDE中搜索20个中国开放平台的65,600+ API文档
...
- **标签：** `mcp`, `skills`, `ui`, `dsh-plugin`
- **推荐理由：** 在DSH Web GUI中直接管理技能和MCP服务器
...
- **标签：** `mcp`, `mcp-server`, `skill-management`
- **推荐理由：** UI粘贴JSON即连MCP服务器，热加载免重启。
...
- **标签：** `figma-alternative`, `ui-generator`, `dsh-plugin`
- **推荐理由：** 开源Figma替代品，AI驱动设计原型
...
- **标签：** `design`, `vector`, `ui`, `vibedesign`, `mcp`
- **推荐理由：** AI原生矢量设计工具，支持Agent协作提示生成UI。
...
- **标签：** `design`, `ui`, `vibe-coding`, `skills`
...
- **标签：** `video`, `image`, `audio`, `ai-director`
- **推荐理由：** AI视频/图像/音频创作，含分镜和模型工具
...
- **标签：** `theming`, `webui`, `visual`
- **推荐理由：** 支持自定义上传和预设的背景主题插件
...
- **标签：** `knowledge-cards`, `deepseek-harness`, `ui`
- **推荐理由：** 将聊天记录转为可分享知识卡片。
...
- **推荐理由：** 跨平台AI内容发现Agent，支持B站/抖音/小红书等。
...
- **推荐理由：** 基于Playwright的浏览器自动化插件，专为DSH打造
...
- **标签：** `dsh-plugin`, `self-hosted`, `resume-builder`
- **推荐理由：** 隐私优先的开源简历构建工具
...
- **标签：** `low-code`, `dsh-plugin`, `workflows`
- **推荐理由：** AI驱动的低代码业务系统开发平台
...
- **标签：** `model-switch`, `cli`, `multi-agent`
- **推荐理由：** 一键安装并切换Claude/Codex/Grok/DeepSeek等模型。
...
- **标签：** `plugin-registry`, `ecosystem`, `automation`
- **推荐理由：** 自动发现15900+插件并实时K8s级验证
...
- **标签：** `remote-access`, `infrastructure`, `security`
- **推荐理由：** 多端P2P远程访问方案，端到端加密安全。
...
- **标签：** `infrastructure`, `plugin-management`, `console`
- **推荐理由：** DSH插件生态基建，含控制台和开发指南
...
- **标签：** `upgrade`, `auto`, `utility`
- **推荐理由：** 随DSH版本自动升级的插件技能
...
- **标签：** `container`, `manager`, `dshbox`
- **推荐理由：** 本地容器化DSH管理器带嵌入式WebView
...
- **标签：** `sync`, `mcp`, `codex`, `dsh-plugin`
- **推荐理由：** Codex与DSH双向同步桥梁，支持实时技能与MCP镜像
...
- **标签：** `migration`, `agent-migration`, `configuration-migration`
...
- **推荐理由：** DSH实时Token用量、成本图表与API余额监控。
...
- **标签：** `conflict-resolution`, `utilities`
- **推荐理由：** 解决插件冲突与版本不兼容问题
...
- **推荐理由：** 惊艳的WebGPU海洋主题视觉皮肤，美化DSH界面。
...
- **标签：** `color-scheme`, `themes`, `ui`
- **推荐理由：** 34款经典配色一键切换，无需重启。
...
- **标签：** `ai`, `prompt`, `tutorial`, `deepseek`
- **推荐理由：** AI资源大全，涵盖主流大模型教程与Prompt合集。
...
- **标签：** `tutorial`, `dsh`, `harness-engineering`, `agent`
- **推荐理由：** 从零开始的DSH引擎入门教程，适合初学者。
...
- **推荐理由：** 自动搜索8大中文平台生成30天研究报告，适合深度调研
...
- **推荐理由：** 插件市场，11000+条目，支持搜索、排序和API。
...
- **标签：** `research`, `latex`, `paper-writing`
- **推荐理由：** 一站式科研工作台，支持LaTeX编译与实验追踪。
...
- **标签：** `research`, `industry`, `cordis`
- **推荐理由：** 产业链映射与政策跟踪研究工具包
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产品说明书与导航：按场景选插件，了解���装、权限、兼容性和风险
- **标签：** `directory`, `guide`
- **推荐理由：** 插件说明书与兼容性导航指南
...
- **标签：** `discoverability`, `search`, `deepseek-harness`
...
- **标签：** `recommender`, `ai-agents`, `deepseek-harness`
- **推荐理由：** AI推荐190+插件，覆盖12个分类。
...
- **标签：** `armor-breaking`, `specialized`, `generation`
- **推荐理由：** DeepSeek专用破甲增强生成插件
...
- **标签：** `awesome-list`, `dsh-plugin`, `curation`
- **推荐理由：** DSH插件精选列表，快速发现优质插件。
...
- **标签：** `awesome-list`, `cordis`, `curation`
- **推荐理由：** Cordis官方精选DSH插件自动生成列表。
...
- **标签：** `plugin-marketplace`, `community`, `catalog`
...
- **标签：** `awesome-list`, `plugins`, `skills`
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `dsh-repository-plugin`
- **推荐理由：** 通用DSH插件仓库及生态列表。
...
- **标签：** `awesome-list`, `deepseek-harness`, `auto-update`
- **推荐理由：** 自动更新双语插件列表，部署于Vercel。
...
- **标签：** `curated-list`, `deepseek-harness`, `plugins`
- **推荐理由：** 覆盖模型、工具、UI等插件精选。
