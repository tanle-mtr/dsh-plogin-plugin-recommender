...
- **标签：** `ai-agents`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 插件生态基石，万物皆插件
...
- **标签：** `multi-agent`, `mcp`, `agent-memory`, `dsh-plugin`
- **推荐理由：** 支持记忆与技能的自进化多智能体Harness
...
- **标签：** `terminal`, `cli`, `prompt-caching`, `deepseek`
...
- **标签：** `dsh-plugin`, `routing`, `corderis`
...
- **推荐理由：** 自进化Agent操作系统，支持分阶段评估与预算循环
...
- **标签：** `agent-control-plane`, `long-horizon`, `codex`
- **推荐理由：** 长期Agent控制面，支持多Harness的统一治理与持久化工作流。
...
- **推荐理由：** 首个视觉插件，为纯文本Agent提供图像识别与结构化输出。
...
- **推荐理由：** 虚拟终端加VFS，为Agent提供沙箱化状态保持Shell会话。
...
- **推荐理由：** 官方插件开发知识库，中英双语文档+Cordis入门
...
- **标签：** `documentation`, `official`, `skills`
- **推荐理由：** 官方技能文档与示例，适合DSH插件开发入门
...
- **推荐理由：** 开发DSH插件的Agent Skill，提供标准规范。
...
- **标签：** `agent-tools`, `pre-flight`, `reliability`
- **推荐理由：** DSH Agent插件预检提升可靠性
...
- **推荐理由：** 动画宠物画廊，为编码Agent桌面增添个性化视觉体验。
...
- **推荐理由：** 可扩展侧边栏底座，内置文件/终端/Git/子Agent等页面。
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** 功能丰富的TUI增强：鲸鱼顶栏/流式思考/回滚与上下文进度。
...
- **标签：** `floating-widget`, `dsh-plugin`
- **推荐理由：** 可爱浮动组件监控DeepSeek账户余额
...
- **标签：** `desktop`, `gui`, `ssh`, `remote-access`
- **推荐理由：** 零配置桌面GUI，支持SSH和移动端远程控制。
...
- **标签：** `desktop`, `mcp`, `office-automation`, `local-first`
- **推荐理由：** 本地优先桌面工作区，支持Web研究与MCP集成。
...
- **标签：** `workspace`, `canvas`, `conversation`, `nonlinear`
...
- **标签：** `office`, `univer`, `docs`, `spreadsheets`
- **推荐理由：** 将电子表格、文档、幻灯片等办公能力原生引入DSH
...
- **推荐理由：** 本地优先的多模型桌面端，Claude Cowork的开源替代
...
- **推荐理由：** 原生iOS客户端，将DSH工作区和智能体轨迹带入iPhone
...
- **标签：** `desktop-app`, `electron`, `cross-platform`, `codepilot`
- **推荐理由：** 灵感来自CodePilot的跨平台桌面客户端，支持macOS/Windows/Linux
...
- **⭐ 星标：** 166
- **语言：** TypeScript
- **描述：** 开源版锤子便签，复刻锤科��学，一键Docker私有化部署，支持skill调用，支持dsh plugin，支持多租户，一键生成公众号格式，支持导出便签为图片
- **标签：** `notes`, `productivity`, `social`
- **推荐理由：** 开源锤子便签支持IM盯盘与公众号导出
...
- **推荐理由：** 面向临床的.bioinformatics桌面Agent，支持离线
...
- **推荐理由：** 替换composer统计行为，显示提供商/模型/余额信息
...
- **标签：** `context`, `ui`, `audit`
- **推荐理由：** 上下文注入审计工具，Web UI面板+token成本分析
...
- **标签：** `agent-memory`, `agentic-rag`, `self-evolving`, `context-database`
...
- **标签：** `rag`, `vector-search`, `knowledge-base`, `wiki`
- **推荐理由：** 腾讯开源RAG平台，支持自维护知识库与推理
...
- **推荐理由：** 本地优先的便携记忆层，Markdown原生跨应用持久化
...
- **标签：** `agent-memory`, `rag`, `skills`, `token-optimization`
...
- **推荐理由：** Agent记忆管理套件，集成RAG实现记忆存储与精炼。
...
- **标签：** `rag`, `skill-management`, `retrieval`, `embeddings`
- **推荐理由：** 将SKILL.md文件整理为可检索的Agent技能知识库。
...
- **标签：** `multi-agent`, `swarm`, `mcp-server`, `agentic-workflow`
- **推荐理由：** 部署多智能体群体与自主工作流
...
- **标签：** `low-code`, `internal-tools`, `workflows`, `dsh-plugin`
...
- **标签：** `agent-workspaces`, `skills`, `no-code`, `mcp`
- **推荐理由：** 跨设备统一工作空间，集中管理所有Agent
...
- **标签：** `agent-collaboration`, `skills`, `multi-agent`
...
- **标签：** `creative-writing`, `novel-writing`, `rag`
- **推荐理由：** 从灵感到审稿的全流程AI小说创作工具
...
- **标签：** `creative-writing`, `novel`, `workflow`, `fiction`
- **推荐理由：** 内置小说、短剧、游戏脚本创作工作流。
...
- **标签：** `agent-observability`, `multimodal`, `ocr`, `video-understanding`
- **推荐理由：** 赋予AI视觉听觉能力，生成可验证的多模态证据
...
- **标签：** `task-management`, `taskboard`, `sqlite`, `agent-workflow`
- **推荐理由：** 本地原生任务看板，支持SQLite项目与智能体认领审核流程
...
- **推荐理由：** 完整逆向工程与授权渗透测试技能包，87篇SKILL.md
...
- **标签：** `skills`, `reasonix`
- **推荐理由：** Superpowers技能适配Reasonix编码工具链
...
- **推荐理由：** Superpowers TDD/调试/规划/协作技能适配DSH
...
- **⭐ 星标：** 77
- **语言：** JavaScript
- **描述：** 安装即自带mattpocock/skills v1.2.3的25个���程与效率技能，无需手动装技能。300亿token打造。本插件在原始技能之上提供10倍的开发效率。主力支持GitHub issue；Markdown为预览版；GitLab暂不支持。
- **标签：** `skills`, `agent`, `productivity`
- **推荐理由：** 预置25个开发效率技能，300亿token训练成果
...
- **标签：** `skills`, `web-search`, `research`
- **推荐理由：** 面向Agent的网页搜索、研究、金融集成技能包
...
- **推荐理由：** 统一管理加载本地Agent Skills的安全工具
...
- **标签：** `doc`, `office`, `chinese-docs`
- **推荐理由：** 符合国标的公文全流程处理，格式检查与修订
...
- **标签：** `ai-agent`, `bazi`, `iching`, `fengshui`
- **推荐理由：** 独特传统智慧Agent，本地确定性引擎+可视化面板
...
- **标签：** `quality-gate`, `delivery`, `engineering`
- **推荐理由：** 工程纪律 bundle，需求审查+测试+交付验证
...
- **标签：** `math-modeling`, `research`, `data-preprocessing`
- **推荐理由：** 完整数学建模套件，含数据处理/编程/论文写作
...
- **标签：** `multi-agent`, `team`, `memory`
- **推荐理由：** 多Agent编排，持久记忆/笔记/技能跨会话保留
...
- **推荐理由：** 多Agent协作，独立模型/技能/MCP工具+共享工作区
...
- **推荐理由：** 5角色多Agent数学建模，33个模型库+论文模板
...
- **标签：** `skill-picker`, `ui`, `productivity`
- **推荐理由：** 快速选择技能并插入斜杠命令，提升工作效率
...
- **推荐理由：** 从DSH适配的代码简化技能，适用于Codex工作流
...
- **推荐理由：** 跨端加密P2P远程访问DeepSeek Harness
...
- **标签：** `image-generation`, `prompt-engineering`, `skills`, `workflow-automation`
...
- **标签：** `design`, `ai-agents`, `ui`
- **推荐理由：** AI原生矢量设计工具，支持多Agent协作与代码即设计工作流。
...
- **标签：** `ui-components`, `charts`, `forms`, `rendering`
- **推荐理由：** 生成图表、表单等交互UI组件，内联渲染于回复中。
...
- **推荐理由：** 152张电影感镜头配方卡，配套Remotion模板
...
- **标签：** `visual`, `knowledge-cards`, `sharing`
- **推荐理由：** 将聊天历史转为可分享的知识卡片
...
- **推荐理由：** 自进化跨平台内容发现Agent，覆盖B站/YouTube/Reddit等多平台。
...
- **标签：** `web-search`, `free`, `no-signup`, `mcp`
- **推荐理由：** 免费无需注册的网页搜索插件，多模型通用。
...
- **标签：** `ios-simulator`, `mobile-automation`, `webdriveragent`, `ui-automation`
- **推荐理由：** 内置iOS模拟器，提供22个智能体工具进行移动端自动化测试
...
- **标签：** `resume-builder`, `mcp-server`, `self-hosted`, `dsh-plugin`
- **推荐理由：** 隐私优先的自助简历生成器，支持MCP
...
- **推荐理由：** 两阶段预设配置：极简引导后自动接入完整Standard工具集。
...
- **标签：** `multi-model`, `quota`, `routing`
- **推荐理由：** 统一管理多模型账号，支持智能路由与自动故障转移。
...
- **标签：** `oauth`, `llm-provider`, `chatgpt`, `claude`, `grok`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅用作DSH模型提供商
...
- **推荐理由：** 接入Command Code模型，支持计划感知选择和推理优化
...
- **标签：** `backup`, `config`, `mcp`
- **推荐理由：** 完整配置备份还原迁移工具，支持WebDAV同步
...
- **标签：** `container`, `version-management`, `infrastructure`
...
- **推荐理由：** 从Claude Code/Codex迁移会话、记忆和技能的向导
...
- **标签：** `launcher`, `orchestration`, `core-tools`
- **推荐理由：** 统一编排DSH运行时、插件和MCP的启动器
...
- **标签：** `animated-background`, `ocean-skin`, `glassmorphism`, `threejs`
- **推荐理由：** 精美的海洋动态主题，支持波浪和玻璃效果调节
...
- **标签：** `ai-resources`, `vibe-coding`, `prompt`, `rag`
- **推荐理由：** AI资源大全与Vibe Coding零基础教程
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU远程实验管理。
...
- **标签：** `encyclopedia`, `resources`, `learning`, `ai-basics`
- **推荐理由：** AI全维度资源百科：Agent、RAG、MLOps学习导航库。
...
- **标签：** `academic-search`, `arxiv`, `pubmed`, `citation-graph`, `mcp`
- **推荐理由：** 多源学术检索，支持引文核验和图谱，面向中文科研用户
...
- **标签：** `api`, `search`, `chinese-platforms`
- **推荐理由：** 一键搜索20个中国开放平台6.5万+篇API文档
...
- **标签：** `patent`, `legal`, `analysis`
- **推荐理由：** 专利侵权分析，输入公开号生成竞品报告
...
- **标签：** `tutorial`, `dsh-plugin`, `education`
...
- **推荐理由：** DSH终极中文指南，涵盖快速入门、资源推荐和插件工具集
...
- **标签：** `awesome-list`, `curated`, `plugins`
- **推荐理由：** 精选DeepSeek Harness插件合集与资源
...
- **标签：** `awesome-list`, `curated-list`, `dsh-plugins`, `plugin-registry`
- **推荐理由：** 经过滤的DSH插件注册表，含插件、捆绑包和技能。
...
- **标签：** `awesome-list`, `cli`, `web-ui`, `ai-agent`
- **推荐理由：** 涵盖CLI、Web UI和自主Agent的插件列表。
...
- **标签：** `awesome-list`, `ai-curated`, `recommendations`
- **推荐理由：** AI策展的190+插件列表每小时更新
...
- **标签：** `awesome-list`, `evidence-backed`, `radar`
- **推荐理由：** 有据可查的插件雷达与实验资源
...
- **标签：** `awesome-list`, `bilingual`, `automated`
- **推荐理由：** ��过GitHub Actions自动爬取的双语插件目录
...
- **推荐理由：** 汇总80余个经过验证的DSH插件目录，按功能分类整理。
...
- **标签：** `awesome-list`, `archived`, `plugin-directory`
- **推荐理由：** 原awesome列表已迁移，可作为历史DSH插件合集参考。
...
- **推荐理由：** 自动化双语插件列表，YAML驱动并支持Vercel部署更新。
...
- **标签：** `plugin-directory`, `curated-list`, `registry`, `typescript`
- **推荐理由：** 全面的插件注册表，覆盖模型、工具、沙箱、UI和Agent循环。
