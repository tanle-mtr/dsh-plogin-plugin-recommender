...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 197,737 | Core DSH platform powering the entire plugin ecosy |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,746 | Open-source design alternative with local-first de |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,466 | Multi-agent swarm framework with MCP server suppor |

---

## 分类导航

- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 29 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (29)
...
- [实用工具与基础设施](utilities-infrastructure-zh) (15)
- [主题与皮肤](themes-skins-zh) (6)
- [研究与分析](research-analysis-zh) (25)
- [其他](other-zh) (37)
---
...
- **标签：** `agentic-ai`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 多智能体蜂群框架，支持MCP服务器。
...
- **标签：** `coding-agent`, `tui`, `dsh-plugin`
- **推荐理由：** 终端原生AI编程代理，提示词缓存稳定高效。
...
- **标签：** `cordis`, `router`, `harness-tool`
- **推荐理由：** 任务感知推理路由预设，优化Agent工作流。
...
- **标签：** `multi-agent-workspace`, `enterprise`, `visual-editor`
- **推荐理由：** 企��级本地优先多引擎Agent工作台，含可视化编辑器。
...
- **标签：** `preset`, `standard-tools`, `bootstrap`
- **推荐理由：** 两阶段预设：最小引导后加载完整Standard工具集。
...
- **标签：** `claude-code`, `deepseek`, `model-switch`
- **推荐理由：** 一键切换多款AI编程工具
...
- **标签：** `coding-agents`, `architecture-driven-development`, `deepseek-harness`
- **推荐理由：** 架构感知编码代理，基线优先+漂移检查确保长任务安全
...
- **标签：** `vision-tools`, `computer-vision`, `deepseek-harness`
- **推荐理由：** 一行安装视觉工具箱，图片问答+OCR+UI还原赋能代理
...
- **标签：** `armor-breaking`, `deepseek`, `dsh-plugin`
- **推荐理由：** 破甲提示词稳定DeepSeek输出，提升效果。
...
- **标签：** `agent`, `desktop-app`, `multi-model`
- **推荐理由：** 本地优先多模型AI桌面应用，隐私安全可演化。
...
- **标签：** `security`, `redteam`, `dsh-plugin`
...
- **推荐理由：** 通过OAuth使用订阅账户作为DSH后端，无需API密钥。
...
- **推荐理由：** 跨平台CodePilot风格桌面客户端，插件生态丰富。
...
- **标签：** `context-migration`, `session-migration`, `preset-migration`, `cordis`
- **推荐理由：** 跨预设会话迁移工具，保留状态和模型意图
...
- **标签：** `context`, `deepseek-harness`, `dsh`, `tool`, `ui`
- **推荐理由：** 审计上下文注入，检测DSH重复指令冲突
...
- **推荐理由：** 构建验证DeepSeek Harness插件的实用技能
...
- **标签：** `agent-framework`, `database`, `mcp`
- **推荐理由：** AI代理资产包管理器，含5个CLI工具
...
- **标签：** `agent-skills`, `cordis`, `plugin-development`
- **推荐理由：** 便携技能开发审计DeepSeek插件
...
- **标签：** `migration`, `import`, `configuration`
- **推荐理由：** 一键迁移Claude Code/Codex配置到DSH。
...
- **标签：** `agent-tools`, `pre-flight`, `automation`
- **推荐理由：** 任务预检插件库并注入适用提示。
...
- **标签：** `desktop`, `dsh-plugin`, `ui-extension`
- **推荐理由：** 专为DSH生态打造的现代化桌面客户端。
...
- **标签：** `dsh-web`, `plugin-ecosystem`, `marketplace`
- **推荐理由：** 基于Web的插件聚合生态与创意工坊分发平台。
...
- **标签：** `sidebar`, `editor`, `terminal`
- **推荐理由：** 开源侧边栏底座含编辑器/终端/Git
...
- **标签：** `runtime`, `rust`, `desktop`
- **推荐理由：** 高性能Rust Agent运行时+桌面应用
...
- **标签：** `tauri`, `desktop`, `deepseek-harness`
- **推荐理由：** Tauri桌面版，5MB安装包零配置，预置插件开箱即用
...
- **标签：** `desktop-companion`, `plugin-sdk`, `deepseek-harness`
- **推荐理由：** 本地优先桌面伴侣，动态宠物+插件SDK扩展AI助手
...
- **标签：** `desktop-app`, `local-first`, `deepseek-harness`
- **推荐理由：** 本地优先桌面工作台，集成会话/文件/插件一��化管理
...
- **标签：** `ios`, `mobile-automation`, `ui-extension`
- **推荐理由：** 在DSH中嵌入iOS模拟器与真机控制，22个自动化工具。
...
- **标签：** `electron-app`, `desktop`, `ssh-client`
- **推荐理由：** 功能丰富的Electron桌面客户端，支持SSH和移动访问
...
- **标签：** `design`, `openpencil`, `ui-design`, `ppt`
- **推荐理由：** 在对话内直接预览和编辑OpenPencil设计文档
...
- **标签：** `task-management`, `taskboard`, `agent`, `cordis`
- **推荐理由：** SQLite本地任务看板，支持Agent认领审查和原生Web UI
...
- **推荐理由：** Electron桌面控制台：多Tab管理DSH/插件/MCP/技能
...
- **推荐理由：** 整合数据库、SSH、Docker与AI助手的全平台桌面工具。
...
- **推荐理由：** 本地优先、Markdown原生的跨应用自进化记忆层。
...
- **标签：** `memory-management`, `rag`, `self-evolving`, `token-savings`
- **推荐理由：** 超持久混合检索记忆，节省约35%Token。
...
- **标签：** `memory`, `milvus`, `persistent`
- **推荐理由：** 基于Milvus的持久化记忆层
...
- **标签：** `memory`, `rag`, `deepseek-harness`
- **推荐理由：** 无限记忆系统，为AI代理提供持久化知识存储
...
- **推荐理由：** 全维度AI百科知识库，涵盖Agent、RAG、MCP等资源。
...
- **标签：** `agent-memory`, `skill-reranking`, `vector-search`
...
- **标签：** `agent-memory`, `long-term-memory`, `mcp`, `noema`
- **推荐理由：** 持久化长期记忆，支持检索工具和设置页面审查
...
- **标签：** `agent-evaluation`, `rag-evaluation`, `knowledge-engineering`
- **推荐理由：** 基于证据的提示词、RAG和Agent评估
...
- **标签：** `cognitive`, `agent-skills`, `workspace`
- **推荐理由：** 基于全局工作空间研究的AI增强技能
...
- **标签：** `agent-teams`, `multi-agent`, `deepseek-harness`
- **推荐理由：** 多代理团队协作插件，扩展DSH多智能体能力
...
- **标签：** `workflow`, `prompt-management`, `browser-extension`
- **推荐理由：** AI对话转可复用工作流，跨平台支持主流AI系统
...
- **标签：** `agent`, `workflow`, `teamwork`
- **推荐理由：** 将多个编程Agent整合为协同团队，跨会话调度。
...
- **标签：** `search`, `web-fetch`, `agent-skills`
- **推荐理由：** 免费无注册网页搜索桥接，增强任意模型联网能力。
...
- **标签：** `agent-skills`, `math-modeling`, `dsh-plugin`
- **推荐理由：** 全栈自动化数学建模竞赛Skill，支持GitOps
...
- **标签：** `skills`, `agent`, `multi-platform`
- **推荐理由：** 将经验转化为可复用执行，支持多Agent平台
...
- **标签：** `agent-review`, `plan-mode`, `prompt-engineering`, `self-evolving`
- **推荐理由：** 组合式任务控制，支持Agent自我审查和持续改进
...
- **推荐理由：** 将superpowers技能移植到Reasonix编码框架
...
- **推荐理由：** AI编程导师技能：架构分析、阶梯式学习、自动判题与Code Review
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`, `claude-code`, `deep-research`, `deepseek`, `dsh-plugin`, `research-tools`
- **推荐理由：** 88个跨平台研究营销技能，覆盖社交媒体与商业工作流
...
- **标签：** `agent`, `agent-skill`, `ai`, `deepseek-harness`, `dsh-plugin`, `skills`
- **推荐理由：** 开发DSH插件的Agent技能，标准化插件开发流程
...
- **标签：** `agent-skills`, `ai-coding`, `deepseek-harness`, `dsh-plugin`, `spec-driven-development`, `spec-programming`
- **推荐理由：** 可移植规格驱动开发技能，遵循DSH工程模式
...
- **⭐ 星标：** 18
- **语言：** JavaScript
- **描述：** 拨开迷雾看见终点，剩下的���给任务栏。Part the fog, see the end — the task bar handles the rest. 🎮 mattpocock/skills 的 DSH 游戏任务系统：map 拨迷雾，任务栏推进一步。A game-like mission system for Matt Pocock skills in DeepSeek Harness. More by @FeatherHunter: 🎨 dsh-opencode-palette · ⚡ dsh-prompt
- **标签：** `agent`, `deepseek-harness`, `skills`, `wayfinder`
- **推荐理由：** 游戏化任务系统，驱动工作流执行
...
- **标签：** `ai-docs`, `deepseek-harness`, `docx`, `gongwen`
- **推荐理由：** 中文公文全流程处理，符合国家标准
...
- **标签：** `canvas`, `skills`, `agentic-workflow`
- **推荐理由：** 无限画布支持创意工作流与技能管理。
...
- **标签：** `engineering`, `discipline`, `agent-skill`
- **推荐理由：** 可移植技能：工程纪律与诚实报告规范。
...
- **标签：** `marketplace`, `dsh-plugin-bundle`, `dsh-plugins`
- **推荐理由：** 类CodeX插件市场，批量管理与安装技能。
...
- **标签：** `deepseek-harness`, `dsh`, `mcp`
- **推荐理由：** 在DSH Web界面管理技能与MCP服务器
...
- **标签：** `deepseek-harness`, `mcp`, `zhihu`
- **推荐理由：** 知乎搜索与开放平台API的MCP工具
...
- **推荐理由：** 通过OAuth在DSH内使用ChatGPT订阅服务。
...
- **标签：** `ai-design`, `design-systems`, `dsh-plugin`
...
- **标签：** `agent-skills`, `diagrams-as-code`, `mermaid-alternative`
- **推荐理由：** 文本转架构图、序列图等精美可验证可视化图表。
...
- **标签：** `design-tool`, `vector`, `vibedesign`, `mcp`
- **推荐理由：** 开源AI原生矢量设计工具，支持Agent团队并发设计。
...
- **标签：** `design-studio`, `visual-editor`, `ppt`
- **推荐理由：** 原生设计工作室，AI生成+可视化编辑+PPT支持
...
- **标签：** `ui`, `visualization`, `dsh-plugin`
- **推荐理由：** 在对话中内联渲染图表、表单等交互UI组件。
...
- **推荐理由：** 直接在DeepSeek Harness聊天中生成图像
...
- **标签：** `agent-skills`, `claude-code`, `diagram`, `deepseek-harness`, `dsh-plugin`, `legal`, `legal-tech`, `litigation`, `svg`, `visualization`
- **推荐理由：** 法律诉讼可视化工具：生成时间轴与案件图表
...
- **标签：** `deepseek-harness`, `design-skills`, `ui`, `vibe-coding`
- **推荐理由：** 避免AI审美同质化，提升网站设计质感
...
- **标签：** `ai-director`, `ai-image`, `video-editing`
- **推荐理由：** AI视频图像音频生成，支持可配置模型
...
- **标签：** `visualization`, `cost-tracking`, `token-usage`
- **推荐理由：** 实时Token用量图表与API余额看板。
...
- **标签：** `browser-use`, `agent`, `deepseek-harness`
- **推荐理由：** AI代理浏览器自动化CLI+扩展，无缝整合真实登录会话
...
- **标签：** `deepseek-harness`, `dsh`
- **推荐理由：** 内测收官合影墙，支持OAuth登录
...
- **标签：** `code-review`, `lint`, `engineering`
- **推荐理由：** 基于12本经典工程的AI代码审查
...
- **标签：** `context-management`, `dashboard`, `deepseek-harness`
- **推荐理由：** 上下文仪表盘与统计，深度管理DSH对话上下文
...
- **标签：** `cost-tracking`, `token-usage`, `dsh-plugin`
- **推荐理由：** 追踪DSH会话/每日费用、预算和token用量
...
- **标签：** `backup`, `crash-recovery`, `rollback`, `snapshot`, `windows`
- **推荐理由：** 崩溃恢复插件，支持撤销配置和快照，DSH宕机也可用
...
- **标签：** `mascot`, `pixel-art`, `sprites`, `cli`
- **推荐理由：** 面向多平台的代码Agent动画宠物展示画廊。
...
- **标签：** `theme`, `threejs`, `chrome-extension`
- **推荐理由：** 基于WebGPU的海洋主题DeepSeek Harness皮肤
...
- **标签：** `color-scheme`, `dark-mode`, `skins`, `themes`
- **推荐理由：** 34款经典配色一键更换DSH主题皮肤
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重��后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `themes`, `backgrounds`, `webui`
- **推荐理由：** 支持自定义背景或一键切换氛围预设。
...
- **标签：** `content-discovery`, `bilibili`, `xiaohongshu`
- **推荐理由：** 跨平台AI内容发现Agent
...
- **推荐理由：** DeepSeek Harness终极指南，含入门和精选插件
...
- **标签：** `finance`, `quant`, `stock`
- **推荐理由：** 精选24个股票/金融/量化插件合集。
...
- **标签：** `portal`, `static-site`, `discovery`
- **推荐理由：** 零依赖静态门户，发现227+插件。
...
- **推荐理由：** 证据驱动的Bilingual插件目录，适合研究参考。
...
- **标签：** `awesome-list`, `catalog`, `dsh-plugin`
- **推荐理由：** GitHub Actions每日自动爬取，插件目录更新及时。
...
- **标签：** `awesome-list`, `directory`, `bilingual`
- **推荐理由：** 中英双语精选DSH插件与工具资源目录。
...
- **标签：** `awesome-list`, `discovery`, `curated`
- **推荐理由：** 每日自动精选分类，帮你快速找到适合的DSH插件。
...
- **标签：** `plugin-marketplace`, `dsh-plugin`
- **推荐理由：** DeepSeek Harness插件市场与合集
...
- **标签：** `anime`, `bangumi`, `magnet`, `deepseek-harness`
- **推荐理由：** 对话内多源搜番，卡片展示评分并支持磁力链接
...
- **标签：** `notes`, `smartisan`, `docker`, `dsh-plugin`
- **推荐理由：** 开源锤子便签复刻版，支持Docker部署和便签到图片导出
...
- **标签：** `marketplace`, `plugin-market`, `agent`, `nodejs`
- **推荐理由：** 在DSH Web GUI中一键浏览安装更新GitHub插件
...
- **标签：** `awesome`, `awesome-list`, `plugin`
- **推荐理由：** 深度挖掘整理的DSH插件精华目录列表
...
- **标签：** `plugin-market`, `ui`, `web`, `agent-harness`
- **推荐理由：** 在Web GUI内浏览社区插件市场并一键安装卸载
