...
- **推荐理由：** 终端原生DeepSeek编码Agent，前缀缓存稳定可靠
...
- **标签：** `harness`, `tutorial`, `agent`
- **推荐理由：** 从0到1的Harness工程入门教程
...
- **标签：** `agent-runtime`, `ai-coding`, `desktop`
- **推荐理由：** Rust高性能代理运行时，集成桌面应用与代���代理能力。
...
- **标签：** `agent-teams`, `coding-agent`, `dsh`
- **推荐理由：** 在DSH中实现多智能体团队协作，适合复杂任务场景。
...
- **标签：** `modeling`, `competition`, `gitops`
- **推荐理由：** 全栈自动数学建模Agent，含GitOps流水线与代码自证
...
- **标签：** `evolution`, `review`, `agent`
- **推荐理由：** 实现可组合、可审查、自进化的Agent工作流
...
- **标签：** `infra`, `development`, `skills`
- **推荐理由：** AI基础设施开发、优化与调试专用技能集
...
- **标签：** `plugin-creator`, `validation`, `harness`
- **推荐理由：** 构建和验证DSH插件的必备工具包。
...
- **标签：** `code-simplification`, `skill`, `refactoring`
- **推荐理由：** 借鉴官方DSH技能的代码简化模式优化代码。
...
- **标签：** `skill-picker`, `ui`, `workflow`
- **推荐理由：** WorkBuddy风格技能选择器，简化composer中的技能选择。
...
- **标签：** `codex`, `sync`, `integration`
- **推荐理由：** Codex与DSH双向同步桥接，支持技能挂载和MCP镜像。
...
- **标签：** `migration`, `import`, `tool`
- **推荐理由：** 一键迁移Claude Code/Codex/OpenCode会话、记忆和技能到DSH。
...
- **标签：** `plugin-dev`, `skill`, `developer-tools`
- **推荐理由：** DSH插件开发Agent技能，提供插件/服务/事件/工具标准规范。
...
- **标签：** `chatgpt`, `oauth`
- **推荐理由：** OAuth绑定ChatGPT，在DSH中使用其模型
...
- **标签：** `desktop`, `tauri`, `dsh`
- **推荐理由：** 轻量Tauri桌面版，零环境配置，预装插件，跨平台支持。
...
- **推荐理由：** 将电子表格、文档、演示文稿等完整办公套件带入Harness。
...
- **标签：** `task-management`, `ui-extension`, `native`
- **推荐理由：** 原生本地任务看板，SQLite存储，支持Agent任务认领与审查。
...
- **推荐理由：** 原生iOS客户端，将工作区、会话和Agent轨迹带到iPhone。
...
- **标签：** `desktop-app`, `ui-extension`, `electron`
- **推荐理由：** CodePilot风格的跨平台桌面客户端，支持macOS/Windows/Linux。
...
- **标签：** `notes`, `ui`, `smartisan`
- **推荐理由：** 锤子便签开源版，支持Docker部署与微信导出
...
- **标签：** `mcp`, `skills`, `webui`, `plugin`
- **推荐理由：** Web界面管理Skill和MCP，操作直观便捷。
...
- **标签：** `plugin-market`, `webui`, `ui`
- **推荐理由：** 在Web GUI中浏览安装社区插件，一键操作。
...
- **标签：** `mcp`, `capabilities`, `menu`, `ui`
- **推荐理由：** 统一管理MCP工具和技能的暴露级别菜单。
...
- **推荐理由：** Electron桌面控制台，多Tab管理插件/MCP/Skill
...
- **标签：** `ui`, `info-bar`, `balance`, `pricing`
- **推荐理由：** 实时显示余额、定价和模型状态的清晰信息栏。
...
- **标签：** `bioinformatics`, `desktop-app`, `tauri`, `agent`
- **推荐理由：** 面向临床和实验室的生物信息学桌面智能体。
...
- **标签：** `desktop`, `devops`, `ssh`, `docker`, `database`
- **推荐理由：** 一站式DevOps桌面：数据库、SSH、Docker、AI一体化。
...
- **推荐理由：** Electron桌面壳，集成DSH Web服务与工具。
...
- **标签：** `web-portal`, `plugin-browser`
- **推荐理由：** 静态网页门户，浏览筛选227+插件
...
- **推荐理由：** 将文档转化为可查询RAG、推理Agent和自维护维基
...
- **推荐理由：** 自进化记忆OS，节省35%token并支持混合检索
...
- **标签：** `memory`, `continual-learning`, `rag`
- **推荐理由：** 构建状态化持续学习Agent的记忆库
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** 本地优先的记忆中枢，为所有AI代理提供共享持久上下文。
...
- **标签：** `agent-orchestration`, `skills`, `workspaces`
...
- **标签：** `control-plane`, `long-horizon`, `workflow`
- **推荐理由：** 跨Harness长周期Agent控制平面，耐用且可治理
...
- **标签：** `video`, `agent-skill`, `ffmpeg`
- **推荐理由：** 一键生成Vox风格解说视频，从主题到成片全自动。
...
- **推荐理由：** 本地优先的AI小说创作软件，支持世界观、章节与修稿。
...
- **推荐理由：** 为纯文本模型打造的视觉工具箱，支持图像问答与OCR。
...
- **标签：** `agent-skills`, `multimodal`, `video-analysis`
- **推荐理由：** 为Agent提供可验证的多模态感知能力，视频音频屏幕均可作为证据。
...
- **标签：** `agent-skills`, `creative-writing`, `drama-skills`
- **推荐理由：** 内置小说、短剧、游戏、视频解说四大创意写作工作流。
...
- **标签：** `skills`, `framework`, `reusable`
- **推荐理由：** 可复用技能框架，将经验转化为可重复执行流程
...
- **标签：** `plugin`, `discovery`, `installation`
- **推荐理由：** 专为DSH插件发现、安装与验证设计的技能
...
- **标签：** `vision`, `multimodal`, `coding`
- **推荐理由：** 为纯文本LLM赋予多模态视觉能力
...
- **标签：** `agent-skills`, `skill-generator`, `automation`
...
- **推荐理由：** 源自obra/superpowers的TDD与调试技能包。
...
- **推荐理由：** AI编程助手变导师，阶梯式教学+自动判题+Code Review
...
- **标签：** `spec-driven`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 仓库级Spec驱动开发技能，支持AI编程规范流程
...
- **标签：** `chinese-traditional-wisdom`, `dashboard`, `dsh-plugin`
- **推荐理由：** 八字紫微风水玄学技能包，本地确定引擎+可视化面板
...
- **推荐理由：** DSH官方插件开发知识库，按需调用的Agent Skill
...
- **标签：** `quality-gate`, `requirements`, `engineering`
- **推荐理由：** 交付前的质量门禁，确保需求经过严格验证。
...
- **标签：** `qa`, `testing`, `agent-skills`, `playwright`
- **推荐理由：** 含11个技能和知识库的专业QA智能体框架。
...
- **标签：** `skillhub`, `official`, `api`, `tencent`
- **推荐理由：** 腾讯官方SkillHub插件，含Open API文档和技能示例。
...
- **推荐理由：** 152张镜头配方卡+Remotion模板，打造电影感产品视频的智能技能包。
...
- **标签：** `math`, `modeling`, `multi-agent`
- **推荐理由：** 五角色预设+33模型库，数学建模竞赛全栈多智能体解决方案。
...
- **⭐ 星标：** 25
- **语言：** TypeScript
- **描述：** Give DeepSeek Harness a persistent agent team: durable Member identities with private memory, notes and skills across sessions. You set the direction — agents coordinate through Channels and Tasks, and manage their own context.
- **标签：** `agent-team`, `memory`, `multi-agent`
- **推荐理由：** 跨会话持久化智能体团队，支持私有记忆与技能共享。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，运行时自动注册到DSH。
...
- **标签：** `diagram-as-code`, `architecture-diagram`, `dsh-plugin`
- **推荐理由：** 从代码生成精美架构图和流程图，自包含HTML输出
...
- **推荐理由：** 法律可视化：时间轴、庭审对抗图、法律关系图一键生成
...
- **推荐理由：** 通过DirectorX实现视频、图像和音频的AI创意套件。
...
- **标签：** `plugin-registry`, `ecosystem`, `infrastructure`
- **推荐理由：** 自部署生态雷达，自动发现并验证15900+候选插件。
...
- **推荐理由：** 将ChatGPT/Claude/Grok订阅转为LLM提供商，无需API密钥。
...
- **推荐理由：** 跨多提供商追踪会话/每日成本、预算和Token用量。
...
- **标签：** `provider`, `llm-provider`, `utility`
- **推荐理由：** 新增Command Code LLM提供商，支持实时目录和智能模型选择。
...
- **标签：** `backup`, `config-manager`, `webdav`, `migration`
...
- **标签：** `skills`, `management`, `dsh-plugin`
- **推荐理由：** DSH中统一管理加载本机Agent Skills的安全工具
...
- **标签：** `containers`, `management`, `utility`, `infrastructure`
- **推荐理由：** 基于容器的隔离DSH管理，内置WebView界面。
...
- **⭐ 星标：** 23
- **语言：** Rust
- **描述：** 释放 DeepSeek Harness 插件化内核能力，将 Runtime、Provider、Plugin、Skill、MCP 等能力统一编排，让 Harness 真正成为可扩展、可组合、可管理的本地 AI 运行平台。
- **标签：** `launcher`, `infrastructure`, `orchestration`
- **推荐理由：** 统一编排Runtime、Provider、Plugin、Skill、MCP能力。
...
- **标签：** `token-usage`, `cost-tracking`, `visualization`
- **推荐理由：** 实时Token用量、费用图表及API余额追踪工具。
...
- **标签：** `doctor`, `conflict-resolution`, `compatibility`
- **推荐理由：** 解决插件间及版本兼容冲突问题。
...
- **标签：** `skin`, `theme`, `ui`
- **推荐理由：** 鲸鱼娘系列主题皮肤，个性化定制DeepSeek Harness界面。
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或���键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `theme`, `background`, `customization`
- **推荐理由：** 自定义背景图+预设氛围，实时预览所见即所得。
...
- **推荐理由：** 自动搜索中国八大主流平台30天内容，生成有据可查报告。
...
- **推荐理由：** 覆盖CLI、Web UI和代理工具的DSH插件生态精选列表。
...
- **标签：** `academic`, `search`, `citation`
- **推荐理由：** 多源学术检索，支持引用核验与图谱，中文友好
...
- **标签：** `industry-research`, `report`, `company-research`
- **推荐理由：** 产业链地图与资讯追踪，适合行业调研。
...
- **标签：** `awesome-list`, `agents`, `plugins`
- **推荐理由：** 精选DSH工具列表，覆盖生产力与功能扩展。
...
- **⭐ 星标：** 87
- **语言：** Python
- **描述：** 🐳 DeepSeek Harness 插件聚合社区 — 自动同步 dsh-plugin 生态 · 精选目录 · 每 4 小时自���维护 | Oh-My-DSH: a community-maintained catalog of DeepSeek Harness plugins, auto-synced from the dsh-plugin topic
- **标签：** `plugin-marketplace`, `awesome-list`, `ecosystem`
- **推荐理由：** 社区自动同步维护的DSH插件聚合目录。
...
- **推荐理由：** 聚合6.5万+中国平台API文档，零配置Skill搜索
...
- **标签：** `patent`, `legal-tech`, `dsh-plugin`
...
- **标签：** `kline`, `analysis`, `trading`
- **推荐理由：** 多市场行情分析，技术指标与支撑压力位可视化，原生侧栏集成。
...
- **推荐理由：** 基于证据的评估框架，支持提示词、RAG、技能与Agent评测。
...
- **标签：** `stock`, `finance`, `quant`, `trading`
- **推荐理由：** 24个经验证的 stocks/quant插件，中英双语。
...
- **标签：** `plugin-directory`, `guide`
- **推荐理由：** 插件导航指南，场景化选型与风险评估
...
- **标签：** `awesome-list`, `deepseek-harness`, `ai-agents`
- **推荐理由：** 基于证据的双语插件目录，可审计的提交记录
...
- **推荐理由：** The World's First Virtual Terminal for AI Agents
...
- **标签：** `agent`, `ai-agents`, `dsh-plugin`, `memory`, `memoryscope`, `openclaw`, `openclaw-plugin`, `qwenpaw`, `rag`, `reme`
- **推荐理由：** ReMe: Memory Management Kit for Agents - Remember Me, Refine Me.
...
- **推荐理由：** Intelligent Skill routing and workflow orchestration for AI agents — +21.12 pp r
...
- **推荐理由：** One-click install + model switch:Claude Code,Codex CLI (OpenAI), Grok Build (xAI
...
- **推荐理由：** Turn code into clarity. Generate accurate technical docs and AI-ready context in
...
- **标签：** `cordis`, `deepseek`, `deepseek-harness`, `developer-tools`, `dsh`, `dsh-plugin`, `dsh-plugins`, `floating-widget`, `plugin`
- **推荐理由：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动
...
- **标签：** `agent`, `deepseek`, `deepseek-harness`, `deepseek-harness-plugin`, `dsh`, `dsh-plugin`, `harness`
- **推荐理由：** The most comprehensive DeepSeek Harness plugin market — refreshed daily, sourced
...
- **推荐理由：** DeepSeek Harness Desktop App: a local AI desktop workspace for DSH Sessions, pro
...
- **推荐理由：** Mimir — 一站式科研工作台插件：LaTeX 论文边写边编译、arXiv 文献管理、实验追踪、指标图表、GPU 服务器 SSH 任务编排，管理科研全周期。A
...
- **标签：** `ai`, `ai-agent`, `claude`, `claude-code`, `cli`, `codex`, `codex-cli`, `coding-agent`, `deepseek`, `deepseek-harness`, `dsh-plugin`, `go`, `oh-my-pi`, `omp`, `openai`, `pi`, `terminal`, `tui`
- **推荐理由：** a coding agent, rpc plugin, sub-agents, hashline edits, and mcp
...
- **标签：** `animated-background`, `chrome-extension`, `customization`, `deepseek`, `deepseek-harness`, `deepseek-theme`, `dsh`, `dsh-plugin`, `edge-extension`, `glassmorphism`, `ocean`, `ocean-skin`, `skin`, `theme`, `threejs`, `webgpu`
- **推荐理由：** 🌊 DeepSeek Harness 海洋皮肤与动态主题 | Real-time ocean theme with adjustable waves, sun
...
- **标签：** `awesome-list`, `discovery`, `plugin-manager`
- **推荐理由：** 每日自动抓取并复核插件，按场景分类推荐，助你快速选型。
...
- **推荐理由：** DSH终极指南，涵盖入门、资源、精选插件和实用工具集。
...
- **标签：** `learning`, `reference`
- **推荐理由：** 全面的DSH学习资源与最佳实践指南
...
- **标签：** `handbook`, `documentation`
- **推荐理由：** 权威手册，覆盖运行时、插件、MCP与故障排查
...
- **标签：** `catalog`, `directory`
- **推荐理由：** 综合插件目录，支持搜索、分类筛选与排序
...
- **推荐理由：** 精选DeepSeek Harness插件列表，分类浏览发现优质插件。
...
- **推荐理由：** DSH生态全面资源列表，涵盖插件、工具、技能与资源。
...
- **标签：** `awesome-list`, `curated`, `permissions`
- **推荐理由：** 证据导向的插件目录，含权限安装指导。
