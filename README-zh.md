...
- **标签：** `agent-os`, `evaluation`, `self-improving`
- **推荐理由：** 自进化Agent系统，面试门禁式评估与预算演化循环
...
- **标签：** `runtime`, `rust`, `coding-agent`
- **推荐理由：** 高性能Rust Agent运行时，搭配精美桌面IDE编码体验
...
- **标签：** `code-review`, `clean-architecture`, `tech-debt`
...
- **推荐理由：** 将Superpowers技能移植到Reasonix编码夹具
...
- **标签：** `mentor`, `coding`, `skills`
- **推荐理由：** AI编程导师：扫描架构、生成阶梯章节、AI代码审查
...
- **推荐理由：** 指导Agent按标准开发DSH插件，覆盖插槽、服务与打包。
...
- **标签：** `plugin-dev`, `dsh-plugin`, `agent-skill`
- **推荐理由：** Agent构建与管理DSH插件的技能，支持插槽与凭据配置。
...
- **推荐理由：** 可复用Codex技能，从零脚手架构建与验证DSH/Cordis插件。
...
- **标签：** `knowledge-base`, `plugin-dev`, `guide`
- **推荐理由：** 官方DSH插件开发知识库，按需调用，含中英文档与Cordis指南。
...
- **标签：** `dsh-plugin`, `container`, `deepseek-harness`
- **推荐理由：** 本地隔离多实例管理DSH，一键导入插件技能
...
- **推荐理由：** 开放式侧边栏底座，内置文件编辑/终端/对话/Git/子代理页
...
- **推荐理由：** 现代化灵活任务面板，支持嵌入Codex/DeepSeek Harness
...
- **标签：** `TUI`, `terminal`, `UI`
- **推荐理由：** 鲸鱼顶栏/流式思考/上下文进度，类Claude Code终端体验
...
- **标签：** `deepseek`, `floating-widget`, `dsh-plugin`
- **推荐理由：** DSH右下角小鲸鱼娘，实时显示账户余额
...
- **标签：** `desktop-app`, `electron`, `local-first`, `office-automation`
- **推荐理由：** 本地优先的Electron桌面工作区，整合DSH全功能
...
- **标签：** `desktop`, `tauri`, `multi-model`, `privacy`
- **推荐理由：** 本地优先多模型桌面应用，Claude Cowork替代方案
...
- **标签：** `desktop`, `electron`, `ssh-client`, `gui`
- **推荐理由：** Windows桌面客户端，含SSH和移动端远程访问
...
- **标签：** `ios`, `ios-app`, `dsh-plugin`
- **推荐理由：** 原生 iOS 客户端，WebSocket 同步远程工作区与 Agent 执行。
...
- **推荐理由：** Web UI 插件，可视化技能与 MCP 工具管理。
...
- **标签：** `ui`, `marketplace`, `web-gui`
- **推荐理由：** Web GUI插件市场，一键安装卸载插件
...
- **标签：** `ui`, `button`, `webui`, `utility`
- **推荐理由：** DSH Web上下文仪表面板一键折叠按钮
...
- **标签：** `memory`, `rag`, `portable`
- **推荐理由：** 可移植的自进化记忆层，Markdown原生跨Agent持久化
...
- **标签：** `memory`, `token-optimization`, `rag`
- **推荐理由：** 混合检索记忆系统，节省35%Token消耗
...
- **标签：** `memory`, `stateful`, `local-first`
- **推荐理由：** 本地优先的记忆库，支持状态化Agent持续学习
...
- **推荐理由：** 持久统一记忆层，基于Markdown与Milvus语义搜索
...
- **标签：** `embeddings`, `semantic-search`, `skill-routing`
- **推荐理由：** 将SKILL.md转为可检索的向量知识库
...
- **标签：** `finance`, `rag`
- **推荐理由：** 基于tushare的财经数据源，支持多Agent分析
...
- **标签：** `workspace`, `orchestration`, `multi-engine`
- **推荐理由：** 统一Agent工作区，支持多引擎编排和跨设备访问
...
- **标签：** `skill-store`, `automation`, `coding`
- **推荐理由：** 最全技能商店，自动抓取上万个多领域技能项目
...
- **标签：** `control-plane`, `long-horizon`, `orchestration`
- **推荐理由：** 长周期Agent控制面，支持多引擎持久化受控工作流
...
- **标签：** `preset`, `workflow`, `standard`
- **推荐理由：** 两阶段预设：从极简引导到完整标准工具，稳定高效
...
- **标签：** `skills`, `routing`, `orchestration`
- **推荐理由：** 智能技能路由，基准测试奖励+21pp，Token节省近30%
...
- **推荐理由：** 基于Anthropic J-space全局工作区研究的认知增强技能
...
- **标签：** `agentteams`, `deepseekharness`, `dsh-plugin`
- **推荐理由：** 支持多Agent团队协作，增强DSH工作流能力
...
- **推荐理由：** SQLite本地任务看板，支持Agent认领与审查
...
- **标签：** `skill`, `competition`
- **推荐理由：** 全自动数学建模竞赛技能，支持GitOps流水���
...
- **标签：** `agent-review`, `self-evolving`
- **推荐理由：** 可组合任务控制，实现Agent自我进化
...
- **标签：** `skills`, `agent`, `efficiency`, `github-issues`
- **推荐理由：** 内置25个工程效率技能，开箱即用，覆盖任务分流与交接。
...
- **标签：** `spec-driven`, `agent-skills`, `coding`
- **推荐理由：** 仓库级规格驱动开发技能，融合DSH工程模式与spec编程。
...
- **标签：** `fengshui`, `bazi`, `iching`, `tcm`
- **推荐理由：** 八字/风水/易经/中医融合技能包，本地引擎+可视化看板。
...
- **标签：** `multi-agent`, `skills`, `workflow`
- **推荐理由：** 多智能体团队协作，独立模型与共享工作区
...
- **标签：** `session-memory`, `handoff`, `cross-platform`
- **推荐理由：** 跨平台会话记忆技能，保持目标连续性
...
- **标签：** `package`, `skills`, `kubernetes`
- **推荐理由：** 42技能+5CLI覆盖工程、数据库和K8s工作流
...
- **标签：** `ai-director`, `ai-video`, `ai-image`, `dsh-plugin`
- **推荐理由：** AI视频/图像/音频技能，支持可配置视觉与生成工具
...
- **推荐理由：** 开源Figma替代方案，支持本地优先设计与AI生成
...
- **⭐ 星标：** 3,870
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外挂视觉插件，为 DeepSeek、GLM 等纯文本模型外挂视觉能力，粘贴���片即得结构化 JSON 证据（OCR、版面、语义）。
- **标签：** `vision`, `ocr`, `multimodal`
- **推荐理由：** DSH首个视觉插件，支持OCR与多模态图像转结构化JSON
...
- **标签：** `video-generation`, `ffmpeg`, `tts`, `generative-ai`
- **推荐理由：** 一键生成Vox风格纸拼贴解说视频，自动化全流程
...
- **标签：** `computer-vision`, `ocr`, `vision-language-model`
- **推荐理由：** 为纯文本模型设计的视觉工具箱：OCR、多图问答
...
- **标签：** `ai-design`, `visual-editor`, `ppt`, `prototyping`
- **推荐理由：** 原生设计工作室，支持AI生成、可视化编辑与PPT
...
- **⭐ 星标：** 53
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法��人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，或直接读案件材料画准一张时间轴。Claude Skill / DeepSeek Harness 通用。
- **标签：** `visualization`, `legal`, `diagram`, `timeline`
- **推荐理由：** 将复杂诉讼材料转为清晰时间轴与可视化图表，法律利器。
...
- **推荐理由：** 跨平台内容发现Agent，覆盖B站/抖音/YouTube等主流平台
...
- **标签：** `ios`, `mobile-automation`, `simulator`, `webdriveragent`
- **推荐理由：** iOS模拟器与真机控制，22个Agent工具
...
- **标签：** `low-code`, `self-hosted`, `dsh-plugin`
...
- **标签：** `web`, `plugin-marketplace`, `infrastructure`
- **推荐理由：** 基于Web的插件聚合生态，分布式插件市场
...
- **标签：** `installer`, `model-switch`, `utility`
- **推荐理由：** 一键安装多Agent工具并支持Claude/Codex/DeepSeek无缝切换
...
- **标签：** `cordis-plugin`, `dsh-external`, `context-management`
- **推荐理由：** 全面管理DSH上下文洞察、仪表盘和统计信息
...
- **标签：** `awesome-list`, `mcp`, `plugin-ecosystem`
- **推荐理由：** DSH插件、工具、MCP服务器的精选目录
...
- **推荐理由：** OAuth桥接，用订阅账号替代API Key调用大模型
...
- **⭐ 星标：** 166
- **语言：** TypeScript
- **描述：** 一个基于 DeepSeek Harness 插件机制构建的多端远���访问方案，通过安全、低延迟、端到端加密的 P2P 优先网络，支持从 PC、Android 和 Web 随时访问并操作远程 Harness。 (A multi-device remote access solution built on the DeepSeek Harness plugin system, enabling PC, Android, and Web clients to securely access and operate a remote Harness over a low-latency, end-to-end encrypted, P2P-first network.)
- **标签：** `deepseek`, `dsh-plugin`, `remote-access`
- **推荐理由：** 端到端加密 P2P 远程访问，支持 PC/Android/Web 多端操控。
...
- **标签：** `registry`, `infrastructure`, `plugins`
- **推荐理由：** 插件生态基建：注册表控制台与开发引导Skill
...
- **标签：** `gongwen`, `docx`, `wps-office`, `chinese`
- **推荐理由：** 中文公文全流程处理，格式校验、内容优化与模板生成。
...
- **标签：** `skills-manager`, `dsh`, `utility`
- **推荐理由：** DSH中统一管理加载本机Agent Skills的安全管理工具。
...
- **标签：** `desktop`, `devops`, `ssh`, `docker`, `sftp`
- **推荐理由：** 一站式DevOps桌面：数据库、SSH、SFTP、Docker和AI沙箱
...
- **标签：** `token-usage`, `data-visualization`, `ai-cost`, `dsh-plugin`
- **推荐理由：** 实时Token用量追踪，含费用估算和每轮图表
...
- **标签：** `utility`, `debug`, `compatibility`
- **推荐理由：** 解决插件冲突和版本兼容问题
...
- **标签：** `mascot`, `animation`, `decor`
- **推荐理由：** 动画宠物画廊，为多款AI编程工具提供可爱装饰皮肤
...
- **标签：** `theme`, `glass-effect`, `ui-customization`
- **推荐理由：** 磨砂玻璃主题，高度可定制透明度和背景
...
- **标签：** `theme`, `skins`, `dark-mode`, `color-scheme`
...
- **标签：** `ai-resource`, `tutorial`, `prompt`
- **推荐理由：** AI资源大全，覆盖主流模型与Prompt技巧教程
...
- **标签：** `tutorial`, `harness`, `beginner`
- **推荐理由：** 从零开始学Harness工程的入门指南
...
- **标签：** `research`, `web-search`, `tiktok`, `replaybook`
...
- **标签：** `knowledge-base`, `mcp`, `mlops`, `research`
- **推荐理由：** 全维度AI资源百科，覆盖Agent/RAG/MLOps等
...
- **推荐理由：** 88 个开源技能，覆盖深度研究、社交情报和营销自动化。
...
- **标签：** `api-docs`, `research`, `chinese-api`
...
- **标签：** `patent`, `rag`, `multi-agent`, `legal-tech`
- **推荐理由：** 多智能体专利侵权分析，支持RAG检索与竞品报告自动生成。
...
- **推荐理由：** 行业研究方法论包，含产业链图谱、政策追踪与公司简介技能。
...
- **标签：** `kline`, `trading`, `visualization`
- **推荐理由：** 交互式K线图，支持多市场和技术分析
...
- **标签：** `evaluation`, `rag-evaluation`, `prompt-engineering`, `llm-evaluation`
...
- **标签：** `awesome-list`, `directory`, `guidance`
- **推荐理由：** 证据驱动的插件目录，含权限安装指导
...
- **标签：** `awesome-list`, `chinese`, `collection`
- **推荐理由：** 中文实用DSH插件精选集合
...
- **标签：** `awesome-list`, `auto-updating`
- **推荐理由：** YAML驱动自动更新双语插件列表，Vercel托管。
...
- **标签：** `awesome-list`, `evidence-aware`
- **推荐理由：** 证据意识筛选的有用DSH插件精简列表。
...
- **标签：** `reference`, `dsh-plugin`
- **推荐理由：** DSH原理学习与插件最佳实践参考
...
- **推荐理由：** 对话内多源搜番，显示 Bangumi 评分和磁力链接。
...
- **推荐理由：** 开源锤子便签复刻，Docker 私有部署，支持 skill 调用。
...
- **标签：** `community`, `catalog`, `awesome-list`
...
- **推荐理由：** 精选280+ DSH社区插件目录，14类分类索引，必收藏。
...
- **标签：** `awesome-list`, `dsh-plugin`, `plugin-catalog`
...
- **标签：** `awesome-list`, `mcp`, `coding-agent`, `dsh-plugin`
- **推荐理由：** 全面的DSH资源大全，涵盖插件、工具、技能和MCP
...
- **推荐理由：** DeepSeek Harness插件市场，类似CodeX插件商店。
