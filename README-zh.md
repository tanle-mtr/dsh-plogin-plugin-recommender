...
- **标签：** `coding-agent`, `cli`, `terminal`
- **推荐理由：** DeepSeek原生终端编码Agent，前缀缓存稳定
...
- **推荐理由：** 首个视觉插件，为纯文本 agent 提供 OCR 能力。
...
- **标签：** `code-review`, `agent-skills`, `quality`
...
- **标签：** `security`, `audit`, `redteam`, `coding`
- **推荐理由：** AI安全攻防平台，审计幻觉包与代码漏洞。
...
- **推荐理由：** 对话内构建运行Android应用，支持模拟器与真机。
...
- **标签：** `skills`, `coding`, `reasonix`
- **推荐理由：** Superpowers技能适配Reasonix编码工作流。
...
- **推荐理由：** 开发DSH插件的Agent技能，覆盖标准与打包流程
...
- **推荐理由：** 引导开发DSH插件的Agent技能，覆盖槽位和凭证配置
...
- **标签：** `code-simplification`, `codex`, `coding`
- **推荐理由：** DSH代码简化模式Codex智能体技能
...
- **标签：** `mcp`, `skill-management`, `ui`
- **推荐理由：** 在Web界面管理技能和MCP服务器，即点即用
...
- **标签：** `plugin-dev`, `development`, `tools`
- **推荐理由：** 便携Agent技能，用于DSH插件开发与审计
...
- **标签：** `plugin-dev`, `documentation`, `guide`
- **推荐理由：** DSH插件开发踩坑指南，含解决方案
...
- **标签：** `migration`, `claude-code`, `mcp`, `dsh-plugin`
- **推荐理由：** 无缝迁移Claude Code配置到DSH，支持MCP和技能导入
...
- **标签：** `web-ui`, `aggregation`, `creative-workshop`
- **推荐理由：** DSH Web UI 聚合插件，创意工坊分发平台。
...
- **标签：** `sidebar`, `ui`, `desktop-extension`
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/Git/对话
...
- **标签：** `tui`, `ui`, `claude-code-style`
- **推荐理由：** 鲸鱼主���TUI界面，实时状态显示和流式思考
...
- **标签：** `desktop`, `tauri`, `gui`
- **推荐理由：** 仅5MB的Tauri桌面版，零环境配置，跨平台运行。
...
- **标签：** `desktop`, `pet`, `companion`
- **推荐理由：** 本地优先桌面伴侣，带动漫宠物和插件SDK。
...
- **标签：** `widget`, `ui`, `balance-tracker`
- **推荐理由：** 可爱鲸鱼挂件实时监控账户余额，支持拖拽吸附。
...
- **标签：** `desktop`, `workspace`, `office`
- **推荐理由：** 本地优先AI桌面工作台，支持办公与多项目管理。
...
- **推荐理由：** 本地优先多模型AI桌面应用，隐私友好，支持自进化技能。
...
- **标签：** `canvas`, `workspace`, `visual`
- **推荐理由：** 可视化非线性对话工作区，支持分支探索。
...
- **标签：** `dsh`, `ui`, `desktop`, `container`
- **推荐理由：** 容器化多版本DSH管理与嵌入式WebView界面
...
- **标签：** `ui`, `skill-picker`, `dsh-plugin`
- **推荐理由：** DSH技能选择器与Composer界面扩展
...
- **标签：** `canvas`, `creative`, `cordis`
- **推荐理由：** 无限��布配合创意AI技能，扩展生产力体验
...
- **推荐理由：** Electron桌面壳，集成多个DSH工具，开箱即用
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所���即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `ui`, `background`, `personalization`
- **推荐理由：** 支持自定义背景图和预设氛围，实时预览调整
...
- **标签：** `memory`, `local-first`, `markdown-native`, `rag`
- **推荐理由：** 便携本地优先的记忆层，持久保存 agent 知识。
...
- **标签：** `memory`, `self-evolving`, `hybrid-retrieval`, `token-savings`
- **推荐理由：** 自演进记忆系统，混合检索节省 35% token。
...
- **标签：** `memory`, `rag`, `persistent-knowledge`
- **推荐理由：** 为AI提供无限持久记忆，支持跨会话知识存储。
...
- **标签：** `rag`, `skill-retrieval`, `embeddings`
- **推荐理由：** 将SKILL.md转为可检索知识库
...
- **推荐理由：** 锤子便签开源版，Docker私有部署，支持导出图片。
...
- **标签：** `memory`, `handoff`, `context-engineering`
- **推荐理由：** 跨平台智能体交接连续性记忆工具
...
- **标签：** `context`, `audit`, `memory`, `ui`
- **推荐理由：** 上下文注入审计检测重复冲突工具
...
- **标签：** `evaluation`, `rag`, `agent-evaluation`
- **推荐理由：** 支持提示词、RAG、技能的全链路评估与可观测性
...
- **标签：** `multi-agent`, `swarm`, `skills`
- **推荐理由：** 多智能体蜂群框架，自动化工作流编排
...
- **推荐理由：** 企业级多 Agent 工作台，支持 DSH 团队协作。
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `workflow`
...
- **标签：** `workflow`, `prompt-management`, `productivity`
...
- **标签：** `agent`, `team`, `workflow`, `collaboration`
- **推荐理由：** 将多种编程Agent整合为团队协作，支持会话调度。
...
- **标签：** `math-modeling`, `skill`, `automation`
- **推荐理由：** 带GitOps的数学建模竞赛自动化技能
...
- **标签：** `creative-writing`, `novel`, `drama`
- **推荐理由：** 小说创作与短剧制作技能
...
- **推荐理由：** SQLite任务管理，Agent认领审核，原生Web UI。
...
- **推荐理由：** Superpowers技能适配DSH，支持TDD、调试和规划
...
- **推荐理由：** 85个SKILL.md，覆盖逆向工程与授权渗透测试
...
- **推荐理由：** 88个Agent技能，覆盖研究、社交情报和营销工作流
...
- **标签：** `knowledge-base`, `plugin-development`, `dsh-plugin`
- **推荐理由：** 官方DSH插件开发知识库与智能体技能
...
- **标签：** `fengshui`, `chinese-traditional`, `agent-skill`
- **推荐理由：** 八字风水奇门中医融合AI智能体技能包
...
- **标签：** `swift-cycle`, `governance`, `project`
- **推荐理由：** DSH项目治理Swift Cycle技能适配器
...
- **标签：** `docs`, `gongwen`, `office`
- **推荐理由：** 国标公文全流程处理：格式检查、模板生成、修订批注
...
- **标签：** `agent`, `database`, `devkit`
- **推荐理由：** 42技能+5CLI的综合Agent开发套件
...
- **标签：** `agent-skill`, `engineering`, `deepseek-harness`
- **推荐理由：** 便携式工程纪律技能，支持证据驱动与诚实报告实践
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，支持完整开发工作流
...
- **标签：** `agent`, `workflow`, `product`
- **推荐理由：** 模拟产品团队多角色对话，辅助决策与规划
...
- **标签：** `agent`, `plugin`, `automation`
- **推荐理由：** Agent任务前检插件库，自动注入Top-K适用插件
...
- **标签：** `ai-image-generation`, `prompt-engineering`, `workflow-automation`
- **推荐理由：** 工业级GPT图像提示词引擎，530+逆向模板
...
- **标签：** `diagram-as-code`, `architecture-diagram`, `mermaid-alternative`
- **推荐理由：** 从文本生成精美的架构和流程图。
...
- **标签：** `design`, `vector`, `ui`, `vibedesign`
- **推荐理由：** AI 原生矢量设计工具，并发 Agent 团队生成 UI。
...
- **标签：** `video-generation`, `ai-video`, `ffmpeg`
- **推荐理由：** 一键生成Vox风格剪辑解说视频
...
- **标签：** `ui`, `genui`, `visualization`
- **推荐理由：** 在回复中内联渲染图表、表单等交互式UI组件。
...
- **标签：** `agent-skills`, `claude-code`, `claude-skills`, `codex`, `deepseek-harness`, `diagram`, `dsh-plugin`, `legal`, `legal-tech`, `litigation`, `python`, `svg`, `visualization`, `zero-dependency`
- **推荐理由：** 法律可视化技能，生成诉讼时间轴和案情图
...
- **标签：** `video`, `audio`, `image`, `directorx`
- **推荐理由：** 集成DirectorX的AI音视频处理与故事板能力
...
- **标签：** `dingtalk`, `chat-bot`, `messaging`
- **推荐理由：** 钉钉IM通道，支持流式机器人和情感反馈。
...
- **标签：** `self-hosted`, `mcp-server`, `resume-builder`
...
- **标签：** `electron`, `image-uploader`, `cloudflare-r2`
...
- **标签：** `low-code`, `workflows`, `self-hosted`
- **推荐理由：** AI驱动的开源低代码平台，快速构建业务系统
...
- **标签：** `router`, `injector`, `task-aware`
- **推荐理由：** 任务感知路由器，智能请求分发套件。
...
- **标签：** `vfs`, `virtual-filesystem`, `unified`
- **推荐理由：** 统一虚拟文件系统，agent 一致文件访问。
...
- **标签：** `claude-code`, `deepseek-harness`, `utility`
- **推荐理由：** 一键切换Claude Code、Codex、DeepSeek等多模型
...
- **推荐理由：** 通过OAuth使用订阅账号作为LLM提供者，无需API Key。
...
- **标签：** `marketplace`, `plugin-manager`, `utility`
- **推荐理由：** 一键浏览安装更新GitHub上的全部DSH插件。
...
- **推荐理由：** 崩溃恢复插件，支持撤销快照和SAFE MODE安全模式。
...
- **标签：** `utility`, `info`, `monitoring`
- **推荐理由：** 实时显示提供商、模型、余额及峰谷定价
...
- **标签：** `plugin`, `conflict`, `utility`
- **推荐理由：** 诊断解决插件冲突和版本兼容问题，维护生态稳定
...
- **标签：** `mascot`, `pixel-art`, `animation`, `pets`
- **推荐理由：** Coding Agent 的宠物吉祥物动画画廊。
...
- **⭐ 星标：** 384
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套��� DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `ui`, `transparency`
- **推荐理由：** 高自由度磨砂玻璃主题，支持全面自定义，不改源码。
...
- **标签：** `theme`, `dark-mode`, `customization`
...
- **标签：** `research`, `web-search`, `china-platforms`
- **推荐理由：** 自动搜索中文8大平台生成30天研究报告
...
- **推荐理由：** 精选插件雷达，覆盖9000+候选，Top50人工策展。
...
- **标签：** `awesome-list`, `ecosystem`, `index`
- **推荐理由：** 全面索引DSH插件生态与外部工具仓库。
...
- **标签：** `awesome-list`, `plugin`, `curated`
- **推荐理由：** 双语精选DSH插件扩展目录，含验证参考。
...
- **⭐ 星标：** 274
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复���：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `discovery`, `plugin`, `awesome-list`
- **推荐理由：** 每日自动抓取分类插件，含热度排行和使用指南。
...
- **标签：** `ai-pedia`, `resource`, `encyclopedia`
- **推荐理由：** AI资源百科大全，涵盖Agent、RAG、MLOps及学习路线。
...
- **推荐理由：** 多源搜番，Bangumi评分卡片展示，支持磁力复制。
...
- **标签：** `awesome-list`, `directory`, `reference`
- **推荐理由：** 精选DSH插件合集，方便发现和参考。
...
- **标签：** `kline`, `analysis`, `market`, `trading`
- **推荐理由：** 交互式K线图技术分析基本面研究
...
- **标签：** `research`, `mcp`, `zhihu`
- **推荐理由：** 知乎搜索与智答API，支持Skill/CLI/MCP多种调用
...
- **标签：** `awesome-list`, `2026`, `curated`
- **推荐理由：** 2026精选插件列表，含质量验证检查
...
- **标签：** `verified`, `evidence-led`, `directory`
- **推荐理由：** 基于证据的验证目录，含权限感知安装指导
...
- **标签：** `finance`, `stock`, `quant`, `directory`
- **推荐理由：** 精选股票/量化插件目录，中英双语，便于筛选
...
- **标签：** `curated-list`, `awesome`
- **推荐理由：** 基于证据的精选插件短名单，实用性强。
...
- **标签：** `curated`, `typescript`, `registry`
- **推荐理由：** 涵盖模型、工具、UI等多维度的精选插件集。
...
- **标签：** `industry`, `awesome-list`, `niche`
- **推荐理由：** 面向金融、法律、医疗等行业的精选插件目录。
...
- **标签：** `game`, `web-simulation`, `chinese-culture`
- **推荐理由：** 竹知了玩具Web模拟版，零依赖单文件实现
...
- **标签：** `armor-breaking`, `prompt`, `deepseek`
- **推荐理由：** DeepSeek专用破甲提示词稳定化插件。
...
- **⭐ 星标：** 206
- **语言：** Unknown
- **描述：** DeepSeek Harness 终极指南：快速入门、资源推荐、精选插��与实用工具 ｜The Ultimate Guide to DeepSeek Harness: QuickStart, Resources, Plugins&Toolkit
...
- **推荐理由：** cordis.run官方精选并自动生成的DSH插件列表
...
- **标签：** `awesome-list`, `plugin-directory`, `community`
- **推荐理由：** 精选14类280+社区DSH插件目录索引
...
- **标签：** `directory`, `awesome-list`
- **推荐理由：** 社区认证排名插件目录，帮助快速发现优质插件
...
- **标签：** `documentation`, `guide`, `directory`
- **推荐理由：** 按场景导航插件说明书，含安装权限与兼容性说明
...
- **标签：** `radar`, `evidence`, `testing`
- **推荐理由：** 实证驱动的插件雷达与2Origin测试实验室
...
- **标签：** `portal`, `directory`, `static-site`
- **推荐理由：** 零依赖静态门户，浏览安装227+精选插件
...
- **标签：** `directory`, `plugins`
- **推荐理由：** DeepSeek Harness插件仓库列表
...
- **推荐理由：** 仓库已迁移至 awesome-dsh-plugin/awesome-dsh-plugin
