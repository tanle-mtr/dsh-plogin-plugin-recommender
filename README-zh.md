...
- **标签：** `coding-agent`, `prompt-caching`, `terminal`, `tui`
- **推荐理由：** 终端原生编码代理，前缀缓存稳定运行。
...
- **标签：** `dsh-plugin`, `mcp`, `agent-orchestration`
- **推荐理由：** DSH运行时注入器和任务感知推理路由器
...
- **标签：** `agent-os`, `mcp`, `llm-orchestration`, `skills`
- **推荐理由：** 自进化Agent OS，面试级评估驱动进化
...
- **推荐理由：** 跨Codex/Claude Code的长期Agent控制平面
...
- **标签：** `agent-collaboration`, `dsh-plugin`, `coding-agent`
- **推荐理由：** 企业级多引擎统一Agent工作台
...
- **推荐理由：** 首个视觉插件，支持OCR和图片转文本，适配纯文本编码代理。
...
- **标签：** `runtime`, `agent`, `ide`, `dsh-plugin`
- **推荐理由：** 高性能Rust代理运行时搭配精美桌面IDE应用。
...
- **标签：** `ai-agent`, `coding-agent`, `mcp`, `deepseek-harness`
...
- **标签：** `plugin-development`, `agent-skill`, `dsl`
- **推荐理由：** DSH插件开发指导skill，覆盖全生命周期标准
...
- **标签：** `plugin-dev`, `credentials`
- **推荐理由：** 管理已安装插件槽位、凭证和远程配置
...
- **标签：** `skills-manager`, `local-skill`, `management`
- **推荐理由：** 统一加载管理本地Agent Skills，安全可靠
...
- **标签：** `deepseek-harness`, `container`, `plugin-management`
- **推荐理由：** 隔离容器运行多版DSH，内嵌WebView UI管理插件
...
- **推荐理由：** Codex与DSH双向同步，聊天/技能/MCP一键桥接
...
- **标签：** `ui`, `mascot`, `dsh-plugin`, `cli`
- **推荐理由：** 多Agent框架的动画宠物UI展示插件
...
- **标签：** `taskboard`, `ui`, `skills`, `dsh-plugin`
...
- **推荐理由：** Claude Code风格TUI，支持实时状态、流式思考和回滚。
...
- **推荐理由：** 带SSH和远程访问的Windows桌面版DSH客户端
...
- **标签：** `ios-simulator`, `mobile-automation`, `ui-automation`
- **推荐理由：** 在对话中集成iOS模拟器及真机自动化，22个agent工具。
...
- **推荐理由：** 跨平台桌面客户端，CodePilot风格，支持三大系统。
...
- **标签：** `ios`, `mobile`, `ui`
- **推荐理由：** 原生iOS客户端，WebSocket实时同步Harness会话。
...
- **推荐理由：** 在DSH Web界面统一管理Skill和MCP工具
...
- **推荐理由：** Electron桌面控制台，管理Harness、插件、MCP和技能
...
- **标签：** `bioinformatics`, `desktop`, `llm-agent`, `tauri`
- **推荐理由：** 桌面端生物信息学智能体，支持离线模式与技能商店
...
- **推荐理由：** 一站式DevOps桌面：数据库、SSH、Docker与AI助手
...
- **标签：** `ui`, `background`, `theme`, `web`
- **推荐理由：** 支持自定义图片与预设氛围的Web背景插件。
...
- **推荐理由：** Electron桌面壳整合DSH Web及侧边栏和ModLens。
...
- **标签：** `skills`, `dsh-plugin`, `workflow`
- **推荐理由：** 最大技能商店，自动聚合GitHub Skills项目
...
- **标签：** `preset`, `workflow`, `standard-tools`, `dsh-plugin`
- **推荐理由：** 两阶段预设，轻量引导后接入完整Standard工具链。
...
- **标签：** `cognitive`, `skills`, `agent-skills`, `dsh-plugin`
...
- **标签：** `agentteams`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 支持DSH内多代理团队协作
...
- **标签：** `multimodal`, `computer-vision`, `local-first`
- **推荐理由：** 本地多模态感知：视频/音频/屏幕理解能力
...
- **标签：** `task-management`, `sqlite`, `agent-review`
- **推荐理由：** SQLite驱动的本地任务看板，支持Agent认领审查
...
- **标签：** `skill`, `math-modeling`, `gitops`
- **推荐理由：** 全栈数学建模Skill，支持GitOps流水线与代码自证。
...
- **标签：** `finance`, `agent-skills`, `mcp`
- **推荐理由：** 多Agent金融分析系统，支持本地库与MCP扩展。
...
- **标签：** `infra`, `agent-skills`, `development`
...
- **标签：** `reverse`, `security`, `agent-skills`
- **推荐理由：** 逆向工程与授权渗透测试87个专业技能文件。
...
- **标签：** `agent-skills`, `ai-hedge-fund`, `backtesting`, `fintech`
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `agent-skills`, `dashboard`
- **推荐理由：** 八字风水易经融合，本地确定性引擎+可视化面板
...
- **推荐理由：** 类Codex代码简化技能，适配DSH Agent工作流
...
- **标签：** `agent-skills`, `llm`, `evaluation`, `database`
- **推荐理由：** 42个预置技能，覆盖LLM开发评估与数据库工作流
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，支持自动化开发
...
- **推荐理由：** SkillHub官方Open API文档与DSH集成示例
...
- **推荐理由：** Swift Cycle治理技能，支持版本锁定与离线验证
...
- **标签：** `agent-tools`, `pre-flight`, `plugin-check`
- **推荐理由：** 预检自动注入相关插件，提升Agent效率。
...
- **标签：** `ai-image`, `ai-video`, `storyboard`, `text-to-video`
- **推荐理由：** AI视频图像生成与分镜工具，适合创意工作流
...
- **推荐理由：** 自进化跨平台内容发现Agent，覆盖B站、YouTube、小红书等。
...
- **标签：** `electron-app`, `image-uploader`, `s3`
...
- **标签：** `oauth`, `subscriptions`, `providers`
- **推荐理由：** 通过OAuth将订阅服务用作DSH的LLM提供商
...
- **推荐理由：** 追踪会话/日费用、预算、token用量和API余额。
...
- **标签：** `cli`, `plugin-manager`, `resolver`
- **推荐理由：** DSH插件仓库的CLI工具和解析器。
...
- **标签：** `migration`, `compatibility`, `infrastructure`
- **推荐理由：** 打通Pi与DSH生态，零改造运行Pi插件。
...
- **标签：** `migration`, `session`, `infrastructure`
- **推荐理由：** 跨预设会话迁移，保留状态与意图的插件桥。
...
- **标签：** `context-audit`, `utility`, `token-management`
...
- **推荐理由：** DeepSeek API实时Token用量与费用图表监控
...
- **标签：** `utility`, `conflict-resolution`, `debug`
...
- **标签：** `theme`, `glass-effect`, `ui`
- **推荐理由：** 高度可定制的磨砂玻璃质感DSH主题
...
- **标签：** `awesome-list`, `plugins`, `curated-list`
- **推荐理由：** 全面的DSH插件资源导航与精选推荐
...
- **标签：** `knowledge-base`, `mcp`, `ai-pedia`
- **推荐理由：** 涵盖Agent、MCP、RAG的全维度AI资源百科
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪、GPU SSH。
...
- **标签：** `academic-search`, `research`, `mcp`
- **推荐理由：** 跨库学术检索，支持引文核验与图谱，科研必备。
...
- **标签：** `company-research`, `industry-research`, `report`, `research`
- **推荐理由：** 产业链映射+政策新闻追踪，行业研究方法论完善
...
- **标签：** `kline`, `finance`, `market-analysis`
- **推荐理由：** 交互式K线分析，支持技术指标和新闻基本面
...
- **标签：** `awesome-list`, `bilingual`, `reviewed`
- **推荐理由：** 双语证据审核插件目录，质量可靠。
...
- **标签：** `awesome-list`, `auto-updating`, `bilingual`
- **推荐理由：** 通过YAML和脚本自动更新双语插件列表。
...
- **标签：** `low-code`, `crm`, `workflows`, `self-hosted`
- **推荐理由：** AI增强低代码平台，快速构建业务系统。
...
- **标签：** `documentation`, `best-practices`, `marketplace`
...
- **标签：** `awesome-list`, `plugins`, `reference`
- **推荐理由：** DSH插件精选合集，便于发现和参考。
...
- **标签：** `awesome-list`, `mcp`, `skills`
- **推荐理由：** DSH插件、工具、技能和MCP资源精选列表
...
- **推荐理由：** 过滤垃圾的DeepSeek Harness插件开放数据注册表。
...
- **推荐理由：** 2026年DeepSeek Harness经过质量检查的精选插件列表。
...
- **标签：** `curated`, `directory`, `permission-aware`
...
- **标签：** `guide`, `manual`, `navigation`
- **推荐理由：** 覆盖安装、权限和兼容性的插件导航说明书。
...
- **标签：** `directory`, `automated`, `updated`
- **推荐理由：** 通过GitHub Actions每日自动更新的插件目录。
...
- **标签：** `awesome-list`, `plugin-directory`, `registry`, `deepseek-harness`
- **推荐理由：**  curated插件目录，方便发现和安装DSH插件。
