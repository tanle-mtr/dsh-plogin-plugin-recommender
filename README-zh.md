...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 186,576 | Core DSH platform powering the entire plugin ecosy |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 90,578 | Open-source Claude Design alternative for AI-drive |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 68,905 | Multi-agent swarm orchestration with autonomous wo |

---

## 分类导航

- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 34 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (34)
...
- **标签：** `ai-agent`, `multi-model`, `memory`
- **推荐理由：** 自进化多模型智能助手，支持多渠道。
...
- **标签：** `coding-agent`, `cli`, `deepseek`
- **推荐理由：** 终端原生AI编程智能体，前缀缓存稳定。
...
- **推荐理由：** 一键切换Claude Code、Codex、DeepSeek等多AI编程工具。
...
- **标签：** `agent-runtime`, `dsh-plugin`, `vibe-coding`
- **推荐理由：** Rust高性能Agent运行时，集成桌面级代码Agent应用。
...
- **推荐理由：** 插件雷达精选榜，自动发现9000+候选并人工策展Top50。
...
- **标签：** `dsh-plugin`, `agent-skills`, `coding-agents`
- **推荐理由：** 让AI编码代理感知架构，支持基线优先和漂移检测
...
- **标签：** `multi-agent`, `collaboration`, `dsh-plugin`
- **推荐理由：** 多Agent团队协作，支持独立模型与共享工作区。
...
- **推荐理由：** 42个Agent技能+5个CLI的AI开发者资产包管理器。
...
- **标签：** `dev`, `plugin`, `agent`
- **推荐理由：** DSH插件开发审计便携Agent Skill，支持配置文件安装。
...
- **标签：** `dev`, `doc`, `practices`
- **推荐理由：** DSH插件开发踩坑档案，含cordis、tsconfig、zstd等实测记录。
...
- **标签：** `dev`, `skills`, `plugin`
- **推荐理由：** DSH插件脚手架构建测试Agent技能，全程内置操作。
...
- **推荐理由：** 内置GUI管理MCP与插件，热生效无需重启，开箱即用。
...
- **推荐理由：** 粘贴JSON即连MCP、自动创建技能，免重启配置超高效。
...
- **标签：** `capability`, `mcp`, `security`
- **推荐理由：** 统一管理MCP工具暴露级别与执行模式，精细控制。
...
- **标签：** `agent-tools`, `utility`
- **推荐理由：** 插件预检清单，提升智能体库利用率。
...
- **标签：** `ai-agents`, `dsh-plugin`, `plugin`
- **推荐理由：** AI每日更新的DSH插件大全，覆盖12个分类。
...
- **标签：** `dsh-plugin`, `dsh-bundle`, `dsh-ecosystem`
- **推荐理由：** DSH插件生态目录，涵盖bundle与平台分析工具。
...
- **标签：** `deepseek-harness-plugin`, `coding-agents`, `skills`
- **推荐理由：** 全面精选插件列表，覆盖模型、工具、UI和智能体循环。
...
- **标签：** `registry`, `dsh-plugin`, `discovery`
- **推荐理由：** 多源插件注册表，合并awesome列表、GitHub与npm搜索。
...
- **标签：** `dsh-plugin`, `cordis-plugin`
- **推荐理由：** cordis.run官方自动生成的Cordis插件精选列表。
...
- **标签：** `dsh-plugin`, `plugin-market`
- **推荐理由：** 动态插件市场，支持搜索、排名、收藏和一键安装。
...
- **标签：** `web-ui`, `desktop`, `skin`
- **推荐理由：** 完整Web GUI支持移动端和SSH运维，皮肤丰富
...
- **标签：** `pet`, `mascot`, `ui`, `animation`
- **推荐理由：** 为代码代理添加个性的动��宠物画廊
...
- **推荐理由：** 开放侧边栏底座，支持文件终端对话Git等多页面扩展��
...
- **标签：** `dsh-plugin`, `desktop-companion`, `electron`
- **推荐理由：** Electron桌面陪伴平台，支持动画宠物和插件SDK
...
- **推荐理由：** Tauri轻量桌面版，仅5MB安装包，预置插件零配置
...
- **标签：** `dsh-plugin`, `floating-widget`, `cordis`
- **推荐理由：** 可爱的鲸鱼浮动小部件，实时跟踪DeepSeek账户余额
...
- **推荐理由：** Electron桌面控制台，统一管理插件/MCP/Skills
...
- **标签：** `canvas`, `creative`, `ui`
- **推荐理由：** 无限画布插件，支持WeShop创意技能集成。
...
- **推荐理由：** 悬浮HUD面板实时展示Git状态、MCP与服务运行情况。
...
- **⭐ 星标：** 9
- **语言：** Rust
- **描述：** StarHub 是一��跨平台桌面应用（Tauri 2 + Rust 主进程 + DeepSeek Harness React 工作台 + Go Sidecar），把开发运维日常高频工具整合到同一个窗口 —— 数据库、SSH/SFTP、Docker 面板与 AI 助手。目标是减少在 Navicat、Xshell、Portainer、文件管理器和 AI 对话窗口之间来回切换的成本。  前端架构：基于 DeepSeek Harness 原生 React 工作台（/starhub-react 路由）。资产、设置、SSH 终端、SFTP、数据库
- **标签：** `desktop`, `tauri`, `devtools`
- **推荐理由：** 跨平台桌面整合数据库、SSH、Docker与AI助手。
...
- **推荐理由：** Electron桌面壳，集成Dsh侧边栏与Web UI。
...
- **推荐理由：** 为OpenClaw提供无限持久记忆，适合长期对话场景
...
- **标签：** `knowledge-base`, `rag`
- **推荐理由：** 覆盖模型Agent RAG的一站式AI知识库
...
- **⭐ 星标：** 150
- **语言：** TypeScript
- **描述：** 开源版锤子便签，复刻锤科美学，一键Docker私有化部署，支持skill调用，支持dsh plugin，支���多租户，一键生成公众号格式，支持导出便签为图片
- **标签：** `notes`, `memory`, `productivity`
- **推荐理由：** 锤子便签复刻版，支持Docker部署和技能调用
...
- **标签：** `agent-skills`, `embeddings`, `semantic-search`
- **推荐理由：** 将SKILL.md转化为可检索技能库
...
- **推荐理由：** 桥接Claude Code记忆与技能到DSH生态，无缝迁移。
...
- **标签：** `agent-skills`, `knowledge-distillation`, `persona`
- **推荐理由：** 知识蒸馏AI人格技能，打造个性智能体。
...
- **标签：** `architecture-diagram`, `diagrams-as-code`, `mermaid`
- **推荐理由：** 文本生成精美架构图与流程图。
...
- **标签：** `prompt-engineering`, `skills`, `workflow-automation`
- **推荐理由：** 470+工业级提示词模板与可复用Skills
...
- **标签：** `agent-skills`, `ai-agent`, `deepseek-harness`
- **推荐理由：** 基于J-space全局工作空间AI认知增强技能套件。
...
- **标签：** `dsh-plugin`, `agentic-workflow`, `task-manager`
- **推荐理由：** 自托管人工介入任务管理器，支持多端远程控制代理
...
- **标签：** `dsh-plugin`, `agent-teams`, `deepseek-harness`
- **推荐理由：** 支持DSH内多代理团队协作，提升复杂任务处理能力
...
- **标签：** `agent`, `desktop-app`
- **推荐理由：** 本地优先AI桌面，自进化技能多模型
...
- **⭐ 星标：** 168
- **语言：** JavaScript
- **描述：** 帮 DSH 搜索、安��并验证插件的 Skill｜A DSH skill that finds, installs, and verifies GitHub plugins
- **标签：** `plugin-discovery`, `skill`, `automation`
- **推荐理由：** 自动搜索安装验证DSH插件，节省时间
...
- **标签：** `ai-agents`, `creative-writing`, `deepseek-harness`
- **推荐理由：** 小说与短剧创作全流程工具
...
- **标签：** `spec-driven-development`, `vibe-coding`, `developer-tools`
- **推荐理由：** 仓库级规格驱动开发技能，支持vibe编码
...
- **标签：** `code-simplification`, `dsh-plugin`
- **推荐理由：** 来自DSH的代码简化技能，适配Codex使用流程。
...
- **推荐理由：** WorkBuddy风格技能选择器，便捷调用DSH技能。
...
- **推荐理由：** 专为管理DSH Agent技能而设计的技能管理插件。
...
- **标签：** `image-generation`, `vision`, `skill`
- **推荐理由：** 免费图片读取与生成技能，支持DSH视觉模型。
...
- **标签：** `skill`, `practices`, `engineering`
- **推荐理由：** 便携式Agent Skill，提炼DSH工程实践与方法论。
...
- **标签：** `doc`, `gongwen`, `chinese`
- **推荐理由：** 中文公文全流程处理Skill，符合国标格式检查优化。
...
- **标签：** `record`, `replay`, `skill`
- **推荐理由：** 录制macOS桌面操作并生成可复用Agent技能。
...
- **推荐理由：** Godot 4.x 全栈游戏开发技能，运行时自动注册。
...
- **标签：** `ai-design`, `ui-generator`, `figma-alternative`
- **推荐理由：** 开源AI设计工具，Claude Design替代方案。
...
- **标签：** `design`, `vector`, `agent-team`, `ui`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持并发代理
...
- **标签：** `dsh-plugin`, `video-generation`, `agent-skill`
- **推荐理由：** 一键生成Vox风格拼贴解说视频，端到端自动化。
...
- **推荐理由：** 为纯文本模型提供视觉理解能力，支持多图问答和GUI自动化
...
- **标签：** `dsh-plugin`, `vision`, `gui-automation`
- **推荐理由：** DSH原生视觉工具包，支持截图OCR和前端UI还原
...
- **推荐理由：** 6种设计风格避免AI感，提升Vibe Coding视觉效果。
...
- **推荐理由：** DirectorX集成插件，支持AI视频/图像/音频创作技能。
...
- **标签：** `ai-agent`, `bilibili`, `deepseek-harness`
- **推荐理由：** 本地优先跨平台AI内容发现Agent，支持B站抖音小红书等。
...
- **标签：** `browser-use`, `dsh-plugin`, `agent`
- **推荐理由：** 让AI代理无缝使用已登录浏览器执行自动化操作。
...
- **标签：** `dsh-plugin`, `browser-extension`, `productivity`
- **推荐理由：** 浏览器扩展将AI对话转化为可复用的结构化工作流
...
- **推荐理由：** OAuth绑定ChatGPT，在DSH内使用ChatGPT模型。
...
- **标签：** `routing`, `infrastructure`, `dsh-plugin`
- **推荐理由：** 任务感知路由套件，智能选择推理模式
...
- **标签：** `preset`, `bootstrap`, `infrastructure`
- **推荐理由：** 两阶段引导预设，确保稳定工具配置
...
- **标签：** `cost-tracking`, `utility`, `tokens`
- **推荐理由：** 实时跟踪会话和每日成本，支持预算告警
...
- **标签：** `context-migration`, `preset-migration`, `cordis`
- **推荐理由：** 跨预设会话迁移 preserving state
...
- **标签：** `plugin-management`, `console`, `dsh-repository-plugin`
- **推荐理由：** 插件管理控制台与官方开发引导工具集
...
- **标签：** `plugin-development`, `build`
- **推荐理由：** 管理已安装插件的槽位、远程连接和凭据
...
- **标签：** `desktop`, `container`, `plugin-management`
- **推荐理由：** 容器隔离运行多版本DSH，内嵌WebView管理
...
- **标签：** `context`, `audit`, `ui`
- **推荐理由：** 审计优化DSH上下文Token，含可视化Web UI面板。
...
- **标签：** `ai-guide`, `vibe-coding`, `tutorial`
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用��的插件产品说明书与导航：按场景选插件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `directory`
- **推荐理由：** 按场景选择插件，附安装与风险说明。
...
- **标签：** `awesome-list`, `radar`
- **推荐理由：** 证据驱动的插件雷达，含2Origin实验室。
...
- **标签：** `low-code`, `no-code`, `crm`
- **推荐理由：** AI驱动的低代码业务系统构建平台。
...
- **标签：** `awesome-list`, `directory`, `dsh-plugin`
- **推荐理由：** DSH生态精选插件目录，全面实用
...
- **标签：** `dsh-plugin`, `awesome-list`, `plugin-ecosystem`
- **推荐理由：** DSH生态插件精选列表，涵盖工具与基础设施资源
...
- **标签：** `dsh-plugin`, `awesome-list`, `developer-tools`
- **推荐理由：** DSH插件全面精选列表，整合hub与公开生态资源
...
- **标签：** `documentation`, `education`
- **推荐理由：** 学习DSH原理插件市场最佳实践
...
- **标签：** `marketplace`, `plugin-hub`
- **推荐理由：** DSH插件市场聚合器，方便查找插件
...
- **标签：** `awesome`, `awesome-list`, `resources`
- **推荐理由：** 精心整理的DSH插件资源清单
...
- **标签：** `awesome-dsh-plugin`, `plugin-marketplace`, `deepseek-harness`
- **推荐理由：** 社区精选插件目录，每4小时自动同步维护
...
- **标签：** `awesome-deepseek-harness`, `cordis`
- **推荐理由：** cordis.run官方精选自动生成的插件目录列表
...
- **标签：** `community`, `github-oauth`, `photo`
- **推荐理由：** DSH内测收官合影墙，GitHub OAuth零权限登录。
