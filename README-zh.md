...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 194,829 | Core framework for the entire DSH plugin ecosystem |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,403 | Open-source design system with local-first desktop |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,377 | Multi-agent swarm orchestration with autonomous wo |
...
- [Agent 技能与工作流](#agent-技能与工作流) — 46 个插件
...
- [Agent 技能与工作流](agent-skills-workflows-zh) (46)
...
- **标签：** `multi-agent-workspace`, `agent-collaboration`, `visual-editor`
- **推荐理由：** 企业级多引擎Agent工作台
...
- **标签：** `agent-runtime`, `coding-agent`, `desktop`
- **推荐理由：** Rust高性能Agent运行时+桌面应用
...
- **推荐理由：** 本地优先多模型AI代理桌面应用，支持自进化技能与隐私保护。
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** OpenAI Codex与DSH双向同步桥，支持��时技能挂载
...
- **标签：** `dev`, `plugin`, `tool`
- **推荐理由：** 便携式插件开发与审计技能，附带Profile适配器
...
- **标签：** `docs`, `dev`, `plugin`
- **推荐理由：** DSH插件开发踩坑档案，含Cordis、tsconfig等实测记录
...
- **标签：** `migration`, `mcp`, `skills`, `configuration`
- **推荐理由：** 无缝迁移 Claude Code 配置到 DeepSeek Harness
...
- **标签：** `dsh-plugin`, `dsh-web`, `dsh-web-ui`
- **推荐理由：** DSH创意插件聚合Web界面包
...
- **标签：** `desktop`, `companion`
- **推荐理由：** 桌面伴侣平台，动画宠物与插件SDK扩展
...
- **⭐ 星标：** 981
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住�� DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `widget`, `desktop`
- **推荐理由：** 可爱浮动鲸鱼组件监控账户余额
...
- **标签：** `dsh-plugin`, `desktop-app`, `electron`
- **推荐理由：** 类CodePilot桌面客户端，支持macOS/Windows/Linux多平台。
...
- **标签：** `dsh-plugin`, `ios`, `mobile-automation`
- **推荐理由：** 在对话内运行iOS模拟器与真机自动化，提供22个代理工具。
...
- **标签：** `dsh-plugin`, `ui`
- **推荐理由：** 基于画布的视觉化非线性对话工作区，支持分支会话探索。
...
- **推荐理由：** WorkBuddy风格技能选择器，快速选择并插入技能
...
- **推荐理由：** Tauri 桌面应用整合数据库、SSH、Docker 与 AI
...
- **标签：** `ui`, `background`, `customization`
- **推荐理由：** 自定义 DSH 网页背景，支持预设和本地上传
...
- **推荐理由：** LLM知识平台，支持RAG、智能推理与自维护Wiki。
...
- **标签：** `long-term-memory`, `rag`, `self-evolving`
- **推荐理由：** 自演进混合检索记忆，节省token
...
- **标签：** `dsh-plugin`, `mcp`, `rag`
- **推荐理由：** 涵盖技能、MCP、RAG、多模态工具的AI全维度百科知识库。
...
- **标签：** `rag`, `skill-retrieval`, `embeddings`, `vector-search`
- **推荐理由：** 将SKILL.md转化为可检索的Agent知识库
...
- **标签：** `agent-skill`, `knowledge-base`, `cordis`
- **推荐理由：** 官方DSH文档知识库，作为按需Agent技能使用
...
- **标签：** `dsh-plugin`, `harness`, `tutorial`
- **推荐理由：** 从零开始的DSH工程入门教程
...
- **标签：** `agent-orchestration`, `agent-workspaces`, `local-first`
- **推荐理由：** 跨设备统一Agent工作空间面板
...
- **标签：** `dsh-plugin`, `preset`, `standard`
- **推荐理由：** 两阶段预设：极简启动到标准工具
...
- **标签：** `video-generation`, `skill`, `ai-video`
...
- **标签：** `agent`, `dsh-plugin`, `skill`
- **推荐理由：** 将多个编程代理整合为一个团队，支持跨会话调度与协作。
...
- **标签：** `skills`, `agent-framework`, `orchestration`
- **推荐理由：** 将经验转化为可复用的Agent执行模式
...
- **⭐ 星标：** 162
- **语言：** TypeScript
- **描述：** DeepSeek Harness 搜番���件：对话内多源搜索番剧，卡片展示 Bangumi 评分与详情，支持复制磁力。
- **标签：** `anime-search`, `bangumi`, `torrent`
- **推荐理由：** 对话内多源搜番，展示Bangumi评分，支持复制磁力链接。
...
- **标签：** `reasonix`, `skills`, `agent`
- **推荐理由：** Superpowers技能适配Reasonix编码框架。
...
- **标签：** `agent-skills`, `chinese-wisdom`, `dashboard`
- **推荐理由：** 独特八字/风水/易经AI技能包，本地确定性引擎驱动
...
- **推荐理由：** 从DSH适配的代码简化Codex技能，提升代码质量
...
- **标签：** `evaluation`, `agent`, `rag`
- **推荐理由：** 基于证据的评估框架，适用于Agent、RAG和Prompt评测
...
- **⭐ 星标：** 16
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等���整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `document`, `chinese`, `workflow`
- **推荐理由：** 符合国标的中文公文全流程处理工具，支持格式检查和优化
...
- **标签：** `package`, `skills`, `agent`
- **推荐理由：** 42项技能+5个CLI的AI Agent资源包管理器
...
- **标签：** `director`, `media`, `ai`
- **推荐理由：** AI视频/图片/音频导演技能，支持可配置模型工具
...
- **标签：** `game`, `mission`, `skill`
- **推荐理由：** 游戏化任务系统，迷雾地图推进任务管理
...
- **标签：** `agent-skills`, `engineering`, `workflow`
- **推荐理由：** 实用的工程纪律框架，适合规范 Agent 工作流程
...
- **标签：** `godot`, `game-dev`, `gdscript`, `skill`
- **推荐理由：** Godot 4.x 全栈游戏开发技能，助力 Agent 开发
...
- **标签：** `canvas`, `creative`, `mcp`, `skills`
- **推荐理由：** 无限画布结合 Agentic 创意技能集成
...
- **标签：** `macos`, `record-replay`, `skills`, `automation`
- **推荐理由：** 将 macOS 工作流录制为可复现的 Agent 技能
...
- **标签：** `agent`, `plugin-check`, `workflow`
- **推荐理由：** 任务预检插件库并自动注入适用插件提示
...
- **推荐理由：** 在Web界面直接管理技能和MCP服务器，无需配置重启
...
- **标签：** `zhihu`, `search`, `mcp`
- **推荐理由：** 支持知乎搜索和官方API，提供Skill/CLI/MCP多种接入
...
- **推荐理由：** 粘贴JSON连接MCP服务器，热加载免重启，自动导入技能
...
- **标签：** `chatgpt`, `oauth`, `subscription`, `integration`
- **推荐理由：** 通过 OAuth 将 ChatGPT 订阅集成到 DSH 中
...
- **标签：** `design-as-code`, `agent-team`, `vibedesign`
- **推荐理由：** 开源AI矢量设计，并发Agent协作
...
- **标签：** `content-discovery`, `bilibili`, `cross-platform`
- **推荐理由：** 跨平台内容发现Agent，支持8大平台
...
- **推荐理由：** 最强免费网页搜索插件，无需注册即可为无联网能力的模型提供搜索。
...
- **标签：** `low-code`, `no-code`, `crm`, `workflows`
- **推荐理由：** AI增强的无代码平台，快速构建业务系统。
...
- **标签：** `dsh-plugin`, `oauth`, `llm`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅转为免费LLM提供商。
...
- **标签：** `cost-tracking`, `token-usage`, `quota`
- **推荐理由：** 实时追踪会话与日成本，预算管理一目了然
...
- **标签：** `compatibility`, `migration`, `ecosystem`
- **推荐理由：** 打通Pi与DSH生态，无缝迁移Pi扩展插件
...
- **标签：** `backup`, `rollback`, `crash-recovery`
- **推荐理由：** 崩溃恢复、配置回滚与安全快照，离线可用。
...
- **推荐理由：** DSH社区插件市场，一键安装awesome列表插件。
...
- **标签：** `dsh-plugins`, `utility`, `containers`
- **推荐理由：** 在隔离容器中本地运行多个DSH版本，便于管理
...
- **标签：** `config-manager`, `backup`, `mcp`
- **推荐理由：** 全面的DSH配置备份/还原/同步，支持MCP和插件迁移
...
- **标签：** `monitoring`, `cost`, `analytics`, `dashboard`
- **推荐理由：** 实时 Token 用量监控与成本可视化
...
- **标签：** `plugin-doctor`, `conflict-resolution`, `utility`
- **推荐理由：** 诊断并解决插件版本冲突问题
...
- **标签：** `security`, `dsh-plugin`, `agent`
- **推荐理由：** AI安全攻防平台，审计AI生成代码的幻觉与漏洞。
...
- **推荐理由：** 收录14类280+社区插件的精选目录，方便发现好插件
...
- **标签：** `guide`, `directory`, `navigation`
- **推荐理由：** 插件说明书与导航，含兼容性风险分析
...
- **⭐ 星标：** 1,386
- **语言：** Python
- **描述：** DSH 插件雷达与精选榜：多路��动发现 9000+ 候选，容器真实安装路径运行级实测（四档判定），精选 Top 50 · 11 类人工策展，全量索引 PLUGINS-ALL.md，自动更新。
- **标签：** `plugin-registry`, `awesome-list`
- **推荐理由：** 插件雷达榜，9000+候选真实安装级实测精选Top50
...
- **推荐理由：** 双语精选DSH插件、扩展、工具、技能与参考资源列表。
...
- **标签：** `dsh-plugin`, `armor-breaking`
- **推荐理由：** DeepSeek专用破甲插件，稳定化提示词以增强能力。
...
- **标签：** `dsh-plugin`, `education`
- **推荐理由：** 一站式学习DSH原理、浏览插件市场与最佳实践。
...
- **标签：** `awesome-list`, `deepseek-harness`, `resource`
- **推荐理由：** DSH生态最全资源索引与插件推荐指南
...
- **标签：** `awesome-list`, `mcp`, `plugin-directory`
- **推荐理由：** 精选插件、MCP、技能与界面资源汇总
...
- **标签：** `marketplace`, `plugin-discovery`, `harness`
- **推荐理由：** DSH插件发现与市场平台，生态集成利器
...
- **推荐理由：** DeepSeek Harness插件精选合集，方便发现好插件。
...
- **标签：** `awesome-list`, `directory`, `security`
- **推荐理由：** 证据驱动的插件目录，附带权限安装指南
...
- **标签：** `awesome-list`, `collection`, `community`
- **推荐理由：** DeepSeek Harness 社区插件合集
...
- **标签：** `awesome-list`, `radar`, `registry`
- **推荐理由：** 证据支持的插件雷达与2Origin实验室
...
- **标签：** `portal`, `static-site`, `directory`
- **推荐理由：** 零依赖静态门户，发现并筛选227+插件
...
- **标签：** `awesome-list`, `repository`, `plugins`
- **推荐理由：** 通用插件仓库，含验证和分析支持
...
- **推荐理由：** 每日 GitHub Actions 自动爬取的双语插件目录
