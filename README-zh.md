...
- **推荐理由：** DSH首个视觉插件，图片转结构化JSON，增强编码智能体能力。
...
- **推荐理由：** 虚拟终端+虚拟文件系统，为AI智能体提供沙盒化执行环境。
...
- **标签：** `documentation`, `code-analysis`, `agent-context`
- **推荐理由：** 分钟级生成精准技术文档和AI就绪上下文，提升代码可读性。
...
- **标签：** `vision-tools`, `computer-vision`, `ocr`
- **推荐理由：** 为纯文本模型打造强大视觉工具：图像问答与截图OCR。
...
- **推荐理由：** 编码Agent���支持RPC、子Agent和MCP协议
...
- **标签：** `cli`, `developer-tools`, `plugin-manager`
- **推荐理由：** DSH插件_hub_管理的CLI工具，开发者必备
...
- **标签：** `plugin-discovery`, `automation`, `dsh-plugin`
- **推荐理由：** 自动搜索安装验证DSH插件的一站式Skill
...
- **标签：** `plugin-development`, `dsh-plugin`, `codex`
- **推荐理由：** 可复用的插件构建与验证技能，适合快速开发DSH插件。
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** 双向桥接OpenAI Codex与DSH，同步技能与MCP。
...
- **标签：** `launcher`, `plugin`, `core`
- **推荐理由：** 释放DSH插件化内核，统一编排Runtime/MCP/Skill。
...
- **推荐理由：** Web GUI插件，在设置页管理技能与MCP服务器。
...
- **标签：** `migration`, `import`, `claude`
- **推荐理由：** 从Claude Code/Codex/OpenCode迁移会话与技能。
...
- **标签：** `plugin-dev`, `skills`, `developer`
- **推荐理由：** 开发DSH插件的Agent技能，遵循标准规范。
...
- **推荐理由：** Agent任务前自动预检插件库并注入Top-K提示。
...
- **标签：** `sidebar`, `ui-extension`, `coding-tools`
- **推荐理由：** 可扩展侧边栏，集成文件编辑/终端/Git/对话/子代理面板。
...
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH界面灵活集成。
...
- **推荐理由：** 鲸鱼主题TUI，实时状态/流式思考/双击回滚，体验流畅。
...
- **推荐理由：** 本地优先的桌面工作区，支持会话、项目和Office自动化
...
- **推荐理由：** 免安装Windows桌面客户端，含Codex、SSH和远程访问
...
- **标签：** `desktop`, `local-first`, `agent`
- **推荐理由：** 本地优先桌面Agent，多模型支持，技能自进化
...
- **推荐理由：** 将完整办公套件引入DeepSeek Harness
...
- **标签：** `ios`, `mobile`, `native-app`
- **推荐理由：** 原生iOS客户端，通过WebSocket连接Harness工作区
...
- **推荐理由：** 端到端加密多端远程访问方案，支持PC/安卓/Web
...
- **推荐理由：** 开源锤子便签复刻版，支持Docker部署与公众号导出
...
- **标签：** `plugin-market`, `webui`, `installation`
- **推荐理由：** 社区插件市场，浏览并一键安装DSH插件
...
- **推荐理由：** Electron桌面控制台多Tab管理插件与MCP
...
- **标签：** `ui`, `dsh-plugin`, `usage-tracking`
- **推荐理由：** 持久化底栏显示Provider、余额及峰谷定价信息。
...
- **标签：** `desktop-app`, `tauri`, `bioinformatics`
- **推荐理由：** 面向临床和湿实验室的DSH生物信息学桌面应用。
...
- **推荐理由：** 一站式DevOps桌面：整合数据库、SSH、Docker与AI助手。
...
- **标签：** `desktop-app`, `electron`, `dsh-integration`
- **推荐理由：** 基于Electron的桌面版DSH，集成多工具面板。
...
- **标签：** `agent-memory`, `hybrid-retrieval`, `token-savings`
- **推荐理由：** 自进化内存系统，混合检索节省35%令牌
...
- **标签：** `memory`, `rag`, `agent-skills`
- **推荐理由：** 智能体记忆管理套件，支持记忆精炼与长期上下文保持。
...
- **标签：** `memory`, `rag`, `milvus`, `persistent-storage`
- **推荐理由：** 基于Milvus的持久统一记忆层，支持所有AI智能体长期记忆。
...
- **标签：** `memory`, `long-term-memory`, `agent`
- **推荐理由：** 共享本地记忆中心，为所有AI代理提供持久上下文。
...
- **标签：** `skill-corpus`, `rag`, `agent-skills`
- **推荐理由：** 将SKILL.md转化为可检索的Agent技能库，含评估工具
...
- **推荐理由：** 全面AI百科，覆盖Agent、RAG、MLOps和学习路线
...
- **标签：** `dsh-plugin`, `memory`, `rag`, `consolidation`
- **推荐理由：** 三层自动记忆固化，零提示词注入记忆
...
- **推荐理由：** 多智能体蜂群与自主工作流的原始agent harness
...
- **推荐理由：** 530+ GPT Image提示词模板与可复用技能库
...
- **标签：** `agent-workspaces`, `skill`, `multi-device`
- **推荐理由：** 统一智能体工作空间，跨设备任务追踪
...
- **标签：** `skill-store`, `marketplace`, `skills`
- **推荐理由：** 最全技能商店，自动抓取更新万个Skills
...
- **标签：** `agent-team`, `enterprise`, `multi-agent`
- **推荐理由：** 企业级多智能体工作台，支持可视化编辑
...
- **标签：** `preset`, `workflow`, `agent-bootstrap`
- **推荐理由：** 两阶段预设：最小引导+标准工具，确保智能体稳定运行。
...
- **推荐理由：** 智能技能路由与工作流编排，提升Agent奖励21个百分点。
...
- **标签：** `cognition`, `reasoning`, `agent-skills`
- **推荐理由：** 模型无关推理时控制套件，支持深度推理/验证/恢复。
...
- **标签：** `conversation`, `canvas`, `workflow`
...
- **标签：** `creative-writing`, `agent-skills`, `novel-writing`
- **推荐理由：** 一站式小说短剧游戏视频解说创作工作流插件
...
- **标签：** `task-management`, `workflow`, `native-ui`
- **推荐理由：** 原生SQLite任务板，支持Agent认领审查，无iframe
...
- **标签：** `agent-skills`, `architecture`, `cross-platform`
- **推荐理由：** 跨平台经验复用Skill框架，支持Claude/Codex等
...
- **标签：** `agent-skills`, `skill-generator`, `learning`
- **推荐理由：** 自动将成功会话转为可复用智能体技能
...
- **标签：** `dsh-plugin`, `agent`, `mentoring`, `code-review`
- **推荐理由：** AI导师扫描项目架构并分阶引导手写代码
...
- **标签：** `dsh-plugin`, `skills`, `matt-pocock`, `workflow`
- **推荐理由：** 25个Matt Pocock工程技能开箱即用
...
- **⭐ 星标：** 52
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能���统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `dsh-plugin`, `gongwen`, `document`, `workflow`
- **推荐理由：** 党政机关公文全流程，格式检查与内容优化
...
- **标签：** `dsh-plugin`, `spec-driven`, `development`, `workflow`
- **推荐理由：** 仓库原生规格驱动开发，DSH工程模式
...
- **推荐理由：** 官方DSH知识库作为按需Agent技能，含中英文文档。
...
- **标签：** `multi-agent`, `dsh-plugin`, `skills`
- **推荐理由：** 支持多Agent协作，独立模型和技能，共享工作区。
...
- **推荐理由：** DirectorX影视创作技能，支持多模态工具配置。
...
- **推荐理由：** 11个QA测试技能框架，让AI像资深测试工程师工作。
...
- **标签：** `code-simplification`, `dsh-plugin`, `skill`
- **推荐理由：** 源自DSH内置技能的代码简化能力。
...
- **标签：** `math`, `modeling`, `agent`
- **推荐理由：** 数学建模多Agent求解器，含33个模型库。
...
- **推荐理由：** 类CodeX的DSH插件市场，Agent技能交易枢纽。
...
- **推荐理由：** 在DSH Agent内搜索npm和插件目录，提升可发现性。
...
- **标签：** `finance`, `mcp`, `tushare`, `multi-agent`
- **推荐理由：** 本地财经数据库+自动MCP接口，支持IM盯盘通知
...
- **标签：** `architecture-diagram`, `diagram-as-code`, `dsh-plugin`
- **推荐理由：** 通过代码生成精美架构图和序列图的agent技能
...
- **标签：** `design`, `vector`, `vibedesign`, `flutter`
- **推荐理由：** 开源AI原生矢量设计，并发Agent团队协作
...
- **推荐理由：** AI原生设计系统，含可视化编辑器、模板和PPT工作室。
...
- **推荐理由：** 纯文本LLM多模态视觉能力，支持截图/UI/图表分析
...
- **标签：** `dsh-plugin`, `visualization`, `legal`, `svg`
- **推荐理由：** 诉讼可视化生成可入庭审材料的时间轴
...
- **标签：** `knowledge-cards`, `dsh-plugin`, `ui`
- **推荐理由：** 将聊天记录转化为精美可下载的知识卡片。
...
- **推荐理由：** 面向Gemini、Claude和ChatGPT的浏览器增强套件
...
- **标签：** `web-automation`, `content-discovery`, `cross-platform`
- **推荐理由：** 跨平台内容发现Agent，自动抓取B站/YouTube/Reddit等多源内容。
...
- **推荐理由：** 在DSH对话内运行iOS模拟器或真机，含22个Agent工具
...
- **标签：** `routing`, `infrastructure`, `mcp`
- **推荐理由：** 任务感知推理模式路由器注入套件
...
- **标签：** `orchestration`, `long-horizon`, `control-plane`
- **推荐理由：** 跨多Harness的长程智能体编排控制面
...
- **标签：** `utility`, `model-switch`, `multi-tool`
- **推荐理由：** 一键安装切换Claude Code/Codex/DeepSeek/Kimi等多模型工具。
...
- **标签：** `llm-provider`, `oauth`, `utilities`
...
- **标签：** `llm-provider`, `command-code`, `provider`
- **推荐理由：** 添加Command Code模型提供商，支持计划感知选择
...
- **标签：** `dsh-plugin`, `plugin-registry`, `infrastructure`
- **推荐理由：** 插件生态基建，零补丁管理控制台
...
- **标签：** `dsh-plugin`, `huawei-cloud`, `mcp`, `cloud`
- **推荐理由：** 华为云官方插件含MCP工具与云沙箱
...
- **标签：** `dsh-plugin`, `awesome-list`, `directory`, `curated`
- **推荐理由：** 14类280+精选插件目录，直达链接导航
...
- **标签：** `context`, `tool`, `ui`
- **推荐理由：** 审计上下文Token消耗，检测重复与冲突的实用工具。
...
- **标签：** `utility`, `dsh-plugin`, `containers`
- **推荐理由：** 隔离容器管理多版本DSH，一键导入插件技能。
...
- **推荐理由：** 一键同步MCP、Skills配置至多种AI Agent工具。
...
- **标签：** `diagnostic`, `plugin-conflict`, `utility`
...
- **推荐理由：** 零依赖��态网页，浏览和安装227+ DSH插件的门户。
...
- **标签：** `pets`, `theme`, `pixel-art`, `mascot`
- **推荐理由：** 多智能体动画宠物主题画廊
...
- **标签：** `theme`, `ocean`, `animation`
- **推荐理由：** 实时海洋主题，含动态波浪、日落效果和玻璃质感
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪和GPU管理
...
- **标签：** `awesome-list`, `discovery`, `research`
- **推荐理由：** 每日精选插件发现，分类排行与使用指南
...
- **标签：** `awesome-list`, `reference`, `curation`
- **推荐理由：** 全面的DSH插件技能MCP服务器精选列表
...
- **推荐理由：** DeepSeek Harness终极入门指南与资源合集
...
- **标签：** `academic-search`, `mcp`, `citation`
- **推荐理由：** 跨库学术检索+引用核验，支持中文科研用户
...
- **标签：** `handbook`, `agent-runtime`, `documentation`
- **推荐理由：** 173个主题权威手册，覆盖运行时/插件/MCP等
...
- **标签：** `dsh-plugin`, `api-docs`, `chinese-platforms`, `research`
...
- **标签：** `dsh-plugin`, `patent`, `infringement`, `legal-research`
- **推荐理由：** 专利侵权分析并生成竞品报告
...
- **标签：** `dsh-plugin`, `chinese-wisdom`, `bazi`, `prediction`
- **推荐理由：** 八字/风水/周易预测引擎，本地确定性推理
...
- **推荐理由：** 实时追踪Token用量、API费用及余额的可视化图表。
...
- **推荐理由：** GitHub Actions自动抓取的双语插件目录，每日更新。
...
- **推荐理由：** 证据支持的插件雷达，含2Origin插件实验室分析。
...
- **标签：** `awesome-list`, `review`
- **推荐理由：** 基于证据的插件目录，具备不可变提交审核
...
- **推荐理由：** 涵盖CLI、Web UI和代理工具的插件生态精选列表。
...
- **标签：** `novel-writing`, `rag`, `creative-writing`
- **推荐理由：** 支持世界观构建、角色管��和审稿的流程化创作工具。
...
- **推荐理由：** 11,000+插件市场目录，含搜索排名与公共API
...
- **推荐理由：** 1958仓库/1819插件目录，中英搜索与分类筛选
...
- **标签：** `awesome-list`, `productivity`, `plugins`
- **推荐理由：** 精选涵盖生产力工具和DSH开发资源列表
...
- **标签：** `dsh-plugins`, `dsh-plugin-verify`, `dsh-ecosystem`
- **推荐理由：** 含插件验证和平台分析的DSH插件仓库。
