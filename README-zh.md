...
- [UI 与桌面扩展](#ui-与桌面扩展) — 18 个插件
- [记忆与知识（RAG）](#记忆与知识（rag）) — 11 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 23 个插件
- [MCP 服务器与工具](#mcp-服务器与工具) — 8 个插件
- [视觉与设计](#视觉与设计) — 10 个插件
...
- [UI 与桌面扩展](ui-desktop-extensions-zh) (18)
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (11)
- [Agent 技能与工作流](agent-skills-workflows-zh) (23)
- [MCP 服务器与工具](mcp-servers-tools-zh) (8)
...
- **标签：** `coding-agent`, `terminal`, `tui`
- **推荐理由：** DeepSeek原生终端编码代理，支持前缀缓存
...
- **标签：** `ai-agents`, `cordis`, `dsh-plugin`
- **推荐理由：** 运行时注入器与任务感知推理模式路由套件
...
- **标签：** `agent-os`, `coding-agent`, `loop-engineering`, `mcp`
- **推荐理由：** 自进化Agent OS，内置评估循环持续优化智能
...
- **标签：** `agent-collaboration`, `enterprise-agent-workspace`, `multi-agent-workspace`
- **推荐理由：** 企业级多代理工作区，统一Codex/DSH/Opencode引擎
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 两阶段预设：最小引导启动后加载标准工具集
...
- **推荐理由：** 一键切换多模型，支持DeepSeek/Claude/Codex等
...
- **标签：** `dsh-plugin`, `agent-runtime`, `ai-coding`
- **推荐理由：** Rust高性能Agent运行时+精美桌面AI编程IDE
...
- **标签：** `dsh-plugin`, `code-review`, `developer-tools`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查工具
...
- **标签：** `coding-agent`, `architecture`, `spec-driven`
- **推荐理由：** 架构感知的AI编程，基线优先并防漂移检查。
...
- **⭐ 星标：** 1,095
- **语言：** Rust
- **描述：** Built by DeepSeek, for DeepSeek — a Swift-native macOS coding agent
- **标签：** `macos`, `swift`, `coding-agent`
- **推荐理由：** DeepSeek官方Swift原生macOS编程代理。
...
- **标签：** `coding-agent`, `team`, `automation`
- **推荐理由：** 将多个编码代理整合为协同工作团队
...
- **推荐理由：** 多Agent团队协作，独立模型/技能/MCP，共享���作区
...
- **标签：** `agent-framework`, `dsh-plugin`, `claude-code`, `python`
- **推荐理由：** 42个技能+多CLI的AI Agent资产包管理器
...
- **标签：** `plugin-development`, `cordis`, `dsh-plugin`
- **推荐理由：** 可移植插件开发与审计Agent Skill
...
- **标签：** `best-practices`, `documentation`, `dsh-plugin`
- **推荐理由：** DSH插件开发踩坑指南与最佳实践档案
...
- **标签：** `agent-skills`, `best-practices`, `dsh-plugin`
- **推荐理由：** DSH工程纪律与实践方法提炼
...
- **标签：** `dsh-plugin`, `agent-skills`, `development`
- **推荐理由：** DSH插件脚手架与测试Agent技能
...
- **标签：** `plugin-manager`, `gui`, `hot-reload`
- **推荐理由：** 内嵌插件管理GUI，支持热加载无需重启
...
- **标签：** `desktop`, `docker`, `ssh`, `database`
- **推荐理由：** 集成数据库、SSH、Docker面板的一体化桌面应用
...
- **标签：** `agent-tools`, `plugin-checklist`
- **推荐理由：** 任务前自动预检并注入Top-K插件，提升DSH代理利用率。
...
- **标签：** `cli`, `plugin-manager`, `dsh-plugin`
- **推荐理由：** CLI插件管理工具，支持安装更新与审计
...
- **标签：** `agent-orchestration`, `coding-agent`, `mcp`, `no-code`
- **推荐理由：** 跨设备统一代理工作空间，任务看板可追踪管理
...
- **标签：** `dsh-plugin`, `ui-extension`
- **推荐理由：** 零依赖Web版竹知了玩具模拟，移动端优先
...
- **标签：** `dsh-plugin`, `sidebar`
- **推荐理由：** 开放侧边栏底座，支持文件编辑/终端/Git/子代理
...
- **标签：** `dsh-plugin`, `tui`, `terminal`
- **推荐理由：** 鲸鱼顶栏TUI，实时状态与流式思考体验
...
- **标签：** `desktop`, `electron`, `pets`
- **推荐理由：** 桌面伴侣带动画宠物和插件SDK。
...
- **标签：** `desktop`, `tauri`, `cross-platform`
- **推荐理由：** 轻量5MB桌面应用，零环境配置即可使用。
...
- **标签：** `widget`, `ui`, `floating`
- **推荐理由：** 可爱浮窗鲸鱼小部件监控账户余额。
...
- **标签：** `desktop`, `workspace`, `local-first`
- **推荐理由：** 本地优先桌面工作空间，支持会话和文件管理。
...
- **标签：** `ui-components`, `charts`, `interactive`
- **推荐理由：** 内联生成图表、表单、测验等交互UI组件
...
- **标签：** `task-management`, `ui`, `sqlite`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核
...
- **标签：** `ui`, `usage-tracking`, `deepseek-harness`
- **推荐理由：** 显示模型/余额/计费的底部信息栏
...
- **⭐ 星标：** 9
- **语言：** JavaScript
- **描述：** HUD status panel plugin for DeepSeek Harness (dsh) web: git status, MCP servers, skills, model & token usage in a floating panel
- **标签：** `hud`, `status-panel`, `ui`
- **推荐理由：** 浮动HUD面板显示Git状态、MCP和服务使用信息
...
- **标签：** `desktop-app`, `electron`
- **推荐理由：** Electron桌面版DSH，集成web UI、侧边栏及ModLens等多插件。
...
- **标签：** `agent-memory`, `rag`, `long-term-memory`, `mcp`, `skills`
- **推荐理由：** 所有AI代理的便携式本地优先记忆层，用户拥有数据
...
- **标签：** `long-term-memory`, `mcp`, `rag`, `self-evolving`, `skills`
- **推荐理由：** 自演化记忆OS，混合检索节省35%token，跨任务复用技能
...
- **标签：** `memory`, `r ag`, `deepseek-harness`
- **推荐理由：** 为AI代理提供无限持久化记忆能力。
...
- **标签：** `knowledge-base`, `rag`, `ai-resources`
- **推荐理由：** 涵盖RAG、MCP、Agent的全方位AI知识库
...
- **标签：** `memory`, `rag`, `skills`, `retrieval`
- **推荐理由：** 将SKILL.md转为可检索的智能体技能库
...
- **标签：** `memory`, `handoff`, `skills`
- **推荐理由：** 连接PowerContext实现记忆recall和技能交接
...
- **标签：** `claude`, `memory`, `bridge`
- **推荐理由：** 桥接Claude Code记忆和技能到DeepSeek Harness
...
- **标签：** `multi-agent`, `skills`, `memory`
- **推荐理由：** 多模型通用代理，具自演化记忆与技能
...
- **推荐理由：** 基于J-space全局工作空间的AI认知增强技能套件
...
- **标签：** `agent-skills`, `dsh-plugin`, `workflow-automation`
- **推荐理由：** 智能编排技能与工作流程的通用Agent技能框架
...
- **标签：** `dsh-plugin`, `video-generation`, `agent-skills`
- **推荐理由：** 从话题到成品Vox风视频的一站式自动生成技能
...
- **标签：** `vision`, `multimodal`, `agent-skills`
- **推荐理由：** 视觉工具包让纯文本模型具备图像理解能力。
...
- **⭐ 星标：** 1,088
- **语言：** Go
- **描述：** AgentRQ: Human-in-loop realtime conversational task manager for AI Agents. Self-hosted! Control your own agents from wherever you want Mobile, Web, Desktop. Designed to work well with your own Claude subscriptions and any harness.
- **标签：** `task-manager`, `human-in-loop`, `agent-workflow`
- **推荐理由：** 自托管人工干预任务管理器，支持移动端控制。
...
- **标签：** `workflow`, `browser-extension`, `productivity`
- **推荐理由：** 将AI对话转化为可复用的组织化工作流。
...
- **标签：** `agent-teams`, `multi-agent`, `coordination`
- **推荐理由：** 支持多代理团队协作处理复杂任务。
...
- **⭐ 星标：** 817
- **语言：** TypeScript
- **描述：** [dsh]为纯文本模型设计更强大的视觉工具箱：安装免费使用、粘贴图片直接识别、多张图片问答、截图到前端UI 还原等｜DeepSeek Harness-native integration for agent-vision-toolkit: image Q&A, long-screenshot OCR, UI restoration, grounding, pixel diff, Artifacts, and Web UI.
- **标签：** `vision`, `ocr`, `native-integration`
- **推荐理由：** 原生视觉工具包，支持图像问答和OCR还原。
...
- **标签：** `skills`, `infra`, `coding`
- **推荐理由：** AI基础设施开发的专用智能体技能集合
...
- **推荐理由：** Superpowers技能移植到Reasonix编码框架
...
- **标签：** `agent-skills`, `session-memory`, `context-engineering`
- **推荐理由：** 跨平台Agent会话记忆，实现任务连续性交接
...
- **标签：** `dsh-plugin`, `skill-picker`, `ux`
- **推荐理由：** 类WorkBuddy技能选择器，快速插入官方技能命令
...
- **标签：** `agent-skills`, `code-simplification`, `codex`
- **推荐理由：** 源自DSH的代码简化技能，适配Codex使用
...
- **标签：** `agent-skills`, `governance`, `swift-cycle`
- **推荐理由：** Swift Cycle治理技能适配DSH，版本锁定可离线验证
...
- **标签：** `gongwen`, `docx`, `ai-docs`, `deepseek-harness`
...
- **标签：** `record-replay`, `agent-skills`, `macos`
- **推荐理由：** 录制桌面操作自动生成可复用Agent技能
...
- **⭐ 星标：** 10
- **语言：** Python
- **描述：** DeepSeek Harness 0.1.0-rc.6 数学建模增强套件：外置插件 + Preset + 17 个手动 Skill 卡片 + Windows 便携版脚本
- **标签：** `math-modeling`, `skills`, `preset`
- **推荐理由：** 数学建模增强套件，含17个预设技能卡片
...
- **标签：** `agent-preset`, `product-management`
- **推荐理由：** 虚拟产品团队AI代理预设，模拟PM到QA完整流程辅助项目开发。
...
- **标签：** `agent-skills`, `multimodal`, `ocr`, `vision`
- **推荐理由：** 首个视觉插件，文字代理的视觉桥梁，图像转结构化JSON
...
- **标签：** `web-search`, `mcp`, `free`
- **推荐理由：** 免费全网搜索桥接插件，无需API密钥
...
- **推荐理由：** Codex与DSH双向同步桥接，支持MCP自动镜像
...
- **标签：** `capability-menu`, `mcp`, `skills`
- **推荐理由：** 统一管理MCP工具和技能的暴露级别与执行模式
...
- **推荐理由：** DSH代理内置插件搜索工具，支持npm及精选列表查询。
...
- **标签：** `chatgpt`, `oauth`
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内使用订阅模型聊天。
...
- **标签：** `design-systems`, `figma-alternative`, `ui-generator`
- **推荐理由：** AI代理的开源设计系统与原型工具
...
- **标签：** `diagrams-as-code`, `architecture`, `mermaid`
- **推荐理由：** 生成精美架构与序列图的代理技能
...
- **标签：** `image-prompts`, `prompt-engineering`, `skills`, `workflow-automation`
- **推荐理由：** 470+工业级图像提示词与模板，逆向工程精炼
...
- **标签：** `agent-team`, `flutter`, `react`, `skill`, `vibecoding`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持并发Agent团队
...
- **⭐ 星标：** 16
- **语言：** Python
- **描述：** Free image reading & generation for DeepSeek Harness (rc.7/rc.8/v0.1.1-rc.1) — paste-image reading, DeepSeek-V4-Flash-Vision-Exp / GLM-4V-Flash / SenseNova / Gemini failover, Kolors generation. No keys in repo.
- **标签：** `vision`, `image-generation`, `media`
- **推荐理由：** 图像读取与生成技能，支持多视觉模型
...
- **标签：** `design`, `vibe-coding`, `ui`
- **推荐理由：** 6种设计风格技能包，避免AI感，提升网站审美
...
- **标签：** `ai-video`, `ai-image`, `directorx`, `visual`
- **推荐理由：** AI视频/图像/音频创作与分镜技能
...
- **标签：** `canvas`, `agentic-workflow`, `mcp`, `creative`
- **推荐理由：** 无限画布创意工作流与MCP工具集成
...
- **推荐理由：** 增强Gemini/Claude/ChatGPT的浏览器扩展套件
...
- **标签：** `ai-agent`, `dsh-plugin`, `browser-use`
- **推荐理由：** 本地优先的跨平台AI内容发现Agent，覆盖8+平台
...
- **标签：** `dsh-plugin`, `browser-use`, `agent`
- **推荐理由：** 让AI Agent使用真实登录浏览器进行自动化操作
...
- **⭐ 星标：** 405
- **语言：** TypeScript
- **描述：** dsh plugin: Chrome sidebar extension that lets DeepSeek Harness operate your browser directly, no vision capabilities required. 一款 Chrome 侧边栏扩展程序，可让 DeepSeek Harness 直接操控您的浏览器，无需视觉能力。
- **标签：** `browser-automation`, `chrome-extension`
- **推荐理由：** Chrome侧边栏扩展，让DSH直接操控浏览器，无需视觉能力
...
- **标签：** `community`, `social`, `oauth`
- **推荐理由：** 内测收官合影墙，GitHub OAuth零权限登录
...
- **标签：** `agent-sandbox`, `agent-tools`, `vfs`, `virtual-filesystem`
- **推荐理由：** 统一虚拟文件系统，跨环境支持所有AI代理
...
- **标签：** `dsh-plugin`, `awesome-list`, `plugin-registry`
- **推荐理由：** 9000+候选精选Top50插件雷达，真实安装级实测
...
- **标签：** `oauth`, `llm-provider`, `utility`
- **推荐理由：** 通过OAuth将ChatGPT/Claude订阅用作DSH的LLM提供商
...
- **标签：** `backup`, `crash-recovery`, `utility`
- **推荐理由：** 崩溃恢复与一键回滚快照，安全可靠
...
- **标签：** `migration`, `context`, `utility`
- **推荐理由：** 跨预设会话迁移，保留完整上下文状态
...
- **标签：** `config`, `backup`, `migration`
- **推荐理由：** DSH配置完整备份恢复，支持WebDAV同步迁移
...
- **标签：** `token-usage`, `visualization`, `cost`
- **推荐理由：** 实时Token用量图表和API余额追踪
...
- **标签：** `utilities`, `plugin-manager`
- **推荐理由：** 诊断并解决DSH插件间及版本间的兼容冲突问题。
...
- **标签：** `awesome-list`, `catalog`, `dsh-plugin-directory`
- **推荐理由：** GitHub Actions自动爬取的双语插件目录
...
- **标签：** `dsh-plugin`, `plugin-directory`, `registry`
- **推荐理由：** 覆盖模型、工具、UI的DSH插件综合目录
...
- **标签：** `dsh-plugin`, `industry`
- **推荐理由：** 金融、法律、医疗等行业专用DSH插件
...
- **标签：** `registry`, `plugin-manager`, `discovery`
- **推荐理由：** 多源合并去重的统一插件注册表
...
- **⭐ 星标：** 1
- **语言：** JavaScript
- **描述：** Awesome DSH Cordis Plugins — the official, auto-generated list of DeepSeek Harness Cordis plugins, curated by cordis.run
- **标签：** `dsh-plugin`, `awesome-list`
- **推荐理由：** cordis.run官方自动生成的Cordis插件清单
...
- **⭐ 星标：** 1
- **语言：** JavaScript
- **描述：** DSH plugin: one-click publish planner for skills/plugins — validate completeness, generate repo description, awesome-list entry, PR commands. 技能/插件一键发布助手
- **标签：** `dsh`, `plugin-publisher`
- **推荐理由：** 一键发布插件，自动验证并生成PR
...
- **标签：** `cli`, `developer-tools`, `mascot`, `pixel-art`
- **推荐理由：** 多AI编码代理的动态宠物画廊，个性化桌面体验
...
- **标签：** `theme`, `ui`, `customization`
- **推荐理由：** 高度可定制的磨砂玻璃主题，全界面自由调节
...
- **标签：** `color-scheme`, `theme`, `dark-mode`, `ui`
...
- **标签：** `agent`, `agentic-ai`, `harness`, `llm`
- **推荐理由：** 零到一的DeepSeek Harness入门教程，系统全面
...
- **标签：** `dsh-plugin`, `research`, `web-search`
- **推荐理由：** 自动搜索国内8大平台近30天内容生成研究报告
...
- **⭐ 星标：** 269
- **语言：** JavaScript
- **描述：** 用 30 秒为你的 DeepSeek Harness找到合适的插件。 仓库由脚本每天自动抓取，再经人工逐个核实——真插件进目录，蹭热度的进黑名单，每条剔���理由公开可查。并告诉你每个插件适合谁、从哪里开始。
- **标签：** `awesome-list`, `verified`, `curated`
- **推荐理由：** 每日自动抓取并人工核实的插件目录
...
- **标签：** `documentation`, `guides`, `best-practices`
- **推荐理由：** 学习DSH原理、插件市场与最佳实践
...
- **标签：** `awesome-list`, `guide`, `bilingual`
- **推荐理由：** 终极双语指南，含快速入门、资源与精选插件
...
- **标签：** `kline`, `analysis`, `market`, `deepseek-harness-plugin`
- **推荐理由：** 支持多市场K线分析、技术指标与基本面研究
...
- **标签：** `stock`, `finance`, `quant`, `awesome-list`
- **推荐理由：** 精选股票/金融/量化插件列表
...
- **标签：** `ai-resources`, `tutorial`, `prompt-engineering`
- **推荐理由：** 开发者AI资源大全与Vibe Coding教程
...
- **推荐理由：** DeepSeek Harness精选插件列表，社区维护
...
- **标签：** `awesome-list`, `resources`, `directory`
- **推荐理由：** DSH插件、工具和资源的全面精选列表。
...
- **标签：** `chrome-extension`, `deepseek`, `deepseek-harness`, `dsh`, `dsh-plugin`, `ocean-skin`, `theme`, `threejs`, `webgpu`
- **推荐理由：** WebGPU ocean skin for DeepSeek Harness — DSH plugin, Harness-only Chrome/Edge ex
...
- **标签：** `agent`, `agent-framework`, `ai-agent`, `ai-agents`, `awesome`, `awesome-list`, `coding-agent`, `deepseek`, `deepseek-ai`, `deepseek-harness`, `deepseek-harness-plugin`, `deepseek-harness-plugins`, `dsh`, `dsh-patch`, `dsh-plugin`, `dsh-plugins`, `llm`, `llm-agent`, `mcp`, `plugins`
- **推荐理由：** A curated list of plugins, skills, MCP servers, patch/profile layers, orchestrat
...
- **推荐理由：** A visual, non-linear conversation workspace plugin for DeepSeek Harness ; A canv
...
- **推荐理由：** Open-source Windows desktop client and GUI for DeepSeek Harness — zero-setup ins
...
- **标签：** `cost-tracking`, `deepseek`, `deepseek-api`, `deepseek-harness`, `dsh`, `dsh-plugin`, `dsh-plugins`, `harness`, `llm`, `plugins`, `token-usage`
- **推荐理由：** DeepSeek Harness session cost meter plugin: session/daily cost, budget, history,
...
- **推荐理由：** DeepSeek Harness plugin store, marketplace and hub — 3,100+ dsh plugins with sea
...
- **标签：** `deepseek-harness`, `deepseek-harness-plugin`, `deepseek-v4-pro`, `harness`, `harness-ai`, `plugin-marketplace`, `plugins`
- **推荐理由：** DeepSeek-Harness-Plugins
...
- **标签：** `anime`, `anime-search`, `bangumi`, `deepseek`, `deepseek-harness`, `dsh`, `dsh-plugin`, `magnet`, `mikan`, `plugin`, `torrent`, `typescript`
- **推荐理由：** DeepSeek Harness 搜番插件：对话内多源搜索番剧，卡片展示 Bangumi 评分与详情，支持复制磁力。
...
- **标签：** `ai-agents`, `creative-writing`, `deepseek-harness`, `drama-skills`, `dsh-plugin`, `fiction-writing`, `novel-writing`, `oh-story`, `screenwriting`, `short-drama`
- **推荐理由：** A DSH plugin for novel writing and short-drama production, powered by Oh Story a
...
- **标签：** `ai-agent`, `cordis`, `deepseek`, `deepseek-harness`, `dsh-plugin`, `gemini`, `image-generation`, `openai`, `plugin`, `seedream`
- **推荐理由：** Generate images directly in DeepSeek Harness chats
...
- **标签：** `awesome-list`, `community`, `resource`
- **推荐理由：** DSH插件精选清单，便于发现和选择
...
- **标签：** `marketplace`, `webui`, `plugin`
- **推荐理由：** 集成到Web GUI的社区插件市场
...
- **推荐理由：** 🐳 DeepSeek Harness 插件聚合社区 — 自动同步 dsh-plugin 生态 · 精选目录 · 每 4 小时自动维护 | Oh-My-DSH:
...
- **推荐理由：** Complete reverse-skill (85 SKILL.md) as a DeepSeek Harness (dsh) Cordis plugin —
...
- **推荐理由：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、生成阶梯式 Chapter、
...
- **推荐理由：** DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
...
- **推荐理由：** 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接搜索 20 个中国开放平台的
...
- **推荐理由：** DSH Desktop Hub — DeepSeek Harness 桌面管理控制台（Electron + TypeScript）。多 Tab 管理 Harne
...
- **推荐理由：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，或直接读案件材料画准一张时间轴。
...
- **推荐理由：** An Agent Skills skill for developing DeepSeek Harness (DSH) plugins（开发 DSH 插件的 A
...
- **推荐理由：** Portable Agent Skill for repository-native Spec programming, informed by public 
...
- **推荐理由：** DSH plugin: pixel-to-text image reading for text-only models. image_scan/image_o
...
- **推荐理由：** Agent skill for installed DeepSeek Harness plugins (slots, Typert remotes, crede
...
- **推荐理由：** Installable DSH bundle: the dsh-plugin-guide plugin-development knowledge base a
...
- **推荐理由：** 收录280+个DSH插件的精选目录，分类清晰便于查找
...
- **推荐理由：** DeepSeek Harness社区非官方插件目录
...
- **标签：** `awesome-list`, `registry`, `open-data`
- **推荐理由：** 防垃圾过滤的DSH插件和技能开放数据注册表
...
- **推荐理由：** 2026年度DSH插件精选清单，附质量审核，社区参考首选。
...
- **标签：** `plugin-directory`, `awesome-list`
- **推荐理由：** 按场景推荐插件，含安装权限兼容性指南，新手友好。
...
- **推荐理由：** 数据驱动的插件雷达图与2Origin实验室，生态分析工具。
...
- **推荐理由：** 零依赖静态插件门户，支持227+插件筛选与一键安装。
...
- **标签：** `awesome-list`, `archived`
- **推荐理由：** 已迁移，请访问新仓库 awesome-dsh-plugin/awesome-dsh-plugin。
...
- **标签：** `awesome-list`, `ai-curated`
- **推荐理由：** AI每小时更新的190+插件分类目录，覆盖12大类别。
...
- **标签：** `plugin-repository`, `dsh-bundle`
- **推荐理由：** DSH插件仓库与打包集合，补充生态插件资源。
