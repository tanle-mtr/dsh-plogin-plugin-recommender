...
- **标签：** `agent-orchestration`, `workspace`, `mcp`
- **推荐理由：** 全平台统一的Agent工作区，支持多设备任务跟踪与API访问。
...
- **标签：** `cybersecurity`, `redteam`, `code-audit`
- **推荐理由：** 审计AI生成代码的幻觉包、漏洞和安全缺陷。
...
- **推荐理由：** 支持子Agent、Hashline编辑和MCP的编码代理
...
- **标签：** `plugin-dev`, `agent-skill`, `codex-compatible`
- **推荐理由：** 开发 DSH 插件的标准 Skill，覆盖服务/事件/工具/适配器全流程。
...
- **标签：** `documentation`, `knowledge-base`, `plugin-dev`
- **推荐理由：** 官方文档知识库，按需调用，中英双语支持。
...
- **推荐理由：** 可复用的插件创建与验证 Skill，兼容 Codex。
...
- **推荐理由：** Codex 与 DSH 双向同步，实现聊天和 MCP 镜像。
...
- **标签：** `dsh-plugin`, `skill`, `plugin-development`
- **推荐理由：** 指导AI智能体从零开发、构建和测试DSH插件。
...
- **标签：** `agent-skills`, `plugin-development`, `dsh-plugin`
- **推荐理由：** 便携式插件开发审计技能，支持Bundle适配器。
...
- **标签：** `web-ui`, `plugin-ecosystem`, `dsh-web`
- **推荐理由：** 基于Web的DSH插件聚合生态，万物皆插件，创意工坊分发。
...
- **标签：** `dsh`, `floating-widget`, `deepseek-harness`
- **推荐理由：** 可爱的浮动鲸鱼小部件实时监控DeepSeek账户余额。
...
- **标签：** `desktop-app`, `electron`, `dsh-plugin`, `agentic-workflows`
- **推荐理由：** 本地优先的Electron桌面工作空间，支持会话与办公自动化。
...
- **标签：** `genui`, `inline-ui`, `interactive-components`
- **推荐理由：** 在聊天回复中直接渲染交互式UI组件和图表。
...
- **标签：** `desktop-app`, `ssh-client`, `remote-access`
- **推荐理由：** 零配置Windows桌面客户端，内置SSH和移动端远程访问。
...
- **推荐理由：** 跨平台桌面客户端，支持macOS/Windows/Linux
...
- **推荐理由：** DSH Web GUI 社区插件市场，一键安装卸载
...
- **标签：** `desktop`, `electron`, `typescript`
- **推荐理由：** Electron桌面控制台，多Tab管理插件、MCP和技能
...
- **推荐理由：** 为纯文本模型提供图像 OCR 和像素到文本的视觉能力。
...
- **推荐理由：** Web GUI 技能与 MCP 管理卡片，浏览器内操作。
...
- **标签：** `desktop`, `docker`, `ssh`, `dsh-plugin`
- **推荐理由：** 一站式DevOps桌面，集成数据库、SSH、Docker和AI助手。
...
- **标签：** `rag`, `knowledge-base`, `llm`
- **推荐理由：** LLM知识平台，将文档转为可查询RAG和维基
...
- **标签：** `memory`, `token-savings`, `rag`
- **推荐理由：** 自进化记忆系统，混合检索+省35% token，高效持久。
...
- **标签：** `memory`, `agentic-rag`, `stateful`
- **推荐理由：** 状态化Agent记忆库，支持持续性学习与检索增强。
...
- **标签：** `cordis-plugin`, `dsh-plugin`, `context-management`
- **推荐理由：** DSH最佳上下文管理插件，含仪表盘和统计功能。
...
- **标签：** `agent-memory`, `semantic-search`, `reranking`
- **推荐理由：** 将分散技能文件转化为可检索的RAG知识库。
...
- **标签：** `routing`, `workflow`, `cordis`
- **推荐理由：** 任务感知路由套件，智能分发任务到最佳处理模式。
...
- **标签：** `skills`, `marketplace`, `agent-skills`
- **推荐理由：** 最全最快更新的技能商店，自动抓取整理上万个Skills项目。
...
- **推荐理由：** 自进化Agent操作系统，Agent可自主学习和持续提升。
...
- **标签：** `loop-engineering`, `agent-control`, `workflow-automation`
- **推荐理由：** 长程Agent控制平面，支持Codex/Claude等多Harness协调作业。
...
- **推荐理由：** 企业级本地优先多Agent工作台，支持可视化编辑和团队协。
...
- **标签：** `conversation-canvas`, `non-linear`, `workspace`
- **推荐理由：** 基于画布的非线性对话工作空间，支持会话分支探索。
...
- **推荐理由：** SQLite本地任务管理，Agent协作审批，原生Web UI。
...
- **标签：** `creative-writing`, `drama-skills`, `fiction`
- **推荐理由：** 小说创作和短剧制作技能，支持Oh Story引擎
...
- **标签：** `self-evolving`, `review`, `agent-review`
- **推荐理由：** 可组合任务控制，实现Agent自我进化和人工审查
...
- **标签：** `plugin-discovery`, `install`, `verify`
- **推荐理由：** 搜索、安装并验证GitHub插件的技能工具
...
- **标签：** `session`, `migration`, `workflow`
- **推荐理由：** 跨预设会话迁移，保留状态与意图一致性
...
- **标签：** `superpowers`, `tdd`, `skills`
- **推荐理由：** 适配DSH的TDD、调试与规划智能体技能
...
- **标签：** `reverse`, `security`, `ctf`
- **推荐理由：** 87个逆向工程与安全测试智能体技能
...
- **标签：** `agent-skills`, `research`, `workflows`
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销和商业工作流
...
- **⭐ 星标：** 68
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、��成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `agent-skills`, `coding`, `mentor`
- **推荐理由：** AI编程导师技能，扫描项目架构并生成阶梯式课程
...
- **标签：** `skill-generator`, `agent-learning`
...
- **标签：** `skills-deck`, `engineering`, `efficiency`
- **推荐理由：** 将 Matt Pocock 的 25 个工程技能注入 DSH 面板。
...
- **标签：** `spec-programming`, `agent-skill`, `vibe-coding`
- **推荐理由：** 便携式规格驱动开发 Skill，适合规范编程和 Vibe Coding。
...
- **标签：** `gongwen`, `docx`, `office`, `document-processing`
- **推荐理由：** 中文公文全流程处理：格式检查、优化、模板生成。
...
- **推荐理由：** 多 Agent 协作，支持独立模型/技能/MCP 和共享工作区。
...
- **推荐理由：** 统一管理并安全加载 DSH 本地 Agent Skills。
...
- **推荐理由：** 类 WorkBuddy 技能选择器，提升 DSH 使用体验。
...
- **推荐理由：** 源自 DSH 的代码简化技能，适配 Codex 工作流。
...
- **推荐理由：** 10 个 QA 技能框架，支持测试自动化与用例生成。
...
- **标签：** `agent-framework`, `skills`, `mcp`, `database`
- **推荐理由：** 42 个技能和 5 个 CLI，覆盖 AI 代理工程全链路。
...
- **标签：** `media`, `image`, `vision`, `skill`
- **推荐理由：** 支持图片读取与生成，自动视觉转码。
...
- **标签：** `agent-evaluation`, `deepseek-harness`, `dsh-plugin`, `rag-evaluation`
- **推荐理由：** 为提示词、RAG、智能体和技能提供证据驱动的评估与可观测性。
...
- **标签：** `agent-skills`, `swift-cycle`, `dsh-plugin`
- **推荐理由：** Swift Cycle治理技能适配器，支持版本锁定和离线验证。
...
- **标签：** `agent-tools`, `checklist`
- **推荐理由：** 飞行前检查单，自动注入最相关插件
...
- **标签：** `vision-tools`, `agent-skills`, `dsh-plugin`, `ocr`
- **推荐理由：** 为纯文本模型打造视觉工具箱：OCR、多图问答一行搞定。
...
- **推荐理由：** 通过OAuth使用订阅账号作为LLM提供商，无需API密钥。
...
- **标签：** `mcp`, `multi-agent`, `config-sync`
- **推荐理由：** 一键为多个 AI Agent 工具配置 MCP 和 Skills。
...
- **标签：** `cloud`, `mcp`, `huawei`, `terraform`
- **推荐理由：** 华为云官方插件，含 MCP 工具与云沙箱能力。
...
- **推荐理由：** 通过OAuth在DSH中使用ChatGPT模型对话
...
- **标签：** `diagrams-as-code`, `architecture-diagram`, `mermaid`
- **推荐理由：** 代码生成精美架构图与动态流程演示
...
- **推荐理由：** 首个开源AI原生矢量设计工具，支持并发Agent团队协作设计。
...
- **标签：** `pet`, `mascot`, `ui-personalization`
- **推荐理由：** 多Agent平台动画宠物画廊，为编码体验增添趣味与个性。
...
- **标签：** `video-generation`, `ai-video`, `dsh-plugin`, `ffmpeg`
- **推荐理由：** 端到端自动生成Vox风格拼贴解说视频，支持ffmpeg。
...
- **标签：** `ai-design`, `dsh-plugin`, `presentation`
- **推荐理由：** AI驱动的可视化设计工作室，支持PPT生成与模板市场。
...
- **标签：** `image-generation`, `multi-backend`, `visual`
- **推荐理由：** 在DeepSeek Harness对话中直接生成图像。
...
- **推荐理由：** 在对话中预览和编辑OpenPencil原型设计文档
...
- **标签：** `design`, `ui`, `vibe-coding`, `style`
- **推荐理由：** 6 种设计风格预设，让 AI 生成页面更精美。
...
- **标签：** `ai-video`, `ai-image`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 提供AI视频/图像/音频生成技能和配置化视觉模型工具。
...
- **推荐理由：** 让AI通过CLI和扩展在已登录的真实浏览器中自动化操作。
...
- **标签：** `ios-automation`, `mobile-testing`, `uiautomation`
- **推荐理由：** 在对话中集成iOS模拟器和真机自动化，22项工具。
...
- **标签：** `dingtalk`, `chat-bot`
- **推荐理由：** 钉钉IM渠道支持智能体工具与已读回执
...
- **推荐理由：** 自动发现15900+候选插件并提供k8s级实测验证目录。
...
- **推荐理由：** 开源锤子便签，支持Docker私有化与skill调用
...
- **推荐理由：** 安全P2P远程访问方案，支持PC/Android/Web多端
...
- **标签：** `plugin-management`, `console`, `infrastructure`
- **推荐理由：** DSH插件生态基建，控制台管理和开发引导
...
- **标签：** `plugin-mgmt`, `credentials`, `slots`
- **推荐理由：** 通过 Skill 管理已安装插件的插槽、远程和凭证。
...
- **标签：** `upgrade`, `automation`, `plugin-maintenance`
- **推荐理由：** 自动跟随 DSH 版本升级，简化插件维护流程。
...
- **标签：** `docker`, `multi-version`, `management`
- **推荐理由：** 本地多版本隔离运行 DSH，嵌入 WebView 管理界面。
...
- **标签：** `context-audit`, `token-optimization`, `ui-panel`
- **推荐理由：** 上下文审计工具，统计 token 成本并检测重复冲突。
...
- **标签：** `agent-migration`, `configuration-migration`, `dsh-plugin`
- **推荐理由：** 一键迁移Claude Code等配置到DSH，支持技能和MCP配置。
...
- **标签：** `ai-cost`, `data-visualization`, `dsh-plugin`
- **推荐理由：** 实时追踪Token用量、成本估算和API余额图表。
...
- **标签：** `ai-guide`, `tutorials`, `rag`
- **推荐理由：** AI资源大全，含教程、Prompt和知识库，适合初学者和进阶者。
...
- **标签：** `tutorial`, `dsh`, `beginner`
- **推荐理由：** 从零开始的DSH入门教程，适合新手快速上手。
...
- **推荐理由：** DeepSeek Harness精选插件列表，方便发现和选用插件。
...
- **标签：** `research`, `web-search`, `claude-code`, `dsh-plugin`
- **推荐理由：** 自动搜索国内8大平台并生成有出处的研究报告。
...
- **⭐ 星标：** 237
- **语言：** TypeScript
- **描述：** DSH (DeepSeek Harness) 原��学习、插件市场与最佳实践 · Learn DSH principles, plugin marketplace & best practices
- **标签：** `learn`, `best-practices`, `marketplace`
- **推荐理由：** 学习DSH原理、插件市场和最佳实践的资源库
...
- **标签：** `awesome-list`, `curated`, `mcp`
- **推荐理由：** 精选插件、技能、MCP服务器及编排器列表
...
- **推荐理由：** 1958个仓库、1819个真实插件的中英文搜索目录
...
- **推荐理由：** 跨PubMed、arXiv、CrossRef的中文学术检索工具
...
- **⭐ 星标：** 163
- **语言：** TypeScript
- **描述：** Mimir — 一站式科研���作台插件：LaTeX 论文边写边编译、arXiv 文献管理、实验追踪、指标图表、GPU 服务器 SSH 任务编排，管理科研全周期。An open-source research workbench plugin for the whole research cycle.
- **标签：** `research`, `latex`, `academic`
- **推荐理由：** 一站式科研工作台：LaTeX编译、文献管理与GPU追踪
...
- **标签：** `research`, `industry-chain`, `policy-tracking`
- **推荐理由：** 提供行业研究方法论和技能，支持产业链映射与政策跟踪。
...
- **推荐理由：** 精选 14 类 280+ 插件目录，快速发现优秀社区插件。
...
- **标签：** `kline`, `analysis`, `finance`, `plugin`
- **推荐理由：** 交互式 K 线分析，支持技术指标和新闻资讯。
...
- **标签：** `finance`, `stock`, `trading`
- **推荐理由：** 24个经验证的股票/金融/量化DSH插件
...
- **推荐理由：** 完整DSH工具目录，按star排序，1000+仓库
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产���说明书与导航：按场景选插件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `guide`
- **推荐理由：** DSH用户插件说明书和导航指南
...
- **标签：** `catalog`, `auto-update`
- **推荐理由：** GitHub Actions自动爬取的 bilingual DSH插件目录
...
- **⭐ 星标：** 3,839
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外挂视��插件，为 DeepSeek、GLM 等纯文本模型外挂视觉能力，粘贴图片即得结构化 JSON 证据（OCR、版面、语义）。
...
- **推荐理由：** DeepSeek 专用破甲插件「无限三代」dsh-infinite-gen-3 — armor-breaking plugin for DeepSeek，破甲版
...
- **标签：** `knowledge-base`, `resource-directory`, `learning-path`
- **推荐理由：** 覆盖Agent、RAG、MLOps的AI一站式资源百科。
...
- **标签：** `anime`, `entertainment`, `bangumi`
- **推荐理由：** 多源搜番插件，支持Bangumi评分与磁力链接
...
- **标签：** `awesome-list`, `cordis`, `resource`
- **推荐理由：** 由cordis.run精选维护的DSH插件自动清单
...
- **标签：** `bazi`, `fengshui`, `iching`, `traditional-wisdom`
- **推荐理由：** 融合八字/风水/六爻等传统智慧的本地确定性引擎。
...
- **推荐理由：** 精选DeepSeek Harness插件列表，快速发现优质工具。
...
- **标签：** `awesome-list`, `agent-harness`, `dsh-plugin`
- **推荐理由：** 最全面的DSH生态精选列表，涵盖所有插件和技能。
...
- **标签：** `dsh-bundle`, `dsh-repository-plugin`
- **推荐理由：** DSH插件包管理与仓库工具
...
- **标签：** `awesome-list`, `mcp`, `plugin-recommender`
- **推荐理由：** AI整理的190+插件推荐列表
...
- **标签：** `plugin-directory`, `coding-agents`
