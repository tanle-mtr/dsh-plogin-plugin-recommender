...
- **标签：** `dsh-web`, `dsh-plugin`, `ecosystem`
- **推荐理由：** Web端插件聚合生态，创意工坊分发机制
...
- **标签：** `agent-tools`, `vfs`, `virtual-filesystem`
- **推荐理由：** AI Agent统一虚拟文件系统，跨引擎文件访问
...
- **标签：** `coding-agent`, `agent-runtime`
- **推荐理由：** Rust高性能Agent运行时配合精美桌面编码应用。
...
- **推荐理由：** 基于12本经典工程著作的AI代码审查，支持自动修复。
...
- **标签：** `agent`, `desktop`, `multi-model`, `privacy`
- **推荐理由：** 本地优先多模型AI代理，自进化技能，隐私优先
...
- **标签：** `security`, `ai-agents`, `redteam`
- **推荐理由：** AI代码安全平台，检测幻觉包和缺失防护
...
- **标签：** `multi-agent`, `coordination`, `harness`
- **推荐理由：** 统一管理Claude/Codex/Grok/Harness协同工作
...
- **标签：** `agent-skill`, `awesome-dsh-plugin`, `dsh-plugin`
- **推荐理由：** 开发打包DSH插件的标准技能
...
- **推荐理由：** OpenAI Codex与DSH双向同步桥梁，实时技能与MCP镜像。
...
- **标签：** `package-manager`, `skills`, `database`, `agent-framework`
- **推荐理由：** 含42项技能与5个CLI的智能体工程包管理器。
...
- **标签：** `engineering`, `practices`, `agent-skills`, `development`
- **推荐理由：** DSH实战提炼的工程方法论：证据驱动、持久决策。
...
- **标签：** `cost`, `token-usage`, `visualization`
- **推荐理由：** 实时Token用量图表和成本追踪面板
...
- **标签：** `sidebar`, `dsh-plugin`
- **推荐理由：** 开放的侧边栏底座，内置文件编辑、终端、对话和Git页面。
...
- **标签：** `desktop-companion`, `mcp`, `dsh-plugin`
- **推荐理由：** 桌面萌宠陪伴平台，插件SDK+编码代理集成，本地优先
...
- **标签：** `widget`, `floating-ui`, `dsh-plugin`
- **推荐理由：** 萌系漂浮鲸鱼小部件，监控DSH余额，拖拽吸附动画
...
- **标签：** `desktop`, `local-first`, `mcp`, `dsh-plugin`
- **推荐理由：** 本地优先AI桌面工作台，支持会话、项目、文件与插件管理
...
- **标签：** `ios`, `mobile`, `simulator`
- **推荐理由：** 对话中直接操控iOS模拟器和真机
...
- **标签：** `desktop`, `gui`, `electron`, `windows`
- **推荐理由：** 功能完整的Windows桌面客户端，含SSH
...
- **标签：** `desktop-app`, `dsh`, `dsh-plugin`, `electron`
- **推荐理由：** Electron桌面控制台管理插件与MCP
...
- **标签：** `capability-menu`, `ui`, `mcp-management`
- **推荐理由：** 统一管理MCP工具与技能暴露级别和运行模式的菜单。
...
- **标签：** `ui`, `status-bar`, `pricing`, `web`
- **推荐理由：** 底部常驻信息栏，显示模型、余额与计费信息。
...
- **推荐理由：** Tauri桌面应用整合数据库、SSH、Docker和AI助手
...
- **推荐理由：** Electron桌面包装整合DSH网页UI和插件生态
...
- **标签：** `webui`, `theme`, `customization`
- **推荐理由：** 支持自定义背景图片及氛围预设的UI美化插件
...
- **标签：** `deepseek`, `dsh-plugin`, `workflow`
- **推荐理由：** 两阶段DSH预设：轻量引导后全面标准工具覆盖
...
- **标签：** `agent-skills`, `cognitive-enhancement`
- **推荐理由：** 基于J-space全局工作空间研究的AI认知增强技能。
...
- **推荐理由：** 通用技能智能路由并编排harness工作流，功能强大。
...
- **标签：** `workflow`, `prompt-management`, `ai-assistant`
- **推荐理由：** 将AI对话转为可复用工作流，跨平台支持主流AI模型
...
- **标签：** `prompt`, `generation`, `deepseek`
- **推荐理由：** DeepSeek专属破甲提示词生成器
...
- **标签：** `skills`, `infra`, `workflow`
- **推荐理由：** AI基础设施开发专用agent技能
...
- **标签：** `agent-skills`, `deep-research`, `research-tools`, `skill-md`
- **推荐理由：** 88个开源技能，覆盖研究、营销与业务
...
- **标签：** `agent-skills`, `dsh-plugin`, `spec-driven-development`
...
- **推荐理由：** 多Agent团队协作，独立模型技能上下文共享工作区。
...
- **推荐理由：** Swift Cycle治理技能，版本固定离线可验证。
...
- **标签：** `gongwen`, `docx`, `word`, `chinese-documents`
- **推荐理由：** 中文公文全流程处理，格式检查、优化与模板生成。
...
- **标签：** `video`, `ai-image`, `storyboard`, `vision`
- **推荐理由：** AI视频/图像/音频全栈流程，含分镜与知识库。
...
- **标签：** `skills`, `gamification`, `task-system`, `wayfinder`
- **推荐理由：** 游戏化任务系统，迷雾地图+任务栏推进机制。
...
- **推荐理由：** Godot 4.x全栈开发技能插件，适合DSH游戏开发
...
- **标签：** `agent`, `product-management`, `preset`
- **推荐理由：** 模拟产品团队全流程协作工作流
...
- **标签：** `ocr`, `multimodal`, `vision`, `image-to-text`
- **推荐理由：** 首个视觉桥接插件，将图片转为结构化JSON证据
...
- **标签：** `video-generation`, `ai-video`, `agent-skill`
- **推荐理由：** 从单一主题自动生成Vox风格拼贴解说视频，端到端自动化。
...
- **标签：** `vision`, `ocr`, `agent-vision-toolkit`
- **推荐理由：** 纯文本模型视觉工具箱：粘贴图片识别、OCR、截图还原
...
- **推荐理由：** AI生成可编辑设计系统，模板市场+原生PPT工作室
...
- **标签：** `canvas`, `workspace`, `visualization`
- **推荐理由：** 画布式非线性对话工作区，分支会话探索
...
- **标签：** `agent-skills`, `diagram`, `legal`, `visualization`
- **推荐理由：** 从案件材料直接生成诉讼时间轴图表
...
- **推荐理由：** AI代理无缝操控真实登录态浏览器，CLI+插件双模式
...
- **标签：** `record-replay`, `automation`, `macos`
- **推荐理由：** 录制macOS操作并转化为agent技能，自动化高效
...
- **标签：** `image-uploader`, `electron`
- **推荐理由：** 终极图片上传工具支持60+服务
...
- **标签：** `dsh-plugin`, `routing`, `injector`
- **推荐理由：** DSH运行时注入器与推理路由标准组件套件
...
- **标签：** `dsh-plugin`
- **推荐理由：** 零依赖单文件传统玩具竹知了的Web模拟版，趣味实用。
...
- **推荐理由：** DSH上下文仪表盘与统计管理，深度洞察优化上下文效率
...
- **标签：** `provider`, `oauth`, `subscription`
- **推荐理由：** 订阅转DSH提供商，无需API密钥
...
- **标签：** `plugin-discovery`, `automation`, `utility`
- **推荐理由：** 自动搜索、安装并验证GitHub插件
...
- **标签：** `console`, `dsh`, `dsh-plugin`, `plugin-management`, `ui`
- **推荐理由：** DSH必备插件注册表控制台和开发技能
...
- **标签：** `config-manager`, `backup`, `migration`, `mcp`
- **推荐理由：** 完整配置备份恢复迁移，支持WebDAV同步，运维利器。
...
- **标签：** `context`, `audit`, `optimization`, `ui`
- **推荐理由：** 审计上下文Token成本，检测指令重复冲突，优化效率。
...
- **标签：** `skills-manager`, `utility`, `organization`
- **推荐理由：** 专注于DSH技能管理的高效插件，方便组织维护。
...
- **标签：** `migration`, `claude-code`, `configuration`
- **推荐理由：** 无缝迁移Claude Code配置到DeepSeek Harness
...
- **标签：** `theme`, `dark-mode`, `color-scheme`, `ui`
...
- **推荐理由：** 本地优先的跨平台AI内容发现Agent，支持中文主流平台。
...
- **推荐理由：** 自动搜索中文8大平台近30天内容并生成可查证研究报告。
...
- **推荐理由：** 9000+插件雷达精选Top50，真实运行实测，自动更新索引
...
- **推荐理由：** DSH插件生态精选索引，覆盖工具链与Web UI全品类
...
- **标签：** `awesome-list`, `discovery`, `catalog`
- **推荐理由：** 每日自动抓取插件目录，去伪存真带排行
...
- **标签：** `marketplace`, `education`, `best-practices`
...
- **标签：** `awesome-list`, `mcp`, `plugins`, `skills`
- **推荐理由：** 插件、MCP、技能与UI的精选清单
...
- **标签：** `awesome-list`, `deepseek`, `dsh-plugin`, `plugins`
- **推荐理由：** 精心整理的DSH插件发现清单
...
- **标签：** `awesome-deepseek-harness`, `awesome-dsh-plugin`, `plugin-marketplace`, `plugins`
- **推荐理由：** 自动同步的社区插件目录，便于浏览
...
- **标签：** `ai-agents`, `chinese-api`, `dsh-plugin`, `api-documentation`
- **推荐理由：** 在Agent内直接搜索6.5万+中文平台API文档
...
- **标签：** `kline`, `finance`, `analysis`, `interactive`
- **推荐理由：** 交互式K线分析，支持技术指标、支撑压力位及新闻。
...
- **标签：** `evaluation`, `rag`, `prompt-engineering`, `benchmark`
- **推荐理由：** 提示词、RAG与智能体工作流的实证评估框架。
...
- **推荐理由：** 精选280+社区插件目录，14类分类，快速发现好插件。
...
- **标签：** `deepseek-harness`, `photo-wall`, `oauth`
- **推荐理由：** 内测合影墙，OAuth零权限登录，即时拍立得风格。
...
- **标签：** `catalog`, `community`, `discovery`
- **推荐理由：** 非官方社区目录，扩展插件发现范围
...
- **标签：** `verified`, `curated`, `installation`
- **推荐理由：** 经实证验证的插件目录，附带安装指导
...
- **标签：** `curated`, `quality-check`, `2026`
- **推荐理由：** 2026年精选插件列表，质量审核
...
- **标签：** `awesome-list`, `radar`, `verify`
- **推荐理由：** 有数据背书的插件雷达与评测实验室
...
- **标签：** `awesome-list`, `auto-crawl`, `directory`
- **推荐理由：** Auto-crawled bilingual daily-updated plugin directory
