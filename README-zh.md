...
- **标签：** `multi-agent`, `mcp`, `agent-memory`
- **推荐理由：** 自进化记忆的多模AI助手，支持多通道
...
- **标签：** `agent-runtime`, `ide`, `dsh-plugin`
- **推荐理由：** Rust高性能Agent运行时，精美桌面IDE体验。
...
- **标签：** `code-review`, `agent-skill`, `dsh-plugin`
- **推荐理由：** 基于12本经典工程著作的智能代码审查工具。
...
- **推荐理由：** 架构感知编码Agent，baseline-first安全开发
...
- **标签：** `agent-skills`, `ai-coding`, `dsh-plugin`, `spec-programming`
- **推荐理由：** 仓库级规范驱动开发的编程代理工具
...
- **标签：** `plugin-development`, `cordis`, `audit`
- **推荐理由：** 用于开发和审计DSH插件的便携式技能。
...
- **标签：** `plugin-dev`, `skills`, `testing`
- **推荐理由：** 在代理内完成DSH插件的构建与测试
...
- **推荐理由：** 开源Claude Design替代品，本地优先桌面应用
...
- **⭐ 星标：** 2,877
- **语言：** TypeScript
- **描述：** 开放的���边栏底座，支持三方拓展注册新侧边栏页面。内置文件渲染编辑/终端/侧边对话/Git/子代理页面 ｜ Open sidebar foundation, supports third-party extensions to register new sidebar pages. Built-in file rendering/editing, terminal, side chat, Git, and sub-agent pages.
- **标签：** `sidebar`, `ui`, `dsh-plugin`
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话。
...
- **推荐理由：** Claude Code风格TUI，实时状态与回滚功能。
...
- **标签：** `desktop`, `companion`, `pet`, `sdk`
- **推荐理由：** 桌面宠物伴侣，带插件SDK集成
...
- **标签：** `widget`, `desktop`, `balance`
- **推荐理由：** 可爱鲸鱼悬浮组件，监控账户余额
...
- **标签：** `desktop-app`, `electron`, `windows`, `ssh-client`
- **推荐理由：** Windows桌面客户端，含SSH和远程访问，功能完整
...
- **推荐理由：** 锤子便签复刻版，支持Docker私有化部署和技能调用
...
- **推荐理由：** SQLite驱动的任务管理面板，支持Agent协作
...
- **标签：** `desktop-app`, `dsh-plugin`, `electron`, `typescript`
- **推荐理由：** Electron桌面控制台，管理插件和MCP
...
- **推荐理由：** 集成数据库、SSH、Docker与AI助手的桌面应用
...
- **推荐理由：** Electron桌面封装，集成多个DSH插件，开箱即用
...
- **标签：** `ui`, `customization`, `background`
- **推荐理由：** 支持上传和预设的Web UI背景个性化插件
...
- **标签：** `agent-memory`, `long-term-memory`, `rag`, `mcp`
- **推荐理由：** 本地优先Markdown记忆层，跨AI通用。
...
- **标签：** `agent-memory`, `vector-search`, `embeddings`, `reranking`
...
- **标签：** `dsh-plugin`, `session-memory`
- **推荐理由：** 跨平台会话记忆，实现Agent无缝交接
...
- **标签：** `memory`, `recall`, `rag`
- **推荐理由：** 连接PowerContext实现记忆回溯与知识检索
...
- **标签：** `skills`, `prompt-engineering`, `workflow-automation`
- **推荐理由：** 530+提示词模板提炼为可复用技能
...
- **标签：** `agent-orchestration`, `agent-workspaces`, `no-code`, `multi-device`
- **推荐理由：** 跨设备统一Agent工作空间，任务看板。
...
- **推荐理由：** 基于Anthropic J-space研究的高效认知增强技能集。
...
- **标签：** `agent-skills`, `workflow`, `dsh-plugin`
- **推荐理由：** 智能技能路由与工作流编排，提升效率。
...
- **标签：** `agent-skills`, `skills`, `coding`
- **推荐理由：** 将经验转化为可复用执行流程，跨Agent通用
...
- **标签：** `ai-infra`, `skills`
- **推荐理由：** AI基础设施开发、优化和调试的专业Agent技能集
...
- **标签：** `ctf`, `dsh-plugin`, `pentest`, `reverse-engineering`, `security`
...
- **标签：** `agent-skills`, `deep-research`, `market-research`, `social-listening`
- **推荐理由：** 88个开源技能，覆盖研究、社交情报和营销
...
- **标签：** `agent-skills`, `dsh-plugin`, `llm-agent`, `skill`
- **推荐理由：** DSH插件开发的标准工作流和引导
...
- **标签：** `dsh-plugin`, `chinese-traditional-wisdom`
- **推荐理由：** 独家中华传统智慧技能：八字风水中医易经融合
...
- **标签：** `dsh-plugin`
- **推荐理由：** WorkBuddy风格技能选择器，集成DSH Composer
...
- **标签：** `skills`, `gamification`, `task-bar`
- **推荐理由：** 游戏化任务系统，迷雾地图加任务栏推进。
...
- **推荐理由：** 为DSH代理添加Godot 4.x游戏开发全栈技能
...
- **标签：** `canvas`, `agent-skills`, `mcp`
- **推荐理由：** 无限画布配合创意AI技能，赋能WeShop
...
- **标签：** `agent`, `workflow`, `product-management`
- **推荐理由：** 模拟产品经理到QA的完整工作流，适合项目管理
...
- **标签：** `architecture-diagram`, `mermaid-alternative`, `text-to-diagram`
- **推荐理由：** 自包含HTML动效架构图，可视化出色。
...
- **标签：** `design`, `ui`, `vibedesign`, `agent-team`
...
- **标签：** `design`, `ui`, `ppt`
- **推荐理由：** 直接在对话中预览和编辑OpenPencil设计文档
...
- **⭐ 星标：** 43
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，���直接读案件材料画准一张时间轴。Claude Skill / DeepSeek Harness 通用。
- **标签：** `agent-skills`, `diagram`, `legal`, `svg`, `visualization`
- **推荐理由：** 将诉讼材料转化为清晰的时间轴和案件图表
...
- **标签：** `dsh-plugin`, `design-skills`
- **推荐理由：** 6种设计风格预设，摆脱AI同质化外观
...
- **推荐理由：** 增强Gemini/Claude/ChatGPT的浏览器套件
...
- **标签：** `anime`, `bangumi`, `torrent`, `dsh-plugin`
- **推荐理由：** 多源搜番，显示Bangumi评分和磁力链接
...
- **⭐ 星标：** 17
- **语言：** HTML
- **描述：** DSH 内测收官合影墙：GitHub OAuth 零权限登录 + 冻结白名单校���的拍立得合影站（含 DSH Skill 包装）
- **标签：** `community`, `social`, `dsh-plugin`
- **推荐理由：** GitHub OAuth零权限登录的DSH内测合影墙。
...
- **标签：** `router`, `injector`, `agent-tooling`
- **推荐理由：** 运行时注入器+任务感知路由器，优化推理。
...
- **标签：** `dsh-plugin`, `multi-model`
- **推荐理由：** 多模型一键切换中心，覆盖主流AI编程工具。
...
- **标签：** `marketplace`, `plugin`, `nodejs`
- **推荐理由：** 一键浏览、安装和更新所有DSH插件的插件市场
...
- **标签：** `dsh-plugin`, `config-manager`
- **推荐理由：** 完整配置备份恢复，支持WebDAV云端同步
...
- **标签：** `dsh-plugin`, `context`
- **推荐理由：** 审计上下文注入成本，检测重复与冲突
...
- **标签：** `migration`, `agent-migration`, `utilities`
- **推荐理由：** 将Claude Code等配置迁移到DSH平台
...
- **标签：** `utilities`, `conflict-resolution`, `maintenance`
- **推荐理由：** 诊断解决插件冲突与版本兼容问题，维护系统稳定
...
- **标签：** `utilities`, `checklist`, `agent-tools`
- **推荐理由：** Agent插件起飞前检查单，确保工具选择最优
...
- **标签：** `mascot`, `pixel-art`, `theme`
- **推荐理由：** 多平台编码Agent动效宠物画廊。
...
- **标签：** `theme`, `ocean-skin`, `webgpu`, `chrome-extension`
- **推荐理由：** WebGPU海洋主题皮肤，视觉效果惊艳
...
- **标签：** `dsh-plugin`, `awesome-deepseek-harness`
- **推荐理由：** 交互式K线分析，支持多市场行情技术指标
...
- **标签：** `evaluation`, `rag-evaluation`, `prompt-engineering`
- **推荐理由：** 证据驱动的提示词、RAG及智能体评估框架。
...
- **标签：** `engineering`, `practices`, `evidence`
- **推荐理由：** 从DSH开发中提炼的工程纪律与实践经验。
...
- **标签：** `awesome-list`, `curated-list`, `dsh`
- **推荐理由：** 精选DSH插件目录，便于发现优质插件
...
- **标签：** `registry`, `plugin-registry`, `open-data`
- **推荐理由：** 去垃圾信息的开放数据插件与技能注册表
...
- **标签：** `awesome-list`, `verified`, `permissions`
- **推荐理由：** 基于验证的插件目录，支持权限感知安装
...
- **标签：** `awesome-list`, `directory`, `verified`
- **推荐理由：** 经过验证的社区插件星级目录，便于发现好插件
...
- **标签：** `documentation`, `navigation`, `guide`
- **推荐理由：** 按场景导航的插件说明书，帮助用户选择合适插件
...
- **标签：** `radar`, `testing`, `lab`
- **推荐理由：** 证据支持的插件雷达和2Origin实验室测试平台
...
- **标签：** `portal`, `discovery`, `static-site`
- **推荐理由：** 零依赖静态门户，发现和筛选227+插件
...
- **标签：** `awesome-list`, `official`, `directory`
- **推荐理由：** 官方awesome-dsh插件目录（已迁移）
...
- **标签：** `awesome-list`, `automated`, `catalog`
...
- **标签：** `game`, `toy`, `dsh-plugin`
- **推荐理由：** 传统竹知了玩具Web模拟，零依赖趣味工具。
...
- **推荐理由：** 精选插件雷达，9000+候选，Top50 curated 榜单
...
- **标签：** `awesome-list`, `ecosystem`, `curated`
- **推荐理由：** 全面DSH生态精选插件列表
...
- **标签：** `awesome-list`, `ecossystem`, `curated`
- **推荐理由：** 精选DSH生态插件、工具和基础设施
...
- **推荐理由：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊��、磨砂度、背景
...
- **标签：** `awesome-list`, `dsh-plugin`, `mcp`, `plugins`
- **推荐理由：** 插件MCP大全，分类清晰，资源库丰富
...
- **标签：** `marketplace`, `plugin-store`, `dsh-plugin`
- **推荐理由：** 3100+插件市场，支持搜索评分与API调用
...
- **标签：** `plugin-marketplace`, `dsh-plugin`, `plugins`
- **推荐理由：** DSH插件市场，便于发现和浏览插件
