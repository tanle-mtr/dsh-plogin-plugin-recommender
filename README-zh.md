...
- **推荐理由：** 终端原生DeepSeek编码Agent，支持提示词缓存
...
- **推荐理由：** 编码Agent，支持RPC、子Agent、哈希行编辑及MCP集成
...
- **标签：** `cli`, `plugin-manager`, `schema`, `hub`
- **推荐理由：** DSH插件市场CLI工具和Schema解析器，命令行管理插件
...
- **标签：** `agent`, `desktop-app`, `local-first`, `mcp`
- **推荐理由：** 本地优先的Claude Cowork替代品，支持多模型与隐私保护
...
- **标签：** `skill-manager`, `audit`, `utility`
- **推荐理由：** DSH技能调用审计与内嵌管理面板。
...
- **推荐理由：** 将Superpowers编程技能移植到Reasonix编码工具链。
...
- **推荐理由：** 将Superpowers技能适配到DSH，支持TDD和调试。
...
- **推荐理由：** 三档暴露策略管理Tools/Skills，减少上下文占用并支持动态发现
...
- **推荐理由：** 华为云官方AI Agent工具包，含MCP工具与云沙箱
...
- **推荐理由：** 在DSH中统一加载并安全管理本机Agent Skills
...
- **标签：** `plugin-creator`, `scaffold`, `validation`
...
- **标签：** `launcher`, `orchestration`
- **推荐理由：** 统一编排Runtime、Provider和Plugin能力
...
- **标签：** `agent-tools`, `plugin`, `utility`
- **推荐理由：** DSH agent任务前插件库预检飞行检查单
...
- **⭐ 星标：** 20,111
- **语言：** TypeScript
- **描述：** Enhancement suite for Gemini, AI Studio, Claude & ChatGPT — plus a prompt manager for any websites, DeepSeek Harness included. / 面向 Gemini、AI Studio、Claude 与 ChatGPT 的增强套件；其中的提示词管理器可用��任意网站，如 DeepSeek Harness。
- **标签：** `dsh-plugin`, `browser-extension`
- **推荐理由：** 多平台增强套件，自带提示词管理，实用性强
...
- **标签：** `tui`, `terminal`, `interface`
- **推荐理由：** Claude Code风格TUI，支持流式思考和回滚。
...
- **标签：** `floating-widget`, `balance-monitor`, `dsh-plugin`
- **推荐理由：** 鲸鱼小部件随时监控余额，支持动画和拖拽
...
- **标签：** `runtime`, `desktop`, `agentic`
- **推荐理由：** Rust高性能运行时，精美桌面应用集成
...
- **标签：** `desktop-app`, `electron`, `windows`, `gui`
- **推荐理由：** 零配置Windows桌面客户端，内置Codex、SSH、插件管理等功能
...
- **推荐理由：** 本地优先AI桌面工作台，支持MCP、项目和Office办公套件
...
- **标签：** `genui`, `ui-components`, `charts`, `forms`
- **推荐理由：** 通过dsh-ui围栏在助手回复中内联渲染交互式UI组件
...
- **标签：** `canvas`, `non-linear`, `session-explorer`, `branching`
- **推荐理由：** 基于画布的非线性会话探索器，支持分支对话工作空间
...
- **标签：** `office`, `dsh-plugin`, `productivity`
- **推荐理由：** 将电子表格、文档、幻灯片和画布带入DSH办公环境
...
- **标签：** `mobile`, `ios`, `kotlin-multiplatform`, `ui-design`
- **推荐理由：** 通过WebSocket为DSH提供原生iOS客户端及工作区追踪
...
- **推荐理由：** 类CodePilot的DSH跨平台桌面客户端与插件套件
...
- **推荐理由：** 私有化部署的锤子便签复刻，支持公众号导出和技能调用。
...
- **标签：** `dsh`, `mcp`, `plugin`, `skills`
- **推荐理由：** Web界面技能/MCP管理工具，提升DSH的可视化操作体验。
...
- **标签：** `agent-harness`, `plugin-market`, `ui`, `web`
...
- **推荐理由：** Electron桌面控制台，多Tab统一管理Harness/插件/MCP
...
- **标签：** `web-platform`, `workbench`, `skills`
- **推荐理由：** 基于DSH的插件化Web工作平台，全局Skill管理与工作区集成
...
- **推荐理由：** 一体化DevOps桌面，集成数据库/SSH/Docker/AI沙箱
...
- **推荐理由：** Electron桌面版DSH，集成侧边栏/ModLens/插件市场
...
- **标签：** `ui`, `button`, `context`
- **推荐理由：** 一键折叠DSH上下文计量面板按钮
...
- **推荐理由：** 本地优先的Markdown记忆层，支持跨应用自进化
...
- **标签：** `long-term-memory`, `token-savings`, `rag`
- **推荐理由：** 混合检索记忆系统，节省35%token，跨任务复用
...
- **标签：** `agent-memory`, `continual-learning`
- **推荐理由：** 专为状态持久化Agent设计的记忆库
...
- **标签：** `memory`, `rag`, `persistence`
- **推荐理由：** Agent记忆管理套件，跨会话持久化。
...
- **推荐理由：** 基于Milvus的统一持久化记忆层，跨Agent共享
...
- **标签：** `memory`, `agent`, `context`
- **推荐理由：** 个人AI助手，全工具共享持久化上下文
...
- **标签：** `agent-memory`, `skill-routing`, `embeddings`
- **推荐理由：** 将零散的SKILL.md文件整理为可检索的Agent技能语料库
...
- **标签：** `ai-knowledge`, `encyclopedia`, `rag`, `learning-path`
- **推荐理由：** 全维度AI百科：大模型、Agent、RAG、多模态、学习路线一站式导航
...
- **标签：** `skill-marketplace`, `skills`
- **推荐理由：** ���全技能商店，自动抓取GitHub万级项目
...
- **标签：** `agent-collaboration`, `enterprise`
- **推荐理由：** 企业级本地Agent工作台，支持多Agent协作
...
- **标签：** `skills`, `routing`, `optimization`
- **推荐理由：** 智能技能路由，奖励提升21pp省30%token。
...
- **标签：** `video`, `ffmpeg`, `automation`
- **推荐理由：** 一键自动生成解说视频，端到端自动化
...
- **标签：** `teamwork`, `collaboration`, `agents`
- **推荐理由：** 多Agent团队协作插件，提升工作效率
...
- **标签：** `writing`, `creative`, `novel`
- **推荐理由：** AI辅助小说创作，完整创作工作流
...
- **标签：** `creative-writing`, `novel`, `screenwriting`, `workflow`
- **推荐理由：** 小说、短剧、游戏、视频解说四大创作工作台
...
- **标签：** `agent-skills`, `multimodal`, `computer-vision`, `mcp`
- **推荐理由：** 为AI赋予视听能力，视频音频屏幕观测且可验证结果
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审查与Web UI
...
- **推荐理由：** 将认识论循环框架引入DSH Agent进行科学发现
...
- **标签：** `agent-evolution`, `workflows`, `review`
- **推荐理由：** Agent自我进化工作流，支持可审查任务控制。
...
- **标签：** `ai-infra`, `dsh`, `skills`
- **推荐理由：** 专为AI基础设施开发设计的高级智能体技能集合。
...
- **标签：** `agent-learning`, `automatic-skill-learning`, `skill-generator`
- **推荐理由：** 自动将成功的DSH会话转为可复用的智能体技能。
...
- **推荐理由：** AI编程导师技能，自动生成阶梯式章节并协助手写核心逻辑
...
- **标签：** `skills-deck`, `agent-skill`, `mattpocock`
- **推荐理由：** 内置25个工程与效率技能，右侧面板直接调用无需手动安装
...
- **标签：** `document`, `gongwen`, `wps-office`
- **推荐理由：** 基于国标的公文全流程处理：格式检查、内容优化、模板生成
...
- **⭐ 星标：** 40
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六��/体质全融合，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
...
- **推荐理由：** 多Agent协作框架，支持独立模型、技能和共享工作区
...
- **标签：** `multi-agent`, `memory`, `durable`
- **推荐理由：** 持久化多Agent团队，支持跨会话记忆和技能保留
...
- **推荐理由：** 11个QA技能加共享知识库，支持AI测试工程自动化
...
- **标签：** `planning`, `workflow`
- **推荐理由：** 先议后行协作机制，提升AI任务质量
...
- **标签：** `video`, `motion-design`
- **推荐理由：** 电影感产品视频Agent，含152张镜头卡
...
- **标签：** `agent-skill`, `plugin-dev`, `skills`
- **推荐理由：** DSH插件开发Agent技能，提供标准化工作流程
...
- **标签：** `mcp`, `academic-search`, `citation`
- **推荐理由：** MCP学术搜索，跨PubMed/arXiv/OpenAlex多源检索。
...
- **推荐理由：** 基于Tushare的财经数据MCP，支持自适应接口生成。
...
- **标签：** `skillhub`, `api`, `openapi`
- **推荐理由：** 腾讯SkillHub开放API文档及插件支持
...
- **标签：** `chatgpt`, `oauth`, `ai-agents`, `dsh-plugin`
- **推荐理由：** 通过OAuth绑定ChatGPT，在DSH中访问模型。
...
- **推荐理由：** AI视频/图像/音频生成技能，含分镜和文生视频工具
...
- **标签：** `ui`, `visual`, `knowledge`
- **推荐理由：** 将聊天历史转化为可分享的精美知识卡片
...
- **标签：** `web-search`, `free`, `no-signup`, `plugin`
- **推荐理由：** 最强免费网页搜索插件，无需注册和API密钥，通用各模型
...
- **标签：** `browser-automation`, `anti-detect`, `playwright`, `android`
- **推荐理由：** 引擎级指纹伪造持久身份浏览器，支持Android模拟
...
- **推荐理由：** 对话内集成iOS模拟器及USB iPhone自动化22项Agent工具
...
- **标签：** `browser-automation`, `browser-use`, `playwright`
- **推荐理由：** 基于Playwright的浏览器自动化插件，适用于DSH。
...
- **标签：** `router`, `infrastructure`, `injector`
- **推荐理由：** 运行时注入器加推理路由，实测性能提升
...
- **标签：** `preset`, `configuration`, `bootstrap`
- **推荐理由：** 两阶段预设快速配置，开箱即用。
...
- **标签：** `account-management`, `routing`, `failover`
- **推荐理由：** 统一账号管理，智能路由自动故障转移。
...
- **标签：** `discovery`, `radar`, `registry`
- **推荐理由：** 雷达式发现插件生态，持续更新索引
...
- **标签：** `security`, `redteam`, `vulnerability`, `auditing`
- **推荐理由：** AI���全攻防平台，审计幻觉包、缺失防护及AI模式错误
...
- **推荐理由：** 无需API密钥即可使用ChatGPT Claude Grok订阅作为LLM提供商
...
- **标签：** `cost-tracking`, `token-usage`, `llm-provider`
- **推荐理由：** 跨多LLM提供商追踪会话成本预算及Token使用情况
...
- **标签：** `llm-provider`, `commandcode`, `plugin`
- **推荐理由：** 添加Command Code模型访问支持实时目录与套餐感知选择
...
- **标签：** `backup`, `config-manager`, `mcp`, `migration`, `sync`
- **推荐理由：** 完整的DSH配置备份/恢复/迁移/同步工具，支持WebDAV。
...
- **标签：** `dsh-box`, `container`
- **推荐理由：** 多版本隔离管理，嵌入式WebView界面
...
- **标签：** `theme`, `glass-UI`, `frosted`, `wallpaper`
- **推荐理由：** 高度可定��磨砂玻璃主题，可调模糊度和自定义壁纸
...
- **标签：** `animated-background`, `theme`, `glassmorphism`, `threejs`
- **推荐理由：** 精美的海洋动态主题，可调海浪与玻璃态效果
...
- **标签：** `ai-guide`, `prompt-engineering`, `vibe-coding`
- **推荐理由：** 全面AI资源大全，含教程与提示词百科
...
- **标签：** `arxiv`, `latex`, `gpu-cluster`, `ssh`, `research`
- **推荐理由：** 全周期科研工作台：LaTeX实时编译、arXiv管理、GPU SSH编排
...
- **推荐理由：** 每日抓取GitHub的DSH插件精选目录，场景化分类推荐
...
- **标签：** `awesome-list`, `agent-framework`, `mcp`
- **推荐理由：** DSH插件技能MCP服务器编排器的综合精选清单
...
- **标签：** `awesome-list`, `guide`, `developer-tools`
- **推荐理由：** DSH终极指南含入门资源精选插件和工具包
...
- **推荐理由：** 行业研究技能包，支持公司分析、政策新闻追踪和产业链映射。
...
- **标签：** `web-search`, `agent-skills`, `research`
- **推荐理由：** You.com网络搜索与研究技能，支持AI Agent实时信息获取
...
- **标签：** `plugin-registry`, `skills`, `open-data`
- **推荐理由：** 开源数据注册表，含垃圾过滤的插件发现工具
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产品说明书与导航：按场景选插件，了解安��、权限、兼容性和风险
- **标签：** `guide`, `awesome-list`, `directory`
- **推荐理由：** 含安全权限兼容性说明的插件导航指南
...
- **标签：** `awesome-list`, `directory`, `automation`
- **推荐理由：** GitHub Actions自动爬虫维护的插件目录
...
- **推荐理由：** cordis.run官方精选维护的DSH插件自动列表。
...
- **标签：** `agents`, `awesome`, `awesome-list`
...
- **标签：** `dsh-plugins`, `plugin-directory`, `awesome-dsh-plugins`
- **推荐理由：** 社区精选的DSH插件目录和生态列表。
...
- **标签：** `redirect`, `awesome-dsh-plugins`
- **推荐理由：** 重定向通知，原仓库已迁移。
...
- **标签：** `awesome-list`, `auto-update`, `dsh-plugins`
- **推荐理由：** 自动生成双语插件列表并部署到Vercel。
...
- **标签：** `plugin-directory`, `curated-list`, `deepseek-harness`
- **推荐理由：** 涵盖模型、工具、沙箱和UI的精选插件注册表。
