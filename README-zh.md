...
- **推荐理由：** DeepSeek原生终端编码Agent，支持提示缓存。
...
- **标签：** `low-code`, `internal-tools`, `workflows`
...
- **标签：** `ai-guide`, `tutorials`, `prompts`
- **推荐理由：** 全面的AI资源库，含Vibe Coding教程和提示词大全。
...
- **标签：** `routing`, `injector`, `harness`
- **推荐理由：** 任务感知路由套件，实现智能模式切换。
...
- **标签：** `agent-os`, `evaluation`, `loop`
- **推荐理由：** 自我进化Agent OS，评估门禁驱动进化循环。
...
- **推荐理由：** 两阶段引导预设，提供最小化引导到完整标准工具的平滑过渡。
...
- **推荐理由：** Rust高性能Agent运行时搭配精美桌面编码应用。
...
- **标签：** `code-review`, `code-quality`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，含严重度标签与自动修复
...
- **推荐理由：** 纯文本模型专用视觉工具箱：图片问答、OCR、截图还原
...
- **推荐理由：** 支持子代理、hashline编辑和MCP的丰富编码Agent
...
- **标签：** `plugin-dev`, `tooling`, `coding-agent`
- **推荐理由：** 开发DSH插件的标准技能与工作流
...
- **标签：** `build`, `plugin-management`
- **推荐理由：** 构建和管理已安装的DSH插件及凭证
...
- **推荐理由：** 审计指令链/技能目录，检测重复冲突并统计token成本
...
- **标签：** `quality`, `guard`, `engineering`, `delivery`
- **推荐理由：** 交付前质量门禁技能，验证需求再上线
...
- **标签：** `web-ui`, `plugin`, `ecosystem`
- **推荐理由：** 基于Web的插件聚合生态，Creative Workshop分发。
...
- **推荐理由：** 为AI Agent提供沙盒虚拟终端，支持隔离运行环境。
...
- **标签：** `desktop`, `tauri`
- **推荐理由：** 仅5MB的Tauri桌面版，零配置预装插件，跨平台支持
...
- **推荐理由：** 本地优先的Electron桌面工作台，支持会话、文件与办公
...
- **标签：** `office`, `desktop`
- **推荐理由：** 在Harness内集成电子表格、文档、演示文稿等办公套件
...
- **推荐理由：** 跨平台的CodePilot风格Desktop客户端插件
...
- **标签：** `ui`, `pricing`, `info-bar`, `usage-tracking`
- **推荐理由：** 底部信息栏，实时显示余额、价格峰谷与倒计时
...
- **推荐理由：** 类WorkBuddy技能选择器，Composer内插入/skill-name命令
...
- **推荐理由：** 生物信息学Agent桌面应用，含技能商店与离线模式
...
- **推荐理由：** Electron封装DSH Web为桌面应用，支持离线使用和自托管。
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin`, `dsh-plugins`
- **推荐理由：** 一键压缩DSH上下文面板显示，简洁实用。
...
- **标签：** `agent`, `workspace`, `orchestration`
- **推荐理由：** 统一Agent工作空间和任务看板，多设备同步。
...
- **标签：** `skills`, `store`, `marketplace`
- **推荐理由：** 最全技能商店，自动同步GitHub上万技能项目。
...
- **推荐理由：** 智能技能路由与工作流编排，Agent奖励提升21个百分点。
...
- **标签：** `video-generation`, `agent-skills`
- **推荐理由：** 单一主题自动生成Vox风格拼贴解说视频，全流程自动化
...
- **标签：** `workflow`, `competition`, `agent-skill`
- **推荐理由：** 数学建模竞赛全自动Agent，支持GitOps流水线与代码自证
...
- **标签：** `security`, `ctf`, `pentest`, `reverse-engineering`
- **推荐理由：** 87个逆向工程和渗透测试安全研究技能。
...
- **⭐ 星标：** 71
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项���架构、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `mentor`, `skill`, `code-review`
- **推荐理由：** 让AI化身编程导师并自动Code Review。
...
- **推荐理由：** 基于规范的编码开发技能，支持Vibe Coding
...
- **推荐理由：** 八字/风水/易经/中医融合，本地确定性引擎+可视化面板
...
- **推荐理由：** 多Agent协作，独立模型/技能/上下文共享工作区
...
- **推荐理由：** 跨平台会话记忆，实现Agent间目标与状态无缝交接
...
- **推荐理由：** 11个QA技能+共享知识库，类型决策矩阵助力AI测试工程
...
- **⭐ 星标：** 22
- **语言：** JavaScript
- **描述：** Godot Engine 4.x 全栈游戏开发���能插件 for DeepSeek Harness (DSH) — registers the godot-4-development skill at runtime
- **标签：** `godot`, `game-dev`, `gdscript`, `skill`
- **推荐理由：** Godot 4.x全栈游戏开发技能包，面向DSH智能体
...
- **标签：** `video`, `motion-design`, `remotion`, `skill`
...
- **⭐ 星标：** 3
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞前检查单"——任务���请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `ai-agents`, `cordis`, `dsh`, `llm-agents`
...
- **标签：** `agent`, `deepseek-harness`, `discoverability`, `dsh-plugin`
- **推荐理由：** 从DSH代理内部搜索插件，提升可发现性。
...
- **标签：** `llm-provider`, `provider`, `typescript`
- **推荐理由：** Command Code的LLM提供商插件，支持实时模型目录。
...
- **推荐理由：** 一键同步MCP/Skills配置至Claude Code、DSH、Cursor等7+工具
...
- **标签：** `codex`, `sync`, `mcp`, `bridge`, `cordis`
- **推荐理由：** OpenAI Codex与DSH双向桥接，实时同步+自动MCP镜像
...
- **推荐理由：** SkillHub官方OpenAPI文档与示例，DSH生态必备
...
- **推荐理由：** 首个视觉插件，支持OCR和图片转JSON，增强文本Agent能��。
...
- **标签：** `ai-video`, `ai-image`, `directorx`, `multimodal`
- **推荐理由：** AI视频/图像/音频生成技能，支持可配置视觉与生成模型
...
- **标签：** `design`, `ui`, `styles`, `vibe-coding`
- **推荐理由：** 6种设计风格预设，告别千篇一律的AI网页
...
- **标签：** `awesome-dsh-plugin`, `deepseek-harness`, `knowledge-cards`
...
- **标签：** `content-discovery`, `web-automation`, `multi-platform`
- **推荐理由：** 跨平台AI内容发现Agent，支持B站、YouTube、知乎等多平台。
...
- **推荐理由：** CLI+扩展让AI代理使用真实登录浏览器，不干扰工作流
...
- **标签：** `image-uploader`, `electron`, `s3-uploader`
...
- **推荐理由：** 一键安装并切换Claude Code、Codex、DeepSeek等多模型。
...
- **推荐理由：** 自部署插件生态雷达，自动发现验证15900+候选插件
...
- **推荐理由：** 全面精选插件清单，覆盖智能体、CLI、Web UI等
...
- **标签：** `cost`, `token-tracking`, `utility`
- **推荐理由：** 会话级/日级成本追踪，支持多提供商余额和配额管理
...
- **标签：** `compatibility-layer`, `migration`, `bridge`
- **推荐理由：** 打通Pi与DSH生态，无缝兼容原有扩展插件。
...
- **标签：** `skills-manager`, `utility`, `security`
- **推荐理由：** 统一加载和安全管理局部Agent技能
...
- **标签：** `container`, `infrastructure`, `multi-version`
- **推荐理由：** 容器隔离多版本DSH，内嵌WebView UI一键管理
...
- **标签：** `token-usage`, `data-visualization`, `api-cost`
- **推荐理由：** 实时追踪Token用量并可视化API成本，适合预算控制。
...
- **推荐理由：** 诊断插件冲突和版本问题，是维护插件生态健康的必备工具。
...
- **标签：** `pets`, `theme`, `ui-cosmetic`
- **推荐理由：** 动画宠物画廊，为任意Agent工具添加个性化。
...
- **标签：** `theme`, `skin`, `whale`
- **推荐理由：** 鲸鱼娘系列皮肤，美化DeepSeek Harness界面外观。
...
- **标签：** `background`, `webui`, `personalization`
- **推荐理由：** 支持自定义背景与预设氛围切换，UI个性化程度高。
...
- **标签：** `research`, `web-search`
- **推荐理由：** 自动搜索8大中文平台近30天内容，生成有据可查研究报告
...
- **推荐理由：** 一站式科研工作台：LaTeX写作、arXiv管理与GPU编排
...
- **标签：** `kline`, `financial`, `analysis`, `visualization`
- **推荐理由：** 交互式K线分析，支持多市场行情、技术指标与基本面分析
...
- **标签：** `registry`, `open-data`, `plugins`, `curated`
- **推荐理由：** 去垃圾的DSH插件、Bundle与技能公开数据注册表
...
- **推荐理由：** AI维护的190+插件推荐，每小时更新覆盖12个分类。
...
- **标签：** `resume-builder`, `self-hosted`, `javascript`
- **推荐理由：** 隐私优先的开源简历构建工具。
...
- **标签：** `awesome-list`, `discovery`, `dsh`
- **推荐理由：** DSH生态精选插件发现列表，便于探索。
...
- **标签：** `deepseek`, `armor-breaking`
- **推荐理由：** DeepSeek专用破甲插件，稳定化增强能力
...
- **推荐理由：** The most comprehensive DeepSeek Harness 
...
- **标签：** `agent-observability`, `agent-skills`, `agentic-ai`, `ai-agents`, `audio-understanding`, `computer-vision`, `deepseek`, `deepseek-harness`, `deepwatch`, `ffmpeg`, `local-first`, `mcp`, `multimodal`, `ocr`, `python`, `speech-to-text`, `typescript`, `verification`, `video-analysis`, `video-understanding`
- **推荐理由：** Give AI agents eyes, ears, and verifiabl
...
- **推荐理由：** A DSH plugin for novel writing and short
...
- **推荐理由：** 全面收录DSH插件、技能、MCP服务器与补丁的精选列表
...
- **标签：** `deepseek`, `deepseek-harness`, `deepseek-harness-plugin`, `dsh-plugin`, `ios`, `ios-app`, `ios-swift`
...
- **推荐理由：** DSH插件总目录，含1819个插件，支持中英文搜索分类。
...
- **推荐理由：** 2026年精选插件合集，经质量审核，便于快速发现优质插件。
...
- **推荐理由：** 24个经过验证的金融/量化插件合集，双语说明，专业垂直。
...
- **标签：** `awesome-list`, `plugins`, `collection`
- **推荐理由：** 实用插件集合，收录广泛，适合日常开发参考。
...
- **标签：** `catalog`, `awesome-list`, `comprehensive`
- **推荐理由：** 收录1000+插件的全量清单，按星标排序，覆盖全面。
...
- **标签：** `directory`, `navigation`, `manual`
- **推荐理由：** 插件产品说明书式导航，帮助用户按场景选择并评估风险。
...
- **推荐理由：** GitHub Actions自动爬取更新，每日同步，双语支持。
...
- **推荐理由：** 原版awesome-dsh-plugins仓库，已迁移至新地址。
