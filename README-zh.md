...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 235,068 | Core DSH framework: everything is a plugin ecosyst |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,943 | Open-source design agent with local-first desktop  |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 73,207 | Multi-agent swarm orchestrator with autonomous wor |
...
- **标签：** `multi-agent`, `agent-memory`, `multi-model`
- **推荐理由：** 具备记忆与自我进化能力的超级AI助手
...
- **推荐理由：** 首个视觉插件，让纯文本Agent也能通过OCR理解图像内容。
...
- **标签：** `preset`, `deepseek-harness`, `llm-agent`
- **推荐理由：** 两阶段预设配置，平衡轻量启动与完整工具覆盖。
...
- **标签：** `vfs`, `virtual-terminal`, `agent-tools`
- **推荐理由：** 世界首款虚拟终端，提供虚拟文件系统隔离Agent环境。
...
- **标签：** `multi-model`, `routing`, `cli`
- **推荐理由：** 多模型智能路由，统一管理账户额度并自动故障转移。
...
- **标签：** `runtime`, `coding-agent`, `desktop`
- **推荐理由：** 高性能Rust Agent运行时配合精美桌面界面
...
- **推荐理由：** 编码Agent，支持子Agent、哈希编辑和MCP。
...
- **标签：** `dsh-plugin`, `desktop-app`, `privacy`, `multi-model`
- **推荐理由：** 本地优先多模型桌面AI代理，隐私安全支持自我进化技能
...
- **标签：** `dsh-plugin`, `ios`, `automation`, `mobile-development`
- **推荐理由：** 对话内嵌iOS模拟器与真机自动化，22个Agent工具辅助开发
...
- **标签：** `dsh-plugin`, `desktop`, `electron`, `cross-platform`
- **推荐理由：** 类CodePilot跨平台桌面客户端，支持统一插件生态管理
...
- **标签：** `ai-agents`, `compatibility-layer`, `migration`
...
- **标签：** `agent`, `automation`, `migration`
- **推荐理由：** 从25+ AI编程工具导入对话历史，保留完整上下文。
...
- **标签：** `cordis`, `mcp`, `skills`
- **推荐理由：** 统一能力管理，减少上下文占用，支持动态发现
...
- **标签：** `cordis`, `skills`, `typescript`
- **推荐理由：** 基于插件的工作平台，支持全局技能管理
...
- **标签：** `huaweicloud`, `mcp`, `sdk`, `terraform`
- **推荐理由：** 华为云官方开发套件，含MCP工具与云沙箱
...
- **推荐理由：** DSH插件开发的即时知识助手，含官方文档中英双语存档。
...
- **推荐理由：** 构建和验证DSH/Cordis插件的可复用技能，提升开发效率。
...
- **标签：** `code-simplification`, `coding`, `codex`
...
- **推荐理由：** Agent任务前自动预检并注入Top-K适配插件，提升插件利用率。
...
- **推荐理由：** 通过OAuth连接ChatGPT账号，在DSH内直接使用其模型。
...
- **标签：** `browser-extension`, `prompt-manager`, `ai-platforms`
- **推荐理由：** 增强AI平台，提供提示词管理和浏览器扩展功能。
...
- **标签：** `desktop`, `native`, `application`
- **推荐理由：** DeepSeek Harness原生桌面应用，功能完整。
...
- **标签：** `web`, `aggregation`, `distribution`
- **推荐理由：** 基于Web的插件聚合平台，通过创意工坊分发。
...
- **标签：** `enterprise`, `workbench`, `multi-engine`
- **推荐理由：** 企业级代理工作台，支持多引擎和可视化编辑。
...
- **标签：** `sidebar`, `ui`, `dsh`
- **推荐理由：** 开放侧边栏框架，内置文件编辑、终端、Git和子代理页面。
...
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex和DSH界面使用。
...
- **推荐理由：** 官方推荐TUI插件，含鲸鱼顶栏、流式思考和上下文进度显示。
...
- **标签：** `desktop`, `gui`, `ssh-client`
- **推荐理由：** 零配置Windows桌面GUI，支持SSH、插件和远程访问。
...
- **推荐理由：** 本地优先AI桌面工作区，支持MCP、文件和Office。
...
- **标签：** `deepseek-harness-plugin`, `office`, `desktop-extensions`
- **推荐理由：** 在Harness中集成完整办公软件套件，支持表格文档演示等
...
- **标签：** `dsh-plugin`, `ios`, `mobile`, `native-app`
- **推荐理由：** 原生iOS客户端将工作区会话和Agent执行轨迹带入手机使用
...
- **标签：** `mcp`, `plugin`, `skills`, `web`
- **推荐理由：** Web界面Skill/MCP管理工具，便捷配置插件
...
- **推荐理由：** Electron桌面控制台，多Tab管理Harness与插件
...
- **标签：** `console`, `ui`, `plugin-management`
- **推荐理由：** 浏览器面板管理插件生态，含官方插件开发引导
...
- **标签：** `ui-extension`, `stats`, `dsh-plugin`
- **推荐理由：** 将 Composer 底部状态栏升级为显示 Provider、余额和峰谷定价。
...
- **推荐理由：** 类WorkBuddy的skill选择器，提升Composer体验
...
- **推荐理由：** 集成数据库/SSH/Docker/AI的一体化桌面工具
...
- **推荐理由：** Electron桌面版，集成多款DSH Web插件
...
- **推荐理由：** 开源LLM知识平台，支持RAG、推理智能体与Wiki
...
- **标签：** `memory-library`, `stateful`, `context`
- **推荐理由：** 内存库支持状态化代理，结合上下文工程。
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** Agent记忆管理套件，支持RAG，实现知识的长期积累与优化。
...
- **标签：** `memory`, `context`, `npm`
- **推荐理由：** 三层自动记忆整合，无需提示即可调用记忆
...
- **标签：** `multi-agent`, `swarm-intelligence`, `agentic-workflow`
- **推荐理由：** 多智能体群体编排器，支持自主工作流协调
...
- **标签：** `writing`, `webnovel`, `memory-management`
- **推荐理由：** 专用于长篇网文写作的技能，解决遗忘和幻觉问题。
...
- **推荐理由：** 持久化状态核心，支持跨会话的长周期多Agent工作流。
...
- **推荐理由：** 智能技能路由与工作流编排，显著提升Agent执行效率。
...
- **标签：** `cognitive`, `agent-skills`, `reasoning`
- **推荐理由：** 推理时控增强深度思考与纠错能力
...
- **标签：** `agent-teams`, `orchestration`, `workflow`
- **推荐理由：** DSH多Agent团队协作编排插件
...
- **标签：** `epistemic-loop`, `scientific-discovery`, `agent`
- **推荐理由：** 将认识论循环方法原生集成到DSH工作流中。
...
- **标签：** `agent-skills`, `ai-agents`, `creative-writing`, `deepseek-harness`
- **推荐理由：** 内置小说短剧游戏视频解说四个创作工作台，全能写作插件
...
- **标签：** `agent-skills`, `multimodal`, `video-analysis`, `computer-vision`
- **推荐理由：** 给AI Agent提供视听感知能力，生成可验证的时间戳证据
...
- **标签：** `dsh-plugin`, `task-management`, `sqlite`, `workflow`
- **推荐理由：** 基于SQLite的本地任务看板，支持Agent认领审查原生工作流
...
- **推荐理由：** Agent自我进化模式，支持可组合任务控制与人工审查。
...
- **标签：** `agent`, `architecture`, `skills`
- **推荐理由：** 将经验转化为可重复执行流程，多Agent平台通用。
...
- **标签：** `agent-skills`, `skill-generator`, `agent-learning`
...
- **推荐理由：** 从obra/superpowers适配的TDD、调试和规划技能
...
- **推荐理由：** AI编程导师，扫描架构生成阶梯式Chapter并Code Review
...
- **标签：** `ai-docs`, `gongwen`, `wps-office`
- **推荐理由：** 全流程公文处理，符合国标党政机关公文格式
...
- **标签：** `legal`, `diagram`, `svg`, `python`
- **推荐理由：** 诉讼可视化：时间轴、大事记表、法律关系图自动生成
...
- **标签：** `patent`, `rag`, `multi-agent`, `fastapi`
- **推荐理由：** 专利侵权分析系统，输入公开号即产出竞品分析报告
...
- **推荐理由：** 面向科研人员的生物信息学桌面代理，离线可用，技能商店丰富。
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `dsh-plugin`
- **推荐理由：** 融合八字风水奇门的传统智慧技能包，本地确定性引擎支持可视化。
...
- **推荐理由：** 工程纪律技能：交付前审核需求、验证实现、证明交付质量。
...
- **标签：** `spec-programming`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 基于DSH工程模式的规格驱动开发技能，便携可移植。
...
- **标签：** `math-modeling`, `skill`, `paper-writing`
- **推荐理由：** 数学建模全流程技能，含数据预处理、编程、论文���作与查重。
...
- **标签：** `multi-agent`, `durable-memory`, `dsh-plugin`
- **推荐理由：** 多 Agent 协作系统，支持跨会话持久记忆与共享工作空间。
...
- **推荐理由：** 多 Agent 协作框架，独立模型技能与共享工作区结合。
...
- **标签：** `qa`, `testing`, `dsh-plugin`
- **推荐理由：** 11个测试工程技能，让AI化身资深测试工程师，含决策矩阵。
...
- **推荐理由：** 五角色多Agent数学建模系统，含33篇模型库和论文模板。
...
- **标签：** `planning`, `agent-skills`, `workflow`
- **推荐理由：** 议行分离协作机制，结构化规划AI协作任务再执行。
...
- **推荐理由：** 含152张镜头配方的电影感视频Agent技能，资源齐全
...
- **推荐理由：** 工程管理工作流插件，支持任务阶段、验证记录和提交检查。
...
- **标签：** `ocr`, `image-reading`, `dsh-plugin`
- **推荐理由：** 像素到文本 OCR 技能，让纯文本模型具备图像识别能力。
...
- **标签：** `browser`, `automation`, `session`
- **推荐理由：** 浏览器自动化，让AI代理使用真实登录状态。
...
- **推荐理由：** 基于Playwright的浏览器自动化插件，支持DSH
...
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、YouTube、Reddit等主流平台。
...
- **标签：** `workspace`, `task-management`, `cross-device`
- **推荐理由：** 统一的工作空间，支持跨设备任务跟踪和管理。
...
- **标签：** `routing`, `injector`, `optimization`
- **推荐理由：** 运行时注入器和任务感知路由器，优化推理性能。
...
- **标签：** `mcp`, `credentials`, `secrets`
- **推荐理由：** 集中管理API密钥与凭据的实用工具
...
- **标签：** `radar`, `validation`, `discovery`
- **推荐理由：** 生态雷达持续发现并验证插件质量
...
- **标签：** `dsh-plugin`, `oauth`, `llm-provider`, `subscription`
- **推荐理由：** 通过OAuth复用现有ChatGPT Claude Grok订阅作为DSH模型源
...
- **标签：** `dsh-plugin`, `llm-provider`, `model-access`, `typescript`
- **推荐理由：** 集成Command Code模型接入，支持计划感知选择和推理效率优化
...
- **⭐ 星标：** 32
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面��� + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `audit`, `tool`, `ui`
- **推荐理由：** 上下文审计工具，检测指令重复冲突与token成本
...
- **标签：** `container`, `version-manager`, `ui`
- **推荐理由：** 容器化多版本DSH管理工具，支持WebView嵌入
...
- **标签：** `migration`, `session-import`, `claude`
- **推荐理由：** Claude Code/Codex迁移向导，一键导入会话和技能
...
- **标签：** `utility`, `infrastructure`
- **推荐理由：** 解决插件间及版本间冲突，提供自动修复方案。
...
- **推荐理由：** 类CodeX风格的DSH插件市场，一站式发现与安装插件。
...
- **标签：** `skin`, `theme`, `visual`
- **推荐理由：** 适用于DSH界面的鲸鱼娘主题系列
...
- **标签：** `dsh-plugin`, `theme`, `animation`, `threejs`
- **推荐理由：** 实时海洋动态主题，支持波浪动画和玻璃拟态效果自定义
...
- **标签：** `ai-guide`, `prompt-library`, `vibe-coding`
- **推荐理由：** 全面AI资源库，涵盖DeepSeek/GPT等教程指南
...
- **标签：** `curated-list`, `discovery`, `ecosystem`
- **推荐理由：** 精选插件列表，帮助用户发现和评估DSH生态工具。
...
- **推荐理由：** 一站式科研平台：LaTeX、arXiv、实验追踪与GPU编排。
...
- **标签：** `agent-skills`, `academic-search`, `mcp`, `citation-verification`
- **推荐理由：** 跨PubMed arXiv CrossRef多源学术检索，支持引文核验图谱分析
...
- **标签：** `awesome-list`, `catalog`, `deepseek-harness-plugins`
- **推荐理由：** 11000+插件目录，支持搜索、排序和免费API访问。
...
- **标签：** `agent-runtime`, `awesome-list`, `mcp`
- **推荐理由：** 涵盖173个主题的DSH Agent-first手册，内容详实。
...
- **标签：** `awesome-list`, `catalog`, `plugin-catalog`
- **推荐理由：** 1958个仓库插件目录，支持中英文搜索和分类筛选。
...
- **标签：** `catalog`, `research`
- **推荐理由：** 收录1000+插件的全量清单，按star排序，便于浏览查找。
...
- **标签：** `research`, `ai-curated`
- **推荐理由：** AI驱动的190+插件推荐，12分类每小时自动更新。
...
- **标签：** `radar`, `research`
- **推荐理由：** 基于实证的插件雷达，附带2Origin插件实验室测评。
...
- **标签：** `directory`, `catalog`
- **推荐理由：** 每日GitHub自动爬取的双语插件目录，持续更新。
...
- **标签：** `encyclopedia`, `resources`, `knowledge-base`
...
- **⭐ 星标：** 364
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动��取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `dsh-plugin`, `discovery`, `curated-list`
- **推荐理由：** 每日自动扫描GitHub精选插件目录，场景化分类推荐发现工具
...
- **标签：** `awesome-list`, `dsh-plugin`, `resources`, `curated-list`
- **推荐理由：** 全面插件技能MCP补丁编排器资源索引库，附带可视化导览
...
- **标签：** `awesome-list`, `dsh-plugin`, `guide`, `tutorials`
- **推荐理由：** DeepSeek Harness终极指南，含快速入门教程精选插件实用工具包
...
- **标签：** `awesome-list`, `dsh-plugins`, `curated`
- **推荐理由：** 收录280+插件的精选目录，按14类组织，是探索DSH生态的入口。
...
- **推荐理由：** SkillHub官方API文档与示例，适合参考学习
...
- **标签：** `catalog`, `community`, `awesome`
- **推荐理由：** 非官方社区插件目录，便于发现和探索
...
- **标签：** `awesome-list`, `community`, `resources`
- **推荐理由：** DSH官方精选资源列表，涵盖插件技能和工具
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `ai-agent`
- **推荐理由：** 覆盖面广的DSH插件生态精选目录
...
- **标签：** `dsh-plugins`, `repository`, `ecosystem`
- **推荐理由：** DeepSeek Harness插件仓库与生态聚合列表。
