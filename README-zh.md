...
- **标签：** `ai-agent`, `multi-agent`, `mcp`, `dsh-plugin`, `skills`
- **推荐理由：** 支持多模型、自进化记忆的智能助手超级框架
...
- **标签：** `coding-agent`, `deepseek`, `dsh-plugin`, `tui`, `tool-use`
- **推荐理由：** 终端原生AI编程助手，专为前缀缓存稳定性优化
...
- **标签：** `agent-skills`, `diagrams`, `architecture`
- **推荐理由：** 动态精美的架构图生成器
...
- **标签：** `enterprise`, `agent-workspace`, `multi-agent`
- **推荐理由：** 企业级多引擎Agent工作台
...
- **标签：** `coding-agent`, `ai-ide`, `dsh-plugin`
- **推荐理由：** Rust 高性能 Agent 运行时，深度集成桌面 IDE。
...
- **推荐理由：** 基于 12 本经典工程书籍的 AI 代码审查与修复。
...
- **标签：** `agent-skills`, `coding-agents`, `architecture-driven`
- **推荐理由：** 让AI编程代理感知架构，证据验证防漂移。
...
- **标签：** `vision`, `multimodal`, `browser-harness`
- **推荐理由：** 为纯文本 LLM 添加截图与 UI 分析视觉能力。
...
- **标签：** `android`, `emulator`, `mobile-dev`
- **推荐理由：** 对话内构建交互Android模拟器或真机
...
- **标签：** `agent`, `agent-skill`, `agent-skills`, `deepseek-harness`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 开发DSH插件的智能体技能，涵盖服务、事件、工具和适配器
...
- **标签：** `deepseek-harness`, `plugin-dev`
- **推荐理由：** 一站式构建DSH插件，支持槽位与凭证管理。
...
- **标签：** `agent-framework`, `dsh-plugin`, `mcp`, `database`
- **推荐理由：** 含42项技能、5个CLI的AI代理资产包管理器，一站式开发工具
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `plugin-development`
- **推荐理由：** 可复用的插件构建与验证技能，加速DSH和Cordis开发
...
- **标签：** `agent-skills`, `deepseek-harness`, `dsh-plugin`, `plugin-development`
- **推荐理由：** 便携式插件开发与审计技能，支持可选的DSH bundle适配器
...
- **标签：** `migration`, `mcp`, `skills`
- **推荐理由：** 一键迁移Claude Code/Codex配置到DSH，支持多源
...
- **标签：** `chatgpt`, `oauth`, `integration`, `agent-tool`
...
- **标签：** `dsh-plugin`, `desktop`, `cordis-plugin`, `deepseek-harness`
...
- **标签：** `dsh-web`, `ui`, `plugin-marketplace`
- **推荐理由：** Web UI插件聚合生态包
...
- **标签：** `desktop`, `tauri`, `deepseek-harness`
- **推荐理由：** 轻量Tauri桌面版，零配置，预置插件，跨平台。
...
- **标签：** `desktop-companion`, `mcp`, `electron`
...
- **标签：** `desktop-app`, `electron`, `office-automation`
- **推荐理由：** 本地优先桌面工作区，集成会话文件与办公产出。
...
- **标签：** `agent`, `desktop-app`, `dsh-plugin`
- **推荐理由：** 本地优先的AI桌面助手，支持多模型，注重隐私保护。
...
- **标签：** `desktop-app`, `dsh-plugin`, `electron`
...
- **标签：** `desktop-app`, `electron`, `gui`
- **推荐理由：** 开源 Windows 桌面客户端，内置 SSH 与远程访问。
...
- **推荐理由：** Electron桌面控制台，多Tab管理Harness、插件、MCP和技能
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `usage-tracking`, `web`
- **推荐理由：** 底部常驻信息栏，实时显示Provider、模型余额和定价信息
...
- **推荐理由：** Tauri跨平台桌面，集成数据库SSH Docker与AI助手
...
- **标签：** `desktop`, `electron`, `ui-integration`
- **推荐理由：** Electron桌面封装，集成侧边栏和Web UI增强体验
...
- **标签：** `rag`, `vector-search`, `knowledge-base`, `dsh-plugin`, `multi-tenant`
- **推荐理由：** 开源LLM知识平台，支持RAG与语义搜索
...
- **推荐理由：** 基于 Milvus 的持久化统一记忆层，支持多 Agent。
...
- **标签：** `skill-retrieval`, `embeddings`, `agent-memory`
- **推荐理由：** 将 SKILL.md 文件转为可检索的技能语料库。
...
- **标签：** `memory`, `long-term`, `mcp`, `agent`
- **推荐理由：** 持久化长期记忆，支持检索工具与设置页
...
- **标签：** `context`, `deepseek-harness`, `dsh-plugin`, `ui`
- **推荐理由：** 审计上下文Token成本，检测指令链重复与冲突，优化注入效率
...
- **标签：** `agentic-ai`, `agentic-workflow`, `multi-agent`, `mcp-server`, `dsh-plugin`
- **推荐理由：** 多智能体蜂群与自主工作流编排的元级框架
...
- **标签：** `agent-skills`, `ai-persona`, `dsh-plugin`, `meta-skill`, `knowledge-distillation`
- **推荐理由：** 提炼AI思维模式为可复用元技能的工具
...
- **标签：** `ai-agent`, `low-code`, `workflows`, `dsh-plugin`, `internal-tools`
- **推荐理由：** AI驱动的无代码业务系统搭建平台
...
- **标签：** `harness-engineering`, `tutorial`, `beginner`
- **推荐理由：** 从零开始的DSH入门教程
...
- **标签：** `agent-orchestration`, `workspaces`, `multi-platform`
- **推荐理由：** 全设备统一的Agent工作台
...
- **推荐理由：** 基于 Anthropic J-space 研究的认知增强技能套件。
...
- **标签：** `workflow`, `productivity`, `plugin`
- **推荐理由：** 将AI对话转化为可复用工作流，跨平台支持。
...
- **标签：** `math-modeling`, `gitops`, `competition`
- **推荐理由：** 支持 GitOps 流水线的数学建模竞赛自动化技能。
...
- **标签：** `skills`, `execution`, `agent`
- **推荐理由：** 将经验转化为可重复执行的多平台技能。
...
- **标签：** `task-management`, `webui`, `sqlite`
- **推荐理由：** SQLite任务看板，支持Agent认领与审查
...
- **推荐理由：** Superpowers技能移植到Reasonix，增强编码生产力
...
- **推荐理由：** 将TDD、调试、规划技能适配至DeepSeek Harness工作流
...
- **推荐理由：** 完整的逆向工程和渗透测试技能集，含85个SKILL.md
...
- **推荐理由：** AI编程导师，扫描项目架构、生成阶梯课程并执行代码审查
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`, `deepseek`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 88个开源智能体技能，涵盖研究、营销和商业自动化
...
- **标签：** `agent-skills`, `ai-coding`, `deepseek-harness`, `dsh-plugin`, `spec-driven-development`
- **推荐理由：** 可移植的规范驱动开发技能，支持仓库级编程工作流
...
- **标签：** `agent-skill`, `knowledge-base`, `documentation`
- **推荐理由：** 官方DSH插件开发文档，可作为即需智能体技能使用。
...
- **标签：** `agent-skill`, `chinese-culture`, `dashboard`
- **推荐理由：** 独特八字/风水/易经AI智能体技能，本地确定性引擎。
...
- **标签：** `ux`, `agent-skill`, `composer`
- **推荐理由：** 类WorkBuddy风格技能选择器，快捷插入命令。
...
- **标签：** `multi-agent`, `collaboration`, `team`
- **推荐理由：** 多智能体团队协作，独立模型技能与共享工作区。
...
- **标签：** `codex`, `code-quality`, `simplification`
- **推荐理由：** 源自DSH的代码简化智能体技能，适配Codex。
...
- **标签：** `governance`, `swift-cycle`, `agent-skill`
- **推荐理由：** 版本锁定Swift周期治理技能适配器，支持离线验证。
...
- **⭐ 星标：** 18
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查��修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `ai-docs`, `deepseek-harness`, `dsh-plugin`, `gongwen`
- **推荐理由：** 符合国标的全流程公文处理工具，支持格式修复与内容优化
...
- **标签：** `agent`, `dsh-plugin`, `skills`, `wayfinder`
- **推荐理由：** 游戏化任务系统，迷雾地图引导，增强DSH任务执行体验
...
- **标签：** `documentation`, `best-practices`, `skill`
- **推荐理由：** DSH插件开发实战指南，覆盖常见踩坑与解法
...
- **标签：** `agent-skills`, `canvas`, `agentic-workflow`
- **推荐理由：** 无限画布+创意技能，提升Agent创作工作流
...
- **标签：** `engineering`, `agent-skill`, `practices`
- **推荐理由：** 便携式工程素养技能，倡导证据驱动的Agent开发实践
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，DSH Agent即装即用
...
- **⭐ 星标：** 3
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞前检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件��利用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `capability-index`, `workflow`, `preflight`
- **推荐理由：** 任务型插件预检，自动注入Top-K适用插件
...
- **标签：** `mcp`, `skills`, `webui`, `management`
- **推荐理由：** Web界面管理skills和MCP连接的工具
...
- **标签：** `mcp`, `configuration`, `ui`
- **推荐理由：** 统一菜单管理MCP工具暴露级别与执行模式。
...
- **推荐理由：** 在DSH Web GUI中直接管理技能与MCP服务器，操作便捷
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `mcp`, `zhihu`
- **推荐理由：** 支持知乎搜索、知道问答及官方API，带MCP集成能力
...
- **标签：** `agent-skills`, `ai-design`, `dsh-plugin`, `figma-alternative`, `ui-generator`
- **推荐理由：** 开源设计插件，Figma替代，本地优先桌面应用
...
- **标签：** `ai-image-generation`, `dsh-plugin`, `skills`, `workflow-automation`, `prompt-engineering`
- **推荐理由：** 20+工业级GPT图片提示词模板，提炼为可复用技能
...
- **标签：** `video-generation`, `ai-video`, `dsh-plugin`
- **推荐理由：** 一键生成 Vox 风格拼贴解释视频，全自动化。
...
- **标签：** `ui`, `dsh-plugin`, `design`
- **推荐理由：** 在聊天中直接渲染图表、表单和测验等交互UI组件。
...
- **标签：** `workspace`, `dsh-plugin`, `canvas`
...
- **标签：** `image-generation`, `ai-agent`, `plugin`
- **推荐理由：** 在 Harness 对话中直接生成 AI 图像。
...
- **标签：** `design`, `ui`, `openpencil`, `ppt`
- **推荐理由：** 对话内预览编辑OpenPencil设计稿，UI友好
...
- **标签：** `agent-skills`, `claude-code`, `deepseek-harness`, `diagram`, `dsh-plugin`, `legal`, `timeline`, `visualization`
- **推荐理由：** 法律诉讼可视化工具，生成可直接用于案件的时轴图
...
- **标签：** `design-skills`, `dsh-plugin`, `ui`, `web-design`
- **推荐理由：** 提供6种设计风格，避免AI味，提升网站视觉质感
...
- **标签：** `agent-skills`, `dsh-plugin`, `image-generation`, `vision`
- **推荐理由：** 支持图片读取与生成，自动视觉转录，扩展DSH多模态能力
...
- **标签：** `ai-director`, `dsh-plugin`, `text-to-video`, `storyboard`
- **推荐理由：** AI视频/图像/音频生成技能，支持可配置视觉与分镜工具
...
- **标签：** `background`, `theming`, `webui`, `visual`
- **推荐理由：** 上传自定义背景或切换预设，浏览器端自动压缩
...
- **标签：** `browser-extension`, `dsh-plugin`, `chat-management`, `chrome-extension`, `edge-addon`
- **推荐理由：** 跨浏览器AI工具增强套件，含提示词管理
...
- **标签：** `ai-agent`, `bilibili`, `cross-platform`, `dsh-plugin`
- **推荐理由：** 跨平台内容发现 Agent，支持多平台主动抓取。
...
- **标签：** `web-search`, `dsh-plugin`, `browser`
- **推荐理由：** 最强免费网页搜索桥接插件，无需API即可联网搜索。
...
- **标签：** `mobile`, `automation`, `dsh-plugin`
- **推荐理由：** 在DSH内集成iOS模拟器自动化，22个工具支持真机控制。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `social`
- **推荐理由：** 零权限GitHub OAuth拍立得合影墙，适合内测收官活动使用
...
- **标签：** `dingtalk`, `im`, `chat-bot`, `communication`
- **推荐理由：** 钉钉IM集成，将聊天转化为智能体工具
...
- **标签：** `mcp-server`, `self-hosted`, `dsh-plugin`, `react`
- **推荐理由：** 隐私优先的开源简历生成器，可自建部署
...
- **标签：** `dsh-plugin`, `electron`, `image-uploader`, `s3`, `github`
...
- **标签：** `vfs`, `infrastructure`, `tools`
- **推荐理由：** AI Agent的统一虚拟文件系统
...
- **标签：** `context`, `dsh-external`, `utility`
- **推荐理由：** 上下文管理面板，统计分析与可视化浏览。
...
- **标签：** `widget`, `floating`, `dsh-plugin`
- **推荐理由：** 可爱鲸鱼浮动组件，实时监测账户余额。
...
- **标签：** `oauth`, `llm`, `dsh-plugin`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅转为DSH的LLM提供商。
...
- **标签：** `plugin-marketplace`, `harness`
- **推荐理由：** 精选的 DeepSeek Harness 社区插件集合。
...
- **标签：** `compatibility-layer`, `migration`, `abi`
- **推荐理由：** 打通 Pi 与 DSH 生态的 ABI 兼容层插件。
...
- **标签：** `marketplace`, `plugin-management`, `web-gui`
- **推荐理由：** Web GUI内一键浏览安装更新所有DSH插件
...
- **标签：** `backup`, `rollback`, `windows`, `safety`
- **推荐理由：** 崩溃恢复插件，安全撤销配置变更离线可用
...
- **标签：** `awesome-list`, `plugin`, `resource`
- **推荐理由：** DSH生态精选插件列表，方便发现好插件
...
- **标签：** `devops`, `containers`, `infrastructure`
- **推荐理由：** 容器隔离运行多版本DSH，内嵌WebView界面。
...
- **推荐理由：** 支持备份恢复迁移同步完整DSH配置的WebDAV工具。
...
- **标签：** `skills`, `management`, `utility`
- **推荐理由：** 专门管理DSH技能的组织与控制插件。
...
- **标签：** `cost-tracking`, `data-visualization`, `api`
- **推荐理由：** 实时Token用量图表与DeepSeek API费用追踪
...
- **标签：** `conflict-resolution`, `compatibility`, `utilities`
- **推荐理由：** 自动解决插件冲突和版本兼容问题
...
- **标签：** `marketplace`, `skills`, `extensions`
- **推荐理由：** DSH插件市场，支持技能型扩展的交易平台
...
- **标签：** `verified`, `directory`, `organized`
- **推荐理由：** 70+已验证插件，按功能分类整理
...
- **推荐理由：** 磨砂玻璃主题，支持自定义模糊度和背景，视觉体验极佳。
...
- **标签：** `color-scheme`, `dark-mode`, `dsh-plugin`, `theme`
- **推荐理由：** 一键切换34款opencode经典配色，重启不丢失，即点即用
...
- **标签：** `ai-guide`, `rag`, `vibe-coding`, `dsh-plugin`, `llm`
- **推荐理由：** 全面AI资源库与多模型零基础教程
...
- **推荐理由：** 自动搜索 8 大中文平台近 30 天内容生成研究报告。
...
- **标签：** `awesome-list`, `hub`, `curated`
- **推荐理由：** 深度挖掘DSH外部资源的全量插件索引列表。
...
- **标签：** `awesome-list`, `dsh-plugin`, `research`
- **推荐理由：** 双语精选DSH插件、工具和技能资源列表。
...
- **标签：** `ai-pedia`, `dsh-plugin`, `resource`
- **推荐理由：** 涵盖Agent、RAG、MLOps的全维度AI资源百科。
...
- **标签：** `dsh-plugin`, `learning`, `marketplace`
- **推荐理由：** 学习DSH原理、浏览插件市场与最佳实践的一站式资源。
...
- **⭐ 星标：** 57
- **语言：** TypeScript
- **描述：** 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接搜索 20 个中国开放平台的 65,600+ 篇 API ���档；零配置，支持 Skill 与 DSH 原生插件。
- **标签：** `ai-agents`, `alipay`, `api-documentation`, `chinese-api`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 零配置搜索20个中国开放平台65,600+篇API文档
...
- **标签：** `finance`, `kline`, `analysis`, `dashboard`
- **推荐理由：** 多市场K线交互式分析，支持技术指标与基本面。
...
- **标签：** `evaluation`, `rag-evaluation`, `dsh-plugin`, `prompt-testing`
- **推荐理由：** 基于证据的评估框架，支持提示词、RAG和Agent的可观测性分析
...
- **标签：** `finance`, `quant`, `stock`, `research`
- **推荐理由：** 精选股票/金融/量化插件垂直列表，24条验证条目
...
- **标签：** `plugin-radar`, `discovery`, `awesome-list`
- **推荐理由：** 插件雷达，基于证据的DSH生态发现工具
...
- **标签：** `awesome-list`, `inventory`, `comprehensive`
- **推荐理由：** 千余插件全量清单，按star排序的完整目录
...
- **标签：** `ai-recommender`, `curated`, `hourly-update`
- **推荐理由：** AI推荐190+插件，每小时自动更新精选
...
- **标签：** `awesome-list`, `resources`, `curated`
- **推荐理由：** 精选插件、工具、技能和资源的集合
...
- **标签：** `curated`, `evidence-aware`, `shortlist`
- **推荐理由：** 基于证据的有用DSH插件精选短名单
...
- **标签：** `installable`, `curated`, `typescript`
- **推荐理由：** 可安装的精选插件，覆盖模型、工具和UI
...
- **标签：** `industry`, `sector-specific`, `finance`, `legal`, `medical`
- **推荐理由：** 金融、法律、医疗等行业专属插件目录
...
- **标签：** `security`, `agent`, `dsh-plugin`
- **推荐理由：** AI安全攻防平台，可审计代码漏洞和幻觉包。
...
- **标签：** `armor-breaking`, `dsh-plugin`, `prompt`
- **推荐理由：** DeepSeek专用破甲提示词插件，稳定化越狱提示。
...
- **推荐理由：** DeepSeek Harness 全面指南与精选资源汇总。
...
- **标签：** `awesome-list`, `mcp`, `plugins`
- **推荐理由：** DSH 插件、MCP 服务器与技能的精选目录。
...
- **标签：** `anime`, `bangumi`, `search`, `torrent`
- **推荐理由：** 对话内多源搜番，展示Bangumi评分与详情
...
- **推荐理由：** 复刻锤子便签，一键Docker部署，支持skill调用
...
- **推荐理由：** 官方自动生成的DeepSeek Harness Cordis插件精选列表
...
- **⭐ 星标：** 27
- **语言：** JavaScript
- **描述：** DeepSeek Harness (DSH) 插件精选目录 — 14 类 280+ 个社区插件，覆盖 MCP / Skill / TUI / 多 Agent / 上下���记忆 / UI 皮肤，点链接直达仓库。Curated directory of dsh plugins for DeepSeek Harness.
...
- **推荐理由：** 极速Neovim插件管理器，非DSH插件但值得了解
...
- **标签：** `awesome-list`, `curated`, `permission-aware`
- **推荐理由：** 证据导向精选目录，提供权限安装指南
...
- **标签：** `awesome-list`, `community`, `verified`
- **推荐理由：** 社区验证星级排名插件列表，覆盖DSH生态
...
- **推荐理由：** 实用DeepSeek Harness插件集合，便于使用
...
- **标签：** `awesome-list`, `guide`, `navigation`, `manual`
- **推荐理由：** 按场景选插件的产品说明书与风险导航
...
- **标签：** `portal`, `static-site`, `discovery`, `awesome-list`
- **推荐理由：** 零依赖静态门户，发现并安装227+插件
...
- **标签：** `awesome-list`, `catalog`, `automation`, `directory`
- **推荐理由：** GitHub Actions每日自动爬取的双语插件目录
...
- **标签：** `awesome-list`, `evidence-led`, `bilingual`, `reviewed`
- **推荐理由：** 不可变commit审查的双语文证插件目录
...
- **标签：** `moved`, `redirect`
- **推荐理由：** 仓库已迁移至awesome-dsh-plugin组织
