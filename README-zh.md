...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 211,605 | Core DSH platform powering the entire plugin ecosy |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 93,955 | Open-source Claude-equivalent design tool with loc |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 70,402 | Multi-agent swarm harness for autonomous workflow  |
...
- [MCP 服务器与工具](mcp-servers-tools-zh) (7)
...
- **标签：** `agentic-workflow`, `multi-agent`, `mcp-server`, `dsh-plugin`
- **推荐理由：** 多智能体蜂群编排框架，支持自主工作流与协作
...
- **标签：** `multi-agent`, `mcp`, `skills`, `dsh-plugin`
- **推荐理由：** 具备自我进化记忆的多模型超级AI助手
...
- **推荐理由：** 终端原生DeepSeek编码Agent，支持prefix缓存稳定运行
...
- **标签：** `agent`, `harness`, `dsh-plugin`, `tutorial`
- **推荐理由：** 零基础上手Harness工程的最佳入门教程
...
- **标签：** `preset`, `tools`, `standard`
- **推荐理由：** 两阶段预设，标准化DSH工具链配置
...
- **标签：** `code-review`, `code-quality`, `static-analysis`
...
- **标签：** `armor-breaking`, `code-generation`, `dsh-plugin`
- **推荐理由：** 破甲插件，增强DeepSeek代码生成能力。
...
- **标签：** `coding-agent`, `rpc`, `mcp`, `dsh-plugin`
- **推荐理由：** 支持RPC、子Agent、哈希行编辑和MCP的完整编码Agent
...
- **推荐理由：** TDD/调试/规划技能，源自Superpowers实战
...
- **标签：** `coding`, `skills`, `harness`
- **推荐理由：** 将Superpowers编码技能适配至Reasonix编框。
...
- **标签：** `deepseek-harness`, `dsh`, `container`
- **推荐理由：** 隔离容器运行多版本DSH，内嵌WebView便捷管理
...
- **推荐理由：** 42技能+CLI+插件的Agent工程资产包管理器
...
- **标签：** `agent-tools`, `automation`, `capability`
- **推荐理由：** 任务预检自动注入合适插件，提升插件利用率
...
- **标签：** `browser-extension`, `dsh-plugin`, `chat-management`, `edge-addon`
- **推荐理由：** 浏览器增强插件，覆盖Gemini/Claude/ChatGPT及提示词管理
...
- **标签：** `dsh-web`, `dsh-plugin`, `plugin-ecosystem`
...
- **推荐理由：** 本地优先的Electron桌面工作空间，整合多功能。
...
- **标签：** `dsh-plugin`, `ui`, `charts`, `forms`
- **推荐理由：** 在DSH回复中直接渲染图表、表单、测验等交互UI组件。
...
- **标签：** `desktop-app`, `tauri`, `local-first`, `dsh-plugin`
- **推荐理由：** 本地优先的桌面AI代理，多模型支持，隐私安全。
...
- **标签：** `desktop-app`, `electron`, `gui`, `dsh-plugin`
- **推荐理由：** Windows桌面GUI客户端，支持插件、SSH和移动端远程控制。
...
- **标签：** `ios`, `mobile-automation`, `ui-testing`, `dsh-plugin`
- **推荐理由：** DSH内嵌iOS模拟器，22项Agent工具实现移动端自动化测试。
...
- **标签：** `office`, `deepseek-harness-plugin`, `dsh-plugin`
- **推荐理由：** 将完整办公套件引入DeepSeek Harness桌面端
...
- **标签：** `desktop-app`, `electron`, `deepseek-harness`, `dsh-plugin`
...
- **推荐理由：** Electron桌面管理台，多标签统一管理DSH组件。
...
- **⭐ 星标：** 28
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema �� token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `ui`, `audit`, `dsh-plugin`
- **推荐理由：** 审计上下文注入，检测重复冲突，提供Web UI面板
...
- **标签：** `bioinformatics`, `desktop-app`, `tauri`, `offline`
- **推荐理由：** 生信桌面代理，一键安装离线可用，适合临床和湿实验室
...
- **推荐理由：** 运维一体桌面，SSH Docker数据库终端合而为一
...
- **推荐理由：** Electron桌面壳集成ModLens和Web UI，体验完整
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正���；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `ui`, `background`, `webui`, `customization`
- **推荐理由：** 自定义DSH Web背景，支持预设和实时预览
...
- **标签：** `dsh-plugin`, `ui`, `compact`
- **推荐理由：** 一键折叠DSH上下文面板，简洁高效。
...
- **推荐理由：** 自演进上下文数据库，统一Agent记忆与RAG知识库
...
- **标签：** `rag`, `knowledge-base`, `multi-tenant`, `dsh-plugin`
- **推荐理由：** 开源LLM知识平台，集成RAG、推理Agent与Wiki
...
- **标签：** `agent-memory`, `rag`, `mcp`, `long-term-memory`
- **推荐理由：** 本地优先的Agent记忆层，Markdown原生跨应用
...
- **标签：** `agent-memory`, `agentic-rag`, `mcp`
- **推荐理由：** 状态持久化Agent记忆库，支持持续学习
...
- **标签：** `agent-skills`, `embeddings`, `vector-search`, `skill-routing`
- **推荐理由：** 将SKILL.md转化为可检索的知识库，支持向量搜索和重排序。
...
- **标签：** `notes`, `smartisan`
- **推荐理由：** 私有化便签+可Docker部署，支持skill调用
...
- **标签：** `manager`, `skills`, `security`
- **推荐理由：** 统一加载管理本地技能，保障安全性
...
- **标签：** `rag-evaluation`, `prompt-testing`, `knowledge-engineering`
- **推荐理由：** 基于证据的评估框架，支持提示词RAG智能体评测
...
- **标签：** `agent-skills`, `diagrams-as-code`, `mermaid-alternative`, `dsh-plugin`
- **推荐理由：** AI生成精美架构流程图，支持序列/数据流可视化
...
- **标签：** `skills`, `workflow-automation`, `prompt-engineering`, `dsh-plugin`
- **推荐理由：** 530+工业级提示词模板，提炼为可复用Agent技能
...
- **标签：** `dsh-plugin`, `no-code`, `ai-agent`, `internal-tools`
...
- **标签：** `agent-orchestration`, `mcp`, `no-code`, `dsh-plugin`
...
- **标签：** `agent-os`, `mcp`, `skills`, `llm-orchestration`
- **推荐理由：** 自进化Agent OS，评估门控迭代升级
...
- **标签：** `agent-control-plane`, `workflow-automation`, `dsh-plugin`
- **推荐理由：** 长周期Agent控制面，持久化任务治理
...
- **标签：** `video-generation`, `ffmpeg`, `skill`
- **推荐理由：** 一键生成Vox风格解说视频，端到端全自动。
...
- **标签：** `dsh-plugin`, `workflow`, `canvas`, `conversation`
- **推荐理由：** 可视化画布支持分支对话和非线性会话探索。
...
- **标签：** `task-management`, `dsh-plugin`, `sqlite`
- **推荐理由：** SQLite驱动的任务看板，支持Agent认领审核，结构化项目管理。
...
- **标签：** `creative-writing`, `drama-skills`, `fiction`, `dsh-plugin`
- **推荐理由：** 专为小说和短剧创作设计的技能插件，功能丰富。
...
- **标签：** `agent`, `math-modeling`, `skill`, `gitops`
- **推荐理由：** 全栈式自动数学建模Skill，支持GitOps流水线与代码自证
...
- **标签：** `agent-review`, `plan-mode`, `self-evolving`, `dsh-plugin`
...
- **标签：** `agent-skill`, `coding`, `mentor`
- **推荐理由：** AI编程导师技能，阶梯式引导手写核心逻辑。
...
- **标签：** `legal`, `timeline`, `agent-skills`, `svg`
- **推荐理由：** 法律诉讼时间轴可视化工具，适合法律人使用
...
- **推荐理由：** 自带25个工程技能，支持GitHub Issue处理
...
- **标签：** `plugin-dev`, `skills`, `llm-agent`
- **推荐理由：** DSH插件开发标准流程技能，含打包安装
...
- **标签：** `spec`, `vibe-coding`, `agent-skills`
- **推荐理由：** 仓库原生规范驱动开发技能，提升编码质量
...
- **标签：** `guide`, `knowledge-base`, `plugin-dev`
- **推荐理由：** 插件开发知识库，按需调用，中英双语
...
- **标签：** `creator`, `codex`, `validation`
- **推荐理由：** 可复用的插件构建验证技能，支持Codex
...
- **标签：** `multi-agent`, `team-collaboration`, `dsh-plugin`
- **推荐理由：** 多Agent协作，独立模型技能与共享工作区
...
- **标签：** `fintech`, `backtesting`, `mcp`, `llm-agents`
- **推荐理由：** 合规优先AI投资台，多策略回测，金融研究必备
...
- **标签：** `project-governance`, `swift-cycle`, `offline`
- **推荐理由：** 版本锁定离线可验的治理技能适配器，项目合规保障
...
- **标签：** `dsh-plugin-bundle`, `skills`, `marketplace`
- **推荐理由：** 类CodeX技能市场，扩展DSH能力。
...
- **标签：** `web-search`, `dsh-plugin`, `mcp`
- **推荐理由：** 免费网络搜索插件，无需注册，适配所有无联网能力的模型。
...
- **推荐理由：** 开源本地优先设计工具，类Claude的AI设计替代方案
...
- **标签：** `ai-video`, `ai-image`, `text-to-video`, `storyboard`
- **推荐理由：** AI音视频图生成，支持可配置模型工具，内容创作利器
...
- **标签：** `content-discovery`, `bilibili`, `multi-platform`
- **推荐理由：** 跨平台内容发现Agent，本地优先兼顾隐私
...
- **推荐理由：** 基于Playwright的浏览器自动化插件，功能强大。
...
- **标签：** `dingtalk`, `communication`, `bot`, `im`
- **推荐理由：** 钉钉IM渠道机器人，支持流式Agent交互。
...
- **标签：** `dsh-plugin`, `electron`, `image-uploader`, `s3`
- **推荐理由：** 支持60+图床的终极图片上传工具，创作效率神器
...
- **标签：** `plugin-registry`, `ecosystem`, `utilities`
- **推荐理由：** 自动发现并实测15900+插件的生态雷达工具。
...
- **标签：** `security`, `redteam`, `vulnerabilities`, `dsh-plugin`
- **推荐理由：** AI安全审计平台，检测幻觉包和漏洞，保障代码安全。
...
- **标签：** `oauth`, `subscriptions`, `dsh-plugin`, `utility`
- **推荐理由：** 通过OAuth将已有ChatGPT/Claude订阅转为DSH的LLM提供者。
...
- **标签：** `cost-tracking`, `deepseek-api`, `token-usage`, `dsh-plugin`
- **推荐理由：** 实时追踪会话/每日成本、预算和API用量
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `plugin-marketplace`
- **推荐理由：** 学习DSH原理、浏览插件市场、发现最佳实践
...
- **标签：** `plugin-store`, `marketplace`, `catalog`, `dsh-plugin`
- **推荐理由：** 11,000+插件商店，支持搜索、排行和免费API
...
- **标签：** `catalog`, `plugin-marketplace`, `directory`, `awesome`
- **推荐理由：** 1958个仓库的综合性DSH插件目录，支持中英文搜索筛选
...
- **标签：** `compatibility-layer`, `migration`, `pi-agent`, `dsh-plugin`
- **推荐理由：** 通过兼容层将Pi扩展直接作为DSH原生插件运行
...
- **标签：** `context-migration`, `dsh-plugin`
- **推荐理由：** 跨预设无缝迁移会话状态与上下文
...
- **标签：** `remote-access`, `dsh-plugin`
- **推荐理由：** P2P加密远程访问，手机电脑随时操作DSH
...
- **标签：** `documentation`, `api`, `skillhub`
- **推荐理由：** 腾讯官方技能 Hub API 文档与插件示例，开发参考
...
- **标签：** `catalog`, `community`, `directory`
- **推荐理由：** 非官方社区插件目录，探索 DSH 生态必备
...
- **推荐理由：** 公开 GitHub 插件目录，含安装命令和生态发现
...
- **推荐理由：** 快速优雅的 Neovim 插件管理器，纯 Lua 编写
...
- **推荐理由：** 实时 Token 用量成本追踪，按轮次 API 余额图表
...
- **标签：** `registry`, `plugins`, `open-data`
- **推荐理由：** 过滤垃圾的开放数据插件注册表，纯净可靠
...
- **标签：** `conflict-resolution`, `utilities`, `dsh`
- **推荐理由：** 自动解决插件冲突和版本不兼容问题
...
- **标签：** `ai`, `deepseek`, `rag`, `vibecoding`
- **推荐理由：** AI资源大全与教程知识库，适合入门学习
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugin`, `guide`
...
- **标签：** `awesome-list`, `mcp`, `coding-agent`, `dsh-plugins`
- **推荐理由：** 全面的插件、技能、MCP服务器和补丁精选清单
...
- **标签：** `finance`, `multi-agent`, `mcp`, `tushare`
- **推荐理由：** 基于Tushare的多Agent金融分析系统，支持MCP和IM盯盘
...
- **标签：** `research`, `latex`, `arxiv`, `gpu-cluster`, `dsh-plugin`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪和GPU调度
...
- **标签：** `academic-search`, `research-workflow`, `mcp`
- **推荐理由：** 跨源学术检索+引文图谱与核验，科研必备
...
- **⭐ 星标：** 58
- **语言：** Python
- **描述：** 专利侵权分析系统 —— 输入专利公开号，产出竞品侵权分析报告；同时打包成 skill，可被任��� agent（dsh, codex, claudecode 等） 调用。
...
- **标签：** `awesome-list`, `bilingual`, `review`
- **推荐理由：** 双语文档插件目录，附带审核记录。
...
- **标签：** `recommender`, `plugin-list`, `ai-curated`
- **推荐理由：** AI推荐190+插件，覆盖12大分类。
...
- **标签：** `awesome-list`, `auto-update`, `bilingual`
- **推荐理由：** 自动更新双语文档，Vercel部署支持。
...
- **标签：** `awesome-list`, `evidence-aware`, `shortlist`
- **推荐理由：** 基于证据的DSH插件精选清单。
...
- **标签：** `dsh-plugin`, `self-hosted`, `mcp-server`, `typescript`
- **推荐理由：** 隐私优先的自托管简历构建工具，完全可定制
...
- **标签：** `awesome-list`, `dsh`, `dsh-plugin`
- **推荐理由：** DSH插件精选列表，发现优质插件的最佳入口
...
- **标签：** `developer-tools`, `mascot`, `dsh-plugin`
- **推荐理由：** AI编程Agent社区宠物画廊，趣味互动
...
- **标签：** `awesome-list`, `curated-list`, `plugins`
- **推荐理由：** 双语插件精选目录，DSH生态入门��选资源。
...
- **标签：** `awesome-list`, `mcp`, `rag`, `ai-pedia`
- **推荐理由：** 一站式AI资源百科，涵盖Agent、RAG、MCP及学习路线。
...
- **标签：** `awesome-list`, `plugin-curator`, `dsh-plugin`
- **推荐理由：** 每日自动精选推荐插件，场景化分类，快速上手指南。
...
- **推荐理由：** 多源番剧搜索+Bangumi评分，ACG爱好者利器
...
- **标签：** `chinese-wisdom`, `fengshui`, `bazi`, `dashboard`
...
- **标签：** `awesome-list`, `quality-check`, `dsh`
- **推荐理由：** 经质量筛选的2026年插件清单，节省探索时间
...
- **标签：** `finance`, `quant`, `trading`, `awesome-list`
- **推荐理由：** 专注金融量化插件垂直列表，交易Agent必备
...
- **标签：** `verified`, `plugin-directory`, `safety`
- **推荐理由：** 证据驱动的已验证插件目录，附安全安装指南
...
- **标签：** `documentation`, `plugin-directory`, `guide`
- **推荐理由：** 插件说明书和导航，含兼容性和风险提示
