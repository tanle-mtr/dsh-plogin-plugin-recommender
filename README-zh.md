...
- **标签：** `router`, `injector`, `runtime`
- **推荐理由：** DSH运行时任务感知路由与注入套件
...
- **推荐理由：** 首个视觉插件，为文本代理提供OCR图像转JSON功能
...
- **标签：** `vfs`, `virtual-terminal`, `dsh-plugin`, `agent-sandbox`
- **推荐理由：** 首个AI代理虚拟终端，支持虚拟化文件系统和终端
...
- **标签：** `context`, `dashboard`, `harness-tool`
- **推荐理由：** DSH最佳上下文洞察、仪表板与管理工具
...
- **推荐理由：** 支持RPC子Agent、hashline编辑和MCP的DSH编码代理。
...
- **推荐理由：** 为Claude Code添加多模态视觉，支持截图/UI分析
...
- **标签：** `plugin-creator`, `validation`, `codex`
- **推荐理由：** 用于构建和验证DSH插件的可复用工具，开发者必备。
...
- **标签：** `workbench`, `typescript`, `skills`
- **推荐理由：** 基于DSH的插件化Web工作台，全局技能管理，替代WorkBuddy。
...
- **标签：** `code-simplification`, `codex`, `refactor`
- **推荐理由：** 官方DSH灵感的代码简化Codex技能，自动化重构。
...
- **标签：** `dsh-plugin`, `codex`, `mcp`, `sync`
- **推荐理由：** Codex与DSH双向同步桥梁，支持会话和技能同步
...
- **标签：** `dsh-plugin`, `cordis`, `infra`
- **推荐理由：** 统一编排DSH核心能力：运行时、插件与MCP调度
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话/Git等面板
...
- **标签：** `taskboard`, `dsh-plugin`, `cli`
- **推荐理由：** 现代化可嵌入任务面板，灵活支持Codex和DSH工作流
...
- **标签：** `floating-widget`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** DSH右下角萌系鲸鱼挂件，实时监视账户余额
...
- **标签：** `agentic-runtime`, `desktop-app`, `ai-ide`
- **推荐理由：** Rust高性能代理运行时配合精美桌面应用
...
- **推荐理由：** 5MB轻量级Tauri桌面版DeepSeek Harness
...
- **推荐理由：** 零配置Windows桌面GUI客户端，支持Codex、SSH和移动远程访问。
...
- **标签：** `desktop-app`, `local-first`, `react`, `typescript`
- **推荐理由：** 本地优先桌面工作区，整合DSH会话、项目、文件和MCP。
...
- **标签：** `dsh-plugin`, `interactive-ui`, `charts`, `forms`
...
- **推荐理由：** 本地优先Tauri桌面AI代理，支持多模型和隐私优先设计。
...
- **标签：** `office`, `desktop-extension`, `ui`
- **推荐理由：** 将电子表格文档幻灯片等办公套件引入DSH
...
- **标签：** `notes`, `ui`, `docker`
- **推荐理由：** 开源锤子便签，支持Docker私有化+skill调用
...
- **推荐理由：** Electron桌面控制台，一站式管理插件与MCP
...
- **推荐理由：** 持久信息栏显示provider、余额和定价，实用UI增强。
...
- **标签：** `bioinformatics`, `desktop`, `single-cell`
- **推荐理由：** 面向临床和实验科学家的生物信息学桌面代理，离线可用。
...
- **标签：** `dsh-plugin`, `ui`
- **推荐理由：** 类WorkBuddy技能选择器，提升DSH Composer体验
...
- **⭐ 星标：** 7
- **语言：** JavaScript
- **描述：** 桌面版 DeepSeek Harness：Electron 壳 + 自托管 dsh web 服���，集成 dsh-better-sidebar / dsh-web-ui / ModLens / awesome-dsh-plugin / dshmarket
...
- **标签：** `knowledge-cards`, `ui`, `awesome-dsh-plugin`
...
- **标签：** `long-term-memory`, `memory-management`, `rag`
- **推荐理由：** 自演进记忆系统，混合检索大幅节省token消耗
...
- **标签：** `long-term-memory`, `personal-ai`, `mcp`
- **推荐理由：** 共享持久记忆中枢，统一所有AI代理上下文
...
- **标签：** `agent-memory`, `skill-routing`, `vector-search`, `embeddings`
- **推荐理由：** 将分散的SKILL.md转为可检索Agent技能语料库，���评估工具。
...
- **标签：** `mcp`, `rag`, `ai-knowledge`, `resource-encyclopedia`
- **推荐理由：** 涵盖技能、MCP、RAG和多模态工具的AI资源百科大全。
...
- **标签：** `context`, `memory`, `deepseek-harness`
- **推荐理由：** 零提示关联记忆，三层自动知识整合机制
...
- **推荐理由：** 过滤垃圾插件， curated DSH 插件注册表，便于生态发现
...
- **推荐理由：** 公开GitHub目录，含安装命令，便于插件发现与安装
...
- **标签：** `awesome-list`, `catalog`, `inventory`
...
- **标签：** `agent-workspaces`, `agent-orchestration`, `tasks`
- **推荐理由：** 统一的智能体工作空间，支持跨设备任务追踪
...
- **标签：** `skills`, `marketplace`, `skill-store`
- **推荐理由：** 最全技能商店，数千个开箱即用的技能包
...
- **推荐理由：** 模型无关的推理控制套件，支持深度推理与长程验证恢复
...
- **标签：** `agent-teams`, `agentic`, `dsh-plugin`
- **推荐理由：** 在DSH中实现多代理团队协作协调
...
- **标签：** `vision-tools`, `ocr`, `agent-skills`
- **推荐理由：** 为纯文本模型提供强大视觉能力的DSH插件
...
- **标签：** `agent-skills`, `video-analysis`, `multimodal`
- **推荐理由：** 赋予AI视觉听觉能力，支持视频音频屏幕感知
...
- **标签：** `ios`, `automation`, `agent-skills`
- **推荐理由：** iOS模拟器自动化配备22个Agent工具
...
- **推荐理由：** 数学建模全栈Skill，GitOps+代码自证，国赛美赛通用
...
- **标签：** `agent-review`, `self-evolution`, `plan-mode`
- **推荐理由：** Agent可组合审查与自我进化，Web插件形式
...
- **推荐理由：** 将经验转化为可复用Skill，支持多Agent平台
...
- **推荐理由：** 88个开源Skill覆盖研究/营销/社交情报工作流
...
- **推荐理由：** AI基础设施开发优化调试专用Agent Skills
...
- **推荐理由：** 包含87个SKILL.md的逆向工程和渗透测试技能包
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`
- **推荐理由：** 网页搜索与研究技能，支持多平台AI代理
...
- **标签：** `deepseek`, `dsh`, `dsh-plugins`
- **推荐理由：** 统一加载和安全管理局部Agent Skills
...
- **⭐ 星标：** 54
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与���复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `ai-docs`, `gongwen`, `dsh-plugin`
- **推荐理由：** 符合国标的公文全流程处理，含格式检查和模板生成
...
- **标签：** `ai-agent`, `bazi`, `fengshui`, `iching`
- **推荐理由：** 融合八字、风水、奇门等传统智慧，本地确定性引擎，独特点子技能。
...
- **标签：** `handoff`, `session-memory`, `codex`
- **推荐理由：** 跨平台智能体交接技能，保持会话间上下文连续性。
...
- **标签：** `dsh-plugin`, `skills`, `tencent`
- **推荐理由：** 腾讯官方SkillHub开放API文档及DSH插件
...
- **推荐理由：** 开发DSH插件的Agent Skill，遵循标准规范，兼容Claude Code
...
- **标签：** `deepseek-harness`, `coding-agents`, `registry`, `skills`, `ui`
- **推荐理由：** DSH插件注册表，覆盖模型、工具和代理。
...
- **标签：** `dsh-plugin`, `mcp`, `agent-config`
- **推荐理由：** 一键为多款AI Agent框架配置MCP和服务技能
...
- **推荐理由：** AI视频/图像/音频创作套件，含分镜和文生视频能力。
...
- **标签：** `dsh-plugin`, `video`, `remotion`
- **推荐理由：** 电影感产品视频Agent技能，含152张镜头配方卡
...
- **推荐理由：** 一键安装切换Claude Code、Codex、DeepSeek等多种AI编码工具
...
- **标签：** `plugin-shop`, `marketplace`, `ecosystem`
- **推荐理由：** DSH全面每日更新的插件市场
...
- **标签：** `cybersecurity`, `redteam`, `vulnerabilities`, `aisecurity`
- **推荐理由：** AI安全攻防平台，审计幻觉包和代码漏洞的安全插件。
...
- **标签：** `llm-provider`, `oauth`, `deepseek-harness-plugin`
- **推荐理由：** OAuth登录ChatGPT/Claude/Grok作为LLM提供商
...
- **标签：** `llm-provider`, `commandcode`, `infrastructure`
- **推荐理由：** 添加Command Code模型支持和计划感知选择
...
- **标签：** `awesome-list`, `marketplace`, `plugin-directory`
- **推荐理由：** 11000+插件市场，含搜索API和安装命令
...
- **推荐理由：** 全面的Agent运行时手册，覆盖插件/MCP/排障
...
- **标签：** `catalog`, `plugin-directory`, `awesome-list`
- **推荐理由：** 1958仓库插件总目录，支持中英文搜索分类
...
- **标签：** `plugin-discovery`, `automation`, `utilities`
...
- **标签：** `containers`, `management`, `infrastructure`
- **推荐理由：** 容器化管理多个DSH实例，内嵌WebView UI，便捷部署。
...
- **标签：** `dsh-plugin`, `migration`, `codex`
- **推荐理由：** Claude Code/Codex等工具数据迁移到DSH向导
...
- **推荐理由：** 实时追踪Token用量与费用估算，支持API余额查看
...
- **标签：** `debugging`, `conflict-resolution`, `utilities`
- **推荐理由：** 解决插件间及版本冲突问题，提供解决方案
...
- **标签：** `theme`, `glassmorphism`, `customization`, `dsh-plugin`
- **推荐理由：** 高度可定制DSH网页端毛玻璃主题，支持调节磨砂度和背景。
...
- **标签：** `ai-tutorial`, `prompt-collection`, `rag`
- **推荐理由：** 全面AI资源库，涵盖DeepSeek/GPT/Claude教程
...
- **标签：** `awesome-list`, `bilingual`, `plugins`, `curated-list`
- **推荐理由：** 双语精选DSH插件目录和验证参考资���列表。
...
- **标签：** `arxiv`, `latex`, `paper-writing`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv管理、实验追踪、GPU SSH。
...
- **标签：** `awesome-list`, `plugin-curation`, `discovery`
- **推荐理由：** 每日自动抓取GitHub并验证的精选插件目录
...
- **标签：** `awesome-list`, `resources`, `discovery`
- **推荐理由：** 全面的DSH插件技能MCP精选列表
...
- **标签：** `finance`, `mcp`, `stock-analysis`
- **推荐理由：** 基于tushare的财经数据库+多Agent金融分析
...
- **标签：** `dsh-plugin`, `math-modeling`, `agent`
- **推荐理由：** 5角色数学建模多Agent求解器，含33篇模型库
...
- **标签：** `dsh-plugin`, `math-modeling`, `research`
- **推荐理由：** 涵盖建模、写作、科研绘图的全套数学建模资源
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `ai-agents`
- **推荐理由：** 精心整理的DSH插件生态目录
...
- **标签：** `dsh-plugin-market`, `dsh-plugins`, `awesome-dsh-plugin`
- **推荐理由：** DeepSeek Harness的CodeX风格插件市场
...
- **标签：** `plugin-aggregation`, `dsh-web`, `ecosystem`
...
- **推荐理由：** DSH插件、工具、技能与资源的综合Awesome列表
...
- **标签：** `dsh-plugin`, `plugin-verify`, `awesome-list`
- **推荐理由：** DSH插件合集，含插件验证与解析功能。
