...
- [实用工具与基础设施](utilities-infrastructure-zh) (30)
...
- **标签：** `agent-orchestration`, `mcp`, `dsh-plugin`
- **推荐理由：** 统一智能体工作空间，多端无缝同步
...
- **标签：** `agent-ops`, `workflow-automation`, `dsh-plugin`
- **推荐理由：** 长周期智能体控制面，支持持久化工作流
...
- **标签：** `agent-runtime`, `coding`, `rust`, `desktop-app`
- **推荐理由：** Rust高性能Agent��行时，搭配精美桌面应用。
...
- **标签：** `vision`, `ocr`, `screenshot`, `computer-vision`
- **推荐理由：** 为纯文本模型提供OCR、截图与多图问答的视觉工具集。
...
- **标签：** `coding-agent`, `mcp`, `cli`, `tui`
- **推荐理由：** 编码Agent，支持RPC插件、子Agent、哈希行编辑和MCP
...
- **推荐理由：** 开源CLI和Schema管理工具，用于DSH插件Hub解析安装
...
- **标签：** `launcher`, `orchestration`, `cordis`
- **推荐理由：** 统一编排DSH运行时、插件、技能和MCP能力
...
- **推荐理由：** 增强DSH：任务通知、LLM追踪、快捷键、内置终端。
...
- **⭐ 星标：** 3,671
- **语言：** TypeScript
- **描述：** 开放的侧边栏底座，支持三方拓展注册新侧边栏页面。内置文件渲染编辑/终端/侧边对话/Git/子代理���面 ｜ Open sidebar foundation, supports third-party extensions to register new sidebar pages. Built-in file rendering/editing, terminal, side chat, Git, and sub-agent pages.
- **标签：** `sidebar`, `ui`, `extension`
- **推荐理由：** 可扩展侧边栏，支持文件/终端/Git/子代理
...
- **⭐ 星标：** 3,103
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录的 TUI 补位插件：Claude Code 风，鲸鱼顶栏/实时状态/流式思考/双击 Esc ��滚/上下文进度+TPS。npm 一键装。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏与实时状态
...
- **标签：** `widget`, `balance`, `dsh-plugin`, `floating`
- **推荐理由：** 可爱鲸鱼浮动控件，实时追踪DeepSeek账户余额。
...
- **标签：** `desktop`, `tauri`, `dsh-desktop`, `installer`
- **推荐理由：** 轻量Tauri桌面版，仅需5MB无需配置即可使用。
...
- **标签：** `electron`, `gui`, `windows-desktop`, `ssh-client`
- **推荐理由：** 零配置Windows桌面客户端，集成Codex、SSH、插件和远程访问
...
- **标签：** `desktop-app`, `mcp`, `local-first`, `react`
- **推荐理由：** 本地优先的AI桌面工作区，支持会话、项目、网页研究和Office
...
- **推荐理由：** 在回复中内联生成交互式UI组件：布局、图表、表单和测验
...
- **标签：** `dsh-plugin`, `conversation`, `canvas`
- **推荐理由：** 可视化非线性对话工作区，支持画布式会话探索和分支
...
- **标签：** `desktop-app`, `local-first`, `mcp`, `privacy`
- **推荐理由：** 本地优先多模型AI桌面Agent，自进化技能与隐私优先设计
...
- **标签：** `office`, `ui-extension`, `spreadsheet`, `documents`
- **推荐理由：** 将电子表格、文档、幻灯片等办公组件直接集成到DSH界面中。
...
- **标签：** `mobile`, `ios`, `android`, `kmp`
- **推荐理由：** 通过WebSocket实现iOS/Android原生客户端，移动端管理DSH工作区。
...
- **标签：** `desktop-app`, `electron`, `macos`, `windows`, `linux`
- **推荐理由：** 类CodePilot的跨平台桌面客户端，附带完整插件套件。
...
- **标签：** `notes`, `ui`, `docker`
- **推荐理由：** 开源锤子便签，一键Docker部署，支持Skill调用与导出。
...
- **标签：** `capability`, `menu`, `ui`
- **推荐理由：** 统一能力管理，支持常驻/按需/禁用三档策略
...
- **推荐理由：** Electron桌面管理控制台，多Tab管理插件和Skill
...
- **标签：** `web-platform`, `workbench`, `skills`, `typescript`
- **推荐理由：** 基于DSH的插件化Web工作平台，全局Skill管理
...
- **标签：** `ui`, `usage-tracking`, `dsh-plugins`
- **推荐理由：** 持久状态栏显示模型信息与实时价格
...
- **⭐ 星标：** 12
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运���每天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `devops`, `docker`, `ssh`
- **推荐理由：** 一站式DevOps桌面：数据库、SSH、SFTP、Docker、AI集成。
...
- **推荐理由：** Electron桌面壳，集成DSH Web服务和插件。
...
- **标签：** `memory`, `rag`, `skills`, `dsh-plugin`
...
- **标签：** `agent-memory`, `rag`, `continual-learning`
- **推荐理由：** 构建持久化状态智能体的记忆库
...
- **标签：** `memory`, `rag`, `milvus`, `embeddings`, `agent-memory`
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持多AI共享。
...
- **标签：** `memory`, `agent`, `personal-ai`, `mcp`
- **推荐理由：** 本地优先的个人AI记忆中枢，跨AI持久共享记忆。
...
- **标签：** `agent-memory`, `embeddings`, `semantic-search`, `vector-search`
- **推荐理由：** 将SKILL.md文件转为可检索的Agent技能库，支持评估工具
...
- **标签：** `memory`, `context`, `rag`, `associative`
- **推荐理由：** 三层主动联想记忆，自动整合和技能结晶
...
- **标签：** `swarm-intelligence`, `multi-agent`, `agentic-workflow`
- **推荐理由：** 多智能体蜂群部署，自主工作流协调。
...
- **标签：** `cognition`, `reasoning`, `control`
- **推荐理由：** 推理时控制套件，支持深度推理与验证恢复
...
- **标签：** `writing`, `novel`, `rag`, `long-form`, `creative`
- **推荐理由：** 全流程AI小说创作工具，支持角色与世界构建。
...
- **标签：** `agent-observability`, `agent-skills`, `computer-vision`, `multimodal`
- **推荐理由：** 多模态智能体技能，提供视频/音频/屏幕观测与可验证证据。
...
- **标签：** `agent-skills`, `creative-writing`, `novel-writing`, `drama-skills`
...
- **标签：** `ios`, `mobile-automation`, `webdriveragent`, `ui-automation`
- **推荐理由：** 对话内操控iOS模拟器，22项Agent工具实现移动自动化。
...
- **推荐理由：** 数学建模全栈自动化Skill，支持国赛美赛与代码自证。
...
- **推荐理由：** 可组合、可审查的Agent自进化工作流，支持人工审核。
...
- **推荐理由：** 将经验转化为可复用技能，支持多平台AI Agent。
...
- **推荐理由：** 为Claude Code赋能多模态视觉，支持截图与UI分析。
...
- **推荐理由：** 全面的逆向工程与渗透测试技能包，含87个SKILL.md文件
...
- **标签：** `web-search`, `research`, `mcp-server`, `agent-skills`
- **推荐理由：** 为AI代理提供网页搜索和研究的技能包
...
- **标签：** `coding-agent`, `mentor`, `code-review`, `agent-skill`
- **推荐理由：** AI编程导师技能，扫描架构并生成阶梯式教程
...
- **标签：** `skills-deck`, `agent-skills`, `wayfinder`, `engineering`
- **推荐理由：** 25个内置工程技能面板，安装即用无需手动装配
...
- **标签：** `api-docs`, `chinese-platforms`, `search`, `integration`
- **推荐理由：** 聚合20个中国开放平台65,600+篇API文档零配置搜索
...
- **标签：** `plugin-management`, `registry`, `development`, `ui`
- **推荐理由：** 插件生态基建，浏览器面板管理+开发引导
...
- **标签：** `skills`, `manager`, `load`, `safety`
- **推荐理由：** 统一加载管理本地Agent Skills的安全工具
...
- **⭐ 星标：** 56
- **语言：** Python
- **描述：** 公文全流程处理��具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `chinese`, `standards`, `office`
- **推荐理由：** 符合国标党政机关公文格式的全流程处理技能
...
- **标签：** `agent-skills`, `developer-tools`
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `ai-agent`
- **推荐理由：** 八字风水易经融合本地Agent技能包
...
- **标签：** `multi-agent`, `agent-orchestration`, `session-memory`
- **推荐理由：** 跨会话持久化记忆多Agent协作系统
...
- **标签：** `skillhub`, `official`, `tencent`
- **推荐理由：** 腾讯官方SkillHub插件，含开放API文档和示例
...
- **标签：** `math-modeling`, `data-prep`, `writing`
- **推荐理由：** 数学建模全套技能：数据处理、写作、绘图
...
- **标签：** `workflow`, `planning`, `collaboration`
- **推荐理由：** 先议后行机制，提升AI协作任务效率
...
- **推荐理由：** 一键同步MCP和Skill配置到多个AI Agent工具
...
- **标签：** `design`, `mcp`, `ui`, `dsh-plugin`
- **推荐理由：** 开源AI原生矢量设计工具，支持并发智��体
...
- **标签：** `ai-video`, `ai-image`, `text-to-video`
- **推荐理由：** AI视频图像音频创作与分镜脚本支持
...
- **推荐理由：** DSH最强免费网页搜索插件，模型无需API即可联网搜索
...
- **标签：** `browser-automation`, `anti-detect-browser`, `mcp-server`
- **推荐理由：** 为Agent提供持久身份的浏览器，支持指纹伪装和本地配置
...
- **推荐理由：** 基于Playwright的Tabbit浏览器自动化集成
...
- **推荐理由：** 终极图床上传工具，支持60+服务与Obsidian。
...
- **标签：** `plugin-discovery`, `validation`, `ecosystem`, `registry`
- **推荐理由：** 自动发现并验证2万+DSH插件的生态系统雷达工具。
...
- **标签：** `context`, `dashboard`, `management`, `utility`
- **推荐理由：** 全面的DSH上下文仪表盘、浏览器与分析管理工具。
...
- **标签：** `awesome-list`, `curated`, `ecosystem`, `reference`
...
- **标签：** `cybersecurity`, `aisecurity`, `vulnerabilities`, `redteam`
- **推荐理由：** AI安全攻防平台，审计代码幻觉包、漏洞和模式错误
...
- **标签：** `oauth`, `llm-provider`, `chatgpt`, `claude`
- **推荐理由：** OAuth登录支持ChatGPT/Claude/Grok订阅作为LLM提供商，免API密钥。
...
- **推荐理由：** SQLite驱动的本地任务管理，支持Agent认领/审核和原生Web UI。
...
- **标签：** `llm-provider`, `command-code`, `model-provider`
- **推荐理由：** 集成Command Code模型提供商，支持实时目录和计划感知选择。
...
- **推荐理由：** 本地财经数据库，自动生成API与MCP，支持多Agent。
...
- **标签：** `audit`, `skill-management`, `utility`
- **推荐理由：** DSH技能调用审计与应用内管理器，支持CORDIS集成。
...
- **标签：** `awesome-list`, `directory`, `plugins`, `discovery`
- **推荐理由：** 14类280+个DSH插件精选目录，一键直达仓库
...
- **标签：** `container`, `plugin-manager`, `isolation`
- **推荐理由：** 多版本隔离容器管理DSH，一键导入插件和技能
...
- **标签：** `migration`, `compatibility`, `tool-conversion`
...
- **标签：** `subscription`, `oauth`, `chatgpt`
- **推荐理由：** OAuth绑定ChatGPT账号，扩展模型能力。
...
- **标签：** `mascot`, `pixel-art`, `ui`
- **推荐理由：** 为多编码代理提供可爱动画宠物吉祥物
...
- **⭐ 星标：** 405
- **语言：** JavaScript
- **描述：** 是一层高自由度���玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `dsh-plugin`, `glass-effect`
- **推荐理由：** 高度可定制磨砂玻璃主题，调节模糊度和背景壁纸效果
...
- **标签：** `animated-background`, `glassmorphism`, `theme`, `skin`
- **推荐理由：** 动态海洋主题皮肤，支持海浪、日落及玻璃态效果调节。
...
- **标签：** `background`, `theming`, `webui`
- **推荐理由：** 支持自定义背景图片及预设氛围，UI个性化首选。
...
- **标签：** `ai-guide`, `vibe-coding`, `prompt-library`
- **推荐理由：** 全面AI资源库，含教程、提示词与知识库。
...
- **标签：** `documentation`, `analysis`, `wiki`
...
- **推荐理由：** 每日自动抓取并精选的插件目录，含场景分类和热度排行。
...
- **标签：** `awesome-list`, `plugins`, `mcp`, `patches`
- **推荐理由：** 全面的DSH插件、技能、MCP服务器和patch精选资源列表。
...
- **推荐理由：** 学习DSH原理、探索插件市场和发现最佳实践的综合平台。
...
- **标签：** `handbook`, `reference`, `awesome-list`
- **推荐理由：** 173个来源的权威手册，覆盖运行时、插件和MCP。
...
- **标签：** `patent`, `legal-tech`, `rag`, `analysis`, `claim-chart`
...
- **标签：** `legal`, `visualization`, `timeline`, `svg`, `litigation`
...
- **标签：** `radar`, `registry`, `2origin`
- **推荐理由：** 证据驱动的插件雷达，含评测数据。
...
- **标签：** `catalog`, `automation`, `directory`
- **推荐理由：** GitHub Actions自动爬取更新目录。
...
- **标签：** `dsh-plugin`, `harness-engineering`, `llm`
...
- **标签：** `marketplace`, `plugin-shop`, `discovery`, `community`
- **推荐理由：** 每日更新的DSH插件市场，含社区审核与评分。
...
- **推荐理由：** 双语精选DSH插件、扩展、技能客户端的权威资源列表
...
- **标签：** `dsh-bundle`, `knowledge-base`, `ai-encyclopedia`
- **推荐理由：** 覆盖LLM、Agent、RAG、多模态的全维度AI资源百科导航
...
- **⭐ 星标：** 243
- **语言：** JavaScript
- **描述：** DeepSeek Harness plugin store, marketplace and hub — 11,000+ dsh plugins with search, rankings, install commands and a free public API. DeepSeek Harness 插件市场 / 插件��店：自动收集与格式校验，免费搜索 API。deepseek1024.com
- **标签：** `catalog`, `marketplace`, `awesome-list`
- **推荐理由：** 最大DSH插件市场，超1万插件，支持搜索、排名和API。
...
- **推荐理由：** DSH插件总目录，1958仓库，中英文搜索与分类筛选。
...
- **推荐理由：** 精选的DSH插件 awesome 列表，便于快速发现
...
- **标签：** `awesome-list`, `comprehensive`, `ecosystem`
- **推荐理由：** DSH生态最全资源清单：插件、工具、技能
...
- **标签：** `awesome-list`, `tool-inventory`, `registry`
...
- **标签：** `marketplace`, `skills`, `bundle`
- **推荐理由：** 类CodeX插件市场，一站式分发平台。
...
- **标签：** `portal`, `static-site`, `web`
- **推荐理由：** 零依赖静态门户，便于浏览和安装插件。
...
- **标签：** `dsh-plugins`, `dsh-plugin-verify`, `dsh-repository`
- **推荐理由：** 提供DSH插件验证与仓库管理功能。
