...
- **标签：** `multi-agent`, `mcp`, `agent-skills`
- **推荐理由：** 自进化多智能体框架，支持记忆和任务规划。
...
- **标签：** `coding-agent`, `agent-runtime`, `vibe-coding`
- **推荐理由：** Rust高性能代理运行时，搭配精美桌面IDE
...
- **标签：** `coding-agent`, `cli`, `tui`, `rpc`, `mcp`, `go`
- **推荐理由：** 终端编码代理，支持RPC插件、子代理和MCP集成。
...
- **标签：** `cli`, `plugin-manager`, `typescript`, `hub`, `schemas`
- **推荐理由：** DSH插件开发和管理的CLI工具链，含schema解析器。
...
- **标签：** `code`, `simplification`, `refactor`
- **推荐理由：** 改编自DSH官方简化技能，代码重构优化助手
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，运行时注册支持GDScript
...
- **标签：** `agent-tools`, `plugin-management`, `cordis`
- **推荐理由：** DSH Agent插件预检工具，提升库利用率。
...
- **推荐理由：** 通过OAuth将ChatGPT接入DSH，扩展AI模型选择。
...
- **推荐理由：** DeepSeek Harness桌面客户端，体验更完善。
...
- **推荐理由：** 开放侧边栏框架，支持第三方扩展及内置文件/终端/Git面板。
...
- **标签：** `taskboard`, `ui`, `codex-plugin`
- **推荐理由：** 现代化可嵌入任务面板，适配Codex与DSH界面。
...
- **标签：** `floating-widget`, `ui-extension`, `dsh-plugin`
- **推荐理由：** 可爱鲸鱼浮动小组件，实时显示账户余额
...
- **推荐理由：** 一键安装包，内置SSH、插件和远程访问，桌面体验完整。
...
- **标签：** `desktop`, `local-first`, `mcp`, `react`, `typescript`
- **推荐理由：** 本地优先的AI桌面工作台，支持项目管理和MCP集成。
...
- **标签：** `canvas`, `workspace`, `visualization`, `conversations`
- **推荐理由：** 画布式非线性对话工作台，支持会话分支和探索。
...
- **推荐理由：** 本地优先的多模型AI代理桌面应用，支持自我进化技能
...
- **标签：** `ios`, `mobile`, `kmp`
- **推荐理由：** 原生iOS客户端，将DSH工作区和代理轨迹带到iPhone
...
- **推荐理由：** 受CodePilot启发的跨平台桌面客户端和插件套件
...
- **标签：** `agent-memory`, `rag`, `embeddings`, `milvus`
- **推荐理由：** 基于Milvus的统一持久化记忆层，支持所有AI代理
...
- **标签：** `skill-corpus`, `rag`, `embeddings`, `vector-search`, `reranking`
- **推荐理由：** 将分散的技能文件转化为可检索的向量知识库，支持语义搜索与评估。
...
- **推荐理由：** 私有化便签，支持Skill调用与一键导出，锤科美学
...
- **标签：** `memory`, `handoff`, `agent`
- **推荐理由：** 跨平台Agent交接技能，支持本地仓库上下文记忆延续
...
- **标签：** `agent-workspaces`, `mcp`, `coding-agent`
- **推荐理由：** 全平台统一Agent工作空间，跨设备访问。
...
- **标签：** `agent-skill`, `video-generation`, `ffmpeg`
- **推荐理由：** 端到端自动化Vox风格解说视频生成技能
...
- **推荐理由：** 原生SQLite任务看板，支持代理认领和审核工作流
...
- **推荐理由：** 全栈自动数学建模竞赛Agent，支持国赛美赛，GitOps流水线
...
- **推荐理由：** 88个开源Agent技能，覆盖研究、社交与营销工作流
...
- **标签：** `agent-skills`, `mcp`, `web-search`, `cordis`
- **推荐理由：** 网页搜索、内容提取与研究技能，面向AI代理
...
- **标签：** `deepseek`, `dsh-plugin`, `skills`
- **推荐理由：** 统一管理本地Agent Skills加载与安全控制
...
- **标签：** `ai-docs`, `docx`, `gongwen`, `wps-office`
- **推荐理由：** 党政机关公文全流程处理，格式校验与模板生成
...
- **标签：** `legal-tech`, `diagram`, `svg`, `timeline`
- **推荐理由：** 法律可视化：时间轴、诉讼图、庭审对抗图生成
...
- **标签：** `agent-platform`, `workbench`, `skills`, `typescript`
...
- **标签：** `bazi`, `fengshui`, `tcm`, `agent-skill`
- **推荐理由：** 独特玄学中医技能包，含本地确定性引擎
...
- **标签：** `engineering-discipline`, `quality-gate`, `skill`
- **推荐理由：** 工程交付质检技能，需求审查与测试验证
...
- **⭐ 星标：** 39
- **语言：** Python
- **描述：** 数学建模资源，包含辅��建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math-modeling`, `skill`, `research`
- **推荐理由：** 全面的中文数学建模技能框架与资源
...
- **标签：** `planning`, `workflow`, `agent-skill`
- **推荐理由：** 先讨论后执行的AI协作任务变更机制
...
- **标签：** `math-modeling`, `agent`, `cmcm`
- **推荐理由：** 33篇模型库+角色预设+论文模板，数学建模竞赛一站式方案
...
- **标签：** `skill`, `official`, `tencent`
- **推荐理由：** 腾讯官方技能库，提供API文档和插件示例参考
...
- **标签：** `agent`, `design`, `ui`, `vibedesign`, `mcp`
...
- **标签：** `ui-components`, `charts`, `forms`, `visual`, `dsh-ui`
- **推荐理由：** 在对话中内联渲染交互式UI组件：图表、表单、问答等。
...
- **推荐理由：** AI导演插件，支持视频/图像/音频生成，内置分镜脚本功能
...
- **标签：** `video`, `shotcraft`, `cinematic`
- **推荐理由：** 电影感产品视频技能，152张镜头配方卡+Remotion模板
...
- **标签：** `web-search`, `free`, `no-signup`, `search-bridge`
- **推荐理由：** 免费无需注册的网页搜索插件，所有模型均可使用。
...
- **标签：** `browser-automation`, `anti-detect`, `fingerprint`, `playwright`, `mcp`
- **推荐理由：** 引擎级指纹伪装，持久化浏览器身份，支持Playwright自动化。
...
- **推荐理由：** 实时iOS模拟器集成，22个代理工具用于移动自动化
...
- **标签：** `multi-model`, `account-management`, `routing`
- **推荐理由：** 统一管理多模型账户，支持智能路由与自动故障转移。
...
- **标签：** `context-management`, `dsh-external`, `plugin`
- **推荐理由：** 最佳上下文洞察与管理插件，含仪表板侧栏
...
- **标签：** `ecosystem-radar`, `plugin-registry`, `continuous-validation`
- **推荐理由：** 开源生态雷达，持续发现与k8s验证
...
- **标签：** `security`, `cybersecurity`, `vulnerability`, `redteam`, `code-audit`
- **推荐理由：** AI代码安全扫描工具，检测幻觉包、漏洞和安全缺陷。
...
- **推荐理由：** 添加Command Code模型访问，支持计划感知选择和实时目录
...
- **推荐理由：** 打通Pi与DSH生态，兼容��运行Pi扩展为原生插件
...
- **标签：** `management`, `container`, `ui`
- **推荐理由：** 本地DSH管理工具，隔离容器运行+内嵌WebView界面
...
- **标签：** `mcp`, `setup`, `multi-agent`
- **推荐理由：** 一键配置7+平台MCP和Skills，支持多Agent工具本地部署
...
- **标签：** `migration`, `wizard`, `utility`
- **推荐理由：** 四源迁移向导，支持Claude/Codex/OpenCode会话迁移
...
- **标签：** `launcher`, `runtime`, `plugin`
- **推荐理由：** 统一运行时/提供商/插件编排，构建可扩展DSH核心能力
...
- **标签：** `doctor`, `plugin-management`, `deepseek-harness`
- **推荐理由：** 检测并解决插件版本冲突问题
...
- **推荐理由：**  Animated宠物图库，为你的代码智能体添加个性化装饰。
...
- **标签：** `theme`, `glassmorphism`, `ui-customization`, `css`
- **推荐理由：** 高度可定制的磨砂玻璃主题，可调模糊度和不透明度。
...
- **标签：** `theme`, `animated-background`, `glassmorphism`
- **推荐理由：** 精美的实时海洋动态主题，带有玻璃拟态效果
...
- **标签：** `documentation`, `code-analysis`, `deepwiki`
- **推荐理由：** 将代码库转化为精准技术文档与AI可理解上下文。
...
- **标签：** `awesome-list`, `curated`, `bilingual`, `plugin-index`
- **推荐理由：** 中英双语DSH插件大全，收录经过验证的扩展和工具资源。
...
- **推荐理由：** 一站式科研插件：LaTeX写作、arXiv文献管理、GPU任务编排和实验追踪。
...
- **标签：** `knowledge-base`, `resource-catalog`, `ai-encyclopedia`, `learning`
- **推荐理由：** AI全维度资源百科，覆盖大模型、Agent、RAG和学习���线。
...
- **推荐理由：** 全面的DSH插件、MCP服务器、补丁和指南精选列表
...
- **标签：** `mcp`, `finance`, `agent`
- **推荐理由：** 基于Tushare的本地财经数据库，支持多Agent金融分析
...
- **推荐理由：** 交互式K线分析插件，多市场行情+技术指标+侧栏展示
...
- **标签：** `deepseek-harness`, `awesome-list`, `mcp`
- **推荐理由：** Agent-first DSH手册，173个源码支持的全面参考资料
...
- **推荐理由：** 1958个仓库插件总目录，支持中英文搜索与分类筛选
...
- **标签：** `directory`, `awesome-list`, `automation`
- **推荐理由：** GitHub Actions自动爬取的双语文档目录。
...
- **标签：** `search`, `discoverability`, `agent-tools`
- **推荐理由：** 从DSH Agent内直接搜索插件，提升可发现性。
...
- **标签：** `dsh-plugins`, `dsh-bundle`, `plugin-verify`
- **推荐理由：** 支持插件打包与验证的DSH插件清单。
...
- **标签：** `registry`, `curated`, `coding-agents`, `ui`, `skills`
- **推荐理由：** 覆盖模型、工具、UI的全面插件注册表。
