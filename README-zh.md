...
- **标签：** `coding-agent`, `dsh-plugin`, `r1`
- **推荐理由：** DeepSeek原生终端编程智能体，前缀缓存稳定
...
- **标签：** `preset`, `standard`, `dsh-plugin`
- **推荐理由：** 两阶段标准预设，从最小引导到完整工具覆盖。
...
- **标签：** `runtime`, `ide`, `agent`
- **推荐理由：** Rust高性能Agent运行时+精美桌面IDE一体化方案
...
- **标签：** `plugin-dev`, `agent-skill`, `dsh-plugin`
- **推荐理由：** 用于开发打包DSH插件的Agent技能
...
- **标签：** `plugin-creator`, `scaffold`, `validator`
...
- **标签：** `plugin-development`, `audit`, `harness-tools`
- **推荐理由：** 用于DSH插件开发与审计的便携式技能
...
- **标签：** `web-ui`, `plugin-aggregator`, `dsh-web`
- **推荐理由：** Web端插件聚合生态包，创意工坊分发平台。
...
- **推荐理由：** 开放侧边栏底座，内置文件/终端/对话/Git/子代理页面
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** Claude Code风TUI，鲸鱼顶栏/流式思考/Esc回滚
...
- **标签：** `floating-widget`, `developer-tools`, `dsh-plugin`
- **推荐理由：** 可爱浮动鲸鱼组件监控账户余额
...
- **推荐理由：** 在DSH对话中运行iOS模拟器或直接控制iPhone
...
- **推荐理由：** 类CodePilot跨平台桌面客户端，插件生态完整
...
- **标签：** `desktop-app`, `windows`, `ssh`
- **推荐理由：** 功能丰富的Windows桌面客户端，支持SSH和远程
...
- **标签：** `ui`, `info-bar`, `pricing`, `usage-tracking`
- **推荐理由：** 持久化底部信息栏，实时显示模型和余额信息
...
- **标签：** `canvas`, `cordis`, `creative`
- **推荐理由：** 无限画布结合创意技能，丰富DSH工作流
...
- **推荐理由：** Tauri桌面应用整合数据库SSH Docker与AI
...
- **标签：** `dsh-plugin`, `ui`, `customization`, `theme`
- **推荐理由：** 支持上传图片或切换预设氛围自定义DSH界面背景。
...
- **标签：** `dsh-plugin`, `dsh-plugin-market`, `ui`
- **推荐理由：** DSH Web上下文计量面板的一键折叠按钮插件
...
- **标签：** `memory`, `rag`, `agent-memory`, `long-term-memory`
- **推荐理由：** 本地优先的可移植记忆层，Markdown原生自演进存储。
...
- **推荐理由：** 持久统一Agent记忆层，Markdown+Milvus双引擎支撑
...
- **标签：** `skill-md`, `embeddings`, `semantic-search`
- **推荐理由：** 将技能文档转为可检索的向量知识库
...
- **标签：** `agent-orchestration`, `multi-device`, `no-code`
- **推荐理由：** 跨设备统一Agent工作空间，支持任务看板与自建部署。
...
- **推荐理由：** 自我进化的Agent OS，内置LLM评估与循环工程。
...
- **推荐理由：** 企业级多引擎Agent工作bench，支持Codex/DSH/OpenClaw。
...
- **⭐ 星标：** 3,775
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外挂视觉插件，为 DeepSeek、GLM 等纯文本��型外挂视觉能力，粘贴图片即得结构化 JSON 证据（OCR、版面、语义）。
- **标签：** `vision`, `ocr`, `multimodal`
- **推荐理由：** 首个视觉插件，为纯文本Agent提供OCR和图像理解。
...
- **推荐理由：** 智能技能路由编排，降Token 29.6%提reward +21.12pp
...
- **标签：** `video`, `generation`, `skill`
- **推荐理由：** 端到端Vox风格纸片拼贴视频生成Agent技能
...
- **标签：** `ai-workflow`, `browser-extension`, `dsh-plugin`
- **推荐理由：** 将AI对话转化为可复用工作流
...
- **标签：** `agent-skills`, `research`, `deepseek-harness`
- **推荐理由：** 88个开源技能，覆盖研究、营销与社交工作流
...
- **标签：** `agent-skills`, `credentials`, `slots`
- **推荐理由：** 管理已安装插件、插槽和凭证的一体化技能
...
- **标签：** `skills`, `task-board`, `agent`
- **推荐理由：** 在DSH中可视化任务板，清晰掌控技能执行进度
...
- **标签：** `fengshui`, `iching`, `bazi`, `tcm`
- **推荐理由：** 八字风水易经理医全融合的传统智慧AI技能
...
- **⭐ 星标：** 25
- **语言：** JavaScript
- **描述：** DSH Skills Manager — 在 DeepSeek Harness ���统一加载并安全管理本机 Agent Skills · Load and safely manage local Agent Skills in DSH
- **标签：** `dsh-plugin`, `agent-skills`
- **推荐理由：** 统一管理本机Agent技能的安全加载
...
- **标签：** `code-simplification`, `skills`
- **推荐理由：** 源自DSH的代码简化Agent技能
...
- **标签：** `skill-generator`, `agent-learning`, `dsh`
...
- **标签：** `dsh-plugin`, `skill`, `oauth`
- **推荐理由：** DSH技能封装的合影墙，OAuth登录即开即用
...
- **推荐理由：** 一键配置多AI工具的MCP和Skills，跨平台同步
...
- **标签：** `agent-tools`, `plugin-discovery`, `capability-index`
- **推荐理由：** 任务型请求时自动预检并注入Top-K适用插件。
...
- **标签：** `chatgpt`, `oauth`, `mcp`, `integration`
...
- **标签：** `ai-design`, `ppt`, `dsh-plugin`
- **推荐理由：** AI生成可编辑设计系统与PPT工具
...
- **标签：** `image-generation`, `stable-diffusion`, `genai`
...
- **标签：** `canvas`, `workspace`, `non-linear`
- **推荐理由：** 基于画布的非线性对话工作区，支持分支探索
...
- **标签：** `browser-automation`, `playwright`, `deepseek-harness`
...
- **推荐理由：** 钉钉IM频道，将聊天转化为具备工具能力的agent
...
- **标签：** `vfs`, `virtual-filesystem`, `tools`
- **推荐理由：** 统一虚拟文件系统，为AI Agent抽象跨语言存储。
...
- **推荐理由：** 一键切换多模型CLI，Claude/Codex/Grok/DeepSeek全覆盖
...
- **推荐理由：** 安全P2P远程访问你的DeepSeek Harness。
...
- **标签：** `dsh-plugin`, `capability-menu`, `mcp`
- **推荐理由：** 统一管理MCP工具与技能暴露级别的控制台
...
- **标签：** `migration`, `mcp`, `configuration`
- **推荐理由：** 将Claude Code配置一键迁移至DSH的工具
...
- **标签：** `monitoring`, `cost`, `visualization`
- **推荐理由：** 实时Token用量与成本监控仪表板
...
- **标签：** `deepseek-harness`, `dsh`, `electron`, `integration`
- **推荐理由：** Electron壳集成DSH Web服务与多插件，开箱即用。
...
- **标签：** `ai`, `llm`, `rag`, `vibecoding`
- **推荐理由：** 全面的AI资源库，涵盖大模型、提示词和vibe-coding教程。
...
- **标签：** `content-discovery`, `research`, `privacy-first`
- **推荐理由：** 本地私有AI内容发现Agent，自动追踪多平台热点内容
...
- **标签：** `research`, `china`, `skill`
- **推荐理由：** 自动搜索8大中文平台近30天内容生成有据研究报告
...
- **推荐理由：** 在Agent中直接搜索20个中国平台6.5万篇API文档
...
- **标签：** `industry-research`, `report`, `dsh`
- **推荐理由：** 行业研究方法、产业链映射及政策新闻追踪
...
- **标签：** `directory`, `awesome-list`, `typescript`
- **推荐理由：** 公开插件目录含安装命令，便于发现
...
- **标签：** `awesome-list`, `dsh`, `deepseek-harness`
- **推荐理由：** 涵盖插件、工具、技能的全面DSH资源目录。
...
- **标签：** `awesome-list`, `plugin-guide`, `dsh-plugin`
- **推荐理由：** 按场景选插件，含安装权限与兼容性说明。
...
- **标签：** `plugin-portal`, `awesome-list`, `web`
- **推荐理由：** 静态插件门户，浏览安装227+精选DSH插件。
...
- **推荐理由：** 每日自动爬取更新的插件目录，GitHub Actions驱动
...
- **标签：** `discoverability`, `dsh-plugin`, `search`
- **推荐理由：** 在agent内搜索npm和精选列表的插件发现工具
...
- **标签：** `awesome-list`, `deepseek-harness`, `auto-updating`
- **推荐理由：** 自动更新的 bilingual 插件列表，支持Vercel部署
...
- **标签：** `deepseek-harness-plugins`, `curated-list`, `ui`
- **推荐理由：** 覆盖模型、工具、沙箱、UI的全方位插件精选集
...
- **标签：** `industry`, `awesome-list`, `plugin-directory`
- **推荐理由：** 计算机、金融、法律、HR、制造等行业精选插件
...
- **标签：** `toy`, `web`, `culture`
- **推荐理由：** 零依赖单文件竹知了Web模拟，真实录音采样移动端优先
...
- **标签：** `encyclopedia`, `resource`, `ai-nav`
- **推荐理由：** 一站式AI资源百科，覆盖Agent、RAG、MLOps
...
- **推荐理由：** 有趣的番剧搜索插件，支持Bangumi评分和磁力链接。
