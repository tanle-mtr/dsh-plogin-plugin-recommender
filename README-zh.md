...
- **标签：** `vision`, `ocr`, `harness-tool`
- **推荐理由：** 首个面向纯文本编码Agent的视觉插件，图片转结构化JSON。
...
- **标签：** `preset`, `bootstrap`, `harness`
- **推荐理由：** 两阶段DSH预设，轻量启动后加载标准工具集。
...
- **推荐理由：** 全球首个AI Agent虚拟终端，支持VFS和虚拟Shell。
...
- **推荐理由：** 高性能Rust代理运行时，配合精美桌面IDE，支持协同编码。
...
- **推荐理由：** 纯文本模型的视觉工具箱，支持OCR、多图问答和UI还原。
...
- **标签：** `coding-agent`, `rpc`, `sub-agents`, `mcp`, `go`
- **推荐理由：** 编码Agent RPC插件，支持子Agent、哈希行编辑和MCP集成
...
- **标签：** `awesome-list`, `handbook`, `deepseek-harness`, `mcp`
- **推荐理由：** 全面的Agent优先手册，涵盖运行时插件MCP等173个主题
...
- **标签：** `compatibility-layer`, `migration`, `deepseek-harness`, `dsh-plugin`
...
- **标签：** `dsh-plugins`, `skill-management`
- **推荐理由：** 统一加载和管理本地Agent Skills的插件
...
- **标签：** `bioinformatics`, `desktop-app`, `llm-agent`, `tauri`
- **推荐理由：** 一键安装的生物信息学 Agent 桌面，适合临床与湿实验科研人员。
...
- **推荐理由：** 官方文档与插件开发知识库，以按需 Skill 形式提供。
...
- **标签：** `plugin-creation`, `validation`, `codex`
- **推荐理由：** 可复用的 DSH/Cordis 插件开发与验证 Skill，提升开发效率。
...
- **标签：** `agent-tools`, `pre-flight`
- **推荐理由：** Agent起飞前预检插件库，提升利用率可预期
...
- **推荐理由：** 通过OAuth将ChatGPT模型集成到DSH，支持编码辅助。
...
- **推荐理由：** 可扩展侧边栏，内置文件/终端/Git/对话等多功能页面。
...
- **标签：** `taskboard`, `ui-extension`, `productivity`
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex和DSH使用。
...
- **标签：** `floating-widget`, `plugin`, `ui`
- **推荐理由：** 可爱鲸鱼小部件，追踪账户余额，支持拖拽吸附和动画。
...
- **标签：** `context-management`, `dashboard`, `ui`
- **推荐理由：** 上下文洞察仪表盘，支持浏览器和侧边栏管理。
...
- **标签：** `desktop`, `gui`, `electron`, `windows`, `ssh-client`
- **推荐理由：** 零配置Windows桌面GUI，集成Codex、插件、SSH和移动端远程访问
...
- **标签：** `desktop`, `local-first`, `office-automation`, `react`
- **推荐理由：** 本地优先的AI桌面工作台，支持DSH会话、项目和Office文档创作
...
- **标签：** `canvas`, `nonlinear`, `session-explorer`, `ui`
- **推荐理由：** 基于画布的非线性对话工作台，支持会话浏览和��支导航
...
- **推荐理由：** 本地优先多模型桌面客户端，隐私友好的Cowork替代品。
...
- **标签：** `office`, `productivity`, `integration`
- **推荐理由：** 将表格文档幻灯片等办公能力引入Harness，实用高效。
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核工作流。
...
- **推荐理由：** KMP原生移动客户端，随时随地接入Harness工作区。
...
- **推荐理由：** 类CodePilot桌面客户端，跨平台插件套件丰富。
...
- **推荐理由：** 锤子便签复刻，支持Docker私有化部署和skill调用
...
- **标签：** `plugin-market`, `webui`, `community`
...
- **推荐理由：** 底部状态栏实时显示 Provider、余额与峰谷定价信息。
...
- **标签：** `desktop`, `docker`, `ssh`, `devops`, `deepseek-harness`
- **推荐理由：** 集数据库、SSH、SFTP、Docker和AI于一体的桌面工作台
...
- **标签：** `desktop`, `electron`, `deepseek-harness`, `modlens`
- **推荐理由：** 基于Electron的DSH桌面版，集成侧边栏和AI工具
...
- **标签：** `web-ui`, `personalization`
- **推荐理由：** 支持图片上传与氛围预设的Web背景定制
...
- **标签：** `memory`, `rag`, `markdown-native`
- **推荐理由：** 本地优先的AI agent便携记忆层
...
- **标签：** `agent-memory`, `rag`, `milvus`, `semantic-search`
- **推荐理由：** 基于Markdown和Milvus的持久记忆层，支持语义搜索。
...
- **标签：** `memory`, `skill-corpus`, `semantic-search`, `RAG`
- **推荐理由：** 将SKILL.md文件转为可检索的Agent技能语料库，支持语义搜索和评估
...
- **标签：** `knowledge-base`, `encyclopedia`, `AI-resources`, `learning-paths`
- **推荐理由：** 全维度AI资源百科：大模型、Agent、RAG、多模态及学习路线
...
- **推荐理由：** 跨平台Agent交接，支持仓库级连续记忆与会话恢复。
...
- **标签：** `workspace`, `multi-device`
- **推荐理由：** 跨设备统一的agent工作区管理
...
- **标签：** `agent-os`, `mcp`, `self-evolving`
- **推荐理由：** 自进化agent操作系统，带MCP服务
...
- **标签：** `agent-skills`, `ai-agent`, `inference-time-control`
- **推荐理由：** 模型无关推理时控制，支持深度推理与长程任务。
...
- **标签：** `ai-video`, `video-generation`, `agent-skill`
- **推荐理由：** 一句话生成Vox风格解说视频，全流程自动化。
...
- **推荐理由：** 在DSH中实现多代理团队协作，提升复杂任务处理能力。
...
- **标签：** `novel-writing`, `creative-writing`, `rag`
- **推荐理由：** 结构化小说创作流程，支持世界观构建和审稿修稿。
...
- **标签：** `writing`, `novel`, `creative-workflow`, `fiction`, `screenwriting`
- **推荐理由：** 网文创作工作流插件，内置小说、短剧、游戏、视频解说四个工作台
...
- **标签：** `agent-observability`, `computer-vision`, `multimodal`
- **推荐理由：** 为Agent添加视听屏感知能力，生成可验证证据。
...
- **标签：** `agent-preset`, `epistemic-loop`, `scientific`
- **推荐理由：** 将认知循环框架引入Harness，适合科研探索场景。
...
- **标签：** `agent-skills`, `deepseek-harness`, `math-modeling`
- **推荐理由：** 全栈自动数学建模竞赛技能，GitOps流水线
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skill-management`, `audit`
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `agent-review`, `prompt-engineering`
- **推荐理由：** 可组合可审查的Agent自进化工作模式
...
- **标签：** `agent-skills`, `deepseek-harness`, `claude-code`, `codex`
- **推荐理由：** 将经验转化为多Agent可重复执行技能
...
- **标签：** `agent-skills`, `multimodal`, `vision`, `deepseek-harness`
...
- **标签：** `agent-learning`, `skill-generator`, `automatic`
- **推荐理由：** 将成功会话自动转为可复用技能
...
- **推荐理由：** 从superpowers适配的TDD调试和规划技能
...
- **标签：** `skills`, `dsh-better-sidebar`
- **推荐理由：** 内置25个工程技能面板，开箱即用
...
- **⭐ 星标：** 47
- **语言：** TypeScript
- **描述：** A plugin-based Web work platform built on DeepSeek Harness, with global Skill management and native workspace integration. 基于 DeepSeek Harness 的插件化 Web ��作平台。
- **标签：** `workbench`, `agent-platform`, `typescript`
- **推荐理由：** 基于DSH的插件化Web工作台与技能管理
...
- **标签：** `bazi`, `fengshui`, `iching`, `tcg`
- **推荐理由：** 整合八字/风水/周易等传统智慧的本地确定性 Agent 技能包。
...
- **推荐理由：** 借鉴 DSH 工程模式的规范驱动开发 Agent Skill。
...
- **推荐理由：** 跨会话持久化记忆的 Agent 团队，支持角色分工与技能继承。
...
- **推荐理由：** 11 个测试技能框架，让 AI 像资深测试工程师一样工作。
...
- **推荐理由：** 议行分离机制，先讨论决策再执行的 AI 协作 Skill。
...
- **标签：** `skillhub`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 官方SkillHub API文档与示例，DSH技能开发入门首选。
...
- **推荐理由：** WorkBuddy风格技能选择器，快速插入斜杠命令。
...
- **推荐理由：** 多Agent数学建模求解器，含5角色预设和33篇模型库。
...
- **推荐理由：** 开发DSH插件的标准Agent Skill，兼容Claude Code和Codex
...
- **推荐理由：** 为DSH添加通知、追踪、快捷键和内置终端等���强功能
...
- **标签：** `mcp`, `financial`, `deepseek-harness`, `agent`
- **推荐理由：** 基于Tushare的本地金融数据库，支持MCP自动生成
...
- **标签：** `mcp`, `agent-config`, `multi-platform`
- **推荐理由：** 一键配置多平台AI工具的MCP、Skills和规则。
...
- **标签：** `agent-team`, `ui`, `vibecoding`, `design`
...
- **标签：** `design`, `prototyping`, `presentation`
- **推荐理由：** AI生成可编辑设计系统，内置模板市场和PPT功能。
...
- **标签：** `uigeneration`, `charts`, `interactive`, `inline-ui`
- **推荐理由：** GenUI在回复中渲染交互式内联UI：布局、图表、表单、问答
...
- **标签：** `ai-video`, `ai-image`, `dsh-plugin`
- **推荐理由：** AI视频/图像/音频创作，支持可配置模型工具，适合创意工作流。
...
- **推荐理由：** 电影感产品视频技能，含152张镜头配方和Remotion模板。
...
- **标签：** `browser`, `fingerprint-spoofing`, `playwright`, `automation`
- **推荐理由：** 持久浏览器身份+引擎级指纹伪造，无限本地配置文件
...
- **推荐理由：** 对话内集成iOS模拟器与真机自动化，22个Agent工具。
...
- **推荐理由：** 跨平台AI内容发现Agent，覆盖B站/YouTube/知乎等。
...
- **标签：** `account-management`, `multi-model`, `utility`
- **推荐理由：** 多模型账户统一管理，支持智能路由和自动故障转移。
...
- **推荐理由：** 免费免注册免API的Web搜索桥接插件，兼容所有缺原生搜索的模型
...
- **标签：** `security`, `auditing`, `vulnerability`, `hallucination-detection`
- **推荐理由：** AI安全审计平台，检测幻觉包、缺失防护和安全漏洞
...
- **推荐理由：** 开源CLI和Schema解析器，管理DSH插件市场：安装、搜索、发现
...
- **推荐理由：** 多Provider成本追踪与预算管理，节省开支必备。
...
- **推荐理由：** 接入Command Code模型，支持实时目录与套餐选择。
...
- **标签：** `awesome-list`, `marketplace`, `plugin-directory`, `deepseek-harness`
- **推荐理由：** 超万插件目录，支持搜索排序与公开API
...
- **标签：** `deepseek`, `dsh-plugin`, `remote-access`, `p2p`
- **推荐理由：** 端到端加密多端远程访问DSH的方案
...
- **标签：** `awesome-list`, `catalog`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 中英文插件总目录，支持搜索分类与STAR排序
...
- **标签：** `context`, `audit`, `ui`
- **推荐理由：** 上下文审计工具，统计 Token 成本并检测重复与冲突指令。
...
- **标签：** `dsh-box`, `container`, `dsh-plugin`
- **推荐理由：** 容器化运行多版本DSH，内嵌WebView，一键导入插件。
...
- **标签：** `launcher`, `cordis`, `orchestration`
- **推荐理由：** 统一编排DSH核心能力，支持Runtime/Plugin/MCP扩展。
...
- **标签：** `migration`, `session-import`, `multi-platform`
- **推荐理由：** 多平台会话/技能/指令迁移向导，支持Claude/Codex等。
...
- **推荐理由：** 实时监控Token用量、成本和API余额的Web插件
...
- **推荐理由：** 接入免费小米MiMo TTS API，支持自定义声音朗读
...
- **标签：** `plugin-diagnostic`, `conflict-resolution`, `dsh-plugin`
...
- **标签：** `theme`, `glass`, `translucent`, `customization`
- **推荐理由：** DSH网页端磨砂玻璃主题，可调模糊度、磨砂度和自定义壁纸
...
- **标签：** `animated-background`, `glassmorphism`, `theme`
- **推荐理由：** 动态海洋主题，可调波浪与玻璃态效果，视觉极佳。
...
- **标签：** `documentation`, `wiki`, `research`
- **推荐理由：** 将代码快速转化为精准技术文档和AI可读上下文。
...
- **推荐理由：** 一站式科研工作台：LaTeX写作、arXiv管理、实验追踪、GPU服务器SSH
...
- **推荐理由：** Harness终极入门指南，含资源推荐与精选插件导航。
...
- **标签：** `academic-search`, `mcp`, `citation-management`, `deepseek-harness`
- **推荐理由：** 跨库学术搜索引文图谱与核验工具
...
- **标签：** `agent-skills`, `deepseek-harness`, `research-automation`, `market-research`
- **推荐理由：** 88个开源Agent技能，覆盖研究社交营销
...
- **标签：** `kline`, `finance`, `dsh-better-sidebar`
- **推荐理由：** 交互式K线分析，支持技术指标、支撑压力位及市场新闻。
...
- **推荐理由：** 生态雷达，通过k8s发现���验证2万+插件，自动筛选。
...
- **推荐理由：** 中英文双语DSH插件/扩展/工具精选列表，含验证参考
...
- **推荐理由：** 收录 280+ 插件的精选目录，按 14 类整理方便查找。
...
- **标签：** `catalog`, `community`, `dsh-plugins`
- **推荐理由：** 非官方社区DSH插件目录，便于发现第三方工具。
...
- **推荐理由：** DSH插件、技能、MCP工具资源精选合集，一网打尽。
...
- **标签：** `ai-agents`, `plugin-registry`, `skills`, `deepseek-harness`
- **推荐理由：** 开放数据插件注册表，含垃圾过滤，便于发现插件
...
- **标签：** `awesome-list`, `typescript`, `plugin-directory`
- **推荐理由：** 公开GitHub插件目录，支持一键安装命令和生态发现
...
- **标签：** `deepseek-harness`, `dsh-plugins`, `awesome-list`
- **推荐理由：** 收录80+插件的目录，按功能分类方便查找。
...
- **标签：** `deepseek-harness`, `dsh-plugins`, `dsh-repository-plugin`
- **推荐理由：** DSH插件仓库，支持生态分析功能。
