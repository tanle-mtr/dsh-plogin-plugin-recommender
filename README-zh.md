...
- [实用工具与基础设施](utilities-infrastructure-zh) (35)
...
- **推荐理由：** DeepSeek原生终端编程代理，前缀缓存稳定高效
...
- **标签：** `model-switch`, `one-click`, `installer`
- **推荐理由：** 一键安装切换多编码Agent模型
...
- **推荐理由：** 高性能Rust Agent运行时，集成精美桌面应用
...
- **标签：** `agent-teams`, `coding`, `orchestration`
- **推荐理由：** 在DeepSeek Harness中实现多Agent协作能力
...
- **推荐理由：** 编码代理含RPC插件、子代理、hashline编辑和MCP支持
...
- **标签：** `ai-agent`, `oauth`, `llm-provider`, `deepseek-harness`
- **推荐理由：** 无需API密钥即可使用主流订阅账号作为LLM提供商
...
- **推荐理由：** Superpowers技能移植至Reasonix编码栈
...
- **标签：** `plugin-development`, `codex`, `cordis`
- **推荐理由：** 可复用插件构建验证技能，加速DSH插件开发
...
- **标签：** `electron`, `desktop-app`, `ssh-client`, `gui`
- **推荐理由：** 零配置Windows桌面客户端，支持插件、SSH和移动端访问
...
- **标签：** `electron`, `local-first`, `mcp`, `office-automation`
- **推荐理由：** 本地优先AI桌面工作台，支持项目、文件和Office办公
...
- **标签：** `canvas`, `visualization`, `non-linear`, `workspace`
- **推荐理由：** 基于画布的可视化对话探索器，支持分支工作空间
...
- **标签：** `office`, `deepseek-harness-plugin`, `dsh`
- **推荐理由：** 将完整办公套件带入DeepSeek Harness桌面端
...
- **标签：** `ios-app`, `kotlin-multiplatform`, `deepseek-harness`
- **推荐理由：** 原生iOS客户端，将DSH工作区与Agent带到手机
...
- **标签：** `ui`, `mcp`, `skills`, `webui`
- **推荐理由：** Web界面集成skill/MCP管理面板
...
- **标签：** `ui`, `marketplace`, `webui`, `plugin`
- **推荐理由：** DSH Web GUI社区插件市场一键安装
...
- **标签：** `ui`, `audit`, `skills`, `manager`
- **推荐理由：** 技能调用审计日志与应用内管理器
...
- **推荐理由：** Electron桌面控制台，多标签管理插件与MCP工具
...
- **标签：** `workbench`, `web`, `plugin-system`
...
- **推荐理由：** 集成数据库/SSH/SFTP/Docker/AI的一体化桌面工具。
...
- **推荐理由：** 基于Electron的DSH桌面应用，集成侧边栏和ModLens。
...
- **标签：** `tts`, `xiaomi`, `voice`, `audio`
- **推荐理由：** 接入免费小米MiMo语音合成API，支持自定义声音。
...
- **推荐理由：** 腾讯开源RAG平台，文档转可查询知识库与自维护Wiki
...
- **标签：** `agent-memory`, `rag`, `dsh-plugin`, `mcp`
- **推荐理由：** 可移植的本地优先内存层，跨AI代理共享。
...
- **标签：** `long-term-memory`, `rag`, `dsh-plugin`, `mcp`
- **推荐理由：** 自进化内存OS，混合检索节省35% token。
...
- **推荐理由：** 基于Milvus的统一持久化记忆层，支持所有AI Agent
...
- **标签：** `memory`, `mcp`, `agent`
- **推荐理由：** 共享持久化记忆中枢，让所有AI Agent保持记忆
...
- **标签：** `skill-md`, `embeddings`, `reranking`, `vector-search`
- **推荐理由：** 将SKILL.md转化为可检索的agent技能库，带评估工具
...
- **标签：** `agent-workspaces`, `no-code`, `dsh-plugin`, `mcp`
- **推荐理由：** 跨设备统一代理工作台，支持看板任务跟踪。
...
- **标签：** `cordis`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 任务感知的推理模式路由器与注入套件。
...
- **标签：** `agent-os`, `mcp`, `dsh-plugin`, `loop-engineering`
- **推荐理由：** 自进化代理OS，带评估循环与预算控制。
...
- **标签：** `agent-control-plane`, `loop-engineering`, `dsh-plugin`
- **推荐理由：** 长期代理控制面，支持工作流治理。
...
- **标签：** `cognitive-enhancement`, `reasoning`, `inference`
- **推荐理由：** 推理时控制套件，支持深度推理
...
- **标签：** `skill`, `video`, `automation`
- **推荐理由：** 一键生成Vox风格解说视频，全自动工作流
...
- **标签：** `writing`, `creative`, `workflow`
- **推荐理由：** 结构化小说创作流程，支持角色和世界管理
...
- **标签：** `multimodal`, `ocr`, `video-analysis`, `audio-understanding`
- **推荐理由：** 赋予AI视觉听觉能力，支持音视频分析和时间戳取证
...
- **⭐ 星标：** 371
- **语言：** Python
- **描述：** DeepSeek 写网文/小说的工作流插件：DeepSeek Harness 社区插件，内置小说、短剧、游��、视频解说四个工作台 | Community DeepSeek Harness plugin with novel, short-drama, game and video-recap workbenches.
- **标签：** `agent-skills`, `creative-writing`, `novel-writing`, `deepseek-harness`
- **推荐理由：** 网文/短剧/游戏创作全流程AI写作工作台
...
- **⭐ 星标：** 169
- **语言：** Python
- **描述：** 为 Claude Code 赋能多模态视觉能力，适配 纯文本 LLM 底座，用于截图 / UI / 图表分析；搭��� browser-harness 可做前端布局自动化检查。
- **标签：** `vision`, `multimodal`, `claude-code`, `ui-analysis`
- **推荐理由：** 为纯文本LLM赋能视觉能力，分析截图UI
...
- **标签：** `skills`, `automation`, `agent-learning`
...
- **⭐ 星标：** 56
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直��调用的 Skill，完全自包含，克隆即用。
- **标签：** `document`, `gongwen`, `workflow`
- **推荐理由：** 符合国标党政机关公文格式的全流程写作处理工具
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `agent-skill`
- **推荐理由：** 八字风水易经AI技能包，本地确定性引擎+可视化
...
- **标签：** `testing`, `qa`, `playwright`, `pytest`
- **推荐理由：** AI测试工程技能框架，11个技能+共享知识库
...
- **标签：** `skill-picker`, `composer`, `ui`
- **推荐理由：** Composer内技能选择器，WorkBuddy风格体验
...
- **标签：** `skill`, `code-simplification`, `dsh-plugin`
- **推荐理由：** 从DSH改编的代码简化技能，实用高效
...
- **标签：** `agent-config`, `mcp`, `skills`, `multi-agent`
- **推荐理由：** 一键配置多AI工具的MCP和Skills，省心省力
...
- **标签：** `skillhub`, `api`, `dsh-plugin`, `tencent`
- **推荐理由：** 腾讯官方SkillHub，含DSH插件集成
...
- **标签：** `video`, `skill`, `remotion`, `motion-design`
- **推荐理由：** 电影感视频制作技能，152张镜头配方卡
...
- **标签：** `math-modeling`, `skill`, `chinese`, `paper`
- **推荐理由：** 中文数学建模全套资源，含降AIGC查重
...
- **推荐理由：** 基于标准的DSH插件开发Agent Skill工具。
...
- **推荐理由：** 新增Command Code模型接入，智能套餐感知选择
...
- **标签：** `content-discovery`, `cross-platform`, `personal-ai`
- **推荐理由：** 跨平台自进化内容发现Agent
...
- **推荐理由：** 在DSH内嵌iOS模拟器，22个Agent自动化测试工具
...
- **标签：** `utility`, `discovery`, `validation`
- **推荐理由：** 持续发现和k8s验证的插件生态系统扫描器
...
- **标签：** `resource`, `curated-list`, `ecosystem`
- **推荐理由：** DSH生态精选插件、工具和资源目录
...
- **标签：** `marketplace`, `discovery`, `plugin`
- **推荐理由：** 每日更新的全面DSH插件市场
...
- **标签：** `backup`, `config`, `utility`, `sync`
- **推荐理由：** 配置全量备份恢复与WebDAV同步工具
...
- **标签：** `utility`, `tools`, `skills`, `management`
- **推荐理由：** 统一能力暴露管理与运行时动态控制
...
- **标签：** `utility`, `info-bar`, `usage-tracking`
- **推荐理由：** 实时显示模型余额与峰谷定价信息栏
...
- **标签：** `dsh-plugin`, `infrastructure`, `containers`
- **推荐理由：** 容器化多版本DSH管理，隔离部署方便
...
- **标签：** `migration`, `utility`, `dsh-plugin`
- **推荐理由：** 从Claude/Codex迁移会话和技能的向导工具
...
- **标签：** `launcher`, `orchestration`, `dsh-plugin`, `cordis`
...
- **⭐ 星标：** 7
- **语言：** TypeScript
- **描述：** 致力于解决各��件之间也与版本之间可能存在的冲突问题并提供解决方案
- **标签：** `diagnostic`, `conflict-resolution`, `health-check`
...
- **标签：** `ai-recommender`, `curated-list`
...
- **推荐理由：** 通过OAuth在DSH内调用ChatGPT订阅模型
...
- **标签：** `animated-background`, `chrome-extension`, `deepseek-theme`, `glassmorphism`
- **推荐理由：** 精美海洋动态主题，毛玻璃效果沉浸式体验
...
- **标签：** `latex`, `arxiv`, `research`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX编辑、arXiv管理、GPU远程调度
...
- **标签：** `awesome-list`, `mcp`, `coding-agent`, `deepseek-harness`
- **推荐理由：** 全面覆���插件、技能、MCP服务等的精选清单
...
- **推荐理由：** 基于Tushare本地金融库，多Agent分析盯盘
...
- **标签：** `api-docs`, `search`, `chinese-api`
- **推荐理由：** 一键检索20家中国开放平台6.5万+篇API文档
...
- **标签：** `evaluation`, `rag`, `benchmark`, `observability`
- **推荐理由：** 基于证据的提示词和RAG评估观测框架
...
- **标签：** `awesome-list`, `ecosystem`, `resources`
- **推荐理由：** DSH生态全面awesome列表，插件/工具/技能
...
- **标签：** `knowledge-base`, `mcp`, `rag`, `awesome-list`
- **推荐理由：** 全面AI资源百科，涵盖Agent、RAG、MCP和学习路线
...
- **标签：** `dsh-plugins`, `plugin-verify`, `dsh-repository`
- **推荐理由：** DSH插件仓库，含验证和统计分析功能。
...
- **标签：** `awesome-list`, `dsh-plugins`, `redirect`
- **推荐理由：** 已迁移至新仓库，建议访问最新地址。
...
- **标签：** `plugin-directory`, `deepseek-harness`, `registry`
- **推荐理由：** 涵盖模型、工具、沙箱和UI插件的综合目录。
