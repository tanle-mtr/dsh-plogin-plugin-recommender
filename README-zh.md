...
- [实用工具与基础设施](utilities-infrastructure-zh) (50)
...
- **标签：** `coding-agent`, `tui`, `r1`
- **推荐理由：** 终端原生AI编程智能体，R1缓存稳定。
...
- **标签：** `code-review`, `code-quality`, `architecture`
- **推荐理由：** 基于12本工程经典著作的AI代码审查工具
...
- **标签：** `coding-agent`, `architecture`, `baseline`
- **推荐理由：** 架构感知编程，基线优先保障长任务安全
...
- **推荐理由：** 对话内构建运行Android模拟器或USB真机调试
...
- **标签：** `plugin-dev`, `skills`, `tools`, `developer`
- **推荐理由：** 开发DSH插件的Agent技能，遵循官方标准规范
...
- **标签：** `plugin-dev`, `harness`
- **推荐理由：** 支持插件开发，含槽位和凭证管理。
...
- **标签：** `container`, `management`
- **推荐理由：** 容器隔离运行多版本DSH，内嵌WebView UI。
...
- **标签：** `plugin`, `codex`, `development`
- **推荐理由：** 用Codex技能构建和验证DSH插件，开发利器。
...
- **标签：** `plugin`, `development`, `audit`
- **推荐理由：** 便携式插件开发与审计技能，助力DSH开发。
...
- **推荐理由：** Claude Code风格TUI，实时状态与流式思考展示
...
- **推荐理由：** Windows桌面GUI客户端，支持SSH和远程访问
...
- **标签：** `electron`, `desktop`, `ui`, `plugin-management`
- **推荐理由：** Electron桌面管理控制台，多Tab管理Harness/Plugin/MCP/Skills
...
- **标签：** `ui`, `status`, `pricing`
- **推荐理由：** 底部常驻栏显示模型余额与实时资费信息。
...
- **标签：** `visualization`, `token-usage`, `dashboard`
- **推荐理由：** 实时 Token 用量与费用可视化看板
...
- **推荐理由：** Tauri 桌面应用整合数据库/SSH/Docker+AI
...
- **推荐理由：** Electron 桌面壳集成 DSH Web 工作台
...
- **标签：** `agent-memory`, `rag`, `context-database`
...
- **标签：** `memory`, `rag`, `local-first`, `portable`, `agent-memory`
- **推荐理由：** 可移植的本地优先AI智能体记忆层，支持RAG检索
...
- **标签：** `memory`, `self-evolving`, `token-savings`, `hybrid-retrieval`
- **推荐理由：** 自演化记忆OS，混合检索节省35%令牌，跨任务复用
...
- **标签：** `workspace`, `task-tracking`, `multi-device`, `no-code`
- **推荐理由：** 一站式智能体工作空间，跨设备任务追踪与看板管理
...
- **标签：** `agent-os`, `self-improving`, `loop-engineering`, `grading`
- **推荐理由：** 自改进智能体OS，集成评分命令和循环工程能力
...
- **标签：** `enterprise`, `multi-agent`, `workbench`, `collaboration`
- **推荐理由：** 企业级多引擎智能体工作台，统一跨引擎工作空间
...
- **标签：** `vision`, `ocr`, `image-to-text`, `multimodal`, `first`
- **推荐理由：** 首款视觉��件：OCR图像转JSON证据，赋能纯文本智能体
...
- **推荐理由：** 全栈自动数学建模竞赛Skill，支持GitOps流水线
...
- **标签：** `agent-skills`, `multi-agent`, `framework`
- **推荐理由：** 将经验转化为可重复执行的Agent框架
...
- **标签：** `task-management`, `agent`, `webui`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审查，无iframe
...
- **推荐理由：** 超级力量技能移植，支持TDD、调试、规划等开发工作流
...
- **推荐理由：** 超级力量技能适配Reasonix编码工具，适合AI辅助开发
...
- **标签：** `skills`, `pentest`, `reverse-engineering`, `security`
- **推荐理由：** 逆向工程与安全测试技能，含85个SKILL文件
...
- **标签：** `skills`, `mentor`, `education`, `coding`
- **推荐理由：** AI编程导师技能，自动扫描项目架构并指导手写核心逻辑
...
- **标签：** `skills`, `research`, `marketing`, `multi-agent`
- **推荐理由：** 88个跨平台Agent技能，覆盖研究、营销和业务工作流
...
- **标签：** `spec-driven`, `skills`, `development`, `workflow`
- **推荐理由：** 仓库级规范驱动开发技能，参考DSH工程模式
...
- **标签：** `skill`, `chinese-wisdom`
- **推荐理由：** 中华传统智慧AI技能：八字风水易经分析。
...
- **⭐ 星标：** 18
- **语言：** JavaScript
- **描述：** 拨开迷雾看见终点，剩下的交给��务栏。Part the fog, see the end — the task bar handles the rest. 🎮 mattpocock/skills 的 DSH 游戏任务系统：map 拨迷雾，任务栏推进一步。A game-like mission system for Matt Pocock skills in DeepSeek Harness. More by @FeatherHunter: 🎨 dsh-opencode-palette · ⚡ dsh-prompt
- **标签：** `skills`, `mission`, `gamification`
- **推荐理由：** 游戏化任务系统让工作流更有趣高效。
...
- **⭐ 星标：** 18
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生���持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `chinese`
- **推荐理由：** 符合国标的全流程中文公文处理工具，专业实用。
...
- **标签：** `agent-tools`, `checklist`, `capabilities`
- **推荐理由：** Agent起飞前插件预检，提升可预期性
...
- **标签：** `diagrams`, `architecture`, `visualization`, `mermaid-alternative`
- **推荐理由：** 从代码生成可验证的架构图和序列图，支持动效
...
- **标签：** `design`, `vector`, `ui`, `agent-teams`, `design-as-code`
- **推荐理由：** 开源AI原生矢量设计工具，支持并发智能体团队协作
...
- **标签：** `design`, `theme`, `vibe-coding`
- **推荐理由：** 6种风格避免AI同质化设计，提升网站审美。
...
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、抖音、小红书等
...
- **标签：** `social`, `photo`, `oauth`
- **推荐理由：** GitHub OAuth合影墙，适合社区内测留念活动。
...
- **标签：** `router`, `injector`, `infrastructure`, `preset`
- **推荐理由：** 运行时注入器和任务感知推理模式路由器预设工具包
...
- **标签：** `bootstrap`, `preset`, `standard-tools`, `infrastructure`
- **推荐理由：** 两阶段最小化对齐引导预设，含标准工具链支持
...
- **标签：** `vfs`, `virtual-filesystem`, `fuse`, `infrastructure`, `unified`
- **推荐理由：** AI智能体统一虚拟文件系统，跨语言和平台支持
...
- **标签：** `dsh-plugin`, `model-switch`, `multi-agent`
- **推荐理由：** 一键安装并切换多个AI编程工具，非常方便
...
- **标签：** `plugin-discovery`, `dsh-plugin`
- **推荐理由：** 在DSH中搜索安装验证GitHub插件
...
- **⭐ 星标：** 146
- **语言：** JavaScript
- **描述：** DSH插件市场 / DSH Plugin Marketplace: 在 DeepSeek Harness Web GUI 中一键浏览、安装与���新 GitHub topic:dsh-plugin 的全部插件 | browse, install & update all GitHub dsh-plugin plugins in the DSH Web GUI
...
- **推荐理由：** 崩溃恢复与配置回滚，安全快照，支持SAFE MODE
...
- **标签：** `awesome-list`, `marketplace`, `community`
- **推荐理由：** 社区自动同步的DSH插件聚合目录，每4小时更新
...
- **标签：** `awesome-list`, `official`, `plugins`
...
- **标签：** `ocr`, `image`, `vision`, `tools`
- **推荐理由：** 图像OCR识别技能，支持纯文本模型处理图片
...
- **标签：** `context`, `audit`, `token`
- **推荐理由：** 审计上下文 token 成本，检测重复冲突，实用性强。
...
- **标签：** `package`, `skills`, `database`
- **推荐理由：** 42个技能5个CLI的包管理器，一站式agent资产库。
...
- **标签：** `mcp`, `manager`, `gui`
- **推荐理由：** 在Web GUI中直接管理技能和MCP服务器，便捷。
...
- **标签：** `awesome-list`, `plugin-registry`, `skills`
- **推荐理由：** 防垃圾插件注册表，开放数据验证
...
- **标签：** `documentation`, `guide`, `navigation`
- **推荐理由：** 插件产品说明书，按场景导航
...
- **标签：** `repository`, `verification`, `plugins`
- **推荐理由：** 通用DSH插件仓库和验证平台
...
- **推荐理由：** GitHub Actions每日自动更新的插件目录
...
- **标签：** `awesome-list`, `2origin`, `plugin-registry`
- **推荐理由：** 插件雷达与2Origin实验结合的工具集
...
- **推荐理由：** 指向官方awesome-dsh-plugin仓库的重定向
...
- **标签：** `registry`, `curated-list`, `ui`
- **推荐理由：** 安装扩展模型、工具、沙箱与UI的插件集
...
- **标签：** `pixel-art`, `mascot`, `gallery`, `themes`, `skins`
- **推荐理由：** 公共动态像素宠物画廊，为智能体提供陪伴主题皮肤
...
- **标签：** `webui`, `themes`, `personalization`
- **推荐理由：** Web背景个性化，支持预设氛围切换
...
- **标签：** `ai-guide`, `prompt-library`, `tutorials`
- **推荐理由：** 全面AI资��指南，含教程与提示词大全。
...
- **标签：** `api`, `documentation`, `chinese-platforms`, `search`
- **推荐理由：** 搜索20个中国平台6.5万+API文档，零配置多工具支持
...
- **标签：** `tutorial`, `harness-engineering`, `beginner`, `learning`
- **推荐理由：** 从零开始的DSH插件工程入门教程，适合新手
...
- **标签：** `awesome-list`, `curated`, `directory`, `ecosystem`
- **推荐理由：** DSH生态精选插件目录，一站式发现优质插件
...
- **标签：** `web`, `aggregation`, `ecosystem`, `creative-workshop`
- **推荐理由：** DSH Web插件聚合生态包，创意工坊分发平台
...
- **标签：** `toy`, `web`, `zero-dependency`
- **推荐理由：** 零依赖竹知了玩具网页模拟版，有趣的小工具
...
- **标签：** `documentation`, `best-practices`, `dsh-plugin`
- **推荐理由：** DSH 插件开发踩坑实战记录与最佳实践
