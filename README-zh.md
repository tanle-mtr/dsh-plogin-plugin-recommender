...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 233,174 | Core DSH framework; the foundation of everything i |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,602 | Open-source design alternative with local-first de |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 73,056 | Multi-agent swarm orchestration with autonomous wo |
...
- **标签：** `agent-orchestration`, `workspace`, `mcp`, `multi-device`
- **推荐理由：** 跨设备统一的智能体工作区与看板管理
...
- **标签：** `routing`, `agent-runtime`, `reasoning`
- **推荐理由：** 任务感知推理路由器与运行时注入套件
...
- **标签：** `agent-os`, `evaluation`, `mcp`, `evolution`
- **推荐理由：** 自演进智能体OS，分阶段评估与预算演化循环
...
- **标签：** `coding-agent`, `cli`, `mcp`, `tui`
- **推荐理由：** 强大的CLI编程代理，支持RPC、子代理和MCP。
...
- **推荐理由：** 统一管理本地Agent Skills的安全加载工具
...
- **推荐理由：** 增强DSH：任务通知、LLM追踪、快捷键与内置终端
...
- **标签：** `sidebar`, `ui`, `deepseek-harness`, `extension`
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/Git/对话等页面。
...
- **标签：** `taskboard`, `ui`, `skills`, `codex`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex与DSH界面集成。
...
- **推荐理由：** TUI增强插件，支持鲸鱼顶栏/流式思考/实时状态显示。
...
- **标签：** `ios`, `android`, `mobile`
- **推荐理由：** 原生移动客户端，将DSH工作区、会话和Agent轨迹带到手机
...
- **推荐理由：** 跨平台桌面客户端，类CodePilot体验的DSH插件套件
...
- **推荐理由：** 开源锤子便签复刻，Docker私有化部署支持DSH插件
...
- **推荐理由：** 替换composer统计行为，显示模型余额与资费信息
...
- **推荐理由：** 一体化DevOps桌面：数据库、SSH、SFTP、Docker、AI沙箱
...
- **推荐理由：** Electron桌面封装，集成侧边栏、ModLens等
...
- **标签：** `memory`, `library`, `stateful-agents`
- **推荐理由：** 轻量级状态记忆库，助力智能体开发
...
- **推荐理由：** Agent记忆管理套件，支持长期记忆存储与检索优化。
...
- **标签：** `skill-corpus`, `vector-search`, `reranking`
- **推荐理由：** 将零散技能文件转为可检索的知识库，支持评估工具。
...
- **推荐理由：** 长周期Agent控制面，支持跨Codex/Claude Code的持久化工作流。
...
- **标签：** `skills`, `workflow-automation`, `agent-framework`, `vibecoding`
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `deep-reasoning`
- **推荐理由：** 推理时控制，支持深度推理与恢复能力
...
- **标签：** `creative-writing`, `novel`, `workflow`
- **推荐理由：** 网文、短剧、游戏解说创作全流程工作流插件。
...
- **标签：** `agent-observability`, `video-analysis`, `computer-vision`
- **推荐理由：** 赋予AI视听能力，将屏幕/音视频转为可验证证据
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领与审核工作流
...
- **标签：** `skill-manager`, `audit`, `dsh-plugin`
...
- **标签：** `self-evolving`, `plan-mode`, `agent-review`
- **推荐理由：** 可组合任务控制，支持人类审核的自进化模式
...
- **推荐理由：** 完整的逆向工程和渗透测试技能包，含87个SKILL文件。
...
- **标签：** `ai-infra`, `skills`
- **推荐理由：** AI基础设施开发专项技能，支持高性能系统编写与调试。
...
- **推荐理由：** 适配DSH的Superpowers技能：TDD、调试和协作。
...
- **⭐ 星标：** 62
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政��关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `document`, `chinese`, `workflow`
- **推荐理由：** 公文全流程处理：格式检查、修订、模板生成
...
- **标签：** `bazi`, `fengshui`, `iching`, `agent-skill`
- **推荐理由：** 八字风水易经融合AI技能包，本地确定性引擎，开箱即用
...
- **标签：** `math-modeling`, `paper-writing`, `data-processing`
- **推荐理由：** 数学建模全流程：数据处理、编程、论文写作一站式覆盖
...
- **标签：** `spec-programming`, `agent-skill`, `workflow`
...
- **标签：** `multi-agent`, `memory`, `agent-team`
- **推荐理由：** 持久化多Agent团队，跨会话记忆笔记技能不丢失
...
- **标签：** `multi-agent`, `collaboration`, `shared-workspace`
- **推荐理由：** 多Agent协作，独立模型与共享工作区，MCP工具集成
...
- **推荐理由：** 11项测试工程Skill框架，Playwright+pytest全覆盖
...
- **标签：** `planning`, `debate`, `workflow`
- **推荐理由：** 先议后行机制，支持跨平台AI协作任务规划
...
- **推荐理由：** 5角色多Agent数学建模，33篇模型库+论文模板+进度可视化
...
- **标签：** `skillhub`, `api`, `tencent`
- **推荐理由：** 官方SkillHub文档，含DSH插件接入示例
...
- **标签：** `agent-skill`, `plugin-development`, `standards`
- **推荐理由：** DSH插件开发标准技能，兼容Claude Code和Codex
...
- **标签：** `marketplace`, `skills`, `agent`
- **推荐理由：** 面向DSH的智能体技能分发类插件市场
...
- **标签：** `pre-flight`, `capability`, `agent-tools`
- **推荐理由：** 按任务类型预检并注入Top-K适用插件的智能体能力检查单
...
- **推荐理由：** 通过OAuth绑定ChatGPT账户在DSH内直接对话的订阅插件
...
- **标签：** `mcp`, `api-keys`, `proxy`, `registry`
- **推荐理由：** 代理工具API密钥注册与代理管理
...
- **标签：** `oauth`, `llm-provider`, `subscriptions`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅转为DSH模型来源
...
- **推荐理由：** 新增Command Code模型提供商，支持实时目录和智能选型
...
- **标签：** `api`, `mcp`, `chinese-api`
- **推荐理由：** 一键搜索20个中国开放平台6.5万+ API文档
...
- **标签：** `cloud`, `mcp`, `sdk`
- **推荐理由：** 华为云官方MCP工具与技能，支持AI Agent云端部署
...
- **标签：** `diagram-as-code`, `mermaid-alternative`, `dsh-plugin`
- **推荐理由：** 生成精美架构图、时序图和数据流图。
...
- **标签：** `image-generation`, `prompt-as-code`, `dsh-plugin`
- **推荐理由：** 530+ GPT图片提示词与模板，含可复用技能。
...
- **标签：** `design`, `agent-team`, `vibedesign`, `ui`
...
- **标签：** `design`, `ppt`, `prototyping`, `visual`
- **推荐理由：** AI驱动设计工作室，支持PPT与原型
...
- **⭐ 星标：** 61
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提���庭前三大法宝（大事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **标签：** `legal`, `visualization`, `diagram`
- **推荐理由：** 法律可视化工具：时间轴、诉讼图、庭审对抗图
...
- **推荐理由：** 提供22个Agent工具，支持iOS模拟器与真机自动化
...
- **标签：** `preset`, `deepseek-harness`, `configuration`
- **推荐理由：** 两阶段预设配置，实现DSH快速引导与标准工具链部署。
...
- **标签：** `virtual-terminal`, `agent-sandbox`, `vfs`, `bash`
- **推荐理由：** 首个AI代理虚拟终端，支持VFS沙箱与Bash环境隔离。
...
- **标签：** `account-management`, `multi-model`, `routing`, `utility`
...
- **推荐理由：** 打通Pi与DSH生态，一键运行Pi扩展为DSH插件
...
- **标签：** `utility`, `conflict-resolution`, `diagnostics`
...
- **标签：** `automation`, `crawl`, `directory`
- **推荐理由：** GitHub Actions自动爬取的每日插件目录
...
- **推荐理由：** 在DSH智能体内搜索npm和精选列表的插件发现工具
...
- **标签：** `animated-background`, `glassmorphism`, `threejs`
- **推荐理由：** 实时海洋动态主题，支持波浪、日落和玻璃透明度调节
...
- **推荐理由：** 全面AI资源库与Vibe Coding从入门到进阶教程。
...
- **标签：** `documentation`, `analysis`, `deepwiki`, `code-understanding`
- **推荐理由：** 自动将代码仓库生成结构化技术文档与AI可读���下文。
...
- **标签：** `plugin-registry`, `discovery`, `analysis`
- **推荐理由：** 生态雷达，持续发现与验证插件
...
- **推荐理由：** 一站式科研工作台，支持LaTeX、arXiv和GPU编排。
...
- **推荐理由：** 跨PubMed/arXiv/CrossRef等数据库检索，支持引文图谱与核验
...
- **标签：** `ai-curated`, `recommender`, `updated`
- **推荐理由：** AI策展的190+插件清单，12分类每小时更新
...
- **标签：** `skills`, `marketplace`, `aggregation`
- **推荐理由：** 最全面的技能商店，自动抓取GitHub技能项目
...
- **标签：** `knowledge-base`, `resource-directory`, `awesome-list`
- **推荐理由：** 涵盖模型、Agent、RAG的AI资源百科大全。
...
- **推荐理由：** 全面的DSH生态精选合集，覆盖插件、技能、MCP和UI
...
- **推荐理由：** DSH终极指南，含快速入门、资源推荐与精选插件合集
...
- **标签：** `handbook`, `mcp`, `agent-runtime`
- **推荐理由：** 173个资源手册，覆盖运行时、MCP与插件排查
...
- **推荐理由：** 280+精选插件目录，覆盖14类，DSH必备参考索引
...
- **标签：** `documentation`, `knowledge-base`, `scaffold`
- **推荐理由：** 官方文档知识库，即装即用，插件开发者必备参考
...
- **推荐理由：** 好用的DeepSeek Harness插件中文精选集合
...
- **推荐理由：** 覆盖CLI、Web UI和编码代理的全景插件生态列表
...
- **标签：** `ai-agents`, `dsh-plugins`, `awesome-list`
- **推荐理由：** 收录80+经认证的DSH插件目录
...
- **标签：** `plugin-directory`, `dsh-plugin`, `skills`
- **推荐理由：** 涵盖模型、工具、UI的插件注册表
