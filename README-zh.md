...
- **标签：** `coding-agent`, `cli`, `tui`, `r1`
- **推荐理由：** 基于前缀缓存稳定性的DeepSeek终端编码智能体。
...
- **标签：** `routing`, `injector`, `performance`
- **推荐理由：** 运行时注入器配合任务感知路由优化DSH性能
...
- **标签：** `deepwiki`, `docs`, `dsh-plugin`
- **推荐理由：** 快速生成结构化技术文档和AI就绪上下文。
...
- **推荐理由：** 高性能Rust Agent运行时，配备精美桌面UI。
...
- **推荐理由：** 支持RPC子agent、hashline编辑和MCP的编码agent工具。
...
- **推荐理由：** 管理DSH插件和agent工具的CLI及Schema工具集。
...
- **推荐理由：** 内置iOS模拟器与真机调试，22项Agent自动化能力
...
- **推荐理由：** 插件创建验证技能，Codex可复用，快速生成DSH插件
...
- **推荐理由：** DSH Composer技能选择器，类似WorkBuddy体验
...
- **标签：** `dsh-plugin-desktop`, `cordis`, `desktop`
...
- **标签：** `desktop`, `official`, `ui`
- **推荐理由：** DeepSeek Harness官方桌面客户端，原生体验
...
- **推荐理由：** 开放式侧边栏底座，支持三方拓展与文件/终端/Git内嵌
...
- **标签：** `taskboard`, `cli`, `skills`, `codex`
- **推荐理由：** 现代化可灵活嵌入的任务面板，支持Codex与DSH
...
- **标签：** `tui`, `terminal`, `deepseek-harness`, `react`
- **推荐理由：** 官方公众号收录TUI插件，含实时状态与流式思考显示
...
- **推荐理由：** 零配置Windows桌面客户端，支持远程访问和全功能DSH集成。
...
- **标签：** `desktop`, `mcp`, `electron`, `office-automation`
- **推荐理由：** 本地优先的AI桌面工作空间，支持MCP和Office自动化。
...
- **推荐理由：** 在回复中内联渲染交互式UI组件：图表、表单、测验等。
...
- **推荐理由：** 本地优先的AI代理桌面应用，支持多模型与自演化技能
...
- **标签：** `ios-app`, `mobile`, `dsh-plugin`
- **推荐理由：** DeepSeek Harness原生iOS客户端，支持会话与Agent追踪
...
- **标签：** `desktop-app`, `electron`, `multi-platform`
- **推荐理由：** CodePilot风格跨平台桌面客户端，插件生态丰富
...
- **标签：** `workbench`, `desktop`, `typescript`
- **推荐理由：** 基于DSH插件生态的工作台桌面应用，类似WorkBuddy。
...
- **推荐理由：** 通过TCP直接控制Blender，AI驱动3D建模渲染
...
- **推荐理由：** 一站式DevOps桌面中心，集成数据库、SSH、Docker和AI助手
...
- **标签：** `enhancement`, `terminal`, `keybindings`, `notifications`
- **推荐理由：** 增强DSH体验：任务通知、LLM追踪检查、快捷键和内嵌终端
...
- **标签：** `desktop`, `electron`, `integration`, `ui`
- **推荐理由：** Electron桌面封装，集成DSH Web、侧边栏和插件市场
...
- **标签：** `memory`, `stateful-agent`, `context-engineering`
- **推荐理由：** 专为有状态AI智能体构建的记忆库
...
- **推荐理由：** Agent记忆管理套件，支持记忆召回与跨会话持久化
...
- **推荐理由：** 持久统一记忆层，基于Milvus与Markdown构建。
...
- **标签：** `rag`, `skill-routing`, `vector-search`, `embeddings`
- **推荐理由：** 将SKILL.md文件转为可检索的agent技能语料库，支持语义搜索。
...
- **标签：** `marketplace`, `catalog`, `dsh-plugin`
- **推荐理由：** 最大DSH插件市场，11,000+插件，支持搜索和API
...
- **标签：** `handbook`, `reference`, `agent-runtime`
- **推荐理由：** Agent优先的全面手册，覆盖运行时、插件、MCP等74个资源
...
- **推荐理由：** 完整DSH插件目录，1958仓库，支持中英文搜索与分类排序
...
- **标签：** `knowledge-distillation`, `meta-skill`, `llm-agent`
...
- **标签：** `workspace`, `agent-orchestration`, `no-code`
- **推荐理由：** 跨设备统一智能体工作空间，支持低代码开发
...
- **标签：** `agent-control-plane`, `multi-agent`, `workflow-automation`, `long-horizon`
- **推荐理由：** 持久化状态控制面，支持长周期Agent跨会话持续工作
...
- **标签：** `skills`, `workflow-automation`, `context-engineering`, `multi-agent`
...
- **标签：** `agent-skills`, `deep-reasoning`, `dsh-plugin`
- **推荐理由：** 模型无关推理控制，支持深度推理与恢复能力。
...
- **标签：** `agent-skills`, `creative-writing`, `deepseek-harness`
- **推荐理由：** 内置网文/短剧/游戏写作工作流，开箱即用
...
- **标签：** `task-management`, `dsh-plugin`
- **推荐理由：** 原生任务看板，支持Agent认领审核，SQLite存储
...
- **标签：** `math-modeling`, `workflow`, `automation`
- **推荐理由：** 全栈自动数学建模竞赛Skill，支持GitOps流水线与代码自证
...
- **推荐理由：** 87个SKILL.md逆向工程技能，用于CTF与安全研究
...
- **标签：** `multimodal`, `claude-code`, `agent-skills`
- **推荐理由：** 为Claude Code添加视觉分析能力，支持截图和UI解析。
...
- **推荐理由：** 将热门Superpowers技能移植到Reasonix代码平台。
...
- **推荐理由：** 源自superpowers的TDD、调试和规划技能，提升开发效率
...
- **标签：** `mcp`, `skills`, `capability`
- **推荐理由：** 统一能力管理插件，支持技能工具动态暴露策略
...
- **标签：** `agent`, `mentor`, `code-review`
- **推荐理由：** AI编程导师，引导架构设计与代码评审
...
- **标签：** `guard`, `quality-gate`, `skill`
- **推荐理由：** 工程纪律质控包，需求审查+交付验证一站式
...
- **⭐ 星标：** 42
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。��建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math-modeling`, `paper-writing`, `skill`
- **推荐理由：** 数学建模全流程：数据预处理、绘图、论文写作、降AIGC
...
- **推荐理由：** 5角色多Agent数学建模，33模型库+论文模板+审稿
...
- **推荐理由：** 纯文本模型图像识别技能，34图训练，OCR+像素读取
...
- **推荐理由：** AI Agent测试工程Skill框架，11个Skills+共享知识库
...
- **标签：** `directorx`, `video`, `skills`
- **推荐理由：** AI视频/图像/音频生成技能与知识语料库
...
- **标签：** `agent-skill`, `plugin-dev`, `development`
- **推荐理由：** 开发DSH插件必备Skill套件，兼容Claude Code和Codex
...
- **⭐ 星标：** 2
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞前��查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `dsh-plugin`, `automation`
...
- **标签：** `browser`, `automation`, `cli`
- **推荐理由：** AI智能体真实登录浏览器自动化，无缝操作
...
- **标签：** `bilibili`, `content-discovery`, `self-hosted`, `privacy-first`
- **推荐理由：** 本地私有跨平台内容发现Agent，覆盖B站、YouTube、小红书等
...
- **推荐理由：** 基于Playwright的浏览器自动化插件，集成Tabbit。
...
- **推荐理由：** Agent工具版OpenRouter，统一API密钥与代理管理
...
- **标签：** `cli`, `credentials`, `multi-agent`, `deepseek-harness`
- **推荐理由：** 多模型账户管理工具，支持智能路由与自动故障转移
...
- **标签：** `jailbreak`, `utilities`, `dsh-plugin`
- **推荐理由：** 一键破甲所有模型，支持自定义��示词替换。
...
- **推荐理由：** 为DSH提供Command Code模型接入与智能选模
...
- **推荐理由：** 安全P2P远程访问方案，支持多端加密操作Harness
...
- **推荐理由：** 打通Pi与DSH生态，Pi扩展可作为原生DSH插件运行
...
- **推荐理由：** 备份还原DSH配置、插件���技能，支持WebDAV同步。
...
- **⭐ 星标：** 58
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄控制台（浏览器面板管��官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
- **标签：** `plugin-management`, `ui`, `console`
- **推荐理由：** 官方插件管理控制台，零侵入，生态基建必备
...
- **标签：** `migration`, `utility`, `cli-tools`
- **推荐理由：** 从Claude Code/Codex迁移会话、技能和配置的向导
...
- **标签：** `utility`, `compatibility`
- **推荐理由：** 自动解决插件间冲突与版本兼容问题
...
- **标签：** `pixel-art`, `sprites`, `mascot`, `cli`
- **推荐理由：** 公共像素风动画��物画廊，支持多款CLI AI工具
...
- **标签：** `theme`, `animated-background`, `threejs`
- **推荐理由：** 实时海洋主题，可调节波浪与玻璃态效果
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪与GPU SSH编排。
...
- **推荐理由：** 跨PubMed/arXiv/OpenAlex学术检索，支持引文图谱
...
- **标签：** `finance`, `research`, `multi-agent`
- **推荐理由：** 本地财经数据库，多Agent金融分析，支持IM盯盘通道
...
- **标签：** `api-docs`, `chinese-api`, `research`
...
- **标签：** `patent`, `rag`, `ip-analysis`
- **推荐理由：** 专利侵权分析系统，RAG驱动竞品侵权报告生成
...
- **标签：** `stock`, `analysis`, `quantitative`
- **推荐理由：** 多市场股票分析，39个工具+20个策略技能
...
- **推荐理由：** 实时Token用量图表、成本估算和API余额监控面板
...
- **标签：** `marketplace`, `web`, `ecosystem`
- **推荐理由：** DSH Web插件集市生态，创意工坊分发平台
...
- **推荐理由：** 两阶段预设：轻量启动后加载标准工具集的DSH配置方案
...
- **标签：** `novel-writing`, `ai-writing`, `dsh-plugin`
- **推荐理由：** AI小说创作软件，支持本地与在线模型协作。
...
- **标签：** `encyclopedia`, `ai-resources`, `mcp`, `learning`
- **推荐理由：** AI全维度资源百科：技能、MCP、RAG、多模态和学习路线。
...
- **标签：** `awesome`, `awesome-list`, `cordis`
- **推荐理由：** cordis.run官方维护的自动化插件精选列表。
...
- **标签：** `awesome-list`, `marketplace`, `ecosystem`
...
- **标签：** `awesome-list`, `directory`, `meta`
- **推荐理由：** 精选280+插件目录，14类覆盖，快速发现插件
...
- **推荐理由：** 去垃圾信息的DSH插件、bundle和技能开放数据注册表
...
- **推荐理由：** 双语精选插件列表，便于中英文用户发现优质DSH插件
...
- **标签：** `awesome-list`, `plugin-directory`, `discoverability`
- **推荐理由：** 公开GitHub目录，含安装命令，方便生态发现和快速集成
...
- **标签：** `awesome-list`, `curated`, `practical`
- **推荐理由：** 精选实用DSH插件集合，适合日常开发场景
...
- **标签：** `verified`, `evidence-led`, `installation-guide`
- **推荐理由：** 基于实证的已验证插件目录，附带权限感知的安装指南
...
- **标签：** `awesome-list`, `ecosystem`, `categorized`
- **推荐理由：** 覆盖CLI、Web UI和自主Agent的DSH插件生态分类列表
...
- **标签：** `curated-list`, `quality-check`, `2026`
- **推荐理由：** 2026年经过质量校验的DSH插件精选列表
...
- **标签：** `directory`, `curated`
- **推荐理由：** 精选插件目录，覆盖模型与工具定制
...
- **标签：** `dsh-plugins`, `repository`
- **推荐理由：** DSH插件仓库，用于生态探索与发现
