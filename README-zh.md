...
- **推荐理由：** DeepSeek原生终端编码代理，前缀缓存稳定持续运行。
...
- **标签：** `agent-runtime`, `ai-coding`, `computer-use-agent`
- **推荐理由：** Rust高性能智能体运行时配合桌面编码IDE。
...
- **标签：** `agent-skills`, `coding-agents`, `dsh-plugin`
- **推荐理由：** 架构感知编码代理，基线优先防止漂移
...
- **标签：** `coding`, `agent`, `teamwork`
- **推荐理由：** 将多个编程Agent组织为协同团队工作。
...
- **标签：** `compatibility-layer`, `migration`, `deepseek-harness`
- **推荐理由：** 打通Pi与DSH生态，原生运行Pi插件。
...
- **标签：** `marketplace`, `plugin-market`, `agent`
...
- **标签：** `agent`, `agent-skill`, `ai-agent`, `deepseek-harness`, `dsh-plugin`, `llm-agent`, `skill`
- **推荐理由：** 开发 DSH 插件的 Agent 技能，涵盖插件/适配器标准流程。
...
- **推荐理由：** Codex与DSH双向桥接，支持聊天同步和MCP镜像
...
- **标签：** `agent-framework`, `mcp`, `database`
- **推荐理由：** AI智能体资产包管理器，42项技能与5种CLI集成
...
- **标签：** `plugin-development`, `cordis`, `auditing`
- **推荐理由：** 便携插件开发审计技能，含bundle适配器
...
- **推荐理由：** DSH插件开发踩坑实录，涵盖cordis与tsconfig等
...
- **标签：** `agent-tools`, `checklist`, `utility`
- **推荐理由：** 预检自动注入Top-K相关插件
...
- **标签：** `dsh`, `sidebar`, `extensions`
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/Git/子代理。
...
- **标签：** `desktop-companion`, `plugin-sdk`, `dsh-plugin`
- **推荐理由：** 桌面伴侣含动画宠物，支持插件SDK
...
- **推荐理由：** 对话内驱动Android模拟器或USB设备开发测试
...
- **⭐ 星标：** 57
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄��制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开发引导
- **标签：** `console`, `deepseek-harness`, `dsh`, `dsh-plugin`, `dsh-repository-plugin`, `plugin-management`, `ui`
- **推荐理由：** 轻量控制台管理插件仓库，零补丁改造，开箱即用。
...
- **推荐理由：** Electron 桌面控制台，多标签管理 Harness、插件与 MCP。
...
- **标签：** `desktop`, `tauri`, `devops`
- **推荐理由：** 跨平台桌面应用，整合数据库SSH Docker与AI助手
...
- **标签：** `portal`, `static-site`, `discover`
- **推荐理由：** 静态网页门户浏览安装227+插件
...
- **推荐理由：** LLM知识平台，支持RAG检索、语义搜索与自维护Wiki。
...
- **标签：** `agent-memory`, `vector-search`, `embeddings`
- **推荐理由：** 将SKILL.md文件转化为可检索的Agent知识库。
...
- **标签：** `multi-agent`, `swarm`, `agent-skills`
- **推荐理由：** 多智能体群体框架，支持自主工作流与协作AI系统。
...
- **推荐理由：** 530+工业级图像生成提示词，提炼为可复用智能体技能。
...
- **标签：** `agent-orchestration`, `workspaces`, `mcp`
- **推荐理由：** 统一代理工作区，全设备可访问
...
- **标签：** `multimodal`, `vision`, `agent-skills`
...
- **标签：** `anime`, `bangumi`, `torrent`, `plugin`
- **推荐理由：** 对话内多源搜番，支持Bangumi评分与磁力复制
...
- **标签：** `notes`, `smartisan`, `private-deploy`
- **推荐理由：** 开源版锤子便签，支持Docker部署和公众号导出
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领与审核
...
- **标签：** `skills`, `reasonix`, `agent`
- **推荐理由：** Superpowers技能套件移植至Reasonix
...
- **推荐理由：** 超级 Powers 技能，覆盖 TDD、调试与规划，适合 DSH 开发流。
...
- **推荐理由：** AI 编程导师，扫描架构、生成章节、带练手写核心逻辑。
...
- **标签：** `agent-plugins`, `agent-skills`, `ai-agents`, `claude-code`, `codex-skills`, `deep-research`, `fact-checking`, `market-research`
- **推荐理由：** 88 个开源 Agent 技能，覆盖研究、社交智能与营销工作流。
...
- **标签：** `agent-skills`, `ai-coding`, `deepseek-harness`, `developer-tools`, `dsh-plugin`, `spec-driven-development`
- **推荐理由：** 基于 DSH 工程规范的 Spec 驱动开发 Agent 技能。
...
- **推荐理由：** 融合八字/风水/中医的传统智慧AI技能包，本地确定性引擎
...
- **推荐理由：** 从DSH改编的代码简化技能，适用于Codex工作流
...
- **标签：** `skills-manager`, `organization`, `plugin`
- **推荐理由：** 专业Skills管理插件，方便组织和管理Agent技能
...
- **标签：** `swift-cycle`, `governance`, `agent-skill`
...
- **标签：** `skills`, `gamification`, `task-system`
- **推荐理由：** 游戏化任务系统，迷雾地图+任务栏自动推进
...
- **标签：** `agent-skill`, `engineering`
- **推荐理由：** 便携工程技能，规范开发流程与决策纪律
...
- **推荐理由：** Godot 4全栈游戏开发技能，支持GDScript
...
- **标签：** `design-systems`, `desktop-app`, `figma-alternative`
- **推荐理由：** 开源设计插件，本地优先桌面应用，媲美Claude Design。
...
- **标签：** `agent-skills`, `diagrams-as-code`, `architecture`
- **推荐理由：** 自包含动效架构图表，视觉清晰精美
...
- **标签：** `pixel-art`, `sprites`, `ui`, `gallery`
- **推荐理由：** 动画宠物精灵公共画廊，丰富AI代理体验
...
- **标签：** `video-generation`, `ffmpeg`, `tts`, `ai-video`
- **推荐理由：** 一键生成Vox风格纸拼贴解说视频。
...
- **标签：** `image-generation`, `deepseek-harness`, `ai-agent`
- **推荐理由：** 在DSH聊天中直接生成AI图像，便捷高效。
...
- **标签：** `agent-skills`, `claude-code`, `diagram`, `dsh-plugin`, `legal`, `svg`, `timeline`, `visualization`
- **推荐理由：** 诉讼可视化工具集，自动生成时间轴与法律图表。
...
- **标签：** `background`, `visual`, `customization`
- **推荐理由：** 自定义背景上传与实时预览
...
- **推荐理由：** AI聊天Web UI增强套件，支持Gemini/Claude/ChatGPT等。
...
- **推荐理由：** 隐私优先的简历生成器，支持MCP服务器，完全可定制。
...
- **标签：** `routing`, `infrastructure`, `injector`
- **推荐理由：** 注入器+路由标准套件，支持任务感知推理
...
- **标签：** `context`, `dsh-plugin`, `utility`
- **推荐理由：** 上下文仪表板与管理工具，支持统计和浏览器视图
...
- **标签：** `utility`, `oauth`, `subscription`
- **推荐理由：** 利用现有订阅作为免费LLM接入方案。
...
- **标签：** `backup`, `undo`, `crash-recovery`, `rollback`
- **推荐理由：** 崩溃恢复、安全快照与一键SAFE MODE
...
- **标签：** `dsh-plugins`, `container`, `infrastructure`
- **推荐理由：** 多版本隔离运行，内置WebView界面，插件一键导入
...
- **标签：** `capability-menu`, `mcp`, `tools-management`
...
- **标签：** `context-audit`, `ui`, `tools`
- **推荐理由：** 上下文注入审计工具，可视化检测重复和冲突
...
- **标签：** `migration`, `tooling`, `import`
- **推荐理由：** 一键迁移Claude Code配置到DSH，支持多种工具源
...
- **标签：** `monitoring`, `cost-tracking`, `dashboard`
- **推荐理由：** 实时Token用量、成本追踪和API余额面板
...
- **推荐理由：** DeepSeek Harness的类CodeX插件市场
...
- **推荐理由：** 全面AI资源库，涵盖DeepSeek/GPT/Gemini教程与提示词。
...
- **标签：** `prompt`, `research`, `breaking`
- **推荐理由：** DeepSeek专用稳定化破甲提示词插件。
...
- **标签：** `ai-agents`, `alipay`, `api-documentation`, `chinese-api`, `claude-code`, `cursor`, `deepseek-harness`, `dingtalk`, `douyin`, `dsh-plugin`
- **推荐理由：** 一键搜索 20 个中国开放平台 6.5 万篇 API 文档，零配置。
...
- **标签：** `kline`, `financial`, `technical-analysis`
- **推荐理由：** 交互式K线分析插件，支持多市场和技术指标
...
- **标签：** `evaluation-framework`, `rag-evaluation`, `agent-evaluation`
- **推荐理由：** 证据驱动的全栈评估框架，覆盖提示词、RAG与智能体
...
- **推荐理由：** 证据驱动的精选插件目录，immutable提交审核
...
- **标签：** `recommender`, `awesome-list`, `mcp`
- **推荐理由：** AI策展的190+插件列表，每小时更新12个分类
...
- **标签：** `awesome-list`, `dsh-plugin`, `ui`
- **推荐理由：** 覆盖模型、工具、沙箱、UI和Agent循环的精选插件
...
- **⭐ 星标：** 1
- **语言：** Python
- **描述：** 计算���｜金融｜法律｜HR｜制造｜医疗｜电商等行业精选 dsh 插件目录
- **标签：** `industry`, `dsh-plugin`
- **推荐理由：** 金融、法律、医疗、电商等行业精选插件目录
...
- **标签：** `web`, `aggregation`, `marketplace`
- **推荐理由：** DSH网络插件聚合生态，一站式分发
...
- **标签：** `awesome-list`, `curated`, `discover`
- **推荐理由：** 每日自动筛选的真实插件推荐平台。
...
- **标签：** `learning`, `marketplace`, `docs`
- **推荐理由：** 学习DSH原理与插件市场的最佳实践指南。
...
- **标签：** `plugin-marketplace`, `catalog`, `dsh-store`
- **推荐理由：** 3100+插件市场，支持搜索排名和免费API。
...
- **标签：** `awesome-deepseek-harness`, `awesome-dsh-plugin`, `awesome-list`, `deepseek-harness`, `dsh`, `dsh-ecosystem`, `dsh-plugin`
- **推荐理由：** 社区维护的插件聚合目录，每 4 小时自动同步更新。
...
- **推荐理由：** cordis.run 精选维护的官方自动生成插件目录。
...
- **推荐理由：** 精选280+社区插件目录，覆盖14大类，方便发现新插件
...
- **标签：** `group-photo`, `oauth`, `community`
- **推荐理由：** GitHub授权合影墙，即拍即得拍立得风格
...
- **标签：** `awesome-list`, `2026`
- **推荐理由：** 2026年精选DSH插件列表，含质量审查
...
- **标签：** `radar`, `2origin`, `evidence`
- **推荐理由：** 实证插件雷达与2Origin实验室
...
- **标签：** `awesome-list`, `auto`, `directory`
- **推荐理由：** GitHub Actions自动爬取双语目录
