...
- **推荐理由：** 虚拟终端与虚拟文件系统，为Agent提供安全沙箱环境。
...
- **标签：** `cli`, `codex-cli`, `go`, `tui`, `mcp`
- **推荐理由：** 支持RPC、子代理和MCP的编码代理
...
- **标签：** `cybersecurity`, `vulnerability`, `redteam`, `vscode`
- **推荐理由：** AI生成代码的安全审计与漏洞防护
...
- **推荐理由：** AI编程导师，生成阶梯式章节并自动判题和Review。
...
- **标签：** `agent-platform`, `workbench`, `dsh-plugin`
- **推荐理由：** 基于DSH的插件化Web工作台，原生集成工作区。
...
- **推荐理由：** DSH/Cordis插件开发知识库与官方文档归档，按需调用
...
- **标签：** `plugin-creator`, `codex-skill`, `dsh-plugin`
- **推荐理由：** 可复用的插件构建与验证脚手架技能
...
- **标签：** `desktop`, `dsh-desktop`, `ui`
- **推荐理由：** DSH官方桌面版，原生客户端体验。
...
- **推荐理由：** 开放侧边栏底座，内置文件/终端/Git/对话等面板。
...
- **标签：** `task`, `ui`, `dashboard`
- **推荐理由：** 现代化灵活任务面板，支持嵌入DSH和Codex工作流。
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** TUI增强插件：鲸鱼顶栏、实时状态与流式思考可视化。
...
- **推荐理由：** 可爱鲸鱼浮动小部件，带动画监控DeepSeek账户余额。
...
- **推荐理由：** Windows原生桌面端，支持SSH和移动远程访问
...
- **标签：** `desktop-app`, `electron`, `mcp`, `office-automation`
- **推荐理由：** 本地优先的桌面工作区，管理会话和项目
...
- **标签：** `visual`, `canvas`, `conversation-workspace`
- **推荐理由：** 基于画布的非线性会话探索工作区
...
- **推荐理由：** 将表格、文档、幻灯片等办公功能直接集成到 DeepSeek Harness
...
- **推荐理由：** 本地优先的多模型桌面AI Agent应用，支持技能自我进化
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核，无需iframe
...
- **推荐理由：** 原生移动端客户端，WebSocket同步工作区、会话和Agent轨迹
...
- **推荐理由：** CodePilot风格跨平台桌面客户端，支持完整插件生态
...
- **推荐理由：** 面向科研人员的生物信息桌面Agent，支持离线与技能商店
...
- **推荐理由：** 替换Composer底部状态栏，实时显示模型、余额与定价信息
...
- **推荐理由：** WorkBuddy风格技能选择器，提升DSH操作体验。
...
- **标签：** `container`, `ui`, `manager`
- **推荐理由：** 容器化多版本DSH管理工具，内置WebView界面。
...
- **标签：** `ui`, `knowledge-card`, `sharing`
- **推荐理由：** 将聊天记录转为精美知识卡片，方便下载分享。
...
- **标签：** `multi-agent`, `self-evolving`
- **推荐理由：** 具备自我进化记忆与知识能力的AI助手。
...
- **标签：** `knowledge-base`, `vector-search`
...
- **标签：** `agent-memory`, `memory-library`, `continual-learning`
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** 专为Agent设计的记忆管理套件，支持RAG持久化记忆。
...
- **推荐理由：** 基于Markdown和Milvus的持久统一记忆层，支持所有AI智能体。
...
- **标签：** `ai-knowledge`, `rag`, `resource-encyclopedia`
- **推荐理由：** 涵盖大模型、Agent、RAG的全维度AI资源百科
...
- **标签：** `agentic-workflow`, `mcp-server`
- **推荐理由：** 多智能体群体自动化工作流框架。
...
- **标签：** `ai-writing`, `chinese-novel`, `writing-assistant`
- **推荐理由：** AI辅助长篇网文创作，解决遗忘和幻觉问题。
...
- **标签：** `agent-harness`, `multi-agent`, `workflow`
- **推荐理由：** 本地优先的多Agent持久化控制面，跨会话保持目标推进。
...
- **标签：** `agent-skills`, `inference-control`, `reasoning`
- **推荐理由：** 模型无关推理控制，增强深度推理与恢复工作流。
...
- **标签：** `agent-skills`, `skill-md`, `embeddings`, `semantic-search`
- **推荐理由：** 从SKILL.md整理技能并支持检索的工具箱
...
- **推荐理由：** 网文、短剧、游戏、视频解说全流程工作台，创作效率倍增
...
- **推荐理由：** 在DSH内嵌iOS模拟器及真机控制，22个自动化调试工具
...
- **标签：** `mcm`, `competition`, `gitops`, `agent-skills`
- **推荐理由：** 数学建模全自动流水线技能
...
- **标签：** `self-evolving`, `prompt-engineering`, `review`
- **推荐理由：** Agent可组合可审查的自我进化
...
- **标签：** `agent-skills`, `research`, `business`
- **推荐理由：** 88个跨平台研究与业务流程技能
...
- **⭐ 星标：** 187
- **语言：** Python
- **描述：** 基于tushare构建本地财经数据库。AI原生的多Agent金融分��系统，支持skill拓展Agent能力并自适应生成http接口与mcp调用。支持本地数据库访问的skill对接deepseek-harness/openclaw等通用智能体与微信/QQ/飞书等IM通道盯盘
- **标签：** `finance`, `agent`, `mcp`
- **推荐理由：** 本地财经数据库多Agent盯盘系统
...
- **推荐理由：** 预装25个工程技能，30亿token训练，效率提升10倍。
...
- **推荐理由：** 提供网页搜索、内容提取、金融和研究类AI Agent技能。
...
- **标签：** `chinese-traditional-wisdom`, `bazi`, `fengshui`, `dsh-plugin`
- **推荐理由：** 整合八字、风水、奇门等中华传统智慧的本地确定性Agent技能包
...
- **推荐理由：** 可移植的规格驱动开发技能，受DeepSeek Harness工程模式启发
...
- **推荐理由：** 数学建模全流程技能：建模、数据处理、写作、绘图一站式
...
- **推荐理由：** 多Agent协作框架，支持独立模型、MCP工具与共享工作区
...
- **标签：** `math-modeling`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 5角色多Agent数学建模求解器，含33模型库与论文模板
...
- **标签：** `agent-skill`, `planning`, `workflow`
- **推荐理由：** 议行分离机制，先讨论后执行，Claude Code/DSH/ZCode通用
...
- **推荐理由：** 电影级视频制作技能，含152张镜头配方卡，创意强大。
...
- **推荐理由：** 开发DSH插件的Agent Skill，支持标准流程
...
- **标签：** `mcp`, `api-keys`, `registry`
- **推荐理由：** 集中式API密钥管理，类似智能体工具的OpenRouter。
...
- **推荐理由：** 直接在Agent中搜索6.5万+中国开放平台API文档。
...
- **推荐理由：** 一键同步MCP和配置，支持7+主流AI Agent平台。
...
- **推荐理由：** 通过OAuth绑定ChatGPT账户，在DSH内直接对话。
...
- **标签：** `design`, `ai-agent`, `ui`, `vibedesign`
...
- **标签：** `design`, `ai-design`, `ppt`
- **推荐理由：** AI驱动设计工作室，内置可视化编辑、模板和PPT创作。
...
- **标签：** `dsh-ui`, `interactive`, `layouts`
- **推荐理由：** 内联UI组件，支持图表、表单和���验
...
- **⭐ 星标：** 64
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，��接读案件材料画准一张时间轴，并提供庭前三大法宝（大事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **标签：** `visualization`, `legal`, `agent-skills`
- **推荐理由：** 诉讼可视化工具，生成时间轴、案情图和对抗图。
...
- **标签：** `image-uploader`, `cloud-storage`
...
- **标签：** `account`, `routing`, `utility`
- **推荐理由：** 统一账户管理与多模型智能路由，支持自动故障转移。
...
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅用作DSH的LLM提供商
...
- **推荐理由：** 添加Command Code模型支持，含实时目录和智能模型选择
...
- **标签：** `migration`, `chat-import`, `sessions`
- **推荐理由：** 导入25+ AI平台对话历史为会话
...
- **标签：** `backup`, `config-manager`, `restore`, `webdav`
- **推荐理由：** 支持WebDAV备份恢复配置、插件和MCP服务
...
- **推荐理由：** 收录280+插件的精选目录，14大类全覆盖，发现插件必备
...
- **推荐理由：** Claude Code/Codex迁移向导，数据无缝转移。
...
- **标签：** `ai-resources`, `vibe-coding`
- **推荐理由：** 全面的AI资源大全与零基础教程。
...
- **标签：** `documentation`, `analysis`, `code`
- **推荐理由：** 自动生成精准技术文档和AI可用上下文，提升代码可读性。
...
- **推荐理由：** 集论文撰写、arXiv管理、实验追踪于一体的科研平台
...
- **⭐ 星标：** 270
- **语言：** Python
- **描述：** Academic Paper Search：中文科研用户的 Codex / Claude Code / DeepSeek Harness Skill + MCP；跨 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC 检索去重，支持 MeSH、引用核验、引文图谱、试验检���与审计导出。
- **标签：** `academic`, `mcp`, `research`
- **推荐理由：** 跨库学术检索，支持引文图谱、MeSH和临床试验核验
...
- **推荐理由：** 输入专利号即产出竞品侵权分析报告，支持多Agent调用。
...
- **标签：** `kline`, `finance`, `analysis`, `sidebar`
...
- **推荐理由：** 千余插件按star排序的全量清单，便于发现优质工具。
...
- **标签：** `ai-curate`, `recommendation`, `awesome-list`
- **推荐理由：** AI策展190+插件，按12分类实时更新，发现效率高。
...
- **标签：** `radar`, `evidence-backed`, `directory`
- **推荐理由：** 基于证据的插件雷达，辅助理性选择合适插件。
...
- **语言：** Unknown
- **描述：** DSHA精选插件合集
- **标签：** `curated`, `collection`, `dsha`
- **推荐理由：** DSHA精选插件合集，帮助用户快速找到优质工具。
...
- **推荐理由：** GitHub Actions自动抓取每日更新的插件目录。
...
- **推荐理由：** 在DSH agent内直接搜索npm和插件列表，提升发现效率。
...
- **⭐ 星标：** 7,971
- **语言：** TypeScript
- **描述：** DeepSeek Harness (DSH) Web 插件���合生态 · 万物皆插件，通过创意工坊分发｜｜DeepSeek Harness (DSH) Web Plugin Aggregation Ecosystem · Everything is a plugin, distributed via the Creative Workshop
...
- **标签：** `ecosystem`, `radar`, `discovery`
- **推荐理由：** 生态雷达发现2万+插件，支持K8s持续验证。
...
- **推荐理由：** 每日自动抓取GitHub插件，场景化分类精选，快速发现好插件
...
- **推荐理由：** DeepSeek Harness插件、技能、MCP服务器全面精选资源目录
...
- **标签：** `marketplace`, `awesome-list`, `catalog`
- **推荐理由：** 万级插件市场目录含API接口
...
- **标签：** `awesome`, `plugin-market`, `curated`
- **推荐理由：** cordis.run官方精选插件目录，自动更新维护
...
- **推荐理由：** 官方SkillHub API文档与示例，适合学习参考。
...
- **标签：** `catalog`, `community`, `resources`
...
- **标签：** `registry`, `plugin-registry`, `skills`
- **推荐理由：** 去垃圾插件注册表，含技能与Bundle信息
...
- **推荐理由：** DeepSeek Harness插件市场，类似CodeX的插件分发平台。
...
- **标签：** `dsh-plugins`, `dsh-repository`, `plugin-verify`
- **推荐理由：** 包含分析功能的DSH插件精选仓库。
