...
- **标签：** `dsh-plugin`, `coding-agent`, `cli`
- **推荐理由：** DeepSeek原生终端编码代理，前缀缓存稳定
...
- **标签：** `harness-engineering`, `tutorial`, `dsh-plugin`
- **推荐理由：** DSH插件工程入门最佳教程，从零到一
...
- **标签：** `preset`, `standard-tools`, `dsh-plugin`
- **推荐理由：** 两阶段预设：精简引导+标准工具集
...
- **标签：** `vfs`, `agent-tools`, `typescript`
- **推荐理由：** AI Agent统一虚拟文件系统抽象层
...
- **推荐理由：** 一键切换Claude/Grok/DeepSeek等9大编码工具
...
- **标签：** `coding-agent`, `agent-runtime`, `dsh-plugin`
- **推荐理由：** Rust高性能Agent运行时+桌面IDE一体化
...
- **标签：** `code-review`, `dsh-plugin`, `developer-tools`
- **推荐理由：** 基于12本经典工程著作的AI代码审查与修复
...
- **标签：** `architecture`, `coding-agent`, `drift-check`
- **推荐理由：** 让编码代理具备架构感知和漂移检测能力
...
- **标签：** `swift`, `macos`, `coding-agent`
- **推荐理由：** DeepSeek官方Swift原生macOS编码代理
...
- **标签：** `ios`, `mobile-automation`
- **推荐理由：** 22个Agent工具实现iOS模拟器真机自动化
...
- **标签：** `plugin-discovery`, `automation`, `skill`
- **推荐理由：** 自动搜索安装验证DSH插件的实用Skill
...
- **标签：** `plugin-development`, `agent-skills`, `cordis`
- **推荐理由：** 便携Agent Skill，用于DSH插件开发与审计
...
- **标签：** `engineering`, `practices`, `agent-skill`
- **推荐理由：** 从DSH开发提炼的工程纪律与最佳实践
...
- **标签：** `plugin-development`, `skills`, `testing`
- **推荐理由：** 在DSH内构建和测试插件的Agent技能
...
- **标签：** `agent-tools`, `llm-agents`
- **推荐理由：** DSH agent任务请求的插件预检工具
...
- **标签：** `dsh-plugin`, `ui-generator`, `figma-alternative`
- **推荐理由：** 开源Claude设计替代方案，本地优先桌面应用
...
- **标签：** `dsh-web-ui`, `desktop-extension`, `themes`
- **推荐理由：** Web UI插件，含任务看板、移动端与GL皮肤
...
- **标签：** `sidebar`, `dsh-plugin`, `ui`
- **推荐理由：** 开放侧边栏支持文件编辑/终端/Git/子代理页
...
- **⭐ 星标：** 2,331
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录的 TUI 补位插件：Claude Code ��，鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上下文进度+TPS。npm 一键装。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `dsh-plugin`, `terminal`
- **推荐理由：** 鲸鱼顶栏TUI，流式思考/进度/TPS实时显示
...
- **标签：** `tauri`, `desktop`, `cross-platform`
- **推荐理由：** 轻量级Tauri桌面应用，零配置开箱即用
...
- **标签：** `desktop`, `electron`, `workspace`
- **推荐理由：** 本地优先的DSH桌面工作空间应用
...
- **标签：** `widget`, `floating`, `balance-monitor`
- **推荐理由：** 可爱的浮动鲸鱼小组件监控账户余额
...
- **标签：** `desktop-app`, `local-first`
- **推荐理由：** 本地优先的Tauri桌面应用，开源版Claude Cowork替代品
...
- **标签：** `ui-components`, `interactive`
- **推荐理由：** 交互式UI组件内联渲染，支持图表表单和测验
...
- **推荐理由：** 全平台Electron桌面客户端，类CodePilot体验
...
- **标签：** `desktop`, `electron`, `gui`, `ssh`
- **推荐理由：** Electron桌面GUI客户端，支持SSH与远程访问
...
- **推荐理由：** 锤子便签开源复刻版，支持Docker部署和技能调用
...
- **标签：** `marketplace`, `dsh-plugin`, `plugin-marketplace`
- **推荐理由：** 通过Web GUI一键浏览安装更新所有插件
...
- **标签：** `task-management`, `taskboard`, `dsh-plugin`
- **推荐理由：** 原生SQLite任务看板，无需iframe的Web UI
...
- **标签：** `plugin-market`, `webui`, `dsh-plugin`
- **推荐理由：** 集成到DSH Web GUI的社区插件市场
...
- **标签：** `desktop-app`, `electron`, `ui`
- **推荐理由：** 桌面级DSh管理控制台，支持多Tab管理插件与MCP
...
- **标签：** `ui`, `info-bar`, `usage-tracking`
- **推荐理由：** 底部常驻信息栏，显示模型、余额与定价
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `hud`, `ui`
- **推荐理由：** 浮动HUD面板，显示Git、MCP、技能和Token状态
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `desktop`, `docker`, `ssh`
- **推荐理由：** 跨平台桌面应用，集成数据库、SSH、Docker和AI助手
...
- **⭐ 星标：** 4
- **语言：** JavaScript
- **描述：** 桌面版 DeepSeek Harness：Electron 壳 + 自��管 dsh web 服务，集成 dsh-better-sidebar / dsh-web-ui / ModLens / awesome-dsh-plugin / dshmarket
- **标签：** `desktop-app`, `electron`
- **推荐理由：** Electron桌面壳集成DSH web服务
...
- **标签：** `dsh-plugin`, `agent-memory`, `agentic-rag`
- **推荐理由：** 自演化上下文数据库，统一代理记忆与RAG技能
...
- **标签：** `dsh-plugin`, `rag`, `knowledge-base`
- **推荐理由：** LLM知识平台，将文档转化为可查询RAG和维基
...
- **标签：** `agent-memory`, `long-term-memory`, `mcp`
- **推荐理由：** 持久化长期记忆，支持检索与可视化设置
...
- **推荐理由：** 连接PowerContext服务，实现记忆回取与交接
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `claude-code`, `memory`
- **推荐理由：** 将Claude Code的记忆和技能桥接到DeepSeek Harness
...
- **⭐ 星标：** 9
- **语言：** JavaScript
- **描述：** DSH plugin: bridge Claude Code's .claude/ directory (skills, commands, rules) into DeepSeek Harness natively
- **标签：** `deepseek-harness`, `dsh-plugin`, `claude-code`, `compat`
- **推荐理由：** 原生桥接Claude Code目录到DSH
...
- **标签：** `dsh-plugin`, `multi-agent`, `swarm-intelligence`
- **推荐理由：** 多智能体蜂群编排器，支持自主工作流协调
...
- **标签：** `dsh-plugin`, `agent-skills`, `ai-persona`
- **推荐理由：** 数字生命技能，将记忆转化为可复用的AI人格
...
- **标签：** `routing`, `cordis`, `dsh-plugin`
- **推荐理由：** 任务感知推理路由套件，提升Agent效率
...
- **标签：** `agent-os`, `loop-engineering`, `mcp`
- **推荐理由：** 自我进化Agent OS，持续改进的智能体系统
...
- **标签：** `multi-agent`, `workspace`, `dsh-plugin`
- **推荐理由：** 企业级多引擎Agent工作台，本地优先
...
- **标签：** `agent-skills`, `workflow-automation`, `dsh-plugin`
- **推荐理由：** 智能编排本地技能与工作流程的多Agent路由
...
- **标签：** `video-generation`, `skill`, `dsh-plugin`
- **推荐理由：** 一键自动生成Vox风格拼贴解说视频
...
- **标签：** `vision`, `multimodal`, `skill`
- **推荐理由：** 让纯文本模型具备图像理解能力的视觉工具包
...
- **标签：** `task-manager`, `workflow`, `mcp`
- **推荐理由：** 支持人机协同的跨平台AI代理任务管理器
...
- **标签：** `vision`, `ocr`, `gui-automation`
- **推荐理由：** DSH原生视觉工具包，支持OCR和UI还原
...
- **⭐ 星标：** 169
- **语言：** Python
- **描述：** 为 Claude Code 赋能多模态视觉能力，支持豆包、通义千问、GPT-4o 等模型，用于截图 / UI / 图表分析；适配 DeepSeek 等无视觉底座，搭配 browser-harness 可做前端布局自动化检查。
- **标签：** `vision`, `multimodal`, `agent-skill`
- **推荐理由：** 为Claude Code添加多模态视觉能力，支持截图分析
...
- **标签：** `creative-writing`, `drama-skills`, `dsh-plugin`
- **推荐理由：** AI辅助小说创作与短剧生产工作流
...
- **标签：** `agent-skills`, `embeddings`, `vector-search`
- **推荐理由：** 将SKILL.md整理为可检索的Agent技能库
...
- **标签：** `superpowers`, `skills`, `reasonix`
- **推荐理由：** Superpowers技能移植至Reasonix编码环境
...
- **推荐理由：** TDD、调试、规划等Superpowers技能适配
...
- **⭐ 星标：** 65
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、生成阶梯式 Chapter、带你手��核心逻辑、自动判题、AI Code Review。
- **标签：** `agent-skills`, `learning`
- **推荐理由：** AI编程导师，自动生成阶梯课程并辅助代码审查
...
- **⭐ 星标：** 39
- **语言：** Unknown
- **描述：** An Agent Skills skill for developing DeepSeek Harness (DSH) plugins（开发 DSH 插件的 Agent Skill）——插件/���务/事件/工具/LLM 适配器/打包安装的标准。Works with Claude Code, Codex, DSH, VS Code Copilot & any compatible agent.
- **标签：** `agent-skills`, `plugin-development`
- **推荐理由：** 开发DSH插件的标准技能指南，兼容多平台
...
- **标签：** `agent-skills`, `spec-driven`, `ai-coding`
- **推荐理由：** 仓库级规范驱动开发技能，支持vibe coding
...
- **标签：** `agent-skill`, `knowledge-base`, `plugin-development`
- **推荐理由：** 插件开发知识库，提供官方文档与Cordis入门指南
...
- **标签：** `gongwen`, `docx`, `chinese-docs`
- **推荐理由：** 符合国标的中文公文全流程处理工具
...
- **标签：** `canvas`, `agent-skills`, `cordis`
- **推荐理由：** 原生WeShop集成，无限画布与创意技能
...
- **标签：** `record-replay`, `computer-use`, `workflow`
- **推荐理由：** 录制macOS桌面流程并转为Agent技能
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skill-management`, `mcp`
- **推荐理由：** 通过Web界面热加载MCP和技能，粘贴JSON即连
...
- **标签：** `deepseek-harness`, `math`, `skills`
- **推荐理由：** 17张数学建模技能卡，含预设和Windows便携脚本
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `godot`, `game-development`
...
- **标签：** `dsh-plugin`, `agent-skills`, `diagrams`
- **推荐理由：** 代理技能，生成精美可验证的架构图和序列图
...
- **标签：** `design`, `vibecoding`, `mcp`, `flutter`
- **推荐理由：** AI原生矢量设计工具，支持并发Agent团队
...
- **标签：** `image-generation`, `ai-agent`, `dsh-plugin`
- **推荐理由：** 在DSH对话中直接调用多模型生成图片
...
- **推荐理由：** AI视频/图像/音频生成，支持分镜与知识 corpus
...
- **标签：** `dsh-plugin`, `browser-extension`, `chat-management`
- **推荐理由：** 覆盖全浏览器的AI工具增强套件
...
- **标签：** `ai-agent`, `content-discovery`, `dsh-plugin`
- **推荐理由：** 自动从B站/小红书/抖音等平台发现推荐内容
...
- **标签：** `browser-extension`, `chrome`, `workflow`
- **推荐理由：** Chrome插件将AI对话转化为可复用工作流
...
- **推荐理由：** 基于Playwright的Tabbit浏览器自动化工具
...
- **标签：** `dsh-plugin`, `self-hosted`, `mcp-server`
- **推荐理由：** 隐私优先的简历构建工具，完全自托管且免费
...
- **标签：** `dsh-plugin`, `electron-app`, `image`
- **推荐理由：** 终极图片上传工具，支持60+托管服务和编辑器
...
- **标签：** `dsh-plugin`, `low-code`, `ai-agents`
- **推荐理由：** 开源AI+无代码平台，快速构建业务系统
...
- **⭐ 星标：** 3,533
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外挂视觉插件，为 DeepSeek、GLM 等纯文本模型外���视觉能力，粘贴图片即得结构化 JSON 证据（OCR、版面、语义）。
- **标签：** `vision`, `ocr`, `multimodal`, `dsh-plugin`
- **推荐理由：** 视觉插件：图片转JSON证据，赋能文本Agent
...
- **⭐ 星标：** 1,340
- **语言：** Python
- **描述：** DSH 插件雷达与精选榜：多路自动发现 9000+ 候选，容器真实安装路径运行级实测（��档判定），精选 Top 50 · 11 类人工策展，全量索引 PLUGINS-ALL.md，自动更新。
- **标签：** `plugin-registry`, `awesome-list`, `dsh-plugin`
- **推荐理由：** 扫描9000+候选插件并实测验证的精选榜单
...
- **标签：** `context`, `dashboard`, `utilities`
- **推荐理由：** 提供上下文仪表盘和管理统计功能
...
- **⭐ 星标：** 266
- **语言：** JavaScript
- **描述：** 用 30 秒为你的 DeepSeek Harness找到合适的插件。 仓库由脚本每天自动抓取，再经人工逐个核实——真插件进目录，蹭热度的进黑名单，每条剔除理由公开可查。并告诉你每个插件适合谁、从哪里开始。
- **标签：** `awesome-list`, `discovery`
- **推荐理由：** 每日自动抓取验证的插件发现工具
...
- **标签：** `awesome-list`, `bilingual`
- **推荐理由：** 双语整理的DSH插��工具和集成目录
...
- **标签：** `oauth`, `subscription`
- **推荐理由：** 通过OAuth使用订阅账号作为DSH模型提供商
...
- **标签：** `documentation`, `learning`
- **推荐理由：** 学习DSH原理、插件市场和最佳实践
...
- **标签：** `awesome-list`, `guide`
- **推荐理由：** DSH终极入门指南，含精选插件和资源
...
- **标签：** `cost`, `token-usage`, `tracking`
- **推荐理由：** 实时追踪会话日开销与Token用量，多提供商支持
...
- **标签：** `migration`, `compatibility`, `pi-agent`
- **推荐理由：** 打通Pi生态，兼容层直接运行Pi插件
...
- **标签：** `backup`, `crash-recovery`, `undo`
- **推荐理由：** 崩溃恢复、撤销变更、一键安全模式
...
- **标签：** `context-migration`, `session-migration`, `preset-migration`
- **推荐理由：** 跨预设会话迁移，保留状态与意图
...
- **标签：** `plugin-management`, `ui`, `console`
- **推荐理由：** DSH生态基建插件，提供插件管理与开发引导
...
- **标签：** `desktop`, `plugin-management`
- **推荐理由：** 隔离容器管理多版本DSH，一键导入插件
...
- **标签：** `dsh-plugin`, `plugin-manager`
- **推荐理由：** 内置GUI插件管理器，热加载无需重启
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `token-usage`, `data-visualization`
...
- **标签：** `dsh-plugin`, `cli`, `plugin-manager`
- **推荐理由：** 用于发现、安装和管理DSH插件的CLI工具
...
- **⭐ 星标：** 1
- **语言：** JavaScript
- **描述：** DSH 插件市场动态插件：浏览 awesome-dsh-plugin 社区精选（227），搜索/分类/星标排名/一键安装/已装检测/收藏
- **标签：** `dsh-plugin`, `market`
- **推荐理由：** 带搜索分类和一键安装的插件市场动态插件
...
- **标签：** `pet`, `mascot`, `pixel-art`, `dsh-plugin`
- **推荐理由：** AI编码工具动态宠物吉祥物画廊
...
- **标签：** `theme`, `threejs`, `webgpu`
- **推荐理由：** WebGPU+Three.js海洋主题皮肤，视觉效果惊艳
...
- **标签：** `theme`, `skin`, `color-scheme`
- **推荐理由：** 一键切换34款opencode经典配色主题
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `themes`, `webui`
- **推荐理由：** 自定义背景插件，支持预设和浏览器端压缩
...
- **标签：** `dsh-plugin`, `ai`, `rag`
- **推荐理由：** 全面AI资源库，含教程和提示词百科
...
- **标签：** `research`, `skill`, `dsh-plugin`
- **推荐理由：** 自动检索8大中文平台近30天内容生成研究报告
...
- **⭐ 星标：** 54
- **语言：** TypeScript
- **描述：** 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接搜索 20 个中国开放平台的 65,600+ 篇 API 文���；零配置，支持 Skill 与 DSH 原生插件。
- **标签：** `api-documentation`, `chinese-api`, `research`
...
- **标签：** `kline`, `analysis`, `deepseek-harness`
- **推荐理由：** 交互式K线分析，支持技术指标与多市场行情
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugins`, `registry`
- **推荐理由：** 防垃圾的DSH插件开放数据注册表
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugin`, `stock`, `finance`
- **推荐理由：** DSH股票/金融/量化插件精选列表
...
- **标签：** `awesome-list`
- **推荐理由：** 原始awesome-dsh-plugins仓库（已重定向）
...
- **标签：** `ai-agents`, `bilingual`
- **推荐理由：** 证据导向双语插件目录，不可变审查
...
- **推荐理由：** 通过GitHub Actions自动爬取的双语插件目录
...
- **标签：** `dsh-plugin`, `entertainment`
- **推荐理由：** 竹知了网页模拟版，真实录音采样，零依赖
...
- **标签：** `awesome-list`, `plugin-catalog`, `mcp`
- **推荐理由：** DSH生态插件技能MCP精选目录，必备索引资源
...
- **标签：** `marketplace`, `plugin-store`, `catalog`
- **推荐理由：** 3100+插件商店，支持搜索排名和公共API
...
- **标签：** `marketplace`, `plugin-hub`, `v4-pro`
- **推荐理由：** DeepSeek v4 Pro插件市场中心
...
- **标签：** `awesome-list`, `dsh-plugin`, `agents`
- **推荐理由：** DSH插件精选清单，覆盖生产力与工具
...
- **标签：** `awesome-dsh-plugin`, `plugin-marketplace`
- **推荐理由：** 社区维护的自动同步插件目录，方便发现优质插件
...
- **推荐理由：** cordis.run精选维护的官方自动生成的插件目录
...
- **推荐理由：** Use this cross-platform skill in Codex or Claude Code to establish repository-lo
...
- **推荐理由：** DeepSeek Harness (DSH) 插件精选目录 — 14 类 280+ 个社区插件，覆盖 MCP / Skill / TUI / 多 Agent /
...
- **推荐理由：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质
...
- **推荐理由：** One-tap Codex move-in to DSH: auto-import project chats, two-way Skills & MCP sy
...
- **推荐理由：** DSH Skills Manager 基于 DeepSeek Harness 的Skills管理插件
...
- **推荐理由：** Multi-agent team collaboration for DeepSeek Harness, with independent models, sk
...
- **推荐理由：** DSH 内测收官合影墙：GitHub OAuth 零权限登录 + 冻结白名单校验的拍立得合影站（含 DSH Skill 包装）
...
- **推荐理由：** Free image reading & generation for DeepSeek Harness (rc.7/rc.8/v0.1.1-rc.1) — p
...
- **推荐理由：** Design aesthetics skill pack for DeepSeek Harness (DSH) - keeps vibe-coded websi
...
- **⭐ 星标：** 15
- **语言：** TypeScript
- **描述：** 面向 DeepSeek Harness Web GUI 的正式插件包：在设置页的「Web UI 插件」分组中新增一张「技能与 MCP」卡片，用于在浏���器里管理技能（skills）与 MCP 服务器。
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** 面向 DeepSeek Harness Web GUI 的正式插件包：在设置页的「Web UI 插件」分组中新增一张「技能与 MCP」卡片，用于在浏���器里管
...
- **推荐理由：** Swift Cycle governance skill adapter for DeepSeek Harness; user-invoked, version
...
- **推荐理由：** Package manager for AI agent assets — 42 skills, 5 CLIs, 1 plugin. Skills for AI
...
- **标签：** `awesome-list`, `curated`, `directory`
- **推荐理由：** 精选DSH插件列表与发现资源
...
- **标签：** `cordis`, `dsh-plugin`
- **推荐理由：** 官方自动生成的Cordis插件列表
