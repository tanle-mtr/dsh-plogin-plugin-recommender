...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 203,183 | Core DSH ecosystem—everything is a plugin framewor |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 92,575 | Open-source design system generator with local-fir |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,705 | Meta-harness for multi-agent swarms and autonomous |
...
- **标签：** `ai-agent`, `multi-agent`, `self-evolving`
- **推荐理由：** 具备自我进化记忆的超级AI助手框架。
...
- **推荐理由：** 首个视觉插件，让纯文本Agent处理图像并输出JSON
...
- **标签：** `deepseek-harness`, `claude-code`, `multi-model`
- **推荐理由：** 一键切换多款 AI 编码工具和模型。
...
- **推荐理由：** 高性能 Rust Agent 运行时，精美桌面应用。
...
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，带严重性标签和自动修复
...
- **标签：** `security`, `coding`, `redteam`
- **推荐理由：** 为AI生成代码提供安全防护，检测幻觉包与漏洞。
...
- **推荐理由：** 为纯文本LLM赋能多模态视觉能力，支持截图/图表分析
...
- **推荐理由：** 可组合可审查的Agent自进化工作流，支持人工审核
...
- **标签：** `codex`, `deepseek-harness`, `mcp`, `sync`
- **推荐理由：** 双向同步Codex与DSH对话，自动镜像MCP工具。
...
- **标签：** `agent-framework`, `package-manager`, `deepseek-harness`
- **推荐理由：** 42个AI技能包管理器，覆盖评估驱动和规格驱动开发。
...
- **标签：** `agent`, `plugin`, `utility`, `coding`
- **推荐理由：** Agent任务预检插件库提升利用率
...
- **标签：** `ai-agents`, `chatgpt`, `dsh-plugin`, `oauth`, `subscription`
...
- **标签：** `desktop`, `dsh-plugin`, `tauri`
- **推荐理由：** 5MB安装包，零配置，跨平台桌面版，预置插件开箱即用
...
- **⭐ 星标：** 1,318
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮���盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `floating-widget`, `dsh-plugin`, `desktop-pet`
- **推荐理由：** 可爱鲸鱼挂件监控账户余额，支持拖拽吸附和数字动画
...
- **标签：** `desktop-app`, `dsh-plugin`, `local-first`
- **推荐理由：** 本地优先桌面工作空间，整合会话、文件、研究和插件
...
- **推荐理由：** 本地优先多模型桌面Agent，支持技能自进化与隐私保护。
...
- **推荐理由：** 跨平台桌��客户端，类CodePilot体验，完整插件支持。
...
- **推荐理由：** DSH官方Windows桌面GUI，集成SSH、技能与移动端远程访问
...
- **标签：** `office`, `productivity`, `ui`
- **推荐理由：** 将电子表格、文档、幻灯片等办公套件集成进DSH
...
- **⭐ 星标：** 11
- **语言：** Rust
- **描述：** StarHub 是一款跨平台桌面应用（Tauri 2 + Rust 主进程 + DeepSeek Harness React 工作台 + Go Sidecar），把开发运维日常高频工具整合到同一个窗口 —— 数据库、SSH/SFTP、Docker 面板与 AI 助手。目标是减少��� Navicat、Xshell、Portainer、文件管理器和 AI 对话窗口之间来回切换的成本。  前端架构：基于 DeepSeek Harness 原生 React 工作台（/starhub-react 路由）。资产、设置、SSH 终端、SFTP、数据库
- **标签：** `desktop`, `tauri`, `docker`, `ssh`
- **推荐理由：** 跨平台桌面应用整合DB/SSH/Docker与AI助手。
...
- **标签：** `memory`, `rag`, `agent-memory`, `markdown`
- **推荐理由：** 可移植的本地优先记忆层，Markdown原生自进化存储
...
- **标签：** `memory`, `rag`, `token-savings`, `hybrid-retrieval`
...
- **推荐理由：** 基于 Milvus 的持久统一记忆层，支持语义搜索。
...
- **标签：** `memory`, `dsh-plugin`, `mcp`
- **推荐理由：** 通过OpenClaw集成提供无限持久记忆，AI代理共享上下文
...
- **标签：** `memory`, `agentic-ai`, `dsh-plugin`
...
- **标签：** `rag`, `skills`, `retrieval`
- **推荐理由：** 将分散的技能文件整理为可检索的Agent技能知识库。
...
- **标签：** `evaluation`, `rag`, `benchmark`, `deepseek-harness`
- **推荐理由：** 提示词、RAG、技能的证据驱动评估和可观测框架。
...
- **标签：** `prompt-engineering`, `skills`, `image-generation`
- **推荐理由：** 530+工业级提示词模板，提取为可复用技能。
...
- **标签：** `skills`, `knowledge-distillation`, `agent`
...
- **标签：** `skills`, `workflow`, `multi-agent`
- **推荐理由：** 智能编排本地技能的工作流自动化工具。
...
- **推荐理由：** 基于 Anthropic J-space 研究的 AI 认知增强技能。
...
- **标签：** `video`, `ffmpeg`, `agent-skill`
- **推荐理由：** 一键生成 Vox 风格剪纸解说视频的智能技能。
...
- **标签：** `agent-teams`, `dsh-plugin`, `agentic-workflows`
- **推荐理由：** 多代理协作框架，实现DSH中协调工作流
...
- **标签：** `ai-workflow`, `plugin`, `productivity`
- **推荐理由：** 将AI对话转化为可组织可复用的跨平台工作流
...
- **标签：** `math-modeling`, `skill`, `competition`
...
- **标签：** `task-management`, `plugin`
- **推荐理由：** 本地SQLite任务板，支持Agent认领与审查流程
...
- **推荐理由：** 源自Superpowers的TDD调试规划协作技能集
...
- **推荐理由：** 将Superpowers技能移植到Reasonix编程工具
...
- **标签：** `ctf`, `reverse-engineering`, `pentest`
- **推荐理由：** 87个SKILL.md的逆向工程与渗透测试技能
...
- **标签：** `agent-skills`, `spec-driven-development`
- **推荐理由：** 仓库原生Spec编程Skill，支持DSH规范驱动开发
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `dashboard`
- **推荐理由：** 八字风水中医全融合AI技能包，含可视化看板
...
- **标签：** `skills`, `task-board`, `wayfinder`
- **推荐理由：** MattPocock技能驱动的任务看板，清晰派活
...
- **推荐理由：** 类WorkBuddy风格，DSH内一键选取Skill
...
- **⭐ 星标：** 25
- **语言：** JavaScript
- **描述：** DSH Skills Manager — �� DeepSeek Harness 中统一加载并安全管理本机 Agent Skills · Load and safely manage local Agent Skills in DSH
- **标签：** `skills-manager`, `dsh-plugins`
- **推荐理由：** 统一加载和管理本地Agent Skills，安全可靠
...
- **标签：** `gongwen`, `docx`, `agent-skills`, `deepseek-harness`
- **推荐理由：** 公文全流程处理工具，符合国家标准格式要求。
...
- **标签：** `agent-skills`, `swift-cycle`, `deepseek-harness`
- **推荐理由：** Swift Cycle治理技能适配器，版本锁定离线可用。
...
- **标签：** `agent-skills`, `image-generation`, `vision`, `deepseek-harness`
- **推荐理由：** 图片读取与生成技能，支持粘贴图片和自动视觉转录。
...
- **标签：** `qa`, `test-automation`, `agent-skills`
- **推荐理由：** 模拟资深测试工程师的QA技能框架，支持多Agent。
...
- **标签：** `skill-generator`, `agent-skills`, `auto-learning`
- **推荐理由：** 自动将成功会话转化为可复用Agent技能，提升效率。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，助力AI辅助游戏创作。
...
- **推荐理由：** 一键配置MCP/Skills，支持Claude/Codex/DSH等多工具
...
- **推荐理由：** Gemini/Claude/ChatGPT增强套件与提示词管理器。
...
- **推荐理由：** 跨平台 AI 内容发现 Agent，支持中英文主流平台。
...
- **标签：** `search`, `web`, `free`
- **推荐理由：** 免费网页搜索插件，无需注册即可为所有模型提供搜索能力。
...
- **推荐理由：** 在对话中运行iOS模拟器，提供22个移动自动化工具。
...
- **推荐理由：** 锤子便签复刻版，支持Docker私有部署和公众号导出
...
- **标签：** `social`, `photo`, `github-oauth`, `deepseek-harness`
...
- **标签：** `ai-agent`, `chat-bot`, `deepseek-harness`, `dingtalk`, `dsh-plugin`
- **推荐理由：** 钉钉IM通道，聊天即变工具化代理，支持流式模式。
...
- **标签：** `low-code`, `no-code`, `business-systems`
...
- **推荐理由：** DeepSeek专用破甲插件，稳定化提示词突破限制
...
- **推荐理由：** 通过OAuth使用ChatGPT/Claude订阅作为DSH模型提供者。
...
- **标签：** `plugin-management`, `dsh-plugin`, `console`
- **推荐理由：** 插件生态基础设施，含注册表控制台和开发指南
...
- **标签：** `dsh-plugins`, `container`, `ui`
- **推荐理由：** 容器隔离运行多版本DSH，一键导入插件
...
- **标签：** `mcp`, `skills`, `deepseek-harness`, `ui`
- **推荐理由：** Web GUI插件，从设置页直接管理技能和MCP服务器。
...
- **标签：** `migration`, `import`, `developer-tools`
- **推荐理由：** 无缝迁移Claude Code配置至DSH，降低切换成本。
...
- **推荐理由：** 快速优雅的Neovim插件管理器，Lua编写极简高效。
...
- **标签：** `data-visualization`, `cost-tracking`, `dashboard`
- **推荐理由：** 实时Token用量、成本估算和API余额可视化面板。
...
- **标签：** `utility`, `plugin`, `conflict-resolution`
- **推荐理由：** 自动检测和解决插件版本冲突
...
- **标签：** `pet`, `mascot`, `pixel-art`, `theme`
- **推荐理由：** 多平台编程Agent的宠物动画画廊
...
- **标签：** `theme`, `skin`, `whale`
- **推荐理由：** DeepSeek Harness 鲸鱼娘系列主题皮肤。
...
- **标签：** `themes`, `skins`, `opencode`, `deepseek-harness`
- **推荐理由：** 34款opencode经典配色一键应用，重启不丢失。
...
- **标签：** `theme`, `background`, `webui`
- **推荐理由：** 可自定义网页背景，支持预设和自动保存
...
- **标签：** `ai`, `llm`, `rag`, `prompt`
- **推荐理由：** AI资源大全，涵盖教程、提示词和知识百科
...
- **标签：** `agent`, `harness`, `tutorial`, `dsh`
- **推荐理由：** DSH入门教程，从零开始掌握Harness工程
...
- **推荐理由：** 自动发现15900+插件候选，k8s实测验证，生态雷达工具
...
- **标签：** `awesome-list`, `dsh-plugins`, `ecosystem`
- **推荐理由：** 全面的DSH生态插件与工具精选目录
...
- **标签：** `encyclopedia`, `research`, `learning`
...
- **标签：** `industry-research`, `report`, `deepseek-harness`
- **推荐理由：** 行业公司研究工具包，含方法论和公开源政策追踪。
...
- **标签：** `awesome-list`, `plugin`, `deepseek-harness`
- **推荐理由：** 2026年经过质量检查的DSH插件精选目录
...
- **标签：** `awesome-list`, `verified`, `plugin`
- **推荐理由：** 证据驱动的已验证插件目录含权限指引
...
- **标签：** `awesome-list`, `verified`, `ranking`
...
- **标签：** `portal`, `static-site`, `plugin-directory`
- **推荐理由：** 零依赖静态插件浏览和安装门户
...
- **推荐理由：** GitHub上按星标排序的1000+ DSH工具全量清单
...
- **标签：** `industry`, `directory`, `curated`
- **推荐理由：** 精选多行业D SH插件目录，便于快速发现适合的工具。
...
- **标签：** `toy`, `web`, `lightweight`
- **推荐理由：** 传统玩具竹知了的 Web 模拟版，零依赖。
...
- **⭐ 星标：** 296
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。���天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `discovery`, `plugin-directory`, `awesome`
- **推荐理由：** 每日验证插件发现引擎，场景化分类与热度排行。
...
- **标签：** `awesome-list`, `mcp`
- **推荐理由：** Curated list of DSH plugins, skills, MCP servers & orchestrators
...
- **标签：** `deepseek-harness`, `dsh-bundle`, `dsh-plugin`, `dsh-plugins`
- **推荐理由：** DSH插件生态仓库，便于插件管理和发现。
...
- **标签：** `ai-agent`, `awesome-list`, `deepseek-harness`, `plugin-registry`
...
- **标签：** `agent`, `discoverability`, `deepseek-harness`, `search`
- **推荐理由：** 在DSH代理内搜索npm和插件列表，提升可发现性。
...
- **标签：** `ai-agents`, `awesome-list`, `deepseek-harness`, `dsh-plugins`
- **推荐理由：** 70+经验证的DSH插件，按功能分类整理。
...
- **推荐理由：** YAML驱动的自动更新插件列表，部署于Vercel。
...
- **标签：** `agent-harness`, `coding-agents`, `deepseek-harness-plugin`, `llm`, `typescript`
- **推荐理由：** 覆盖模型、工具、沙箱、UI和代理循环的精选插件集。
