...
- **标签：** `multi-agent`, `mcp-server`, `swarm`
- **推荐理由：** 多智能体蜂群编排器，MCP集成。
...
- **标签：** `agent-os`, `mcp`, `skill`
- **推荐理由：** 自进化智能体操作系统带评估循环
...
- **标签：** `agent`, `rust`, `coding`
- **推荐理由：** 高性能Rust代理运行时，结合精美桌面应用，适合AI编码工作流。
...
- **标签：** `security`, `code-audit`, `vulnerability`
- **推荐理由：** 审计AI生成代码的安全漏洞与幻觉包，保障安全
...
- **标签：** `coding-agent`, `tui`, `rpc`, `mcp`
- **推荐理由：** 终端编码代理，支持子代理、RPC与MCP扩展
...
- **推荐理由：** 实现Agent可组合、可审查、可持续自我进化的工作流。
...
- **标签：** `coding`, `skills`
- **推荐理由：** Superpowers技能移植到Reasonix编码
...
- **标签：** `plugin-creator`, `dsh`, `codex`
- **推荐理由：** 可复用插件模板，简化DSH插件构建与验证流程
...
- **标签：** `code-simplification`, `refactoring`, `skill`
- **推荐理由：** 类Codex代码简化技能，自动化重构利器
...
- **标签：** `agent-orchestration`, `multi-agent`, `dsh-plugin`
...
- **标签：** `cordis`, `launcher`, `orchestration`, `dsh-plugin`
- **推荐理由：** 统一编排Runtime、Provider、插件、技能、MCP
...
- **标签：** `evaluation`, `benchmark`, `rag`, `dsh-plugin`
- **推荐理由：** 基于证据的评估框架，覆盖Prompt、RAG、技能
...
- **推荐理由：** 开发DSH插件的Agent Skill，含标准与工具链
...
- **标签：** `design`, `ui-generator`, `desktop-app`
...
- **标签：** `widget`, `balance`, `desktop`
- **推荐理由：** 可爱的小鲸鱼浮动部��，实时显示余额，支持拖拽和动画效果。
...
- **推荐理由：** 轻量级5MB Tauri桌面应用，预置插件，支持多平台。
...
- **推荐理由：** 本地优先的DSH桌面客户端，集成会话/项目/文件管理
...
- **推荐理由：** 免安装Windows桌面客户端，内置SSH与插件系统
...
- **标签：** `agent`, `desktop-app`, `tauri`, `typescript`
- **推荐理由：** 本地优先桌面AI代理，多模型支持，隐私友好
...
- **标签：** `office`, `desktop-app`
- **推荐理由：** 将表格、文档、幻灯片等办公能力带入DeepSeek Harness
...
- **标签：** `task-management`, `ui-design`, `typescript`
- **推荐理由：** SQLite驱动的本地任务看板，支持Agent认领与审核
...
- **⭐ 星标：** 302
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将工作���、会话、实时回复和 Agent 执行轨迹带到 iPhone，同时延续 DeepSeek WebUI 克制、清晰的视觉语言
- **标签：** `ios`, `mobile-automation`, `typescript`
- **推荐理由：** DSH原生iOS客户端，WebSocket连接Agent控制
...
- **标签：** `ios-simulator`, `mcp`, `ui-automation`
- **推荐理由：** 内置iOS模拟器及22个Agent工具用于自动化操作
...
- **推荐理由：** 开源锤子便签复刻版，支持Docker私有化与多平台消息推送。
...
- **标签：** `bioinformatics`, `desktop-app`, `turan`
- **推荐理由：** 面向临床的生物信息学桌面代理，支持离线模式
...
- **标签：** `web`, `workbench`, `plugin-system`
...
- **推荐理由：** 集成SSH/SFTP/Docker/数据库/AI的DevOps桌面中心
...
- **推荐理由：** Electron桌面壳集成DSH Web UI和ModLens插件生态
...
- **标签：** `theming`, `background`, `ui-customization`
- **推荐理由：** 支持自定义图片和预设氛围的Web UI背景美化插件
...
- **标签：** `ui-enhancement`, `button`, `compact`
- **推荐理由：** DSH Web上下文中一键折叠按钮的小工具
...
- **标签：** `documentation`, `wiki`, `knowledge`
- **推荐理由：** 从代码生成技术文档和AI可用上下文
...
- **标签：** `memory`, `milvus`, `vector-db`, `rag`
...
- **标签：** `skill-retrieval`, `rag`, `embeddings`
- **推荐理由：** 将SKILL.md整理为可检索的技能知识库，支持评测
...
- **标签：** `context`, `memory`, `dsh-plugins`
- **推荐理由：** 零提示关联记忆，三层自动巩固与技能结晶
...
- **标签：** `agent-orchestration`, `agent-skills`, `mcp`
- **推荐理由：** 跨设备统一智能体工作空间
...
- **标签：** `agent-skills`, `skill`
- **推荐理由：** 最大技能商店，自动抓取GitHub项目
...
- **标签：** `agent-skills`, `agent-collaboration`
- **推荐理由：** 企业级多智能体可视化工作空间
...
- **标签：** `video`, `ai`, `automation`
- **推荐理由：** 从主题到Vox风格解释视频的全自动AI生成工具。
...
- **标签：** `writing`, `novel`, `workflow`
- **推荐理由：** 完整的AI小说创作软件，含角色、世界观和审稿流程。
...
- **标签：** `vision`, `ocr`, `gui`
- **推荐理由：** 纯文本模型的强大视觉工具箱：图像问答、OCR和UI还原。
...
- **标签：** `agent-skills`, `workflow`, `experience-automation`
- **推荐理由：** 将经验转化为可重复执行的Skill，跨Agent平台通用。
...
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销与商业自动化场景。
...
- **推荐理由：** 自动搜索安装验证DSH插件的Skill，提升插件发现效率。
...
- **推荐理由：** AI基础设施开发专用Skill，支持高性能系统调试与优化。
...
- **推荐理由：** AI编程导师，自动判题、Code Review、阶梯式学习
...
- **标签：** `agent`, `skills`, `dsh-better-sidebar`, `github-issues`
- **推荐理由：** 25个工程技能开箱即用，右侧面板直接调用
...
- **标签：** `ai-docs`, `gongwen`, `docx`
- **推荐理由：** 党政机关公文全流程处理，格式检查与模板生成
...
- **标签：** `spec-driven-development`, `vibe-coding`, `agent-skills`
...
- **标签：** `quality`, `guard`, `engineering`
- **推荐理由：** 交付前质量审查工程纪律套件，保障代码质量
...
- **⭐ 星标：** 31
- **语言：** Python
- **描述：** 让 AI 像资深测试工程师一样工作：面向 AI Agent 的测试工程 Skill 框架——11 Skills + 共享知识库 + 类���决策矩阵（Claude Code / dsh 等 Agent 可用）
- **标签：** `qa`, `testing`, `automation`
- **推荐理由：** 11个测试工程Skill框架，AI自动化测试利器
...
- **推荐理由：** 官方SkillHub API文档与示例，助力技能开发集成
...
- **标签：** `video`, `remotion`, `agent-skill`, `dsh-plugin`
...
- **标签：** `math-modeling`, `agent`, `competition`, `dsh-plugin`
- **推荐理由：** 5角色数学建模Agent，含33篇模型库和论文模板
...
- **标签：** `visual`, `design`, `agent-team`
- **推荐理由：** 开源AI矢量设计工具支持并发智能体
...
- **推荐理由：** AI生成的可编辑设计系统，含模板市场和PPT工作室。
...
- **推荐理由：** 为纯文本LLM赋能多模态视觉能力，支持截图与UI分析。
...
- **推荐理由：** 集成DirectorX的AI视频/图像/音频创作工具包
...
- **标签：** `browser`, `automation`, `logged-in`
...
- **标签：** `browser-automation`, `playwright`, `mcp-server`
- **推荐理由：** 持久浏览器身份+指纹伪装，支持Android模拟
...
- **标签：** `content-discovery`, `social`, `bilibili`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站/小红书/抖音等
...
- **标签：** `preset`, `bootstrap`, `configuration`
- **推荐理由：** 两阶段配置预设，轻量启动后启用全工具集
...
- **标签：** `installer`, `switcher`, `multi-agent`
- **推荐理由：** 一键安装和切换多种AI编程工具的集合器
...
- **标签：** `radar`, `validation`, `ecosystem`
- **推荐理由：** 2.1万+插件发现和k8s运行时验证的生态雷达工具。
...
- **标签：** `llm-provider`, `oauth`, `typescript`
...
- **标签：** `cost-tracking`, `token-usage`, `typescript`
- **推荐理由：** 跟踪会话/每日成本、预算及多提供商余额
...
- **标签：** `llm-provider`, `commandcode`, `typescript`
- **推荐理由：** 添加Command Code模型提供商，支持实时目录与计划感知选择
...
- **标签：** `debugging`, `plugin-conflict`, `utility`
- **推荐理由：** 诊断并解决DSH插件与版本冲突问题
...
- **标签：** `ai`, `deepseek`, `mcp`, `rag`
- **推荐理由：** 全面的AI资源与开发者教程合集
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪、GPU管理
...
- **标签：** `awesome-list`, `plugin-manager`
- **推荐理由：** 每日自动抓取精选插件目录，按场景分类推荐
...
- **标签：** `awesome-list`, `developer-tools`, `typescript`
- **推荐理由：** DSH终极指南：快速入门、资源与精选插件
...
- **标签：** `plugin-directory`, `catalog`, `marketplace`
- **推荐理由：** 超万款DSH插件目录，支持搜索排名与免费API调用。
...
- **推荐理由：** Agent优先的DSH全面手册，涵盖运行时、插件与MCP。
...
- **标签：** `plugin-catalog`, `directory`, `marketplace`
- **推荐理由：** 1958个插件总目录，中英文搜索与STAR排序，找插件首选。
...
- **推荐理由：** Official auto-generated plugin catalog by cordis.run
...
- **推荐理由：** GitHub Actions每日自动爬取，双语目录
...
- **标签：** `marketplace`, `installation`
- **推荐理由：** DSH插件官方市场，一键安装浏览
...
- **推荐理由：** AI百科大全，覆盖模型/Agent/RAG/MCP等前沿资源
