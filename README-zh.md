...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 221,570 | Core DSH framework enabling everything as a plugin |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 95,780 | Open-source design plugin with local-first desktop |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 72,232 | Deploy multi-agent swarms and autonomous workflows |
...
- **推荐理由：** 基于前缀缓存稳定的终端编码Agent，专为R1打造。
...
- **标签：** `agent-orchestration`, `workspace`, `local-first`
- **推荐理由：** 跨设备统一Agent工作台与任务管理
...
- **标签：** `agent-os`, `llm-orchestration`, `evaluation`
- **推荐理由：** 自进化Agent操作系统评测门控演进循环
...
- **标签：** `agent-skills`, `deepseek`, `dsh-plugin`, `vision`, `ocr`
- **推荐理由：** 首个DSH视觉插件，为纯文本编码代理提供OCR和图像理解能力。
...
- **推荐理由：** 两阶段预设模板，先最小启动再加载标准工具，适合稳定Agent运行。
...
- **推荐理由：** DSH内置插件市场，支持浏览、搜索和一键安装插件的可视化工具。
...
- **标签：** `agent-sandbox`, `dsh-plugin`, `vfs`, `virtual-terminal`
- **推荐理由：** AI代理的虚拟终端与虚拟文件系统，提供隔离沙箱运行环境。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `claude-code`, `codex`
- **推荐理由：** 一键安装并切换Claude Code/Codex/DSH/Kimi Code等多个编码代理模型。
...
- **标签：** `dsh-plugin`, `agentic-runtime`, `vibe-coding`, `computer-use-agent`
- **推荐理由：** 高性能Rust Agent运行时搭配精美桌面应用，融合代码代理深度与协作体验。
...
- **推荐理由：** 提供Schema和解析器的DSH插件Hub CLI工具，支持编程式插件管理
...
- **标签：** `provider`, `model-selection`, `commandcode`, `harness`
- **推荐理由：** Command Code模型接入，支持智能选择与实时目录
...
- **推荐理由：** 将Superpowers编码技能移植到Reasonix
...
- **标签：** `coding`, `tdd`, `debugging`
- **推荐理由：** 为DSH适配Superpowers的TDD调试规划技能
...
- **标签：** `capability`, `management`, `security`
- **推荐理由：** 统一管理MCP/skill暴露级别与执行模式
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `coding-agent`
- **推荐理由：** AI编程导师，自动扫描项目并生成阶梯式教程
...
- **标签：** `coding`, `code-simplification`, `codex`
- **推荐理由：** 适配DSH的代码简化技能提升代码可读性
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git等页面，支持三方扩展注册。
...
- **标签：** `dsh-plugin`, `skills`, `cli`, `taskboard`
- **推荐理由：** 现代化可灵活嵌入的任务面板，支持Codex和DSH界面扩展。
...
- **标签：** `dsh-plugin`, `tui`, `terminal`, `claude-code`
- **推荐理由：** DSH官方收录的TUI插件，鲸鱼顶栏、实时状态、流式思考与双击回滚。
...
- **⭐ 星标：** 2,256
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小��鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `dsh-plugin`, `floating-widget`, `developer-tools`
- **推荐理由：** DSH右下角萌系小鱼娘浮动组件，实���显示账户余额，支持拖拽吸附。
...
- **标签：** `desktop-app`, `electron`, `local-first`
- **推荐理由：** 本地优先的DSH桌面工作空间，支持多模块管理。
...
- **推荐理由：** 开源Windows桌面客户端，内置SSH和远程访问。
...
- **标签：** `canvas`, `non-linear`, `conversation-workspace`
- **推荐理由：** 可视化非线性对话画布，支持分支探索会话。
...
- **推荐理由：** 为DeepSeek Harness引入电子表格、文档、演示文稿、画布和关系表等办公能力
...
- **标签：** `ios`, `mobile-app`, `native-client`, `websocket`
- **推荐理由：** DeepSeek Harness原生iOS客户端，通过WebSocket同步工作区、会话和Agent执行轨迹
...
- **标签：** `desktop`, `electron`, `multi-platform`, `client`
- **推荐理由：** 类CodePilot风格的DeepSeek Harness桌面客户端插件套件，跨macOS/Windows/Linux
...
- **标签：** `notes`, `smartisan`, `docker`, `ui`
- **推荐理由：** 复刻锤子便签，支持Docker私有化和DSH插件调用
...
- **标签：** `usage-tracking`, `balance`, `web`
- **推荐理由：** 持久底部状态栏显示模型余额与定价信息
...
- **⭐ 星标：** 11
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运维每天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI ���动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `devops`, `docker`, `ssh`
- **推荐理由：** 集成数据库、SSH、Docker的DevOps桌面中心。
...
- **推荐理由：** Electron桌面封装，集成侧边栏与ModLens。
...
- **推荐理由：** 将原始文档转化为RAG、Agent推理与自维护Wiki。
...
- **标签：** `agent-memory`, `memory-management`, `token-savings`
- **推荐理由：** 自演进混合检索记忆系统节省35%Token
...
- **标签：** `agent`, `dsh-plugin`, `memory`, `rag`
- **推荐理由：** Agent记忆管理套件，帮助Agent记住并持续优化自身知识库。
...
- **标签：** `dsh-plugin`, `memory`, `rag`, `milvus`, `semantic-search`
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持Markdown与混合语义搜索。
...
- **标签：** `rag-evaluation`, `skill-evaluation`, `prompt-engineering`
- **推荐理由：** RAG与Agent评估框架，证据驱动可观测
...
- **标签：** `agent-skills`, `skill-store`, `workflow`
- **推荐理由：** 最全技能商店自动分类更新直达安装
...
- **标签：** `dsh-plugin`, `agent-skills`, `workflow-automation`, `vibe-coding`
- **推荐理由：** 智能技能路由与工作流编排，提升奖励并降低Token消耗。
...
- **标签：** `dsh-plugin`, `agent-skills`, `cognitive-enhancement`, `global-workspace`
...
- **标签：** `video`, `agent`, `ffmpeg`
- **推荐理由：** 端到端自动化生成Vox风格解说视频
...
- **标签：** `agent-skills`, `skill-routing`, `vector-search`
- **推荐理由：** 将碎片化技能文件转为可检索的AI技能库。
...
- **标签：** `multimodal`, `video-analysis`, `ocr`
- **推荐理由：** 赋予AI视听能力，视频音频转可验证证据。
...
- **⭐ 星标：** 342
- **语言：** Python
- **描述：** Community plugin for DeepSeek Harness with novel, short-drama, game and video-recap workbenches. Not affiliated with DeepSeek.
- **标签：** `creative-writing`, `novel`, `short-drama`
- **推荐理由：** 小说、短剧、游戏创作工作台插件。
...
- **推荐理由：** SQLite驱动的原生任务看板，支持Agent认领/审核流程，无需iframe的干净Web UI
...
- **标签：** `math-modeling`, `competition`, `gitops`, `autonomous-agent`
- **推荐理由：** 全栈式自动数学建模竞赛Skill，支持国赛/美赛，含GitOps流水线与代码自证
...
- **标签：** `skills`, `agent`, `architecture`, `coding`
- **推荐理由：** 将经验转化为可复现的Agent执行工作流框架
...
- **标签：** `finance`, `mcp`, `multi-agent`, `stock`
- **推荐理由：** 基于tushare的金融分析多Agent系统，支持IM盯盘
...
- **标签：** `plugin-discovery`, `installation`, `verification`, `skill`
- **推荐理由：** 自动发现、安装并验证DSH插件的Skill工具
...
- **推荐理由：** 为纯文本LLM添加多模态视觉能力，支持截图UI分析
...
- **标签：** `skills`, `research`, `marketing`, `social-intelligence`
- **推荐理由：** 88个开源Skill，覆盖研究、营销和商业智能
...
- **标签：** `ai-infra`, `skills`, `infrastructure`, `optimization`
- **推荐理由：** AI基础设施开发专用Skill，支持编写优化调试
...
- **标签：** `ai-agent`, `chinese-traditional-wisdom`, `fengshui`, `iching`
- **推荐理由：** 融合八字风水易经于一身的本土确定性引擎技能包
...
- **标签：** `ai-roleplay`, `agent-skills`, `narrative`
- **推荐理由：** 沉浸式成人叙事技能含记忆与Yaml存档
...
- **标签：** `agent`, `skill`, `math-modeling`
- **推荐理由：** 5角色数学建模流水线，33模型库，竞赛实用
...
- **标签：** `skill`, `guard`, `quality-gate`
- **推荐理由：** 工程交付质量门禁，需求验证与测试保障
...
- **标签：** `skill`, `deepseek-harness`
- **推荐理由：** 腾讯官方SkillHub API插件，含文档示例
...
- **标签：** `agent-skills`, `video`, `remotion`
- **推荐理由：** 电影感产品视频Agent，152镜头配方卡
...
- **标签：** `agent-skill`, `dsh-plugin`, `skills`
- **推荐理由：** 跨平台DSH插件开发标准Agent Skill
...
- **标签：** `agent-tools`, `workflow`
- **推荐理由：** Agent任务前自动预检插件库，注入Top-K适用插件。
...
- **标签：** `mcp`, `webui`, `management`
- **推荐理由：** Web界面统一管理skills和MCP服务器
...
- **标签：** `mcp`, `config-sync`, `claude-code`, `codex`
- **推荐理由：** 一键配置多平台AI Agent的MCP和技能
...
- **标签：** `diagram-as-code`, `dsh-plugin`, `mermaid-alternative`
- **推荐理由：** 精美自包含架构与序列图生成工具。
...
- **标签：** `image-generation`, `prompts`, `dsh-plugin`
- **推荐理由：** 530+ GPT图片提示词与工业级模板库。
...
- **标签：** `ai-video`, `text-to-video`, `ai-image`, `directorx`
- **推荐理由：** AI视频图像音频创作技能集成可配置模型工具
...
- **标签：** `browser-extension`, `prompt-manager`, `dsh-plugin`
- **推荐理由：** 面向AI聊天平台的多浏览器增强套件与提示词管理器。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `content-discovery`, `privacy-first`
- **推荐理由：** 跨平台AI内容发现Agent，本地私有，支持B站/小红书/YouTube等多平台。
...
- **标签：** `ios`, `mobile-automation`, `webdriveragent`, `xcode`
- **推荐理由：** 在DSH对话中运行iOS模拟器或USB连接iPhone，提供22个自动化Agent工具
...
- **标签：** `chatgpt`, `oauth`
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内直接对话。
...
- **标签：** `infrastructure`, `plugin`, `radar`
- **推荐理由：** 自部署雷达发现15900+ DSH插件
...
- **标签：** `context`, `dashboard`, `dsh`
- **推荐理由：** 全面的上下文管理仪表盘与侧边栏工具
...
- **推荐理由：** OAuth登录，无需API Key即可使用ChatGPT/Claude/Grok订阅作为DSH LLM提供方
...
- **推荐理由：** 每日自动抓取并复核GitHub插件，按场景分类、热度排行，帮你快速找到真正有用的插件
...
- **推荐理由：** 实时跟踪会话/每日费用、预算限制和Token用量，支持多Provider费用统一管理
...
- **标签：** `awesome-list`, `guide`, `resource-curation`
- **推荐理由：** DeepSeek Harness终极指南：快速入门、精选插件推荐与实用工具资源汇总
...
- **推荐理由：** 学习DSH原理、插件市场和最佳实践的一站式资源入口
...
- **推荐理由：** 深度精选DSH生态插件、技能、MCP服务器、补丁、编排器和UI的权威列表
...
- **标签：** `plugin-store`, `marketplace`, `api`, `catalog`
- **推荐理由：** 收录11,000+插件的DSH插件商店，支持搜索、排行、安装命令和免费公开API
...
- **推荐理由：** P2P加密多端远程访问Harness，支持随时操作
...
- **标签：** `docker`, `container`, `multi-version`
- **推荐理由：** Docker容器运行多版本DSH隔离管理工具
...
- **标签：** `utility`, `infrastructure`, `cordis`
- **推荐理由：** 统一编排Runtime/Provider/Plugin/MCP能力
...
- **标签：** `cost-tracking`, `visualization`, `api-usage`
- **推荐理由：** 实时可视化Token用量与API余额监控。
...
- **标签：** `utilities`, `debugging`, `conflict-resolution`
- **推荐理由：** 诊断并解决DSH插件版本冲突问题。
...
- **标签：** `dsh`, `dsh-plugin`, `theme`
- **推荐理由：** 鲸鱼娘系列主题皮肤，个性化UI
...
- **标签：** `theme`, `ui`, `background`, `customization`
- **推荐理由：** 支持自定义背景与预设氛围的UI美化插件。
...
- **标签：** `ai`, `vibecoding`, `deepseek`
- **推荐理由：** 最全AI资源与Vibe Coding零基础教程合集
...
- **标签：** `harness-engineering`, `tutorial`
- **推荐理由：** Harness工程入门最佳教程从零到一
...
- **⭐ 星标：** 1,779
- **语言：** Python
- **描述：** last30days-cn 是一个 AI Agent 技能（Skill），能���自动搜索中国互联网 8 大主流平台最近 30 天的内容，综合分析后生成有据可查的研究报告。
...
- **标签：** `awesome-list`, `knowledge-base`, `encyclopedia`
- **推荐理由：** 一站式AI百科，覆盖技能、MCP���RAG全维度资源。
...
- **推荐理由：** 一站式科研工作台：LaTeX实时编译、arXiv文献管理、实验追踪、GPU服务器SSH编排
...
- **⭐ 星标：** 62
- **语言：** Python
- **描述：** 专利侵权分析系统 —— 输入专��公开号，产出竞品侵权分析报告；同时打包成 skill，可被任意 agent（dsh, codex, claudecode 等） 调用。
...
- **标签：** `kline`, `finance`, `analysis`, `dashboard`
- **推荐理由：** 交互式K线图插件支持技术指标和基本面分析
...
- **标签：** `awesome-list`, `plugin-registry`, `verified`
- **推荐理由：** 经验证的插件目录，附安装指导。
...
- **标签：** `finance`, `stock`, `quant`, `niche`
- **推荐理由：** 专注股票金融量化插件的垂直目录。
...
- **标签：** `catalog`, `plugin-marketplace`, `directory`, `awesome-list`
- **推荐理由：** DSH插件总目录，1958个仓库，支持搜索和筛选
...
- **标签：** `handbook`, `agent-runtime`, `documentation`, `multilingual`
- **推荐理由：** Agent优先的综合手册，覆盖运行时、插件和排查
...
- **标签：** `awesome-list`, `plugin`, `curated`, `resources`
- **推荐理由：** DSH插件精选合集和资源列表
...
- **推荐理由：** cordis.run官方精选插件目录，发现资源必备
...
- **标签：** `community`, `awesome-list`, `catalog`
- **推荐理由：** 每4小时自动同步的DSH插件聚合社区
...
- **标签：** `ai-curated`, `awesome-list`
- **推荐理由：** AI整理190+插件分12类，每小时自动更新。
...
- **推荐理由：** GitHub Actions每日自动爬取，保持目录新鲜。
...
- **语言：** Unknown
- **描述：** DSHA精选插件合集
- **标签：** `awesome-list`, `dsha`
- **推荐理由：** DSHA精选插件合集，方便用户快速选用。
...
- **标签：** `plugin-radar`, `awesome-list`
- **推荐理由：** 基于证据的插件雷达，附2Origin实验插件库。
...
- **标签：** `awesome-list`, `plugin-repository`
- **推荐理由：** 社区筛选的DSH插件库，附验证标记。
...
- **标签：** `awesome-list`, `dsh-plugin`, `auto-update`
- **推荐理由：** YAML数据驱动自动更新插件列表，支持双语生成。
