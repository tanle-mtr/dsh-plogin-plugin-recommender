...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 203,301 | Core DSH platform; everything is a plugin architec |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 92,596 | Open-source Figma alternative with local-first des |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,717 | Meta-harness for multi-agent swarms and autonomous |
...
- **标签：** `multi-agent`, `self-evolves`, `mcp`
- **推荐理由：** 带记忆的多模型多渠道超级AI助手
...
- **标签：** `preset`, `bootstrapping`, `standard`
- **推荐理由：** 两阶段锚定配置，确保Standard工具的可靠引导。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `model-switch`
- **推荐理由：** 一键切换Claude Code、Codex、DeepSeek等多种模型工具。
...
- **标签：** `coding-agent`, `desktop`, `ai-ide`
- **推荐理由：** Rust高性能代理运行时结合精致桌面AI编程应用。
...
- **⭐ 星标：** 1,165
- **语言：** TypeScript
- **描述：** The best DeepSeek Harness plugin for context insight and management, with context dashboard / browser and context command, for context statistics, composition, breakdown, evolution details, understanding how the context is made of, and how it evolves. 一站��� DeepSeek Harness 上下文可视化插件，Context 面板及浏览器与 Context 命令，透视上下文组成、演进、压缩、剪枝等事件与动作。
- **标签：** `context`, `dashboard`, `management`
- **推荐理由：** DSH上下文管理与洞察面板，提升上下文利用率。
...
- **标签：** `plugin`, `developer`, `agent-skill`
- **推荐理由：** 官方插件开发标准，必备技能
...
- **标签：** `codex`, `mcp`, `sync`
- **推荐理由：** OpenAI Codex与DSH双向同步桥，自动MCP镜像
...
- **标签：** `migration`, `claude-code`, `mcp`, `agent-migration`
- **推荐理由：** 一键迁移Claude Code配置到DSH，含Skills/命令/MCP等，支持Codex和OpenCode。
...
- **推荐理由：** 可移植Agent Skill，用于开发和审计DSH插件，含Bundle适配。
...
- **标签：** `mcp`, `hot-reload`, `skill-management`, `config`
- **推荐理由：** 热加载MCP和技能管理，粘贴JSON即连，无需重启配置。
...
- **标签：** `awesome-list`, `coding-agents`, `dsh-plugin`, `ui`, `skills`
- **推荐理由：** 涵盖模型、工具、UI和智能体循环的综合插件列表。
...
- **推荐理由：** 可扩展侧边栏底座，内置终端/Git/对话等页面功能。
...
- **标签：** `tui`, `ui`, `deepseek-harness`
- **推荐理由：** Claude Code风格TUI插件，鲸鱼顶栏/流式思考/上下文进度。
...
- **标签：** `desktop`, `tauri`, `dsh-desktop`
- **推荐理由：** 5MB轻量桌面端，预置插件零配置开箱即用。
...
- **标签：** `widget`, `desktop-pet`, `utility`
- **推荐理由：** 可爱小鲸鱼挂件，实时查看账户余额。
...
- **标签：** `genui`, `interactive-ui`, `charts`
- **推荐理由：** 在聊天回复中内联渲染图表、表单等交互UI组件。
...
- **标签：** `desktop-client`, `codepilot`, `cross-platform`
...
- **推荐理由：** 零配置Windows桌面GUI，支持SSH和移动端远程访问
...
- **推荐理由：** 基于Electron的桌面控制台，管理DSH插件、MCP和技能。
...
- **推荐理由：** 纯Lua编写的快速简洁优雅的Neovim插件管理器。
...
- **标签：** `desktop`, `tauri`, `rust`, `docker`, `ssh`
- **推荐理由：** 跨平台Tauri 2桌面应用，集成数据库/SSH/SFTP/Docker/AI于一体。
...
- **标签：** `memory`, `portable`, `markdown-native`
- **推荐理由：** 本地优先的可移植记忆层，跨AI Agent持久化存储。
...
- **标签：** `memory`, `self-evolving`, `token-optimization`
- **推荐理由：** 自演进记忆系统，混合检索可节省35%的token消耗。
...
- **标签：** `agent`, `memory`, `rag`
- **推荐理由：** 专为AI代理设计的记忆管理套件，支持RAG检索增强。
...
- **标签：** `memory`, `long-term-memory`, `ai-agent`
...
- **标签：** `memory`, `personal-ai`, `mcp`
- **推荐理由：** 个人AI代理本地记忆中枢，多代理共享上下文。
...
- **标签：** `rag`, `skill-corpus`, `vector-search`
- **推荐理由：** 将分散技能文件转为可检索的RAG知识库。
...
- **标签：** `workspace`, `agent-orchestration`, `multi-device`
- **推荐理由：** 跨设备统一管理Agent和工作流的聚合工作空间。
...
- **推荐理由：** 视觉桥接插件，将图片转为结构化JSON供文本Agent使用。
...
- **推荐理由：** 基于Anthropic J-space全局工作区研究的认知增强技能套件。
...
- **推荐理由：** SQLite任务管理，支持智能体认领审查和原生Web界面
...
- **推荐理由：** 全栈数学建模竞赛自动化技能，支持GitOps和代码自证
...
- **标签：** `skills`, `workflow`, `agent`
- **推荐理由：** 将经验转化为可重复执行的工作流技能
...
- **标签：** `vision`, `multimodal`, `screenshot`
- **推荐理由：** 为纯文本LLM添加多模态视觉能力用于截图和UI分析
...
- **标签：** `agent-skills`, `self-evolving`, `prompt-engineering`
- **推荐理由：** 可组合可审查的Agent工作流，支持自进化
...
- **标签：** `marketplace`, `webui`, `plugin-management`
...
- **推荐理由：** Superpowers技能移植至Reasonix，增强编程工作流。
...
- **标签：** `agent-framework`, `skills`, `package-manager`
- **推荐理由：** 42技能5CLI的AI代理工程包管理器
...
- **标签：** `skill-learning`, `automation`, `agent-skills`
...
- **推荐理由：** AI视频/图像/音频生成技能，支持多模型配置的创意工作流插件。
...
- **推荐理由：** 10个测试工程Skill，共享知识库+类型决策矩阵，AI Agent可复用。
...
- **推荐理由：** Godot引擎4.x全栈游戏开发Skill，运行时注册，AI辅助游戏开发。
...
- **标签：** `canvas`, `cordis`, `creative`, `mcp`
- **推荐理由：** WeShop原生Cordis插件，无限画布+无限创意AI技能协作。
...
- **标签：** `design`, `vector`, `agent-teams`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持多Agent并发协作。
...
- **标签：** `video-generation`, `ai-video`, `skill`
- **推荐理由：** 端到端自动化Vox风格拼贴解说视频生成Agent技能。
...
- **标签：** `ai-design`, `design`, `visual-editor`
- **推荐理由：** AI驱动的设计系统，内置PPT工作室和模板市场。
...
- **标签：** `image-generation`, `seedream`, `visual`
...
- **推荐理由：** 在对话中预览、检查和编辑OpenPencil设计稿
...
- **标签：** `ai-agent`, `content-discovery`, `bilibili`
- **推荐理由：** 本地优先的跨平台AI内容发现Agent，覆盖B站/抖音/YouTube等。
...
- **标签：** `ios`, `mobile-automation`, `ui-testing`
- **推荐理由：** 内置iOS模拟器与USB真机自动化测试能力。
...
- **推荐理由：** 官方Tabbit浏览器插件，支持Playwright自动化。
...
- **标签：** `ai-agent`, `dingtalk`, `dsh-plugin`, `chat-bot`
- **推荐理由：** 将智能体工具带入聊天机器人的钉钉IM渠道。
...
- **标签：** `self-hosted`, `resume-builder`, `mcp-server`
- **推荐理由：** 隐私优先、可自部署的简历构建工具
...
- **标签：** `cost-tracking`, `budget`, `tokens`
- **推荐理由：** 追踪会话/每日成本、预算和token使用情况
...
- **标签：** `session-migration`, `preset-migration`, `context`
- **推荐理由：** 跨预设会话迁移，保持状态和意图完整
...
- **推荐理由：** 通过WebDAV备份、恢复、迁移和同步完整DSH配置。
...
- **标签：** `cost`, `token-usage`, `visualization`, `dashboard`
- **推荐理由：** 实时Token用量与成本图表，支持API余额监控和数据可视化。
...
- **标签：** `plugin`, `conflict-resolution`
- **推荐理由：** 诊断并解决插件间及版本间的兼容冲突
...
- **标签：** `pets`, `animation`, `cli-theme`
- **推荐理由：** 公共动画宠物画廊，为CLI Agent增添个性和视觉魅力。
...
- **推荐理由：** 鲸鱼娘系列主题皮肤，专为DeepSeek Harness设计。
...
- **标签：** `theme`, `ui`, `glass-morphism`
- **推荐理由：** 高度可定制的磨砂玻璃主题，实时调节UI。
...
- **标签：** `theme`, `skin`, `threejs`, `webgpu`
- **推荐理由：** 基于WebGPU的海洋主题视觉皮肤Chrome扩展
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己���图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `theme`, `webui`, `background`
- **推荐理由：** 自定义DSH Web背景，支持预设与精细调节
...
- **标签：** `ai-guide`, `resources`, `llm`
- **推荐理由：** 全面的AI资源库，涵盖模型、提示词和RAG知识。
...
- **标签：** `research`, `skill`, `web-search`
- **推荐理由：** 自动搜索国内8大平台近30天内容并生成有据可查的报告。
...
- **推荐理由：** 自动发现并验证15900+ DSH插件，生态雷达工具。
...
- **标签：** `awesome-list`, `reference`, `ecosystem`
- **推荐理由：** DSH生态工具与基础设施的精选参考列表。
...
- **标签：** `ai-pedia`, `encyclopedia`, `research`
- **推荐理由：** AI全维度资源百科，覆盖Agent、RAG与学习路线。
...
- **标签：** `marketplace`, `registry`, `directory`
- **推荐理由：** 11000+插件市场，支持搜索、排名和安装命令
...
- **标签：** `marketplace`, `plugins`, `discovery`
- **推荐理由：** DeepSeek Harness生态的插件市场和发现工具
...
- **推荐理由：** 在编码Agent内直接搜索65,600+中国开放平台API文档。
...
- **标签：** `documentation`, `knowledge-base`
- **推荐理由：** 官方文档知识库，中英双语
...
- **标签：** `industry-research`, `report`, `policy`
- **推荐理由：** 行业研究方法论、产业链映射与政策追踪包
...
- **推荐理由：** 通过GitHub Actions自动爬取的双语DSH插件目录。
...
- **标签：** `ai-agents`, `awesome-list`, `mcp`, `dsh-plugin`
- **推荐理由：** AI策展的190+插件每小时更新列表。
...
- **标签：** `awesome-list`, `dsh-plugin`, `auto-updating`
- **推荐理由：** YAML驱动的自动更新双语DSH插件列表。
...
- **标签：** `dsh-plugin`, `toy`, `web-simulation`
- **推荐理由：** 零依赖单文件Web版竹知了玩具，真实录音采样，移动端优先。
...
- **标签：** `awesome-list`, `plugins`, `agents`
- **推荐理由：** 精选DSH插件与开发资源列表，快速发现优质工具。
...
- **推荐理由：** cordis.run官方自动生成的插件目录，持续同步更新。
...
- **标签：** `plugin-marketplace`, `awesome-deepseek-harness`
...
- **推荐理由：** GitHub OAuth零权限登录，DSH社区合影活动墙，有趣实用。
...
- **标签：** `awesome-list`, `nextjs`, `directory`, `typescript`
- **推荐理由：** 公开GitHub插件目录，含安装命令和生态发现功能。
...
- **推荐理由：** 社区维护的DSH插件目录，非官方但全面的插件检索资源。
...
- **标签：** `registry`, `awesome-list`, `open-data`, `official`
- **推荐理由：** 去垃圾信息的官方DSH插件/Bundle/Skill注册表，数据开放。
