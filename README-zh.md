...
- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 27 个插件
- [UI 与桌面扩展](#ui-与桌面扩展) — 18 个插件
- [记忆与知识（RAG）](#记忆与知识（rag）) — 9 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 42 个插件
- [视觉与设计](#视觉与设计) — 11 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (27)
- [UI 与桌面扩展](ui-desktop-extensions-zh) (18)
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (9)
- [Agent 技能与工作流](agent-skills-workflows-zh) (42)
...
- **标签：** `multi-agent`, `mcp`, `dsh-plugin`
- **推荐理由：** 自进化多模型智能体平台，支持记忆
...
- **标签：** `dsh-plugin`, `harness-engineering`, `tutorial`
- **推荐理由：** 从零开始的DSH插件开发入门教程，适合新手。
...
- **标签：** `agent-os`, `loop-engineering`, `evaluation`
...
- **标签：** `agent-teams`, `dsh-plugin`, `vibe-coding`
...
- **标签：** `agent-skills`, `code-review`, `dsh-plugin`
...
- **标签：** `deepseek-harness`, `macos`, `swift`, `coding-agent`
- **推荐理由：** DeepSeek官方Swift原生macOS编码Agent
...
- **标签：** `coding-agent`, `team`, `dsh-plugin`
- **推荐理由：** 将多个编程 Agent 整合为一个协作团队
...
- **标签：** `plugin-development`, `knowledge-base`, `dsh-plugin`
- **推荐理由：** 官方DSH插件开发知识库，按需调用的Agent技能
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** Codex与DSH双向同步桥，实时技能与MCP镜像
...
- **标签：** `dsh-plugin`, `plugin-development`, `agent-skills`
- **推荐理由：** 在DSH内完成插件脚手架与测试的Agent技能
...
- **标签：** `publisher`, `dsh-plugin`, `automation`
- **推荐理由：** 一键发布插件，自动校验生成文档并提交PR。
...
- **标签：** `web-ui`, `gui`, `skins`
- **推荐理由：** DSH Web GUI，含任务看板、SSH运维与优化皮肤。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `sidebar`
- **推荐理由：** 可扩展侧边栏，内置终端/编辑器/Git等功能
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `tui`
- **推荐理由：** Claude Code风格TUI，实时状态与流式思考
...
- **标签：** `desktop-pet`, `electron`, `mcp`, `plugin-sdk`
- **推荐理由：** 桌面宠物伴侣平台，支持插件SDK和Agent集成
...
- **标签：** `widget`, `floating`, `utility`
- **推荐理由：** 可爱鲸鱼挂件，实时监控账户余额
...
- **标签：** `desktop`, `electron`, `local-first`, `office-automation`
- **推荐理由：** 本地优先的桌面工作空间，集成多项目管理
...
- **推荐理由：** 本地优先的多模型 AI Agent 桌面应用替代方案
...
- **推荐理由：** 跨平台 CodePilot 风格 DSH 桌面客户端
...
- **推荐理由：** 开源Electron桌面客户端，支持SSH和Codex
...
- **推荐理由：** Electron桌面控制台，多标签管理Harness/插件/MCP
...
- **标签：** `deepseek-harness`, `dsh`, `desktop-app`
...
- **标签：** `dsh-plugin`, `usage-tracking`, `ui`
- **推荐理由：** 底部常驻状态栏，显示模型/余额/峰谷定价
...
- **推荐理由：** 基于Electron的DSH桌面版，集成侧边栏和多种插件
...
- **标签：** `dsh-plugin`, `memory`, `rag`
- **推荐理由：** 为OpenClaw提供无限记忆能力
...
- **标签：** `memory`, `mcp`, `long-term-memory`
- **推荐理由：** 持久可查的长期记忆与召回工具。
...
- **标签：** `skill`, `retrieval`, `rag`
- **推荐理由：** 将SKILL.md组织为可检索技能库。
...
- **标签：** `memory`, `powercontext`, `handoff`
- **推荐理由：** 连接 PowerContext 实现记忆和经验召回
...
- **推荐理由：** 桥接 Claude Code 记忆和技能到 DSH
...
- **标签：** `agent-skills`, `skill-generator`, `dsh-plugin`
- **推荐理由：** 将专家思维提炼为可复用智能体技能
...
- **标签：** `image-generation`, `skills`, `prompt-engineering`
- **推荐理由：** 470+逆向提示词模板库，提炼为可复用技能。
...
- **标签：** `agent-skills`, `ai-agent`, `dsh-plugin`
- **推荐理由：** 基于全球工作空间研究的认知增强技能
...
- **标签：** `dsh-plugin`, `video-generation`, `agent`
- **推荐理由：** 一键生成Vox风格解说视频，自动化全流程
...
- **标签：** `agent-skills`, `coding-agents`, `architecture`
- **推荐理由：** 架构驱动编程，基准优先的安全Agent方案
...
- **⭐ 星标：** 1,097
- **语言：** Python
- **描述：** 为纯文本模型"看图“设计更好的视觉工具箱和���能，支持多图理解，图片问答，前端UI还原、GUI 自动化等，并可选无缝接入多个主流agent，直接识别粘贴图片｜ A vision toolkit and skill designed for text-only llms — image Q&A, long-screenshot OCR, frontend UI restoration, and GUI automation, with optional seamless integration for Codex, Claude Code, Pi, Oh My Pi, and OpenCode
- **标签：** `agent-skills`, `vision`, `multimodal`, `computer-use`
- **推荐理由：** 让纯文本模型拥有视觉能力的工具箱
...
- **标签：** `browser-extension`, `ai-workflow`, `productivity`
...
- **标签：** `agent-teams`, `multi-agent`, `dsh`
- **推荐理由：** 多Agent团队协调，提升协作效率
...
- **标签：** `vision-tools`, `ocr`, `gui-automation`
- **推荐理由：** DSH原生视觉插件，支持OCR和UI还原
...
- **标签：** `creative-writing`, `drama`, `skill`
- **推荐理由：** 小说创作与短剧制作工作流技能
...
- **标签：** `notes`, `productivity`, `utility`
- **推荐理由：** 复刻锤子便签，支持Docker私有化与公众号导出
...
- **标签：** `task-management`, `ui`
- **推荐理由：** 本地任务看板，SQLite驱动项目管理。
...
- **推荐理由：** AI编程导师，扫描架构、生成章节、自动判题和代码审查
...
- **推荐理由：** 88个Agent技能，覆盖研究、社交情报和营销自动化
...
- **标签：** `skill-picker`, `ui`
- **推荐理由：** WorkBuddy风格技能选择器，composer内快速插入
...
- **标签：** `chinese-wisdom`, `dashboard`, `bazi`
- **推荐理由：** 八字风水奇门等中华传统智慧融合，本地确定性引擎
...
- **标签：** `agent-framework`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 42项技能+多CLI，AI工程工作流一站式包
...
- **标签：** `gongwen`, `docx`, `ai-docs`, `dsh-plugin`
- **推荐理由：** 符合国标党政机关公文格式全流程处理工具
...
- **标签：** `agent-skills`, `deepseek-harness`, `engineering`
- **推荐理由：** 深度提炼的DSH工程纪律与决策方法论
...
- **标签：** `godot`, `gdscript`, `game-development`
- **推荐理由：** Godot 4.x 全栈游戏开发技能插件
...
- **推荐理由：** 录制 macOS 桌面操作并转为 agent 技能
...
- **标签：** `agent-tools`, `plugin-injection`
- **推荐理由：** 任务前自动预检并注入适用插件，提升利用率
...
- **推荐理由：** 通过OAuth接入ChatGPT订阅账号在DSH内使用
...
- **标签：** `architecture-diagram`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 生成精美架构图的智能体技能
...
- **标签：** `design`, `ppt`, `visual-editor`, `dsh-plugin`
- **推荐理由：** AI 驱动的设计工作室，支持 PPT 与可视化编辑
...
- **标签：** `image-generation`, `visual`, `ai`
- **推荐理由：** 在DSH对话中直接生成图片，支持多AI提供商
...
- **⭐ 星标：** 40
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，或��接读案件材料画准一张时间轴。Claude Skill / DeepSeek Harness 通用。
- **标签：** `legal-tech`, `visualization`, `diagram`, `svg`
- **推荐理由：** 法律诉讼可视化工具，生成时间轴和SVG图表
...
- **标签：** `directorx`, `ai-video`, `ai-image`, `dsh-plugin`
- **推荐理由：** AI视频/图像/音频生成，支持分镜与配置化模型
...
- **标签：** `canvas`, `agent-skills`, `mcp`, `weshop`
- **推荐理由：** 原生WeShop无限画布插件，融合创意技能
...
- **标签：** `hud`, `status-panel`, `git`
- **推荐理由：** 浮动 HUD 面板显示 git/MCP/技能及用量
...
- **标签：** `ai-agent`, `bilibili`, `dsh-plugin`
- **推荐理由：** 本地优先的跨平台AI内容发现Agent
...
- **标签：** `mobile`, `ios`, `automation`, `dsh-plugin`
- **推荐理由：** 在 DSH 中内嵌 iOS 模拟器与真机自动化控制
...
- **标签：** `self-hosted`, `mcp-server`, `dsh-plugin`
- **推荐理由：** 隐私优先的开源简历构建工具
...
- **标签：** `preset`, `bootstrap`, `standard`
- **推荐理由：** 两阶段最小对齐引导，快速搭建标准工具链。
...
- **标签：** `vfs`, `virtual-filesystem`, `sandbox`
- **推荐理由：** 统一虚拟文件系统，赋予Agent持久化文件访问。
...
- **标签：** `deepseek-harness`, `context`, `dashboard`
- **推荐理由：** 上下文仪表盘与统计管理工具
...
- **标签：** `utility`, `oauth`, `provider`, `dsh-plugin`
- **推荐理由：** 通过 OAuth 使用订阅服务作为 LLM 提供商
...
- **标签：** `web-search`, `utility`, `dsh-plugin`
- **推荐理由：** 免费无注册的通用 Web 搜索桥接插件
...
- **标签：** `cost`, `tracking`, `utility`
- **推荐理由：** 会话成本与token用量追踪，支持多Provider
...
- **标签：** `console`, `plugin-management`, `ui`, `dsh-repository-plugin`
- **推荐理由：** DSH插件生态基建，浏览器面板管理+开发引导
...
- **标签：** `image-reading`, `ocr`, `vision`, `dsh-plugin`
- **推荐理由：** 纯文本模型的图像OCR和读取工具
...
- **标签：** `desktop`, `ssh`, `docker`, `database`
- **推荐理由：** 一体化桌面应用：SSH/数据库/Docker+AI
...
- **标签：** `utility`, `plugin-verify`
- **推荐理由：** 检测并解决插件间及版本间的冲突问题
...
- **标签：** `registry`, `plugin-manager`, `dsh-plugin`
- **推荐理由：** 整合多源发现并去重的中心化插件注册表。
...
- **标签：** `pet`, `pixel-art`, `mascot`
- **推荐理由：** 多平台支持的像素风动画宠物画廊。
...
- **⭐ 星标：** 371
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶��、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `ui`, `dsh-plugin`
- **推荐理由：** 磨砂玻璃主题，支持高度自定义界面风格
...
- **推荐理由：** WebGPU海洋主题皮肤，Three.js渲染视觉效果出色
...
- **标签：** `theme`, `skins`, `dark-mode`, `opencode`
- **推荐理由：** 34款经典opencode配色一键切换，重启不丢
...
- **⭐ 星标：** 1,605
- **语言：** Python
- **描述：** last30days-cn 是一个 AI Agent 技能（Skill），能够自动搜索中国互联网 8 大主流平台最近 30 ��的内容，综合分析后生成有据可查的研究报告。
- **标签：** `dsh-plugin`, `research`, `web-search`
- **推荐理由：** 自动检索8大中文平台生成研究报告
...
- **标签：** `awesome-list`, `plugin`, `dsh-plugin`
- **推荐理由：** 双语精选插件与工具参考目录
...
- **标签：** `knowledge-base`, `skills`, `dsh-plugin`
- **推荐理由：** 全维度 AI 资源百科与技能知识库
...
- **标签：** `guide`, `dsh-plugin`, `learning`
- **推荐理由：** DSH 原理学习与插件最佳实践指南
...
- **标签：** `guide`, `awesome-list`, `dsh-plugin`
- **推荐理由：** 终极双语入门指南与精选插件推荐
...
- **标签：** `marketplace`, `catalog`, `awesome-list`
- **推荐理由：** 3100+插件市场，支持搜索排名和API调用
...
- **标签：** `dsh-plugin`, `kline`, `financial-analysis`
- **推荐理由：** 多市场行情K线分析，支持技术指标与新闻基本面
...
- **推荐理由：** GitHub Actions每日自动更新的双语插件目录。
...
- **标签：** `plugin-directory`, `registry`, `dsh-plugin`
- **推荐理由：** 覆盖模型、工具、UI和智能体循环的综合插件目录。
...
- **标签：** `industry`, `dsh-plugin`, `directory`
- **推荐理由：** 按行业分类（金融/法律/医疗等）的精选插件目录。
...
- **标签：** `cordis`, `awesome-list`, `dsh-plugin`
- **推荐理由：** Cordis插件的官方自动生成精选列表。
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `dsh`
- **推荐理由：** DSH插件生态精选列表，社区宝藏资源
...
- **标签：** `awesome-list`, `deepseek-harness`, `tools`
- **推荐理由：** DSH生态工具与基础设施精选资源
...
- **标签：** `plugins`, `marketplace`
- **推荐理由：** DeepSeek Harness插件合集市场
...
- **标签：** `awesome-list`, `deepseek-harness`, `plugin-marketplace`
- **推荐理由：** cordis.run官方精选的自动生成插件目���
...
- **标签：** `community`, `photo`, `oauth`
- **推荐理由：** GitHub OAuth合影墙，DSH内测社区纪念
...
- **推荐理由：** 2026年度DSH精选插件列表，附质量检查便于发现
...
- **标签：** `awesome-list`, `verified-plugins`
- **推荐理由：** 按星标排序的已验证社区插件目录
...
- **标签：** `plugin-portal`, `web`
- **推荐理由：** 静态网站门户，发现筛选安装227+插件
...
- **标签：** `ai-recommender`, `awesome-list`
- **推荐理由：** AI推荐每小时更新的190+插件目录分12类
...
- **标签：** `plugin-repository`, `dsh-ecosystem`
- **推荐理由：** DSH插件仓库索引，供生态参考
