...
- [Agent 技能与工作流](#agent-技能与工作流) — 30 个插件
- [MCP 服务器与工具](#mcp-服务器与工具) — 1 个插件
- [视觉与设计](#视觉与设计) — 10 个插件
- [浏览器与 Web 自动化](#浏览器与-web-自动化) — 9 个插件
- [通信与社交](#通信与社交) — 1 个插件
...
- **标签：** `multi-agent`, `skills`, `mcp`
- **推荐理由：** 多模多通道超级AI助手，支持自进化记忆。
...
- **推荐理由：** 一键切换Claude Code、DeepSeek、Kimi等多种AI编程工具。
...
- **推荐理由：** 鲸鱼顶栏TUI，支持流式思考、双击回滚和实时TPS显示。
...
- **标签：** `marketplace`, `plugin`, `dsh-plugin`
- **推荐理由：** DSH官方插件市场，浏览搜索一键安装插件。
...
- **标签：** `coding-agent`, `rust`, `dsh-plugin`
- **推荐理由：** Rust高性能Agent运行时，搭配精美桌面应用的编程代理工具。
...
- **标签：** `code-review`, `static-analysis`, `dsh-plugin`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，6种分析模式。
...
- **标签：** `awesome-list`, `plugin-registry`, `dsh-plugin`
- **推荐理由：** 插件雷达精选榜，扫描9000+候选并实测验证。
...
- **标签：** `coding-agents`, `agent-skills`
- **推荐理由：** 架构感知编码代理，验证代码漂移确保安全开发。
...
- **标签：** `coding-agent`, `macos`
- **推荐理由：** DeepSeek官方Swift原生macOS编码代理，性能优秀。
...
- **标签：** `agent`, `coding-agent`, `dsh-plugin`
- **推荐理由：** 将多个代码Agent组建成协作团队，统一调度
...
- **标签：** `compatibility`, `migration`, `pi-agent`, `bridge`
- **推荐理由：** 打通Pi与DSH生态，兼容层无缝迁移插件。
...
- **标签：** `codex`, `sync`, `mcp`, `deepseek-harness`
- **推荐理由：** Codex与DSH双向同步，自动MCP镜像，开发者利器
...
- **标签：** `plugin-dev`, `agent-skills`, `cordis`
- **推荐理由：** 便携式插件开发与审计Agent Skill，含DSH包适配器。
...
- **标签：** `plugin-dev`, `best-practices`, `documentation`
- **推荐理由：** 插件开发踩坑指南，含cordis与构建最佳实践。
...
- **标签：** `plugin-dev`, `scaffolding`, `testing`
- **推荐理由：** 在Harness内完成插件脚手架搭建与测试。
...
- **标签：** `mcp`, `menu`, `capability`
- **推荐理由：** 统一管理MCP工具暴露级别和执行模式的菜单
...
- **标签：** `agent-tools`, `pre-flight-check`
- **推荐理由：** 任务型预检插件库并注入Top-K推荐插件
...
- **标签：** `design`, `desktop-app`, `figma-alternative`
- **推荐理由：** 本地优先设计插件，让编码代理变身设计工具。
...
- **标签：** `workspace`, `orchestration`, `multi-device`
- **推荐理由：** 统一代理工作区，跨设备看板任务跟踪
...
- **标签：** `web-ui`, `themes`, `skin`
- **推荐理由：** Web GUI与WebGL优化主题皮肤市场
...
- **标签：** `sidebar`, `ui-extension`, `dsh-plugin`
- **推荐理由：** 开放侧边栏底座，内置文件编辑、终端、对话、Git等功能。
...
- **标签：** `widget`, `floating-widget`
- **推荐理由：** 可爱的小鲸鱼浮动组件，实时显示账户余额。
...
- **标签：** `desktop-app`, `agent`, `privacy`, `dsh-plugin`
- **推荐理由：** 本地优先多模型桌面Agent，隐私优先的替代方案
...
- **标签：** `ui`, `dsh-plugin`, `interactive`
- **推荐理由：** 在对话中直接渲染交互式UI组件：图表、表单、测验
...
- **推荐理由：** 类CodePilot跨平台桌面客户端，自带插件套件
...
- **标签：** `ui`, `canvas`, `workspace`, `conversation`
- **推荐理由：** 可视化非线性对话画布，支持分支探索。
...
- **推荐理由：** 功能丰富的Windows桌面客户端，含SSH与远程访问。
...
- **标签：** `marketplace`, `plugin-market`, `javascript`
- **推荐理由：** 在DSH Web GUI中一键浏览安装更新所有插件
...
- **标签：** `task-management`, `taskboard`, `agent`
- **推荐理由：** 原生SQLite任务看板，Agent认领审查，无需iframe
...
- **标签：** `webui`, `plugin-market`, `ui`
- **推荐理由：** DSH Web GUI社区插件市场，一键安装卸载
...
- **标签：** `ui`, `status-bar`, `usage-tracking`
- **推荐理由：** 底部状态栏实时显示提供商、余额与定价信息。
...
- **标签：** `hud`, `status`, `ui`
- **推荐理由：** 悬浮HUD面板展示git状态、MCP服务器和Token用量
...
- **标签：** `desktop`, `tauri`, `ssh`, `docker`
- **推荐理由：** Tauri跨平台桌面应用整合数据库/SSH/Docker与AI
...
- **推荐理由：** Electron桌面壳集成DSH Web服务与多插件
...
- **标签：** `memory`, `rag`, `token-optimization`
- **推荐理由：** Self-evolving memory OS with 35% token savings via hybrid retrieval
...
- **标签：** `memory`, `dsh-plugin`
- **推荐理由：** 为AI提供无限记忆，支持长会话持续学习。
...
- **推荐理由：** 涵盖Agent、MCP、RAG的全方位AI知识库百科
...
- **标签：** `notes`, `smartisan`, `docker`, `private`
- **推荐理由：** 锤子便签开源版，支持私有部署与公众号导出。
...
- **标签：** `embeddings`, `semantic-search`, `reranking`, `vector-search`
- **推荐理由：** 将分散技能文件转为可检索语料库，含评估工具
...
- **标签：** `agent-handoff`, `session-memory`, `context-engineering`
- **推荐理由：** 跨平台会话记忆，确保Agent接力时状态不丢失
...
- **标签：** `memory`, `powercontext`, `recall`
- **推荐理由：** 连接PowerContext服务实现持久记忆和技能记忆
...
- **标签：** `multi-agent`, `swarm`, `workflow`
- **推荐理由：** 原生多智能体工作流编排与群体智能框架。
...
- **⭐ 星标：** 23,854
- **语言：** Python
- **描述：** Distilly — Distill how they think into reusable Skills for any Agent or Bot. Formerly Colleague Skill / colleague-skill（原同事 Skill）.
- **标签：** `agent-skills`, `skill-generator`, `knowledge-distillation`
- **推荐理由：** 将专家思维蒸馏为可复用技能，赋能任意AI代理。
...
- **⭐ 星标：** 12,594
- **语言：** JavaScript
- **描述：** Prompt as Code | GPT-Image2 工业级提示词引擎与模板库，470+ 个案例逆向工程，20+ 套工业级模板，并���炼出Skills，持续更新中
- **标签：** `image-generation`, `skills`, `prompts`
- **推荐理由：** 470+图片提示词与可直接使用的Skills工作流
...
- **标签：** `agent-os`, `evaluation`, `loop-engineering`
- **推荐理由：** 自进化Agent OS，自动化评估与循环工程
...
- **标签：** `enterprise`, `workspace`, `multi-agent`
- **推荐理由：** 企业级多引擎可视化Agent工作台
...
- **推荐理由：** 基于Anthropic J-space全局工作空间研究的AI增强技能套件。
...
- **推荐理由：** 自动路由和编排Harness工作流与技能的通用Agent技能。
...
- **标签：** `task-manager`, `agentic-workflow`
- **推荐理由：** 自托管任务管理器，支持人机协同的实时Agent控制。
...
- **标签：** `skills`, `agent`, `workflow`, `reusable`
- **推荐理由：** 将经验转化为可重复执行的工作流。
...
- **标签：** `vision`, `multimodal`, `skill`, `ui-analysis`
- **推荐理由：** 为纯文本LLM添加视觉能力，支持截图分析。
...
- **标签：** `plugin-discovery`, `skill`, `automation`
- **推荐理由：** 自动搜索安装验证GitHub插件的实用技能。
...
- **标签：** `creative-writing`, `novel`, `drama`, `skill`
- **推荐理由：** 小说创作与短剧制作专用AI写作技能。
...
- **推荐理由：** 适配DSH的Superpowers技能：TDD、调试、规划协作
...
- **标签：** `code-simplification`, `dsh-inspired`, `codex-skill`
...
- **标签：** `skill-picker`, `dsh-plugin`, `workflow`
- **推荐理由：** WorkBuddy风格技能选择器，快速插入DSH技能命令
...
- **标签：** `chinese-traditional-wisdom`, `bazi`, `fengshui`, `dashboard`
- **推荐理由：** 独特八字/风水/易经技能包，含可视化Dashboard
...
- **标签：** `multi-agent`, `team-collaboration`, `shared-workspace`
- **推荐理由：** 多Agent协作，独立模型/技能/上下文，共享工作空间
...
- **标签：** `image-generation`, `vision`, `media-skills`
- **推荐理由：** 免费图像读取/生成，支持多视觉模型集成
...
- **标签：** `swift-cycle`, `governance`, `version-pinned`
- **推荐理由：** Swift Cycle治理技能，用户触发，版本锁定可离线验证
...
- **标签：** `gongwen`, `docx`, `agent-skills`
- **推荐理由：** 国标公文全流程处理，含格式检查与内容优化。
...
- **标签：** `engineering`, `skill`, `best-practices`
- **推荐理由：** 提炼工程纪律与事实查证的最佳实践Skill。
...
- **标签：** `canvas`, `agent-skills`, `agentic-workflow`
- **推荐理由：** 无限画布结合创意技能，支持智能体工作流。
...
- **标签：** `record-replay`, `skills`, `macos`
- **推荐理由：** 录制macOS操作并自动转换为可复用的Agent技能
...
- **标签：** `math`, `skills`, `modeling`
- **推荐理由：** 数学建模增强套件，含17张手工技能卡片
...
- **标签：** `mcp`, `skills`, `web-ui`
- **推荐理由：** Web界面统一管管理技能与MCP工具
...
- **标签：** `design`, `vector`, `ui-generation`
...
- **标签：** `video-generation`, `ffmpeg`, `dsh-plugin`
- **推荐理由：** 端到端自动生成Vox风格拼贴解说视频。
...
- **标签：** `vision`, `agent-skills`
- **推荐理由：** 让纯文本模型具备视觉能力，支持多图理解与GUI自动化。
...
- **标签：** `vision`, `dsh-plugin`
- **推荐理由：** DSH原生视觉工具箱，支持OCR和前端UI还原。
...
- **标签：** `image-generation`, `visual`, `seedream`, `gemini`
- **推荐理由：** 在对话中直接生成AI图像，便捷高效。
...
- **标签：** `deepseek-harness`, `design`, `ui-design`, `ppt`
- **推荐理由：** 在对话中直接预览和编辑OpenPencil设计文档
...
- **标签：** `design`, `vibe-coding`, `ui-styles`, `web-design`
- **推荐理由：** 6种设计风格预设，避免AI生成代码的千篇一律
...
- **标签：** `ai-video`, `ai-image`, `visual`
- **推荐理由：** 集成AI视频/图像/音频生成与可配置视觉工具。
...
- **标签：** `bilibili`, `ai-agent`, `dsh-plugin`
- **推荐理由：** 本地优先的跨平台AI内容发现Agent，支持B站、YouTube等。
...
- **标签：** `browser-automation`, `cli`, `dsh-plugin`
- **推荐理由：** 让AI代理使用你的真实登录浏览器进行跨平台自动化。
...
- **标签：** `browser-extension`, `productivity`
- **推荐理由：** 浏览器扩展，将AI对话转化为可复用工作流。
...
- **标签：** `browser-automation`, `chrome-extension`, `dsh-plugin`
- **推荐理由：** Chrome侧边栏直控浏览器，无需视觉能力
...
- **标签：** `web-search`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 最强免费网页搜索桥接，零注册全模型可用
...
- **标签：** `ios`, `automation`, `mcp`, `dsh-plugin`
- **推荐理由：** 对话内实时控制iOS模拟器与真机，22个自动化工具
...
- **推荐理由：** Tabbit浏览器插件，Playwright驱动的浏览器自动化
...
- **标签：** `routing`, `infrastructure`, `cordis`
- **推荐理由：** 任务感知推理路由，核心路由基础设施
...
- **标签：** `vfs`, `sandbox`, `infrastructure`
- **推荐理由：** 统一虚拟文件系统，跨平台代理文件访问
...
- **标签：** `dsh-plugin`, `web-sim`
- **推荐理由：** 零依赖Web玩具模拟，真实录音采样，移动端优先。
...
- **标签：** `oauth`, `llm`, `dsh-plugin`, `utility`
- **推荐理由：** OAuth方式复用ChatGPT/Claude/Grok订阅作为DSH模型源
...
- **标签：** `cost-tracking`, `utility`, `tokens`, `budget`
- **推荐理由：** 实时追踪对话成本与Token用量，预算管理必备。
...
- **标签：** `backup`, `crash-recovery`, `rollback`, `snapshot`
- **推荐理由：** 崩溃恢复与撤销，安全快照，一键安全模式
...
- **标签：** `context-migration`, `preset-migration`, `session-migration`
- **推荐理由：** 跨预设会话迁移，保留状态和意图
...
- **标签：** `context-audit`, `token-optimization`, `ui`
- **推荐理由：** 审计上下文Token成本，检测重复与冲突，Web UI面板
...
- **标签：** `skills-manager`, `dsh-plugin`, `management`
- **推荐理由：** DSH技能管理专用插件，便捷管理所有Skills
...
- **标签：** `mcp-manager`, `skills-manager`, `web-gui`
- **推荐理由：** Web GUI管理技能与MCP服务器，浏览器内便捷配置
...
- **标签：** `package-manager`, `agent-assets`, `multi-tool`
- **推荐理由：** AI Agent资产包管理器：42技能+5CLI+1插件一站式
...
- **标签：** `config`, `backup`, `migration`, `mcp`
- **推荐理由：** 配置备份恢复工具，支持同步MCP与技能。
...
- **标签：** `mcp`, `skill`, `hot-reload`
- **推荐理由：** 粘贴JSON直连MCP，自动导入技能，配置即生效
...
- **标签：** `token`, `cost`, `visualization`
- **推荐理由：** 浏览器内实时Token用量图表和费用估算
...
- **标签：** `plugin-doctor`, `conflict-resolution`
- **推荐理由：** 自动检测解决插件间及版本冲突
...
- **标签：** `oauth`, `chatgpt`
- **推荐理由：** 通过OAuth绑定ChatGPT订阅账号在DSH中使用
...
- **标签：** `cli`, `plugin-manager`, `dsh`
- **推荐理由：** 用于发现、安装和管理的DSH插件CLI工具。
...
- **标签：** `publisher`, `plugin`, `cli`
- **推荐理由：** 一键发布插件，自动校验完整性并生成PR。
...
- **标签：** `pets`, `mascot`, `skins`
- **推荐理由：** 公开动画宠物画廊，为所有主流CLI提供可爱皮肤
...
- **推荐理由：** DeepSeek Harness鲸鱼娘系列主题皮肤。
...
- **标签：** `theme`, `dark-mode`, `customization`, `ui`
...
- **标签：** `theme`, `background`, `personalization`
- **推荐理由：** 支持自定义图片/预设氛围个性化DSH Web背景
...
- **标签：** `ai-guide`, `tutorial`, `vibe-coding`
- **推荐理由：** 全面AI资源库，涵盖DeepSeek/GPT/Gemini教程。
...
- **标签：** `awesome-list`, `dsh-plugin`, `reference`
- **推荐理由：** 双语精选插件百科，DSH必备参考索引
...
- **标签：** `learning`, `dsh-plugin`, `marketplace`
...
- **标签：** `awesome-list`, `quickstart`, `dsh-plugin`
- **推荐理由：** DSH终极入门指南，附精选插件与实用工具集
...
- **标签：** `stock`, `finance`, `quant`, `research`
- **推荐理由：** 24个经认证的股票/金融/量化DSH插件
...
- **标签：** `awesome-list`, `automated`
- **推荐理由：** GitHub Actions自动每日爬取的双语插件目录
...
- **标签：** `awesome-list`, `plugin`, `recommender`
- **推荐理由：** AI驱动的全面DSH插件推荐列表，190+插件每小时更新。
...
- **标签：** `awesome-list`, `coding-agents`, `ui`
- **推荐理由：** 覆盖模型、工具、UI和智能体的广泛插件精选列表。
...
- **标签：** `industry`, `awesome-list`
- **推荐理由：** 面向金融、法律、医疗等行业的专业DSH插件目录。
...
- **推荐理由：** cordis.run官方自动生成的Cordis插件列表。
...
- **标签：** `awesome-list`, `plugins`
- **推荐理由：** DeepSeek Harness插件与工具综合精选列表。
...
- **标签：** `marketplace`, `bundle`, `plugin`
- **推荐理由：** DeepSeek Harness插件市场整合包。
...
- **标签：** `awesome`, `awesome-list`, `agents`
- **推荐理由：** 精心整理的DSH插件清单，覆盖生产力功能工具
...
- **标签：** `awesome-list`, `dsh-plugins`, `plugin-directory`
- **推荐理由：** 收录280+ DSH插件，14分类目录，社区精选集合
...
- **标签：** `registry`, `open-data`, `spam-filtered`
- **推荐理由：** 去垃圾数据的DSH插件/技能开放数据注册表
