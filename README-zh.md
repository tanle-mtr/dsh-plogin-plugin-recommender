...
- **标签：** `control-plane`, `long-horizon`, `governance`
- **推荐理由：** 长期Agent治理控制面。
...
- **标签：** `agent-teams`, `orchestration`, `dsh-plugin`
- **推荐理由：** DeepSeek Harness 多智能体团队协作编排插件。
...
- **标签：** `context`, `dashboard`, `dsh-plugin`, `tools`
- **推荐理由：** 上下文洞察仪表盘与侧边栏管理工具。
...
- **标签：** `vision`, `ocr`, `computer-vision`, `agent-tools`
- **推荐理由：** 纯文本模型的视觉增强工具箱，支持OCR和截图分析。
...
- **推荐理由：** DSH插件管理CLI，支持Hub schema解析安装。
...
- **标签：** `infrastructure`, `coding`, `skills`
- **推荐理由：** AI基建开发专用技能，支持代码优化与调试
...
- **标签：** `superpowers`, `coding`, `reasonix`
- **推荐理由：** 适配Reasonix的代码增强技能包
...
- **标签：** `superpowers`, `coding`, `tdd`, `debugging`
- **推荐理由：** TDD调试规划协作编程技能集
...
- **标签：** `code-simplification`, `coding`
- **推荐理由：** 代码简化技能，源自官方Harness
...
- **标签：** `documentation`, `skillhub`, `dsh-plugin`
- **推荐理由：** 官方技能中心文档与示例，助力DSH插件开发
...
- **标签：** `codex`, `mcp`, `sync`, `dsh-plugin`
- **推荐理由：** OpenAI Codex与DSH双向同步，支持MCP自动镜像
...
- **推荐理由：** Lua编写的高效Neovim插件管理器，适合DSH编码工作流
...
- **标签：** `desktop`, `dsh`, `dsh-plugin`, `cordis`
- **推荐理由：** 基于DSH插件生态的现代桌面客户端
...
- **标签：** `browser-extension`, `dsh-plugin`, `chat-management`, `prompt-manager`
- **推荐理由：** 覆盖Gemini、Claude、ChatGPT的浏览器增强套件
...
- **推荐理由：** 可扩展侧边栏，内置文件编辑、终端、对话和Git面板
...
- **标签：** `taskboard`, `ui-extension`, `coding-agents`
- **推荐理由：** 现代化任务面板，灵活嵌入Codex和DSH界面
...
- **标签：** `widget`, `ui-extension`, `floating`
- **推荐理由：** 浮动小鲸鱼 widget，实时监控账��余额并带动画
...
- **标签：** `desktop`, `tauri`, `dsh-plugin`, `native-app`
- **推荐理由：** Tauri免安装桌面版，预置插件，跨平台支持。
...
- **标签：** `desktop-app`, `windows`, `gui`
- **推荐理由：** Windows桌面GUI，集成Codex、SSH和远程访问
...
- **标签：** `ui`, `dsh-ui`, `interactive`
- **推荐理由：** 通过DSH围栏生成内联UI组件：图表、表单、测验
...
- **推荐理由：** CodePilot风格桌面客户端，跨平台插件扩展套件。
...
- **推荐理由：** 原生iOS客户端，WebSocket连接，手机端查看Agent轨迹。
...
- **推荐理由：** 开源锤子便签复刻版，支持DSH插件与Docker部署
...
- **标签：** `mcp`, `ui`, `skills`, `dsh-plugin`
- **推荐理由：** Web GUI插件管理面板，浏览器内管理技能与MCP
...
- **推荐理由：** 一站式DevOps桌面：SSH/数据库/Docker/SFTP/AI助手集成
...
- **标签：** `electron`, `desktop`, `ui`
- **推荐理由：** Electron桌面壳集成DSH Web UI与侧边栏
...
- **标签：** `theming`, `webui`, `customization`
- **推荐理由：** 支持自定义图片与预设氛围的Web背景美化插件
...
- **标签：** `rag`, `dsh-plugin`, `knowledge-base`, `vector-search`
- **推荐理由：** 开源RAG平台，将文档转化为可查询知识
...
- **推荐理由：** 基于Milvus的持久统一记忆层，支持混合语义搜索
...
- **标签：** `skills`, `rag`, `retrieval`, `infrastructure`
- **推荐理由：** 将零散技能文件转为可检索的AI技能知识库。
...
- **推荐理由：** 全面AI知识库百科，覆盖LLM、Agent、RAG等
...
- **标签：** `knowledge`, `api`, `documentation`
- **推荐理由：** 6.5万+中国API文档，零配置搜索
...
- **标签：** `agent-skills`, `dsh-plugin`, `llm-agent`, `meta-skill`
...
- **标签：** `workspaces`, `multi-device`, `agents`
- **推荐理由：** 全设备统一的Agent工作空间。
...
- **标签：** `preset`, `bootstrap`, `agent-skills`
- **推荐理由：** 两阶段预设优化DSH启动流程，集成Standard工具集
...
- **标签：** `cognitive-enhancement`, `agent-skills`, `research`
- **推荐理由：** 基于Anthropic J-space研究的高级认知增强技能集
...
- **标签：** `video`, `agent-skill`, `ffmpeg`, `automation`
- **推荐理由：** 一键自动生成 vox 风格拼贴讲解视频。
...
- **标签：** `skill`, `multimodal`, `ocr`
- **推荐理由：** 视频/音频/屏幕分析技能，生成带时间戳证据
...
- **推荐理由：** 原生任务看板，SQLite存储，Agent认领审核一体化。
...
- **推荐理由：** 22个iOS模拟器Agent工具，对话内完成构建驱动。
...
- **推荐理由：** 全栈自动数学建模竞赛Skill，GitOps流水线支持。
...
- **标签：** `agent-skills`, `self-evolve`, `workflow`
- **推荐理由：** 实现可组合、可审查的Agent自演进工作流
...
- **推荐理由：** 为Claude Code添加多模态视觉分析截图/UI/图表
...
- **推荐理由：** 88个开源Agent技能覆盖研究/营销/商业工作流
...
- **标签：** `skill-learning`, `automation`, `reusable`
- **推荐理由：** 将成功会话自动转为可复用技能
...
- **标签：** `patent`, `legal`, `skill`
- **推荐理由：** 专利侵权分析系统，可被任意agent调用
...
- **标签：** `legal`, `visualization`, `litigation`
- **推荐理由：** 法律诉讼时间轴和案件可视化工具
...
- **标签：** `spec`, `programming`, `agent`
- **推荐理由：** 仓库原生规格编程可移植Agent技能
...
- **标签：** `quality-gate`, `engineering-discipline`, `skills`
- **推荐理由：** 工程质量工作流，校验需求、测试实现、确保交付质量
...
- **推荐理由：** Godot 4.x全栈游戏开发技能包，专为DSH代理设计
...
- **标签：** `plugin-dev`, `skills`, `dsh`
- **推荐理由：** 指导AI从0到1开发、构建和安装DSH插件
...
- **推荐理由：** 电影感视频Agent技能，含152张镜头配方与Remotion模板
...
- **推荐理由：** DSH插件开发Agent技能，覆盖服务/事件/工具/适配器标准
...
- **标签：** `agent-skills`, `diagrams-as-code`, `dsh-plugin`, `mermaid-alternative`
- **推荐理由：** 生成可验证架构图与数据流图的代码工具
...
- **标签：** `ai-image-generation`, `dsh-plugin`, `skills`, `workflow-automation`
- **推荐理由：** 530+提示词模板与GPT图像生成技能库
...
- **标签：** `content-discovery`, `browser-automation`, `cross-platform`
- **推荐理由：** 跨平台内容发现代理，支持B站、YouTube、小红书等
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内直聊ChatGPT模型。
...
- **推荐理由：** 钉钉IM渠道，每次聊天变身为带工具能力的Agent。
...
- **标签：** `dsh-plugin`, `electron`, `image`, `upyun`
- **推荐理由：** 支持60+图床的通用图片上传工具
...
- **标签：** `ai-agent`, `dsh-plugin`, `low-code`, `self-hosted`
...
- **标签：** `plugin-registry`, `ecosystem`, `tooling`
- **推荐理由：** 自部署雷达，自动发现并实测验证海量插件。
...
- **推荐理由：** 通过OAuth将ChatGPT/Claude订阅转为DSH模型提供者
...
- **标签：** `cost-tracking`, `token-usage`, `api-cost`
- **推荐理由：** 实时追踪会话与日费用，预算管理用量历史。
...
- **推荐理由：** 为DSH添加Command Code模型接入与智能选型能力
...
- **标签：** `catalog`, `plugin-discovery`, `directory`
- **推荐理由：** DSH插件完整目录，1958仓库中英搜索分类筛选
...
- **推荐理由：** P2P加密多端远程访问DSH，支持PC/安卓/Web
...
- **标签：** `evaluation`, `llm-judge`, `rag-evaluation`, `dsh-plugin`
- **推荐理由：** 基于证据的评估框架，支持prompt/RAG/skills/Agent评测
...
- **标签：** `migration`, `claude-code`, `dsh-plugin`
- **推荐理由：** 一键迁移Claude Code/Codex/Hermes会话与数据到DSH
...
- **标签：** `cost-tracking`, `visualization`, `api-monitoring`
- **推荐理由：** 实时显示Token用量、成本和API余额的Web插件
...
- **标签：** `diagnostics`, `conflict-resolution`, `compatibility`
- **推荐理由：** 检测并解决插件间及版本冲突问题的诊断工具
...
- **标签：** `theme`, `animation`, `threejs`
- **推荐理由：** Three.js海洋动态主题，支持Chrome/Edge扩展
...
- **标签：** `tutorial`, `learning`
- **推荐理由：** 面向初学者的Harness工程教程。
...
- **推荐理由：** 一站式科研工作台，LaTeX/arXiv/GPU追踪实验管理。
...
- **标签：** `marketplace`, `store`, `catalog`
- **推荐理由：** 11000+插件商店，支持搜索排行安装命令与API。
...
- **推荐理由：** 基于tushare本地财经库，多Agent金融分析与盯盘
...
- **标签：** `community`, `marketplace`, `awesome-list`
- **推荐理由：** 每4小时自动同步的社区插件目录
...
- **标签：** `awesome-list`, `directory`, `research`
- **推荐理由：** 280+精选插件目录，14类覆盖
...
- **标签：** `guide`, `knowledge`, `development`
- **推荐理由：** 插件开发知识库，按需Agent技能
...
- **标签：** `kline`, `finance`, `chart`
- **推荐理由：** 交互式K线图，含技术指标和新闻分析
...
- **推荐理由：** Vertical curated list of 24 stock/finance/quant DSH plugins
...
- **标签：** `awesome-list`, `ecosystem`, `curation`
- **推荐理由：** DSH生态精选插件与工具集合列表。
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `curation`
- **推荐理由：** 社区维护的DSH插件生态精选列表。
...
- **标签：** `registry`, `awesome-list`, `curated`
...
- **标签：** `awesome-list`, `registry`, `resources`
- **推荐理由：** 涵盖插件、工具、技能和MCP的DSH精选列表
...
- **标签：** `verified`, `directory`, `curated`
- **推荐理由：** 证据驱动的已验证插件目录，附带权限安装指导
...
- **标签：** `awesome-list`, `2026`, `quality-checked`
...
- **标签：** `directory`, `comprehensive`, `star-ranked`
- **推荐理由：** 按star排序的1000+ DSH生态插件完整清单
...
- **标签：** `verified`, `community`, `directory`
- **推荐理由：** 经社区验证的DSH插件排行目录
...
- **标签：** `documentation`, `navigation`, `guidance`
- **推荐理由：** 按场景推荐插件的产品说明与风险导航
...
- **标签：** `awesome-dsh-plugin`, `deepseek-harness`, `plugin-portal`
...
- **标签：** `dsh-plugin-bundle`, `dsh-plugin-market`, `skills`
- **推荐理由：** 类CodeX插件市场，集中发现和管理DSH技能。
...
- **推荐理由：** 基于证据的DSH插件雷达，含2Origin实验室支持。
...
- **推荐理由：** 从DSH Agent内部直接搜索插件，提升可发现性。
