...
- **推荐理由：** 虚拟终端与文件系统，为Agent提供隔离沙盒执行环境。
...
- **推荐理由：** Rust高性能Agent运行时，配套精美桌面应用，专注编程。
...
- **标签：** `context-management`, `dashboard`, `sidebar`
- **推荐理由：** 深度上下文分析与可视化管理面板，实时监控上下文状态。
...
- **推荐理由：** 支持RPC、子Agent、哈希行编辑和MCP的编码智能体插件
...
- **推荐理由：** DSH插件市场CLI工具，支持安装、解析和管理插件
...
- **标签：** `auto-pilot`, `gitops`, `math-modeling`
- **推荐理由：** 全栈自动数学建模，支持GitOps流水线与代码自证
...
- **推荐理由：** 为Claude Code添加多模态视觉能力，支持UI截图分析
...
- **推荐理由：** 官方DSH插件开发知识库，按需调用的技能Bundle。
...
- **推荐理由：** 构建验证DSH/Cordis插件的可复用Codex技能。
...
- **标签：** `api`, `documentation`, `skillhub`
- **推荐理由：** 官方SkillHub Open API文档和插件开发示例。
...
- **推荐理由：** 通过OAuth将ChatGPT集成到DSH中，扩展AI工具集。
...
- **标签：** `ai-studio`, `browser-extension`, `chrome-extension`, `deepseek-harness`, `chatgpt`, `claude-ai`
- **推荐理由：** 多平台AI增强浏览器扩展，含提示词管理
...
- **标签：** `deepseek-harness-desktop`, `dsh-plugin-desktop`, `desktop`
- **推荐理由：** 官方桌面版DSH，突破浏览器限制的原生应用
...
- **标签：** `dsh-web`, `dsh-web-ui`, `deepseek-harness`
...
- **⭐ 星标：** 3,688
- **语言：** TypeScript
- **描述：** 开放的侧边栏底座，支持三方拓展注册新侧边栏页面。内置文件渲染编辑/终端/侧边对话/Git/子���理页面 ｜ Open sidebar foundation, supports third-party extensions to register new sidebar pages. Built-in file rendering/editing, terminal, side chat, Git, and sub-agent pages.
- **标签：** `sidebar`, `ui-extension`, `third-party`
- **推荐理由：** 开放侧边栏底座，支持三方扩展与内置多页面。
...
- **标签：** `taskboard`, `ui-extension`, `codex-plugin`
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex与DSH界面。
...
- **⭐ 星标：** 3,115
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录的 TUI 补位插件：Claude Code 风，鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上下文进度+TPS。npm 一键��。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `terminal`, `claude-code-style`
- **推荐理由：** 鲸鱼主题TUI，支持实时状态与流式思考展示。
...
- **标签：** `floating-widget`, `balance-tracker`
- **推荐理由：** 可爱浮动小鲸鱼，实时追踪账户余额，支持拖拽吸附。
...
- **标签：** `desktop-app`, `electron`, `gui`, `windows-desktop`
- **推荐理由：** 零配置Windows桌面客户端，内置SSH、插件、移动端远程访问
...
- **标签：** `desktop-app`, `electron`, `mcp`, `local-first`
- **推荐理由：** 本地优先AI桌面工作台，支持DSH会话、MCP、办公自动化
...
- **标签：** `agent`, `desktop-app`, `privacy`
- **推荐理由：** 本地优先AI桌面应用，支持多模型与隐私保护
...
- **标签：** `office`, `productivity`, `ui-extension`
- **推荐理由：** 将电子表格文档幻灯片集成到Harness环境
...
- **推荐理由：** 原生iOS客户端，通过WebSocket连接Harness
...
- **标签：** `desktop-app`, `electron`, `coding-agent`
...
- **标签：** `dsh-plugin`, `desktop`, `electron`, `ui`
...
- **标签：** `dsh-plugin`, `workbench`, `web`, `platform`
- **推荐理由：** 基于DSH的插件化Web工作平台，全局技能管理。
...
- **标签：** `bioinformatics`, `desktop-app`, `dsh-plugin`
- **推荐理由：** 生物信息学桌面Agent，支持一键安装和离线模式。
...
- **标签：** `desktop`, `devops`, `electron`
- **推荐理由：** 集数据库、SSH、Docker于一体的DevOps桌面工具。
...
- **标签：** `agent-memory`, `deepseek-harness`, `mcp`, `rag`, `long-term-memory`
- **推荐理由：** 本地优先Markdown记忆层，跨应用知识存储
...
- **标签：** `agent-memory`, `mcp`, `rag`, `long-term-memory`, `token-savings`
- **推荐理由：** 自演进记忆系统，混合检索，显著节省Token
...
- **标签：** `agent-memory`, `agentic-rag`, `context-engineering`, `continual-learning`
- **推荐理由：** 专注状态持久化的智能体记忆库
...
- **标签：** `memory`, `rag`, `agent-framework`
- **推荐理由：** 持久化记忆管理套件，让Agent具备长期记忆能力。
...
- **标签：** `rag`, `long-term-memory`, `milvus`
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持混合语义搜索。
...
- **标签：** `memory-hub`, `mcp`, `local-first`
- **推荐理由：** 个人AI Agent与本地记忆中心，全平台共享持久上下文。
...
- **标签：** `dsh-plugin`, `memory`, `context-management`
- **推荐理由：** 主动联想记忆，零提示注入回忆，三层自动巩固。
...
- **标签：** `agent-orchestration`, `agent-skills`, `mcp`, `local-first`, `no-code`
- **推荐理由：** 统一多平台智能体工作台，看板式任务管理
...
- **标签：** `agent-skills`, `claude-skills`, `gemini-skills`, `skill-md`
- **推荐理由：** 最全技能商店，自动抓取GitHub海量项目
...
- **标签：** `agent-collaboration`, `multi-agent-workspace`, `visual-editor`, `enterprise-agent`
- **推荐理由：** 企业级多智能体工作台，支持团队协作与可视化编辑
...
- **标签：** `agent-os`, `loop-engineering`, `mcp`, `llm-orchestration`, `skills`
- **推荐理由：** 自演进智能体OS，分阶段评估与预算化进化循环
...
- **标签：** `agent-control-plane`, `workflow-automation`, `long-horizon`
- **推荐理由：** 长周期Agent控制面，支持多框架耐久工作流编排。
...
- **标签：** `cognitive-enhancement`, `inference-control`, `agent-skills`
- **推荐理由：** 模型无关的推理时控制套件，支持深度推理与自我修复。
...
- **标签：** `creative-writing`, `novel`, `long-form`
- **推荐理由：** 结构化AI小说���作流程，角色世界观管理与审修一体。
...
- **推荐理由：** 纯文本模型增强视觉能力：OCR、多图问答、UI还原。
...
- **标签：** `agent-skills`, `skill-routing`, `vector-search`, `embeddings`
- **推荐理由：** 将分散技能文件转化为可检索的Agent技能库，带评估工具
...
- **标签：** `creative-writing`, `novel-writing`, `agent-skills`, `workflow`
- **推荐理由：** 网文/小说创作工作流，内置短剧、游戏、视频解说模板
...
- **标签：** `multimodal`, `computer-vision`, `agent-skills`
- **推荐理由：** 为代理赋予视听能力，分析视频音频屏幕内容
...
- **标签：** `task-management`, `workflow`, `sqlite`
- **推荐理由：** 本地SQLite任务板，支持���理认领与审核
...
- **标签：** `ios-simulator`, `automation`, `mobile`
- **推荐理由：** 对话内嵌iOS模拟器，配22个自动化工具
...
- **标签：** `skill-audit`, `management`, `dsh-plugin`
- **推荐理由：** DSH技能调用审计与内置管理器，支持日志与控制
...
- **标签：** `skills`, `agent`, `cross-platform`
- **推荐理由：** 将经验转化为跨AI平台的可复用Agent技能
...
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销与商业自动化
...
- **标签：** `plugin-discovery`, `skill`, `verification`
- **推荐理由：** 自动从GitHub发现、安装并验证DSH插件的Skill
...
- **标签：** `dsh-plugin`, `agent-skill`, `coding-mentor`
- **推荐理由：** AI编程导师技能，扫描架构生成阶梯章节并代码评审。
...
- **标签：** `agent-skills`, `web-search`, `multi-agent`
- **推荐理由：** 提供网页搜索、内容提取、研究等AI Agent通用技能。
...
- **标签：** `dsh-plugin`, `skills-deck`, `engineering`
- **推荐理由：** 内置25个工程效率技能，右侧面板一键调用。
...
- **标签：** `dsh-plugin`, `patent`, `legal-tech`
- **推荐理由：** 专利侵权分析技能，输入公开号生成竞品侵权报告。
...
- **标签：** `dsh-plugin`, `gongwen`, `docx`, `wps`
- **推荐理由：** 党政机关公文全流程处理技能，格式检查与优化。
...
- **标签：** `dsh-plugin`, `legal`, `visualization`, `svg`
- **推荐理由：** 诉讼可视化工具，生成时间轴、对抗图与关系图。
...
- **推荐理由：** 类似WorkBuddy的快捷技能选择器，提升工作效率
...
- **标签：** `agent-skills`, `deepseek-harness`, `workflow`
- **推荐理由：** 议行分离机制，结构化AI协作变更任务
...
- **标签：** `math-modeling`, `cumcm`, `dsh-plugin`
- **推荐理由：** 数学建模多Agent方案，含33模型库和论文模板
...
- **⭐ 星标：** 28
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用��种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math-modeling`, `skill`, `chinese`
- **推荐理由：** 中文数学建模全流程资源：数据/编程/论文一体化
...
- **标签：** `dsh-plugin`, `api-documentation`, `mcp`
...
- **标签：** `vibedesign`, `ui`, `react-native`, `flutter`, `mcp`, `skill`
- **推荐理由：** 开源AI原生矢量设计工具，并发智能体团队设计即代码
...
- **推荐理由：** 原生AI设计工作室，可视化编辑、模板市场与PPT生成。
...
- **标签：** `dsh-plugin`, `visual`, `ui-components`, `interactive`
- **推荐理由：** 在回复中内联生成交互式UI组件：图表、表单、测验
...
- **标签：** `video`, `ai-image`, `directorx`
- **推荐理由：** AI视频图像音频技能，可配置视觉媒体工具。
...
- **标签：** `kline`, `finance`, `deepseek-harness-plugin`
...
- **标签：** `video`, `motion-design`, `dsh-plugin`
- **推荐理由：** 电影级产品视频Agent，含152张镜头配方卡
...
- **推荐理由：** 自进化跨平台内容发现Agent，覆盖B站/抖音/YouTube等。
...
- **⭐ 星标：** 513
- **语言：** TypeScript
- **描述：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without native web access. Free, no signup, no API key. Ask the web or X, get structured JSON evidence. | 🥇 全网最强的 DeepSeek Harness 免费联网搜索插件，免费免注册免 API key。为不能联网的模��补上搜索，问网页或 X，拿回结构化 JSON 证据（搜索、抓取、引用）。
- **标签：** `web-search`, `web-fetch`, `agent-skills`, `agentic-workflow`
...
- **标签：** `browser-automation`, `anti-detect`, `playwright`, `mcp`
- **推荐理由：** 反检测浏览器，持久身份、指纹伪装，支持Playwright
...
- **标签：** `marketplace`, `dsh-plugin`, `plugin-browser`
- **推荐理由：** DSH内置插件市场，支持一键安装和搜索浏览。
...
- **标签：** `account-management`, `multi-model`, `smart-routing`
- **推荐理由：** 统一管理多模型账号与配额，支持智能路由与自动故障转移。
...
- **推荐理由：** 扫描并K8s验证2万+插件，持续发现优质DSH生态插件。
...
- **标签：** `awesome-list`, `resource`, `curation`
- **推荐理由：** 精心整理的DSH生态插件与工具资源大全导航。
...
- **标签：** `marketplace`, `plugin-store`, `discovery`
- **推荐理由：** 全面插件商城，每日更新，发布前审核保障质量。
...
- **推荐理由：** 将ChatGPT Claude Grok订阅转为DSH模型提供商
...
- **标签：** `cost-tracking`, `utility`, `monitoring`
- **推荐理由：** 跟踪会话日成本预算和各提供商Token用量
...
- **标签：** `llm-provider`, `model-access`, `utility`
...
- **推荐理由：** 安全P2P远程访问方案，支持多端操作远程Harness
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skill-management`
- **推荐理由：** 支持技能/工具三级管理，减少上下文占用，动态发现执行。
...
- **标签：** `dsh-plugin`, `plugin-management`, `infrastructure`
- **推荐理由：** DSH插件生态基建，薄控制台+官方开发引导。
...
- **标签：** `context`, `audit`, `ui`
- **推荐理由：** 上下文注入审计工具，含Token成本分析和Web面板。
...
- **标签：** `mcp`, `config-sync`, `dsh`, `claude-code`
- **推荐理由：** 一键同步MCP和配置，支持7种主流AI编码工具
...
- **标签：** `dsh-plugin`, `cordis`, `infrastructure`
- **推荐理由：** 统一编排DSH核心能力，构建可扩展插件生态
...
- **标签：** `migration`, `dsh-plugin`, `claude-code`
- **推荐理由：** 从Claude Code/Codex迁移会话和技能到DSH
...
- **标签：** `cost-tracking`, `visualization`, `token`
- **推荐理由：** 实时追踪Token用量、费用估算和API余额。
...
- **标签：** `mascot`, `pixel-art`, `ui-enhancement`
- **推荐理由：** 可动宠物画廊，个性化编码Agent界面。
...
- **标签：** `theme`, `ui`, `glass-morphism`, `customizable`
- **推荐理由：** 高度可定制的毛玻璃主题，支持磨砂模糊效果自定义
...
- **标签：** `theme`, `animated`, `chrome-extension`
- **推荐理由：** 动态海洋主题，可调波浪与玻璃拟态效果
...
- **标签：** `webui`, `background`, `themes`
- **推荐理由：** Web背景个性化，支持实时预览
...
- **标签：** `documentation`, `deepwiki`, `code-analysis`
- **推荐理由：** 快速从代码库生成精准技术文档与AI可读上下文。
...
- **标签：** `arxiv`, `latex`, `research`, `experiment-tracking`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪、GPU SSH编排
...
- **标签：** `guide`, `resources`, `awesome-list`
- **推荐理由：** DSH终极指南含快速入门与插件推荐
...
- **标签：** `documentation`, `learning`, `marketplace`
- **推荐理由：** 学习DSH原理浏览插件市场最佳实践
...
- **推荐理由：** 跨平台学术论文检索，支持引用图谱与核验，MCP集成
...
- **标签：** `ai-curated`, `recommendation`
- **推荐理由：** AI精选千余插件，12分类每小时更新
...
- **标签：** `automation`, `directory`
- **推荐理由：** GitHub Actions自动每日更新目录
...
- **标签：** `agent`, `dsh-plugin`, `harness-engineering`, `llm`
- **推荐理由：** 零基础至精通Harness工程的入门教程
...
- **推荐理由：** 中英双语DSH插件精选导航库，收录工具、技能与扩展
...
- **标签：** `cybersecurity`, `security`, `vulnerabilities`, `redteam`
- **推荐理由：** AI安全攻防平台，审计幻觉包、漏洞和安全模式错误
...
- **标签：** `knowledge-base`, `awesome-list`, `mcp`, `agent`
- **推荐理由：** 全维度AI资源百科：技能、MCP、RAG、Agent学习路线
...
- **标签：** `awesome-list`, `resources`, `reference`
- **推荐理由：** 全面的DSH插件技能MCP服务器策展列表
...
- **推荐理由：** 插件市场，收录超1万插件，支持搜索评分及免费API
...
- **推荐理由：** 全面手册，涵盖运行时、插件、MCP及74项资源指南
...
- **推荐理由：** Complete reverse-skill (87 SKILL.md) as 
...
- **推荐理由：** Automatically turn successful DeepSeek H
...
- **推荐理由：** DSH官方awesome列表：插件/工具/技能资源汇总
...
- **标签：** `awesome-list`, `registry`, `curated`
- **推荐理由：** 垃圾过滤的开放插件注册表，内容可信。
...
- **标签：** `awesome-list`, `verified`, `curation`
- **推荐理由：** 基于证据的插件目录，含安装指导。
...
- **推荐理由：** 自动更新的 bilingual 插件列表，数据驱动。
...
- **标签：** `plugin-directory`, `curated-list`
- **推荐理由：** 涵盖模型、工具和UI的插件注册表。
