...
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (20)
...
- **标签：** `ai-agent`, `multi-agent`, `mcp`
- **推荐理由：** 超级AI助手平台，支持自进化记忆与多模型。
...
- **推荐理由：** 终端原生DeepSeek编码Agent，支持前缀缓存。
...
- **推荐理由：** 跨平台统一Agent工作空间，桌面/移动/浏览器通用
...
- **标签：** `routing`, `injector`, `cordis`
- **推荐理由：** 任务感知推理模式路由器，DSH关键运行时注入器
...
- **标签：** `coding-agent`, `architecture`, `safety`
...
- **标签：** `dsh-plugin`, `ui`, `coding-agent`
- **推荐理由：** DSH内联交互式UI组件，图表/表单/测验直接渲染
...
- **标签：** `compatibility-layer`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 通过兼容层将Pi扩展作为DSH插件原生运行。
...
- **标签：** `plugin-dev`, `skill`, `meta`
- **推荐理由：** 开发DSH插件的元技能，覆盖槽位、事件、工具、打包全流程。
...
- **推荐理由：** 为纯文本模型提供OCR和图片读取能力，附34张样本训练方法。
...
- **标签：** `plugin-dev`, `tools`, `credentials`
- **推荐理由：** 管理DSH插件槽位、Typert远程和凭据的开发辅助技能。
...
- **标签：** `mcp`, `capability-menu`, `dsh-plugin`
- **推荐理由：** 统一管理MCP工具与技能暴露级别和执行模式
...
- **推荐理由：** Codex与DSH双向同步桥梁，聊天技能MCP互通
...
- **标签：** `agent-framework`, `skills`, `database`
- **推荐理由：** 42技能5工具集的AI代理开发包管理器
...
- **标签：** `documentation`, `best-practices`, `plugin`
- **推荐理由：** DSH插件开发踩坑实录，含多平台实践
...
- **标签：** `plugin-creator`, `dsh-plugin`, `codex`
- **推荐理由：** 快速构建和验证DSH插件的通用技能工具。
...
- **标签：** `dsh-web`, `web-ui`, `plugin-aggregator`
- **推荐理由：** Web插件聚合生态包，创意工坊分发模式
...
- **推荐理由：** 可扩展侧边栏，内置文件编辑/终端/对话/Git功能
...
- **推荐理由：** Rust高性能Agent运行时与精美桌面应用相结合
...
- **标签：** `desktop`, `pet`, `electron`, `sdk`
- **推荐理由：** 带动画宠物的桌面伴侣平台，支持插件SDK
...
- **标签：** `widget`, `balance`, `ui`
- **推荐理由：** 可爱漂浮鲸鱼小部件监控账户余额
...
- **推荐理由：** 带SSH和移动访问的Windows桌面DSH GUI。
...
- **推荐理由：** Electron桌面控制台，多Tab管理插件/MCP/技能，双击即用。
...
- **推荐理由：** Tauri跨平台桌面：数据库+SSH+Docker+AI一体。
...
- **推荐理由：** Electron桌面壳集成DSH Web UI及侧边栏。
...
- **推荐理由：** 可移植的本地优先Markdown记忆层，跨应用通用
...
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持混合语义检索
...
- **推荐理由：** AI百科大全，覆盖DSH/Agent/RAG/多模态等全维度���识
...
- **标签：** `agent-skills`, `embeddings`, `dsh-plugin`
- **推荐理由：** 将零散的SKILL.md文件转为可检索的技能库。
...
- **标签：** `recommender`, `awesome-list`, `curated`
- **推荐理由：** AI维护的190+插件清单，每小时更新
...
- **推荐理由：** 重定向至主awesome-dsh-plugin列表
...
- **标签：** `curated`, `evidence-aware`
- **推荐理由：** 基于证据筛选的DSH插件精简清单
...
- **标签：** `curated`, `registry`, `plugins`
- **推荐理由：** 全面的DSH精选插件，覆盖模型和UI
...
- **标签：** `agent-skills`, `cognitive`, `global-workspace`
- **推荐理由：** 基于J-space全局工作区研究的认知增强技能套件
...
- **标签：** `agent`, `ai-agent`, `tauri`
- **推荐理由：** 本地优先AI代理，自进化技能，多模型支持，隐私安全
...
- **标签：** `security`, `cybersecurity`, `agent`
...
- **标签：** `agent`, `multi-agent`, `orchestration`
- **推荐理由：** 多Agent协同编排，Claude/Codex/Grok统一调度
...
- **标签：** `deepseek`, `prompt-engineering`, `agent`
- **推荐理由：** DeepSeek破甲神器，无限代稳定提示词生成
...
- **标签：** `desktop`, `agent`, `electron`
- **推荐理由：** 类CodePilot跨平台桌面客户端，macOS/Windows/Linux全支持
...
- **标签：** `ios`, `mobile-automation`, `agent`
- **推荐理由：** DSH内嵌iOS模拟器，22个Agent工具驱动真机自动化
...
- **标签：** `canvas`, `workspace`, `agent`
- **推荐理由：** 画布式非线性对话工作区，会话分支可视化探索
...
- **标签：** `creative-writing`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 用于小说和短剧创作的创意写作技能。
...
- **推荐理由：** 为Claude Code和文本LLM添加多模态视觉分析。
...
- **标签：** `agent-skills`, `coding`, `dsh`
- **推荐理由：** 将经验转化为可重复执行技能的框架。
...
- **推荐理由：** 对话内多源搜番，含Bangumi评分与磁力复制功能。
...
- **推荐理由：** 移植自Superpowers的TDD、调试、规划技能，实用性强。
...
- **标签：** `security`, `reverse-engineering`, `pentest`
- **推荐理由：** 85个SKILL覆盖逆向与授权渗透，安全研究必备。
...
- **标签：** `skill`, `mentor`, `coding`, `review`
- **推荐理由：** AI编程助手秒变导师，自动生成学习路径并审查代码。
...
- **标签：** `skills`, `research`, `marketing`, `business`
- **推荐理由：** 88个开源技能覆盖研究、营销和商业务流程，跨平台可用。
...
- **标签：** `spec-driven`, `development`, `skill`
- **推荐理由：** 基于规范的编程技能，可移植且贴合项目仓库结构。
...
- **标签：** `chinese-wisdom`, `fengshui`, `agent-skill`
- **推荐理由：** 八字风水易经融合AI技能，本地确定性引擎
...
- **推荐理由：** Swift Cycle治理技能适配，版本锁定可离线验证
...
- **推荐理由：** DSH代理起飞前检查单，自动注入Top-K相关插件
...
- **标签：** `api`, `mcp`, `documentation`, `chinese-platforms`
- **推荐理由：** 搜索20个中国平台6.5万篇API文档，零配置即开即用。
...
- **标签：** `chatgpt`, `oauth`, `subscription`, `codex`
...
- **标签：** `architecture-diagram`, `mermaid-alternative`, `diagrams-as-code`
- **推荐理由：** 美观可验证的架构图和时序图，支持动效
...
- **推荐理由：** AI原生矢量设计工具，支持多Agent并发与直接生成UI
...
- **标签：** `video`, `vox`, `generative-ai`
- **推荐理由：** 一键生成Vox风格拼贴解说视频，全流程自动化
...
- **标签：** `image-generation`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 在DSH聊天中直接生成AI图像。
...
- **推荐理由：** 将诉讼材料可视化，生成法庭可用的图表和时间轴，法律科技利器。
...
- **标签：** `ai-video`, `storyboard`, `text-to-video`
- **推荐理由：** AI视频图像音频技能，支持可配视觉工具
...
- **标签：** `canvas`, `creative`, `mcp-tools`
- **推荐理由：** 无限画布+创意技能，适合视觉创作工作流。
...
- **推荐理由：** 增强Gemini、Claude和ChatGPT等Web界面功能。
...
- **标签：** `content-discovery`, `bilibili`, `local-first`
- **推荐理由：** 本地优先的跨平台内容发现Agent，隐私友好
...
- **标签：** `multi-agent`, `claude-code`, `deepseek`
- **推荐理由：** 一键安装切换多种CLI编程Agent，省时省力
...
- **标签：** `discovery`, `plugin`, `marketplace`
- **推荐理由：** 每日自动抓取插件市场，真伪辨别，场景化分类
...
- **标签：** `llm`, `oauth`, `integration`
- **推荐理由：** OAuth整合多个订阅账号作为DSH LLM提供商
...
- **标签：** `documentation`, `learning`, `plugin`
- **推荐理由：** DSH原理学习+插件市场+最佳实践一站式指南
...
- **标签：** `cost-tracking`, `deepseek-api`, `dsh-plugin`
- **推荐理由：** 追踪DSH中的会话成本、token用量和预算。
...
- **标签：** `plugin-discovery`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 从GitHub搜索、安装并验证DSH插件。
...
- **标签：** `task-management`, `sqlite`, `web-ui`
...
- **标签：** `market`, `community`, `web-gui`
- **推荐理由：** 社区插件市场集成到DSH Web GUI，一键安装。
...
- **标签：** `infrastructure`, `plugin-management`, `ui`
- **推荐理由：** DSH插件生态基建，提供浏览器控制台和官方开发引导。
...
- **标签：** `dsh-plugins`, `containers`, `webview`
- **推荐理由：** 隔离多版本容器管理，一键导入插件技能
...
- **标签：** `dsh`, `oauth`, `community`
- **推荐理由：** 零权限GitHub登录的社群合影墙工具
...
- **标签：** `migration`, `config-migration`, `import`
- **推荐理由：** 一键迁移Claude Code配置至DSH，支持多平台。
...
- **标签：** `token-usage`, `cost-tracking`, `data-viz`
...
- **标签：** `diagnostics`, `conflict-resolution`, `plugin-health`
...
- **标签：** `mascot`, `pixel-art`, `animated`
- **推荐理由：** 编程Agent的像素动画宠物画廊，丰富界面体验
...
- **标签：** `theme`, `ui`, `deepseek-harness`
- **推荐理由：** 磨砂玻璃主题，可自定义模糊度和背景，颜值极高
...
- **标签：** `theme`, `chrome-extension`, `dsh-plugin`
- **推荐理由：** 为DSH界面提供WebGPU海洋主题。
...
- **标签：** `theme`, `dark-mode`, `color-scheme`
...
- **标签：** `webui`, `background`, `theme`, `personalization`
- **推荐理由：** 自定义网页背景插件，支持预设和实时预览
...
- **标签：** `tutorial`, `harness-engineering`, `dsh`
- **推荐理由：** 从零开始深入讲解Harness工程的完整入门教程
...
- **标签：** `kline`, `trading`, `analysis`, `dashboard`
- **推荐理由：** 交互式K线分析插件，支持多市场技术指标
...
- **标签：** `agent-evaluation`, `rag-evaluation`, `prompt-engineering`
- **推荐理由：** 证据驱动的评估框架，支持提示词和RAG评测
...
- **标签：** `awesome-list`, `2026`, `quality-check`
- **推荐理由：** 2026年精选DSH插件，带质量审核。
...
- **标签：** `awesome-list`, `community`, `ecosystem`
- **推荐理由：** DSH插件生态精选收录列表
...
- **标签：** `awesome-list`, `community`, `plugins`
- **推荐理由：** 社区精选的DSH插件与基础设施收录
...
- **标签：** `awesome-list`, `deepseek-harness`, `mcp`
- **推荐理由：** 精选DSH插件、技能、MCP服务器和补丁列表。
...
- **标签：** `deepseek-harness`, `plugins`, `dsh-plugin`
- **推荐理由：** DeepSeek Harness插件集合。
...
- **推荐理由：** DeepSeek Harness插件精选Awesome列表。
...
- **标签：** `awesome-list`, `official`, `discovery`
- **推荐理由：** cordis.run官方自动生成的插件列表，权威可靠。
...
- **标签：** `awesome-list`, `plugins`, `community`
- **推荐理由：** 精选14类280+插件目录，社区宝藏汇总
...
- **标签：** `plugins`, `collection`, `chinese`
- **推荐理由：** 好用的DeepSeek Harness插件中文合集
...
- **标签：** `plugin-directory`, `awesome-list`, `guide`
- **推荐理由：** 按场景推荐的插件说明书与导航，含安装风险说明
...
- **标签：** `plugin-portal`, `static-site`, `directory`
- **推荐理由：** 零依赖静态门户，发现和安装227+ DSH插件
...
- **标签：** `dsh-plugins`, `registry`, `ecosystem`
- **推荐理由：** DSH插件目录和生态列表
...
- **推荐理由：** 每日GitHub Actions自动抓取的双语DSH插件目录
...
- **标签：** `awesome-list`, `evidence-led`, `bilingual`
- **推荐理由：** 不可变提交审查的DSH插件双语证据目录
...
- **标签：** `plugin-radar`, `awesome-list`, `2origin`
- **推荐理由：** 插件雷达和2Origin插件实验室，证据支持列表
