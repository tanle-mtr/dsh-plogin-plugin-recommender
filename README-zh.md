...
- **标签：** `agent-workspace`, `enterprise`, `multi-engine`
- **推荐理由：** 企业级多引擎Agent工作台，内置可视化编辑器
...
- **标签：** `marketplace`, `dsh-plugin`, `install`
- **推荐理由：** DSH插件市场，一键安装必备工具
...
- **标签：** `preset`, `bootstrap`, `agent-config`
- **推荐理由：** 两阶段预设，快速引导标准工具集成
...
- **标签：** `terminal`, `vfs`, `virtual-filesystem`
- **推荐理由：** 首个AI虚拟终端，支持VFS文件操作
...
- **标签：** `model-switch`, `claude-code`, `codex`
- **推荐理由：** 一键切换多模型，支持主流AI编程工具
...
- **标签：** `taskboard`, `cli`, `coding`
- **推荐理由：** 现代化灵活任务面板，支持Codex集成
...
- **标签：** `tui`, `terminal`, `official`
- **推荐理由：** 官方TUI插件，实时状态与流式思考
...
- **推荐理由：** Rust高性能Agent运行时，桌面IDE体验优秀
...
- **标签：** `agent-teams`, `agentic`, `harness`
- **推荐理由：** 在DSH中启用多Agent团队协作能力
...
- **推荐理由：** AI安全攻防平台，审计AI生成代码中的幻觉包和安全漏洞
...
- **推荐理由：** 编码Agent，支持RPC子Agent、哈希行编辑和MCP集成
...
- **标签：** `workbench`, `plugin`, `platform`
- **推荐理由：** 插件化Web工作平台，支持全局Skill管理与原生工作区集成。
...
- **标签：** `launcher`, `orchestration`, `kernel`
- **推荐理由：** 释放DSH插件化内核，统一编排Runtime、Provider、MCP等能力。
...
- **推荐理由：** 开发DSH插件的Agent Skill，提供插件/事件/工具打包标准。
...
- **标签：** `agent-tools`, `awesome-dsh-plugin`, `automation`
- **推荐理由：** 任务前自动预检并注入Top-K插件，提升Agent效率
...
- **标签：** `web-ui`, `plugin-aggregation`
- **推荐理由：** 基于Web的插件生态聚合器，创意分发平台
...
- **推荐理由：** 本地优先的Electron桌面工作区，支持会话、项目和网页研究
...
- **标签：** `desktop`, `windows`, `gui`
- **推荐理由：** Windows桌面GUI客户端，零安装，支持SSH和移动远程访问
...
- **标签：** `desktop`, `local-first`, `multi-model`
- **推荐理由：** 本地优先的多模型桌面应用，支持自进化技能和隐私优先架构
...
- **标签：** `office`, `desktop`, `productivity`
- **推荐理由：** 为DeepSeek Harness桌面端带来完整办公套件：表格、文档、幻灯片等。
...
- **推荐理由：** 原生iOS客户端，通过WebSocket连接，随时随地查看会话和Agent轨迹。
...
- **标签：** `desktop`, `electron`, `ui-client`
- **推荐理由：** 类CodePilot桌面客户端，支持macOS/Windows/Linux全平台。
...
- **标签：** `dsh-plugin`, `notes`, `smartisan`, `docker`
- **推荐理由：** 开源锤子便签复刻，支持Docker私有化部署和skill调用。
...
- **标签：** `mcp`, `ui`, `skills`
- **推荐理由：** Web界面可视化管skills/MCP工具，操作直观
...
- **标签：** `ui`, `info-bar`, `balance`
- **推荐理由：** 底部栏显示供应商、余额、定价和倒计时信息
...
- **推荐理由：** 一站式DevOps桌面，集成数据库、SSH、Docker、AI
...
- **推荐理由：** Electron桌面封装，集成DSH Web服务及插件
...
- **标签：** `ui`, `button`, `context-meter`
- **推荐理由：** 一键紧凑按钮，用于DSH上下文计量面板
...
- **推荐理由：** 智能技能路由，SkillsBench效率提升21pp
...
- **标签：** `novel-writing`, `creative-writing`, `local-first`
- **推荐理由：** 完整小说创作流程，含角色与世界观管理
...
- **推荐理由：** 赋予AI智能体视听观察能力，生成��时间戳的可验证证据。
...
- **标签：** `task-management`, `agent-workflow`, `native-ui`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领/审核工作流，无iframe。
...
- **标签：** `agent-skills`, `deepseek-harness`, `mcm`, `automation`
- **推荐理由：** 全栈式数学建模竞赛Skill，支持国赛美赛及GitOps流水线。
...
- **标签：** `agent-skills`, `skills`, `dsh`, `coding`
- **推荐理由：** 将经验转化为可重复执行的工作流，兼容Claude/Codex/DSH。
...
- **标签：** `agent-skills`, `research`, `marketing`, `deepseek-harness`
...
- **推荐理由：** DSH插件搜索安装验证Skill，从GitHub一键获取插件。
...
- **标签：** `agent-skills`, `vision`, `multimodal`, `deepseek`
- **推荐理由：** 纯文本LLM的多模态视觉Skill，支持截图/UI/图表分析。
...
- **标签：** `dsh-plugin`, `skills`, `ai-infra`, `agent-skills`
...
- **推荐理由：** TDD/调试/规划技能适配DeepSeek Harness
...
- **标签：** `bioinformatics`, `desktop`, `agent`
- **推荐理由：** 为临床医生打造的生物信息学AI代理桌面应用
...
- **推荐理由：** WorkBuddy风格技能选择器，快速插入到DSH编辑器
...
- **推荐理由：** 官方SkillHub API，含示例和DSH插件技能发现
...
- **推荐理由：** 电影感产品视频Agent技能，含152张镜头配方卡与Remotion模板。
...
- **推荐理由：** 5角色多Agent数学建模求解器，含33个模型库与论文模板。
...
- **推荐理由：** 证据驱动的综合评估框架，覆盖Prompt、RAG、Skill与Agent。
...
- **标签：** `data-visualization`, `ai-cost`, `deepseek-api`
- **推荐理由：** 实时Token用量、成本图表和API余额监控
...
- **标签：** `dsh-plugin`, `knowledge-cards`, `ui`
...
- **标签：** `browser`, `content-discovery`, `cross-platform`
- **推荐理由：** 本地优先跨平台AI，多平台内容发现
...
- **标签：** `browser-automation`, `web-use`, `agent`
- **推荐理由：** 复用已登录浏览器实现Agent网页自动化
...
- **⭐ 星标：** 487
- **语言：** TypeScript
- **描述：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without native web access. Free, no signup, no API key. Ask the web or X, get structured JSON evidence. | 🥇 全网最强的 DeepSeek Harness 免费联网搜索插件，���费免注册免 API key。为不能联网的模型补上搜索，问网页或 X，拿回结构化 JSON 证据（搜索、抓取、引用）。
- **标签：** `web-search`, `free`, `no-signup`
- **推荐理由：** 最强的免费网页搜索插件，无需注册，无API费用
...
- **推荐理由：** 浏览器指纹伪造+持久身份+Android模拟，适合反检测Web自动化。
...
- **推荐理由：** 对话内集成iOS模拟器及真机控制，22个Agent自动化调试工具。
...
- **推荐理由：** 基于Playwright的浏览器自动化，网页操作利器
...
- **推荐理由：** 通过OAuth使用ChatGPT/Claude/Grok订阅作为LLM提供商，无需API密钥。
...
- **推荐理由：** DSH插件中心的CLI管理工具，高效从终端管理插件。
...
- **推荐理由：** 追踪会话/日Token成本、预算警报和多提供商余额监控。
...
- **推荐理由：** 为DSH添加Command Code模型提供商，支持实时目录和智能选型。
...
- **标签：** `dsh-plugin`, `agent-review`, `plan-mode`, `self-evolving`
- **推荐理由：** 支持Agent可组合任务控制与自我进化的Web插件。
...
- **标签：** `dsh-plugin`, `remote-access`, `p2p`, `multi-device`
- **推荐理由：** 安全低延迟P2P远程访问方案，支持多端操作Harness。
...
- **标签：** `config-manager`, `backup`, `restore`
- **推荐理由：** 支持备份/恢复/同步配置和插件，实用工具
...
- **标签：** `container`, `management`, `devops`
- **推荐理由：** 在隔离容器中运行多个DSH版本，一键导入插件
...
- **标签：** `tts`, `text-to-speech`, `xiaomi-mimo`
- **推荐理由：** 集成免费小米MiMo TTS，支持文本朗读
...
- **标签：** `plugin-conflict`, `version-management`, `utility`
- **推荐理由：** 解决插件冲突和版本兼容问题，提供解决方案
...
- **标签：** `dsh-plugin-market`, `marketplace`
- **推荐理由：** DSH版类CodeX插件市场，便于发现和安装技能插件
...
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内直接调用ChatGPT模型
...
- **标签：** `mascot`, `theme`, `pixel-art`
- **推荐理由：** 多AI工具的动画宠物画廊，丰富个性化体���
...
- **标签：** `skin`, `ocean`, `animated`
- **推荐理由：** 实时海洋动态主题，可调波浪、日落和玻璃透明度效果
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单���受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `background`, `customization`, `themes`
- **推荐理由：** 自定义DSH网页背景，支持图片与视觉效果
...
- **标签：** `ai-guide`, `resources`, `tutorials`
- **推荐理由：** 全面的AI资源库，含教程、提示词和大模型玩法指南
...
- **标签：** `cognitive`, `reasoning`, `inference`
- **推荐理由：** 模型无关推理控制，深度推理与验证
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv管理、实验追踪和GPU编排
...
- **标签：** `finance`, `mcp`, `multi-agent`, `research`, `deepseek-harness`
...
- **标签：** `industry-research`, `report`, `research`
- **推荐理由：** 行业研究方法链+政策新闻追踪，研究必备
...
- **标签：** `kline`, `analysis`, `technical-indicators`
- **推荐理由：** 交互式K线分析，支持技术指标、支撑压力位及新闻基本面。
...
- **标签：** `awesome-list`, `ai-curated`, `mcp`
- **推荐理由：** AI每小时更新，190+插件按12类整理，推荐精准
...
- **标签：** `awesome-list`, `static-site`, `web-portal`
- **推荐理由：** 零依赖静态站点，227+插件可视化浏览筛选
...
- **推荐理由：** DSH Agent内直接搜索npm和精选列表，发现体验流畅
...
- **标签：** `dsh-plugins`, `ecosystem`
- **推荐理由：** 涵盖插件生态分析和多种功能插件的综合目录
...
- **推荐理由：** 原始Awesome DSH插件仓库，现已迁移至新地址
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `directory`
- **推荐理由：** 全面的DSH插件生态精选目录
...
- **标签：** `plugin-market`, `dsh`, `community`
- **推荐理由：** 每日更新的DSH插件市场，社区审核，资源丰富
...
- **⭐ 星标：** 340
- **语言：** JavaScript
- **描述：** 30 ��找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `discovery`, `plugin-curator`
- **推荐理由：** 每日抓取GitHub插件，精选分类、热度排行，帮你快速找到合适插件。
...
- **推荐理由：** 全面的DSH生态精选清单，涵盖插件、技能、MCP服务器和补丁。
...
- **推荐理由：** DeepSeek Harness终极指南，覆盖入门、资源、精选插件与工具。
...
- **标签：** `awesome-list`, `marketplace`, `catalog`, `dsh-plugin`
- **推荐理由：** DSH插件市场目录，11000+插件及免费API查询。
...
- **标签：** `handbook`, `agent-runtime`, `mcp`, `deepseek-harness`
- **推荐理由：** Agent优先的DSH手册，涵盖运行时、插件、MCP等173个指南。
...
- **标签：** `catalog`, `awesome-list`, `dsh-plugin`, `plugin-marketplace`
- **推荐理由：** DSH插件总目录，1958个仓库，1819个真实插件分类筛选。
...
- **⭐ 星标：** 56
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（大事记表、庭审对抗图、���律关系图）。Claude Skill / DeepSeek Harness 通用。
...
- **标签：** `awesome-list`, `directory`, `deepseek-harness`
- **推荐理由：** 全面的DSH工具清单，按star排序1000+仓库
