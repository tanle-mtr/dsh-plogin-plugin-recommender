...
- **标签：** `agent-sandbox`, `vfs`, `virtual-filesystem`
- **推荐理由：** 统一虚拟文件系统，为AI代理提供沙盒隔离支持。
...
- **标签：** `claude-code`, `model-switch`, `coding-agent`
- **推荐理由：** 一键切换Claude Code、Codex等十余种主流AI编程代理。
...
- **标签：** `agent-runtime`, `rust`, `ai-coding`
- **推荐理由：** 高性能Rust代理运行时，搭配精致桌面IDE。
...
- **标签：** `agent-skills`, `ai-code-review`, `code-quality`, `dsh-plugin`
...
- **标签：** `agent-vision-toolkit`, `computer-vision`, `ocr`, `dsh-plugin`
- **推荐理由：** 强大视觉工具箱，支持OCR和图片问答功能
...
- **标签：** `security`, `code-audit`
- **推荐理由：** 审计AI代码中的幻觉包、安全漏洞与错误
...
- **标签：** `agent-review`, `prompt-engineering`
- **推荐理由：** 实现可组合、可审查的自我进化Agent工作流
...
- **标签：** `marketplace`, `webui`, `plugin`, `community`
...
- **标签：** `dsh-plugin`, `plugin-creator`, `cordis`
- **推荐理由：** 用于构建和验证DSH/Cordis插件的复用技能。
...
- **标签：** `package`, `skills`, `mcp`, `database`
- **推荐理由：** 一站式AI资产包：42技能+5工具+数据库集成。
...
- **标签：** `agent-tools`, `capability-check`, `plugin-library`
- **推荐理由：** Agent插件库起飞前检查单，提升利用率可预期性
...
- **标签：** `deepseek-harness`, `sidebar`, `ui-extension`
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话/Git视图。
...
- **标签：** `taskboard`, `skills`, `deepseek-harness`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH集成。
...
- **标签：** `deepseek-harness`, `desktop`, `dsh`, `tauri`
- **推荐理由：** 轻量Tauri桌面应用，零配置且内置预设插件
...
- **标签：** `deepseek-harness`, `floating-widget`, `plugin`
- **推荐理由：** 可爱漂浮鲸鱼挂件，实时监控DeepSeek账户余额
...
- **标签：** `desktop-app`, `dsh-plugin`, `electron`, `local-first`
- **推荐理由：** 本地优先的DSH桌面工作空间，支持文件和办公产物
...
- **标签：** `desktop`, `multi-model`
- **推荐理由：** 本地优先的多模型桌面应用，隐私安全
...
- **标签：** `skills`, `mcp`, `manager`, `ui`
- **推荐理由：** Web界面技能与MCP管理器，集成于设置页。
...
- **⭐ 星标：** 17
- **语言：** HTML
- **描述：** DSH 内测收官合影墙：GitHub OAuth 零权限��录 + 冻结白名单校验的拍立得合影站（含 DSH Skill 包装）
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 趣味社区合影墙，支持OAuth登录
...
- **标签：** `desktop`, `docker`
- **推荐理由：** 集成SSH、Docker、数据库客户端的一体化工具
...
- **推荐理由：** Electron桌面版DSH，集成侧边栏和Web UI
...
- **推荐理由：** DSH Web背景个性化插件，支持预设氛围和自动保存
...
- **标签：** `memory`, `rag`, `mcp`, `local-first`
- **推荐理由：** 本地优先的可移植记忆层，跨应用持久存储
...
- **推荐理由：** 基于Markdown与Milvus的持久记忆层，支持混合搜索。
...
- **标签：** `long-term-memory`, `mcp`, `dsh-plugin`, `personal-ai`
...
- **标签：** `agent-memory`, `rag`, `embeddings`
- **推荐理由：** 将SKILL.md转为可检索的Agent技能语料库
...
- **标签：** `evaluation`, `rag`, `observability`, `benchmark`
- **推荐理由：** 基于证据的评估框架，覆盖Prompt与RAG。
...
- **标签：** `agent-os`, `loop-engineering`, `mcp`, `skills`
- **推荐理由：** 自进化Agent OS，循环工程与持续评估
...
- **标签：** `agent`, `workflow`, `loop-engineering`
- **推荐理由：** 长时间Agent控制面，支持持久跨工具工作流
...
- **标签：** `agent`, `workspace`, `multi-agent`, `skills`
- **推荐理由：** 企业级多引擎Agent工作台，支持团队协作
...
- **标签：** `task-management`, `workflow`
- **推荐理由：** 原生SQLite任务看板，支持Agent协作评审
...
- **标签：** `ai-agents`, `coding-agent`
- **推荐理由：** 全栈数学建模竞赛Skill与GitOps流水线
...
- **标签：** `anime-search`, `bangumi`, `magnet`, `deepseek-harness`
- **推荐理由：** 对话内多源搜番，显示Bangumi评分，支持磁力复制。
...
- **标签：** `ai-infra`, `skills`, `agent`, `debugging`
- **推荐理由：** AI基础设施开发专用的agent技能集合。
...
- **标签：** `pentest`, `reverse-engineering`, `security`, `ctf`
- **推荐理由：** 87个逆向工程和渗透测试技能文件。
...
- **标签：** `skills`, `reasonix`
- **推荐理由：** 将经典编码技能移植到 Reasonix Harness
...
- **标签：** `plugin-dev`, `skills`, `agent`
- **推荐理由：** 自动生成符合规范的DSH插件的开发辅助技能
...
- **标签：** `plugin-dev`, `management`
- **推荐理由：** 便捷管理已安装插件的插槽、远程和凭据
...
- **标签：** `skills`, `agent`, `dsh-better-sidebar`, `dsh-plugin`
- **推荐理由：** 游戏化技能任务派发系统，提升Agent效率。
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `tcm`, `dsh-plugin`
- **推荐理由：** 融合八字风水中医的本地确定性Agent技能。
...
- **标签：** `gongwen`, `docx`, `wps-office`, `dsh-plugin`
...
- **标签：** `dsh-plugins`, `skills-manager`, `agent-skills`
- **推荐理由：** DSH中统一加载和管理本地Agent技能的工具。
...
- **标签：** `dsh-plugin`, `skill-picker`, `composer`
- **推荐理由：** WorkBuddy风格技能选择器，快速集成技能。
...
- **标签：** `dsh-plugins`, `multi-agent`, `team-collaboration`
- **推荐理由：** 多Agent协作框架，共享工作区独立工具链。
...
- **⭐ 星标：** 19
- **语言：** Python
- **描述：** 让 AI 像资深测试工程师一样工作：面向 AI Agent 的测试工程 Skill 框架——10 Skills + 共享知识库 + 类型决���矩阵（Claude Code / dsh 等 Agent 可用）
- **标签：** `qa`, `testing`, `skills`, `playwright`
- **推荐理由：** 10个QA技能+共享知识库，覆盖测试全流程。
...
- **标签：** `game-development`, `dsh-plugin`
...
- **推荐理由：** 首个开源AI原生矢量设计工具，支持多Agent并行
...
- **标签：** `ai-design`, `design-studio`, `ppt`, `dsh-plugin`
- **推荐理由：** 原生AI设计系统，支持可视化编辑和PPT工作室
...
- **标签：** `image-generation`, `multimodal`
- **推荐理由：** 在对话中直接生成图片
...
- **推荐理由：** 基于 Playwright 的浏览器自动化插件，功能全面
...
- **标签：** `ai-agents`, `chatgpt`, `oauth`, `dsh-plugin`
- **推荐理由：** 通过OAuth在DSH内集成ChatGPT功能
...
- **标签：** `mcp-server`, `resume-builder`, `dsh-plugin`
- **推荐理由：** 注重隐私的开源简历构建工具。
...
- **标签：** `routing`, `dsh-plugin`, `cordis`
- **推荐理由：** 任务感知路由套件，引导Agent工作流必备
...
- **标签：** `preset`, `dsh-plugin`, `standard`
- **推荐理由：** 两阶段配置达成Project2 98/99标准工具对齐
...
- **标签：** `deepseek-harness-plugin`, `dsh-plugin`, `context`
- **推荐理由：** DeepSeek Harness最佳上下文管理与仪表盘工具
...
- **标签：** `marketplace`, `plugin`, `web-gui`, `nodejs`
...
- **标签：** `utility`, `mcp`, `management`
- **推荐理由：** 统一管理 MCP 工具与技能的暴露级别，实用性强
...
- **标签：** `registry`, `plugin-dev`, `infrastructure`
- **推荐理由：** 官方插件注册中心+开发引导，生态基建必备
...
- **标签：** `dsh-plugins`, `containers`, `infrastructure`
- **推荐理由：** 容器化运行多版本DSH，支持嵌入式UI。
...
- **标签：** `mcp`, `config-sync`, `dsh-skill`, `agent-config`
- **推荐理由：** 一键同步MCP/配置跨7+平台AI Agent工具。
...
- **⭐ 星标：** 25
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token ��本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `audit`, `dsh-plugin`
- **推荐理由：** 审计上下文注入token，检测重复与冲突指令。
...
- **标签：** `upgrade`, `plugin`, `automation`
- **推荐理由：** 插件随DSH版本自动升级，保持兼容。
...
- **标签：** `data-visualization`, `token-usage`
- **推荐理由：** 实时Token用量与费用追踪面板
...
- **标签：** `utility`, `plugin-management`, `compatibility`
- **推荐理由：** 插件冲突诊断工具，解决版本兼容问题
...
- **标签：** `deepseek-harness`, `ecosystem-radar`, `plugin-registry`, `dsh-plugin`
- **推荐理由：** 自动发现15900+插件并基于k8s实测验证
...
- **标签：** `awesome-list`, `deepseek-harness`, `plugin-ecosystem`, `dsh-plugin`
...
- **标签：** `awesome-list`, `bilingual`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 双语DSH插件扩展精选列表及验证参考
...
- **标签：** `directory`, `discovery`
- **推荐理由：** 每日精选插件目录，含排行和图文导览
...
- **标签：** `awesome-list`, `developer-tools`
- **推荐理由：** DSH终极指南与精选资源合集
...
- **标签：** `plugin-directory`, `marketplace`
- **推荐理由：** 含11,000+插件的可搜索大市场
...
- **⭐ 星标：** 159
- **语言：** Python
- **描述：** Academic Paper Search：���文科研用户的 Codex / Claude Code / DeepSeek Harness Skill + MCP；跨 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC 检索去重，支持 MeSH、引用核验、引文图谱、试验检索与审计导出。
- **标签：** `academic-search`, `arxiv`, `pubmed`, `mcp`, `citation`
- **推荐理由：** 跨数据库学术检索，支持引文核验与图谱分析。
...
- **标签：** `research`, `latex`, `arxiv`, `gpu-cluster`, `experiment-tracking`
- **推荐理由：** 一站式科研平台：LaTeX写作、文献管理、GPU SSH编排。
...
- **标签：** `research`, `agent-skills`, `marketing`
- **推荐理由：** 88个技能覆盖深度研究、社交情报与营销工作流
...
- **标签：** `api`, `research`, `chinese-api`
- **推荐理由：** 一键搜索20个中国开放平台6万+API文档
...
- **标签：** `kline`, `finance`, `analysis`, `dsh-plugin`
- **推荐理由：** 交互式K线分析，支持多市场指标与技术分析。
...
- **标签：** `awesome-list`, `stock`, `finance`, `quant`
- **推荐理由：** 中英双语股票金融插件精选，24个已验证条目
...
- **推荐理由：** GitHub topic下1000+ DSH工具按星排序全量清单
...
- **标签：** `ai-agents`, `coding-agents`, `mcp`, `awesome-list`
- **推荐理由：** AI托管小时更新190+插件列表
...
- **标签：** `agent-harness`, `coding-agents`, `curated-list`, `ui`, `skills`
- **推荐理由：** 覆盖模型工具沙箱UI技能的全栈插件目录
...
- **标签：** `deepseek-harness`, `plugins`
- **推荐理由：** 基于证据筛选的有用DSH插件短名单
...
- **标签：** `armor-breaking`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** DeepSeek专用破甲生成插件
...
- **标签：** `deepseek-harness-plugin`, `plugin-marketplace`
- **推荐理由：** DeepSeek Harness插件市场资源
...
- **标签：** `awesome-list`, `cordis`, `curated`
- **推荐理由：** cordis.run官方维护的自动生成插件列表。
...
- **标签：** `marketplace`, `plugin-market`, `dsh-plugin-bundle`
