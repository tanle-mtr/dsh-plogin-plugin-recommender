...
- **标签：** `harness`, `framework`, `core`
- **推荐理由：** DSH插件生态核心框架，支持AI插件化开发
...
- **标签：** `agents`, `harness`, `skills`
- **推荐理由：** 自进化多Agent框架，支持记忆和技能
...
- **标签：** `coding`, `terminal`, `agent`
- **推荐理由：** 终端原生AI编程Agent，前缀缓存稳定
...
- **标签：** `agent-harness`, `workspace`, `no-code`, `mcp`
- **推荐理由：** 跨设备统一Agent工作空间与任务管理工具。
...
- **标签：** `routing`, `injector`, `agent-workflow`
...
- **标签：** `agent-ops`, `loop-engineering`, `workflow-automation`
- **推荐理由：** 跨Harness长周期Agent编排控制面。
...
- **标签：** `bootstrap`, `preset`, `standard`
- **推荐理由：** 两阶段引导预设，对齐AI行为标准
...
- **标签：** `virtual-terminal`, `sandbox`, `agent-tools`
- **推荐理由：** AI代理虚拟终端，首创沙箱环境
...
- **标签：** `agent-teams`, `multi-agent`, `harness`
- **推荐理由：** DSH内多代理团队协作能力插件
...
- **推荐理由：** DSH插件Hub的CLI工具和解析器，适合插件开发与管理。
...
- **标签：** `plugin-discovery`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 自动搜索安装验证DSH插件的Skill，省去手动查找
...
- **推荐理由：** AI基础设施开发专用Skill，支持代码编写与调试
...
- **标签：** `skills`, `coding`
- **推荐理由：** Superpowers编码技能移植到Reasonix框架
...
- **标签：** `skills`, `manager`, `harness`
- **推荐理由：** 统一加载和安全管理DSH本机Agent Skills
...
- **标签：** `plugin-creator`, `deepseek-harness`, `coding`
- **推荐理由：** 可复用插件开发工具，一键构建验证DSH插件
...
- **标签：** `codex`, `sync`, `mcp`, `migration`
- **推荐理由：** OpenAI Codex与DSH双向桥接，支持MCP镜像同步
...
- **标签：** `troubleshooting`, `plugin-manager`
- **推荐理由：** 诊断并解决插件与版本冲突
...
- **推荐理由：** 在DSH代理内直接搜索npm和插件目录，提升可发现性。
...
- **标签：** `chatgpt`, `oauth`, `subscription`, `coding-agents`
- **推荐理由：** 通过OAuth绑定ChatGPT订阅账户，在DSH内使用付费模型。
...
- **⭐ 星标：** 2,447
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh���装
- **标签：** `widget`, `balance`, `whale`
- **推荐理由：** 可爱鲸鱼小部件，监控DeepSeek账户余额
...
- **推荐理由：** 原生iOS客户端，通过WebSocket连接Harness，随时随地管理会话和Agent轨迹。
...
- **推荐理由：** 类CodePilot风格的跨平台桌面客户端，为DSH提供更优雅的UI体验。
...
- **标签：** `vision`, `multimodal`, `claude-code`, `deepseek-harness`
...
- **⭐ 星标：** 163
- **语言：** TypeScript
- **描述：** 开源版锤子便签，复刻锤科美学，��键Docker私有化部署，支持skill调用，支持dsh plugin，支持多租户，一键生成公众号格式，支持导出便签为图片
- **标签：** `notes`, `dsh-plugin`, `docker`
- **推荐理由：** 开源锤子便签复刻，Docker私有化部署，支持skill调用
...
- **标签：** `mcp`, `ui`, `skill-management`
- **推荐理由：** Web界面skill与MCP管理工具，可视化操作
...
- **推荐理由：** 一站式DevOps桌面，集成数据库SSH Docker AI
...
- **推荐理由：** Electron桌面封装集成DSH与ModLens
...
- **标签：** `knowledge`, `rag`, `wiki`
- **推荐理由：** 开源LLM知识平台，支持RAG和Wiki
...
- **标签：** `agent-memory`, `rag`, `token-savings`, `skill-reuse`
...
- **标签：** `memory`, `milvus`, `search`
- **推荐理由：** 基于Milvus的统一Agent记忆层
...
- **推荐理由：** 三层自动 consolidation 的主动联想记忆系统
...
- **标签：** `agents`, `swarm`, `workflow`
- **推荐理由：** 多Agent蜂群编排，自动化工作流协同
...
- **标签：** `skills`, `distill`, `agents`
- **推荐理由：** ���炼Agent思维为可复用技能库
...
- **标签：** `skills`, `marketplace`, `automation`, `coding`
- **推荐理由：** 最全Skills商店，自动抓取GitHub技能包分类整理。
...
- **标签：** `cognition`, `inference`, `reasoning`
- **推荐理由：** 模型无关推理控制，支持验证和恢复
...
- **标签：** `video-generation`, `agent-skill`, `automation`
- **推荐理由：** 一键生成Vox风格解说视频全流程
...
- **标签：** `agent-observability`, `computer-vision`, `video-analysis`
- **推荐理由：** 赋予AI视觉、音频和屏幕监控能力，生成可验证的时间戳证据。
...
- **标签：** `creative-writing`, `novel-writing`, `fiction-writing`
- **推荐理由：** 内置小说、短剧、游戏和视频解说四大创作工作台，支持网文写作流程。
...
- **推荐理由：** 在对话内集成iOS模拟器和真机控制，22个Agent工具实现全流程自动化。
...
- **推荐理由：** 全栈数学建模Skill，支持国赛美赛，带GitOps流水线
...
- **标签：** `agent-review`, `prompt-engineering`, `deepseek-harness`
- **推荐理由：** 可组合可审查的Agent工作模式，实现自进化
...
- **标签：** `agent-skill`, `ai-coding`, `mentor`
- **推荐理由：** AI编程导师，扫描项目架构并生成阶梯式章节引导手写代码
...
- **标签：** `skills`, `agent`, `deck`
- **推荐理由：** 侧边栏内置25个工程与效率技能，开箱即用
...
- **标签：** `spec`, `development`, `agent-skill`
- **推荐理由：** 仓库原生规格驱动开发可移植技能
...
- **推荐理由：** 11项测试Skill+共享知识库，AI测试工程师框架
...
- **推荐理由：** WorkBuddy风格Skill选择器，快速插入技能指令
...
- **推荐理由：** 数学建模竞赛多Agent方案，33个模型+论文模板
...
- **推荐理由：** AI智能推荐的插件库，190+插件按场景分类，每小时更新。
...
- **标签：** `design`, `ui`, `visual`
- **推荐理由：** 开源Claude Design替代方案，AI设计师必备
...
- **标签：** `design`, `vector`, `vibedesign`, `agent-team`
...
- **标签：** `ai-video`, `ai-image`, `director`
- **推荐理由：** 集成AI视频/图像/音频生成及分镜功能，创作利器
...
- **推荐理由：** 电影感视频制作，152张镜头配方卡+Remotion模板
...
- **标签：** `browser`, `extension`, `prompts`
- **推荐理由：** Gemini、Claude、ChatGPT增强套件
...
- **推荐理由：** 提供指纹伪装、无限本地配置和安卓模拟的持久化浏览器自动化方案。
...
- **标签：** `marketplace`, `install`
- **推荐理由：** 官方插件市场，一键安装浏览插件
...
- **推荐理由：** 一键切换Claude Code、Codex、DeepSeek等模型
...
- **推荐理由：** 将ChatGPT/Claude/Grok订阅转为DSH的LLM提供商，无需API密钥。
...
- **推荐理由：** 将表格、文档、幻灯片和关系表直接带入DSH工作空间，打造办公环境。
...
- **推荐理由：** 每日自动抓取并筛选真实插件，按场景分类和热度排行，帮你快速找到适合的插件。
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领/审查流程，纯Web UI无iframe。
...
- **推荐理由：** 涵盖插件、技能、MCP服务器、补丁和UI的全面DSH生态指南与资源列表。
...
- **推荐理由：** 实时跟踪会话/日成本、预算、Token用量和提供商余额，清晰明了。
...
- **推荐理由：** DSH终极指南，包含快速入门、精选插件和实用工具推荐。
...
- **推荐理由：** 为DSH接入Command Code模型，支持实时目录、计划感知选择及推理效率优化。
...
- **推荐理由：** 1万+插件目录，支持搜索排序与API，生态基础设施
...
- **标签：** `plugin-catalog`, `deepseek-harness`, `awesome-list`
- **推荐理由：** 近两千插件总目录，中英文搜索分类筛选，查插件必看
...
- **标签：** `remote-access`, `encryption`, `deepseek-harness`
- **推荐理由：** 端到端加密P2P远程访问，PC/手机/Web多端操作
...
- **推荐理由：** 容器隔离运行多版本DSH，支持WebView内嵌UI
...
- **标签：** `launcher`, `orchestration`, `infrastructure`
- **推荐理由：** 统一编排Runtime/Provider/Skill/MCP的可扩展平台
...
- **推荐理由：** 从Claude Code/Codex/OpenCode迁移会话和技能
...
- **标签：** `mascot`, `pixel-art`, `ui`, `animation`
- **推荐理由：** 公开动态宠物画廊，个性化Agent界面。
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一���切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
...
- **标签：** `academic-search`, `mcp`, `research`, `deepseek-harness`
- **推荐理由：** 跨源学术检索MCP，支持引用图谱与核验，科研利器
...
- **标签：** `handbook`, `deepseek-harness`, `agent-runtime`
- **推荐理由：** 全面覆盖DSH运行时、插件、MCP的手册与awesome-list
...
- **标签：** `documentation`, `examples`, `skillhub`
- **推荐理由：** 官方 SkillHub 文档与示例，适合入门研究
...
- **标签：** `prompts`, `image`, `gpt`
- **推荐理由：** GPT Image 2提示词库，530+工业案例
...
- **标签：** `traditional-wisdom`, `bazi`, `fengshui`, `dashboard`
- **推荐理由：** 八字/风水/奇门/中医传统智慧AI技能面板
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产品���明书与导航：按场景选插件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `dsh-plugin-directory`
- **推荐理由：** 面向用户的插件说明书，覆盖安装、权限和兼容性。
...
- **推荐理由：** DeepSeek Harness的类CodeX插件市场，方便发现和安装。
...
- **标签：** `catalog`, `awesome-list`, `dsh-plugin-directory`
- **推荐理由：** 自动化每日更新的双语插件目录，持续跟踪新插件。
...
- **标签：** `plugin-registry`, `awesome-list`, `2origin`
- **推荐理由：** 数据驱动的插件雷达图，含2Origin实验实验室。
...
- **标签：** `static-site`, `web`, `plugin-portal`
- **推荐理由：** 零依赖静态网页门户，支持筛选浏��227+插件。
...
- **推荐理由：** 已迁移至新仓库，原awesome-dsh插件列表存档。
