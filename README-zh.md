...
- **标签：** `agentic-framework`, `multi-agent`, `swarm`, `mcp-server`
- **推荐理由：** 原始agent元harness，支持多智能体集群与自主工作流
...
- **标签：** `ai-agent`, `multi-agent`, `mcp`, `skills`
- **推荐理由：** 开源超级AI助手，多模型支持，具备记忆与自��化能力
...
- **标签：** `coding-agent`, `deepseek`, `tui`, `tool-use`
- **推荐理由：** DeepSeek原生终端编码agent，支持���缀缓存稳定性
...
- **标签：** `harness`, `tutorial`, `beginner`
- **推荐理由：** 从零学习DSH插件开发的最佳入门教程
...
- **标签：** `orchestration`, `agent-workspace`, `multi-device`
- **推荐理由：** 跨平台统一智能体编排平台，一站式管理
...
- **标签：** `routing`, `agent-control`, `harness`
...
- **标签：** `model-switch`, `cli`, `dsh-plugin`
- **推荐理由：** 一键安装切换Claude/Codex/Grok/DeepSeek等多模型
...
- **标签：** `agent-runtime`, `desktop`, `rust`, `dsh-plugin`
...
- **标签：** `plugin-dev`, `codex`
- **推荐理由：** 构建和验证DSH/Cordis插件的复用Skill
...
- **标签：** `web-ui`, `plugin-ecosystem`, `distribution`
...
- **标签：** `taskboard`, `ui-extension`, `dsh-plugin`
...
- **标签：** `tui`, `ui-extension`, `dsh-plugin`
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏/实时状态/流式思考
...
- **推荐理由：** 开源Windows桌面客户端，支持SSH和移动端远程控制
...
- **推荐理由：** 在对话中运行iOS模拟器和真机，含22个自动化工具
...
- **推荐理由：** Web界面技能/MCP管理面板，提升DSH操作体验。
...
- **标签：** `desktop`, `electron`, `ui`
- **推荐理由：** Electron桌面控制台：多标签管理Harness/插件/MCP/技能
...
- **标签：** `mcp`, `skill`, `ui`, `dsh-plugin`
- **推荐理由：** Web GUI插件，在浏览器中管理技能和MCP服务器
...
- **标签：** `bioinformatics`, `desktop`, `llm-agent`
- **推荐理由：** 面向临床医生的生信桌面Agent，支持离线模式
...
- **推荐理由：** 集成数据库、SSH、Docker与AI沙箱的一体化桌面
...
- **标签：** `desktop`, `electron`, `harness`
- **推荐理由：** Electron桌面壳集成多款DSH工具
...
- **标签：** `background`, `ui`, `webui`
- **推荐理由：** 可自定义Web背景，支持预设与压缩
...
- **标签：** `knowledge-cards`, `ui`, `shareable`
...
- **标签：** `rag`, `knowledge-base`, `vector-search`, `multi-tenant`
- **推荐理由：** LLM知识平台，将文档转化为RAG、推理Agent和Wiki
...
- **标签：** `skill-routing`, `vector-search`, `embeddings`
- **推荐理由：** 将SKILL.md转化为可检索的向量知识库，支持评测
...
- **标签：** `low-code`, `crm`, `erp`, `workflows`, `ai-agent`
- **推荐理由：** AI+无代码平台，快速构建CRM、ERP及内部管理系统
...
- **标签：** `agent-skills`, `workflow`, `vibe-coding`, `dsh-plugin`
- **推荐理由：** 智能技能路由与流程编排，奖励提升21pp
...
- **标签：** `research`, `web-search`, `ai-agent`
- **推荐理由：** 自动搜索中文8大平台并生成研究报告
...
- **标签：** `video`, `agent-skills`, `automation`
- **推荐理由：** 端到端自动生成Vox风格解说视频
...
- **推荐理由：** SQLite原生任务看板，支持Agent认领与审核流程
...
- **推荐理由：** 为Claude Code增加多模态视觉能力，支持截图和UI分析。
...
- **标签：** `ai-infra`, `skills`
- **推荐理由：** AI基础设施开发专用技能集，覆盖编写优化调试全流程。
...
- **推荐理由：** 将超级powers技能适配至DeepSeek，覆盖TDD和调试。
...
- **推荐理由：** 87个逆向工程与授权渗透测试技能文件，安全研究必备。
...
- **推荐理由：** AI编程导师技能：扫描项目、生成阶梯章节、代码审查
...
- **推荐理由：** 将Matt Pocock工程技能套接入DSH的实用工作流
...
- **标签：** `spec-driven`, `agent-skills`
- **推荐理由：** 支持仓库级规范驱动开发的便携式Skill
...
- **标签：** `skill`, `dsh-plugin`, `codex`
- **推荐理由：** Codex技能适配版，用于DSH代码简化工作流
...
- **标签：** `skill`, `dsh-plugin`, `ui`
- **推荐理由：** WorkBuddy风格技能选择器，快速插入官方技能
...
- **标签：** `design`, `skill`, `ui`, `dsh-plugin`
- **推荐理由：** 设计美学技能包，6种风格避免AI生成感
...
- **标签：** `qa`, `skill`, `testing`, `dsh-plugin`
- **推荐理由：** 10+测试技能加共享知识库，AI辅助质量保障
...
- **标签：** `godot`, `game-dev`, `skill`, `dsh-plugin`
...
- **标签：** `media`, `vision`, `skill`, `dsh-plugin`
- **推荐理由：** 免费图片读取生成，支持自动视觉转码
...
- **标签：** `video`, `image`, `skill`, `dsh-plugin`
...
- **标签：** `skillhub`, `api-docs`, `skills`
- **推荐理由：** SkillHub官方API文档与DSH插件示例
...
- **标签：** `skill`, `plugin-development`, `dsh`
- **推荐理由：** 指导AI Agent从零开发、测试DSH插件
...
- **标签：** `agent-skills`, `ai-design`, `figma-alternative`, `ui-generator`
- **推荐理由：** 开源本地优先设计工具，支持AI生成UI与设计系统
...
- **标签：** `agent-skills`, `architecture-diagram`, `mermaid-alternative`, `diagrams-as-code`
- **推荐理由：** AI生成架构图、流程图和时序图的可视化技能
...
- **标签：** `ai-image-generation`, `prompt-engineering`, `skills`, `workflow-automation`
- **推荐理由：** 工业级GPT图像生成提示词引擎，含530+案例与20+技能模板
...
- **标签：** `mascot`, `pixel-art`, `ui`, `pets`
- **推荐理由：**  animated宠物画廊，增强开发者智能体体验
...
- **标签：** `image-generation`, `multimodal`, `seedream`
- **推荐理由：** 在DSH对话中直接使用多种AI模型生成图片
...
- **推荐理由：** 对话内直接预览编辑OpenPencil设计稿，设计工作流利器。
...
- **标签：** `image-reading`, `ocr`
- **推荐理由：** 为文本模型提供像素级图像识别与OCR能力
...
- **标签：** `content-discovery`, `cross-platform`, `privacy-first`, `dsh-plugin`
- **推荐理由：** 本地优先跨平台内容发现Agent，覆盖多平台
...
- **推荐理由：** Tabbit浏览器自动化插件，集成DeepSeek Harness
...
- **标签：** `dingtalk`, `chat-bot`, `im-channel`
- **推荐理由：** 钉钉IM通道，支持流式机器人和智能体功能
...
- **标签：** `electron-app`, `image-uploader`, `s3`, `github`
- **推荐理由：** 全能图片上��工具，支持60+图床，兼容主流编辑器
...
- **标签：** `infrastructure`, `discovery`, `plugin`
- **推荐理由：** 生态雷达自动发现验证15900+插件
...
- **标签：** `catalog`, `awesome-list`, `plugins`
- **推荐理由：** DSH生态精选插件目录
...
- **标签：** `breaking`, `utility`, `deepseek`
- **推荐理由：** 破甲插件扩展DeepSeek能力边界
...
- **推荐理由：** 通过OAuth使用订阅账号作为DSH的LLM提供商
...
- **标签：** `context-migration`, `utility`
- **推荐理由：** 跨预设会话迁移工具，完整保留上下文状态。
...
- **标签：** `remote`, `p2p`, `security`
- **推荐理由：** 安全P2P远程访问方案，支持PC/Android/Web多端操控。
...
- **标签：** `notes`, `productivity`
- **推荐理由：** 开源锤子便签复刻版，支持Docker部署和公众号导出。
...
- **标签：** `dsh-box`, `management`
- **推荐理由：** 多版本DSH隔离运行与WebView管理工具
...
- **标签：** `skills-mgr`, `utilities`
- **推荐理由：** 统一管理本地Agent Skills的安全加载器
...
- **标签：** `package-manager`, `skill`, `mcp`, `dsh-plugin`
- **推荐理由：** 42技能+5CLI+1插件的Agent开发资源管理器
...
- **标签：** `dashboard`, `cost-tracking`, `token-usage`
- **推荐理由：** 实时Token用量、成本与API余额监控面板
...
- **标签：** `encyclopedia`, `ai-resources`, `knowledge-base`
- **推荐理由：** 全维度AI资源百科，覆盖MCP、RAG、学习路线等
...
- **标签：** `research`, `arxiv`, `latex`
- **推荐理由：** 一站式科研平台，涵盖论文写作、文献管理与实验追踪。
...
- **标签：** `api`, `research`, `chinese-platforms`
- **推荐理由：** 在Agent内直接搜索20个中国开放平台6.5万+篇API文档
...
- **标签：** `patent`, `research`, `legal-tech`
- **推荐理由：** 专利侵权分析系统，输入专利号生成竞品侵权报告
...
- **标签：** `awesome-list`, `plugin-directory`, `dsh-ecosystem`
- **推荐理由：** 全面的DSH插件目录，含验证功能
...
- **标签：** `ai-agent`, `plugin-registry`, `2origin`
- **推荐理由：** DSH生态插件雷达与探索实验室
...
- **标签：** `discoverability`, `search`, `agent-tool`
- **推荐理由：** 在DSH内部搜索npm和插件列表
...
- **语言：** Unknown
- **描述：** DSH插件
- **标签：** `plugin-collection`, `dsh-plugins`
- **推荐理由：** DSH插件资源集合
...
- **标签：** `auto-updating`, `data-driven`, `bilingual`
- **推荐理由：** 自动更新的插件目录，含双语README
...
- **标签：** `curated`, `typescript`, `plugin-directory`
- **推荐理由：** 覆盖模型、工具、沙箱和UI的插件指南
...
- **标签：** `evidence-aware`, `shortlist`, `useful-plugins`
- **推荐理由：** 基于证据的实用DSH插件精选列表
...
- **⭐ 星标：** 164
- **语言：** TypeScript
- **描述：** DeepSeek Harness 搜番插件：对话内多源搜索番剧，卡片展示 Bangumi 评分���详情，支持复制磁力。
- **标签：** `anime`, `torrent`, `bangumi`
- **推荐理由：** 对话内多源搜索番剧，展示Bangumi评分，支持磁力链接。
