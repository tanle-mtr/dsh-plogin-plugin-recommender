...
- **标签：** `dsh-plugin`, `cordis`, `routing`, `ai-agents`
- **推荐理由：** 注入器+路由套件，支持任务感知推理模式选择。
...
- **标签：** `agent-collaboration`, `multi-agent`, `dsh-plugin`, `visual-editor`
- **推荐理由：** 企业级多代理工作台，统一多种Harness引擎。
...
- **标签：** `agent-os`, `mcp`, `llm-orchestration`, `loop-engineering`, `skill`
- **推荐理由：** 自进化Agent操作系统，带面试级评估和预算循环。
...
- **推荐理由：** 首个视觉插件，将图像转化为结构化JSON��赋能文本编码Agent
...
- **标签：** `preset`, `deepseek-harness`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 两阶段预设：极简引导后加载完整工具链，部署稳定可靠
...
- **标签：** `multi-agent`, `switch`, `dsh-plugin`, `coding-agent`
- **推荐理由：** 一键安装并切换Claude Code、Codex、DSH等9大编码Agent平台
...
- **标签：** `agent-runtime`, `rust`, `dsh-plugin`, `coding-agent`
...
- **标签：** `agent`, `dsh-plugin`, `agent-teams`
- **推荐理由：** 在DeepSeek Harness中实现多代理团队协调。
...
- **⭐ 星标：** 874
- **语言：** TypeScript
- **描述：** [dsh]为纯��本模型设计更强大的视觉工具箱：一行安装使用、粘贴图片直接识别、多张图片问答、截图到前端UI 还原等｜DeepSeek Harness-native integration for agent-vision-toolkit: image Q&A, long-screenshot OCR, UI restoration, grounding, pixel diff, Artifacts, and Web UI.
- **标签：** `vision`, `ocr`, `dsh-plugin`
- **推荐理由：** 为纯文本模型提供强大的视觉工具和OCR支持。
...
- **推荐理由：** 支持子代理、RPC、MCP集成的终端编码Agent
...
- **标签：** `provider`, `model-access`, `plan-mode`
- **推荐理由：** 新增Command Code模型接入，支持动态选型与推理效率优化。
...
- **标签：** `ai-infra`, `coding`, `skills`, `optimization`
- **推荐理由：** 专注AI基础设施开发的高性能系统编写与调试技能。
...
- **标签：** `coding`, `tdd`, `debugging`, `skills`
- **推荐理由：** 改编自obra/superpowers的TDD和调试技能
...
- **标签：** `claude-code`, `migration`, `deepseek-harness`
- **推荐理由：** 迁移向导，将Claude Code/Codex会话和技能导入DSH。
...
- **标签：** `diagnostics`, `plugin-management`, `dsh-plugin`
- **推荐理由：** 检测和解决插件间及版本冲突问题
...
- **推荐理由：** 通过OAuth在DSH内绑定ChatGPT订阅账号
...
- **标签：** `mascot`, `pixel-art`, `sprites`, `dsh-plugin`, `cli`
- **推荐理由：** 动态宠物画廊，增强多CLI工具的视觉体验。
...
- **推荐理由：** DSH内置插件市场，支持浏览搜索和一键安装，极大提升体验
...
- **标签：** `sidebar`, `deepseek-harness`, `dsh-plugin`, `ui`
...
- **标签：** `taskboard`, `ui`, `dsh-plugin`, `cli`
- **推荐理由：** 现代化可嵌入任务面板，灵活适配Codex和DSH
...
- **标签：** `tui`, `terminal`, `dsh-plugin`, `ui`
- **推荐理由：** 鲸鱼主题TUI，支持实时状态、流式思考，官方推荐
...
- **推荐理由：** 萌趣浮动小鲸鱼 widget，实时跟踪DeepSeek账户余额
...
- **标签：** `desktop`, `electron`, `local-first`, `ai-workbench`
- **推荐理由：** 本地优先桌面工作区，支持项目、文件、插件管理
...
- **标签：** `desktop`, `windows`, `gui`, `ssh`, `codex`
- **推荐理由：** Windows桌面GUI，支持Codex、SSH、移动端远程访问
...
- **标签：** `desktop`, `tauri`, `multi-model`, `local-first`
- **推荐理由：** 本地优先Tauri桌面应用，支持多模型和自进化技能
...
- **标签：** `office`, `univer`, `productivity`
- **推荐理由：** 为Harness带来完整办公套件能力，表格文档幻灯片齐全
...
- **推荐理由：** 跨平台桌面客户端，仿CodePilot风格，带插件套装
...
- **标签：** `ios`, `mobile`, `kotlin-multiplatform`
- **推荐理由：** 原生iOS客户端，将Harness工作区和Agent轨迹带入手机
...
- **推荐理由：** 端到端加密P2P远程访问，支持PC/Android/Web多端。
...
- **标签：** `notes`, `docker`, `smartisan`, `im-integration`
- **推荐理由：** 复刻锤子便签，Docker私有化，支持IM推送与skill调用。
...
- **标签：** `marketplace`, `ui`, `plugins`, `webui`
- **推荐理由：** DSH Web界面一键安装社区插件的市场
...
- **标签：** `ui`, `status-bar`, `utility`
- **推荐理由：** 底部常驻信息栏，实时显示用量与价格，实用。
...
- **标签：** `deepseek-harness`, `mcp`, `ui`
- **推荐理由：** 在Web设置页面管理技能与MCP服务器的浏览器插件。
...
- **推荐理由：** 集成数据库、SSH、SFTP、Docker的一站式开发运维桌面工具
...
- **标签：** `electron`, `desktop-app`, `modlens`
- **推荐理由：** Electron桌面应用，整合DSH Web服务和ModLens
...
- **标签：** `agent-memory`, `rag`, `mcp`, `skills`, `long-term-memory`
- **推荐理由：** 可移植的本地优先记忆层，Markdown原生存储。
...
- **标签：** `memory`, `rag`, `skills`, `mcp`, `token-savings`
...
- **标签：** `agent-memory`, `agentic-rag`, `memory`, `context-engineering`
- **推荐理由：** 面向有状态代理的记忆库，支持Agent驱动RAG。
...
- **标签：** `memory`, `rag`, `dsh-plugin`, `agent`
- **推荐理由：** 专为Agent设计的记忆管理套件，支持RAG持久化知识库
...
- **标签：** `memory`, `milvus`, `rag`, `dsh-plugin`
- **推荐理由：** 基于Milvus的持久统一记忆层，支持所有AI Agent
...
- **标签：** `memory`, `mcp`, `dsh-plugin`
- **推荐理由：** 个人AI代理，为所有工具提供共享持久记忆。
...
- **标签：** `skill`, `memory`, `dsh-plugin`
- **推荐理由：** 将SKILL.md文件转换为可检索的代理技能库。
...
- **标签：** `evaluation`, `rag`, `knowledge-engineering`
- **推荐理由：** 基于证据的评估框架，支持提示词、RAG、技能与代理评测。
...
- **标签：** `agent-orchestration`, `skills`, `mcp`, `no-code`, `local-first`
- **推荐理由：** 跨设备统一代理工作空间，看板式任务管理。
...
- **标签：** `agent-control-plane`, `workflow-automation`, `long-horizon`, `dsh-plugin`
- **推荐理由：** 长期代理控制面，支持持久化的多代理工作编排。
...
- **标签：** `skills`, `workflow`, `dsh-plugin`, `agent-framework`
- **推荐理由：** 智能技能路由与工作流编排，显著提升奖励和效率
...
- **标签：** `cognitive`, `skills`, `dsh-plugin`, `ai-agent`
...
- **标签：** `ai-video`, `skill`, `dsh-plugin`
- **推荐理由：** 端到端AI视频生成自动化工作流。
...
- **标签：** `ai-writing`, `dsh-plugin`, `novel`
- **推荐理由：** 有组织AI小说创作软件，可控创作流程。
...
- **标签：** `skill`, `computer-vision`, `ocr`, `multimodal`
- **推荐理由：** 将视频、音频和屏幕活动转化为可验证证据的技能
...
- **标签：** `math-modeling`, `competition`, `automation`
- **推荐理由：** 全自动数学建模竞赛Skill，支持国赛美赛，GitOps流水线
...
- **标签：** `skills`, `agent`, `codex`, `claude-code`
- **推荐理由：** 将经验转化为可重复执行的技能，跨多平台兼容。
...
- **标签：** `finance`, `stock`, `multi-agent`, `mcp`
- **推荐理由：** 基于tushare的金融多Agent分析，自动生成MCP接口。
...
- **标签：** `skill-generator`, `agent-learning`, `automation`
...
- **标签：** `fintech`, `backtesting`, `investment`, `mcp`
- **推荐理由：** 合规优先的AI投研工作台，支持多策略回测
...
- **标签：** `agent`, `skill`, `mentor`
- **推荐理由：** AI编程导师，引导手写核心逻辑并自动判题。
...
- **标签：** `document`, `gongwen`, `office`
- **推荐理由：** 国标公文全流程处理，格式检查与模板生成。
...
- **标签：** `spec`, `development`, `workflow`
- **推荐理由：** 基于DSH工程的规格驱动开发技能。
...
- **标签：** `ai-agent`, `chinese-traditional-wisdom`, `skill`
- **推荐理由：** 八字风水中医融合，本地确定性引擎，特色鲜明。
...
- **标签：** `ocr`, `vision`, `image-reading`, `skill`
- **推荐理由：** 让纯文本模型具备图像识别能力，实用性强。
...
- **标签：** `multi-agent`, `collaboration`, `skill`
- **推荐理由：** 多智能体协作框架，支持独立模型与共享工作区。
...
- **标签：** `coding`, `skill`, `code-simplification`
- **推荐理由：** 代码简化技能，源自Codex，适合DSH编码场景。
...
- **标签：** `video`, `ai-creation`, `skill`
- **推荐理由：** AI视频图像音频全流程，含分镜功能，创意工具。
...
- **标签：** `roleplay`, `narrative`, `skill`
- **推荐理由：** 互动叙事RPG技能，记忆系统+随机剧情，沉浸感强。
...
- **标签：** `deepseek-harness`, `engineering-discipline`, `quality-gate`
- **推荐理由：** 工程纪律Bundle，检查需求、测试实现、确保交付质量。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，为DSH代理提供开发支持。
...
- **标签：** `agent-skills`, `remotion`, `video`
- **推荐理由：** 电影感产品视频Agent，含152张镜头配方卡与Remotion模板。
...
- **推荐理由：** 开发DSH插件的Agent技能，涵盖服务/事件/工具/适配器标准。
...
- **标签：** `diagrams-as-code`, `architecture`, `mermaid-alternative`
- **推荐理由：** 从代码生成精美架构与序列图，可视化管理
...
- **标签：** `design`, `vibedesign`, `ui`, `mcp`, `flutter`, `react`
...
- **标签：** `design`, `dsh-plugin`, `visual`
- **推荐理由：** AI原生设计工作室，支持可编辑模板和PPT。
...
- **⭐ 星标：** 56
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能��材料的图，或直接读案件材料画准一张时间轴。Claude Skill / DeepSeek Harness 通用。
- **标签：** `visualization`, `legal`, `diagram`
- **推荐理由：** 诉讼可视化工具，生成时间轴和案情图。
...
- **推荐理由：** 6种设计风格避免AI感，让vibe-coded网站更有质感。
...
- **标签：** `content-discovery`, `bilibili`, `dsh-plugin`, `ai-agent`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、小红书、YouTube等多个平台
...
- **⭐ 星标：** 1,462
- **语言：** Python
- **描述：** DSH Plugin Radar — 开源可自部署的 DSH 插件生态雷达：��动发现 15900+ 候选、k8s 运行级实测管线；自动索引可用Plugin List
- **标签：** `plugin`, `ecosystem`, `dsh-plugin`
- **推荐理由：** 自部署插件雷达，自动发现超15900��选插件。
...
- **标签：** `security`, `redteam`, `audit`, `cybersecurity`
...
- **标签：** `provider`, `oauth`, `subscription`, `utility`
- **推荐理由：** 通过OAuth使用ChatGPT/Claude/Grok订阅作为DSH提供商
...
- **标签：** `task-management`, `project-board`, `sqlite`
- **推荐理由：** 原生任务看板，SQLite存储，无iframe的整洁项目管理界面
...
- **推荐理由：** 实时追踪会话成本、日预算和多提供商Token使用情况
...
- **标签：** `plugin-catalog`, `awesome-list`, `directory`
- **推荐理由：** 收录近2000个DSH插件，支持中英文搜索和分类筛选。
...
- **标签：** `handbook`, `documentation`, `runtime`, `sandbox`
- **推荐理由：** 173条资源的全能手册，覆盖运行时与插件开发。
...
- **推荐理由：** 精选DSH插件Awesome列表，便于快速发现优质插件。
...
- **标签：** `backup`, `config`, `migration`, `sync`
- **推荐理由：** 支持WebDAV的全量配置备份还原工具
...
- **标签：** `mcp`, `tools`, `configuration`, `menu`
...
- **标签：** `context`, `utility`, `ui`, `audit`
- **推荐理由：** 上下文审计工具，检测重复冲突，优化token成本。
...
- **标签：** `infra`, `container`, `management`
- **推荐理由：** 容器隔离多版本管理，嵌入式WebView体验佳。
...
- **标签：** `data-visualization`, `token-usage`, `deepseek-api`
- **推荐理由：** 实时Token用量统计、成本估算和API余额监控
...
- **标签：** `tts`, `xiaomi-mimo`, `utility`
- **推荐理由：** 接入免费小米MiMo TTS，支持自定义声音朗读
...
- **语言：** Unknown
- **描述：** DSH插件
- **标签：** `awesome-list`, `plugin-catalog`, `dsh`
- **推荐理由：** 精选DSH插件合集，便于快速发现可用工具
...
- **标签：** `theme`, `glassmorphism`, `customization`, `transparent`
- **推荐理由：** 高自由度玻璃质感主题，支持磨砂效果和自定义壁纸
...
- **标签：** `skin`, `ocean`, `animated`, `glassmorphism`
- **推荐理由：** 动态海洋主题皮肤，含实时波浪动画和玻璃效果
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、��色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `theme`, `webui`, `customization`
- **推荐理由：** 自定义Web背景插件，支持预设和实时预览
...
- **标签：** `ai`, `llm`, `mcp`, `rag`, `vibe-coding`
- **推荐理由：** 全面的AI资源库，含教程和提示词百科。
...
- **标签：** `web-search`, `free`, `no-api-key`, `research`
- **推荐理由：** 免费无需注册的网页搜索插件，连接无原生搜索的模型
...
- **推荐理由：** 自动收录插件目录，分类推荐，帮你快速找到适合的插件
...
- **推荐理由：** 一站式科研平台：LaTeX论文、arXiv文献、GPU远程实验全管理
...
- **推荐理由：** 11000+插件市场，支持搜索排行和免费API查询
...
- **标签：** `academic-search`, `mcp`, `skills`, `citation-management`
- **推荐理由：** 跨库学术检索，支持引文图谱与核验，适合中文科研用户。
...
- **标签：** `research`, `industry`, `company`, `report`
- **推荐理由：** 行业研究工具包，含公司分析和新闻追踪
...
- **标签：** `marketplace`, `plugin-curator`, `deepseek-harness`
- **推荐理由：** 每日更新的全面插件市场，经过审核确保质量
...
- **标签：** `encyclopedia`, `resources`, `learning`, `ai-pedia`
- **推荐理由：** AI资源百科大全，涵盖LLM、Agent、RAG和学习路线
...
- **标签：** `awesome-list`, `catalog`, `official`
- **推荐理由：** cordis.run官方精选自动维护的插件目录
...
- **标签：** `awesome-list`, `dsh-plugin`, `nextjs`
- **推荐理由：** DSH插件公共目录，提供发现与安装命令入口。
...
- **标签：** `registry`, `awesome-list`, `plugin-registry`
- **推荐理由：** 去垃圾插件注册表，筛选过的开放数据插件库
...
- **标签：** `awesome-list`, `ai-agent`
- **推荐理由：** DSH插件雷达与2Origin插件实验室
...
- **推荐理由：** 已迁移至原awesome-dsh-plugin仓库
