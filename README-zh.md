...
- **标签：** `ai-agents`, `skills`, `dsh-plugin`
- **推荐理由：** 全能助手框架，支持记忆与多模型。
...
- **标签：** `low-code`, `workflows`, `dsh-plugin`
...
- **标签：** `agent-orchestration`, `agent-workspaces`, `tool-use`
- **推荐理由：** 全设备统一的Agent工作区
...
- **标签：** `routing`, `injector`, `reasoning-mode`
- **推荐理由：** 任务感知的智能路由注入器
...
- **标签：** `agent-os`, `loop-engineering`, `self-evolving`
- **推荐理由：** 自主进化的Agent操作系统
...
- **推荐理由：** 两阶段标准预置配置，对齐Project 98/99
...
- **标签：** `virtual-filesystem`, `vfs`, `agent-tools`
...
- **标签：** `code-review`, `agent-skills`, `code-quality`
...
- **标签：** `security`, `cybersecurity`, `dsh-plugin`
- **推荐理由：** AI安全攻防平台，审计AI生成代码漏洞与幻觉包
...
- **标签：** `plugin-development`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 开发DSH插件的Agent技能标准指南
...
- **标签：** `container`, `multi-version`, `plugin-import`
- **推荐理由：** 隔离容器运行多版本DSH，一键导入插件
...
- **⭐ 星标：** 20
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计���件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context-audit`, `optimization`, `tool`
- **推荐理由：** 审计上下文注入Token成本，检测重复与冲突
...
- **标签：** `plugin-development`, `dsh`
- **推荐理由：** 用于开发与审计DSH插件的可移植技能
...
- **推荐理由：** 任务预检插件库并自动注入Top-K适用插件的智能工具
...
- **标签：** `agent-skills`, `ui-generator`, `dsh-plugin`
...
- **标签：** `dsh-web`, `marketplace`, `plugin-hub`
- **推荐理由：** DSH Web插件市场与生态中心
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑、终端、对话、Git等。
...
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏与实时思考流显示。
...
- **推荐理由：** 本地优先多模型AI桌面代理，隐私优先，支持自进化技能
...
- **推荐理由：** 类CodePilot跨平台桌面客户端，附带插件套件
...
- **推荐理由：** Electron桌面客户端，SSH技能远程访问全覆盖
...
- **推荐理由：** Electron桌面控制台管理Harness/Plugin/MCP
...
- **推荐理由：** Electron桌面封装DSH，集成侧边栏与ModLens等工具
...
- **标签：** `rag`, `embeddings`, `skill-corpus`
- **推荐理由：** SKILL.md文件向量化检索，构建智能体知识库
...
- **推荐理由：** 跨预设会话迁移，保留状态与模型意图，支持图片上下文。
...
- **推荐理由：** 连接PowerContext实现记忆召回与技能管理。
...
- **标签：** `agent-skills`, `llm-agent`, `dsh-plugin`
- **推荐理由：** 提炼专家思维为可复用技能。
...
- **标签：** `agent-collaboration`, `multi-agent`, `visual-editor`
- **推荐理由：** 企业级多Agent协作工作台
...
- **⭐ 星标：** 3,605
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外挂视觉插���，为 DeepSeek、GLM 等纯文本模型外挂视觉能力，粘贴图片即得结构化 JSON 证据（OCR、版面、语义）。
- **标签：** `vision`, `ocr`, `image-to-text`
- **推荐理由：** 首款文本Agent视觉插件，图片转结构化JSON
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `ai-agents`
- **推荐理由：** 基于J-space研究的AI认知增强技能套件V3.7。
...
- **标签：** `agent-skills`, `workflow`, `experience`
- **推荐理由：** 将经验转化为可重复执行的智能体技能
...
- **推荐理由：** AI基础设施开发专用技能集，支持代码编写与调试优化。
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核，无iframe设计。
...
- **标签：** `ai-agents`, `orchestration`, `multimodal`, `dsh-plugin`
- **推荐理由：** 从Claude Code/Codex调度DSH子Agent，支持进度追踪。
...
- **推荐理由：** Superpowers技能移植到Reasonix，支持TDD与调试。
...
- **推荐理由：** Superpowers技能适配DSH，支持TDD、调试与协作。
...
- **标签：** `dsh-plugin`, `agent-skills`, `ai-coding`
- **推荐理由：** AI编程导师，引导项目架构并自动判题评审
...
- **推荐理由：** 实现Codex与Claude Code间的跨平台Agent记忆延续
...
- **推荐理由：** 类WorkBuddy技能选择器，快速插入/skill-name命令
...
- **标签：** `chinese-tradition`, `metaphysics`, `niche`
- **推荐理由：** 八字/风水/六爻/中医等中华玄学全融合AI技能包
...
- **标签：** `code-quality`, `simplification`, `codex`
- **推荐理由：** 从DSH改编的代码简化技能，生成更简洁代码
...
- **标签：** `swift-cycle`, `project-governance`
...
- **标签：** `best-practices`, `engineering`
- **推荐理由：** 工程纪律技能：事实核查、证据与诚实报告
...
- **标签：** `agent-skills`, `skill-learning`
...
- **推荐理由：** 虚拟产品团队预设，模拟PM到工程师到QA的协作流程
...
- **推荐理由：** 最强免费网页搜索插件，零注册为任意模型提供搜索桥接
...
- **推荐理由：** Web界面skill/MCP管理面板，集中配置DSH插件。
...
- **标签：** `codex`, `sync`, `mcp-mirror`
- **推荐理由：** Codex与DSH双向桥接，实时同步和MCP镜像
...
- **标签：** `design`, `vibecoding`, `agent-team`
- **推荐理由：** 支持多Agent协作的AI原生设计工具
...
- **标签：** `video-generation`, `ffmpeg`, `generative-ai`
- **推荐理由：** 一键生成Vox风格纸拼���解说视频，端到端自动化。
...
- **标签：** `design`, `presentation`
- **推荐理由：** AI生成设计系统，含可视化编辑器与PPT
...
- **推荐理由：** 对话内直接生成AI图片，Gemini/OpenAI多引擎
...
- **标签：** `content-discovery`, `browser-automation`, `ai-agent`
- **推荐理由：** 跨平台内容发现AI代理，覆盖B站、小红书、抖音等平台。
...
- **推荐理由：** 对话内运行iOS模拟器，提供22项移动自动化Agent工具
...
- **推荐理由：** Tabbit浏览器自动化插件，基于Playwright的Web交互。
...
- **标签：** `agent`, `canvas`, `mcp`, `skills`
- **推荐理由：** 无限画布创意Web技能插件，适合Web自动化创作。
...
- **标签：** `electron`, `image`, `dsh-plugin`
- **推荐理由：** 万能图片上传工具，支持60+图床。
...
- **标签：** `claude-code`, `deepseek-harness`, `installation`
- **推荐理由：** 一键安装并切换Claude、DeepSeek、Codex等主流工具。
...
- **标签：** `marketplace`, `installation`, `dsh-plugin`
...
- **推荐理由：** 通过OAuth使用订阅账户作为DSH LLM提供商，无需API密钥
...
- **标签：** `cost-tracking`, `token-usage`, `utilities`
- **推荐理由：** 实时追踪会话与每日Token用量及预算
...
- **标签：** `migration`, `compatibility`, `ecosystem`
- **推荐理由：** 打通Pi生态，兼容层运行Pi扩展为DSH插件
...
- **标签：** `backup`, `crash-recovery`, `rollback`, `dsh-plugin`
- **推荐理由：** 崩溃恢复插件：撤销配置、安全快照、一键安全模式。
...
- **标签：** `cost`, `data-visualization`, `tokens`
- **推荐理由：** 实时Token用量与成本可视化图表。
...
- **推荐理由：** WebGPU ocean主题皮肤，Three.js视觉惊艳
...
- **标签：** `research`, `web-search`, `agent-skill`
...
- **推荐理由：** 精选280+个DSH插件目录，按14类整理便于发现
...
- **推荐理由：** 基于证据的评测框架，覆盖提示词、RAG与Agent评估
...
- **标签：** `stock`, `quant`, `finance`
- **推荐理由：** 股票/金融/量化领域DSH插件精选集。
...
- **标签：** `portal`, `webui`, `discovery`
- **推荐理由：** 零依赖静态站点，浏览筛选安装227+插件
...
- **推荐理由：** DSH Agent内直接搜索npm和插件列表的发现工具
...
- **推荐理由：** AI维护的190+插件目录，按12类分，每小时更新
...
- **标签：** `mcp-server`, `dsh-plugin`, `javascript`
- **推荐理由：** 隐私优先的开源简历生成工具。
...
- **标签：** `toy`, `simulation`, `fun`
- **推荐理由：** 零依赖传统竹知了玩具Web模拟版，移动端优先。
...
- **推荐理由：** 精选插件雷达，实测Top50榜单，自动更新全量索引
...
- **标签：** `awesome-list`, `resources`, `bilingual`
- **推荐理由：** 双语精选DSH插件资源目录，覆盖扩展与集成
...
- **⭐ 星标：** 252
- **语言：** TypeScript
- **描述：** AI skills大全 mcp ai知识库 Agent 全维度 AI 资源百科，DSH插件 收录大模型、智能 Agent、RAG 检索增强、多模态、MLOps、AI 应用工具、AI面试集、Vibe coding 大全、零基础学习路线，持续���新前沿 AI 开源项目，开发者一站式 AI 导航库
- **标签：** `encyclopedia`, `resources`, `ai-knowledge`
- **推荐理由：** AI技能百科大全，覆盖MCP、RAG、多模态等开源项目
...
- **推荐理由：** 学习DSH原理、浏览插件市场与最佳实践的一站式指南
...
- **标签：** `guide`, `awesome-list`, `tutorial`
- **推荐理由：** DSH终极指南：快速入门、精选插件与实用工具合集
...
- **标签：** `plugin-marketplace`, `bundle`, `harness`
- **推荐理由：** DSH v4 Pro插件聚合商店与 bundles
...
- **⭐ 星标：** 169
- **语言：** JavaScript
- **描述：** 帮 DSH 搜索、安装并验证插��的 Skill｜A DSH skill that finds, installs, and verifies GitHub plugins
- **标签：** `plugin-discovery`, `automation`, `skill`
- **推荐理由：** Skill自动搜索安装验证GitHub DSH插件
...
- **推荐理由：** 锤子便签复刻，支持Docker私有化部署，导出公众号格式。
...
- **推荐理由：** DSH官方插件市场，一键浏览安装GitHub全部dsh-plugin。
...
- **标签：** `plugin-market`, `web`, `dsh-plugin`
- **推荐理由：** 社区插件市场，Web GUI内浏览安装awesome-dsh-plugin目录。
...
- **推荐理由：** DSH内测里程碑合影墙，GitHub OAuth零权限登录
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内使用Plus模型
...
- **标签：** `deprecated`, `redirect`
- **推荐理由：** 已迁移至新仓库，此地址不再维护
...
- **标签：** `awesome-list`, `ai-agents`
- **推荐理由：** 证据驱动的双语插件目录，不可变提交保证可信度
...
- **标签：** `coding-agents`, `plugin-directory`
- **推荐理由：** 精选插件扩展模型、工具、UI和智能体循环
