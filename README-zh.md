...
- **标签：** `coding-agent`, `cli`, `prompt-caching`
...
- **推荐理由：** 跨设备统一代理工作区，支持桌面/手机/浏览器/API
...
- **标签：** `multi-agent-workspace`, `dsh-plugin`, `agent-collaboration`
- **推荐理由：** 企业级多代理工作台，支持多个编码框架
...
- **标签：** `coding-agent`, `agent-runtime`, `desktop`
- **推荐理由：** Rust 高性能 Agent 运行时，配套精美桌面应用。
...
- **标签：** `agent`, `coding`, `teamwork`
- **推荐理由：** 将多种编码Agent整合为一个协作团队
...
- **标签：** `migration`, `compatibility`, `plugin-migration`
- **推荐理由：** 打通Pi与DSH生态，让Pi扩展无缝运行
...
- **标签：** `agent-skills`, `development`
- **推荐理由：** 开发DSH插件的Agent技能指南
...
- **推荐理由：** OpenAI Codex与DSH双向同步桥接，支持聊天和技能。
...
- **标签：** `agent-skills`, `cordis`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 开发和审计DSH插件
...
- **标签：** `migration`, `import`, `multi-agent`
- **推荐理由：** 一键迁移Claude Code等配置到DSH
...
- **推荐理由：** 开源Claude Design替代品，本地优先桌面设计应用
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 鲸鱼主题 Claude Code ��格 TUI，体验流畅。
...
- **推荐理由：** 开源Windows桌面客户端，支持SSH和移动远程访问
...
- **推荐理由：** WorkBuddy风格技能选择器，集成到DSH composer中。
...
- **标签：** `dsh-plugin`, `usage-tracking`, `web`
- **推荐理由：** 显示模型信息和价格
...
- **标签：** `visualization`, `cost`, `dashboard`
...
- **标签：** `long-term-memory`, `rag`, `memory-management`, `dsh-plugin`
- **推荐理由：** 自演进记忆系统，混合检索节省约35%Token
...
- **推荐理由：** 基于 Milvus 的持久统一 Agent 记忆层。
...
- **标签：** `knowledge`, `ai-pedia`, `resources`
- **推荐理由：** 全面AI资源百科，覆盖技能、RAG与多模态
...
- **标签：** `agent-memory`, `skill-corpus`, `vector-search`, `rag`
- **推荐理由：** 将SKILL.md文件转化为可检索的Agent技能语料库
...
- **标签：** `knowledge-base`, `docs`, `plugin-dev`
- **推荐理由：** 插件开发知识库，含官方文档归档和Cordis入门指南。
...
- **标签：** `agent-evaluation`, `rag`, `llm`, `evaluation-framework`
- **推荐理由：** AI工作流证据化评估
...
- **推荐理由：** 基于 J-space 全局工作空间的认知增强技能集。
...
- **标签：** `agent`, `desktop`, `automation`
- **推荐理由：** 本地优先多模型AI桌面，支持自进化技能
...
- **标签：** `creative-writing`, `skills`, `drama`
- **推荐理由：** 强大的小说创作和短剧制作技能插件
...
- **标签：** `skill`, `math-modeling`, `automation`, `competition`
...
- **标签：** `skills`, `reasonix`
- **推荐理由：** Superpowers技能适配Reasonix编码工具
...
- **标签：** `agent-skills`, `mentor`
- **推荐理由：** AI编程导师，分章指导手写代码
...
- **标签：** `agent-skills`, `research`
- **推荐理由：** 88个研究及业务流技能的开源集合
...
- **标签：** `capability-manage`, `mcp`, `ui`
- **推荐理由：** 统一管理MCP工具暴露级别和执行模式的控制中心。
...
- **标签：** `chinese-wisdom`, `skill`, `domain-specific`
- **推荐理由：** 融合八字风水易经的本地确定性引擎技能包，带可视化面板。
...
- **推荐理由：** 跨平台Agent上下文记忆，支持Codex/Claude Code会话恢复。
...
- **推荐理由：** 多Agent团队协作框架，独立模型技能和共享工作空间。
...
- **标签：** `code-simplify`, `refactor`, `skill`
- **推荐理由：** 从DSH适配的代码简化技能，自动重构建议。
...
- **推荐理由：** 版本锁定Swift Cycle治理技能适配器，支持离线验证。
...
- **标签：** `agent`, `ai`, `deepseek-harness`, `dsh`, `skills`
- **推荐理由：** 游戏化任务进度系统
...
- **标签：** `engineering`, `skills`, `methodology`
- **推荐理由：** 将工程纪律沉淀为可复用Agent技能
...
- **标签：** `api-documentation`, `tools`
- **推荐理由：** 直接搜索65600+中国平台API文档
...
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH中对话ChatGPT模型
...
- **标签：** `diagrams-as-code`, `architecture-diagram`, `dsh-plugin`
- **推荐理由：** 生成精美的架构与流程图，支持自包含HTML输出
...
- **标签：** `vibedesign`, `ui`, `dsh-plugin`, `mcp`
...
- **标签：** `agent-skills`, `deepseek-harness`, `image-generation`, `vision`
- **推荐理由：** 免费图像读取和生成
...
- **标签：** `browser-extension`, `chat-management`, `multi-browser`
- **推荐理由：** 跨浏览器AI增强套件，支持多平台提示词管理
...
- **标签：** `content-discovery`, `privacy-first`, `cross-platform`
- **推荐理由：** 跨平台内容发现 Agent，支持多平台且隐私优先。
...
- **标签：** `dsh-plugin`, `deepseek-harness`, `cordis`
- **推荐理由：** 带任务感知路由标准的DSH运行时注入器套件
...
- **标签：** `dsh-plugin`, `deepseek-harness`, `llm-agent`
- **推荐理由：** 两阶段预设：最小启动后加载完整标准工具链
...
- **标签：** `model-switch`, `coding-agent`, `multi-tool`
- **推荐理由：** 一键切换主流 AI 编码工具模型，实用高效。
...
- **标签：** `marketplace`, `plugin`, `infrastructure`
- **推荐理由：** DSH 内置可视化插件市场，一键安装。
...
- **标签：** `oauth`, `subscription`, `llm`
- **推荐理由：** 通过OAuth使用订阅服务作为DSH的LLM提供商
...
- **标签：** `cost-tracking`, `token-usage`, `utils`
- **推荐理由：** 追踪会话和日常成本，预算和Token使用情况
...
- **标签：** `ocr`, `vision`, `utility`
- **推荐理由：** 纯文本模型的图片OCR识别插件
...
- **标签：** `plugin-dev`, `build-tool`
- **推荐理由：** 构建DSH插件的脚手架工具，支持插槽和凭证配置。
...
- **推荐理由：** 完整的DSH配置备份恢复同步工具，支持WebDAV。
...
- **标签：** `sandbox`, `containers`, `management`
- **推荐理由：** 多实例隔离运行DSH，内置WebView和一键插件导入。
...
- **标签：** `agent-framework`, `agent-skills`, `ai-agents`, `dsh`
- **推荐理由：** 42技能包管理器
...
- **标签：** `dsh-plugin`, `utility`
- **推荐理由：** 自动检测和解决插件冲突与版本兼容问题
...
- **⭐ 星标：** 386
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流���或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `ui`, `deepseek-harness`
- **推荐理由：** 磨砂玻璃主题，支持高度自定义模糊与壁纸
...
- **标签：** `color-scheme`, `dark-mode`, `theme`, `ui`
- **推荐理由：** 34款opencode主题一键切换
...
- **推荐理由：** AI资源大全+Vibe Coding教程+知识库百科全书
...
- **标签：** `awesome-list`, `mcp`, `plugins`, `dsh`
- **推荐理由：** 精选插件、MCP服务器和编排器的资源清单
...
- **推荐理由：** 精选280+社区插件目录，覆盖14大类直达仓库链接。
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产品说明书与导航：���场景选插件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `plugin-directory`
...
- **标签：** `portal`, `plugin-directory`, `web`
- **推荐理由：** 静态插件发现门户，支持227+插件筛选安装
...
- **标签：** `ai-agents`, `plugin-recommender`, `mcp`
- **推荐理由：** AI托管每小时更新的190+插件推荐器
...
- **标签：** `curated-list`, `dsh-plugin`, `registry`
- **推荐理由：** 覆盖模型工具沙箱UI的DSH插件精选
...
- **标签：** `marketplace`, `plugin-hub`, `dsh`
- **推荐理由：** DeepSeek Harness生态插件市场中心
...
- **标签：** `marketplace`, `dsh-plugin-bundle`, `dsh-plugin-market`
