...
- **标签：** `multi-model`, `installer`, `coding-agent`
- **推荐理由：** 一键安装和切换Claude Code、Codex、DSH等模型
...
- **推荐理由：** Rust高性能代理运行时，集成桌面应用，适合���码与自动化任务。
...
- **推荐理由：** 纯文本模型视觉增强工具包：OCR、图片问答、UI还原。
...
- **标签：** `coding-agent`, `rpc`, `mcp`, `sub-agent`
- **推荐理由：** 集成RPC插件、子代理和MCP的编码智能体
...
- **标签：** `plugin-development`, `tool`
- **推荐理由：** 构建和验证DeepSeek Harness插件的复用人力技能。
...
- **标签：** `design`, `desktop`, `ui-generator`
- **推荐理由：** 本地优先AI驱动桌面设计工具
...
- **推荐理由：** 可扩展侧边栏，支持文件、终端、对话、Git和子代理
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** Claude Code风格TUI，含鲸鱼顶栏、流式思考和回滚功能
...
- **推荐理由：** 可爱的浮动鲸鱼组件，监控DSH中DeepSeek账户余额
...
- **推荐理由：** 零安装Windows桌面GUI，支持SSH、Codex和插件
...
- **标签：** `canvas`, `conversation`, `workspace`, `branching`
...
- **推荐理由：** 本地优先的Claude Cowork替代品，支持多模型和自进化技能
...
- **标签：** `office`, `ui`, `deepseek-harness-plugin`
- **推荐理由：** 将电子表格、文档、幻灯片等完整办公套件引入Harness
...
- **推荐理由：** 原生iOS客户端，通过WebSocket连接Harness，随时随地工作
...
- **推荐理由：** 在对话中直接操控iOS模拟器或真机，22项Agent工具
...
- **推荐理由：** 类CodePilot跨平台桌面客户端，覆盖macOS/Windows/Linux
...
- **推荐理由：** 为纯文本LLM添加多模态视觉能力，支持截图和UI分析。
...
- **推荐理由：** 开源锤子便签，支持Docker私有化和DSH插件调用。
...
- **标签：** `mcp`, `ui`, `capability-management`
- **推荐理由：** 统一菜单管理MCP工具暴露和执行模式
...
- **标签：** `memory`, `RAG`, `agent`
- **推荐理由：** 统一记忆中枢，多AI共享持久上下文，跨工具记忆管理。
...
- **标签：** `skill-corpus`, `embedding`, `reranking`, `RAG`
- **推荐理由：** 将SKILL.md转化为检索就绪的智能体技能语料库
...
- **标签：** `agent-skill`, `knowledge-base`
- **推荐理由：** 官方DSH文档知识库，支持中英文，按需调用。
...
- **标签：** `preset`, `agent-skills`
- **推荐理由：** 两阶段预设，从小白引导到完整标准工具集成
...
- **标签：** `cognitive-enhancement`, `reasoning`, `agent`
- **推荐理由：** 模型无关推理时控制，支持深度推理和长程工作
...
- **推荐理由：** 自动搜索中国八大主流平台近期内容，生成有据可查报告。
...
- **推荐理由：** 支持DeepSeek Harness内多代理团队协作，处理复杂任务。
...
- **推荐理由：** 结构化长篇小说创作流程，支持角色/世界观/审稿全流程控制。
...
- **标签：** `multimodal`, `video`, `audio`, `ocr`, `skill`
- **推荐理由：** 赋予AI眼耳，将音视频屏幕活动转为可搜索证据
...
- **推荐理由：** 原生本地任务看板，SQLite驱动，支持Agent认领与审核
...
- **推荐理由：** 全栈数学建模自动Skill，支持GitOps流水线与代码自证。
...
- **推荐理由：** 将经验转化为可重复执行的Skill框架，多平台兼容。
...
- **推荐理由：** 88个开源Skill覆盖研究、营销自动化和业务流程。
...
- **标签：** `plugin-discovery`, `automation`, `agent-skills`
- **推荐理由：** 自动发现、安装和验证DSH插件的Skill工具。
...
- **推荐理由：** AI基础设施开发专用Skill，支持代码编写和调试。
...
- **标签：** `agent`, `mentor`, `skill`
- **推荐理由：** AI编程导师，扫描项目架构生成阶梯式教学并自动判题
...
- **推荐理由：** 自带25个工程与效率技能面板，开箱即用无需手动安装
...
- **标签：** `gongwen`, `office`, `skill`
- **推荐理由：** 符合国标党政机关公文格式的全流程处理工具
...
- **标签：** `spec-driven`, `skill`, `development`
- **推荐理由：** 仓库原生规范驱动开发技能，参考DSH工程模式
...
- **⭐ 星标：** 38
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard，一��� dsh plugin add 安装。
- **标签：** `wisdom`, `traditional`, `skill`
- **推荐理由：** 融合八字/风水/周易的中华传统智慧AI技能包
...
- **标签：** `multi-agent`, `collaboration`, `workflow`
- **推荐理由：** 多智能体协作框架，支持共享工作空间和独立技能。
...
- **标签：** `video`, `ai-director`, `multimodal`
- **推荐理由：** 集成AI视频/图像/音频技能，支持故事板和知识库。
...
- **推荐理由：** 官方SkillHub API文档与示例，适合DSH集成
...
- **推荐理由：** 提供Command Code模型接入与计划感知选择，适合DSH智能体增强。
...
- **推荐理由：** 基于tushare的本地金融数据库，支持MCP和多Agent盯盘。
...
- **推荐理由：** 首个视觉插件，为纯文本编码代理提供图像识别和OCR能力
...
- **推荐理由：** 原生设计与PPT工作室，AI生成+可视化编辑+模板市场。
...
- **标签：** `UI`, `charts`, `interactive`, `visual`
- **推荐理由：** 通过dsh-ui围栏生成交互式内联UI组件
...
- **推荐理由：** 像素转文本OCR技能，让纯文本模型具备图像识别能力。
...
- **标签：** `content-discovery`, `multi-platform`, `web`
- **推荐理由：** 跨平台AI内容发现代理，支持B站、抖音、YouTube等
...
- **标签：** `web-search`, `free`, `no-api`, `browser`
...
- **标签：** `browser-automation`, `mcp-server`, `anti-detect-browser`
- **推荐理由：** 提供指纹欺骗、无限配置文件的Agent浏览器自动化方案
...
- **标签：** `plugin-registry`, `discovery`, `infrastructure`
- **推荐理由：** 自部署插件生态雷达，自动发现与验证海量候选插件。
...
- **推荐理由：** 免API Key通过OAuth接入ChatGPT/Claude/Grok等大模型
...
- **推荐理由：** 开源CLI与解析工具，方便管理和查询DSH插件市场
...
- **推荐理由：** 追踪会话成本、日预算和Token用量，支持多提供商
...
- **标签：** `catalog`, `marketplace`, `plugin-store`
- **推荐理由：** 11000+插件目录，支持搜索和免费API的插件市场。
...
- **推荐理由：** 在隔离容器中运行多个DSH版本，内嵌WebView管理。
...
- **标签：** `launcher`, `runtime`, `orchestration`
- **推荐理由：** 统一编排DSH运行时与插件能力
...
- **标签：** `token-usage`, `data-visualization`, `cost`
- **推荐理由：** DSH Web实时Token用量和成本追踪可视化工具
...
- **标签：** `debug`, `plugin-compatibility`, `utility`
...
- **推荐理由：** 鲸鱼娘系列主题皮肤，美观个性化DeepSeek Harness界面。
...
- **标签：** `skin`, `ocean`, `animated`, `theme`
- **推荐理由：** 动态海洋皮肤，实时海浪、日落效果和磨砂控制
...
- **标签：** `documentation`, `code-analysis`, `wiki`
- **推荐理由：** 快速将代码转化为准确技术文档和AI就绪上下文
...
- **标签：** `research`, `latex`, `arxiv`, `experiment`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪、GPU
...
- **推荐理由：** 一站式AI资源百科，覆盖Agent、RAG、MCP等全维度知识
...
- **推荐理由：** 精选DSH插件、MCP服务器、补丁与编排器的资源大全
...
- **标签：** `awesome-list`, `developer-tools`, `deepseek-harness`
- **推荐理由：** DeepSeek Harness终极指南：快速入门、精选插件与实用工具
...
- **标签：** `dsh`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 学习DSH原理、探索插件市场与最佳实践的实用工具
...
- **推荐理由：** 覆盖运行时、插件、MCP和沙箱的全面Agent手册。
...
- **标签：** `registry`, `curated-list`, `dsh-plugins`
- **推荐理由：** 官方反垃圾DSH插件注册表，含skills和技能
...
- **标签：** `awesome-list`, `collection`, `ecosystem`
- **推荐理由：** 涵盖Agent、CLI与Web UI的DSH插件精选合集。
...
- **标签：** `marketplace`, `plugin-shop`, `community`
- **推荐理由：** 每日更新的全平台插件市场，经过审核发布
...
- **标签：** `dsh-bundle`, `dsh-plugin-verify`
- **推荐理由：** 支持插件捆绑与验证的DSH插件仓库
