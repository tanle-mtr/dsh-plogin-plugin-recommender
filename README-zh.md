...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 193,388 | Core DSH ecosystem hub enabling everything-is-a-pl |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,224 | Open-source Claude Design alternative with local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,325 | Multi-agent swarm orchestration with autonomous wo |

---

## 分类导航

- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 28 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (28)
...
- **标签：** `ai-agent`, `multi-model`, `dsh-plugin`
- **推荐理由：** 多功能多模型Agent骨架，具备自进化记忆
...
- **标签：** `agent-orchestration`, `workspace`, `multi-platform`
- **推荐理由：** 跨设备的统一代理工作空间
...
- **标签：** `agent-runtime`, `rust`, `coding-agent`, `desktop-app`
- **推荐理由：** Rust高性能Agent运行时搭配精美桌面IDE体验。
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `agent`, `mcp`
- **推荐理由：** 本地优先多模型AI代理桌面应用，隐私优先，支持自进化技能
...
- **标签：** `deepseek-harness`, `skill`
- **推荐理由：** 统一协调多代理，跨Claude Code/Codex/Grok/DSH协同编程
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `ios`, `mcp`
- **推荐理由：** 对话内运行iOS模拟器与真机自动化，含22个专属代理工具
...
- **标签：** `android`, `emulator`, `coding-agent`
- **推荐理由：** 在对话中构建运行Android模拟器或连接真机
...
- **标签：** `documentation`, `plugin-development`, `knowledge-base`
- **推荐理由：** 官方DSH插件开发知识库，含EN/ZH文档和Cordis入门
...
- **推荐理由：** WorkBuddy同款Skill选择器，Composer内直接插入技能名
...
- **标签：** `code-simplification`, `refactor`, `official-ported`
- **推荐理由：** 官方DSH代码简化技能适配版，保持原生逻辑
...
- **推荐理由：** 上下文注入审计：Token成本统计+重复冲突检测+Web面板
...
- **标签：** `plugin-dev`, `audit`, `bundle`
- **推荐理由：** 便携开发调试DSH插件的技能工具
...
- **标签：** `documentation`, `plugin-dev`, `notes`
- **推荐理由：** DSH插件开发踩坑实录与最佳实践
...
- **标签：** `engineering`, `discipline`, `practices`
- **推荐理由：** 工程纪律提炼：事实核查证据驱动
...
- **标签：** `agent-tools`, `plugin-check`, `llm-agents`
- **推荐理由：** 任务前自动推荐Top-K适用插件。
...
- **标签：** `web-ui`, `plugin-aggregation`, `dsh-web`
- **推荐理由：** DSH插件生态的Web UI聚合
...
- **标签：** `sidebar`, `extensible`, `ui-extension`
- **推荐理由：** 可扩展侧边栏底座，内置文件编辑、终端、聊天、Git等。
...
- **推荐理由：** Claude Code风格TUI，支持鲸鱼顶栏、流式思考与回滚。
...
- **标签：** `desktop`, `companion`, `plugins`
- **推荐理由：** 桌面宠物伴侣平台，支持插件SDK和编码智能体集成。
...
- **标签：** `widget`, `desktop`, `ui`
- **推荐理由：** 可爱的鲸鱼小部件，实时查看DeepSeek账户余额。
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `electron`
- **推荐理由：** CodePilot风格桌面客户端套件，支持macOS/Windows/Linux全平台
...
- **推荐理由：** Electron桌面控制台，多标签管理DSH/插件/MCP/技能
...
- **推荐理由：** 容器化运行多个隔离DSH版本，内嵌WebView UI
...
- **标签：** `oauth`, `photo`, `skill`
- **推荐理由：** 零权限OAuth合影墙，带DSH技能包装
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `inference-control`
- **推荐理由：** 基于J-space全局工作空间研究的AI认知增强技能集。
...
- **标签：** `video-generation`, `ffmpeg`, `agent-skill`
- **推荐理由：** 一键从主题生成Vox风格纸拼贴解说视频的全流程技能。
...
- **标签：** `workflows`, `productivity`, `browser-extension`
- **推荐理由：** 将AI对话转化为可复用、有组织的工作流。
...
- **标签：** `agent-teams`, `multi-agent`, `workflows`
- **推荐理由：** 在DSH中实现多智能体团队协作功能。
...
- **标签：** `dsh`, `dsh-plugin`
- **推荐理由：** 在对话中内联渲染图表、表单等交互式UI组件，提升交互体验
...
- **标签：** `notes`, `skill`, `docker`
- **推荐理由：** 锤子便签复刻版，支持Skill调用和一键部署
...
- **标签：** `task-management`, `agent`, `workflow`
- **推荐理由：** 原生SQLite任务板，支持Agent认领审查和Web UI
...
- **推荐理由：** 含85条技能的逆向工程技能，适用于CTF和安全渗透
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** AI编程导师技能：扫描架构、生成学习路径、代码审查
...
- **推荐理由：** 88个开源Agent技能，覆盖研究、社交情报和营销自动化
...
- **推荐理由：** 管理已安装DSH插件、插槽、Typert远程和凭证的Agent技能
...
- **推荐理由：** 多Agent团队协作，独立模型/技能/MCP工具+共享工作区
...
- **标签：** `skills`, `management`, `lifecycle`
- **推荐理由：** 专业DSH技能管理插件，完整技能生命周期管理
...
- **标签：** `governance`, `swift-cycle`, `verifiable`, `version-pinned`
- **推荐理由：** 用户触发+版本锁定+离线可验证的治理技能适配器
...
- **标签：** `godot-engine`, `game-development`, `gdscript`
...
- **标签：** `agent-skills`, `mcp`, `agentic-workflow`
- **推荐理由：** 无限画布配合创意技能的工作流插件
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `web-search`
- **推荐理由：** 免费网页搜索桥接插件，无需注册或API密钥，适配所有模型
...
- **标签：** `codex`, `bridge`, `sync`, `mcp-mirror`
- **推荐理由：** OpenAI Codex与DSH双向桥接：聊天同步+技能挂载+MCP镜像
...
- **标签：** `mcp-management`, `capability`, `exposure`
- **推荐理由：** 统一管理MCP工具暴露级别和执行模式可见性
...
- **推荐理由：** 开源Claude Design替代品，本地优先桌面体验
...
- **标签：** `ai-image-generation`, `prompt-engineering`, `dsh-plugin`
- **推荐理由：** 工业级GPT-Image2提示词引擎，含530+案例
...
- **标签：** `architecture-diagram`, `diagrams-as-code`, `sequence-diagram`
- **推荐理由：** 美观可验证的架构图，带动态效果
...
- **标签：** `design`, `vector`, `Agent-Team`, `design-as-code`
- **推荐理由：** AI原生矢量设计工具，支持并发代理团队
...
- **标签：** `design`, `ui`, `aesthetic`, `vibe-coding`, `theme`
- **推荐理由：** 6种美学风格(暗SaaS/苹果极简/新拟态)告别AI味网站
...
- **标签：** `video`, `image`, `audio`
- **推荐理由：** AI音视频技能与可配置模型工具
...
- **推荐理由：** 跨平台自进化内容发现Agent，覆盖B站、小红书、YouTube等。
...
- **推荐理由：** 将DSH agent接入钉钉，支持流式对话与工具能力。
...
- **标签：** `router`, `infrastructure`, `agent-routing`
- **推荐理由：** 任务感知路由器套件，智能分发请求
...
- **标签：** `deepseek-harness`, `model-switch`, `multi-agent`
- **推荐理由：** 一键切换Claude、Codex、DeepSeek等多个AI编码模型。
...
- **标签：** `marketplace`, `plugin-management`, `dsh-plugin`
...
- **推荐理由：** OAuth桥接ChatGPT/Claude/Grok订阅为DSH提供LLM，免API密钥
...
- **标签：** `mcp`, `skills`, `utility`
- **推荐理由：** Web界面Skill和MCP配置管理工具
...
- **标签：** `backup`, `config`, `webdav`, `migration`
- **推荐理由：** 完整DSH配置备份恢复，支持WebDAV同步插件/MCP/Skills
...
- **⭐ 星标：** 6
- **语言：** TypeScript
- **描述：** 致力于解决各插件之间也���版本之间可能存在的冲突问题并提供解决方案
- **标签：** `deepseek-harness`, `dsh-plugin`, `conflict-resolution`
- **推荐理由：** 解决插件冲突与版本兼容问题。
...
- **标签：** `marketplace`, `dsh-plugin`, `skills`
- **推荐理由：** 类CodeX的插件市场，便于浏览和管理Dsh技能。
...
- **标签：** `mascot`, `pixel-art`, `animated`, `themes`
- **推荐理由：** 编码代理的动态宠物吉祥物画廊
...
- **推荐理由：** 高自由度玻璃质感主题，磨砂效果与壁纸均可自定义调节
...
- **标签：** `theme`, `color-scheme`, `opencode`
...
- **标签：** `research`, `web-search`, `chinese-platforms`
- **推荐理由：** 自动搜索8大中文平台30天内容，生成有据可查的报告。
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `security`, `vscode`
- **推荐理由：** 审计AI生成代码的安全隐患，检测幻觉包和漏洞模式
...
- **标签：** `awesome-list`, `plugin`, `research`
- **推荐理由：** DSH生态插件的精选汇总列表
...
- **推荐理由：** 在编码Agent中直接搜索20个中国开放平台6.5万+篇API文档
...
- **标签：** `kline`, `finance`, `technical-analysis`, `chart`
- **推荐理由：** 交互式K线分析插件，多市场+技术指标+支撑压力位
...
- **标签：** `awesome-list`, `typescript`, `deepseek-harness`
...
- **标签：** `awesome`, `curated-list`, `dsh-plugin`
- **推荐理由：** 经验证的精选DSH插件目录
...
- **标签：** `awesome`, `curated-list`, `2026`
- **推荐理由：** 2026年DSH插件精选列表含质量检查
...
- **推荐理由：** 收录1000+插件的全量清单，按star排序，便于检索。
...
- **标签：** `recommender`, `awesome-list`, `ai-curated`
- **推荐理由：** AI每小时更新190+插件，覆盖12个分类，精准推荐。
...
- **标签：** `awesome-list`, `curated`, `evidence`
...
- **标签：** `awesome-list`, `curated`, `ui`, `tools`
- **推荐理由：** 覆盖模型、工具、UI等全方位插件的精选目录。
...
- **标签：** `industry`, `awesome-list`, `vertical`
- **推荐理由：** 面向金融、法律、医疗等行业的垂直插件目录。
...
- **推荐理由：** 精选插件雷达，9000+候选人工策展，自动更新索引。
...
- **推荐理由：** DeepSeek Harness生态精选插件、工具和基础设施清单。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** DeepSeek专用破甲插件，稳定化提示词优化生成效果
...
- **标签：** `agent`, `deepseek-harness`, `plugins`
- **推荐理由：** DSH终极指南与插件资源精选列表
...
- **标签：** `agent-framework`, `deepseek-harness`, `plugins`
- **推荐理由：** 精选DSH插件、技能与MCP资源目录
...
- **标签：** `chinese-traditional`, `bazi`, `fengshui`, `niche`
- **推荐理由：** 中华玄学(AI技能包)八字/风水/奇门全融合，本地确定性引擎
...
- **标签：** `awesome-list`, `plugins`, `verified`
- **推荐理由：** 按星标排序的社区插件目录。
...
- **标签：** `plugin-directory`, `guide`, `compatibility`
- **推荐理由：** 插件安装指南与风险评估导航。
...
- **标签：** `plugin-registry`, `radar`, `2origin`
- **推荐理由：** 插件雷达与证据驱动评测实验室。
...
- **标签：** `dsh-plugins`, `repository`, `bundle`
- **推荐理由：** DSH插件仓库与生态目录。
