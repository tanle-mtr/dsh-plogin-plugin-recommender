...
- [实用工具与基础设施](utilities-infrastructure-zh) (33)
...
- **标签：** `coding-agent`, `deepseek`, `tui`
- **推荐理由：** DeepSeek原生终端编码Agent，前缀缓存稳定高效。
...
- **标签：** `harness-engineering`, `dsh-plugin`, `llm`
- **推荐理由：** DSH从入门到精通的最佳初学者教程
...
- **标签：** `multi-agent-workspace`, `agent-skills`, `mcp`
- **推荐理由：** 企业级多引擎智能体工作台，带可视化编辑器
...
- **标签：** `vision`, `multimodal`, `ocr`
- **推荐理由：** 首个DeepSeek视觉插件，支持OCR和多模态
...
- **标签：** `agent-runtime`, `rust`, `desktop`
- **推荐理由：** 高性能Rust Agent运行时与精美桌面应用
...
- **标签：** `code-review`, `lint`, `clean-architecture`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查
...
- **标签：** `architecture`, `coding-agents`
- **推荐理由：** 架构感知编码工具，支持漂移检测与验证
...
- **标签：** `macos`, `coding-agent`
- **推荐理由：** DeepSeek官方原生macOS编程助手
...
- **标签：** `agent`, `coding`, `automation`
- **推荐理由：** 将多个编码代理整合为协作团队，实现任务调度
...
- **标签：** `mcp`, `oauth`, `llm-provider`
- **推荐理由：** 通过OAuth使用现有订阅作为DSH的LLM提供商
...
- **标签：** `orchestration`, `subagent`
- **推荐理由：** 从Claude Code/Codex派遣DSH Agent执行子任务
...
- **标签：** `agent-skill`, `dsh-plugin`, `developer-tools`, `skill`
- **推荐理由：** 开发DSH插件的元技能，遵循官方标准规范
...
- **标签：** `agent-skills`, `spec-programming`, `developer-tools`, `vibe-coding`
- **推荐理由：** 仓库原生规格驱动开发技能，支持vibe编码
...
- **标签：** `deepseek-harness`, `ocr`, `image-reading`, `vision`
- **推荐理由：** 为纯文本模型提供像素级OCR图像读取能力
...
- **标签：** `plugin-dev`, `cordis`, `tools`
- **推荐理由：** 便携插件开发审核技能，支持 Cordis 双副本。
...
- **标签：** `plugin-dev`, `documentation`, `skill`
- **推荐理由：** DSH 插件开发踩���实录与最佳实践指南。
...
- **标签：** `plugin-dev`, `testing`, `skills`
- **推荐理由：** 在 DSH 内完成插件脚手架与测试的全套技能。
...
- **推荐理由：** 用 Lua 编写的快速简洁 Neovim 插件管理器
...
- **推荐理由：** 内置 GUI 管理 MCP/技能/插件，改动热生效无需重启
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内使用订阅模型。
...
- **标签：** `deepseek-harness-plugins`, `ui`, `skills`
- **推荐理由：** 可安装插件涵盖模型、工具、沙箱与智能体循环
...
- **标签：** `agent-orchestration`, `agent-skills`, `no-code`
- **推荐理由：** 跨设备统一智能体工作区，支持看板追踪
...
- **推荐理由：** 功能全面的Windows桌面GUI，支持SSH和移动访问
...
- **标签：** `marketplace`, `web-gui`
- **推荐理由：** Web GUI一键浏览安装全部插件的插件市场
...
- **标签：** `task-management`, `web-ui`
- **推荐理由：** SQLite驱动的原生任务看板，带Agent协作
...
- **标签：** `console`, `dsh-plugin`, `ui`, `plugin-management`
- **推荐理由：** 浏览器面板插件管理控制台加开发引导
...
- **推荐理由：** Electron桌面Hub，多标签管理Harness/插件/MCP
...
- **推荐理由：** 跨平台桌面应用整合数据库、SSH、Docker 与 AI
...
- **推荐理由：** Electron桌面壳集成多款DSH插件，一键体验。
...
- **标签：** `long-term-memory`, `rag`, `skills`
- **推荐理由：** 自进化记忆系统，混合检索节省35%令牌
...
- **标签：** `memory`, `long-term-memory`
- **推荐理由：** 持久化agent长期记忆，带回忆工具和设置页
...
- **标签：** `knowledge-base`, `plugin-development`, `agent-skill`
- **推荐理由：** 官方DSH文档知识库，支持即查即用
...
- **推荐理由：** 连接 PowerContext 实现记忆召回和技能交接
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `react`
- **推荐理由：** 基于全局工作空间研究的认知增强技能套件
...
- **标签：** `agent-skills`, `workflow`, `multi-agent`
- **推荐理由：** 智能编排Agent技能路由与工作流
...
- **标签：** `video`, `ai-video`, `ffmpeg`
- **推荐理由：** 端到端Vox风格拼贴解说视频生成技能
...
- **标签：** `agent`, `desktop`, `privacy`
- **推荐理由：** 本地优先多模型AI代理，支持自进化技能与隐私保护
...
- **推荐理由：** 自动搜索安装验证GitHub DSH插件的Skill
...
- **标签：** `agent-skills`, `deep-research`, `social-listening`, `market-research`
- **推荐理由：** 88个研究、社情、营销和业务工作流Agent技能
...
- **标签：** `ai-agent`, `chinese-wisdom`, `bazi`, `fengshui`
- **推荐理由：** 融合八字、风水、易经等中华传统智慧AI技能包
...
- **标签：** `multi-agent`, `team-collaboration`, `workflow`
- **推荐理由：** 多Agent协作，支持独立模型与共享工作区
...
- **标签：** `agent-framework`, `package-manager`, `skills`, `cli`
- **推荐理由：** 打包42项技能+5个CLI的AI Agent工程工具集
...
- **推荐理由：** 版本固定的 Swift Cycle 治理技能，适合结构化项目管理。
...
- **标签：** `engineering`, `discipline`, `workflow`
- **推荐理由：** 将工程纪律提炼为可移植的智能体技能。
...
- **标签：** `record-replay`, `computer-use`, `skills`
- **推荐理由：** 录制 macOS 桌面工作流并转化为可复用智能体技能
...
- **标签：** `skills`, `gamification`, `workflow`
- **推荐理由：** 游戏化任务系统，帮助组织追踪技能进度
...
- **标签：** `agent-tool`, `workflow`, `plugin-recommendation`
- **推荐理由：** 任务预检插件库并注入Top-K适配插件提示。
...
- **标签：** `mcp`, `skills`, `gui`
- **推荐理由：** 在 DSH Web UI 中集中管理技能与 MCP 服务器，方便实用。
...
- **标签：** `mcp`, `skills`, `menu`
- **推荐理由：** 统一管理 MCP 工具暴露级别和执行模式，简洁高效。
...
- **标签：** `ai-design`, `design-systems`, `ui-generator`
- **推荐理由：** 本地优先的开源设计系统生成器，AI驱动UI原型。
...
- **标签：** `vibedesign`, `ui`, `flutter`
- **推荐理由：** AI原生矢量设计工具，并发智能体团队生成UI
...
- **标签：** `image-generation`, `multimodal`, `design`
- **推荐理由：** 在对话中直接生成AI图像
...
- **标签：** `ui-design`, `openpencil`, `preview`
- **推荐理由：** 在对话中预览和编辑OpenPencil设计文件
...
- **标签：** `ai-video`, `directorx`, `visual`
- **推荐理由：** 集成 DirectorX 实现 AI 视频图像音频生成创作。
...
- **标签：** `canvas`, `creative`, `visual`
- **推荐理由：** 无限画布结合创意技能，可视化协作设计利器。
...
- **标签：** `dsh-web`, `dsh-plugin`
- **推荐理由：** DSH生态Web端插件市场聚合器
...
- **标签：** `bilibili`, `cross-platform`, `privacy-first`
- **推荐理由：** 跨平台隐私优先的内容发现AI Agent
...
- **标签：** `ios`, `mobile`, `ui-automation`
- **推荐理由：** 在DSH对话中集成iOS模拟器与真机调试能力
...
- **标签：** `browser`, `playwright`
- **推荐理由：** Tabbit浏览器自动化插件扩展
...
- **标签：** `social`, `community`, `oauth`, `photo-wall`
...
- **标签：** `image-uploader`, `electron`, `cloudflare-r2`
- **推荐理由：** 支持60+图床的终极图片上传工具，含S3兼容。
...
- **标签：** `vfs`, `virtual-filesystem`, `mcp`
- **推荐理由：** 统一虚拟文件系统，跨智能��文件访问抽象层
...
- **标签：** `marketplace`, `plugins`, `utility`
- **推荐理由：** DSH内置可视化插件市场，一键安装
...
- **标签：** `discovery`, `curation`, `utility`
- **推荐理由：** 每日自动抓取+人工核实的插件发现工具
...
- **标签：** `dsh-plugin`, `agent-skill`, `plugin-management`
- **推荐理由：** 管理已安装DSH插件的插槽、远程和凭证
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `container`, `desktop`
- **推荐理由：** 在隔离容器中运行多个DSH版本并嵌入WebView
...
- **标签：** `awesome-list`, `dsh-plugins`, `plugin-catalog`
...
- **标签：** `dsh-plugin`, `skill-picker`, `ui-tool`
...
- **标签：** `dsh-plugin`, `skills-manager`, `utility`
- **推荐理由：** 集中管理DSH技能，便于组织与维护
...
- **标签：** `config-manager`, `backup`, `migration`, `webdav`
- **推荐理由：** 完整DSH配置备份还原，支持WebDAV同步迁移
...
- **标签：** `usage`, `cost`, `visualization`
- **推荐理由：** 实时 Token 用量图表和成本估算
...
- **标签：** `hud`, `status`, `dashboard`
- **推荐理由：** 浮动面板显示 Git 状态、MCP、技能和 Token 用量
...
- **标签：** `directory`, `curated`, `verified`
- **推荐理由：** 经核实加载的插件目录，附带权限安装指南
...
- **标签：** `finance`, `stock`, `quant`, `curated`
- **推荐理由：** 精选 24 个股票/金融/量化插件的垂直目录
...
- **标签：** `utility`, `debugging`, `compatibility`
...
- **推荐理由：** 在DSH Agent内搜索npm和插件列表，提升发现效率。
...
- **标签：** `dsh-plugin`, `dsh-plugin-verify`, `dsh-repository-plugin`
- **推荐理由：** DSH插件包含验证工具，便于生态系统管理
...
- **标签：** `theme`, `dark-mode`, `ui`
- **推荐理由：** 34款 opencode 经典配色一键切换，重启不丢失。
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH ��置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `theme`, `background`, `ui`
- **推荐理由：** 支持自定义背景、预设主题及精细调优，所见即所得。
...
- **标签：** `ai-tutorial`, `vibe-coding`, `prompt-collection`
...
- **标签：** `knowledge-base`, `ai-pedia`, `resources`
- **推荐理由：** 涵盖Agent、RAG、MLOps的全方位AI资源百科
...
- **标签：** `guide`, `awesome-list`, `bilingual`
- **推荐理由：** DSH终极中英双语指南，含入门教程与工具推荐
...
- **标签：** `recommendation`, `awesome-list`, `ai-curated`
- **推荐理由：** AI每小时更新190+插件、12分类的智能推荐列表。
...
- **标签：** `industry`, `dsh-plugins`
- **推荐理由：** 金融法律医疗电商等行业精选插件分类目录
...
- **标签：** `toy`, `simulation`, `no-dependency`
- **推荐理由：** 零依赖单文件竹知了Web模拟版
...
- **标签：** `awesome-list`, `reference`, `bilingual`
- **推荐理由：** 双语整理的DSH生态插件与扩展资源参考目录
...
- **推荐理由：** cordis.run精选维护的官方自动生成插件目录
...
- **标签：** `guide`, `navigation`, `awesome-list`
- **推荐理由：** 按场景选插件的导航手册，含安装与风险说明。
...
- **标签：** `radar`, `awesome-list`, `research`
- **推荐理由：** 有据可查的插件雷达与2Origin实验室。
