...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 216,187 | Core harness enabling everything-is-a-plugin archi |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 94,922 | Open-source Claude-equivalent design tool, local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 71,664 | Original agent meta-harness for deploying multi-ag |

---

## 分类导航

- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 10 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (10)
...
- **推荐理由：** DeepSeek原生终端编程Agent，前缀缓存稳定适合长时间运行。
...
- **标签：** `compatibility-layer`, `migration`, `mcp`
- **推荐理由：** 打通Pi与DSH生态，一键迁移插件
...
- **标签：** `plugin-dev`, `coding`, `harness`
- **推荐理由：** DSH插件开发标准技能，涵盖服务/MCP/LLM适配器
...
- **标签：** `build`, `plugin`, `credentials`
- **推荐理由：** 构建配置已安装插件的技能、插槽和凭证
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `guard`, `quality-gate`, `skill`
- **推荐理由：** 工程纪律工具，校验需求与实现质量
...
- **标签：** `widget`, `balance`, `floating`
- **推荐理由：** 可爱浮窗小鲸鱼，实时监控账户余额，带动画效果
...
- **标签：** `vision`, `ocr`, `image-qa`
- **推荐理由：** 为纯文本模型打造视觉工具箱，支持OCR和截图还原
...
- **标签：** `office`, `spreadsheet`, `document`, `productivity`
- **推荐理由：** 将完整办公套件内置进Harness，文档表格幻灯片一应俱全
...
- **推荐理由：** 跨平台桌面客户端，类CodePilot体验覆盖三大系统
...
- **标签：** `ios`, `mobile`, `swift`, `native-app`
- **推荐理由：** 原生iOS客户端，工作区与会话随时掌上掌控
...
- **推荐理由：** DSH Web界面管理skill和MCP的实用插件
...
- **标签：** `bioinformatics`, `deepseek`, `desktop-app`, `dsh-plugin`
- **推荐理由：** 离线生物信息学桌面工具，内置技能商店
...
- **推荐理由：** 集成数据库、SSH、Docker和AI沙箱的一体化工具
...
- **推荐理由：** Electron桌面封装，集成DSH Web和侧边栏。
...
- **推荐理由：** 自进化上下文数据库，统一Agent记忆、知识RAG与技能。
...
- **推荐理由：** 便携式本地优先记忆层，Markdown原生存储用户数据
...
- **推荐理由：** 自我演进记忆系统，混合检索节省35% token开销
...
- **标签：** `agent-memory`, `stateful`, `library`
- **推荐理由：** 构建有状态Agent的内存库，支持持续学习
...
- **推荐理由：** 基于Markdown和Milvus的持久统一记忆层，支持多代理。
...
- **标签：** `memory`, `context`, `plugin`
- **推荐理由：** 三层自动记忆整合，零提示词主动回忆
...
- **标签：** `docs`, `knowledge-base`, `guide`
- **推荐理由：** 官方双语文档知识库，随时调用的技能指南
...
- **标签：** `skills`, `skill-store`, `workflows`
- **推荐理由：** 最全技能商店，自动分类上万技能包即时可用
...
- **标签：** `preset`, `workflow`, `standard-tools`
- **推荐理由：** 两阶段预设：精简引导后切换为标准工具套件。
...
- **标签：** `skill`, `math-modeling`, `gitops`, `competition`
- **推荐理由：** 全栈数学建模autopilot，GitOps流水线+代码自证
...
- **标签：** `agent-review`, `plan-mode`, `skills`
- **推荐理由：** 组合式可审查的Agent自进化工作流
...
- **推荐理由：** 将Superpowers的TDD和调试技能适配到DSH
...
- **标签：** `fintech`, `backtesting`, `agent-skills`, `mcp`
- **推荐理由：** 合规优先的AI投资桌，支持多策略基金回测
...
- **推荐理由：** 预装25个工程技能，集成GitHub Issue管理
...
- **标签：** `gongwen`, `docx`, `wps`, `docs`
- **推荐理由：** 中文公文全流程处理：格式检查、修订、模板生成
...
- **标签：** `spec-driven`, `coding`, `agent-skills`
- **推荐理由：** 基于DSH模式的规范驱动开发技能
...
- **标签：** `wisdom`, `fengshui`, `bazi`, `skill`
- **推荐理由：** 融合八字风水等中国传统智慧，带可视化面板
...
- **标签：** `ai-video`, `directorx`, `multimedia`
- **推荐理由：** AI视频/图像/音频创作技能包，功能全面
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `game-development`, `gdscript`, `godot`
...
- **标签：** `agent-skills`, `deepseek-harness`, `motion-design`, `remotion`, `video`
...
- **标签：** `dsh-plugin-bundle`, `dsh-plugin-desktop`
...
- **标签：** `mcp`, `financial`, `datasource`
- **推荐理由：** 基于tushare的本地财经MCP数据库与多Agent分析
...
- **推荐理由：** 一键配置MCP与Skills，支持多平台Agent
...
- **标签：** `sync`, `codex`, `bridge`, `mcp`
- **推荐理由：** Codex与DSH双向同步桥，自动MCP镜像
...
- **标签：** `ui-generator`, `design-systems`, `desktop-app`
- **推荐理由：** 开源设计AI工具，本地优先桌面端生成UI原型。
...
- **推荐理由：** 工业级GPT图像提示词引擎，530+案例模板持续更新。
...
- **推荐理由：** 为纯文本编码代理提供视觉能力，支持图片转结���化JSON。
...
- **标签：** `visualization`, `legal`, `svg`, `diagram`
- **推荐理由：** 生成诉讼时间轴和案件图表，可视化法律流程
...
- **标签：** `deepseek-harness`, `design-skills`, `dsh-plugin`, `ui`, `vibe-coding`
- **推荐理由：** 6种设计美学风格，告别AI感网站
...
- **推荐理由：** 对话内嵌入iOS模拟器与真机，22个自动化工具开箱即用
...
- **标签：** `control-plane`, `workflow`, `orchestration`
- **推荐理由：** 长期代理控制平面，跨框架持续工作编排
...
- **推荐理由：** 自部署插件生态雷达，自动发现并验证15900+候选插件
...
- **推荐理由：** 全面的DSH插件精选列表，覆盖CLI和Web UI
...
- **标签：** `plugin-registry`, `infrastructure`, `ui`
- **推荐理由：** 官方插件仓库管理控制台，提供插件开发引导工具
...
- **推荐理由：** 统一安全管理本机Agent Skills的加载工具
...
- **标签：** `ui`, `info-bar`, `pricing`, `tracking`
- **推荐理由：** 实时显示余额、峰谷定价，成本透明可控
...
- **标签：** `container`, `local`, `management`
- **推荐理由：** 本地容器化多版本管理，一键导入插件
...
- **标签：** `tts`, `xiaomi-mimo`, `dsh-plugin`
- **推荐理由：** 免费小米MiMo TTS语音合成接入插件。
...
- **标签：** `debug`, `compatibility`, `plugin-doctor`
- **推荐理由：** 诊断并解决插件兼容冲突问题。
...
- **推荐理由：** DeepSeek Harness生态精选插件目录，便于发现优质插件
...
- **标签：** `research`, `arxiv`, `latex`, `gpu-cluster`, `experiment-tracking`
- **推荐理由：** 一站式科研工作台：LaTeX论文、arXiv、实验追踪全链路管理
...
- **标签：** `academic-search`, `arxiv`, `mcp`, `citation-management`, `research`
- **推荐理由：** 跨数据库学术检索，支持引用核验、引文图谱与MeSH
...
- **标签：** `research`, `deep-research`, `agent-skills`
- **推荐理由：** 88个开源研究、社交智能与营销分析技能
...
- **推荐理由：** 专利侵权分析系统，支持多Agent RAG流程自动生成报告
...
- **标签：** `agent-evaluation`, `dsh-plugin`, `llm-evaluation`, `rag-evaluation`, `evaluation-framework`
- **推荐理由：** 提示词、RAG和智能体评估框架
...
- **推荐理由：** 实时追踪Token用量和API成本，可视化图表直观。
...
- **标签：** `dsh-ecosystem`, `dsh-plugin-market`
- **推荐理由：** 精选DSH插件仓库与生态总览
...
- **标签：** `awesome-list`, `mcp`, `recommender`
...
- **标签：** `agent-observability`, `agent-skills`, `agentic-ai`, `ai-agents`, `audio-understanding`, `computer-vision`, `deepseek`, `deepseek-harness`, `deepwatch`, `llm`, `local-first`, `mcp`, `multimodal`, `ocr`, `python`, `speech-to-text`, `typescript`, `verification`, `video-analysis`, `video-understanding`
- **推荐理由：** Give AI agents eyes, ears, and verifiable results. Watch Skill turns video, audi
...
- **推荐理由：** Harness终极入门指南，精选插件与实用工具包合集
...
- **推荐理由：** 学习Harness原理、浏览插件市场、掌握最佳实践
...
- **标签：** `awesome-list`, `mcp`, `catalog`, `orchestrator`
- **推荐理由：** 插件技能MCP服务全索引，涵盖patch与编排工具
...
- **推荐理由：** 精选280+社区插件目录，覆盖14类DSH生态组件
...
- **标签：** `agent-harness`, `awesome-list`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 覆盖插件、工具、技能的全面资源清单
...
- **标签：** `ai-agents`, `deepseek-harness`, `dsh-plugins`, `plugin-registry`
- **推荐理由：** 去垃圾数据的开源插件与技能注册表
...
- **推荐理由：** GitHub Actions自动抓取的双语插件目录。
