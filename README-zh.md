...
- **推荐理由：** 企业级多引擎工作区，统一Codex/DSH/OpenCode。
...
- **推荐理由：** 首个DSH视觉插件，让纯文本Agent具备图像理解能力
...
- **推荐理由：** 首个AI Agent虚拟终端，提供隔离沙箱与虚拟文件系统
...
- **标签：** `agent-skills`, `inference`, `deep-reasoning`
- **推荐理由：** 跨模型推理控制套件，支持深度推理与错误恢复
...
- **推荐理由：** 支持RPC、子代理、哈希编辑和MCP的编程智能体插件
...
- **标签：** `provider`, `oauth`, `multi-model`
- **推荐理由：** 通过OAuth使用ChatGPT/Claude/Grok订阅作为LLM提供者
...
- **标签：** `provider`, `command-code`, `model-access`
- **推荐理由：** 添加Command Code模型访问，支持智能模型选择和推理优化
...
- **推荐理由：** 在DSH内搜索、安装和验证GitHub插件的Skill
...
- **推荐理由：** 用于构建和验证DSH插件的Codex技能，插件开发者必备
...
- **标签：** `context`, `dsh`, `tool`, `ui`
- **推荐理由：** 上下文审计工具，检测token成本、重复与冲突指令。
...
- **推荐理由：** 类WorkBuddy技能选择器，快速插入/skill命令。
...
- **标签：** `migration`, `session-import`, `claude-code`, `dsh-plugin`
- **推荐理由：** 跨平台会话/记忆/技能迁移向导，支持Codex/Claude/DSH。
...
- **标签：** `cordis`, `dsh`, `dsh-plugin`, `mcp`
- **推荐理由：** 解锁DSH原生核心能力，统一编排Runtime与MCP。
...
- **标签：** `neovim`, `plugin-manager`, `lua`, `dev-tools`
- **推荐理由：** 轻量快速Neovim插件管理器
...
- **推荐理由：** 通过OAuth将ChatGPT模型集成到DSH中使用。
...
- **标签：** `desktop`, `ui`, `native-app`
- **推荐理由：** 官方桌面版客户端，原生系统运行DSH。
...
- **标签：** `taskboard`, `skills`, `dsh-plugin`
...
- **推荐理由：** TUI补��插件，含鲸鱼顶栏/实时状态/流式思考显示
...
- **标签：** `desktop-app`, `gui`, `electron`
- **推荐理由：** 功能全面的Windows桌面客户端，支持SSH和移动端远程访问
...
- **推荐理由：** 本地优先的AI桌面工作区，集成会话、文件和办公文档
...
- **标签：** `gui`, `interactive`, `visualization`
- **推荐理由：** 在对话中内联生成交互式UI组件，支持图表表单测验
...
- **推荐理由：** 将电子表格、文档、幻灯片等办公套件集成到Harness
...
- **推荐理由：** 本地优先的多模型AI Agent桌面应用，支持自进化技能
...
- **推荐理由：** DeepSeek Harness原生iOS客户端，支持实时Agent执行追踪
...
- **推荐理由：** CodePilot风格的跨平台桌面客户端，集成插件套件
...
- **推荐理由：** Electron桌面管理控制台，多标签管理Harness/插件/MCP
...
- **推荐理由：** 一键安装的桌面生物信息学智能体，支持离线模式和技能商店
...
- **标签：** `desktop`, `ssh`, `docker`, `sftp`, `db-client`
- **推荐理由：** 一体化DevOps桌面指挥中心
...
- **标签：** `desktop`, `electron`, `self-hosted`, `web-ui`
- **推荐理由：** Electron桌面版DSH集成
...
- **标签：** `multi-agent`, `agentic-workflow`, `swarm`
- **推荐理由：** 多智能体协同与自主工作流编排。
...
- **标签：** `agent-workspace`, `task-management`, `multi-device`
- **推荐理由：** 统一Agent工作空间，多设备跨端任务看板。
...
- **推荐理由：** 持久状态控制面，支持长周期多Agent工作流与跨会话连续性
...
- **推荐理由：** 智能技能路由，提升奖励21pp并减少近30%token消耗
...
- **推荐理由：** 赋予AI代理视听能力，提供可验证的音视频与OCR证据
...
- **标签：** `task-management`, `agent-workflow`, `local-first`
- **推荐理由：** 原生本地任务管理，支持Agent认领/审核工作流
...
- **推荐理由：** 全栈数学建模自动化，支持国赛/美赛，GitOps流水线
...
- **标签：** `self-evolving`, `review`, `prompt-engineering`
- **推荐理由：** 支持组合式任务控制与人工审查的自我进���Agent
...
- **推荐理由：** 为纯文本LLM增添多模态视觉能力，支持截图/UI分析
...
- **推荐理由：** 完整的逆向工程与授权渗透测试技能包，含87个SKILL
...
- **标签：** `skills`, `workbench`, `typescript`
- **推荐理由：** 插件化工作平台，支持全局技能管理与原生工作区集成
...
- **推荐理由：** AI编程导师技能，扫描架构生成阶梯式学习章节并自动判题
...
- **推荐理由：** 统一管理加载本地Agent Skills，支持安全管控与发现
...
- **推荐理由：** 党政机关公文格式国家标准全流程处理技能，含检查优化
...
- **推荐理由：** 诉讼时间轴与可视化工具集，自动生成大事记表和对抗图
...
- **推荐理由：** 专利侵权分析技能，输入公开号自动生成竞品侵权分析报告
...
- **标签：** `agent-skill`, `fengshui`, `iching`
- **推荐理由：** 融合八字/风水/易经的传统智慧AI技能，本地确定性引擎
...
- **推荐理由：** 官方文档索引库，作为按需可调用的Agent技能，适合开发者
...
- **推荐理由：** 工程质量门禁：需求审查、测试验证、交付证明，确保高质量
...
- **标签：** `spec-driven`, `agent-skill`
- **推荐理由：** 面向AI编程的智能体规格驱动开发技能，支持仓库级spec编程
...
- **推荐理由：** 数学建模全套技能：数据预处理、编程、论文写作、科研绘图
...
- **推荐理由：** 持久化多智能体协作，跨会话保持记忆和技能的Agent团队
...
- **推荐理由：** 多Agent协作框架，独立模型/MCP工具/共享工作空间
...
- **标签：** `qa`, `testing`
- **推荐理由：** 11个QA技能+共享知识库，让AI像资深测试工程师一样工作
...
- **推荐理由：** 5角色多Agent数学建模求解器，含33篇模型库和论文模板
...
- **标签：** `agent-skill`, `planning`
- **推荐理由：** 议行分离机制，先规划后执行，跨平台AI协作通用技能
...
- **标签：** `agent-skills`, `deepseek-harness`, `plugin`
- **推荐理由：** 腾讯官方SkillHub开放API插件，含完整文档与示例。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，运行时注册GDScript支持。
...
- **标签：** `agent-skill`, `dev-tools`, `plugin-dev`, `llm-agent`
- **推荐理由：** DSH插件开发标准技能包
...
- **标签：** `design-systems`, `prototyping`, `ui-generator`
- **推荐理由：** 本地优先的AI设计系统与原型工具。
...
- **标签：** `agent`, `ui`, `vibedesign`, `mcp`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持Agent团队协作与设计即代码
...
- **标签：** `design`, `visual`, `ppt`, `prototyping`
- **推荐理由：** AI驱动的设计工作室，支持可视化编辑和PPT制作
...
- **⭐ 星标：** 34
- **语言：** TypeScript
- **描述：** 面向 DeepSeek Harness 的电影感产品视频 Agent Skill，包含 152 张镜头配方卡、Remotion 模板、代码���件和音频资产。
...
- **标签：** `ai-video`, `directorx`, `storyboard`, `dsh-plugin`
- **推荐理由：** AI视频/图像/音频技能，支持故事板和多模型配置。
...
- **标签：** `browser-automation`, `Tencent`, `web`
- **推荐理由：** 无缝浏览器自动化，Agent使用真实登录会话。
...
- **标签：** `ios-automation`, `mobile-testing`, `webdriver`
- **推荐理由：** 将iOS模拟器和本地iPhone控制集成到Harness对话中
...
- **推荐理由：** 跨平台AI内容发现Agent，覆盖B站/抖音/小红书/YouTube等
...
- **标签：** `low-code`, `ai-agent`, `self-hosted`
...
- **标签：** `routing`, `infrastructure`, `harness-tools`
- **推荐理由：** 任务感知路由套件，智能请求路由分发。
...
- **推荐理由：** 两阶段预设方案：轻量引导后加载完整Standard工具集
...
- **标签：** `claude-code`, `codex`, `dsh-plugin`
...
- **标签：** `mcp`, `proxy`, `secrets`, `api-keys`
- **推荐理由：** Agent工具OpenRouter，统一MCP代理与密钥管理
...
- **标签：** `shop`, `marketplace`, `plugin-directory`
- **推荐理由：** 每日更新的插件市场，精选审核保障��件质量
...
- **推荐理由：** DSH插件商城的CLI管理工具，便捷安装和管理插件
...
- **标签：** `cybersecurity`, `ai-security`, `redteam`
- **推荐理由：** AI安全审计平台，检测幻觉包和安全漏洞
...
- **标签：** `remote-access`, `encryption`, `p2p`
- **推荐理由：** 端到端加密多端远程访问PC/Android/Web
...
- **推荐理由：** 本地财经数据库，支持MCP调用与微信/QQ/飞书集成
...
- **推荐理由：** 隔离容器管理多版本DSH，内置WebView和一键导入。
...
- **标签：** `curated-list`, `directory`, `dsh-plugin`
- **推荐理由：** 精选DSH插件索引列表
...
- **标签：** `registry`, `open-data`, `dsh-plugins`, `skills`
- **推荐理由：** 去重开放的插件注册中心
...
- **标签：** `directory`, `ecosystem`, `install-commands`
- **推荐理由：** 公共DSH插件发现目录
...
- **标签：** `curated`, `practical`, `dsh-plugin`
- **推荐理由：** ���用DSH插件推荐合集
...
- **标签：** `verified`, `directory`, `install-guidance`
- **推荐理由：** 经验证的DSH插件目录
...
- **标签：** `ecosystem`, `curated-list`, `coding-agent`
- **推荐理由：** DSH插件生态全景索引
...
- **推荐理由：** 为各AI编码Agent提供动画宠物画廊，趣味视觉个性化
...
- **标签：** `theme`, `skin`, `animated`
- **推荐理由：** 动态海洋主题，可调波浪、日落和玻璃拟态效果
...
- **标签：** `arxiv`, `latex`, `experiment-tracking`
- **推荐理由：** 一站式科研平台，支持LaTeX编译、arXiv管理、GPU任务编排
...
- **标签：** `scientific-discovery`, `agent-preset`, `epistemic-loop`
- **推荐理由：** 将认知循环方法论引入科学发现和假设验证
...
- **标签：** `ai-encyclopedia`, `rag`, `awesome-list`
- **推荐理由：** 覆盖大模型、Agent、RAG、MLOps的全维度AI资源百科
...
- **推荐理由：** 零配置搜索6.5万篇中国开放平台API文档，跨多平台支持
...
- **标签：** `kline`, `deepseek-harness-plugin`, `ftshare`
- **推荐理由：** 交互式K线分析插件，支持多市场行情与技术指标分析。
...
- **标签：** `data-visualization`, `token-usage`, `ai-cost`, `api-balance`
- **推荐理由：** 实时Token用量成本可视化
...
- **标签：** `guide`, `navigation`, `manual`
- **推荐理由：** 插件说明书与场景化导航，选型无忧
...
- **标签：** `radar`, `research`, `2origin`
- **推荐理由：** 基于实证的插件雷达图，实验室级参考
...
- **标签：** `directory`, `awesome-list`, `automated`
- **推荐理由：** GitHub Actions每日自动更新双语目录
...
- **标签：** `web-ui`, `plugin-ecosystem`, `aggregation`
...
- **推荐理由：** 全面的DSH插件、技能、MCP服务器和补丁精选列表
...
- **推荐理由：** DeepSeek Harness终极指南和资源库，涵盖入门教程
...
- **推荐理由：** 开源版锤子便签，支持Docker私有化部署和多租户
...
- **推荐理由：** 收录280+个插件的精选目录，按14类分类，方便插件发现
...
- **标签：** `awesome`, `plugin-catalog`
- **推荐理由：** 非官方社区插件目录，供参考而非功能插件。
...
- **标签：** `awesome-list`, `dsh-plugin`, `skills`, `mcp`
- **推荐理由：** 精选DSH插件、工具和技能的社区索引列表。
...
- **推荐理由：** 重定向至更新的awesome-dsh-plugin仓库。
