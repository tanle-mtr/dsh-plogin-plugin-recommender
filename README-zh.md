...
- **推荐理由：** 终端原生DeepSeek编码Agent，支持前缀缓存稳定运行。
...
- **标签：** `agent-os`, `mcp`, `self-improving`, `coding`
- **推荐理由：** 自进化Agent操作系统，评估门控MCP演进
...
- **标签：** `agent-control`, `long-horizon`, `workflow`, `codex`, `claude-code`
- **推荐理由：** 长期Agent控制平面，跨Codex/Claude Code等Harness的持久化治理
...
- **标签：** `preset`, `bootstrap`, `standard-tools`, `dsh-plugin`
...
- **标签：** `virtual-terminal`, `vfs`, `sandbox`, `agent-tools`, `bash`
- **推荐理由：** 首个虚拟终端，为AI Agent提供沙箱化VFS和终端环境
...
- **推荐理由：** 带RPC插件、子代理和MCP支持的编码Agent工具
...
- **标签：** `dsh-plugin`, `plugin-discovery`
...
- **标签：** `bioinformatics`, `desktop-app`, `offline`
- **推荐理由：** 适合生物医学研究者的离线桌面Agent，技能商店+一键安装。
...
- **标签：** `plugin-dev`, `codex`, `scaffold`
- **推荐理由：** 快速搭建和验证DSH/Cordis插件的可复用Codex技能。
...
- **推荐理由：** 统一编排DSH核心能力：Runtime/Provider/Plugin/Skill/MCP
...
- **标签：** `tts`, `xiaomi-mimo`, `plugin`
- **推荐理由：** 接入免费小米MiMo TTS朗读功能
...
- **标签：** `dsh-plugin`, `agent-tools`
- **推荐理由：** 任务前自动预检推荐插件，提升插件库利用率
...
- **推荐理由：** AI驱动的本地优先设计系统工具，类Figma替代品。
...
- **标签：** `browser-extension`, `chat-enhancement`, `multi-platform`
- **推荐理由：** 一键增强各大AI聊天平台，功能全面的浏览器扩展
...
- **标签：** `desktop`, `ui`, `official`
- **推荐理由：** DeepSeek Harness官方桌面客户端，本地化体验
...
- **标签：** `sidebar`, `ui`, `terminal`, `git`, `file-editor`
- **推荐理由：** 开���侧边栏底座，内置文件编辑/终端/对话/Git/子代理页面
...
- **标签：** `taskboard`, `ui`, `codex-plugin`, `workflow`, `productivity`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH工作区
...
- **标签：** `tui`, `terminal`, `status`, `dsh-plugin`, `ui`
- **推荐理由：** TUI补充插件，提供鲸鱼顶栏/实时状态/流式思考显示
...
- **标签：** `floating-widget`, `ui`, `balance-monitor`
- **推荐理由：** 可爱鲸鱼悬浮组件，实时监控账户余额
...
- **标签：** `desktop-app`, `gui`, `electron`, `ssh-client`
- **推荐理由：** 零配置Windows桌面客户端，内置SSH和插件支持
...
- **推荐理由：** 本地优先的AI桌面工作空间，支持会话和项目文件管理
...
- **标签：** `gui`, `ui-components`, `charts`, `forms`
- **推荐理由：** 在回复中内联渲染交互式UI组件的插件
...
- **标签：** `conversation`, `canvas`, `workspace`, `non-linear`
- **推荐理由：** 基于画布的非线性对话工作空间，支持分支会话
...
- **标签：** `desktop-app`, `desktop-automation`, `multi-model`, `privacy`
- **推荐理由：** 本地优先的多模型桌面AI助手，支持自进化技能与隐私保护
...
- **标签：** `ios`, `android`, `mobile-app`, `kmp`
- **推荐理由：** 原生移动客户端，通过WebSocket同步工作区、会话和Agent轨迹
...
- **推荐理由：** 在对话中嵌入iOS模拟器和真机，提供22个Agent自动化工具
...
- **推荐理由：** CodePilot风格的跨平台桌面客户端和插件套件
...
- **推荐理由：** 桌面管理控制台，多Tab管理Harness/插件/MCP/Skills。
...
- **标签：** `dsh-plugin`, `workbench`, `plugin-system`, `skills`
- **推荐理由：** 基于DSH的插件化Web工作台，支持全局技能管理与工作区。
...
- **标签：** `ui-extension`, `status-bar`, `info-display`
- **推荐理由：** 底部状态栏实时显示模型、余额、峰谷定价与倒计时。
...
- **⭐ 星标：** 31
- **语言：** TypeScript
- **描述：** DSH 上下文注入审���插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `ui-tool`, `audit`
- **推荐理由：** 上下文审计工具：可视化Token成本，检测重复与冲突。
...
- **推荐理由：** 类似WorkBuddy的skill选择器，提升输入体验
...
- **推荐理由：** 集成数据库/SSH/Docker等DevOps桌面工具
...
- **推荐理由：** 自演化上下文数据库，统一Agent记忆与RAG能力。
...
- **推荐理由：** 腾讯开源LLM知识平台，支持RAG与自维护Wiki。
...
- **标签：** `memory`, `rag`, `long-term-memory`, `local-first`
- **推荐理由：** 本地优先的便携记忆层，支持Markdown原生RAG
...
- **标签：** `memory`, `rag`, `knowledge`, `agent-skills`, `scope`
- **推荐理由：** Agent记忆管理套件，支持记忆存储、精炼和检索
...
- **标签：** `agent-memory`, `skill-md`, `embeddings`, `vector-search`
- **推荐理由：** 将零散技能文件转化为可检索的Agent技能知识库
...
- **推荐理由：** 将经验提炼为可复用的AI Agent技能知识蒸馏工具。
...
- **标签：** `vision`, `ocr`, `image-to-text`, `multimodal`, `agent-skills`
- **推荐理由：** 首个视觉插件，让纯文本Agent具备图像理解和OCR能力
...
- **标签：** `skills`, `routing`, `workflow`, `orchestration`, `vibe-coding`
- **推荐理由：** 智能技能路由和工作流编排，奖励提升21个百分点
...
- **标签：** `agent-skills`, `deep-reasoning`, `cognitive-enhancement`
- **推荐理由：** 推理时控制，支持深度推理与长程任务自动化
...
- **标签：** `novel-writing`, `creative`, `rag`
- **推荐理由：** 系统化小说创作流程，支持本地Ollama模型
...
- **标签：** `novel-writing`, `creative-writing`, `workflows`, `screenwriting`
- **推荐理由：** 网文、短剧、游戏和视频解说创作工作流插件
...
- **标签：** `agent-observability`, `multimodal`, `video-analysis`, `ocr`
- **推荐理由：** 为AI代理提供音视频感知能力，生成可验证的时间戳证据
...
- **推荐理由：** 将认识论循环框架引入Agent，实现自动化科学发现
...
- **标签：** `dsh-plugin`, `agent-review`
- **推荐理由：** 可组合可审查的Agent自演进工作流
...
- **标签：** `agent-skills`, `marketplace`
- **推荐理由：** 88个开源Skill覆盖研究营销工作流
...
- **推荐理由：** AI编程导师，扫描架构生成阶梯式教程，自动判题与Code Review。
...
- **推荐理由：** 自带25个工程技能，300亿token训练，开发效率提升10倍。
...
- **推荐理由：** Web搜索、研究、金融等技能，支持多Agent平台调用。
...
- **标签：** `agent-skill`, `tradition-wisdom`, `dashboard`
- **推荐理由：** 融合八字风水中医的本地确定性Agent技能，附可视化面板。
...
- **推荐理由：** 官方插件开发知识库，即需即用的Agent技能，中英双语。
...
- **推荐理由：** 基于仓库的规格驱动开发技能，遵循DeepSeek工程模式。
...
- **标签：** `quality-gate`, `engineering-discipline`, `guard`
- **推荐理由：** 发布前的工程纪律门禁：需求审查+实现测试+交付证明。
...
- **标签：** `math-modeling`, `skill`, `research`
- **推荐理由：** 数学建模全流程技能包：数据预处理、编程、论文写作、绘图。
...
- **推荐理由：** 多Agent团队协作框架，独立模型/技能/共享工作区。
...
- **推荐理由：** 11个QA Agent技能，AI驱动测试自动化与用例编写。
...
- **标签：** `skillhub`, `official`
- **推荐理由：** 腾讯官方SkillHub文档示例与插件
...
- **推荐理由：** 新增Command Code模型提供商，支持实时目录和推理感知选择
...
- **标签：** `mcp`, `deepseek-harness`
- **推荐理由：** 基于Tushare的本地财经数据库+MCP
...
- **推荐理由：** 零配置搜索65,600+中国开放平台API文档，支持Skill调用。
...
- **标签：** `dsh-plugin`, `mcp`, `cloud`, `sdk`
- **推荐理由：** 华为云官方插件，提供MCP工具、安全护栏和云端沙箱。
...
- **标签：** `chatgpt`, `oauth`, `subscription`, `deepseek-harness`
- **推荐理由：** 通过OAuth在DSH中直接访问ChatGPT模型
...
- **推荐理由：** 530+图像生成提示词库，含20+工业级可复用模板。
...
- **标签：** `design`, `vector`, `agent-teams`, `flutter`, `react`
- **推荐理由：** AI原生矢量设计工具，支持并发Agent团队和代码化设计
...
- **标签：** `ai-video`, `ai-image`, `media`
- **推荐理由：** AI视频/图像/音频创作工具，支持多模态模型
...
- **标签：** `video`, `cinematic`, `remotion`
- **推荐理由：** 电影感视频Agent，含152张镜头配方和Remotion模板
...
- **标签：** `browser`, `content-discovery`, `bilibili`, `youtube`, `privacy`
- **推荐理由：** 跨平台AI内容发现Agent，支持B站/YouTube/小红书/知乎等
...
- **标签：** `web-search`, `web-fetch`, `free`, `no-signup`
- **推荐理由：** 最强免费网页搜索插件，无需API或注册即可使用
...
- **标签：** `browser-automation`, `playwright`, `anti-detect`, `mcp-server`
- **推荐理由：** 持久化浏览器身份，支持指纹伪装和自动化操作
...
- **推荐理由：** Tabbit浏览器与Playwright集成，实现DSH自动化
...
- **标签：** `web`, `plugin-marketplace`, `ecosystem`
- **推荐理由：** DSH插件Web分发生态，创意工坊式插件市场
...
- **标签：** `routing`, `infrastructure`, `runtime`
- **推荐理由：** 任务感知推理模式路由套件，运行时注入增强
...
- **标签：** `marketplace`, `dsh-plugin`, `installer`
...
- **标签：** `routing`, `quota`, `failover`, `multi-model`, `utility`
- **推荐理由：** 多模型智能路由，支持配额管理和自动故障转移
...
- **标签：** `cybersecurity`, `ai-security`, `redteam`, `vulnerabilities`
- **推荐理由：** AI代码安全审计平台，检测幻觉包和安全漏洞
...
- **推荐理由：** 将电子表格、文档、幻灯片等完整办公套件集成到Harness中
...
- **标签：** `llm-provider`, `oauth`, `subscription`, `multi-model`
...
- **推荐理由：** 原生本地任务管理，SQLite存储项目，支持Agent认领与审查
...
- **推荐理由：** 追踪会话与每日费用、预算和多LLM提供商的token用量
...
- **标签：** `directory`, `marketplace`, `dsh-plugin`
- **推荐理由：** 1958个仓库的中英文插件目录
...
- **推荐理由：** 开源锤子便签复刻，支持Docker一键部署和skill调用
...
- **⭐ 星标：** 86
- **语言：** TypeScript
- **描述：** 一个面向 DeepSeek Harness 的统一能力管理插件，为 Tools/Skills 提���常驻、按需、禁用三档暴露策略，以减少上下文占用并支持运行时动态发现与执行。
- **标签：** `dsh-plugin`, `mcp`, `skills`, `cordis`
- **推荐理由：** 统一能力管理，支持按需/禁用三档策略，减少上下文占用。
...
- **标签：** `dsh-plugin`, `skills`, `deepseek-harness`
- **推荐理由：** 统一加载并安全管理本地Agent Skills，支持安全校验。
...
- **推荐理由：** 插件生态基建：浏览器控制台管理官方仓库，附开发引导。
...
- **标签：** `container`, `multi-version`, `tooling`
- **推荐理由：** 多版本DSH隔离容器管理，一键导入插件
...
- **推荐理由：** 从Claude Code/Codex迁移会话、技能和记忆的向导
...
- **标签：** `utility`, `conflict-resolution`, `plugin`
- **推荐理由：** 自动解决插件及版本冲突问题
...
- **标签：** `mascot`, `pixel-art`, `animated`, `cli`, `theme`
- **推荐理由：** 编程Agent宠物动效画廊，支持Codex/Claude Code等终端装饰
...
- **标签：** `theme`, `glassmorphism`, `desktop`, `customizable`
...
- **标签：** `animated-background`, `theme`, `glassmorphism`, `ocean`
- **推荐理由：** 精美的动态海洋主题，支持可调波浪、日落效果与玻璃拟态
...
- **推荐理由：** 程序员鱼皮整理的AI资源大全与Vibe Coding入门教程。
...
- **标签：** `documentation`, `wiki`, `code-analysis`, `ai-context`, `research`
- **推荐理由：** 将代码转换为结构化技术文档，为AI Agent和团队提供上下文
...
- **标签：** `awesome-list`, `curated-list`, `bilingual`, `plugins`
- **推荐理由：** 双语精选DSH插件与工具参考���源目录
...
- **⭐ 星标：** 536
- **语言：** TypeScript
- **描述：** Mimir — 一站式科研工作台插件：LaTeX 论���边写边编译、arXiv 文献管理、实验追踪、指标图表、GPU 服务器 SSH 任务编排，管理科研全周期。An open-source research workbench plugin for the whole research cycle.
- **标签：** `research-tools`, `latex`, `arxiv`, `gpu-cluster`
- **推荐理由：** 一站式中文科研工作台，支持LaTeX、arXiv和GPU管理
...
- **标签：** `ai-knowledge-base`, `awesome-list`, `mcp`, `rag`
- **推荐理由：** 全面的中文AI资源百科和开发者一站式导航库
...
- **标签：** `awesome-list`, `plugin-curation`, `ecosystem`
- **推荐理由：** 全面的DSH生态资源索引，涵盖插件、技能、MCP服务器和补丁
...
- **推荐理由：** DeepSeek Harness终极指南，含快速入门、精选插件与实用工具
...
- **标签：** `handbook`, `mcp`, `sandbox`
- **推荐理由：** 涵盖运行时、MCP、沙箱的Agent手册
...
- **标签：** `industry-research`, `company-research`
- **推荐理由：** 行业研究与公司分析方法论技能包，含产业链图谱
...
- **推荐理由：** cordis.run官方自动生成的Cordis插件精选清单
...
- **推荐理由：** 输入专利号自动生成竞品侵��分析报告，支持多Agent调用。
...
- **标签：** `plugin-registry`, `curated-list`, `deepseek-harness`
- **推荐理由：** 开放数据插件注册表，含垃圾过滤
...
- **标签：** `awesome-list`, `enhancement`, `terminal`
- **推荐理由：** 精选插件含通知和LLM跟踪检查功能
...
- **标签：** `plugin-discovery`, `ecosystem`, `validation`
- **推荐理由：** 持续发现并验证2万+ DSH插件的生态雷达
...
- **推荐理由：** 收录280+插件的精选目录，Dsh生态发现入口必备。
...
- **标签：** `dsh-plugin`, `portal`
- **推荐理由：** 无依赖静态页面，浏览和安装插件体验流畅
...
- **标签：** `dsh-plugins`, `dsh-ecosystem`, `awesome-list`
- **推荐理由：** DSH插件集合及生态资源汇总
...
- **标签：** `awesome-list`, `repository`, `deepseek-harness`
- **推荐理由：** 已迁移，请前往新仓库地址
...
- **标签：** `awesome-list`, `bilingual`, `auto-updating`, `dsh-plugins`
- **推荐理由：** Auto-updating curated DSH plugin list with bilingual README
...
- **标签：** `awesome-list`, `curated`, `dsh-plugins`, `ui`, `skills`
- **推荐理由：** Curated DSH plugins covering models, tools, UI and agent loops
