...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 194,922 | Core plugin platform for the entire DSH ecosystem |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,417 | Best DSH design plugin with local-first desktop ex |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,381 | Deploy multi-agent swarms with autonomous workflow |
...
- [UI 与桌面扩展](#ui-与桌面扩展) — 13 个插件
- [记忆与知识（RAG）](#记忆与知识（rag）) — 11 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 41 个插件
...
- [UI 与桌面扩展](ui-desktop-extensions-zh) (13)
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (11)
- [Agent 技能与工作流](agent-skills-workflows-zh) (41)
...
- **推荐理由：** 原生DeepSeek终端编程Agent，支持前缀缓存
...
- **标签：** `agent-orchestration`, `workspace`, `agent-skills`
- **推荐理由：** 统一多设备智能体工作区，支持跨平台任务看板。
...
- **标签：** `rust`, `agent`, `desktop`
- **推荐理由：** Rust高性能智能体运行时，桌面体验优秀。
...
- **标签：** `code-review`, `lint`, `quality`
- **推荐理由：** 基于12部经典工程著作的AI代码审查工具。
...
- **标签：** `dsh`, `containers`, `ui`
- **推荐理由：** 隔离容器跑多版本DSH，内置WebView界面
...
- **标签：** `mcp`, `capabilities`, `ui`
- **推荐理由：** 统一管理MCP工具与技能暴露级别和模式
...
- **标签：** `plugin-development`, `cordis`, `development`
- **推荐理由：** 便携技能，用于开发审计 DSH 插件，含 DSH bundle 适配器
...
- **⭐ 星标：** 13
- **语言：** Unknown
- **描述：** DSH 插件开��踩坑与做法档案（skill + 文档）：cordis 双副本、tsconfig 三件套、Windows junction、多帧 zstd 等实测记录
- **标签：** `plugin-development`, `documentation`, `best-practices`
- **推荐理由：** 插件开发踩坑指南，含 cordis 双副本、tsconfig、Windows junction 等实测方案
...
- **标签：** `dsh-web`, `ui`, `plugin-aggregator`
- **推荐理由：** DSH Web插件聚合生态，一切皆插件的创意工坊。
...
- **标签：** `desktop-companion`, `electron`
- **推荐理由：** 桌面陪伴平台，含动画宠物与插件SDK。
...
- **推荐理由：** 功能丰富的Windows桌面客户端，支持SSH与远程访问
...
- **标签：** `canvas`, `creative`, `agentic-workflow`
- **推荐理由：** 无限画布结合创意代理技能，拓展协作空间。
...
- **标签：** `desktop`, `tauri`, `devops`, `ssh`, `docker`
...
- **推荐理由：** 本地优先、用户自持的Markdown记忆层，支持RAG自进化。
...
- **标签：** `embeddings`, `retrieval`, `semantic-search`
- **推荐理由：** 将SKILL.md文件转为可检索的Agent知识库
...
- **标签：** `skills`, `image-generation`, `dsh-plugin`
- **推荐理由：** 530+图像生成提示词提炼为可复用技能
...
- **标签：** `harness-engineering`, `tutorial`, `dsh`
- **推荐理由：** 从零开始学习DSH harness工程的入门教程。
...
- **标签：** `routing`, `cordis`, `agent-workflow`
- **推荐理由：** 任务感知推理模式路由注入器，优化智能体工作流。
...
- **标签：** `agent-skills`, `cognitive`, `j-space`
- **推荐理由：** 基于Anthropic认知架构的智能体增强技能套件。
...
- **标签：** `skills`, `workflow`, `automation`
- **推荐理由：** 智能路由和编排智能体工作流的核心技能。
...
- **标签：** `video`, `generative-ai`, `skill`
- **推荐理由：** 一键自动生成Vox风格纸拼贴解释视频。
...
- **标签：** `math-modeling`, `gitops`, `autonomous`
- **推荐理由：** 全栈式自动数学建模竞赛Skill，国赛美赛通用
...
- **标签：** `vision`, `multimodal`, `ui-analysis`
- **推荐理由：** 为纯文本LLM赋予视觉能力，支持截图/UI分析
...
- **推荐理由：** 跨Claude/Codex/OpenCode的通用技能平台
...
- **推荐理由：** 移植superpowers的TDD、调试、规划技能
...
- **标签：** `bazi`, `fengshui`, `agent-skill`
- **推荐理由：** 融合八字风水中医，本地确定性引擎+可视化
...
- **标签：** `agent-handoff`, `context-engineering`, `memory`
- **推荐理由：** 跨平台会话记忆，Agent接力无缝衔接
...
- **标签：** `skill-picker`, `ui`, `dsh-plugin`
- **推荐理由：** WorkBuddy风格技能选择器，嵌入composer
...
- **标签：** `code-simplification`, `agent-skill`, `codex`
- **推荐理由：** 源自DSH官方的代码简化技能，适配Codex
...
- **标签：** `gongwen`, `docx`, `wps-office`
- **推荐理由：** 遵循国标党政机关公文格式，支持写作、检查、模板生成全流程
...
- **标签：** `agent-skills`, `cli`, `database`
- **推荐理由：** 包管理器含 42 技能，覆盖 Agent 工程、数据库、K8s 等��景
...
- **标签：** `directorx`, `video`, `image-generation`
- **推荐理由：** DirectorX 插件，支持 AI 视频/图片/音频技能和知识语料库
...
- **标签：** `skills`, `gamification`, `wayfinder`
- **推荐理由：** 游戏化任务系统，迷雾地图配合任务栏推进，增强执行力
...
- **标签：** `agent-skills`, `engineering-practices`
- **推荐理由：** 适合AI代理的工程规范技能，支持决策与证据报告。
...
- **标签：** `migration`, `agent-migration`, `mcp`
- **推荐理由：** 一键将Claude Code/Codex配置迁移至DSH，支持技能与MCP导入。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，覆盖GDScript与引擎工作流。
...
- **标签：** `codex`, `mcp`, `sync`
- **推荐理由：** OpenAI Codex与DSH双向桥接，实时MCP镜像
...
- **推荐理由：** 在 Web UI 设置页统一管理技能与 MCP 服务器，无需手动配置
...
- **标签：** `mcp`, `zhihu`, `search`
- **推荐理由：** 知乎搜索与知答 MCP 服务，支持官方开放平台 API
...
- **推荐理由：** 粘贴 JSON 即连 MCP 热加载，上传 zip 自动识别技能，免重启
...
- **标签：** `design-tool`, `vector`, `design-as-code`
- **推荐理由：** AI原生矢量设计工具，支持并发Agent团队和设计即代码。
...
- **标签：** `dingtalk`, `communication`, `dsh-plugin`
- **推荐理由：** 钉钉IM通道支持情绪感知流式模式
...
- **标签：** `cost-tracking`, `token-usage`
- **推荐理由：** 会话/每日费用追踪，支持预算提醒
...
- **标签：** `anime`, `bangumi`, `magnet`
- **推荐理由：** 多源搜番，Bangumi评分，支持磁力链接。
...
- **标签：** `backup`, `undo`, `crash-recovery`
- **推荐理由：** 崩溃恢复、撤销、快照和一键安全模式。
...
- **标签：** `backup`, `config-manager`, `sync`
- **推荐理由：** 完整配置备份恢复，支持WebDAV同步迁移
...
- **推荐理由：** 实时展示Token用量、成本估算与API余额的Web面板。
...
- **推荐理由：** 提供暗 SaaS、苹果极简等 6 种风格，让设计更有质感
...
- **标签：** `themes`, `color-scheme`, `dark-mode`
- **推荐理由：** 34 款经典配色一键切换，重启不丢失，告别默认皮肤
...
- **标签：** `awesome-list`, `plugins`, `catalog`
- **推荐理由：** 收录280+社区插件，14类目录便于探索
...
- **推荐理由：** 面向提示词、RAG 和 Agent 的可观测评估框架
...
- **标签：** `no-code`, `internal-tools`, `dsh-plugin`
- **推荐理由：** AI驱动的零代码平台，快速构建业务系统
...
- **⭐ 星标：** 383
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源��。
- **标签：** `deepseek-harness`, `deepseek-harness-plugin`, `dsh`, `dsh-plugin`, `theme`
- **推荐理由：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（
...
- **标签：** `agent`, `agent-framework`, `agentic-ai`, `agents`, `ai-agent`, `ai-assistant`, `automation`, `desktop-app`, `desktop-automation`, `dsh-plugin`, `dsh-plugins`, `llm`, `local-first`, `mcp`, `privacy`, `productivity`, `react`, `tauri`, `typescript`
- **推荐理由：** Open-source alternative to Claude Cowork — a local-first AI agent desktop app · 
...
- **标签：** `dsh`, `dsh-plugin`
- **推荐理由：** GenUI for DeepSeek Harness: interactive UI components rendered inline in assista
...
- **推荐理由：** DeepSec — AI Security Offense & Defense Platform. Shield audits AI-generated cod
...
- **标签：** `agent`, `claude-code`, `codex`, `deepseek-harness`, `grok`, `skill`
- **推荐理由：** ccteam turns the coding agents you already run (Claude Code, Codex, Grok, Kimi, 
...
- **推荐理由：** Curated DeepSeek Harness (DSH) plugins, extensions, tools, skills, clients, runt
...
- **推荐理由：** DeepSeek 专用破甲插件「无限二代」dsh-infinite-gen-2 — armor-breaking plugin for DeepSeek稳定化破
...
- **推荐理由：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭
...
- **标签：** `ai-agent`, `chatgpt`, `claude`, `codex`, `deepseek-harness`, `dsh-plugin`, `grok`, `llm`, `oauth`, `typescript`
- **推荐理由：** Use ChatGPT (Codex), Claude, and Grok (X Premium) subscriptions as DeepSeek Harn
...
- **推荐理由：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bri
...
- **标签：** `dsh`, `dsh-bundle`, `dsh-plugin`, `dsh-plugin-market`, `dsh-plugin-verify`, `dsh-plugins`, `dsh-skill`
- **推荐理由：** AI skills大全 mcp ai知识库 Agent 全维度 AI 资源百科，DSH插件 收录大模型、智能 Agent、RAG 检索增强、多模态、MLOps、
...
- **推荐理由：** DeepSeek Harness (DSH) plugin: a live iOS Simulator — and a USB-connected iPhone
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** DSH (DeepSeek Harness) 原理学习、插件市场与最佳实践 · Learn DSH principles, plugin marketplace
...
- **标签：** `dsh`, `oauth`, `photo-wall`
- **推荐理由：** 内测收官合影墙，GitHub OAuth 零权限登录，实用社交功能
...
- **标签：** `awesome-list`, `curated`, `catalog`
- **推荐理由：** 精选插件目录，适合快速发现优质DSH插件。
...
- **标签：** `awesome-list`, `registry`, `discovery`
- **推荐理由：** 公开插件目录，提供安装命令和生态发现功能。
...
- **推荐理由：** 去垃圾插件的开源数据注册表，含bundles和技能目录。
...
- **标签：** `ai-agent`, `deepseek`, `deepseek-harness`, `desktop-app`, `dsh`, `electron`, `harness`, `modlens`, `plugin`
- **推荐理由：** 桌面版 DeepSeek Harness：Electron 壳 + 自托管 ds
