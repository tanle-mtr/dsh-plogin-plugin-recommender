...
- **推荐理由：** DSH核心底座，万物皆插件的AI Agent开发框架
...
- **标签：** `multi-agent`, `memory`, `mcp`, `skills`
- **推荐理由：** 自我进化的多模型AI助手，自带记忆与知识库
...
- **标签：** `coding-agent`, `prompt-caching`, `tool-use`, `tui`
...
- **标签：** `vision`, `ocr`, `image-to-text`, `dsh-plugin`
- **推荐理由：** 首个视觉插件，为纯文���编码代理提供结构化JSON图像分析能力
...
- **标签：** `preset`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 两阶段预设：最小引导+标准工具集(98/99)的完整配置方案
...
- **推荐理由：** 一键安装并切换Claude Code/Codex/DeepSeek Harness等多个AI编码工具
...
- **标签：** `code-review`, `clean-architecture`, `lint`
- **推荐理由：** 基于12本经典工程的AI代码审查，含引用与严重性标签
...
- **标签：** `armor-breaking`, `generation`, `performance`
- **推荐理由：** DeepSeek专用破甲插件，稳定提升生成质量
...
- **标签：** `coding-agent`, `mcp`, `terminal`
- **推荐理由：** 支持子智能体、hashline编辑和MCP的全功能编码代理。
...
- **标签：** `context-migration`, `dsh-plugin`, `session`
- **推荐理由：** 跨预设会话迁移，保留状态与模型意图
...
- **标签：** `plugin-dev`, `agent-skill`, `dsh`
- **推荐理由：** 开发DSH插件的标准Agent技能，覆盖服务/事件/工具/打包全流程
...
- **标签：** `plugin-creator`, `codex`, `cordis`
- **推荐理由：** 可复用的DSH和Cordis插件脚手架、构建与验证技能
...
- **标签：** `plugin-guide`, `knowledge-base`, `scaffold`
- **推荐理由：** 按需调用的插件开发知识库，含官方文档存档与脚手架
...
- **标签：** `ui-generator`, `design-systems`, `desktop-app`, `figma-alternative`
- **推荐理由：** 本地优先的AI设计工具，将编码Agent变为设计师
...
- **标签：** `desktop`, `dsh-plugin-desktop`, `cordis`
- **推荐理由：** 为DSH插件生态打造的现代化桌面端解决方案
...
- **标签：** `dsh`, `web-ui`, `plugin`
- **推荐理由：** DSH插件Web分发聚合生态平台
...
- **标签：** `sidebar`, `ui`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话/Git等页面，支持三方拓展
...
- **标签：** `taskboard`, `ui`, `cli`, `dsh-plugin`
- **推荐理由：** 现代化可灵活嵌入的任务面板，支持Codex和DeepSeek Harness
...
- **标签：** `tui`, `terminal`, `ui`, `dsh-plugin`
- **推荐理由：** 鲸鱼顶栏/流式思考/实时状态的Claude Code风格TUI补位插件
...
- **标签：** `widget`, `desktop`, `monitoring`
- **推荐理由：** 可爱漂浮鲸鱼组件，实时追踪DeepSeek账户余额
...
- **标签：** `desktop`, `electron`, `office`
- **推荐理由：** 本地优先Electron桌面工作台，支持DSH会话与办公
...
- **标签：** `ui-components`, `genui`, `charts`
- **推荐理由：** 聊天内直接渲染图表、表单、测验等交互UI
...
- **标签：** `desktop-client`, `windows`, `ssh`
- **推荐理由：** Windows桌面客户端，内置SSH和插件系统
...
- **标签：** `office`, `deepseek-harness`
- **推荐理由：** 为DSH提供电子表格、文档、幻灯片等完整办公功能。
...
- **标签：** `desktop`, `electron`, `deepseek-harness`
- **推荐理由：** CodePilot风格的DSH桌面客户端插件套件。
...
- **标签：** `ios`, `mobile`, `deepseek-harness-plugin`
- **推荐理由：** 原生iOS客户端，随时随地接入DeepSeek Harness
...
- **标签：** `ui`, `info-bar`, `pricing`, `balance-tracking`
- **推荐理由：** 底部信息栏，实时显示余额、价格和定价周期
...
- **推荐理由：** 自进化上下文数据库，统一Agent记忆与知识RAG
...
- **标签：** `rag`, `vector-search`, `knowledge-base`, `llm`
- **推荐理由：** 开源知识平台，集成RAG检索与语义搜索能力
...
- **推荐理由：** 专注记忆管理的AI代理套件，支持上下文记忆与知识迭代优化
...
- **推荐理由：** 基于Milvus和Markdown的持久统一记忆层，支持混合与语义检索
...
- **标签：** `context`, `memory`, `dashboard`
- **推荐理由：** 最佳上下文管理与洞察工具，含面板和浏览器功能
...
- **标签：** `skill-corpus`, `vector-search`, `retrieval`
- **推荐理由：** 将技能文件转为可检索的知识库，支持评估
...
- **标签：** `context-audit`, `token-tracking`, `ui-panel`
- **推荐理由：** 审计上下文注入，检测Token成本与冲突
...
- **标签：** `handoff`, `session-memory`, `cross-platform`
- **推荐理由：** 跨平台会话记忆，Agent交接连续性保障
...
- **标签：** `multi-agent`, `swarm`, `mcp-server`, `skills`
- **推荐理由：** 多Agent蜂群元框架，支持自主工作流编排
...
- **标签：** `knowledge-distillation`, `meta-skill`, `agent-persona`
- **推荐理由：** 将优秀Agent思维模式提炼为可复用的通用技能
...
- **标签：** `agent`, `workspace`, `mcp`
- **推荐理由：** 跨设备统一Agent工作空间与任务看板
...
- **标签：** `agent`, `workflow`, `loop`
- **推荐理由：** 长时间跨度Agent控制面，持久治理工作流
...
- **标签：** `agent`, `enterprise`, `multi-agent`
- **推荐理由：** 企业级多Agent工作台，支持可视化编辑器
...
- **标签：** `skills`, `workflow`, `orchestration`, `dsh-plugin`
- **推荐理由：** 智能技能路由与工作流编排，SkillsBench准确率提升21pp并减少29.6%token
...
- **标签：** `cognitive`, `agent-skills`, `global-workspace`, `dsh-plugin`
- **推荐理由：** 基于Anthropic J-space全局工作空间研究的AI认知增强技能套件
...
- **推荐理由：** 为纯文本模型打造视觉工具箱：OCR、图片问答、UI还原
...
- **标签：** `desktop-agent`, `local-first`, `multi-model`
- **推荐理由：** 本地优先的AI代理，支持自进化技能
...
- **标签：** `taskboard`, `sqlite`, `workflow`
- **推荐理由：** 原生SQLite任务看��，支持Agent协作
...
- **标签：** `skill`, `math-modeling`
- **推荐理由：** 支持GitOps流水线的全自动数学建模技能。
...
- **标签：** `agent`, `self-evolving`
- **推荐理由：** 支持可组合任务控制和人工审查的Agent进化工具。
...
- **标签：** `finance`, `multi-agent`
- **推荐理由：** 基于tushare的多Agent金融分析系统，支持IM盯盘。
...
- **⭐ 星标：** 69
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架���、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `skills`, `coding-agent`
- **推荐理由：** AI编程导师技能，逐步指导项目
...
- **标签：** `skills`, `agent`, `github-issues`, `wayfinder`
- **推荐理由：** 预装25个工程技能面板，开箱即用涵盖分诊到交付全流程
...
- **标签：** `spec-driven`, `agent-skills`, `developer-tools`
- **推荐理由：** 基于仓库原生的规范驱动开发技能，参考DeepSeek工程模式
...
- **标签：** `skills-manager`, `dsh`, `security`
- **推荐理由：** 在DSH环境中统一加载并安全管理本地Agent技能
...
- **标签：** `chinese-wisdom`, `fengshui`, `bazi`, `iching`, `tcm`
- **推荐理由：** 八字/风水/易经/中医融合的本地确定性引擎，带可视化面板
...
- **标签：** `multi-agent`, `collaboration`, `workspace`, `skill-sharing`
- **推荐理由：** 多Agent协作，共享工作区，独立模型和技能配置
...
- **标签：** `code-simplification`, `codex`, `skill`, `refactoring`
- **推荐理由：** Codex风格代码简化技能，自动化重构
...
- **标签：** `skill-picker`, `completion`, `ui-helper`
- **推荐理由：** WorkBuddy风格技能选择器，支持补全命令
...
- **标签：** `fintech`, `backtesting`, `investment`, `multi-strategy`
- **推荐理由：** AI投资工作台，多策略回测与合规分析
...
- **标签：** `qa`, `testing`, `playwright`, `pytest`, `skill-framework`
- **推荐理由：** 10个测试技能，共享知识库与决策矩阵
...
- **标签：** `ai-agents`, `chatgpt`, `deepseek-harness`
- **推荐理由：** OAuth 集成，在 DSH 内使用 ChatGPT 模型对话。
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`, `software-architecture`
- **推荐理由：** 生成精美的架构图、流程图等自包含HTML可视化
...
- **推荐理由：** 530+工业级图片提示词模板库，提炼为可复用Skills
...
- **标签：** `video-generation`, `tts`, `explainer`
- **推荐理由：** 一键将主题生成为Vox风格解说视频，端到端自动化
...
- **标签：** `visualization`, `legal`, `litigation`, `svg`
- **推荐理由：** 根据案件材料直接生成专业诉讼时间轴和可视化图表
...
- **标签：** `browser-extension`, `chrome-extension`, `chat-management`, `prompt-manager`
- **推荐理由：** 增强主流AI平台的浏览器扩展套件与提示词管理器
...
- **标签：** `content-discovery`, `cross-platform`, `browser`, `dsh-plugin`
- **推荐理由：** 本地优先的跨平台内容发现Agent，覆盖B站/YouTube/X等主流平台
...
- **标签：** `ai-agent`, `dingtalk`, `chat-bot`
- **推荐理由：** 钉钉 IM 渠道，支持流式 Agent 能力。
...
- **标签：** `electron`, `image-uploader`, `s3`, `cloudflare-r2`
- **推荐理由：** 支持60+图床的终极图片上传工具，创作者必备
...
- **标签：** `virtual-terminal`, `vfs`, `agent-tools`, `dsh-plugin`
- **推荐理由：** 虚拟终端与文件系统，为AI代理提供沙盒化可重现运行环境
...
- **推荐理由：** DSH内置可视化插件市场，支持浏览搜索和一键安装社区插件
...
- **标签：** `plugin-registry`, `ecosystem`, `discovery`
- **推荐理由：** 自部署生态雷达，自动发现并验证15900+插件
...
- **标签：** `oauth`, `subscription`, `multi-provider`
- **推荐理由：** 通过OAuth使用ChatGPT/Claude订阅作为LLM
...
- **标签：** `remote-access`, `dsh-plugin`, `infrastructure`
- **推荐理由：** 多端远程操控Harness，端到端加密安全可靠
...
- **标签：** `plugin-management`, `registry`
- **推荐理由：** 插件生态注册表控制台与开发指南
...
- **标签：** `plugin`, `utility`, `credentials`
- **推荐理由：** 高效管理已安装DSH插件的插槽、远端Typert和凭据
...
- **标签：** `gongwen`, `docx`, `wps-office`, `templates`
- **推荐理由：** 符合国标的中文公文全流程处理：格式校验、优化与模板生成
...
- **推荐理由：** 精选280+ DSH插件目录，覆盖14大类，必备插件发现工具
...
- **标签：** `ocr`, `vision`, `image-reading`, `pixel-art`
- **推荐理由：** 专为纯文本模型设计的图像OCR识别技能，像素级精准读取
...
- **标签：** `codex`, `sync`, `bridge`, `mcp-mirror`
- **推荐理由：** Codex与DSH双向桥梁，聊天同步与MCP镜像
...
- **标签：** `package-manager`, `skills`, `clis`, `agent-assets`
- **推荐理由：** 资产包管理器，42技能5工具1插件
...
- **推荐理由：** 搜索 npm 和精选列表，提升 DSH 插件发现能力。
...
- **标签：** `ai-agents`, `coding-agents`, `dsh-plugin`
- **推荐理由：** AI 精选 190+ 插件列表，每小时自动更新。
...
- **标签：** `skin`, `theme`, `customization`, `dsh-plugin`
- **推荐理由：** 专为DeepSeek Harness设计的鲸鱼娘系列主题皮肤
...
- **标签：** `theme`, `glass-effect`, `transparent`
- **推荐理由：** 磨砂玻璃主题，支持自定义模糊度和壁纸
...
- **标签：** `ai`, `rag`, `vibecoding`
- **推荐理由：** 最全AI资源库和Vibe Coding入门教程，适合新手
...
- **标签：** `dsh`, `agent`, `tutorial`
- **推荐理由：** 零基础上手的DSH平台工程入门教程
...
- **标签：** `research`, `web-search`, `cn-content`
- **推荐理由：** 自动搜索八大中文平台，生成有据可查的研究报告
...
- **标签：** `awesome-list`, `plugin-discovery`, `curated`
- **推荐理由：** 自动精选插件排行，按场景分类推荐
...
- **标签：** `documentation`, `best-practices`
- **推荐理由：** 学习DSH原理、插件市场和最佳实践的工具。
...
- **标签：** `marketplace`, `catalog`
- **推荐理由：** 1.1万+插件商店，支持搜索排名和API。
...
- **标签：** `research`, `latex`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验和GPU管理。
...
- **标签：** `catalog`, `awesome-list`
- **推荐理由：** 1819个真实插件目录，支持搜索和排序。
...
- **标签：** `patent-analysis`, `rag`, `legal-tech`
- **推荐理由：** 端到端专利侵权分析，支持RAG与多Agent报告生成
...
- **标签：** `industry-research`, `report`, `cordis`
- **推荐理由：** 完整的行业研究框架，支持政策与新闻追踪技能
...
- **标签：** `kline`, `technical-analysis`, `market-data`, `finance`
- **推荐理由：** 交互式K线分析，技术指标与新闻基本面
...
- **推荐理由：** DeepSeek Harness 插件精选目录，便于探索生态。
...
- **推荐理由：** 插件雷达 + 2Origin 实验室，追踪 DSH 生态。
...
- **标签：** `low-code`, `no-code`, `crm`, `erp`, `workflows`
- **推荐理由：** AI驱动的无代码平台，快速构建企业级业务系统
...
- **标签：** `agent-runtime`, `desktop`, `runtune`, `dsh-plugin`
- **推荐理由：** Rust高性能Agent运行时结合桌面应用，整合Agent团队与IDE能力
...
- **推荐理由：** The bioinformatics agent desktop for cli
...
- **推荐理由：** SkillHub Open API documentation, example
...
- **推荐理由：** Unofficial community catalog for DeepSee
...
- **推荐理由：** StarHub — All-in-One DevOps Desktop Comm
...
- **推荐理由：** A fast, simple, and elegant Neovim plugi
...
- **推荐理由：** Awesome DeepSeek Harness (dsh) — curated
...
- **推荐理由：** Awesome DSH Plugins: a public GitHub dir
...
- **推荐理由：** A DeepSeek Harness Web plugin for real-t
...
- **推荐理由：** 桌面版 DeepSeek Harness：Electron 壳 + 自托管 dsh web 服务，集成 dsh-better-sidebar / dsh-web
...
- **推荐理由：** Product Team Mode - a DeepSeek Harness agent preset: user-led conversation with 
...
- **推荐理由：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸
...
- **推荐理由：** Verified, star-ranked community plugins for DeepSeek Harness (dsh).
...
- **推荐理由：** 🐳 DeepSeek Harness（DSH）工具全量清单——GitHub topic:dsh-plugin 生态 1000+ 仓库按 star 排序。Awe
...
- **推荐理由：** 面向 DeepSeek Harness 用户的插件产品说明书与导航：按场景选插件，了解安装、权限、兼容性和风险
...
- **推荐理由：** Awesome dsh plugin list — automated DeepSeek Harness plugin directory. Daily Git
...
- **推荐理由：** DSH Plugin Portal — a zero-dependency static web portal for the awesome-dsh-plug
...
- **推荐理由：** one-click Compact context button for the DSH Web context meter panel.
...
- **推荐理由：** DeepSeek Harness 插件市场，类似 CodeX 商店。
...
- **标签：** `awesome-list`, `dsh-plugin`, `registry`, `plugin-directory`
- **推荐理由：** DSH插件精选目录，便于发现和安装相关插件。
