...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 193,202 | Core DSH framework, everything is a plugin |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,203 | Local-first desktop design app, Figma alternative |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,321 | Multi-agent swarm orchestration with auto workflow |
...
- [实用工具与基础设施](utilities-infrastructure-zh) (27)
...
- **标签：** `dsh-plugin`, `harness-engineering`
- **推荐理由：** 任务感知推理路由，可量化性能提升。
...
- **标签：** `agent`, `runtime`, `desktop`
- **推荐理由：** 高性能Rust Agent运行时桌面应用
...
- **推荐理由：** 架构感知编程Agent，基线优先+漂移检测，长任务安全可控
...
- **标签：** `agent`, `coding-agent`
- **推荐理由：** 将多个代码代理整合为协同工作流
...
- **标签：** `dsh-plugin`, `deepseek-harness`, `agent`
- **推荐理由：** AI编程导师，逐步指导架构设计与代码审查
...
- **标签：** `agent-skills`, `dsh-plugin`, `development`
- **推荐理由：** 端到端指导DSH插件开发的标准技能
...
- **标签：** `codex`, `coding-agent`
- **推荐理由：** 源自DSH的代码简化Codex技能
...
- **标签：** `desktop`, `plugins`
- **推荐理由：** 可爱桌面宠物助手，内置插件SDK和Agent集成，趣味实用
...
- **标签：** `desktop`, `mcp`
- **推荐理由：** 本地优先桌面工作区，集成会话、文件、研究和插件管理
...
- **标签：** `desktop-app`, `agent`, `local-first`
- **推荐理由：** 本地优先多模型AI代理桌面应用，支持自进化技能
...
- **推荐理由：** 功能丰富的Windows桌面客户端，支持Codex和SSH。
...
- **推荐理由：** Electron桌面控制台，多Tab管理插件与技能
...
- **推荐理由：** 跨平台桌面应用整合 DB/SSH/Docker/AI
...
- **标签：** `desktop`, `electron`, `sidebar`
- **推荐理由：** Electron 桌面壳集成 DSH Web 服务与侧边栏
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消��气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
...
- **标签：** `memory`, `agent`
- **推荐理由：** 无限记忆能力，跨会话持久化上下文，Agent必备插件
...
- **标签：** `knowledge-base`, `rag`, `mcp`
- **推荐理由：** 涵盖MCP、RAG、Agent的AI百科知识库
...
- **标签：** `rag`, `embeddings`, `semantic-search`
...
- **标签：** `notes`, `smartisan`, `multi-tenant`, `docker`
- **推荐理由：** 复刻锤子便签风格，支持skill调用与多租户，可一键生成公众号格式
...
- **推荐理由：** 持久可 inspect 的长期记忆插件，配备召回工具与配置面板
...
- **推荐理由：** 连接 PowerContext 实现记忆与技能调用
...
- **标签：** `multi-agent`, `swarm`, `agent-workflow`
- **推荐理由：** 多智能体蜂群编排与自动化工作流
...
- **标签：** `multi-agent`, `enterprise`, `agent-skills`
...
- **标签：** `cognitive`, `skills`, `agent-skills`
- **推荐理由：** 基于全局工作空间研究的认知增强技能
...
- **推荐理由：** 原生支持DSH多Agent团队协作，复杂任务高效分工
...
- **标签：** `ai-infra`, `skills`, `high-performance`, `debugging`
- **推荐理由：** 专为AI基础设施开发设计的agent技能集，覆盖编写优化与调试全流程
...
- **标签：** `task-management`, `taskboard`, `sqlite`, `cordis`
- **推荐理由：** 原生SQLite任务看板，Agent认领审核机制，纯WebUI无需嵌套
...
- **标签：** `agent-workflow`, `automation`, `orchestration`
- **推荐理由：** Agent工作流编排插件，支持复杂多步自动化流程定义与执行
...
- **标签：** `superpowers`, `reasonix`, `skills`, `tdd`
- **推荐理由：** Superpowers技能移植到Reasonix编码harness，支持TDD与调试规划
...
- **标签：** `superpowers`, `skills`, `tdd`, `obra`, `collaboration`
- **推荐理由：** 将obra/Superpowers技能适配到DSH，覆盖TDD调试规划协作
...
- **标签：** `agent-skills`, `spec-driven-development`, `developer-tools`
- **推荐理由：** 与DSH工程模式对齐的规格驱动开发技能
...
- **标签：** `dsh-plugin`, `agent-skill`, `knowledge-base`
- **推荐理由：** 官方DSH插件开发知识库，双语技能文档
...
- **标签：** `chinese-traditional-wisdom`, `dashboard`, `agent-skill`
- **推荐理由：** 八字紫微奇门风水全融合的AI Agent技能包
...
- **标签：** `agent-team`, `multi-agent`
- **推荐理由：** 多Agent团队协作与共享工作区
...
- **推荐理由：** 录制 macOS 操作并转为可复用 Agent 技能
...
- **推荐理由：** Web界面统一管理服务skills与MCP服务器，集中化扩展管理
...
- **推荐理由：** 为纯文本模型赋能视觉能力：图片问答、OCR及UI还原
...
- **标签：** `design`, `ppt`
- **推荐理由：** AI驱动设计工作室，模板市场+可视化编辑器+PPT生成
...
- **推荐理由：** 在DSH对话中直接使用Gemini/OpenAI生成图片。
...
- **标签：** `design`, `ui-design`, `ppt`, `openpencil`
- **推荐理由：** 支持在DSH内直接预览编辑OpenPencil设计稿，提升设计协作效率
...
- **推荐理由：** 无缝浏览器自动化，复用已登录会话，Agent能力大幅扩展
...
- **推荐理由：** Chrome扩展将AI对话转为可复用工作流，跨平台高效协作
...
- **标签：** `browser-automation`, `playwright`, `tabbit`, `web-automation`
- **推荐理由：** 基于Tabbit浏览器和Playwright的网页自动化插件，支持浏览器控制
...
- **标签：** `image-uploader`, `electron`, `utilities`
- **推荐理由：** 必用图片上传工具，支持60+图床服务
...
- **标签：** `model-switch`, `utility`, `installer`
- **推荐理由：** 一键切换多个AI编码工具的模型
...
- **标签：** `marketplace`, `utility`, `installer`
- **推荐理由：** 内置插件市场支持一键安装
...
- **标签：** `marketplace`, `plugin`, `javascript`, `nodejs`
- **推荐理由：** DSH Web GUI一键浏览安装更新全部插件，集中化管理插件生态
...
- **标签：** `context-migration`, `session`, `preset`, `cordis`
- **推荐理由：** 跨预设会话迁移插件，保留状态与上下文，修复schema断点
...
- **标签：** `backup`, `crash-recovery`, `rollback`, `windows`, `powershell`
- **推荐理由：** 崩溃救援插件，支持配置撤销、安全快照及离线CLI/GUI回滚
...
- **标签：** `marketplace`, `community`, `webui`, `plugin-market`
- **推荐理由：** DSH Web GUI集成社区插件市场，一键安装卸载至profile
...
- **标签：** `plugin-management`, `dsh-plugin`, `utility`
- **推荐理由：** 通过Agent技能管理插件槽位与凭据
...
- **标签：** `dsh-plugin`, `desktop`, `container`
- **推荐理由：** 本地运行多隔离DSH容器，内嵌WebView一键导入插件
...
- **推荐理由：** 将 Claude Code/Codex/OpenCode 配置迁移到 DSH
...
- **标签：** `utility`, `conflict-resolution`
- **推荐理由：** 自动修复插件冲突问题
...
- **标签：** `theme`, `fun`, `simulation`
- **推荐理由：** 零依赖传统玩具网页模拟趣味插件
...
- **标签：** `ai-resources`, `vibe-coding`, `education`
- **推荐理由：** 全面AI资源库与入门学习指南
...
- **标签：** `research`, `analysis`, `skill`
- **推荐理由：** 自动搜索8大中文平台生成研究报告
...
- **推荐理由：** 精选Top50插件雷达，自动发现加实测排名，便于快速选型
...
- **标签：** `security`, `cybersecurity`
- **推荐理由：** AI安全攻防平台，审计AI生成代码漏洞
...
- **标签：** `prompt`, `armor-breaking`
- **推荐理由：** 专为DeepSeek设计的破甲提示词插件
...
- **标签：** `marketplace`, `plugins`
- **推荐理由：** DeepSeek Harness生态插件市场。
...
- **推荐理由：** DSH插件精选清单，社区插件生态的重要发现与导航资源
...
- **推荐理由：** 改配置+重启才能加 MCP？DeepSeek Harness (DSH) 插件「
...
- **标签：** `best-practices`, `documentation`, `dsh`, `dsh-plugin`, `skill`
- **推荐理由：** DSH 插件开发踩坑与做法档案（skill + 文档）：cordis 双副本、t
...
- **⭐ 星标：** 13
- **语言：** JavaScript
- **描述：** 拨开迷雾看见终点，剩下的交给任务栏。Part the fog, see the end — the task bar handles the rest. 🎮 mattpocock/skills 的 DSH 游戏任务系统：map 拨迷雾，��务栏推进一步。A game-like mission system for Matt Pocock skills in DeepSeek Harness. More by @FeatherHunter: 🎨 dsh-opencode-palette · ⚡ dsh-prompt
...
- **推荐理由：** Engineering discipline distilled from De
...
- **标签：** `curated-list`, `registry`
- **推荐理由：** 覆盖模型、工具、沙箱、UI的DSH插件目录
