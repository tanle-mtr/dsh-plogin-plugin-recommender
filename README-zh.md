...
- **标签：** `ai-agents`, `multi-agent`, `skills`
- **推荐理由：** 多功能AI助手，支持多模型与记忆
...
- **标签：** `coding-agent`, `terminal`, `r1`
- **推荐理由：** 终端原生编码智能体，支持提示词缓存
...
- **标签：** `agent-runtime`, `desktop`, `coding`
- **推荐理由：** Rust高性能Agent运行时搭配精美桌面IDE应用
...
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，带严重性分级
...
- **标签：** `coding-agents`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 架构感知编码代理��防漂移确保安全
...
- **标签：** `vision`, `agent-skills`, `ocr`, `dsh-plugin`
- **推荐理由：** 纯文本模型的强大视觉工具箱
...
- **标签：** `context-migration`, `preset`, `session`
- **推荐理由：** 跨预设会话迁移，保持状态与模型意图一致性。
...
- **推荐理由：** 通过模拟器或USB设备流在对话中构建测试Android应用。
...
- **标签：** `dsh-plugin`, `agent-skills`, `plugin-dev`
- **推荐理由：** DSH插件开发标准流程的Agent技能
...
- **标签：** `coding-agent`, `code-simplification`, `codex`
- **推荐理由：** 从DSH适配的Codex代码简化技能，支持自动重构。
...
- **推荐理由：** 双向桥接Codex与DSH的会话、技能和MCP同步工具。
...
- **推荐理由：** 审计优化DSH上下文注入，分析Token成本与冲突检测。
...
- **标签：** `plugin-development`, `documentation`
- **推荐理由：** DSH插件开发踩坑实录：tsconfig/junction/zstd等方案。
...
- **标签：** `deepseek-harness`, `migration`, `agent-migration`, `mcp`
- **推荐理由：** 无缝迁移Claude Code/Codex配置到DSH。
...
- **标签：** `plugin-tools`, `conflict-resolution`
- **推荐理由：** 自动检测并解决插件冲突问题
...
- **推荐理由：** 开放侧边栏底座，支持文件编辑/终端/Git等内置功能
...
- **标签：** `tauri`, `desktop`, `dsh-plugin`
- **推荐理由：** 仅5MB桌面应用，零配置跨平台运行
...
- **标签：** `desktop`, `electron`, `mcp`, `dsh-plugin`
- **推荐理由：** 带动漫宠物的桌面伴侣，支持插件扩展
...
- **标签：** `floating-widget`, `dsh-plugin`, `ui`
- **推荐理由：** DSH右下角小鲸鱼监控余额，可爱实用
...
- **标签：** `canvas`, `conversation`, `ui`
- **推荐理由：** 可视化非线性对话画布，支持会话探索和分支工作区
...
- **推荐理由：** 原生SQLite任务看板，支持Agent任务分配审核。
...
- **标签：** `deepseek-harness`, `desktop`, `dsh-plugins`
- **推荐理由：** 基于容器的本地DSH管理工具，含嵌入式UI
...
- **标签：** `dsh-plugin`, `ui`, `skill-picker`
- **推荐理由：** 类WorkBuddy的技能选择器，便于在组合器中快速调用。
...
- **标签：** `dsh-plugin`, `skills`, `mcp`
- **推荐理由：** Web GUI插件，浏览器内管理技能与MCP服务器。
...
- **标签：** `deepseek-harness`, `data-visualization`, `token-usage`
- **推荐理由：** 实时Token用量、费用和API余额图表。
...
- **标签：** `desktop`, `docker`, `ssh`, `deepseek-harness`
- **推荐理由：** 集成数据库、SSH、Docker面板的跨平台桌面应用。
...
- **推荐理由：** Electron桌面壳集成DSH Web服务及侧边栏工具。
...
- **标签：** `knowledge-base`, `rag`, `encyclopedia`
- **推荐理由：** 覆盖Agent、RAG、MLOps的全维度AI资源百科知识库
...
- **标签：** `agent-memory`, `embeddings`, `reranking`
- **推荐理由：** 构建可检索的 agent 技能语料库
...
- **推荐理由：** 持久可查的Agent长期记忆，支持recall工具。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `memory`, `agent`
- **推荐理由：** 连接PowerContext实现记忆、回忆和技能交接。
...
- **标签：** `agent-skills`, `knowledge-distillation`
- **推荐理由：** 提炼人类思维为可复用AI技能
...
- **标签：** `cognitive`, `agent-skills`, `research`
...
- **标签：** `workflow`, `agent-skills`, `automation`
- **推荐理由：** 智能路由本地技能并编排DSH工作流的通用Skill
...
- **推荐理由：** 端到端Vox风格剪纸解说视频自动生成Agent技能
...
- **标签：** `workflow`, `chrome-extension`, `ai-workflow`
- **推荐理由：** 将AI对话转化为可复用工作流
...
- **标签：** `agent`, `desktop`, `privacy`, `multi-model`
- **推荐理由：** 本地优先多模型AI代理，支持自进化技能和隐私保护
...
- **标签：** `agent`, `orchestration`, `coding-agents`
- **推荐理由：** 统一编排多个编码代理的团队协作工作流
...
- **推荐理由：** 在DSH对话中运行iOS模拟器并提供22项自动化代理工具
...
- **标签：** `agent-skills`, `research-tools`, `market-research`
- **推荐理由：** 88个开源技能，覆盖研究、营销和社交情报
...
- **标签：** `spec-programming`, `agent-skills`, `developer-tools`
- **推荐理由：** 可移植的规格驱动开发技能
...
- **标签：** `agent-handoff`, `agent-skills`, `session-memory`
- **推荐理由：** 支持跨平台Agent会话记忆继承，实现平滑交接。
...
- **推荐理由：** 多Agent团队协作，支持独立模型/技能与共享工作区。
...
- **推荐理由：** 专门的技能管理插件，用于组织控制DSH Agent技能。
...
- **推荐理由：** 版本固定的Swift Cycle治理技能适配器，确定性工作流。
...
- **标签：** `agent-skills`, `vision`, `image-generation`
- **推荐理由：** 支持图片读取与生成，扩展智能体视觉能力。
...
- **标签：** `agent-skills`, `package-manager`, `mcp`
- **推荐理由：** 42个技能包，AI智能体工程一站式资源管理器。
...
- **标签：** `skills`, `agent`, `gamification`
- **推荐理由：** 游戏化任务系统，迷雾地图推进技能任务链。
...
- **标签：** `deepseek-harness`, `godot`, `game-development`
- **推荐理由：** Godot 4.x游戏开发技能，全栈GDScript支持。
...
- **标签：** `agent-skills`, `dsh-plugin`, `canvas`, `mcp`
- **推荐理由：** WeShop无限画布与创意技能集成。
...
- **标签：** `deepseek-harness`, `record-replay`, `macos`, `computer-use`
...
- **⭐ 星标：** 3
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞��检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `capability-index`
- **推荐理由：** 任务预检自动注入Top-K适用插件
...
- **标签：** `design-systems`, `figma-alternative`
- **推荐理由：** AI驱动的开源自设计工具
...
- **标签：** `image-generation`, `prompt-engineering`
- **推荐理由：** 工业级图像生成提示词库，530+模板
...
- **标签：** `ui`, `visualization`, `interactive`
- **推荐理由：** 在DSH回复中内联渲染交互式UI组件、图表和测验
...
- **推荐理由：** 在DSH对话中预览和编辑OpenPencil设计文档。
...
- **推荐理由：** DirectorX集成：AI视频/图像/音频多模态创作技能。
...
- **标签：** `browser-extension`, `chat-management`
- **推荐理由：** 跨浏览器增强套件与提示词管理
...
- **标签：** `content-discovery`, `cross-platform`, `privacy`
- **推荐理由：** 本地优先的跨平台内容发现Agent，支持多平台抓取
...
- **标签：** `deepseek-harness`, `community`
- **推荐理由：** DSH内测社区合影留念站，零权限GitHub登录。
...
- **标签：** `electron`, `image-uploader`
- **推荐理由：** 支持60+图床的高效图片上传工具
...
- **标签：** `model-switch`, `cli`, `utility`
- **推荐理由：** 一键切换多模型AI编码工具，统一安装管理
...
- **标签：** `toy`, `web`, `entertainment`
- **推荐理由：** 零依赖单文件竹知了玩具Web模拟器，移动端优先
...
- **标签：** `marketplace`, `utility`, `dsh`
- **推荐理由：** DSH官方可视化插件市场，支持浏览和一键安装
...
- **标签：** `context`, `dsh-plugin`, `utilities`
- **推荐理由：** DSH上下文管理与洞察仪表盘工具
...
- **标签：** `security`, `auditing`, `cybersecurity`
- **推荐理由：** AI代码安全审计平台，检测幻觉包和漏洞防护缺失
...
- **标签：** `desktop`, `client`, `cross-platform`
- **推荐理由：** 跨平台桌面客户端，CodePilot风格的DSH插件套件
...
- **标签：** `plugin-store`, `marketplace`, `registry`
- **推荐理由：** 大型插件市场与搜索 API
...
- **标签：** `plugin-management`, `dsh-repository-plugin`, `ui`
- **推荐理由：** 插件管理控制台及官方开发引导工具
...
- **推荐理由：** 支持通过WebDAV备份、恢复和同步完整DSH配置。
...
- **推荐理由：** 磨砂玻璃质感主题，支持毛玻璃模糊度和背景高度自定义
...
- **标签：** `research`, `china`, `web-search`
- **推荐理由：** 自动搜索中国8大平台近30天内容并生成研究报告
...
- **标签：** `agent-harness`, `awesome-list`, `deepseek-harness`
- **推荐理由：** DSH 全面指南与资源列表
...
- **标签：** `awesome-list`, `deepseek-harness-plugins`, `mcp`
- **推荐理由：** 精选插件与 MCP 服务器列表
...
- **标签：** `ai-agents`, `chinese-api`, `api-documentation`
- **推荐理由：** 跨工具搜索6.5万+中国开放平台API文档
...
- **推荐理由：** 官方DSH插件开发知识库，含双语文档与Cordis入门指南。
...
- **推荐理由：** 收录280+插件的精选目录，覆盖14类便于快速发现。
...
- **标签：** `kline`, `analysis`, `market-data`
- **推荐理由：** 交互式K线分析插件，支持多市场行情与技术指标。
...
- **标签：** `awesome-list`, `ai-agents`, `mcp`
- **推荐理由：** AI维护的190+插件列表，12分类，每小时更新
...
- **标签：** `awesome-list`, `industry`, `dsh-plugin`
- **推荐理由：** 面向IT、金融、法律、医疗等行业的插件精选
...
- **标签：** `low-code`, `internal-tools`, `crm`
- **推荐理由：** AI增强的低代码业务系统搭建平台
...
- **推荐理由：** Agent skill for beautiful, verifiable ar
...
- **标签：** `agent`, `agentic`, `agentic-ai`, `ai`, `ai-agent`, `ai-agents`, `dsh`, `dsh-plugin`, `harness`, `harness-engineering`, `harness-framework`, `llm`
- **推荐理由：** Harness engineering beginner tutorial, f
...
- **推荐理由：** A curated list of plugins for DeepSeek H
...
- **推荐理由：** One portable memory layer for every AI a
...
- **推荐理由：** Self-evolving memory OS for LLM & AI Age
...
- **推荐理由：** ✨ All your agents and workspaces in one 
...
- **推荐理由：** dsh-routing-suite — injector + router-st
...
- **推荐理由：** DeepSeek Harness（DSH）Web 插件聚合生态包 · 一切皆插件
...
- **推荐理由：** Agent OS: the agent gets smarter on its 
...
- **推荐理由：** The world's first open-source AI-native 
...
- **推荐理由：** Enterprise-grade, local-first Agent Work
...
- **推荐理由：** A public gallery of animated pets for Co
...
- **推荐理由：** The World's First Unified Virtual Filesy
...
- **标签：** `awesome-list`, `discovery`, `curation`
- **推荐理由：** 30秒快速发现插件，场景化分类与热度排行推荐
...
- **标签：** `armor-breaking`, `jailbreak`, `prompt`
- **推荐理由：** DeepSeek专用破甲插件，稳定化破解提示词
...
- **标签：** `awesome`, `awesome-list`, `directory`
- **推荐理由：** DSH插件精选目录与资源合集。
...
- **标签：** `reasonix`, `skills`, `superpowers`
- **推荐理由：** 适配Reasonix的代码技能包，非原生DSH插件。
...
- **标签：** `awesome-list`, `dsh-plugin`, `cordis`
...
- **标签：** `registry`, `awesome-list`
- **推荐理由：** 经审核的插件注册表，含垃圾过滤
...
- **标签：** `guide`, `documentation`
- **推荐理由：** 按场景选插件的导航手册和说明书
