...
- **标签：** `ai-agent`, `multi-agent`, `memory`
- **推荐理由：** 超级AI助手，支持记忆、多智能体与多模型
...
- **推荐理由：** DeepSeek原生TUI编码智能体，前���缓存稳定
...
- **标签：** `low-code`, `ai`, `business-systems`
...
- **标签：** `agent`, `harness-engineering`, `dsh-plugin`, `llm`
- **推荐理由：** 从零到一学习Harness工程的入门教���
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `cordis`
- **推荐理由：** 运行时注入器+路由标准套件，提升推理效率
...
- **标签：** `agent-os`, `mcp`, `llm-orchestration`, `skill`, `codex`
- **推荐理由：** 自进化Agent OS，面试级评估与预算控制进化循环
...
- **标签：** `vision`, `ocr`, `deepseek-harness`
- **推荐理由：** 首款DSH视觉插件，支持OCR与图片转结构化JSON
...
- **标签：** `preset`, `deepseek-harness`, `agent`
- **推荐理由：** 两阶段预设，轻量启动后接入完整标准工具链
...
- **标签：** `dsh`, `security`, `cybersecurity`, `code-audit`
- **推荐理由：** AI安全攻防平台，审计代码幻觉与漏洞，增强安全性。
...
- **标签：** `dsh`, `coding-agent`, `mcp`, `rpc`
- **推荐理由：** 编码Agent含子Agent、RPC插件和MCP集成的强大工具。
...
- **标签：** `dsh-plugin`, `plugin-development`, `knowledge-base`
- **推荐理由：** DSH插件开发知识库技能
...
- **标签：** `deepseek-harness`, `dsh-web`, `dsh-plugin`
- **推荐理由：** DSH插件Web聚合生态，一切皆插件分发
...
- **⭐ 星标：** 2,159
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻���、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `widget`, `ui`, `deepseek-harness`
- **推荐理由：** 可爱浮动小鲸鱼，实时显示DeepSeek账户余额
...
- **标签：** `deepseek-harness`, `desktop`, `tauri`, `dsh`
- **推荐理由：** 轻量级Tauri桌面版，预置插件，一键安装无需配置
...
- **标签：** `dsh`, `desktop`, `electron`, `local-first`
- **推荐理由：** 本地优先的Electron桌面工作站，支持项目、文件和插件管理。
...
- **标签：** `dsh`, `desktop`, `electron`, `windows`
- **推荐理由：** Windows桌面GUI客户端，集成Codex、SSH和远程访问功能。
...
- **标签：** `dsh`, `ui-components`, `react`
- **推荐理由：** 在对话内联渲染交互式UI组件，支持图表和表单布局。
...
- **标签：** `dsh`, `desktop`, `tauri`, `local-first`
...
- **标签：** `office`, `documents`, `spreadsheets`
- **推荐理由：** 表格文档幻灯片全套办公环境
...
- **推荐理由：** 开源锤子便签风格笔记，支持Docker部署与DSH插件
...
- **推荐理由：** Web界面可视化管理skills和MCP，操作直观
...
- **标签：** `desktop`, `devops`, `all-in-one`
- **推荐理由：** 集成数据库、SSH、Docker的一站式DevOps桌面。
...
- **标签：** `electron`, `desktop`, `webui`
- **推荐理由：** DSH桌面版Electron壳，集成侧边栏与ModLens
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin`, `web`
- **推荐理由：** 静态插件门户，方便浏览和筛选227+个DSH插件。
...
- **标签：** `rag`, `agent-memory`, `context-database`
- **推荐理由：** 自演进上下文数据库，统一智能体记忆与RAG
...
- **标签：** `agent-memory`, `rag`, `mcp`, `long-term-memory`, `skills`
- **推荐理由：** 自进化记忆系统，混合检索，节省35%Token消耗
...
- **标签：** `agent-memory`, `agentic-rag`, `continual-learning`, `mcp`
...
- **标签：** `memory`, `agent`, `dsh-plugin`, `RAG`
- **推荐理由：** 共享持久记忆中枢，让所有AI代理拥有共同记忆
...
- **标签：** `memory`, `skill-corpus`, `RAG`, `dsh-plugin`
- **推荐理由：** 将散落的SKILL.md文件整理为可检索的知识语料库
...
- **标签：** `memory`, `handoff`, `session`, `continuity`
- **推荐理由：** 跨Agent会话连续性记忆
...
- **标签：** `agent-orchestration`, `mcp`, `no-code`, `deepseek-harness`
...
- **标签：** `agent-skills`, `deepseek-harness`, `skill-md`, `mcp`
- **推荐理由：** 最全技能商店，自动抓取GitHub上万技能项目
...
- **标签：** `agent-control-plane`, `loop-engineering`, `workflow-automation`, `codex`
- **推荐理由：** 跨Harness长周期Agent管控平面，支持持久治理工作流
...
- **标签：** `agent-collaboration`, `multi-agent`, `visual-editor`, `deepseek-harness`
- **推荐理由：** 企业级本地Agent工作台，支持多引擎协作与可视化编辑
...
- **推荐理由：** 基于Anthropic J-space研究的认知增强Agent技能
...
- **标签：** `video`, `agent-skill`, `dsh-plugin`, `ffmpeg`
- **推荐理由：** 自动生成Vox风格纸拼贴解说视频，端到端一键出片
...
- **标签：** `agent-skill`, `research`, `dsh-plugin`, `web-search`
...
- **标签：** `ai-writing`, `novel`, `dsh-plugin`, `creative`
- **推荐理由：** 从灵感到成稿的完整AI小说创作工作流与桌面工具
...
- **标签：** `dsh`, `canvas`, `conversation`, `workflow`
- **推荐理由：** 可视化非线性会话画布，支持分支探索和工作区导航。
...
- **标签：** `dsh`, `multimodal`, `ocr`, `video-analysis`
- **推荐理由：** 赋予AI视觉听觉能力，将视频音频转为可查证时间戳证据。
...
- **标签：** `ctf`, `security`, `reverse-engineering`, `pentest`
- **推荐理由：** 完整的逆向工程与渗透测试技能包，覆盖87个SKILL.md
...
- **推荐理由：** 源自Superpowers的TDD、调试与规划技能集
...
- **标签：** `dsh-plugin`, `patent`, `legal-tech`, `rag`
- **推荐理由：** 专利侵权分析，自动生成竞品报告
...
- **标签：** `dsh-plugin`, `gongwen`, `docx`
- **推荐理由：** 党政机关公文全流程处理，格式检查与修复
...
- **标签：** `dsh-plugin`, `spec-programming`, `agent-skills`
- **推荐理由：** 仓储级规格驱动开发技能
...
- **标签：** `roleplay`, `narrative`, `npc`, `skills`
- **推荐理由：** 叙事角色扮演NPC技能
...
- **标签：** `engineering`, `quality-gate`, `workflow`
- **推荐理由：** 工程质量检查工作流，保障交付质量。
...
- **标签：** `plugin-development`, `skill`, `guide`
...
- **标签：** `agent-skill`, `plugin-dev`, `standards`
- **推荐理由：** DSH插件开发标准Agent技能。
...
- **推荐理由：** AI自动整理的190+插件推荐，12分类每小时更新。
...
- **标签：** `design`, `figma-alternative`, `ui-generator`
- **推荐理由：** 开源设计插件，本地优先桌面端与Figma替代方案
...
- **标签：** `ai`, `ui`, `vibedesign`, `flutter`, `react`, `mcp`
...
- **标签：** `vision`, `computer-vision`, `dsh-plugin`, `OCR`
- **推荐理由：** 为纯文本模型打造视觉工具箱：图片问答与OCR识别
...
- **标签：** `dsh-plugin`, `legal-tech`, `diagram`, `svg`
- **推荐理由：** 法律文书可视化，生成时间轴和案件图
...
- **标签：** `video`, `image`, `director`, `storyboard`
- **推荐理由：** AI视频图像生成导演技能
...
- **标签：** `visualization`, `api-cost`, `webui`
- **推荐理由：** DSH Web实时用量图表与成本估算
...
- **标签：** `browser-extension`, `chrome`, `claude`
- **推荐理由：** ���浏览器AI增强套件，支持Gemini/Claude/ChatGPT
...
- **推荐理由：** 本地优先跨平台内容发现Agent，覆盖B站/YouTube等
...
- **标签：** `agent`, `browser-use`, `dsh-plugin`, `automation`
- **推荐理由：** 让AI代理使用真实登录浏览器，无缝自动化网页操作
...
- **推荐理由：** Tabbit浏览器自动化插件，集成Playwright能力
...
- **标签：** `ai-agents`, `chatgpt`, `dsh-plugin`
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内直接与ChatGPT对话。
...
- **标签：** `ai-agent`, `dingtalk`, `dsh-plugin`
- **推荐理由：** 钉钉IM通道，支持流式机器人和情绪已读回执。
...
- **标签：** `image-uploader`, `electron`, `cdn`
...
- **标签：** `multi-agent`, `tool-switch`, `utility`
- **推荐理由：** 一键切换Claude Code/Codex/DeepSeek/Kimi等Agent
...
- **标签：** `ecosystem-radar`, `dsh-plugin`, `plugin-registry`
- **推荐理由：** 自部署生态雷达，自动发现索引15900+可用插件
...
- **标签：** `context`, `dsh-plugin`, `dashboard`, `management`
- **推荐理由：** 全面的上下文仪表盘与管理工具，洞察对话状态
...
- **标签：** `dsh`, `oauth`, `llm-provider`, `subscription`
- **推荐理由：** OAuth桥接ChatGPT/Claude/Grok订阅为DSH提供LLM服务。
...
- **标签：** `commandcode`, `llm-provider`, `dsh-plugin`
- **推荐理由：** 提供Command Code模型接入与智能模型选择
...
- **标签：** `handbook`, `runtime`, `agent-runtime`
- **推荐理由：** 权威的DSH运行时与插件MCP综合参考手册
...
- **标签：** `remote`, `p2p`, `dsh-plugin`
- **推荐理由：** 端到端加密的跨设备远程访问Harness方案
...
- **推荐理由：** 精选DSH插件awesome列表，快速发现优质插件
...
- **标签：** `dsh-plugin`, `plugin-management`, `console`
...
- **标签：** `dsh-plugin`, `agent-skills`, `plugin-management`
- **推荐理由：** 统一加载和管理本机Agent技能
...
- **标签：** `containers`, `multi-version`, `management`
- **推荐理由：** 多版本DSH容器管理工具
...
- **推荐理由：** SkillHub官方API文档与示例，适合DSH集成参考。
...
- **标签：** `awesome-list`, `registry`, `dsh`
- **推荐理由：** DSH插件与技能的开放数据注册表
...
- **标签：** `diagnostic`, `conflict-resolution`, `plugin`
- **推荐理由：** 检测并解决DSH插件间及版本冲突问题
...
- **标签：** `mascot`, `sprites`, `pixel-art`, `deepseek-harness`, `claude-code`
- **推荐理由：** 各Agent工具的动画宠物公共画廊，趣味化定制
...
- **标签：** `dsh`, `theme`, `webgpu`, `threejs`
- **推荐理由：** WebGPU驱动的海洋主题视觉皮肤，基于Three.js渲染。
...
- **⭐ 星标：** 405
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃���。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `deepseek-harness`, `deepseek-harness-plugin`, `dsh`, `dsh-plugin`, `theme`
- **推荐理由：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃���。玻璃模糊度、磨砂度、背
...
- **标签：** `awesome-list`, `ai-resources`, `knowledge-base`
- **推荐理由：** 全维度AI资源百科，涵盖大模型、Agent、RAG和学习路线。
...
- **标签：** `bioinformatics`, `desktop`, `clinicians`, `tauri`
- **推荐理由：** 生物信息学临床Agent桌面应用
...
- **标签：** `curated-list`, `awesome`, `directory`
- **推荐理由：** 精选DSH插件列表，含详细描述。
...
- **标签：** `directory`, `discovery`, `awesome`
...
- **标签：** `dsh-bundle`, `dsh-plugins`, `dsh-repository-plugin`
- **推荐理由：** DSH插件仓库合集，含插件验证功能。
...
- **标签：** `plugin-list`, `curated`, `evidence`
- **推荐理由：** 基于证据的DSH插件精选清单
