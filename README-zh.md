...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 204,369 | Core harness framework powering the entire DSH plu |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 92,772 | Open-source design tool for coding agents, local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,792 | Multi-agent swarm orchestration with MCP server su |

---

## 分类导航

- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 30 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (30)
...
- **标签：** `multi-agent`, `mcp`, `skills`, `memory`
- **推荐理由：** 超级AI助手，支持任务规划、技能和自我进化记忆
...
- **标签：** `coding-agent`, `terminal`, `tui`, `typescript`
- **推荐理由：** DeepSeek原生终端编程Agent，前缀缓存稳定运行
...
- **标签：** `harness-engineering`, `plugin-dev`, `tutorial`
- **推荐理由：** 从零到一掌握DSH插件开发的最佳教程
...
- **标签：** `routing`, `agent-tool`, `cordis`
- **推荐理由：** 任务感知推理路由器，性能可量化提升
...
- **推荐理由：** 企业级多Agent协同工作站，支持Codex/DSH
...
- **标签：** `desktop`, `windows`, `ssh`
- **推荐理由：** 零配置Windows桌面含SSH远程
...
- **标签：** `marketplace`, `plugin`, `web-gui`
- **推荐理由：** Web端一键浏览、安装和更新所有DSH插件
...
- **推荐理由：** 源自superpowers的TDD、调试与规划技能
...
- **标签：** `skills-manager`, `deepseek-harness`
- **推荐理由：** 统一管理并安全加载本地Agent Skills
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** Codex与DSH双向同步，支持Skills挂载与MCP镜像
...
- **⭐ 星标：** 21
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，��测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `audit`, `ui`
- **推荐理由：** 审计指令链与工具schema，检测重复与冲突
...
- **标签：** `pre-flight`, `plugin-check`, `agent-tools`
- **推荐理由：** Agent插件预检注入工具
...
- **标签：** `web-ui`, `marketplace`, `plugin-store`
- **推荐理由：** Web插件市场与创意工坊分发平台
...
- **标签：** `floating-widget`, `ui`, `balance-tracker`
- **推荐理由：** 萌系浮动鲸鱼挂件，实时显示账户余额
...
- **推荐理由：** 本地优先Electron桌面工作区，支持项目与插件
...
- **推荐理由：** Electron桌面控制台管理Harness、插件和MCP
...
- **标签：** `usage-tracking`, `dsh-plugin`
- **推荐理由：** 持久底部状态栏，显示模型、余额与价格
...
- **标签：** `canvas`, `agent-skills`, `mcp`, `dsh-plugin`
- **推荐理由：** 无限画布+创意技能，原生WeShop Cordis集成
...
- **⭐ 星标：** 6
- **语言：** JavaScript
- **描述：** ���面版 DeepSeek Harness：Electron 壳 + 自托管 dsh web 服务，集成 dsh-better-sidebar / dsh-web-ui / ModLens / awesome-dsh-plugin / dshmarket
- **标签：** `electron`, `desktop`, `integration`
- **推荐理由：** Electron桌面壳集成多种DSH插件
...
- **推荐理由：** DSH Web上下文面板一键折叠按钮，提升交互体验
...
- **标签：** `rag`, `knowledge-base`, `semantic-search`, `wiki`
- **推荐理由：** LLM知识库平台，支持RAG语义搜索和自维护Wiki
...
- **推荐理由：** 持久统一记忆层，基于Milvus和Markdown
...
- **标签：** `knowledge-base`, `skills`, `ai-pedia`
- **推荐理由：** AI技能知识库全维度百科
...
- **推荐理由：** 在DSH/Claude中直接搜索65000+中国平台API文档
...
- **标签：** `agent-orchestration`, `workspace`, `workflow`
- **推荐理由：** 跨设备统一Agent工作区与任务看板
...
- **标签：** `skills`, `workflow`, `vibe-coding`
- **推荐理由：** 智能技能路由，奖励提升21个百分点
...
- **标签：** `task-management`, `webui`
- **推荐理由：** 原生Web UI任务板，支持Agent审核
...
- **标签：** `competition`, `skill`
- **推荐理由：** 全自动数学建模竞赛技能，支持国赛美赛
...
- **标签：** `skills`, `knowledge-base`
- **推荐理由：** 将经验转化为可重复执行的AI技能
...
- **标签：** `deepseek-harness`, `coding-agent`, `skills`
- **推荐理由：** AI编程导师，生成章节、代码审查和测试
...
- **标签：** `agent-skills`, `ai-coding`, `spec-programming`
...
- **推荐理由：** MattSkillsDeck可视化任务板，让技能派得更清晰
...
- **推荐理由：** 版本锁定Swift Cycle治理Skill，离线可验证
...
- **标签：** `package-manager`, `agent-framework`, `skills`
- **推荐理由：** 42个Skills+5个CLI的AI代理资产包管理器
...
- **标签：** `evaluation`, `rag-evaluation`, `prompt-testing`
- **推荐理由：** 提示词、RAG与Agent的可信评估框架
...
- **推荐理由：** Godot 4.x游戏开发技能，全栈支持DeepSeek Harness
...
- **标签：** `agent-preset`, `product-team`, `workflow`
- **推荐理由：** 虚拟产品团队Agent预设
...
- **标签：** `image-reading`, `ocr`, `vision`, `deepseek-harness`
- **推荐理由：** 为纯文本模型提供图像OCR与视觉工具
...
- **标签：** `mcp`, `mcp-server`, `skill-management`, `dsh-plugin`
- **推荐理由：** MCP热加载配置+技能管理，粘贴JSON即连，即点即用
...
- **标签：** `architecture-diagram`, `diagrams-as-code`, `agent-skills`
...
- **标签：** `video`, `generation`, `ffmpeg`
- **推荐理由：** 端到端Vox风格拼贴视频自动生成技能
...
- **标签：** `image-gen`, `multimodal`, `vision`
- **推荐理由：** 对话内直接生成图片
...
- **标签：** `design-skills`, `ui`, `web-design`
- **推荐理由：** 6种设计风格防止AI感，适合vibe coding
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `community`
- **推荐理由：** 社区合影墙，OAuth零权限登录，拍立得风格分享独特
...
- **标签：** `no-code`, `low-code`, `ai-agent`, `workflows`
...
- **标签：** `vfs`, `filesystem`, `infrastructure`
- **推荐理由：** 统一虚拟文件系统，AI跨平台访问文件
...
- **标签：** `deepseek-harness`, `mcp`, `dsh-plugin`
...
- **⭐ 星标：** 28
- **语言：** TypeScript
- **描述：** 一键配置各��� AI Agent 工具的 MCP、Skills 和规则；支持 Claude Code、Codex、DeepSeek Harness、OpenCode、Cursor、Gemini CLI、Qwen Code 等。 One-click local setup for MCP servers, skills, and rules across AI coding agents.
- **标签：** `mcp`, `config-sync`, `deepseek-harness`
- **推荐理由：** 一键配置多Agent工具的MCP与Skills
...
- **标签：** `migration`, `agent-migration`, `dsh-plugin`, `utils`
- **推荐理由：** 一键迁移Claude Code配置到DSH，支持多源导入
...
- **标签：** `theme`, `transparent-ui`, `glass-effect`
- **推荐理由：** 磨砂玻璃主题，全自由度调节
...
- **标签：** `background`, `theme`, `personalization`
- **推荐理由：** DSH Web个性化背景主题插件
...
- **标签：** `ai-guide`, `tutorial`, `vibecoding`
- **推荐理由：** 全面AI资源与Vibe Coding入门教程
...
- **标签：** `plugin-registry`, `ecosystem-radar`, `discovery`
- **推荐理由：** 自动发现15900+插件，k8s实测，生成插件目录
...
- **标签：** `awesome-list`, `ecosystem`, `tools`
- **推荐理由：** DSH生态精选插件、工具与基础设施
...
- **标签：** `academic-search`, `arxiv`, `pubmed`, `mcp`
- **推荐理由：** 多源学术检索，支持引用核验与引文图谱
...
- **标签：** `arxiv`, `latex`, `research`, `ssh`
- **推荐理由：** 一站式科研工作台：LaTeX编译、文献管理、实验追踪
...
- **标签：** `patent`, `rag`, `intellectual-property`
- **推荐理由：** 基于RAG的专利侵权分析，输出竞品报告
...
- **标签：** `awesome-list`, `collection`, `plugins`
- **推荐理由：** 好用的DSH插件集合
...
- **标签：** `awesome-list`, `collection`, `repository`
- **推荐理由：** DSH插件仓库集合
...
- **推荐理由：** 证据支持的插件雷达与2Origin实验室，便于发现
...
- **标签：** `awesome-list`, `deepseek-harness`, `catalog`
- **推荐理由：** 每日自动抓取更新，全面覆盖Dsh插件生态
...
- **推荐理由：** 在Agent内直接搜索npm和精选列表，高效发现插件
...
- **标签：** `ai-agents`, `awesome-list`, `coding-agents`, `mcp`
- **推荐理由：** AI精选190+插件，12分类，每小时更新，覆盖面广
...
- **标签：** `ai-agents`, `awesome`, `dsh-plugins`
- **推荐理由：** 70+经过验证的插件，按功能分类，易于查找
...
- **标签：** `agent-harness`, `awesome`, `coding-agents`, `ui`
- **推荐理由：** 覆盖模型、工具、沙盒、UI的全面插件精选列表
...
- **标签：** `awesome-list`, `deepseek-harness`, `industry`
- **推荐理由：** 面向金融、法律、医疗、电商等行业精选插件
...
- **标签：** `armor-breaking`, `prompt`, `specialized`
- **推荐理由：** DeepSeek专用破甲稳定化提示词插件
