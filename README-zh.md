...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 216,144 | Core framework powering the entire DSH plugin ecos |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 94,901 | Open-source Figma alternative with local-first des |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 71,636 | Meta-harness for multi-agent swarms and autonomous |
...
- **推荐理由：** DeepSeek原生终端编程Agent，支持���示缓存
...
- **标签：** `routing`, `agent-injection`, `reasoning-mode`
- **推荐理由：** 任务感知推理路由套件，智能注入Agent决策。
...
- **标签：** `agent-os`, `loop-engineering`, `mcp`, `evaluation`
- **推荐理由：** 自进化Agent OS，分阶段评估与预算演化循环。
...
- **标签：** `loop-engineering`, `agent-ops`, `long-horizon`, `workflow-automation`
- **推荐理由：** 长周期Agent控制面，支持跨Codex/Claude工作流。
...
- **标签：** `multi-agent`, `enterprise`, `visual-editor`, `agent-collaboration`
- **推荐理由：** 企业级本地优先多引擎Agent协作工作台。
...
- **标签：** `vision`, `OCR`, `coding-agent`
- **推荐理由：** DSH首个视觉插件，将图片转为结构化JSON，增强纯文本agent能力
...
- **推荐理由：** 首个AI虚拟终端，支持VFS沙盒隔离，适合安全的agent代码执行环境
...
- **标签：** `runtime`, `desktop`, `agent-ecosystem`
- **推荐理由：** Rust高性能agent运行时+精美桌面应用，结合代码agent深度与开放生态
...
- **推荐理由：** 基于12本经典工程的AI代码审查工具，带严重性标签。
...
- **标签：** `security`, `vulnerability`, `code-audit`
- **推荐理由：** AI代码安全审计平台，检测幻觉包与漏洞
...
- **推荐理由：** 编码Agent，支持RPC插件、子Agent与MCP
...
- **推荐理由：** 开源CLI与schema，管理DSH插件中心的工具集
...
- **标签：** `provider`, `llm`, `typescript`, `command-code`
- **推荐理由：** Command Code非官方LLM提供者，支持计划感知模型选择。
...
- **推荐理由：** DSH Web界面Skill与MCP管理工具，便捷高效
...
- **标签：** `plugin-market`, `webui`, `deepseek`
...
- **标签：** `capability`, `mcp`, `menu`, `dsh-plugin`
...
- **推荐理由：** 用此可复用 Codex 技能构建并验证 DSH 插件。
...
- **标签：** `codex`, `skill`, `code-simplification`, `dsh`
- **推荐理由：** 源自 DSH 的代码简化 Codex 技能，提升代码质量。
...
- **标签：** `sync`, `codex`, `mcp`, `bridge`
- **推荐理由：** OpenAI Codex 与 DSH 双向同步桥接，无缝切换协作。
...
- **标签：** `mascot`, `pixel-art`, `animated-pets`, `ui`
- **推荐理由：** 为代码Agent添加个性动画宠物的画廊。
...
- **标签：** `marketplace`, `plugin-store`, `ui`
- **推荐理由：** DSH内置插件市场，支持浏览搜索和一键安装，极大提升插件发现效率
...
- **标签：** `sidebar`, `ui`, `extensible`
- **推荐理由：** 开放式侧边栏底座，支持三方扩展，内置文件编辑/终端/Git等面板
...
- **推荐理由：** 现代化灵活任务面板，嵌入Codex/DSH，支持结构化工作流管理
...
- **推荐理由：** 鲸鱼顶栏+流式思考+上下文进度，Claude Code风格TUI，官方公众号推荐
...
- **推荐理由：** 仅5MB的Tauri桌面版，预装插件，零配置即可运行。
...
- **推荐理由：** 本地优先���Electron桌面工作区，支持DSH会话和项目。
...
- **推荐理由：** 开源Windows桌面客户端，集成SSH与技能系统
...
- **标签：** `workspace`, `canvas`, `conversation`
- **推荐理由：** 画布式非线性会话工作区，支持分支探索
...
- **标签：** `codepilot`, `desktop-app`, `electron`, `typescript`
- **推荐理由：** CodePilot风格桌面客户端，支持三平台的插件套件
...
- **标签：** `ios`, `ios-app`, `websocket`
- **推荐理由：** 原生iOS客户端，将DSH工作区与Agent轨迹带到iPhone
...
- **标签：** `notes`, `desktop`, `docker`, `smartisan`
- **推荐理由：** 开源锤子便签，支持Docker部署和公众号格式导出。
...
- **推荐理由：** Electron桌面控制台，多Tab管理DSH/插件/MCP。
...
- **标签：** `context`, `audit`, `ui`, `token`
- **推荐理由：** 上下文审计工具，统计 token 成本并检测重复与冲突。
...
- **标签：** `ui`, `skill-picker`, `composer`, `ux`
- **推荐理由：** 类 WorkBuddy 技能选择器，快速插入官方技能指令。
...
- **推荐理由：** 内置技能商店与离线模式的生物信息学桌面应用，面向科研人员。
...
- **标签：** `devops`, `desktop`, `ssh`, `docker`
- **推荐理由：** 集成数据库、SSH、Docker与AI助手的DevOps桌面中心。
...
- **标签：** `agent-memory`, `rag`, `memory-management`, `markdown-native`
- **推荐理由：** 本地优先的跨应用持久记忆层，用户自有数据。
...
- **标签：** `agent-memory`, `rag`, `long-term-memory`, `token-savings`
- **推荐理由：** 自演进记忆系统，混合检索节省35%token开销。
...
- **推荐理由：** 专为agent设计的记忆管理套件，支持RAG持久化记忆，实现跨会话知识留存
...
- **推荐理由：** 基于Markdown+Milvus的统一持久记忆层，支持多agent跨会话语义搜索
...
- **标签：** `agent-memory`, `skill-routing`, `vector-search`, `dsh-plugin`
- **推荐理由：** 将分散的技能文件转化为可检索的代理知识库。
...
- **标签：** `agent-orchestration`, `agent-skills`, `multi-device`, `no-code`
...
- **标签：** `agent-skills`, `skill-marketplace`, `automation`, `coding`
- **推荐理由：** 最全技能商店，覆盖文档、编程、金融与自动化。
...
- **标签：** `preset`, `workflow`, `bootstrap`
- **推荐理由：** 两阶段预设：轻量启动后加载完整Standard工具集，适合新手入门
...
- **推荐理由：** 智能技能路由与工作流编排，在SkillsBench上+21pp收益，显著降本增效
...
- **标签：** `cognitive-enhancement`, `agent-skills`, `research-driven`
- **推荐理由：** 基于Anthropic J-space全局工作空���研究的认知增强技能套件，提升agent推理
...
- **标签：** `agent-skills`, `video-generation`, `ffmpeg`, `dsh-plugin`
- **推荐理由：** 一键生成Vox风格 explanatory video，自动化全流程。
...
- **标签：** `web-search`, `agent-skills`, `free`
- **推荐理由：** 免费无需注册的跨模型网页搜索插件
...
- **标签：** `multimodal`, `ocr`, `video-analysis`, `observability`
- **推荐理由：** 多模态技能，将视频音频转为可查证时间戳证据
...
- **推荐理由：** 支持可组合Agent工作流与人工审查的自我进化机制。
...
- **推荐理由：** 为纯文本LLM增加多模态视觉能力，支持截图/UI分析。
...
- **标签：** `infrastructure`, `skills`, `ai-infra`
...
- **标签：** `reverse-engineering`, `pentest`, `security`, `ctf`
- **推荐理由：** 87个专项技能文件，覆盖逆向、渗透和安全研究。
...
- **标签：** `agent-skills`, `skill-generator`, `run-to-skill`
...
- **标签：** `skills`, `mentor`, `ai-agent`
- **推荐理由：** AI编程导师技能，阶梯式项目指导与Code Review
...
- **推荐理由：** 25个工程技能开箱即用，含Triaging和Wayfinder。
...
- **标签：** `plugin-dev`, `skills`, `tooling`
- **推荐理由：** 开发DSH插件的Agent技能，覆盖完整生命周期。
...
- **标签：** `document`, `chinese`, `office`
- **推荐理由：** 党政机关公文全流程处理，符合国标GB/T 9704。
...
- **标签：** `plugin-management`, `utility`
- **推荐理由：** 管理已安装DSH插件、插槽和凭据的Agent技能。
...
- **标签：** `guide`, `knowledge-base`, `plugin-development`, `docs`
- **推荐理由：** 官方文档库，按需调用插件开发知识技能。
...
- **标签：** `agent-skill`, `fengshui`, `bazi`, `iching`, `tcm`
- **推荐理由：** 融合八字/风水/六爻/中医的中华智慧AI技能包。
...
- **标签：** `qa`, `testing`, `agent-skills`, `pytest`
- **推荐理由：** 11 个测试技能+共享知识库，让 AI 像资深测试工程师。
...
- **标签：** `handoff`, `session-memory`, `cross-platform`, `continuity`
- **推荐理由：** 跨平台会话记忆技能，保持任务上下文连续性。
...
- **推荐理由：** 官方 SkillHub API 文档与插件，是构建技能的起点。
...
- **推荐理由：** Godot 4.x 全栈游戏开发技能，适合 AI 辅助游戏开发。
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin-bundle`, `dsh-plugin-desktop`, `dsh-plugin-market`, `dsh-plugins`
- **推荐理由：** 类CodeX技能市场，扩展DSH插件生态。
...
- **标签：** `design-systems`, `ui-generator`, `local-first`
- **推荐理由：** 本地优先的开源设计工具，AI驱动原型生成
...
- **标签：** `design`, `vector`, `ui`, `agent-team`, `vibedesign`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持Agent协同设计。
...
- **推荐理由：** DirectorX AI 视频/图像/音频技能插件，支持可视化创作。
...
- **推荐理由：** 跨平台开源AI内容发现agent，自动从B站/抖音/YouTube/小红书等平台挖掘内容
...
- **推荐理由：** 对话内集成iOS模拟器与真机控制，22个agent工具
...
- **标签：** `multi-platform`, `switcher`, `utility`
- **推荐理由：** 一键切换多coding平台：DSH/Claude Code/Codex/Grok/Kimi，统一管理多agent环境
...
- **标签：** `marketplace`, `plugin-shop`, `curation`
- **推荐理由：** 每日更新的插件市场，审核发布质量有保障
...
- **推荐理由：** OAuth接入ChatGPT/Claude/Grok订阅账号作LLM源
...
- **推荐理由：** 追踪会话/每日费用、预算和token使用，多提供商支持
...
- **标签：** `compatibility`, `migration`, `mcp`
- **推荐理由：** 打通Pi与DSH生态，Pi插件可无缝迁移为DSH插件。
...
- **标签：** `config`, `backup`, `migration`, `mcp`, `webdav`
- **推荐理由：** DSH配置备份还原迁移同步工具，支持WebDAV
...
- **⭐ 星标：** 59
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件��发引导
- **标签：** `plugin-management`, `infrastructure`
- **推荐理由：** 零补丁插件注册台+官方开发引导。
...
- **标签：** `container`, `utility`, `multi-version`, `dshbox`
- **推荐理由：** 隔离容器管理多版本 DSH，内嵌 WebView 界面便捷易用。
...
- **标签：** `troubleshoot`, `compatibility`, `utility`
- **推荐理由：** 诊断并解决插件兼容冲突问题
...
- **推荐理由：** 鲸鱼娘系列主题皮肤，为DSH提供可爱的二次元视觉定制体验
...
- **标签：** `chrome-extension`, `ocean-skin`, `threejs`, `webgpu`
- **推荐理由：** 基于WebGPU的海洋主题视觉皮肤，Chrome/Edge扩展
...
- **标签：** `ai-agent`, `deepseek`, `prompt`, `tutorials`
- **推荐理由：** 最全AI资源库，覆盖主流大模型、提示词与教程。
...
- **标签：** `arxiv`, `latex`, `research-tools`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、GPU实验追踪
...
- **推荐理由：** 跨库学术检索Skill，支持引文图谱与MeSH核验
...
- **标签：** `finance`, `research`, `mcp`
- **推荐理由：** 基于Tushare的多Agent金融分析系统，支持MCP调用。
...
- **标签：** `research`, `marketing`, `skills`, `workflows`
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销和商务自动化。
...
- **标签：** `research`, `industry`, `company-research`, `report`
- **推荐理由：** 行业研究方法包，含公司研究与政策追踪
...
- **标签：** `api-documentation`, `chinese-api`, `research`, `agents`
- **推荐理由：** 在AI Agent中直接检索20个中国开放平台6.5万篇API文档
...
- **标签：** `catalog`, `awesome-list`, `inventory`
- **推荐理由：** 千余插件全量清单，按星标排序
...
- **标签：** `awesome-list`, `agent-framework`, `dsh`
- **推荐理由：** DSH终极指南：快速入门、资源推荐与精选插件工具
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `tutorials`
...
- **推荐理由：** 精选插件、技能、MCP服务器与补丁的DSH资源列表
