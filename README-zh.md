...
- **标签：** `agent-harness`, `agent-orchestration`, `multi-device`, `workspace`
- **推荐理由：** 统一Agent工作空间，跨设备任务看板管理。
...
- **标签：** `routing`, `injector`, `harness`, `task-aware`
- **推荐理由：** 运行时注入器和任务感知路由器，智能Agent路由。
...
- **标签：** `agent-control-plane`, `long-horizon`, `orchestration`, `workflow`
- **推荐理由：** 长周期Agent控制面，支持持久化多Agent编排。
...
- **标签：** `agent-workspace`, `multi-agent`, `enterprise`, `visual-editor`
- **推荐理由：** 企业级多引擎Agent工作台，支持可��化编辑。
...
- **推荐理由：** AI安全攻防平台，审计幻觉包和漏洞，保护AI生成代码。
...
- **标签：** `oauth`, `subscriptions`, `multi-provider`
...
- **推荐理由：** 提供实时模型目录和推理强度控制的LLM Provider插件。
...
- **推荐理由：** 将Superpowers的TDD和调试技能带入DSH
...
- **标签：** `coding`, `mentor`, `skill`
- **推荐理由：** AI编程导师，自动讲解架构与代码审查
...
- **推荐理由：** 开发DSH插件的元技能，覆盖插槽/事件/工具/适配器
...
- **标签：** `plugin-dev`, `build`, `dsh`
- **推荐理由：** 用于构建和管理已安装DSH插件的Agent技能
...
- **标签：** `context-audit`, `token-optimization`, `tool`
- **推荐理由：** 审计指令链、检测冲突，降低Token消耗。
...
- **标签：** `sidebar`, `ui`, `extension-platform`
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git/对话等多页面。
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** Claude Code风格TUI，含鲸鱼顶栏、流式思考与上下文进度。
...
- **推荐理由：** 可爱浮窗鲸鱼监控DeepSeek余额，支持拖拽动画
...
- **标签：** `ui-components`, `charts`, `forms`, `dsh-ui`
...
- **标签：** `desktop`, `electron`, `windows`, `ssh-client`
- **推荐理由：** 开源Windows桌面客户端，集成Codex、插件、SSH和移动端远程访问。
...
- **推荐理由：** 本地优先多模型AI代理桌面应用，支持自进化技能和隐私优先。
...
- **推荐理由：** 原生iOS客户端，WebSocket同步Harness，移动办公利器。
...
- **标签：** `remote`, `p2p`, `encryption`
- **推荐理由：** 端到端加密P2P方案，实现多端远程访问和操作Harness。
...
- **标签：** `dsh`, `mcp`, `skills`, `webui`
- **推荐理由：** Web界面一站式管理Skills和MCP工具
...
- **标签：** `plugin-market`, `webui`, `dsh`
- **推荐理由：** Web GUI内一键浏览安装社区插件
...
- **推荐理由：** Electron桌面控制台，统一管理DSH插件、MCP和技能
...
- **推荐理由：** 集成数据库、SSH、SFTP、Docker的一站式DevOps桌面。
...
- **推荐理由：** Electron桌面壳集成DSH Web，内嵌侧边栏和ModLens等工具。
...
- **推荐理由：** 一键切换DSH Web上下文面板紧凑模式，简洁高效。
...
- **标签：** `agent-memory`, `rag`, `markdown-native`, `long-term-memory`
- **推荐理由：** 便携本地优先的记忆层，持久化Agent知识。
...
- **标签：** `memory`, `agentic-rag`, `stateful-agents`, `continual-learning`
- **推荐理由：** 状态感知Agent内存库，支持上下文工程。
...
- **标签：** `memory`, `rag`, `refinement`
- **推荐理由：** 专注代理记忆管理的工具包，支持记忆精炼与持久化。
...
- **推荐理由：** 持久统一记忆层，基于Markdown与Milvus的RAG方案。
...
- **标签：** `skill-md`, `vector-search`, `reranking`
- **推荐理由：** 将SKILL.md文件转为可检索的agent技能语料库��支持重排序和评估。
...
- **标签：** `knowledge-base`, `docs`, `agent-skill`
- **推荐理由：** 官方文档存档，作为按需调用的插件开发技能。
...
- **标签：** `skills`, `skill-store`, `agent-skills`, `automation`
- **推荐理由：** 最大技能商店，自动抓取多领域技能包。
...
- **标签：** `cognitive`, `skills`, `global-workspace`
...
- **标签：** `multimodal`, `video-understanding`, `ocr`, `deepwatch`
- **推荐理由：** 赋予AI视频、音频和屏幕理解能力，带时间戳可验证证据。
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领/审核，界面简洁无iframe。
...
- **标签：** `creative-writing`, `fiction`, `screenwriting`, `drama`
- **推荐理由：** 专注于小说创作和短剧生产的技能插件，集成Oh Story创作能力。
...
- **标签：** `skills`, `workflows`, `competition`
- **推荐理由：** 支持国赛美赛的自动化数学建模全流程skill
...
- **标签：** `agent`, `self-evolution`
- **推荐理由：** 可组合任务控制实现Agent自我进化与审查
...
- **推荐理由：** 基于Tushare的多Agent金融分析，支持MCP和IM盯盘。
...
- **推荐理由：** 专注AI基础设施开发、优化和调试的专业Agent技能。
...
- **标签：** `skills`, `research`, `marketing`, `cross-platform`
- **推荐理由：** 88个跨平台技能，覆盖研究、营销和商业工作流。
...
- **标签：** `security`, `reverse-engineering`, `pentest`, `ctf`
- **推荐理由：** 87个逆向工程和渗透测试技能，适用于安全研究。
...
- **标签：** `agent-skills`, `skill-generator`, `deepseek`
- **推荐理由：** 自动将成功会话转化为可复用Skills
...
- **推荐理由：** 专注专利侵权分析，支持多agent调用的RAG技能
...
- **标签：** `legal-tech`, `visualization`, `skill`
- **推荐理由：** 诉讼可视化工具集，生成时间轴和法律图表
...
- **标签：** `spec-programming`, `agent-skill`, `dev`
- **推荐理由：** 仓库原生Spec驱动开发技能，支持vibe编程
...
- **标签：** `chinese-traditional-wisdom`, `bazi`, `fengshui`
- **推荐理由：** 独特融合八字风水周易，本地确定性引擎。
...
- **推荐理由：** 在DSH Web GUI设置页统一管理技能与MCP服务器。
...
- **推荐理由：** 腾讯官方SkillHub Open API文档及DSH插件。
...
- **推荐理由：** 插件预检提升Agent利用率，任务前自动匹配最佳插件
...
- **标签：** `diagrams-as-code`, `visualization`
- **推荐理由：** 生成可验证的架构图表。
...
- **标签：** `design`, `ui`, `vibedesign`, `vector`, `agent-team`
...
- **标签：** `video`, `ai-video`, `generative`
- **推荐理由：** 自动生成Vox风格剪纸解释视频，端到端自动化
...
- **标签：** `design-skills`, `design-system`, `ui`, `vibe-coding`
- **推荐理由：** 提供6种设计风格模板，避免AI感强烈的网页外观。
...
- **标签：** `motion-design`, `remotion`, `video`
- **推荐理由：** 电影感产品视频制作技能，含152张镜头配方卡。
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内与ChatGPT对话
...
- **推荐理由：** 一键安装与多平台模型切换，支持Claude Code、Codex等。
...
- **标签：** `radar`, `ecosystem`, `infrastructure`
- **推荐理由：** 自部署插件雷达，k8s级自动发现测试15900+候选
...
- **标签：** `plugin-management`, `infrastructure`, `dsh`
- **推荐理由：** DSH插件生态基础设施，含浏览器控制台和脚手架技能
...
- **标签：** `skills-manager`, `utility`, `dsh`
- **推荐理由：** 统一加载和管理本地Agent Skills的工具
...
- **标签：** `containers`, `multi-version`, `infrastructure`
- **推荐理由：** 隔离容器运行多DSH版本，内嵌WebView界面。
...
- **标签：** `agent-evaluation`, `rag-evaluation`, `llm-evaluation`
- **推荐理由：** 基于证据的提示词、RAG及Agent评估框架。
...
- **标签：** `engineering-discipline`, `quality-gate`, `delivery-gate`
- **推荐理由：** 工程纪律质检插件，审核需求并验证交付质量。
...
- **标签：** `token-usage`, `data-visualization`, `deepseek-api`
- **推荐理由：** 实时追踪Token用量、成本及API余额，数据可视化一目了然。
...
- **推荐理由：** 诊断并解决插件间冲突与版本兼容性问题，插件必备工具。
...
- **标签：** `mascot`, `pixel-art`, `cli`, `theme`, `skins`
- **推荐理由：** Coding Agent动画宠物画廊，为CLI增添个性。
...
- **⭐ 星标：** 1,964
- **语言：** TypeScript
- **描述：** Whale Girl skin series for DeepSeek Harness. 适用于 DeepSeek Harness 的，鲸鱼���系列皮肤。
- **标签：** `skin`, `theme`, `whale`
- **推荐理由：** 鲸鱼娘系列皮肤，为DeepSeek Harness提供视觉美化。
...
- **推荐理由：** 高自由度玻璃质感主题，可调磨砂度、背景模糊和壁纸效果。
...
- **推荐理由：** WebGPU海洋主题，视觉炫酷的DSH界面美化插件。
...
- **标签：** `background`, `webui`, `theme`, `personalization`
- **推荐理由：** 自定义DSH Web背景，支持上传图片或切换预设氛围主题。
...
- **标签：** `ai`, `ai-agent`, `chatgpt`, `deepseek`
- **推荐理由：** 全面AI资源库，含教程和Prompt大全。
...
- **标签：** `agent`, `dsh`, `harness`, `tutorial`
- **推荐理由：** 从零开始的最佳Harness工程入门教程。
...
- **推荐理由：** 跨平台开源内容发现Agent，支持B站、YouTube、知乎等。
...
- **推荐理由：** LaTeX编译、arXiv管理与SSH编排的一站式科研工具
...
- **标签：** `research`, `mcp`, `academic`
- **推荐理由：** 跨源学术搜索含引文图谱与核验功能
...
- **标签：** `runtime`, `desktop`, `agent-os`
- **推荐理由：** Rust高性能代理运行时，搭配精美桌面应用体验。
...
- **标签：** `deepseek`, `plugin`, `specialized`
...
- **推荐理由：** 中英双语DSH插件、技能、工具精选列表，一站式导航。
...
- **标签：** `knowledge-base`, `resources`, `awesome-list`, `mcp`
- **推荐理由：** 全维度AI资源百科，涵盖Agent、RAG、多模态和零基础学习路线。
...
- **标签：** `plugin-market`, `marketplace`, `curation`
- **推荐理由：** 最全面的DSH插件市场，每日更新，发布前审核。
...
- **标签：** `marketplace`, `education`
- **推荐理由：** 学习DSH原理与探索插件市场的综合平台
...
- **标签：** `plugin-marketplace`, `community`, `awesome-list`
- **推荐理由：** 社区维护的DSH插件聚合目录，定时同步
...
- **推荐理由：** 精选DeepSeek Harness插件列表及网页展示。
...
- **标签：** `awesome-list`, `deepseek-harness`
- **推荐理由：** 全面的DeepSeek Harness插件资源精选列表。
...
- **标签：** `plugin-registry`, `curated-list`
- **推荐理由：** 去垃圾信息的开放数据插件注册中心。
...
- **推荐理由：** 2026年精选插件目录，附带质量检查，方便筛选可靠插件。
...
- **推荐理由：** 24个精选股票/金融/量化插件，中英双语，垂直领域聚合。
...
- **标签：** `awesome-list`, `community`, `collection`
- **推荐理由：** 精心挑选的DSH社区实用插件合集，便于快速发现。
...
- **推荐理由：** DSH工具完整清单，1000+仓库按星标排序，一站式查询。
...
- **标签：** `automated`, `directory`, `daily-sync`
- **推荐理由：** GitHub Actions每日自动爬取，保持插件目录最新且双语。
...
- **标签：** `deepseek-harness`, `dsh-plugins`, `dsh-ecosystem`
- **推荐理由：** DeepSeek Harness插件精选目录
