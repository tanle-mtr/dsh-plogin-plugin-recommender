...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 194,613 | Core DSH framework enabling everything-as-plugin a |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,364 | Open-source Claude Design alternative with local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,366 | Multi-agent swarm harness for autonomous workflow  |
...
- **推荐理由：** 多代理蜂群元harness，协调自主工作流与对话AI系统。
...
- **标签：** `routing`, `reasoning-mode`, `orchestration`
- **推荐理由：** 任务感知推理路由套件，提升代理决策能力
...
- **标签：** `agent-os`, `self-improving`, `evaluation`
- **推荐理由：** 自我进化的代理操作系统，自动学习与成长
...
- **标签：** `security`, `code-review`, `redteam`
- **推荐理由：** 审计AI生成代码，检测幻觉包、安全漏洞与模式错误。
...
- **标签：** `agent-coordination`, `multi-agent`, `coding`
- **推荐理由：** 将多个编码Agent整合为一个协同团队，统一调度。
...
- **标签：** `jailbreak`, `prompt-engineering`, `breaking`
- **推荐理由：** DeepSeek专用破甲插件，稳定化提示词实现无限制输出。
...
- **推荐理由：** WorkBuddy风格技能选择器，提升DSHComposer使用效率。
...
- **推荐理由：** Codex与DSH双向桥梁，支持实时技能与MCP同步。
...
- **标签：** `context`, `audit`, `optimization`, `token`
- **推荐理由：** 上下文注入审计，检测重复冲突，可视化反馈。
...
- **标签：** `plugin-dev`, `agent-skills`, `auditing`
- **推荐理由：** 便携插件开发审计技能，附带DSH Bundle适配器
...
- **标签：** `engineering`, `discipline`, `portable-skill`
- **推荐理由：** 从DSH实战提炼的工程规范技能
...
- **标签：** `agent-tools`, `pre-flight`, `plugin-discovery`
- **推荐理由：** 任务前自动预检并注入适用插件的智能检查工具。
...
- **标签：** `image-uploader`, `electron-app`, `dsh-plugin`
- **推荐理由：** 终极图片上传工具，支持60+图床及多款编辑器集成。
...
- **标签：** `web-ui`, `plugin-marketplace`, `agggregator`
- **推荐理由：** Web界面插件聚合包，一站式插件市场
...
- **标签：** `tui`, `terminal`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 鲸鱼顶栏TUI，支持流式思考与实时状态。
...
- **标签：** `desktop`, `mcp`, `dsh-plugin`
- **推荐理由：** 本地AI桌面工作区，支持MCP与插件。
...
- **推荐理由：** 本地优先多模型Agent桌面应用，隐私保护，技能可进化。
...
- **标签：** `ui`, `components`, `gen-ui`
- **推荐理由：** 在聊天回复中内联渲染图表、表单等交互式UI组件。
...
- **推荐理由：** CodePilot风格跨平台桌面客户端，含插件生态套件。
...
- **推荐理由：** 原生SQLite任务看板，无需iframe即可使用
...
- **标签：** `ui`, `github-oauth`, `photo-wall`
- **推荐理由：** GitHub OAuth登录的合影墙功能，轻量社交UI
...
- **标签：** `ui`, `balance`, `pricing`, `status-bar`
- **推荐理由：** 底部状态栏实时显示模型余额和定价
...
- **推荐理由：** Tauri 2桌面整合数据库、SSH、Docker面板与AI助手
...
- **标签：** `electron`, `desktop`, `dsh`
- **推荐理由：** Electron桌面壳，集成完整DSH生态组件。
...
- **推荐理由：** LLM知识平台，将文档转化为RAG、代理和自维护Wiki。
...
- **标签：** `memory-management`, `long-term-memory`, `RAG`
- **推荐理由：** 可移植、用户自有的AI代理长期记忆层
...
- **标签：** `memory-management`, `hybrid-retrieval`, `token-savings`
...
- **标签：** `knowledge-base`, `encyclopedia`, `rag`
- **推荐理由：** 涵盖Agent、RAG、MLOps的全维度AI知识库百科。
...
- **标签：** `vector-search`, `embeddings`, `reranking`
...
- **标签：** `memory`, `recall`, `dsh-plugin`
- **推荐理由：** 连接PowerContext实现记忆与经验召回
...
- **标签：** `agent-workspaces`, `cross-platform`, `no-code`
- **推荐理由：** 跨平台统一AI代理工作空间，支持任务看板
...
- **推荐理由：** 面向Reasonix的Superpowers TDD调试技能
...
- **标签：** `skills`, `superpowers`
- **推荐理由：** 适配DSH的Superpowers TDD与规划技能
...
- **标签：** `agent-skills`, `deep-research`, `market-research`, `skill-md`
- **推荐理由：** 88个开源Agent技能，覆盖调研营销与业务。
...
- **推荐理由：** 教写DSH插件的Agent Skill，开发标准指南。
...
- **标签：** `agent-skills`, `spec-driven-development`, `vibe-coding`
- **推荐理由：** 可移植的规格驱动开发工作流，契合DSH范式。
...
- **标签：** `agent-skill`, `chinese-wisdom`, `fengshui`, `iching`
- **推荐理由：** 融合八字/风水/易学等传统智慧，本地确定性引擎。
...
- **标签：** `agent-handoff`, `session-memory`, `multi-agent`
- **推荐理由：** 跨Agent会话记忆与任务交接，支持多Agent工作流连续性。
...
- **推荐理由：** 将DSH代码简化技能适配Codex，自动化代码精炼。
...
- **推荐理由：** Swift Cycle治理技能适配器，版本锁定离线可验。
...
- **标签：** `design`, `skills`, `aesthetics`, `vibe-coding`
- **推荐理由：** 6种设计美学技能包，避免AI生成网站的同质化外观。
...
- **标签：** `vision`, `image-generation`, `skills`
- **推荐理由：** 支持图片识别与生成技能，扩展AI视觉能力
...
- **标签：** `gongwen`, `docx`, `office`, `chinese-docs`
- **推荐理由：** 符合国标公文格式的全流程处理技能
...
- **标签：** `agent-framework`, `mcp`, `database`, `eval-driven`
- **推荐理由：** 42项技能加5个CLI，AI工程化一站式
...
- **标签：** `video`, `image`, `audio`, `directorx`
- **推荐理由：** 整合AI视频/图像/音频生成技能工具链
...
- **标签：** `skills`, `gamification`, `wayfinder`, `task`
- **推荐理由：** 游戏化任务系统，迷雾探索推进行程
...
- **标签：** `mcp`, `skill-management`, `hot-reload`, `config`
- **推荐理由：** 热加载MCP和技能，配置即生效免重启
...
- **标签：** `mcp`, `skills`, `canvas`
- **推荐理由：** 无限画布结合创意技能与MCP工具支持
...
- **标签：** `mcp`, `zhihu`, `search`, `cli`
- **推荐理由：** 知乎搜索/知答/MCP接口一体化工具
...
- **推荐理由：** 开源设计插件，本地优先桌面应用，类Claude Design体验。
...
- **推荐理由：** 530+个GPT图像提示词模板，工业级案例逆向工程提炼。
...
- **标签：** `design`, `vector`, `UI-generation`, `vibecoding`
- **推荐理由：** AI原生矢量设计工具，多代理协作生成UI
...
- **标签：** `agent-skills`, `diagram`, `legal`, `timeline`, `svg`
- **推荐理由：** 生成诉讼时间轴图表，法律可视化利器。
...
- **推荐理由：** 面向Gemini/Claude/ChatGPT的跨浏览器增强套件。
...
- **标签：** `ai-agent`, `bilibili`, `youtube`, `dsh-plugin`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站抖音等。
...
- **推荐理由：** 免费无注册网页搜索插件，为无联网能力的模型提供搜索桥接。
...
- **推荐理由：** 在对话中操控iOS模拟器与真机，22个自动化Agent工具。
...
- **标签：** `dingtalk`, `chat-bot`, `deepseek-harness`
- **推荐理由：** 钉钉IM频道，将聊天转为Agent工具
...
- **推荐理由：** AI+低代码平台，快速构建CRM、ERP及内部业务系统。
...
- **标签：** `vfs`, `virtual-filesystem`, `abstraction`
- **推荐理由：** 统一的虚拟文件系统抽象层，服务所有代理
...
- **标签：** `context-management`, `dsh-plugin`, `utility`
- **推荐理由：** 上下文仪表盘与管理，支持统计洞察。
...
- **推荐理由：** 通过OAuth使用各平台订阅作为DSH LLM提供商，无需API密钥。
...
- **标签：** `marketplace`, `plugin-hub`, `registry`
- **推荐理由：** 3100+插件市场，支持搜索和免费API
...
- **标签：** `container`, `plugin-management`, `dsh`
- **推荐理由：** 容器化管理多版本DSH，一键导入插件，本地部署便捷。
...
- **推荐理由：** 完整配置备份恢复同步，支持WebDAV，迁移必备工具。
...
- **⭐ 星标：** 20
- **语言：** JavaScript
- **描述：** DSH Skills Manager 基��� DeepSeek Harness 的Skills管理插件
- **标签：** `skills`, `manager`, `organization`
- **推荐理由：** 专业DSH技能管理插件，高效组织技能。
...
- **标签：** `migration`, `mcp`, `agent-migration`
- **推荐理由：** 将Claude Code配置迁移至DSH的实用工具
...
- **标签：** `utility`, `conflict-resolution`, `plugin-health`
...
- **推荐理由：** 基于WebGPU和threejs的精美海洋主题皮肤
...
- **标签：** `theme`, `skin`, `dark-mode`, `opencode`
- **推荐理由：** 34款主题一键切换，重启不丢失设置
...
- **推荐理由：** 程序员鱼皮的AI资源大全，涵盖DeepSeek/GPT/Claude教程。
...
- **标签：** `ai-agents`, `api-documentation`, `chinese-api`, `deepseek-harness`
...
- **标签：** `awesome-list`, `plugin-discovery`, `directory`
- **推荐理由：** 精选280+社区插件目录，覆盖14类，快速发现优质插件。
...
- **标签：** `kline`, `stock`, `analysis`, `chart`
- **推荐理由：** 交互式K线分析，支持多市场技术指标与基本面分析。
...
- **标签：** `evaluation`, `rag`, `benchmark`, `agent-evaluation`
- **推荐理由：** 证据驱动评估框架，覆盖RAG、提示和Agent
...
- **标签：** `radar`, `plugin-lab`, `research`
- **推荐理由：** 基于证据的插件雷达和2Origin插件实验室。
...
- **标签：** `recommender`, `ai-agents`, `mcp`
- **推荐理由：** AI精选190+插件，每小时更新
...
- **标签：** `curated-list`, `typescript`, `ui`, `skills`
- **推荐理由：** 模型、工具、沙箱、UI插件精选
...
- **推荐理由：** 每日自动抓取GitHub，过滤蹭标签项目，场景化分类推荐。
...
- **标签：** `awesome-dsh-plugin`, `dsh-ecosystem`, `plugin-marketplace`
...
- **标签：** `redirect`, `meta`
- **推荐理由：** 重定向至迁移后的awesome-dsh-plugin仓库。
