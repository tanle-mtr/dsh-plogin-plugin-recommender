...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 191,878 | Core harness platform powering the entire DSH plug |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,087 | Open-source design alternative with AI-powered UI  |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,295 | Deploy multi-agent swarms with autonomous workflow |
...
- [Agent 技能与工作流](#agent-技能与工作流) — 43 个插件
...
- [Agent 技能与工作流](agent-skills-workflows-zh) (43)
...
- **标签：** `coding-agent`, `terminal`, `deepseek`
- **推荐理由：** 终端原生DeepSeek编程智能体，支持提示缓存。
...
- **标签：** `agent-runtime`, `rust`, `coding-agent`, `agentic-os`
- **推荐理由：** 高性能 Rust Agent 运行时搭配精致桌面应用，面向 Agentic 编码场景。
...
- **标签：** `code-review`, `agent-skill`, `clean-architecture`, `static-analysis`
- **推荐理由：** 基于12本经典工程书籍的 AI 代码审查，支持严重程度标签与自动修复。
...
- **标签：** `coding-agents`, `architecture`, `safety`
- **推荐理由：** 架构感知编码代理，基线优先防止长期任务漂移
...
- **标签：** `coding-agent`, `dsh-plugin`, `multimodal`, `orchestration`, `mcp`
- **推荐理由：** 从Claude Code/Codex调度DSH agent的子代理编排工具
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `containers`
- **推荐理由：** 容器隔离多版本管理，WebView嵌入，一键导入插件
...
- **标签：** `plugin-development`, `audit`, `cordis`
...
- **标签：** `plugin-dev`, `documentation`, `best-practices`
- **推荐理由：** DSH插件开发经验档案与最佳实践文档
...
- **标签：** `plugin-manager`, `mcp`, `skills`
- **推荐理由：** 内嵌GUI插件管理器，支持MCP/技能热更新
...
- **推荐理由：** Agent任务预检插件库，自动注入Top-K相关提示
...
- **标签：** `sidebar`, `ui`, `deepseek-harness`
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话/Git 等面板。
...
- **推荐理由：** 鲸鱼顶栏 Claude Code 风格终端 UI，支持流式思考与上下文进度显示。
...
- **标签：** `widget`, `desktop`, `ui`, `balance`
- **推荐理由：** 右下角浮动鲸鱼，实时显示账户余额
...
- **标签：** `desktop`, `workspace`, `electron`, `local-first`
- **推荐理由：** 本地优先桌面工作台，支持项目与Office协作
...
- **标签：** `desktop-app`, `agent`, `dsh-plugin`
- **推荐理由：** 本地优先多模型桌面代理，隐私安全，技能可自进化。
...
- **标签：** `desktop-app`, `dsh-plugin`
- **推荐理由：** 跨平台桌面客户端，类CodePilot体验，附插件套件。
...
- **标签：** `ui`, `workspace`, `dsh-plugin`
- **推荐理由：** 画布式非线性对话工作区，支持会话分叉探索。
...
- **标签：** `desktop`, `electron`, `gui`, `windows`, `ssh`
- **推荐理由：** 原生Windows桌面客户端，内置SSH和远程访问功能。
...
- **标签：** `console`, `dsh-repository-plugin`, `plugin-management`, `ui`
- **推荐理由：** 零补丁控制台管理DSH插件仓库
...
- **推荐理由：** Electron桌面端多标签管理Harness/插件/MCP
...
- **标签：** `ui-extension`, `info-bar`, `pricing`, `usage-tracking`
- **推荐理由：** 显示模型、余额和实时价格的底部信息栏
...
- **⭐ 星标：** 9
- **语言：** Rust
- **描述：** StarHub 是一款跨平台桌面应用（Tauri 2 + Rust 主进程 + DeepSeek Harness React 工作台 + Go Sidecar），把开发运维日常高频工具整合到同一个窗口 —— 数据库、SSH/SFTP、Docker 面板与 AI 助手。目标是减少在 Navicat、Xshell、Portainer、文件管理器和 AI 对话窗口之间来回切换的成本。  前端架构：基于 DeepSeek Harness 原生 React 工作台（/starhub-react 路由）。资产、设置、SSH ���端、SFTP、数据库
- **标签：** `desktop`, `tauri`, `docker`, `ssh`
- **推荐理由：** Tauri跨平台桌面集成DB/SSH/Docker+AI助手
...
- **标签：** `memory`, `token-savings`, `self-evolving`
- **推荐理由：** 自进化记忆OS，节省35% token开销。
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** 专为 Agent 设计的记忆管理工具，支持记忆提炼与持久化。
...
- **推荐理由：** 基于 Milvus 的持久统一记忆层，支持混合搜索，适用于所有 AI Agent。
...
- **标签：** `memory`, `rag`, `persistent-storage`
- **推荐理由：** 为AI代理提供无限持久化记忆与知识存储
...
- **标签：** `rag`, `embeddings`, `vector-search`, `memory`
- **推荐理由：** 将分散的技能文件转化为可检索的知识库。
...
- **标签：** `agent-memory`, `dsh-plugin`, `long-term-memory`, `mcp`
- **推荐理由：**  durable长期记忆插件，支持检索工具与可检查存储
...
- **标签：** `agent-skill`, `knowledge-base`, `plugin-development`, `scaffold`
- **推荐理由：** 按需调用的插件开发知识库，含官方中英文档
...
- **标签：** `context`, `audit`, `ui`, `dsh-plugin`
- **推荐理由：** 审计上下文token成本，检测重复与冲突
...
- **推荐理由：** 接入PowerContext实现持久记忆和技能交接
...
- **标签：** `prompt-engineering`, `image-generation`, `skills`
- **推荐理由：** GPT图像生成工业级提示词模板与技能库。
...
- **标签：** `agent-orchestration`, `workspaces`, `task-tracking`
- **推荐理由：** 统一Agent工作空间，跨设备任务追踪。
...
- **标签：** `enterprise`, `workbench`, `multi-agent`
- **推荐理由：** 企业级本地优先Agent工作台，多引擎协作。
...
- **推荐理由：** 基于全局工作空间研究的认知增强技能集，提升 Agent 推理能力。
...
- **标签：** `agent-skills`, `workflow-automation`, `multi-agent`
- **推荐理由：** 智能技能路由与编排工具，自动分配和协调 Agent 任务流。
...
- **标签：** `workflow`, `productivity`, `cross-platform`
...
- **标签：** `creative-writing`, `drama`, `fiction`, `screenwriting`
- **推荐理由：** 专注小说创作与短剧剧本生成的智能技能插件。
...
- **标签：** `skills`, `agent`, `architecture`, `automation`
- **推荐理由：** 将经验转化为可复用的AI代理执行技能。
...
- **标签：** `vision`, `multimodal`, `claude-code`, `agent-skills`
...
- **推荐理由：** 锤子便签复刻，支持skill调用与一键Docker私有化部署
...
- **推荐理由：** 原生本地任务看板，SQLite驱动支持agent认领与审查
...
- **标签：** `dsh-plugin`, `agent-workflow`
- **推荐理由：** DSH Agent工作流插件，支持多步骤任务编排
...
- **标签：** `dsh-plugin`, `skills`, `superpowers`
- **推荐理由：** Superpowers技能集，涵盖TDD/调试/规划/协作
...
- **标签：** `agent-skills`, `deep-research`, `marketing-automation`, `social-listening`
- **推荐理由：** 88个开源技能，覆盖研究、营销、社交分析
...
- **标签：** `agent-skills`, `spec-driven-development`, `ai-coding`, `dsh-plugin`
- **推荐理由：** 基于DSH工程规范的规格驱动开发技能
...
- **标签：** `agent-handoff`, `skill`, `session-memory`
- **推荐理由：** 跨平台会话记忆，Agent交接无缝衔接
...
- **标签：** `codex`, `skill`, `code-simplification`
- **推荐理由：** 适配Codex的代码简化技能，跨平台可用
...
- **标签：** `agent-skills`, `mcp`, `database`, `package-manager`
- **推荐理由：** 42个技能、CLI和数据库工具的Agent资产包管理器
...
- **推荐理由：** 版本锁定、可离线验证的Swift Cycle治理技能
...
- **⭐ 星标：** 14
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公��写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `chinese-docs`, `wps-office`
...
- **标签：** `engineering`, `best-practices`, `agent-skills`
- **推荐理由：** 工程纪律技能：事实核查、持久决策、诚实报告
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，游戏开发Agent首选
...
- **推荐理由：** 虚拟产品团队Agent预设，模拟PM/工程/QA协作
...
- **标签：** `dsh-plugin`, `mcp`, `skills`
- **推荐理由：** Web界面skill/MCP管理工具
...
- **推荐理由：** 通过OAuth绑定ChatGPT账户，在DSH中使用其模型
...
- **标签：** `design-systems`, `ui-generator`, `figma-alternative`
- **推荐理由：** 开源AI设计替代方案，支持UI生成。
...
- **标签：** `diagrams`, `mermaid-alternative`, `agent-skills`
- **推荐理由：** 自包含HTML架构动图，视觉效果出色。
...
- **标签：** `video-generation`, `ffmpeg`, `agent-skill`, `vox`
- **推荐理由：** AI Agent 驱动的 Vox 风格拼贴解说视频端到端自动生成。
...
- **标签：** `image-generation`, `design`, `gemini`, `openai`
- **推荐理由：** 在DSH对话中直接生成图片，支持多模型。
...
- **标签：** `design`, `ui-design`, `openpencil`, `ppt`
...
- **标签：** `design`, `ui`, `vibe-coding`, `web-design`
- **推荐理由：** 6种设计风格预设，告别AI感外观
...
- **标签：** `ai-video`, `ai-image`, `storyboard`, `directorx`
- **推荐理由：** AI视频/图像/音频技能，支持分镜和可配置模型
...
- **标签：** `web`, `plugin-market`, `ecosystem`
- **推荐理由：** DSH Web插件聚合生态，创意工坊分发。
...
- **标签：** `bilibili`, `cross-platform`, `ai-agent`, `local-first`
- **推荐理由：** 自进化跨平台内容发现 Agent，覆盖 B站、抖音、YouTube 等主流平台。
...
- **标签：** `web-search`, `skill`, `dsh-plugin`
- **推荐理由：** 免费无需注册，为无联网能力模型提供万能搜索桥接。
...
- **推荐理由：** 22个工具在DSH内控制iOS模拟器或真机，支持自动化。
...
- **标签：** `dsh-plugin`, `browser-automation`, `playwright`
- **推荐理由：** Tabbit浏览器自动化插件，基于Playwright
...
- **标签：** `routing`, `injector`, `runtime`
- **推荐理由：** 注入器+路由标准套件，任务感知推理。
...
- **标签：** `deepseek-harness`, `claude-code`, `codex`
- **推荐理由：** 一键安装切换多个 AI 编码工具，极大简化多模型管理。
...
- **标签：** `marketplace`, `deepseek-harness`, `plugin`
- **推荐理由：** DSH 官方插件市场，支持浏览搜索和一键安装。
...
- **标签：** `utility`, `oauth`, `dsh-plugin`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅接入DSH作为LLM源。
...
- **标签：** `cost-tracking`, `tokens`, `utility`, `budget`
- **推荐理由：** 追踪会话与每日费用，支持多API提供商余额管理。
...
- **标签：** `compatibility`, `migration`, `bridge`, `ecosystem`
- **推荐理由：** 无缝桥接Pi与DSH生态，直接运行Pi扩展插件。
...
- **标签：** `dsh-plugin`, `marketplace`, `plugin-marketplace`
- **推荐理由：** DSH插件市场，Web GUI中一键浏览安装所有插件
...
- **标签：** `context-migration`, `dsh-plugin`, `session-migration`
- **推荐理由：** 跨预设会话迁移插件，保留状态与模型意图
...
- **标签：** `backup`, `dsh-plugin`, `crash-recovery`, `undo`
- **推荐理由：** 崩溃恢复插件，支持撤销配置与密钥安全快照
...
- **标签：** `dsh-plugin`, `plugin-market`, `webui`
- **推荐理由：** DSH社区插件市场，Web GUI一键安装卸载
...
- **标签：** `config-manager`, `backup`, `restore`, `sync`
- **推荐理由：** 配置备份恢复迁移，WebDAV同步完整设置
...
- **标签：** `awesome-dsh-plugin`, `plugin`, `utilities`
- **推荐理由：** 检测并解决DSH插件间冲突与版本问题
...
- **标签：** `registry`, `discovery`, `multi-source`
- **推荐理由：** 整合多数据源的统一插件发现工具
...
- **标签：** `cli`, `plugin-manager`, `audit`
- **推荐理由：** 用于发现、安装和审计DSH插件的CLI工具
...
- **推荐理由：** DeepSeek Harness 鲸鱼娘系列皮肤，增添个性化视觉体验。
...
- **标签：** `theme`, `ocean`, `threejs`, `webgpu`
- **推荐理由：** 基于WebGPU的深海主题皮肤，视觉效果出色。
...
- **标签：** `themes`, `skins`, `dark-mode`, `ui-customization`
...
- **标签：** `dsh-plugin`, `webui`, `themes`
- **推荐理由：** 支持上传自定义背景与氛围预设，实时预览
...
- **标签：** `research`, `web-search`, `skill`, `chinese-platforms`
- **推荐理由：** 自动搜索中国八大主流平台近30天内容并生成有据可查的研究��
...
- **标签：** `awesome-list`, `plugin-registry`, `discovery`
- **推荐理由：** 插件雷达与精选榜，自动更新索引全量插件
...
- **标签：** `awesome-list`, `ecosystem`, `infrastructure`
- **推荐理由：** DSH生态精选插件、工具与���础设施汇总
...
- **推荐理由：** 双语精选插件资源目录，覆盖Agent与各类工具集成。
...
- **标签：** `ai-pedia`, `dsh-plugin`
- **推荐理由：** 涵盖Agent、RAG、MLOps的一站式AI资源百科导航。
...
- **标签：** `awesome`, `dsh-plugin`, `awesome-list`
- **推荐理由：** 精选DSH插件清单，方便发现与参考
...
- **标签：** `kline`, `finance`, `analysis`, `research`
- **推荐理由：** 交互式K线分析，多市场技术指标支撑压力位
...
- **标签：** `evaluation`, `llm-evaluation`, `rag-evaluation`, `agent-evaluation`
- **推荐理由：** 面向RAG、提示词和Agent的证据化评测框架
...
- **标签：** `awesome-list`, `plugin-directory`, `dsh`
- **推荐理由：** 插件导航手册，含安装、权限与兼容性说明
...
- **标签：** `awesome-dsh-plugin`, `web`, `plugin-portal`
- **推荐理由：** 静态网页门户，浏览和发现227+个DSH插件
...
- **标签：** `awesome-list`, `dsh-plugin`
- **推荐理由：** 已迁移的原始awesome-dsh-plugins仓库
...
- **标签：** `awesome-list`, `mcp`, `coding-agents`
- **推荐理由：** AI每小时更新，覆盖12类190+插件的综合推荐
...
- **标签：** `awesome-list`, `bilingual`, `auto-update`
...
- **标签：** `awesome-list`, `reviewed`, `evidence-led`
- **推荐理由：** 基于证据审核的插件目录，提交不可变
...
- **标签：** `awesome-list`, `curated`, `shortlist`
- **推荐理由：** 精选的有用DSH插件短名单，具证据意识
...
- **标签：** `awesome-list`, `verified`, `organized`
- **推荐理由：** 70+个已验证的DSH插件，按功能组织
...
- **标签：** `awesome-list`, `comprehensive`, `multi-domain`
- **推荐理由：** 覆盖模型、工具、UI和智能体循环的全面列表
...
- **标签：** `awesome-list`, `industry`, `vertical`
- **推荐理由：** 按行业筛��的DSH插件，适用于金融、法律等
...
- **标签：** `awesome-list`, `comprehensive`, `star-sorted`
- **推荐理由：** 1000+按星标排序的DSH工具，生态全景地图
...
- **标签：** `low-code`, `no-code`, `crm`, `erp`
- **推荐理由：** AI驱动的无代码商业系统快速构建平台。
...
- **标签：** `gallery`, `pets`, `community`
- **推荐理由：** 多AI编码Agent的动画宠物公开画廊。
...
- **标签：** `toy`, `single-file`, `no-dependency`
- **推荐理由：** 零依赖网页版传统玩具竹知了，轻量有趣的小项目。
...
- **标签：** `awesome-list`, `plugin-catalog`, `deepseek-harness`
- **推荐理由：** DSH生态插件一站式索引目录，方便发现各类资源。
...
- **⭐ 星标：** 182
- **语言：** TypeScript
- **描述：** DeepSeek Harness plugin store, marketplace and hub — 3,100+ dsh plugins with search, rankings, install commands and a free public API. DeepSeek Harness 插件市场 / 插件商店：自动收集与格式校���，免费搜索 API。deepseek1024.com
- **标签：** `marketplace`, `plugin-store`, `catalog`
- **推荐理由：** 3100+插件市场，支持搜索和免费公共API。
...
- **标签：** `marketplace`, `plugin-hub`, `harness`
- **推荐理由：** DSH生态插件市场与聚合中心。
...
- **标签：** `awesome-list`, `dsh-plugins`, `community`
- **推荐理由：** 280+社区插件精选目录，14类全覆盖
...
- **标签：** `awesome-list`, `ecosystem`, `discovery`
- **推荐理由：** 精选DSH插件列表，帮助发现生态资源
...
- **标签：** `awesome-list`, `directory`, `plugin-discovery`
- **推荐理由：** 公开DSH插件目录，含安装命令和生态发现
...
- **标签：** `registry`, `awesome-list`, `plugins`
- **推荐理由：** 开源数据过滤垃圾的DSH插件/技能注册表
