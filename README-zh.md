...
- [编程 Agent 与 Harness 工具](#编程-agent-与-harness-工具) — 14 个插件
- [UI 与桌面扩展](#ui-与桌面扩展) — 27 个插件
- [记忆与知识（RAG）](#记忆与知识（rag）) — 9 个插件
- [Agent 技能与工作流](#agent-技能与工作流) — 44 个插件
...
- [编程 Agent 与 Harness 工具](coding-agents-harness-tools-zh) (14)
- [UI 与桌面扩展](ui-desktop-extensions-zh) (27)
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (9)
- [Agent 技能与工作流](agent-skills-workflows-zh) (44)
...
- **标签：** `ai-agent`, `mcp`, `multi-agent`, `skills`
- **推荐理由：** 具备记忆能力的超级 AI 智能体平台，支持多智能体协作
...
- **标签：** `coding-agent`, `cli`, `prompt-caching`, `tui`
- **推荐理由：** DeepSeek 原生终端编码智能体，前缀缓存稳定运行
...
- **⭐ 星标：** 3,984
- **语言：** TypeScript
- **描述：** The first vision plugin for DeepSeek Harness, and the vision bridge for every text-only coding agent. Paste an image, get structured JSON evidence (OCR, layout, semantics). | 全网最强 DeepSeek Harness 外��视觉插件，为 DeepSeek、GLM 等纯文本模型外挂视觉能力，粘贴图片即得结构化 JSON 证据（OCR、版面、语义）。
- **标签：** `vision`, `ocr`, `multimodal`
- **推荐理由：** 首个视觉插件，让文本Agent也能处理图片输入
...
- **推荐理由：** 两阶段启动预设，确保Standard工具稳定初始化
...
- **标签：** `vfs`, `virtual-terminal`, `sandbox`
- **推荐理由：** 虚拟文���系统和终端，安全隔离Agent工作空间
...
- **标签：** `agent-teams`, `harness`, `collaboration`
- **推荐理由：** 在DeepSeek Harness中实现多代理团队协作能力。
...
- **标签：** `context`, `dashboard`, `harness-tools`
- **推荐理由：** 提供上下文仪表盘、浏览器和侧边栏，深度洞察管理上下文。
...
- **推荐理由：** 为纯文本模型打造视觉工具箱：OCR、截图还原、多图问答。
...
- **标签：** `cli`, `plugin-manager`, `developer-tools`, `typescript`
- **推荐理由：** DSH插件发现、解析和管理的CLI与智能体工具。
...
- **推荐理由：** TDD调试规划技能，源自Superpowers适配DSH
...
- **标签：** `cordis-plugin`, `desktop`, `dsh-plugin`
- **推荐理由：** 为 DSH 插件生态打造的现代化桌面端解决方案
...
- **标签：** `low-code`, `internal-tools`, `ai-agent`, `workflows`
- **推荐理由：** AI 驱动的无代码平台，快速构建企业内部系统
...
- **标签：** `taskboard`, `ui`, `productivity`
- **推荐理由：** 现代化灵活任务面板，深度集成Codex/DSH
...
- **标签：** `desktop`, `agent-runtime`, `tauri`
- **推荐理由：** Rust驱动的高性能代理运行时，附带精致桌面应用。
...
- **标签：** `desktop`, `tauri`, `ui`
- **推荐理由：** 轻量级Tauri桌面客户端，5MB安装包，零环境配置。
...
- **标签：** `novel-writing`, `desktop`, `creative-writing`
- **推荐理由：** 桌面小说创作软件，支持角色、世界观和审稿工作流。
...
- **推荐理由：** 将表格、文档、演示文稿和关系表原生引入DSH办公环境。
...
- **标签：** `task-management`, `native-ui`, `sqlite`, `workflow`
- **推荐理由：** SQLite驱动的本地任务板，支持智能体认领与审核工作流。
...
- **标签：** `ios`, `mobile`, `native-app`, `kotlin`
- **推荐理由：** 原生iOS客户端，将DSH工作区和智能体轨迹带到iPhone。
...
- **标签：** `ios`, `simulator`, `automation`, `ui-automation`
- **推荐理由：** 在DSH对话内嵌实时iOS模拟器，提供22个智能体自动化工具。
...
- **标签：** `desktop`, `electron`, `cross-platform`, `ui`
- **推荐理由：** CodePilot风格跨平台桌面客户端和DSH插件套件。
...
- **标签：** `notes`, `desktop`, `docker`, `ui`
- **推荐理由：** 开源锤子便签，Docker部署支持skill调用和导出
...
- **标签：** `skill-panel`, `dsh-plugin`, `ui`
- **推荐理由：** DSH技能调用审计日志和应用内技能管理器
...
- **标签：** `desktop-app`, `electron`, `dsh-plugin`, `ui`
- **推荐理由：** Electron桌面控制台，多标签管理插件/MCP/技能
...
- **标签：** `workbench`, `plugin-system`, `dsh-plugin`, `ui`
- **推荐理由：** 基于DSH的插件化Web工作台，全局技能管理和工作区集成
...
- **标签：** `dsh-plugins`, `usage-tracking`, `web`
- **推荐理由：** 持久显示模型用量余额及峰谷计价的底部信息栏。
...
- **推荐理由：** 集成数据库、SSH、Docker和AI助手的DevOps桌面中心。
...
- **推荐理由：** 精选DSH插件：任务通知、LLM追踪、快捷键和内置终端。
...
- **标签：** `electron`, `desktop-app`, `dsh-web`
- **推荐理由：** Electron桌面壳集成DSH Web服务及ModLens等插件。
...
- **标签：** `dsh-plugin`, `ui`, `webui`
- **推荐理由：** 为DSH上下文面板添加一键紧凑模式按钮。
...
- **推荐理由：** LLM 知识库平台，将文档转化为可查询的 RAG 系统
...
- **推荐理由：** 自演化记忆系统，节省35%Token且支持混合检索
...
- **推荐理由：** 基于Milvus的统一持久化记忆层，支持多AI代理共享。
...
- **标签：** `memory`, `agent`, `personal-ai`
- **推荐理由：** 个人AI代理记忆中心，为所有AI代理提供持久化共享上下文。
...
- **推荐理由：** 零提示关联记忆，三层自动 Consolidation 提升上下文召回
...
- **标签：** `agentic-ai`, `multi-agent`, `skills`, `swarm`
- **推荐理由：** 部署多玩家智能体群体和自主工作流
...
- **标签：** `agent-skills`, `mcp-server`, `resume-builder`, `self-hosted`
- **推荐理由：** 隐私优先的开源简历生成器，支持 MCP
...
- **标签：** `ai-image-generation`, `prompt-as-code`, `skills`, `workflow-automation`
- **推荐理由：** 530+ GPT 图像提示词及工业级可复用技能模板
...
- **标签：** `agent-orchestration`, `agent-skills`, `workflow`
- **推荐理由：** 跨设备统一代理工作区，支持看板任务追踪
...
- **标签：** `agent-skills`, `skills-store`, `marketplace`
- **推荐理由：** 最全技能商店，自动同步GitHub上万个技能项目
...
- **标签：** `agent-control-plane`, `workflow-automation`
- **推荐理由：** 长周期代理控制面，支持跨平台持续工作流
...
- **标签：** `skills`, `workflow`, `optimization`
- **推荐理由：** 智能技能路由，Token节省30%同时提升奖励
...
- **标签：** `cognition`, `reasoning`, `inference`
- **推荐理由：** 推理时控制套件，支持深度推理验证与恢复
...
- **推荐理由：** AI代理自动生成Vox风格纸拼贴解说视频，端到端自动化。
...
- **标签：** `agent-observability`, `computer-vision`, `video-analysis`, `multimodal`
- **推荐理由：** 赋予AI视听能力，提供可验证的时间戳证据。
...
- **标签：** `creative-writing`, `novel-writing`, `drama-skills`, `workflows`
- **推荐理由：** 内置小说、短剧、游戏、视频解说四个创作工作台。
...
- **标签：** `agent-skills`, `math-modeling`, `gitops`, `deepseek-harness`
- **推荐理由：** 全栈数学建模自动化的GitOps流水线，支持国赛美赛
...
- **标签：** `agent-evolution`, `plan-mode`, `self-improvement`, `review`
- **推荐理由：** 可组合可审查的Agent工作流，实现Agent自我进化
...
- **标签：** `skills`, `agent-framework`, `claude-code`, `codex`
- **推荐理由：** 跨平台可复用Agent技能框架，支持Claude/Codex/GPT
...
- **标签：** `agent-skills`, `research`, `marketing`, `workflows`
- **推荐理由：** 88个开箱即用的研究营销业务工作流技能
...
- **推荐理由：** AI基础设施专用技能包，支持Claude Code协同开发
...
- **标签：** `upgrade`, `skill`, `dsh-plugin`
- **推荐理由：** 随DSH版本自动升级插件的实用技能
...
- **标签：** `skill-generator`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 自动将成功会话转化为可复用Agent技能
...
- **标签：** `skills`, `dsh-plugin`, `agent-skills`
- **推荐理由：** 25个开箱即用工程技能，侧边栏直接调用无需安装
...
- **标签：** `patent`, `legal-tech`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 专利侵权分析系统，输入公开号生成竞品报告
...
- **标签：** `legal`, `visualization`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 诉讼可视化工具集，生成时间轴、对抗图和关系图
...
- **标签：** `gongwen`, `docx`, `dsh-plugin`, `agent-skills`
- **推荐理由：** 国标公文全流程处理，格式检查、优化和模板生成
...
- **推荐理由：** 官方DSH插件开发知识库，含中英文档与Cordis集成。
...
- **标签：** `dsh-plugin`, `plugin-development`, `scaffold`
- **推荐理由：** 可复用插件开发验证技能，加速DSH插件构建流程。
...
- **标签：** `chinese-traditional-wisdom`, `dsh-plugin`, `dashboard`
- **推荐理由：** 融合八字风水中医的本地确定性AI Agent技能包。
...
- **标签：** `deepseek-harness-plugin`, `multi-agent`
- **推荐理由：** 多Agent协作框架，支持独立模型与共享工作区。
...
- **标签：** `delivery-gate`, `quality-gate`, `skill`
- **推荐理由：** 工程纪律技能包，严格验证需求与交付质量。
...
- **推荐理由：** 11个QA Agent技能+共享知识库，提升测试效率。
...
- **标签：** `multi-agent`, `session-memory`, `dsh-plugin`
- **推荐理由：** 持久化多Agent团队，跨会话保持私有记忆与技能。
...
- **标签：** `skill`, `code-simplification`, `codex`
- **推荐理由：** 源自DeepSeek Harness的代码简化技能
...
- **标签：** `math`, `modeling`, `competition`
- **推荐理由：** 5角色多Agent数学建模，含33篇模型库和模板
...
- **⭐ 星标：** 24
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少��学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math`, `modeling`, `chinese`
- **推荐理由：** 面向中文用户的完整数学建模技能集
...
- **⭐ 星标：** 12
- **语言：** Unknown
- **描述：** An Agent Skills skill for developing DeepSeek Harness (DSH) plugins（��发 DSH 插件的 Agent Skill）——插件/服务/事件/工具/LLM 适配器/打包安装的标准。Works with Claude Code, Codex, DSH, VS Code Copilot & any compatible agent.
- **标签：** `agent-skill`, `plugin-dev`, `deepseek-harness`
- **推荐理由：** 用于开发DSH插件的Agent技能，提供标准规范。
...
- **标签：** `agent-tools`, `pre-flight`, `capability-check`
- **推荐理由：** 任务前自动预检插件库，提升DSH agent成功率。
...
- **标签：** `agent-plugins`, `mcp-server`, `web-search`, `dsh-plugin`
- **推荐理由：** You.com网络搜索和内容提取工具，支持多平台AI智能体
...
- **标签：** `api-documentation`, `chinese-api`, `dsh-plugin`, `mcp`
- **推荐理由：** 零配置检索20个中国平台6.5万+API文档
...
- **推荐理由：** 华为云官方插件，含技能/MCP工具/安全护栏和云沙箱
...
- **标签：** `mcp`, `chatgpt`, `oauth`, `subscription`
...
- **标签：** `agent-skills`, `ai-design`, `design-systems`, `desktop-app`
- **推荐理由：** 开源设计工具，本地优先的桌面端 AI 设计替代方案
...
- **标签：** `agent-skills`, `architecture-diagram`, `diagrams-as-code`, `mermaid-alternative`
- **推荐理由：** 用代码生成精美架构图和序列图
...
- **标签：** `design`, `ui`, `vibecoding`
- **推荐理由：** 开源AI原生矢量设计工具，支持并发代理团队协作
...
- **标签：** `pixel-art`, `mascot`, `visual`
- **推荐理由：** 为各编码Agent添加可爱像素宠物，增添趣味性
...
- **标签：** `widget`, `visual`, `whale`
- **推荐理由：** 可爱漂浮鲸鱼挂件，动画展示账户余额
...
- **标签：** `browser-use`, `browser-automation`, `cli`
- **推荐理由：** 让AI代理无缝使用你已登录的真实浏览器进行操作
...
- **推荐理由：** 运行时注入器+任务感知路由，智能分流D SH插件请求
...
- **标签：** `multi-agent`, `switch`, `utility`
- **推荐理由：** 一键切换各大编码Agent及模型，无缝衔接
...
- **⭐ 星标：** 1,467
- **语言：** Python
- **描述：** DSH Plugin Radar — open-source ecosystem radar for DeepSeek Harness plugins: continuous discovery (21k+ candidates), k8s runtime validation (13k+ tests), 15-min snapshots; the catalog is a generated artifact — 开源 DSH 插��生态雷达：持续发现 2.1 万+ 候选、k8s 运行级实测 1.3 万+、15 分钟快照；插件目录为自动生成的产物
- **标签：** `plugin-registry`, `ecosystem`, `validation`
- **推荐理由：** 插件生态雷达，通过k8s运行时验证发现21k+插件。
...
- **标签：** `plugin-market`, `shop`, `ecosystem`
- **推荐理由：** 每日更新的全面插件市场，含预审核评价。
...
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅接入DSH作为LLM提供商。
...
- **标签：** `cost-tracking`, `token-usage`, `budget`, `infrastructure`
- **推荐理由：** 追踪DSH会话/每日费用、预算和各提供商用量。
...
- **标签：** `llm-provider`, `command-code`, `model-selection`
- **推荐理由：** 新增Command Code模型支持，含实时目录和智能选型。
...
- **标签：** `remote-access`, `p2p`, `infrastructure`, `encryption`
- **推荐理由：** 安全端到端加密的远程访问方案，支持多端操作Harness
...
- **标签：** `config`, `backup`, `restore`, `dsh-plugin`
- **推荐理由：** DSH配置备份恢复同步工具，支持WebDAV
...
- **标签：** `config`, `tool-management`, `dsh-plugin`
- **推荐理由：** 统一能力管理，支持运行时动态暴露策略
...
- **标签：** `plugin-management`, `dsh-plugin`, `ui`
- **推荐理由：** 插件生态基建，浏览器面板管理和开发引导
...
- **标签：** `awesome-list`, `dsh-plugins`, `utilities`
- **推荐理由：** 精选14类280+社区插件目录，直达仓库链接
...
- **标签：** `container`, `management`, `local`
- **推荐理由：** 本地容器隔离运行多版本DSH，管理便捷
...
- **标签：** `migration`, `claude`, `utility`
- **推荐理由：** 从Claude Code等工具迁移会话和技能到DSH
...
- **标签：** `launcher`, `orchestration`, `plugin`
- **推荐理由：** 统一DSH运行时、插件和技能编排能力
...
- **推荐理由：** 接入免费小米MiMo TTS API，支持多种声音选择。
...
- **标签：** `plugin-doctor`, `conflict-resolution`, `debugging`
- **推荐理由：** 解决插件间及版本间冲突并提供解决方案。
...
- **推荐理由：** 鲸鱼娘系列皮肤，为DeepSeek Harness增添视觉美感。
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层���单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
- **标签：** `dsh-plugin`, `webui`, `background`, `themes`
- **推荐理由：** 支持上传自定义背景或切换预设氛围，实时预览所见即所得。
...
- **标签：** `ai-resources`, `llm`, `rag`, `prompt-engineering`
- **推荐理由：** 全面的 AI 资源库，涵盖主流模型教程与知识库
...
- **标签：** `documentation`, `wiki`, `analysis`
- **推荐理由：** 从代码库生成结构化技术文档和AI可用上下文
...
- **推荐理由：** 自动搜索国内8大主流平台近30天内容，生成研究报告。
...
- **标签：** `academic-search`, `citation-verification`, `mcp`, `agent-skills`
- **推荐理由：** 跨库学术检索，支持引文核验与图谱，适合中文科研用户
...
- **标签：** `finance`, `stock`, `multi-agent`, `database`
- **推荐理由：** 本地财经数据库多Agent分析，支持微信飞书盯盘
...
- **标签：** `reverse-engineering`, `security`, `ctf`, `pentest`
- **推荐理由：** 87个逆向工程与安全渗透测试Skill，适合CTF
...
- **标签：** `research`, `industry-research`, `dsh-plugin`
- **推荐理由：** 行业研究方法论包，含政策新闻跟踪能力
...
- **标签：** `kline`, `finance`, `visualization`
- **推荐理由：** 交互式K线图分析，支持技术指标和多市场行情
...
- **⭐ 星标：** 405
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（���体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
...
- **推荐理由：** 🌊 DeepSeek Harness 海洋皮肤与动态主题 | Real-tim
...
- **标签：** `awesome-list`, `plugins`, `mcp`, `coding-agent`
- **推荐理由：** 全面的DSH插件、技能、MCP和补丁索引目录。
...
- **标签：** `awesome-list`, `guide`, `resources`, `quickstart`
- **推荐理由：** DSH终极指南，涵盖入门、资源、精选插件和实用工具。
...
- **标签：** `learning`, `marketplace`, `best-practices`, `guide`
- **推荐理由：** 学习DSH原理，探索插件市场并掌握最佳实践。
...
- **标签：** `marketplace`, `plugin-directory`, `catalog`, `awesome-list`
- **推荐理由：** 收录超1.1万插件的DSH插件市场，支持搜索和API
...
- **标签：** `compatibility-layer`, `migration`, `pi-agent`, `bridge`
- **推荐理由：** 打通Pi与DSH生态，原生运行Pi扩展的兼容层
...
- **标签：** `catalog`, `directory`, `plugin-marketplace`, `awesome-list`
- **推荐理由：** 1958个仓库的DSH插件总目录，中英文搜索分类
...
- **标签：** `plugin-discovery`, `plugin-manager`, `automation`
- **推荐理由：** 智能搜索安装验证DSH插件的一键Skill工具
...
- **标签：** `awesome-list`, `cordis`, `dsh-plugin`
- **推荐理由：** Cordis官方精选DSH插件自动生成目录
...
- **标签：** `api`, `documentation`, `official`
- **推荐理由：** 官方SkillHub API文档与示例
...
- **标签：** `awesome-list`, `comprehensive`, `resources`
- **推荐理由：** 全面的DeepSeek Harness插件资源清单
...
- **推荐理由：** DeepSeek Harness插件生态精选列表，方便浏览查找。
...
- **标签：** `awesome-list`, `dsh-plugin`, `guide`
- **推荐理由：** 按场景推荐插件的导航说明，含安装兼容性指南。
...
- **标签：** `dsh-plugin-market`, `marketplace`, `skills`
- **推荐理由：** 类CodeX风格的DSH插件市场，便于浏览安装。
...
- **推荐理由：** 每日通过GitHub Actions自动更新的插件目录。
...
- **标签：** `dsh-plugins`, `plugin-repository`, `awesome-list`
- **推荐理由：** DSH插件仓库与验证列表，方便查找和确认。
...
- **标签：** `awesome-list`, `dsh-plugins`, `curated-list`
- **推荐理由：** 精选DSH插件列表，已迁移至新仓库。
...
- **标签：** `awesome-list`, `dsh-plugins`, `auto-updating`
- **推荐理由：** 自动更新的 bilingual 插件列表，YAML驱动数据。
...
- **标签：** `plugin-directory`, `dsh-plugins`, `registry`, `awesome-list`
- **推荐理由：** 覆盖模型、工具、沙箱、UI的插件注册表。
