...
- **标签：** `dsh-plugin`, `multi-agent`, `mcp`
- **推荐理由：** 具备自进化记忆的多通道超级AI助手
...
- **标签：** `dsh-plugin`, `coding-agent`, `terminal`
...
- **标签：** `agent-os`, `mcp`, `skills`
- **推荐理由：** 自进化Agent OS，基于评估循环智能进化
...
- **推荐理由：** 首个DSH视觉插件，支持OCR和图像理解��增强文本智能体能力。
...
- **标签：** `preset`, `dsh-plugin`, `toolchain`
- **推荐理由：** 两阶段DSH配置方案：轻量启动后加载全套标准工具，部署便捷。
...
- **标签：** `multi-agent`, `model-switch`, `dsh-plugin`
- **推荐理由：** 一键安装并切换Claude Code/Codex/DSH/Kimi Code等多款编码Agent。
...
- **标签：** `runtime`, `ide`, `dsh-plugin`
- **推荐理由：** 高性能Rust Agent运行时搭配精美桌面IDE，专为编码Agent设计。
...
- **标签：** `code-review`, `static-analysis`, `refactoring`
...
- **标签：** `security`, `code-audit`, `cybersecurity`
- **推荐理由：** AI 代码安全审计工具，检测幻觉包和漏洞。
...
- **标签：** `plugin-development`, `agent-skill`, `dsh-plugin`, `llm-agent`
- **推荐理由：** 开发DSH插件的标准Agent技能，覆盖服务/事件/工具/打包。
...
- **标签：** `dsh-plugin`, `agent-skill`, `plugins`
- **推荐理由：** 管理已安装DSH插件的槽位、远端和凭据配置。
...
- **标签：** `dsh-plugin`, `codex`, `plugin-creator`
- **推荐理由：** Codex可复用技能，帮助构建和验证DSH/Cordis插件。
...
- **推荐理由：** 管理多版本DSH，容器隔离，内置WebView界面。
...
- **标签：** `package-manager`, `skills`, `tools`
- **推荐理由：** 42技能+5工具包，AI Agent资产综合管理器。
...
- **标签：** `agent-tools`, `ai-agents`, `cordis`, `llm-agents`
- **推荐理由：** 自动预检并注入Top-K插件提示，提升利用率
...
- **标签：** `dsh-plugin`, `electron`, `image-uploader`
...
- **推荐理由：** DeepSeek Harness插件生态的现代桌面客户端
...
- **标签：** `ui`, `mascot`, `pixel-art`
- **推荐理由：** Agent界面向动宠物画廊，增加趣味性
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/对话/Git等页面，支持三方扩展。
...
- **推荐理由：** 类Claude Code风格TUI插件，鲸鱼顶栏/流式思考/双击回滚功能。
...
- **推荐理由：** 在 DSH 对话中直接渲染交互 UI 组件，提升体验。
...
- **标签：** `desktop-app`, `multi-model`, `local-first`
- **推荐理由：** 本地优先多模型桌面客户端，支持自进化技能。
...
- **标签：** `desktop-app`, `windows`, `ssh-client`
- **推荐理由：** 开源 Windows 桌面客户端，内置 SSH 和远程访问。
...
- **标签：** `office`, `desktop`, `spreadsheet`
- **推荐理由：** 在DSH中集成完整办公套件，文档处理更高效
...
- **标签：** `ios`, `dsh-plugin`, `mobile`
- **推荐理由：** DSH原生iOS客户端，支持实时会话与Agent轨迹
...
- **⭐ 星标：** 115
- **语言：** JavaScript
- **描述：** DSH Web UI plugin: skill and MCP management（Web界面的skill/MCP管��工具）
- **标签：** `mcp`, `skills`, `dsh-plugin`
- **推荐理由：** Web界面可视化Skill与MCP管理工具
...
- **标签：** `desktop-app`, `electron`, `typescript`, `deepseek-harness`
- **推荐理由：** Electron桌面控制台，多Tab统一管理Harness插件、MCP和服务。
...
- **标签：** `dsh-plugin`, `skills-manager`, `deepseek-harness`
- **推荐理由：** DeepSeek Harness中统一加载和安全管理本机Agent技能的控件。
...
- **标签：** `ui`, `dsh-plugin`, `info-bar`
- **推荐理由：** 底部信息栏，实时显示模型、余额和定价。
...
- **推荐理由：** 一站式DevOps桌面：数据库/SSH/SFTP/Docker/AI沙箱。
...
- **推荐理由：** 专注Agent记忆管理套件，支持记忆存储、精炼与高效检索。
...
- **推荐理由：** 基于Markdown和Milvus的持久统一Agent记忆层，支持语义检索。
...
- **标签：** `skill-corpus`, `embeddings`, `vector-search`, `rag`
- **推荐理由：** 将 SKILL.md 转化为向量检索语料，支持 RAG 评估。
...
- **标签：** `knowledge-base`, `plugin-development`, `scaffold`, `cordis`
- **推荐理由：** 官方DSH插件开发文档知识包，中英双语，按需调用。
...
- **标签：** `context`, `audit`, `dsh-plugin`
- **推荐理由：** 审计上下文token成本，检测指令冲突。
...
- **标签：** `dsh-plugin`, `multi-agent`, `swarm`
- **推荐理由：** 多智能体蜂群框架，支持自主协调工作流
...
- **标签：** `dsh-plugin`, `agent-skills`, `knowledge-distillation`
...
- **标签：** `skills`, `dsh-plugin`, `marketplace`
- **推荐理由：** 最全技能商店，自动抓取整理万个Skill包
...
- **标签：** `agent-control`, `workflow`, `dsh-plugin`
- **推荐理由：** 长时间跨度Agent控制面，支持持久化工作流
...
- **标签：** `agent-workspace`, `enterprise`, `skills`
- **推荐理由：** 企业级多Agent工作台，支持可视化编辑
...
- **推荐理由：** 智能技能路由与工作流编排，SkillsBench实验提升21.12%奖励。
...
- **推荐理由：** 支持DeepSeek Harness中多代理团队协作
...
- **推荐理由：** SQLite 本地任务管理，支持 Agent 认领与审查。
...
- **标签：** `math-modeling`, `skill`, `workflow`
- **推荐理由：** 全栈数学建模自动化工具，支持竞赛GitOps流程
...
- **标签：** `finance`, `stock`, `multi-agent`
- **推荐理由：** AI原生多Agent金融分析系统，支持IM盯盘推送
...
- **标签：** `agent`, `skills`, `wayfinder`, `github-issues`, `dsh-better-sidebar`
- **推荐理由：** 一键安装25个工程技能面板，含GitHub Issue管理支持。
...
- **标签：** `spec-programming`, `vibe-coding`, `agent-skills`, `developer-tools`
- **推荐理由：** 仓库级规格驱动开发技能，支持vibe coding和Spec编程。
...
- **标签：** `chinese-traditional-wisdom`, `fengshui`, `bazi`, `iching`, `tcm`
- **推荐理由：** 八字/六爻/风水/中医等传统智慧AI技能，本地确定性引擎+可视化面板。
...
- **标签：** `multi-agent`, `deepseek-harness`, `collaboration`
- **推荐理由：** 多Agent协作，共享工作区，独立配置模型和工具。
...
- **推荐理由：** Godot 4.x游戏开发技能，运行时注册，适合DSH智能体。
...
- **标签：** `skillhub`, `api`, `documentation`
- **推荐理由：** 官方SkillHub API文档与DSH集成示例。
...
- **推荐理由：** 版本固定的Swift Cycle治理技能，支持离线验证。
...
- **标签：** `huaweicloud`, `mcp`, `sdk`, `terraform`, `ai-coding`
- **推荐理由：** 华为云官方MCP插件，含SDK、云沙箱和安全护栏能力。
...
- **标签：** `ocr`, `image-reading`, `vision`, `pixel-art`, `dsh-plugin`
- **推荐理由：** 像素级图像识别工具集，让纯文本模型具备视觉能力。
...
- **标签：** `dsh-plugin`, `diagram-as-code`, `architecture`
- **推荐理由：** 从代码生成精美架构图和序列图
...
- **标签：** `design`, `agent-team`, `ui`
- **推荐理由：** 首个AI原生矢量设计工具，支持Agent团队协作
...
- **标签：** `video-generation`, `tts`, `motion-graphics`
- **推荐理由：** 一键生成纸拼贴风格解说视频，全流程自动化
...
- **标签：** `visualization`, `litigation`, `legal`, `svg`, `timeline`
- **推荐理由：** 将诉讼材料重画为可提交的时间轴和可视化图表，零依赖。
...
- **标签：** `design`, `ui`, `aesthetics`
- **推荐理由：** 6种设计风格包，提升AI生成网站美观度。
...
- **推荐理由：** AI视频/图像/音频技能，支持多模型配置，适合内容创作。
...
- **推荐理由：** 面向Gemini/Claude/ChatGPT的增强套件与提示管理
...
- **标签：** `content-discovery`, `browser`, `dsh-plugin`
- **推荐理由：** 跨平台内容发现Agent，本地私有关于B站/YouTube/知乎等多平台抓取。
...
- **标签：** `web-search`, `browser`, `free`
- **推荐理由：** 免费网页搜索插件，无需注册，支持所有模型。
...
- **推荐理由：** 在对话中运行 iOS 模拟器，支持 22 项自动化操作。
...
- **标签：** `virtual-terminal`, `vfs`, `dsh-plugin`
- **推荐理由：** 全球首个AI智能体虚拟终端，支持VFS虚拟文件系统。
...
- **推荐理由：** DSH内置可视化插件市场，支持浏览、搜索和一键安装插件。
...
- **标签：** `ecosystem`, `plugin-registry`, `infrastructure`
- **推荐理由：** 可自部署的插件生态雷达，自动发现与实测管线
...
- **标签：** `context-management`, `dashboard`, `utilities`
- **推荐理由：** 提供上下文仪表盘与统计分析的管理工具
...
- **标签：** `oauth`, `subscriptions`, `llm-provider`
- **推荐理由：** 通过 OAuth 将订阅账号用作 DSH LLM 提供商。
...
- **标签：** `cli`, `plugin-manager`, `utility`
- **推荐理由：** 高效的DSH插���Hub命令行管理工具
...
- **标签：** `gongwen`, `docx`, `wps-office`, `ai-docs`
- **推荐理由：** GB/T 9704标准中文公文全流程处理，格式检查/模板/修订增强。
...
- **标签：** `token-usage`, `cost`, `monitoring`
- **推荐理由：** 实时Token用量、成本估算和API余额监控插件。
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin-market`, `dsh-plugins`
- **推荐理由：** DSH插件市场整合包，便于发现和安装插件
...
- **标签：** `chatgpt`, `codex`, `deepseek-harness`, `oauth`
...
- **推荐理由：** 鲸鱼娘系列主题皮肤，为DeepSeek Harness提供个性化视觉外观。
...
- **标签：** `theme`, `glass-morphism`, `customizable`
- **推荐理由：** 毛玻璃质感高自由度主题，可自定义模糊度和背景。
...
- **标签：** `theme`, `skin`, `webgpu`
- **推荐理由：** 基于WebGPU的海洋主题皮肤，视觉效果出色
...
- **标签：** `guide`, `awesome-list`, `reference`
- **推荐理由：** DSH全面入门指南，含精选插件与资源
...
- **标签：** `marketplace`, `catalog`, `reference`
- **推荐理由：** 超万款插件市场，支持搜索排序与API访问
...
- **标签：** `catalog`, `directory`, `reference`
- **推荐理由：** 完整DSH插件目录，中英双语搜索分类
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、实验追踪与GPU管理
...
- **标签：** `academic-search`, `mcp`, `research-workflow`
- **推荐理由：** 多源学术检索，支持引文核验与图谱构建
...
- **标签：** `company-research`, `industry-research`, `report`, `cordis`
- **推荐理由：** 企业调研与产业链分析方法论技能包，支持政策新闻追踪。
...
- **标签：** `finance`, `backtesting`, `investment`
- **推荐理由：** AI投资研究台，多策略回测分析技能包。
...
- **标签：** `dsh-plugin`, `image-generation`, `skills`
- **推荐理由：** 530+工业级图像生成提示词和可复用技能
...
- **标签：** `dsh-plugin`, `low-code`, `workflows`
...
- **标签：** `armor-breaking`, `deepseek`
- **推荐理由：** DeepSeek专用破甲插件，突破生成限制
...
- **推荐理由：** 中英双语 curated 插件列表，适合新手快速上手 DSH 生���。
...
- **标签：** `encyclopedia`, `mcp`, `rag`, `skills`
- **推荐理由：** AI 全维度资源百科，涵盖技能、MCP、RAG 等。
...
- **标签：** `awesome-list`, `discovery`, `plugin-curator`
- **推荐理由：** 每日自动抓取筛选，场景化分类推荐优质插件。
...
- **⭐ 星标：** 164
- **语言：** TypeScript
- **描述：** DeepSeek Harness 搜番插件：对话内多源搜索番剧，卡片展示 Bangumi 评分与详情，��持复制磁力。
- **标签：** `anime-search`, `dsh-plugin`, `bangumi`
- **推荐理由：** 对话内搜番插件，支持多源检索与磁力复制
...
- **推荐理由：** 精选14类280+社区插件目录，覆盖MCP/Skill/UI皮肤等。
...
- **标签：** `directory`, `awesome-list`, `discovery`
- **推荐理由：** 公开GitHub目录，含安装命令和生态发现功能。
...
- **标签：** `dsh-bundle`, `dsh-ecosystem`, `dsh-plugin-verify`
- **推荐理由：** 经过验证的DSH插件目录，含插件核验功能
...
- **标签：** `ai-agents`, `awesome-list`, `deepseek`
...
- **推荐理由：** 证据支持的DSH插件雷达与2Origin插件实验室
...
- **标签：** `ai-agents`, `awesome`, `awesome-list`
- **推荐理由：** 按功能分类的80+已验证DSH插件目录
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugins`
- **推荐理由：** 数据驱动自动更新的插件列表，双语README
