...
- **标签：** `coding-agent`, `deepseek`, `mcp`, `dsh-plugin`
- **推荐理由：** DeepSeek原生终端编码Agent，优化前缀缓存稳定性
...
- **标签：** `routing`, `cog-stack`, `dsh-plugin`
- **推荐理由：** 任务感知推理模式路由器，优化DSH工作流
...
- **推荐理由：** 支持子Agent、RPC插件、哈希行编辑和MCP集成的编码Agent。
...
- **标签：** `plugin-creator`, `scaffold`, `cordis`
- **推荐理由：** 从零构建和验证DSH/Cordis插件的Codex脚手架技能。
...
- **标签：** `agent-skill`, `plugin-dev`, `coding-agent`
- **推荐理由：** 用于开发DSH插件的Agent Skill，兼容Claude Code/Codex
...
- **标签：** `dsh`, `desktop`, `dsh-plugin`, `cordis-plugin`
- **推荐理由：** DSH生态现代化桌面端，桌面本身也是插件
...
- **标签：** `low-code`, `ai-agent`, `crm`, `dsh-plugin`
...
- **推荐理由：** DeepSeek Harness原生桌面客户端，增强用户体验
...
- **推荐理由：** 开放侧边栏底座，内置文件/终端/Git/子代理页面
...
- **推荐理由：** 零配置Electron桌面GUI，内置SSH、插件和移动端远程访问。
...
- **推荐理由：** 本地优先的AI桌面工作区，支持DSH会话、项目、文件和Web研究。
...
- **推荐理由：** 在回复中内联渲染交互UI组件：布局、图表、表单和测验。
...
- **标签：** `conversation`, `canvas`, `session-explorer`, `visual`
- **推荐理由：** 可视化非线性对话工作区，支持画布式会话探索和分支导航。
...
- **推荐理由：** 将表格、文档、幻灯片等办公能力内置到 Harness。
...
- **推荐理由：** 原生 iOS 客户端，通过 WebSocket 接入 Harness 工作区。
...
- **推荐理由：** 跨平台 CodePilot 风格桌面客户端与插件套件。
...
- **标签：** `workbench`, `plugin-system`, `platform`
- **推荐理由：** 插件化Web工作平台，全局技能管理
...
- **标签：** `desktop`, `electron`, `management`, `ui`
- **推荐理由：** 桌面管理控制台，统一管理Harness/插件/MCP
...
- **推荐理由：** 替换composer统计栏，显示provider、余额和价格信息。
...
- **推荐理由：** 一体化DevOps桌面：SSH/SFTP/Docker/数据库/AI
...
- **推荐理由：** 自演进上下文数据库，统一Agent记忆与RAG知识
...
- **推荐理由：** 开源RAG知识平台，含推理Agent与自维护Wiki
...
- **推荐理由：** 自进化记忆系统，混合检索可节省35%Token消耗
...
- **标签：** `deepwiki`, `dsh-plugin`, `rust`
- **推荐理由：** 从代码快速生成结构化技术文档和AI可用上下文。
...
- **推荐理由：** 基于Milvus的持久统一AI记忆层，支持语义搜索。
...
- **标签：** `agent-memory`, `vector-search`, `skill-md`, `embeddings`
- **推荐理由：** 将分散的SKILL.md文件整理为可检索的Agent技能语料库，支持向量搜索。
...
- **标签：** `skills-store`, `agent-skills`, `marketplace`
- **推荐理由：** 全面的技能商店，覆盖编程、写作、金融和自动化
...
- **标签：** `multi-agent`, `enterprise`, `workspace`
- **推荐理由：** 企业级本地优先智能体工作台，支持多引擎协作
...
- **标签：** `agent-harness`, `multi-agent`, `workflow-automation`
- **推荐理由：** 持久状态控制平面，支持长期多Agent协作工作流
...
- **标签：** `llm-agent`, `preset`, `deepseek-harness`
- **推荐理由：** 两阶段引导预设，对齐Project2标准工具集
...
- **推荐理由：** 为DSH引入认识论循环框架，支持结构化科学发现和Agent推理。
...
- **标签：** `creative-writing`, `novel-writing`, `ai-writing`, `screenwriting`
- **推荐理由：** 网文/小说创作工作流插件，内置小说、短剧、游戏和视频解说工作台。
...
- **标签：** `agent-skills`, `multimodal`, `ocr`
- **推荐理由：** 为 Agent 提供视频音频 OCR 能力，证据可核验。
...
- **推荐理由：** 本地 SQLite 任务看板，支持 Agent 认领与审核。
...
- **标签：** `upgrade`, `plugin`, `skill`
- **推荐理由：** 随DSH版本自动升级插件的便捷技能
...
- **推荐理由：** 提供TDD、调试和规划技能，源自superpowers
...
- **标签：** `skills`, `mcp`, `capability-management`
...
- **标签：** `mentor`, `education`, `code-review`
- **推荐理由：** AI编程导师，引导式学习+自动判题+代码审查
...
- **标签：** `gongwen`, `docx`, `wps-office`, `skills`
- **推荐理由：** 公文全流程处理，符合国标GB/T 9704
...
- **推荐理由：** 八字/风水/中医等传统智慧融合为本地确定性AI技能。
...
- **推荐理由：** 基于DSH模式的规格驱动开发技能，跨Agent可用。
...
- **标签：** `multi-agent`, `agent-orchestration`, `dsh-plugins`
- **推荐理由：** 多Agent协作，支持独立模型、技能和共享工作区。
...
- **标签：** `video`, `motion-design`, `agent-skills`
...
- **推荐理由：** QA工程技能框架：11项技能+共享知识库用于AI测试。
...
- **推荐理由：** WorkBuddy风格技能选择器，支持命令快速插入
...
- **推荐理由：** 免费小米MiMo TTS集成，支持自定义和浏览器语音
...
- **标签：** `workflow`, `task-management`, `deepseek-harness`
- **推荐理由：** 提供结构化任务管理、验证和提交工作流。
...
- **推荐理由：** Agent���具的OpenRouter，社区驱动的API密钥注册
...
- **标签：** `mcp`, `web-search`, `research`, `agent-skills`
- **推荐理由：** 提供网页搜索、内容提取和研究技能
...
- **推荐理由：** 最强免费Web搜索插件，无需注册和API密钥，支持所有模型。
...
- **推荐理由：** 浏览器自动化插件，支持引擎级指纹伪装和无限制本地配置文件。
...
- **推荐理由：** 22 个 Agent 工具，会话内操控 iOS 模拟器与真机。
...
- **标签：** `electron`, `image`, `dsh-plugin`, `cloud-storage`
- **推荐理由：** 终极图片上传工具，支持60+图床服务包括S3
...
- **标签：** `cli`, `api-keys`, `credentials`
- **推荐理由：** 多模型账户管理器，支持智能路由和自动容灾
...
- **推荐理由：** DSH插件Hub的CLI工具和解析器，从命令行管理发现插件。
...
- **推荐理由：** OAuth 登录接入 ChatGPT/Claude/Grok 订阅，免 API Key。
...
- **推荐理由：** 接入 Command Code 模型，支持计划感知选型。
...
- **标签：** `migration`, `sessions`
- **推荐理由：** 从25+AI编码代理导入会话到DSH
...
- **推荐理由：** 一键配置多款AI Agent工具的MCP和技能规则
...
- **标签：** `migration`, `dsh-plugin`, `session-import`
- **推荐理由：** 四源迁移向导，轻松从Claude/Codex等迁移
...
- **⭐ 星标：** 405
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全���能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `glassmorphism`, `deepseek-harness`
- **推荐理由：** 磨砂玻璃主题，全参数可调，不改动源码。
...
- **标签：** `ai`, `rag`, `prompt-engineering`, `dsh-plugin`
- **推荐理由：** 全面AI资源库，含教程、提示词和模型指南
...
- **推荐理由：** 双语精选DSH插件、扩展、工具和技能的 awesome 资源导航列表。
...
- **标签：** `arxiv`, `latex`, `research`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、实验追踪和GPU SSH编排。
...
- **推荐理由：** 全面AI资源百科，涵盖模型、Agent、RAG、多模态工具和 learning 路线。
...
- **标签：** `awesome-list`, `dsh-plugin-market`
- **推荐理由：** 11,000+插件市场，支持搜索和免费API
...
- **标签：** `awesome-list`, `agent-runtime`, `mcp`
- **推荐理由：** 173页源码支持的全面DSH手册
...
- **标签：** `catalog`, `plugin-catalog`
- **推荐理由：** 1958个仓库的完整DSH插件目录
...
- **标签：** `patent`, `rag`, `legal-tech`, `analysis`
...
- **标签：** `stock-analysis`, `technical-analysis`, `kline`
- **推荐理由：** 多市场行情K线分析，支持技术指标与基本面
...
- **标签：** `stock-analysis`, `quantitative-trading`, `backtesting`
- **推荐理由：** 39项工具+20策略，覆盖多市场量化分析
...
- **标签：** `awesome-list`, `coding-agent`, `mcp`
- **推荐理由：** 覆盖所有DSH插件和资源的全面精选列表
...
- **标签：** `awesome-list`, `cli`, `web-ui`, `agent`
- **推荐理由：** 涵盖CLI、Web UI和Agent的广泛插件生态列表
...
- **标签：** `recommender`, `ai-curved`, `awesome-list`
- **推荐理由：** AI推荐190+插件，按12类实时更新
...
- **标签：** `awesome-list`, `dsh-plugin`, `resource`
- **推荐理由：** DeepSeek Harness插件精选目录，便于发现资源
...
- **⭐ 星标：** 2,253
- **语言：** JavaScript
- **描述：** DeepSeek Harness 破甲：让所有模型都能破甲，不同模型可换不同提示词；默认提示词面向国模「小码酱」。Jailbreak for every model — swap prompts per model. 求 Star ��藏 ⭐
- **标签：** `jailbreak`, `dsh-plugin`, `llm`
- **推荐理由：** 模型破甲插件，支持自定义提示词突破限制。
...
- **标签：** `writing`, `ai-novel`, `long-form-fiction`
- **推荐理由：** AI辅助小说创作，含世界观、角色和章节工作流。
...
- **标签：** `cybersecurity`, `ai-security`, `vulnerabilities`, `redteam`
...
- **推荐理由：** DeepSeek Harness 终极指南：入门、插件与工具汇总。
...
- **推荐理由：** 精选280+社区插件目录，覆盖14类，链接直达仓库。
