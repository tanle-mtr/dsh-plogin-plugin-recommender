...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 231,077 | Core platform enabling everything is a plugin arch |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,265 | Open-source Claude Design alternative with local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 72,919 | Deploy multi-player swarms and coordinate autonomo |
...
- **推荐理由：** DeepSeek原生终端编程智能体，前缀缓存稳定运行
...
- **标签：** `agent-orchestration`, `coding-agent`, `mcp`
- **推荐理由：** 统一多平台AI代理编排与任务看板
...
- **标签：** `vision`, `ocr`, `computer-vision`, `toolkit`
- **推荐理由：** 为纯文本模型打造强大视觉工具包，支持OCR与图像问答
...
- **标签：** `coding-agent`, `rpc`, `mcp`, `tui`
- **推荐理由：** 带RPC插件系统的编码agent，支持子agent、hashline编辑和MCP。
...
- **标签：** `cli`, `plugin-manager`, `hub`, `typescript`
- **推荐理由：** DSH插件市场开源CLI和管理工具，含schema定义和解析器。
...
- **推荐理由：** 适配Reasonix编程框架的Superpowers技能
...
- **标签：** `bioinformatics`, `desktop-app`, `llm-agent`, `dsh-plugin`
- **推荐理由：** 面向临床的生物信息学Agent桌面，支持离线和技能商店。
...
- **标签：** `agent-tools`, `pre-flight`, `cordis`
- **推荐理由：** DSH代理插件预检工具
...
- **标签：** `deepseek-harness`, `desktop`, `dsh-plugin`
...
- **标签：** `sidebar`, `extension`, `terminal`
- **推荐理由：** 开放侧边栏底座，支持终端/Git/子代理扩展。
...
- **标签：** `taskboard`, `skills`, `cli`
- **推荐理由：** 现代化可嵌入任务面板，支持灵活扩展。
...
- **标签：** `tui`, `terminal`, `react`
- **推荐理由：** 鲸鱼顶栏、流式思考、上下文进度追踪的TUI。
...
- **⭐ 星标：** 2,806
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直��喊来你的dsh安装
- **标签：** `widget`, `ui`, `balance`
- **推荐理由：** 可爱鲸鱼悬浮窗，实时监控账户余额与动画交互
...
- **标签：** `desktop`, `tauri`, `installation`
- **推荐理由：** 轻量级Tauri桌面版，零配置开箱即用，预装插件
...
- **推荐理由：** 零配置Windows桌面GUI客户端，集成Codex、插件、SSH和移动端远程访问。
...
- **标签：** `desktop`, `local-first`, `mcp`, `react`
- **推荐理由：** 本地优先的DSH桌面工作区，支持会话、项目、文件和MCP集成。
...
- **标签：** `canvas`, `conversation`, `visualization`, `workspace`
- **推荐理由：** 基于画布的可视化非线性对话工作区，支持分支会话探索。
...
- **标签：** `desktop-app`, `agent`, `multi-model`
- **推荐理由：** 本地优先的多模型AI桌面助手，注重隐私安全
...
- **标签：** `office`, `productivity`, `spreadsheet`
- **推荐理由：** 在DSH中集成电子表格、文档、演示等办公工具
...
- **推荐理由：** 原生iOS客户端，WebSocket连接DSH移动访问
...
- **标签：** `mobile`, `ios-simulator`, `automation`
- **推荐理由：** 在对话中直接使用iOS模拟器和真机控制
...
- **标签：** `desktop`, `cross-platform`, `electron`
- **推荐理由：** 跨平台CodePilot风格桌面客户端支持DSH
...
- **标签：** `skill-panel`, `audit`, `ui-extension`
- **推荐理由：** 技能调用审计+应用内skill管理器，可视化操作
...
- **标签：** `ui`, `plugin-market`, `webgui`
- **推荐理由：** DSH Web界面社区插件市场，一键安装
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `desktop`, `electron`
- **推荐理由：** Electron桌面管理控制台，多Tab统一管理Harness、插件、MCP和技能。
...
- **标签：** `ui-extension`, `usage-tracking`, `dsh-plugin`
- **推荐理由：** 底部信息栏展示模型/余额/峰谷定价实时状态。
...
- **推荐理由：** 一体化DevOps桌面：数据库/SSH/Docker/AI工具
...
- **标签：** `keybindings`, `terminal`, `trace`, `dsh-web`
...
- **标签：** `desktop-app`, `electron`, `dsh`, `harness`
- **推荐理由：** Electron desktop shell integrating DSH web services and tools
...
- **标签：** `memory`, `rag`, `memory-management`
- **推荐理由：** 全面记忆管理套件，让智能体持续学习和积累。
...
- **标签：** `documentation`, `deepwiki`, `code-to-text`
- **推荐理由：** 分钟级生成结构化技术文档和AI就绪上下文。
...
- **标签：** `memory`, `rag`, `milvus`, `embedding`
- **推荐理由：** 基于Milvus的统一AI持久记忆层，支持跨Agent共享
...
- **推荐理由：** 个人AI记忆中枢，为所有Agent提供共享持久记忆
...
- **标签：** `skill-corpus`, `embeddings`, `reranking`, `semantic-search`
- **推荐理由：** 将SKILL.md文件转化为可检索的agent技能语料库，支持嵌入和重排序。
...
- **标签：** `knowledge-base`, `rag`, `mcp`, `resource-catalog`
- **推荐理由：** 全面AI知识库百科，涵盖技能、RAG、多模态和AI应用工具资源。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `memory`, `context`
- **推荐理由：** 主动联想记忆系统，三层自动巩固，模型响应前零提示召回关键信息。
...
- **标签：** `agentic-workflow`, `multi-agent`, `swarm-intelligence`
...
- **推荐理由：** Distill thinking patterns into reusable skills for any agent
...
- **标签：** `agent-skills`, `skills-store`
- **推荐理由：** 最全面的技能市场，覆盖开发到自动化
...
- **标签：** `cognitive-enhancement`, `inference-time`, `global-workspace`
- **推荐理由：** 模型无关的推理时控制，支持深度推理与自恢复。
...
- **标签：** `video`, `agent-skill`, `ffmpeg`, `ai-video`
- **推荐理由：** 自动化Vox风格拼贴视频生成全流程Agent技能
...
- **标签：** `agent-teams`, `workflow`, `multi-agent`
- **推荐理由：** 在DSH中实现多Agent团队协作工作流
...
- **标签：** `writing`, `novel`, `creative`, `rag`
- **推荐理由：** 结构化AI小说创作流程，支持角色与世界观管理
...
- **标签：** `creative-writing`, `novel`, `short-drama`, `workflow`
- **推荐理由：** 创作型写作工作流插件，内置网文、短剧、���戏和视频解说工作台。
...
- **标签：** `multimodal`, `computer-vision`, `agent-observability`
- **推荐理由：** 赋予AI视觉听觉能力，视频音频屏幕观测验证
...
- **标签：** `agent-skills`, `math-modeling`, `workflow`
- **推荐理由：** 全栈数学建模竞赛Skill，GitOps流水线+代码自证
...
- **标签：** `epistemic-loop`, `agent-preset`, `scientific-discovery`
- **推荐理由：** 将认知循环引入DSH，增强科学发现推理能力
...
- **标签：** `agent-skills`, `research-automation`, `marketing`
- **推荐理由：** 88个开源Agent技能，覆盖研究/营销/商业自动化
...
- **推荐理由：** 完整的逆向工程与渗透测试技能包，87个SKILL.md
...
- **标签：** `ai-infra`, `skills`, `infrastructure`
...
- **标签：** `agent-learning`, `skill-generator`, `automation`
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `coding-agent`, `skill`
- **推荐理由：** AI编程导师技能，扫描项目架构并生成阶梯式章节，辅助手写核心逻辑与Code Review。
...
- **标签：** `agent-skills`, `web-search`, `multi-platform`
- **推荐理由：** 覆盖网页搜索、研究与金融的通用技能集，支持多平台AI Agent接入。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skills-deck`, `engineering`
- **推荐理由：** 内置25个工程效率技能，面板直接调用，支持GitHub Issue工作流。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `patent`, `legal-tech`, `skill`
- **推荐理由：** 专利侵权分析技能，输入专利号即生成竞品侵权分析报告。
...
- **⭐ 星标：** 59
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核��、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `deepseek-harness`, `dsh-plugin`, `gongwen`, `document`, `skill`
- **推荐理由：** 符合国标党政机关公文格式的全流程处理技能，支持排版检查与模板生成。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `workbench`, `skills`, `platform`
- **推荐理由：** 基于DSH的插件化Web工作台，全局技能管理与原生工作区集成。
...
- **⭐ 星标：** 40
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质全��合，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
- **标签：** `chinese-traditional-wisdom`, `agent-skill`, `dsh-plugin`
- **推荐理由：** 中华传统智慧AI技能包，涵盖八字/风水/中医等。
...
- **标签：** `spec-driven-development`, `agent-skill`, `dsh-plugin`
- **推荐理由：** 可移植的规范驱动开发技能，支持仓库级原生编程。
...
- **标签：** `multi-agent`, `agent-team`, `dsh-plugin`
- **推荐理由：** 多Agent团队协作框架，支持共享工作空间和独立技能。
...
- **标签：** `multi-agent`, `agent-orchestration`, `dsh-plugin`
- **推荐理由：** 跨会话持久化记忆和技能的 durable 多Agent系统。
...
- **标签：** `qa`, `testing`, `agent-skill`, `dsh-plugin`
- **推荐理由：** 11个QA Agent技能加共享知识库和类型决策矩阵。
...
- **标签：** `agent-skills`, `session-memory`, `handoff`
- **推荐理由：** 跨平台Agent交接记忆，保持任务连续性
...
- **⭐ 星标：** 28
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、���重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math-modeling`, `writing`, `chinese`
- **推荐理由：** 完整中文数模资源，含降AIGC和查重
...
- **推荐理由：** 开源Claude Design替代品，本地优先桌面设计工具
...
- **标签：** `design`, `vibecoding`, `flutter`, `react`
...
- **标签：** `uikit`, `charts`, `interactive`, `generative-ui`
- **推荐理由：** 生成可交互UI组件（图表、表单、测验），在助手回复中内联渲染。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `legal`, `visualization`, `diagram`
- **推荐理由：** 法律人诉讼可视化工具，自动生成时间轴、庭审对抗图和法律关系图。
...
- **标签：** `browser-use`, `browser-automation`
- **推荐理由：** 让AI代理自动化操作已登录的真实浏览器
...
- **推荐理由：** 最强免费网页搜索插件，无需注册和API密钥，为所有模型提供网络接入。
...
- **标签：** `anti-detect`, `browser`, `fingerprint`, `playwright`
- **推荐理由：** 为agent提供持久指纹身份的浏览器，支持无限本地配置和Android模拟。
...
- **标签：** `low-code`, `no-code`, `business-tools`
...
- **标签：** `preset`, `tools`, `bootstrap`
- **推荐理由：** 两阶段预设，从最小引导到完整标准工具链。
...
- **⭐ 星标：** 1,465
- **语言：** Python
- **描述：** DSH Plugin Radar — open-source ecosystem radar for DeepSeek Harness plugins: continuous discovery (21k+ candidates), k8s runtime validation (13k+ tests), 15-min snapshots; the catalog is a generated artifact — 开源 DSH 插件���态雷达：持续发现 2.1 万+ 候选、k8s 运行级实测 1.3 万+、15 分钟快照；插件目录为自动生成的产物
- **标签：** `plugin-discovery`, `validation`, `ecosystem`
- **推荐理由：** 持续发现并验证2万+插件的生态雷达工具
...
- **标签：** `security`, `audit`, `vulnerability`, `redteam`
- **推荐理由：** AI安全攻防平台，审计AI生成代码中的幻觉包和缺失防护。
...
- **推荐理由：** 通过OAuth接入ChatGPT Claude Grok订阅作为LLM
...
- **推荐理由：** 添加Command Code模型访问和智能计划选择
...
- **标签：** `backup`, `config-manager`, `sync`, `migration`
- **推荐理由：** DSH配置备份恢��与迁移同步工具
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `tool-management`
- **推荐理由：** 灵活管理工具暴露策略，减少上下文占用，支持动态发现与执行。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `skills`, `management`
- **推荐理由：** 统一加载并安全管理本机Agent Skills，是DSH技能生态的基础设施。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `plugin-management`, `infrastructure`
- **推荐理由：** DSH插件生态基建，提供官方仓库浏览器控制台与插件开发引导技能。
...
- **标签：** `context-audit`, `utility`, `dsh-plugin`
- **推荐理由：** 上下文审计工具，检测重复冲突，Web UI可视化。
...
- **标签：** `launcher`, `orchestration`, `core`
- **推荐理由：** 核心运行时编排器，统一插件与MCP
...
- **标签：** `migration`, `session-import`, `tool`
- **推荐理由：** 从Claude/Codex迁移会话和技能的一键向导
...
- **标签：** `troubleshooting`, `plugin`, `health-check`
- **推荐理由：** 诊断并解决DSH插件冲突与版本问题
...
- **标签：** `theme`, `glassmorphism`, `ui`, `customizable`
- **推荐理由：** 高度可定制的毛玻璃风格主题，支持调节模糊度和自定义壁纸。
...
- **标签：** `theme`, `animation`, `customization`
- **推荐理由：** 动态海洋主题皮肤，支持自定义波浪和透明度
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地���理不上传任何服务器，关闭开关或一键恢复默认即可完全移除；内置中英文双语界面。
...
- **标签：** `ai-resource`, `vibe-coding`, `prompt-collection`
- **推荐理由：** 全面的AI资源大全及提示词教程合集
...
- **推荐理由：** 一站式科研工作台：LaTeX实时编译、arXiv文献管理、实验追踪和GPU SSH编排。
...
- **标签：** `financial`, `multi-agent`, `research`
- **推荐理由：** 基于Tushare的本地财经数据库+多Agent金融分析
...
- **标签：** `research`, `industry-research`, `report`
- **推荐理由：** 行业研究方法论与产业链分析技能包
...
- **标签：** `plugin-registry`, `skills`, `curated-list`
- **推荐理由：** 过滤垃圾的开源DSH插件注册表
...
- **标签：** `awesome`, `awesome-list`, `deepseek-harness`, `dsh-plugins`, `plugins`
- **推荐理由：** 按功能组织的80+验证DSH插件
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `dsh-plugins`, `dsh-ecosystem`
- **推荐理由：** 支持验证的DSH插件仓库
...
- **标签：** `deepseek-harness`, `dsh`, `awesome-list`, `dsh-plugins`
- **推荐理由：** 自动更新的DSH插件精选列表，含脚本支持
...
- **标签：** `agent-harness`, `curated-list`, `deepseek-harness`, `deepseek-harness-plugin`, `dsh-plugin`, `registry`, `skills`
- **推荐理由：** 涵盖模型、工具、UI的精选DSH插件集
...
- **推荐理由：** 双语精选DSH插件资源列表，涵盖工具、技能和社区参考。
...
- **⭐ 星标：** 348
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目��除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `directory`, `curation`
- **推荐理由：** 每日更新的精选插件目录，场景化分类推荐
...
- **推荐理由：** DSH插件总目录，1958仓库+中英文搜索分类筛选
...
- **标签：** `awesome-list`, `cordis`, `plugins`
...
- **标签：** `awesome-list`, `resources`, `agents`
- **推荐理由：** DSH生产力提升插件与工具资源列表
...
- **推荐理由：** 社区维护的插件聚合目录，每4小时自动同步，发现优质插件的必备入口。
...
- **推荐理由：** 收录280+个DSH插件的精选目录，覆盖14个分类。
...
- **标签：** `skillhub`, `api-docs`, `dsh-plugin`
- **推荐理由：** 官方SkillHub开放API文档及插件开发示例。
...
- **标签：** `deepseek-harness`, `dsh-plugins`
- **推荐理由：** 重定向页面，现已迁移至awesome-dsh-plugin组织
