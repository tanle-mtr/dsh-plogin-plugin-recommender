...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 213,314 | Core framework with rich plugin ecosystem |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 94,313 | Open-source Figma alternative with agent support |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 70,739 | Multi-agent swarm orchestration platform |
...
- **标签：** `tui`, `prompt-caching`, `tool-use`
- **推荐理由：** 终端AI编码Agent，支持R1推理
...
- **标签：** `agent-os`, `evaluation`, `self-evolving`
- **推荐理由：** 带分阶段评估的Agent OS，预算控制自进化
...
- **标签：** `code-review`, `dsh-plugin`, `static-analysis`
...
- **⭐ 星标：** 854
- **语言：** TypeScript
- **描述：** [dsh]为纯文本模型设计更强大的视觉��具箱：一行安装使用、粘贴图片直接识别、多张图片问答、截图到前端UI 还原等｜DeepSeek Harness-native integration for agent-vision-toolkit: image Q&A, long-screenshot OCR, UI restoration, grounding, pixel diff, Artifacts, and Web UI.
- **标签：** `vision-tools`, `dsh-plugin`, `computer-vision`
- **推荐理由：** 为纯文本模型打造强大视觉工具包，支持OCR与截图
...
- **推荐理由：** 编码Agent，支持RPC、子Agent和MCP扩展
...
- **推荐理由：** 打通Pi与DSH生态，免改运行Pi插件为DSH原生插件。
...
- **推荐理由：** Superpowers技能移植到Reasonix编程工具链
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** Codex与DSH双向同步桥接，互通Skills/MCP
...
- **标签：** `deepseek`, `dsh-plugin`, `floating-widget`
- **推荐理由：** 可爱鲸鱼浮动窗口监控DeepSeek账户余额
...
- **推荐理由：** 本地优先的Electron桌面工作区，支持DSH会话和项目
...
- **标签：** `ui`, `interactive`, `dsh-ui`
- **推荐理由：** 通过dsh-ui围栏在对话中内联渲染交互UI组件。
...
- **标签：** `desktop`, `tauri`, `multi-model`
- **推荐理由：** 本地优先的多模型桌面应用，Claude Cowork替代品。
...
- **标签：** `desktop`, `electron`, `ssh`
- **推荐理由：** 零设置Windows桌面客户端，支持SSH和远程访问。
...
- **标签：** `canvas`, `conversation`, `branching`
- **推荐理由：** 基于画布的分支对话工作空间插件。
...
- **标签：** `ios`, `mobile`, `client`
- **推荐理由：** 原生iOS客户端，移动端访问Harness工作区与Agent会话。
...
- **推荐理由：** 端到端加密P2P远程访问，多端随时操控Harness。
...
- **标签：** `notes`, `smartisan`, `docker`, `productivity`
- **推荐理由：** 开源锤子便签复刻版，Docker私有化部署+skill调用。
...
- **标签：** `deepseek`, `dsh`, `mcp`, `plugin`, `skills`
- **推荐理由：** 便捷的Web界面，统一管理skill和MCP服务器
...
- **标签：** `agent-harness`, `dsh-plugin`, `plugin-market`, `ui`, `web`
- **推荐理由：** DSH Web界面一键安装插件市场
...
- **推荐理由：** Electron桌面控制台，多Tab管理DSH插件生态
...
- **推荐理由：** 面向临床医生的生物信息学桌面代理，支持离线和技能商店。
...
- **标签：** `devops`, `desktop`, `ssh`, `docker`, `sftp`
- **推荐理由：** 集成数据库、SSH、SFTP、Docker和AI沙箱的一体化开发工具。
...
- **标签：** `desktop`, `electron`, `integration`, `dsh-plugin`
- **推荐理由：** Electron桌面壳包装DSH Web界面。
...
- **标签：** `ui`, `button`, `compact`, `dsh-plugin`
- **推荐理由：** 一键折叠上下文计量面板按钮。
...
- **标签：** `rag`, `vector-search`, `multi-tenant`
- **推荐理由：** LLM知识库平台，支持RAG和Wiki
...
- **标签：** `memory`, `self-evolving`, `token-saving`
...
- **标签：** `memory`, `rag`, `consolidation`
- **推荐理由：** 零提示联想记忆，三层自动巩固技能结晶
...
- **标签：** `knowledge-base`, `plugin-dev`, `rag`
- **推荐理由：** 插件开发知识库，按需调用的Agent Skill
...
- **标签：** `routing`, `router`, `agent-ops`
- **推荐理由：** 任务感知推理路由，智能Agent调度分发
...
- **标签：** `skills`, `store`, `community`
- **推荐理由：** 最全技能商店，自动抓取分类GitHub技能
...
- **标签：** `ai-video`, `dsh-plugin`, `agent-skills`
- **推荐理由：** 一键自动生成Vox风格解说视频
...
- **⭐ 星标：** 1,754
- **语言：** Python
- **描述：** last30days-cn 是一个 AI Agent 技能（Skill），能够自动搜索中国互联网 8 大主流平台最近 30 天的内容，综合分析后生成有据可查的研究��告。
- **标签：** `agent-skills`, `dsh-plugin`, `research`
- **推荐理由：** 自动搜索中国主流平台近30天内容生成研究报告
...
- **标签：** `agentteams`, `dsh-plugin`, `multi-agent`
...
- **标签：** `agent-skills`, `skill-corpus`, `retrieval`
- **推荐理由：** 将分散的SKILL.md转化为可检索的技能库。
...
- **标签：** `multimodal`, `computer-vision`, `perception`
- **推荐理由：** 本地优先的视频音频屏幕感知能力栈。
...
- **标签：** `creative-writing`, `drama`, `novel`
- **推荐理由：** 专用于小说创作和短剧生产的技能插件。
...
- **标签：** `skill`, `workflow`, `math-modeling`
...
- **标签：** `agent-skills`, `workflow`, `self-evolving`
- **推荐理由：** 可组合可审查的Agent模式，实现自我进化
...
- **标签：** `skills`, `workflow`, `architecture`
- **推荐理由：** 将经验转化为可复用的Agent工作流，跨平台兼容。
...
- **标签：** `plugin-discovery`, `automation`, `cli`
...
- **标签：** `infra`, `ai-infra`, `optimization`, `debugging`
- **推荐理由：** AI基础设施开发专用技能集，写/优/调试高性能系统。
...
- **标签：** `research`, `marketing`, `skills`, `multi-platform`
- **推荐理由：** 88个跨平台技能，覆盖研究、营销与社交情报。
...
- **标签：** `reverse-engineering`, `ctf`, `security`, `pentest`
- **推荐理由：** 87个逆向工程技能，适用于CTF与渗透测试。
...
- **标签：** `skills`, `agent-skills`, `productivity`
- **推荐理由：** 25个即装即用的工程效率技能面板
...
- **标签：** `plugin-dev`, `agent-skill`
- **推荐理由：** 管理插件槽位、远程和凭据的Agent Skill
...
- **标签：** `spec`, `agent-skills`, `coding`
- **推荐理由：** 仓库原生规范驱动开发Agent Skill
...
- **标签：** `plugin-dev`, `codex`, `skill`
- **推荐理由：** 构建和验证DSH插件的可复用Skill
...
- **标签：** `agent-skills`, `backtesting`, `fintech`
- **推荐理由：** 多策略回测技能包，适合AI驱动的投研分析
...
- **标签：** `swift-cycle`, `project-governance`, `agent-skills`
- **推荐理由：** 用户调用、版本锁定、可离线验证的Swift Cycle治理技能。
...
- **标签：** `agent-preset`, `workflow`, `product-management`, `dsh-plugin`
- **推荐理由：** 角色扮演agent预设模拟产品团队。
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`, `system-design`
- **推荐理由：** 精美架构图自动生成技能
...
- **标签：** `design`, `dsh-plugin`, `ppt`
- **推荐理由：** AI驱动设计工作室，支持PPT、原型和可视化编辑
...
- **推荐理由：** 完整AI视频/图像/音频流水线，支持分镜和视觉工具。
...
- **标签：** `content-discovery`, `cross-platform`, `automation`
- **推荐理由：** 自动从多平台发现内容。
...
- **推荐理由：** iOS模拟器与设备自动化，提供22个Agent工具
...
- **标签：** `resume-builder`, `self-hosted`, `hacktoberfest`
- **推荐理由：** 隐私优先的开源简历构建器
...
- **标签：** `model-switch`, `installer`, `utility`
- **推荐理由：** 轻松切换各编码代理模型。
...
- **标签：** `dsh-plugin`, `ecosystem-radar`, `plugin-registry`
- **推荐理由：** 自部署插件生态雷达，覆盖15900+候选插件
...
- **标签：** `dsh-plugin`, `context-management`, `cordis-plugin`
- **推荐理由：** 最佳上下文管理插件，带仪表盘和统计功能
...
- **标签：** `backup`, `config-manager`, `mcp`, `skills`, `sync`
...
- **标签：** `awesome-list`, `directory`, `utility`
- **推荐理由：** 精选280+社区插件目录，14大类一键直达
...
- **标签：** `dsh-plugin`, `containers`, `infrastructure`
- **推荐理由：** 容器化多版本DSH隔离管理，嵌入式UI便捷
...
- **标签：** `utility`, `conflict-resolution`, `dsh-plugin`
- **推荐理由：** 诊断并解决插件冲突与版本问题。
...
- **标签：** `mascot`, `pets`, `ui-personality`
- **推荐理由：** 公开动画宠物画廊，为编码Agent增添趣味
...
- **标签：** `webui`, `theme`, `background`, `dsh-plugin`
- **推荐理由：** 自定义背景主题，支持实时预览。
...
- **标签：** `awesome-list`, `ai-resources`, `encyclopedia`
- **推荐理由：** 一站式AI资源百科，涵盖技能、RAG、MLOps等。
...
- **⭐ 星标：** 245
- **语言：** TypeScript
- **描述：** DSH (DeepSeek Harness) 原理学习、插件市场与最��实践 · Learn DSH principles, plugin marketplace & best practices
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU任务管理
...
- **标签：** `academic-search`, `citation`, `research`
- **推荐理由：** 跨PubMed/arXiv/OpenAlex多源检索，支持引文图谱与核验。
...
- **标签：** `industry-research`, `company-research`, `dsh-plugin`
- **推荐理由：** 行业研究方法论与公司分析技能包
...
- **标签：** `awesome-list`, `typescript`, `directory`, `nextjs`
- **推荐理由：** 公开DSH插件GitHub目录，含安装命令和发现功能。
...
- **标签：** `registry`, `open-data`, `plugin-registry`, `skills`
- **推荐理由：** 反垃圾开源数据注册的DSH插件、捆绑和技能目录。
...
- **标签：** `awesome-list`, `portal`, `dsh-plugin`
- **推荐理由：** 静态网页门户浏览发现227+插件。
...
- **标签：** `deepseek`, `dsh-plugin`, `armor-breaking`
- **推荐理由：** DeepSeek专用破甲插件，增强生成能力
...
- **标签：** `anime`, `bangumi`, `search`, `entertainment`
- **推荐理由：** 对话式搜番，集成Bangumi评分与磁力链接。
...
- **推荐理由：** 精选DSH插件 Awesome 列表，社区发现必备。
...
- **推荐理由：** 官方精选的DeepSeek Harness插件awesome列表
...
- **标签：** `dsh-bundle`, `dsh-plugin-verify`, `dsh-plugins-net`
- **推荐理由：** 经过验证的DSH插件目录，方便查找优质插件。
...
- **标签：** `mcp`, `plugin-recommender`, `coding-agents`
- **推荐理由：** AI策展的190+插件列表，覆盖12个分类，每小时更新。
...
- **推荐理由：** 自动更新的YAML数据驱动插件列表，带Vercel站点。
...
- **标签：** `curated-list`, `plugin-directory`, `registry`, `awesome`
