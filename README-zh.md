...
- **推荐理由：** DeepSeek原生终端编程代理，前缀缓存优化稳定
...
- **标签：** `dsh-plugin`, `cordis`, `ai-agents`
...
- **标签：** `agent-os`, `mcp`, `llm-orchestration`, `dsh-plugin`
- **推荐理由：** 自进化Agent操作系统，带分阶段评估
...
- **推荐理由：** 首个DSH视觉插件，为文本Agent提供OCR能力
...
- **标签：** `cognition`, `reasoning`, `inference-control`
- **推荐理由：** 模型无关的推理时控制套件，支持深度推理与恢复
...
- **推荐理由：** Rust高性能Agent运行时，桌面IDE深度集成
...
- **标签：** `agent-teams`, `orchestration`, `coding`
- **推荐理由：** 多Agent团队协作编排，适合复杂编码工作流
...
- **⭐ 星标：** 880
- **语言：** TypeScript
- **描述：** [dsh]为纯文本��型设计更强大的视觉工具箱：一行安装使用、粘贴图片直接识别、多张图片问答、截图到前端UI 还原等｜DeepSeek Harness-native integration for agent-vision-toolkit: image Q&A, long-screenshot OCR, UI restoration, grounding, pixel diff, Artifacts, and Web UI.
- **标签：** `vision`, `ocr`, `gui-automation`
- **推荐理由：** 纯文本模型视觉增强：OCR、截图UI还原、多图问答
...
- **标签：** `plugin-market`, `curation`, `daily-update`
- **推荐理由：** 全面插件市场，每日更新并审核，DSH一站式获取插件。
...
- **推荐理由：** 编码智能体，支持RPC、子智能体、hashline编辑与MCP。
...
- **推荐理由：** DSH插件市场官方CLI工具含schema和解析器
...
- **标签：** `command-code`, `provider`, `deepseek-harness`
- **推荐理由：** 提供计划感知模型选择和实时目录，增强DSH代理能力。
...
- **标签：** `agent-review`, `prompt-engineering`, `self-evolve`
- **推荐理由：** 可组合Agent工作模式，支持人工审查和持续自我进化。
...
- **标签：** `vision`, `multimodal`, `frontend`
- **推荐理由：** 为LLM添加多模态视觉能力，支持截图和UI分析。
...
- **推荐理由：** DSH插件生态基建，含仓库管理面板与官方插件开发引导技能。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 在DSH中统一加载并安全管理本机Agent Skills的核心工具。
...
- **标签：** `godot`, `game`, `gdscript`, `development`
...
- **⭐ 星标：** 22
- **语言：** TypeScript
- **描述：** dsh-plugin-developer — DeepSeek Harness 插件开发 Skill  > 指导 AI Agent 从 0 到 1 开发、构建、安装、测试 DeepSeek Harness（dsh）插件。基于 **dsh 0.1.1-rc.2** 与 dsh-openmaic 项目的完整实战经验，并内置一个**已通过 web 界面实测**的天气插件作为整包参考案例。  ## 这是什么  dsh 采用"无特权内核、万物皆插件"的��计：模型适配器、工具注册表、Agent 循环、网页界面都是插件
- **标签：** `developer`, `plugin-dev`, `skill`
- **推荐理由：** 指导AI从0到1开发DSH插件的完整工作流
...
- **标签：** `plugin-dev`, `skill`, `adapter`
- **推荐理由：** 开发DSH插件的Agent技能，覆盖插件/服务/事件/工具
...
- **标签：** `debugging`, `diagnostics`
- **推荐理由：** 诊断和解决DSH插件之间的冲突问题
...
- **标签：** `agent-harness`, `agent-skills`, `no-code`, `mcp`
- **推荐理由：** 全端统一的Agent工作区看板
...
- **推荐理由：** 本地优先桌面工作区，支持会话、文件、网页研究与办公集成。
...
- **推荐理由：** 零安装Windows桌面客户端，内置Codex、SSH及移动端远程访问。
...
- **推荐理由：** 本地优先多模型桌面智能体，支持自进化技能与隐私保护。
...
- **推荐理由：** 将表格文档幻灯片引入Harness打造真实办公环境
...
- **⭐ 星标：** 297
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将工作区、会话、实时回复和 Agent 执行轨迹带到 iPhone，同时延�� DeepSeek WebUI 克制、清晰的视觉语言
- **标签：** `ios`, `mobile`, `kmp`
- **推荐理由：** 原生iOS客户端通过WebSocket连接工作区与Agent
...
- **推荐理由：** 类CodePilot跨平台桌面客户端含完整插件套件
...
- **标签：** `mcp`, `skills`, `ui`, `web`
- **推荐理由：** Web界面统一管理Skill和MCP，操作直观
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** Electron桌面控制台，多Tab统一管理Harness/插件/MCP/Skills。
...
- **标签：** `desktop`, `devops`
- **推荐理由：** 整合数据库、SSH、Docker和AI的一体化开发运维桌面
...
- **标签：** `desktop`, `electron`
- **推荐理由：** Electron桌面版DSH，集成sidebar和ModLens
...
- **标签：** `ui`, `button`
- **推荐理由：** One-click compact button for the DSH Web context meter panel
...
- **标签：** `long-term-memory`, `agent-memory`, `rag`, `mcp`
- **推荐理由：** 自进化记忆系统，混合检索省Token
...
- **标签：** `agent-memory`, `agentic-rag`, `memory`, `mcp`
...
- **标签：** `memory`, `milvus`, `rag`, `persistent-memory`
- **推荐理由：** 基于Milvus的持久化统一记忆层，跨Agent共享
...
- **标签：** `memory`, `local-first`, `persistent-context`
- **推荐理由：** 共享持久记忆中枢，让AI Agent记住一切
...
- **标签：** `skill-retrieval`, `vector-search`, `embeddings`
- **推荐理由：** 将SKILL.md转为向量检索语料，支持语义搜索与评估工具。
...
- **标签：** `agent-observability`, `multimodal`, `video-understanding`
- **推荐理由：** 赋予AI视觉音频感知能力，视频音频可追溯验��
...
- **推荐理由：** 原生SQLite任务板支持Agent认领审核与Web UI
...
- **标签：** `math-modeling`, `gitops`, `agent-skill`
- **推荐理由：** 全自动数学建模Skill，含GitOps流水线和代码自证功能。
...
- **标签：** `infra`, `ai-infra`, `debugging`
- **推荐理由：** AI基础设施专用技能，支持编写优化和调试高性能系统。
...
- **标签：** `agent-skills`, `skill-generator`, `automatic-learning`
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** AI编程导师技能，扫描架构、生成阶梯章节、自动判题与Code Review。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 内置25个工程效率技能，右侧面板直接调用无需手动安装。
...
- **⭐ 星标：** 62
- **语言：** Python
- **描述：** 专利侵权分析系统 —— 输入专利公开号，产出竞品侵权���析报告；同时打包成 skill，可被任意 agent（dsh, codex, claudecode 等） 调用。
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 输入专利号自动生成竞品侵权分析报告，可被多Agent调用。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 国标公文全流程处理：格式检查、修订对比、模板生成与版头注入。
...
- **⭐ 星标：** 39
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇门/风水/五运六气/体质全融���，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 八字/风水/六爻/中医等传统智慧AI技能包，含可视化Dashboard。
...
- **标签：** `plugin-development`, `codex`, `validation`
...
- **标签：** `official`, `documentation`, `api`
- **推荐理由：** 官方SkillHub API文档与示例，适合插件开发入门
...
- **标签：** `math`, `competition`, `agent`, `modeling`
- **推荐理由：** 数学建模多智能体方案，含33模型库和论文模板
...
- **推荐理由：** 内联渲染交互UI组件：图表、表单、测验等可视化内容。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 法律可视化SVG工具，将案件材料重画为标准诉讼图和时间轴。
...
- **推荐理由：** 电影感产品视频技能，含152镜头配方和Remotion模板
...
- **推荐理由：** 跨平台内容发现Agent，支持B站/抖音/YouTube等8+平台
...
- **标签：** `web-search`, `free`, `no-api`
- **推荐理由：** 免费无需注册的Web搜索插件，为无网络模型提供搜索桥接。
...
- **推荐理由：** 内置iOS模拟器与USB真机自动化22项Agent工具
...
- **推荐理由：** Tabbit浏览器插件，支持Playwright网页自动化
...
- **推荐理由：** 支持60+图床的终极图片上传工具，适配Obsidian
...
- **推荐理由：** 一键切换9+编码Agent平台，支持Claude Code/Codex等
...
- **标签：** `plugin-registry`, `discovery`, `ecosystem`
- **推荐理由：** 自动发现并实测验证15900+候选插件生态雷达
...
- **标签：** `context`, `dashboard`, `monitoring`
- **推荐理由：** 上下文可视化仪表盘，实时监控和管理上下文
...
- **标签：** `security`, `vulnerability`, `redteam`
- **推荐理由：** AI安全攻防平台，审计幻觉包与代码漏洞的安全工具。
...
- **推荐理由：** 通过OAuth将ChatGPT Claude Grok订阅转为DSH LLM提供商
...
- **标签：** `config-manager`, `backup`, `migration`, `sync`
- **推荐理由：** 完整备份恢复DSH配置、插件和MCP
...
- **标签：** `container`, `management`, `infra`
- **推荐理由：** 在隔离容器中运行多个DSH版本，内置WebView
...
- **标签：** `cost-tracking`, `visualization`
- **推荐理由：** 实时追踪DeepSeek API的token用量和成本估算
...
- **标签：** `mascot`, `dsh-plugin`, `sprites`, `claude-code`
- **推荐理由：** 各Agent Harness的动画宠物挂件画廊
...
- **标签：** `skin`, `animated`, `ocean`
- **推荐理由：** 实时海洋动态主题，可调海浪、日落与玻璃透明度效果。
...
- **标签：** `theming`, `customization`
- **推荐理由：** 支持自定义图片和预设氛围的Web背景插件
...
- **标签：** `ai`, `vibe-coding`, `deepseek`, `rag`
...
- **标签：** `documentation`, `analysis`, `code-research`
- **推荐理由：** 自动生成精准技术文档和AI可读上下文
...
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、实验追踪与GPU调度。
...
- **⭐ 星标：** 338
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合��，以及如何开始使用。欢迎 Star ，让好用的插件更快被发现。
- **标签：** `awesome-list`, `plugin-discovery`, `curation`
- **推荐理由：** 每日自动抓取GitHub插件并分类筛选真实项目
...
- **标签：** `fintech`, `backtesting`, `investment-research`, `llm-agents`
- **推荐理由：** 合规优先的AI投资研究工作台，支持多策略回测
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 零配置搜索65,600+中国开放平台API文档，原生兼容多Agent。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 14类280+ DSH插件精选目录，覆盖MCP/技能/UI等全品类。
...
- **推荐理由：** 社区精选DSH插件目录，覆盖CLI、Web UI和Agent生态
...
- **标签：** `encyclopedia`, `knowledge-base`, `ai-resources`
- **推荐理由：** 全维度AI资源百科，覆盖Agent、RAG、多模态与学习路线。
...
- **推荐理由：** 11000+插件市场与目录，支持搜索、排名和API调用。
...
- **推荐理由：** 全面手册：运行时、插件、MCP和沙箱等173个资源。
...
- **标签：** `awesome-list`
- **推荐理由：** Hand-picked collection of useful DSH plugins
...
- **⭐ 星标：** 3
- **语言：** TypeScript
- **描述：** DeepSeek Harness ��本的类 CodeX 插件市场
- **标签：** `marketplace`, `dsh-plugin-bundle`
- **推荐理由：** 类CodeX插件市场，面向桌面端。
...
- **标签：** `awesome-list`
- **推荐理由：** 原始awesome-dsh-plugin仓库，已迁移。
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 自动更新的插件列表，双语README和Vercel部署。
