...
- **标签：** `terminal`, `coding`, `agent`, `cli`
- **推荐理由：** 终端原生编码代理，前缀缓存稳定高效。
...
- **推荐理由：** 最小化引导至完整标准工具集，对齐Project2规范
...
- **标签：** `claude-code`, `deepseek-harness`, `dsh-plugin`, `model-switch`
- **推荐理由：** 一键切换Claude、Codex、Grok等多种模型。
...
- **标签：** `agent-runtime`, `coding-agent`, `desktop-app`
- **推荐理由：** 高性能Rust代理运行时搭配精美桌面应用。
...
- **标签：** `code-review`, `architecture-review`, `code-quality`
- **推荐理由：** 基于12本经典工程著作的AI代码审查工具。
...
- **标签：** `android`, `emulator`, `development`
...
- **标签：** `plugin-dev`, `skills`
- **推荐理由：** 用于开发DSH插件的标准Agent Skill指南。
...
- **标签：** `plugin-dev`, `scaffolding`, `credentials`
- **推荐理由：** 提供DSH插件脚手架和凭据管理，简化开发流程。
...
- **标签：** `containers`, `ui`, `isolated`
- **推荐理由：** 容器化隔离运行多版本DSH，内置WebView界面。
...
- **推荐理由：** 双向同步Codex与DSH的聊天、技能和MCP工具。
...
- **标签：** `plugin-development`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 构建和验证DSH插件的可复用技能，开发效率倍增。
...
- **标签：** `plugin-development`, `agent-skills`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 便携式插件开发与审计技能，附带bundle适配器。
...
- **标签：** `design`, `ui`, `desktop`, `figma`
- **推荐理由：** 开源AI设计插件，代码代理的Figma替代方案。
...
- **推荐理由：** 开放侧边栏底座，内置文件、终端、Git和子代理页面。
...
- **推荐理由：** 鲸鱼顶栏、流式思考的Claude Code风格TUI插件。
...
- **标签：** `desktop-companion`, `electron`, `dsh-plugin`
- **推荐理由：** 本地优先桌面伴侣，带动画宠物和插件SDK集成
...
- **标签：** `widget`, `desktop`, `dsh-plugin`
- **推荐理由：** 右下角小鲸鱼 Widget 实时监测 DeepSeek 账户余额
...
- **标签：** `desktop-app`, `agent`, `mcp`
- **推荐理由：** 本地优先桌面AI Agent，支持多模型与隐私保护。
...
- **推荐理由：** 功能齐全的Windows桌面GUI客户端，集成SSH和Codex
...
- **推荐理由：** 基于Electron的DSH桌面管理控制台，支持多Tab管理。
...
- **标签：** `ui`, `usage-tracking`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 底部状态栏显示模型/余额/峰谷定价，一目了然。
...
- **标签：** `desktop`, `tauri`, `integrated`
- **推荐理由：** 跨平台桌面整合DB/SSH/Docker/AI
...
- **标签：** `memory`, `rag`, `context`, `database`
...
- **标签：** `long-term-memory`, `rag`, `local-first`
- **推荐理由：** 本地优先的Markdown原生长期记忆层，可移植通用
...
- **标签：** `memory-management`, `token-savings`, `rag`
- **推荐理由：** 自演化记忆系统，混合检索节省35% token开销
...
- **推荐理由：** 基于Markdown和Milvus的AI代理持久统一记忆层。
...
- **⭐ 星标：** 267
- **语言：** TypeScript
- **描述：** AI skills大全 mcp ai知识库 Agent 全维度 AI 资源百科，DSH插件 收录大模型、智能 Agent、RAG 检索增强、多模态、MLOps、AI 应用工具、AI面试集、Vibe coding 大全、零基础学习路线，持续更��前沿 AI 开源项目，开发者一站式 AI 导航库
- **标签：** `rag`, `mcp`, `dsh-plugin`
- **推荐理由：** 覆盖RAG、MCP、多模态的全维度AI知识库。
...
- **标签：** `skill-md`, `vector-search`, `reranking`, `embeddings`
- **推荐理由：** 构建可检索评估的聚合技能语料库
...
- **推荐理由：** 审计上下文Token消耗，检测重复冲突，管理指令链必备。
...
- **标签：** `evaluation`, `rag-evaluation`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 为提示词、RAG和技能提供可量化评估与可观测性。
...
- **标签：** `agentic`, `swarm`, `multi-agent`, `workflow`
- **推荐理由：** 多智能体蜂群协调的原生元工具。
...
- **标签：** `ai-agent`, `skills`, `memory`, `self-evolving`
- **推荐理由：** 具备记忆与技能进化能力的超级AI助手。
...
- **标签：** `skills`, `distill`, `agent`, `workflow`
- **推荐理由：** 将代理思维提炼为可复用技能的利器。
...
- **标签：** `agent-orchestration`, `workspaces`, `no-code`
- **推荐理由：** 跨设备统一Agent工作区，看板式任务跟踪
...
- **标签：** `routing`, `agent-skills`, `cordis`
- **推荐理由：** 任务感知路由注入器，智能切换Agent推理模式
...
- **推荐理由：** 通过Agent技能��到端生成Vox风格拼贴解说视频。
...
- **标签：** `agent`, `skill`, `deepseek-harness`
- **推荐理由：** 将多个编码Agent整合为可协作调度的团队。
...
- **标签：** `skill`, `math-modeling`, `automation`, `gitops`
- **推荐理由：** 全栈式自动数学建模竞赛工作流
...
- **标签：** `vision`, `multimodal`, `agent-skills`, `ui-testing`
...
- **标签：** `task-management`, `sqlite`
- **推荐理由：** 原生SQLite任务板支持Agent认领审核无需iframe
...
- **推荐理由：** 将superpowers技能适配到DSH，支持TDD、调试和规划。
...
- **标签：** `spec-programming`, `agent-skills`
- **推荐理由：** 仓库原生规格驱动开发的DSH技能。
...
- **标签：** `bazi`, `fengshui`, `dashboard`, `chinese`
- **推荐理由：** 融合八字/风水/六爻等传统智慧的可视化AI技能包。
...
- **推荐理由：** 支持Codex/Claude Code跨平台会话记忆与��接。
...
- **标签：** `gongwen`, `docx`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 中文公文全流程处理，格式检查+内容优化一站式。
...
- **标签：** `agent-skills`, `database`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 42项技能包管理器，覆盖数据库、评估和Agent开发。
...
- **标签：** `skill`, `engineering`, `portable`
- **推荐理由：** 可移植的工程纪律Agent技能
...
- **推荐理由：** 首个纯文本Agent视觉桥接插件，OCR结构化输出
...
- **标签：** `api-documentation`, `chinese-api`
- **推荐理由：** 在DSH中直接搜索20个中国平台的65,600+篇API文档。
...
- **标签：** `mcp`, `skills`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 在Web UI设置页直接管理技能和MCP服务器，便捷高效。
...
- **标签：** `mcp`, `zhihu`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 知乎搜索+知了问答+开放平台API，一插件全覆盖。
...
- **标签：** `image`, `prompt`, `design`, `gpt`
- **推荐理由：** 530+逆向案例的工业级提示词引擎。
...
- **标签：** `design-tool`, `vibedesign`, `ui`
- **推荐理由：** AI原生矢量设计工具，支持并发Agent团队设计UI
...
- **标签：** `kline`, `chart`, `finance`, `visualization`
...
- **标签：** `design-skills`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 提供6种设计风格，避免AI生成网站千篇一律。
...
- **标签：** `image-generation`, `vision`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 支持图片读取与生成，自动视觉转录，扩展多模态能力。
...
- **标签：** `ai-video`, `storyboard`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 集成AI视频/图像/音频技能，支持多模型配置调用。
...
- **标签：** `browser`, `extension`, `automation`, `gemini`
- **推荐理由：** 覆盖Gemini/Claude/ChatGPT的增强套件。
...
- **标签：** `dsh-web`, `plugin-marketplace`, `web-ui`
- **推荐理由：** Web端插件聚合生态，创意工坊分发平台
...
- **标签：** `bilibili`, `content-discovery`, `cross-platform`, `web-automation`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、YouTube、Reddit等。
...
- **标签：** `browser-extension`, `workflow`, `plugin`
...
- **标签：** `web-search`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 最强免费网页搜索桥接，无注册无需API密钥。
...
- **标签：** `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 零权限GitHub OAuth合影墙，记录DSH内测珍贵瞬间。
...
- **标签：** `chatgpt`, `integration`
- **推荐理由：** OAuth集成ChatGPT到DSH
...
- **标签：** `context-management`, `dashboard`, `dsh-plugin`
- **推荐理由：** 上下文管理仪表盘，提供统计分析和浏览器集成
...
- **标签：** `oauth`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 通过OAuth将订阅服务转为DSH的免费LLM源。
...
- **标签：** `migration`, `compatibility`, `pi-agent`, `abi`
- **推荐理由：** Pi生态插件与DSH的兼容桥接层
...
- **推荐理由：** 开源锤子便签，一键Docker部署支持skill调用
...
- **标签：** `migration`, `context`
- **推荐理由：** 跨预设会话迁移保留状态和模型意图
...
- **标签：** `documentation`, `best-practices`, `dev`
- **推荐理由：** DSH插件开发踩坑与最佳实践档案
...
- **标签：** `mascot`, `pixel-art`, `customization`
- **推荐理由：** 动画宠物画廊，个性化Agent视觉体验
...
- **标签：** `theme`, `deepseek-harness`, `dsh-plugin`
...
- **标签：** `themes`, `dark-mode`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 34款经典opencode配色一键切换，重启不丢失。
...
- **标签：** `ai`, `resources`, `guide`, `tutorial`
- **推荐理由：** AI资源大全与Vibe Coding保姆级教程。
...
- **推荐理由：** 自动发现9000+插件并实测精选Top50，必备策展雷达
...
- **标签：** `awesome-list`, `bilingual`, `deepseek-harness`
- **推荐理由：** 双语精选DSH插件生态目录与资源导航。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `education`
...
- **标签：** `awesome-list`, `plugin-directory`, `mcp`, `patch`
- **推荐理由：** 插件、MCP服务、技能的精选目录
...
- **标签：** `plugin-marketplace`, `registry`, `catalog`
- **推荐理由：** 3100+插件的市场搜索与公共API
