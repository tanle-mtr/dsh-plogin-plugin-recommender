...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 194,102 | Core DSH platform enabling all plugins; essential  |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,305 | Open-source design tool with local-first approach; |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,350 | Deploy multi-agent swarms and coordinate autonomou |
...
- [MCP 服务器与工具](mcp-servers-tools-zh) (10)
...
- **标签：** `multi-agent`, `mcp`, `self-evolving`
- **推荐理由：** 具备自进化记忆的超级AI助手，多模型支持。
...
- **标签：** `routing`, `injector`, `orchestration`
- **推荐理由：** 运行时注入器+任务感知路由，优化Agent工作流
...
- **推荐理由：** 企业级多引擎Agent工作台，统一Codex/DSH/Hermes开发环境
...
- **推荐理由：** 一键切换多模型，支持Claude Code、DeepSeek等主流AI编码工具。
...
- **标签：** `agent-runtime`, `ai-coding`, `desktop-application`
- **推荐理由：** Rust高性能Agent运行时配合精美桌面应用，高级编码工作流首选。
...
- **标签：** `agent-skill`, `dsh-plugin`, `plugin-management`
- **推荐理由：** 构建管理DSH插件槽位和凭据的技能
...
- **⭐ 星标：** 13
- **语言：** Unknown
- **描述：** DSH 插件开发踩坑与做法档案（skill + 文档）：cordis 双副本、tsconfig 三件套、Windows junction、多帧 zstd 等实测记���
- **标签：** `plugin-dev`, `documentation`, `best-practices`
- **推荐理由：** DSH插件开发踩坑档案，含Windows实践。
...
- **标签：** `development-practices`, `engineering`, `skill`
- **推荐理由：** DSH工程纪律指南：事实驱动、证据优先。
...
- **标签：** `web-ui`, `plugin-hub`, `distribution`
- **推荐理由：** DSH生态Web插件聚合分发平台，创意工坊
...
- **标签：** `sidebar`, `dsh-plugin`, `extension`
- **推荐理由：** 开放侧边栏底座，支持文件编辑、终端、Git等，可扩展性强。
...
- **推荐理由：** Claude Code风格TUI插件，鲸鱼顶栏、实时状态，UI体验出色。
...
- **标签：** `desktop-pet`, `electron`, `dsh-plugin`
- **推荐理由：** 带动画宠物的桌面伴侣平台，支持插件SDK扩展。
...
- **推荐理由：** 本地优先的Electron桌面工作空间，支持DSH会话管理。
...
- **标签：** `ios`, `mobile`, `testing`
- **推荐理由：** 在对话中直接运行iOS模拟器和真机测试。
...
- **推荐理由：** 功能完备的Electron桌面客户端，含SSH与移动远程访问。
...
- **标签：** `capability-menu`, `mcp`, `ui`
- **推荐理由：** 统一菜单管理MCP工具暴露级别和执行模式
...
- **标签：** `desktop`, `electron`, `docker`, `ssh`, `database`
- **推荐理由：** 跨平台桌面应用，集成DB/SSH/Docker面板
...
- **标签：** `desktop`, `electron`, `integration`, `web`
- **推荐理由：** Electron桌面版DSH，集成多组件服务
...
- **标签：** `rag`, `semantic-search`, `knowledge-base`
- **推荐理由：** 具备RAG、语义搜索和Wiki的LLM知识平台。
...
- **推荐理由：** 可移植的本地优先记忆层，Markdown原生，跨应用同步
...
- **标签：** `memory`, `RAG`, `token-optimization`, `self-evolving`
...
- **推荐理由：** 专为Agent设计的记忆管理套件，支持RAG，适合持久化记忆需求。
...
- **标签：** `agent-memory`, `milvus`, `rag`, `embeddings`
- **推荐理由：** 基于Milvus和Markdown的持久统一记忆层，专业级Agent记忆方案。
...
- **标签：** `knowledge`, `encyclopedia`, `rag`
- **推荐理由：** 全面AI知识百科，涵盖技能、RAG和学习路线。
...
- **推荐理由：** 将SKILL.md文件转化为支持检索的Agent技能知识库。
...
- **标签：** `memory`, `migration`, `session`
- **推荐理由：** 保留跨预设会话状态和源模型意图
...
- **标签：** `memory`, `recall`, `handoff`, `mcp`
- **推荐理由：** 通过HTTP连接PowerContext实现记忆召回
...
- **标签：** `workspace`, `orchestration`, `multi-platform`
- **推荐理由：** 统一多端Agent工作台，支持看板任务跟踪
...
- **推荐理由：** 基于J-space全局工作空间研究的认知增强技能，高级Agent编排。
...
- **推荐理由：** 通用技能，智能路由并编排Harness工作流，功能全面强大。
...
- **标签：** `video-generation`, `ffmpeg`, `tts`, `agent-skill`
- **推荐理由：** 端到端Vox风格拼贴视频生成技能，一键生成完整视频。
...
- **标签：** `code-review`, `clean-architecture`, `auto-fix`, `agent-skills`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，支持自动修复，提升代码质量。
...
- **标签：** `agentteams`, `dsh-plugin`, `workflow`
- **推荐理由：** 在DSH中��现多智能体团队协作能力。
...
- **标签：** `agent-vision-toolkit`, `dsh-plugin`, `computer-vision`
- **推荐理由：** 为纯文本模型提供视觉工具箱：OCR、图像问答。
...
- **标签：** `agent`, `team`, `workflow`
- **推荐理由：** 将多个编码代理整合为协作团队工作流。
...
- **标签：** `skills`, `automation`, `agent-framework`
- **推荐理由：** 将经验转化为可复用的AI Agent执行技能。
...
- **标签：** `skills`, `ai-infra`, `debugging`
- **推荐理由：** 专为AI基础设施开发提供的专用代理技能
...
- **标签：** `skill`, `tdd`, `collaboration`
- **推荐理由：** 适配的TDD调试规划和协作技能
...
- **标签：** `agent-skills`, `legal`, `visualization`, `svg`
- **推荐理由：** 法律可视化工具，生成时间轴和诉讼图表
...
- **标签：** `code-simplification`, `agent-skills`, `codex`
- **推荐理由：** 基于Codex的代码简化技能，自动化重构辅助
...
- **标签：** `agent-framework`, `package-manager`, `skills`
- **推荐理由：** 42个技能+5个CLI的AI代理资产包管理器。
...
- **推荐理由：** Godot 4.x全栈游戏开发技能，专为DSH设计
...
- **标签：** `plugin`, `skills`, `development`
- **推荐理由：** 插件开发与测试技能，覆盖脚手架到测试
...
- **标签：** `agent`, `canvas`, `skills`, `mcp`
- **推荐理由：** WeShop无限画布插件，提供创意技能支持
...
- **标签：** `agent-tools`, `workflow`, `checklist`
- **推荐理由：** Agent任务插件预检起飞检查单
...
- **推荐理由：** 首个文字Agent视觉插件，图片转结构化JSON证据
...
- **标签：** `marketplace`, `plugin`, `tool`
- **推荐理由：** 一键浏览、安装和更新所有GitHub DSH插件
...
- **⭐ 星标：** 14
- **语言：** JavaScript
- **描述：** 改��置+重启才能加 MCP？DeepSeek Harness (DSH) 插件「技能配置」解决：设置页粘贴 JSON 即连 MCP 服务器、热加载免重启；上传 zip/.skill 自动识别 SKILL.md，创建/编辑/启停技能，即点即用
- **标签：** `mcp`, `hot-reload`, `skill-management`
- **推荐理由：** 粘贴JSON即连MCP，热加载免重启，自动识别技能。
...
- **标签：** `ai-agent`, `bilibili`, `content-discovery`, `cross-platform`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、YouTube、Reddit等平台，本地优先。
...
- **推荐理由：** 钉钉 IM 频道，将聊天转为可调用工具的 agent 会话。
...
- **推荐理由：** 首个AI Agent统一虚拟文件系统，跨引擎文件桥接
...
- **推荐理由：** DSH官方插件市场，浏览搜索一键安装，必备基础设施工具。
...
- **推荐理由：** DeepSeek Harness生态插件精选索引库。
...
- **推荐理由：** 通过OAuth使用订阅作为LLM提供商，无需API密钥。
...
- **推荐理由：** 打通Pi与DSH生态，无缝运行Pi扩展作为DSH插件。
...
- **标签：** `notes`, `docker`, `export`
- **推荐理由：** 一键Docker部署，支持技能调用和图片导出
...
- **标签：** `android`, `emulator`, `usb`
- **推荐理由：** 通过对话驱动的Android模拟器USB流式交互
...
- **标签：** `plugin-management`, `console`, `ui`, `dsh-plugin`
- **推荐理由：** 插件注册表控制台加开发脚手架引导
...
- **标签：** `migration`, `import`, `utility`, `tools`
- **推荐理由：** 将Claude Code配置迁移到DeepSeek Harness
...
- **标签：** `conflict-resolution`, `utility`
- **推荐理由：** 自动解决插件间冲突问题
...
- **标签：** `marketplace`, `dsh-plugin`, `dsh-plugins`
- **推荐理由：** DeepSeek Harness 版类 CodeX 插件市场，支持插件包。
...
- **标签：** `mascot`, `pixel-art`, `animation`, `themes`
- **推荐理由：** AI编程Agent的动效萌宠展示，增添趣味个性化
...
- **标签：** `skin`, `theme`, `whale`
- **推荐理由：** 鲸鱼娘系列主题皮肤，深受欢迎的DSH视觉个性化选择。
...
- **标签：** `ai-guide`, `prompt-library`, `rag`
- **推荐理由：** 全面的AI资源大全和教程知识库。
...
- **标签：** `research`, `web-search`, `rednotebook`, `tiktok`
- **推荐理由：** 自动搜索中国8大平台近30天内容并生成研究报告，中文研究必备。
...
- **标签：** `awesome-list`, `resource`, `bilingual`
- **推荐理由：** 双语精选DSH插件和工具资源列表。
...
- **标签：** `discovery`, `curated`, `awesome-list`
- **推荐理由：** 每日精选插件发现，场景化分类推荐。
...
- **标签：** `learn`, `marketplace`, `guide`
- **推荐理由：** 学习DSH原理和插件市场最佳实践。
...
- **标签：** `awesome-list`, `mcp`, `plugin-catalog`
- **推荐理由：** 涵盖插件、MCP服务器、补丁的全景式DSH资源库。
...
- **标签：** `evaluation`, `rag-evaluation`, `benchmark`
- **推荐理由：** 基于证据的提示词/RAG/Agent评测框架。
...
- **标签：** `cost`, `data-visualization`, `token-usage`
- **推荐理由：** 实时Token用量图表与成本估算工具
...
- **标签：** `research`, `radar`, `analysis`
- **推荐理由：** 基于证据的插件雷达与研究实验室
...
- **推荐理由：** 支持从 dsh agent 内部搜索 npm 和插件清单。
...
- **标签：** `awesome-list`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** GitHub 1000+ DSH 工具按 star 排序的全量清单。
...
- **推荐理由：** AI 策展，每小时更新，覆盖 12 大类 190+ 插件。
...
- **推荐理由：** 涵盖模型、工具、沙箱、UI 和 agent 回路的精选插件。
...
- **标签：** `mcp-server`, `self-hosted`, `javascript`
- **推荐理由：** 注重隐私的简历构建工具，独特实用。
...
- **标签：** `no-code`, `low-code`, `admin-dashboard`
...
- **标签：** `tutorial`, `education`, `harness-engineering`
- **推荐理由：** 从零开始的DSH开发入门教程，适合新手学习
...
- **标签：** `awesome-list`, `curation`, `directory`
- **推荐理由：** DSH精选插件列表，便于发现和筛选优质插件
...
- **标签：** `web-app`, `toy`, `single-file`
- **推荐理由：** 传统竹知了玩具的Web模拟版，零依赖单文件，有趣实用。
...
- **标签：** `jailbreak`, `armor-breaking`, `uncensored`
- **推荐理由：** 破甲插件，用于绕过DeepSeek内容限制。
...
- **标签：** `plugin-marketplace`, `bundle`
- **推荐理由：** DSH插件市场合集，集成多源插件包。
...
- **推荐理由：** 精选的优质DeepSeek Harness插件资源列表
...
- **标签：** `dsh-plugin`, `oauth`, `community`
- **推荐理由：** GitHub OAuth合影墙，适合DSH内测活动使用。
...
- **标签：** `awesome-list`, `community`, `dsh-plugin`
- **推荐理由：** 非官方社区DSH插件目录，便于发现插件
