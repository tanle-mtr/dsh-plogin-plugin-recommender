...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 222,317 | Core platform powering the entire DSH plugin ecosy |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 95,880 | Local-first design system with UI generation for a |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 72,285 | Multi-agent swarm orchestration with autonomous wo |
...
- **标签：** `multi-agent`, `mcp`, `skills`, `self-evolving`
- **推荐理由：** 具备记忆与技能的自进化多代理Harness
...
- **标签：** `agent-os`, `self-improving`, `mcp`
- **推荐理由：** 具备评估门控进化循环的自改进代理OS
...
- **标签：** `agent-teams`, `agentic-runtime`, `ai-coding`
- **推荐理由：** 高性能Rust运行时，内置桌面客户端，适合编��Agent使用。
...
- **标签：** `agent-teams`, `dsh`, `multi-agent`
- **推荐理由：** 在DSH中实现多Agent团队协作，提升复杂任务效率。
...
- **标签：** `coding-agent`, `mcp`, `sub-agents`, `rpc`, `tui`
- **推荐理由：** 支持子Agent、hashline编辑与MCP集成的编码助手
...
- **标签：** `ios-simulator`, `uiautomation`, `xcode`, `mobile-automation`
- **推荐理由：** DSH内嵌iOS模拟器与真机调试，22个Agent工具实现移动端自动化
...
- **标签：** `provider`, `llm`, `model-access`
- **推荐理由：** 提供Command Code模型接入，支持智能选模
...
- **标签：** `skills-manager`, `dsh-plugin`, `security`
- **推荐理由：** DSH中统一加载和安全管理工作流技能
...
- **标签：** `cloud`, `mcp`, `sdk`, `huaweicloud`
- **推荐理由：** 华为云官方工具包：技能MCP沙箱安全防护
...
- **标签：** `code-simplification`, `codex`, `coding-assist`
- **推荐理由：** Codex技能自动识别并建议代码简化方案。
...
- **推荐理由：** 腾讯官方SkillHub技能中心API文档与插件示例
...
- **推荐理由：** 纯Lua编写的轻量Neovim插件管理器，快速简洁。
...
- **标签：** `agent-skills`, `ai-design`, `ui-generator`, `desktop-app`
- **推荐理由：** 本地优先的设计系统，AI代理的UI生成器
...
- **标签：** `sidebar`, `ui`, `extension-framework`
- **推荐理由：** 开放的侧边栏底座，内置文件渲染/终端/对话/Git/子代理页面
...
- **标签：** `taskboard`, `panel`, `ui`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH，实时状态显示
...
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏/实时思考/双击Esc回滚/上下文进度
...
- **标签：** `widget`, `balance`, `floating-ui`
- **推荐理由：** 可爱的小鲸鱼浮动组件，监控DSH账户余额，支持拖拽吸附动画
...
- **推荐理由：** 为纯文本模型提供OCR、多图问答、UI还原等视觉能力。
...
- **标签：** `desktop-app`, `electron`, `react`, `local-first`, `mcp`
- **推荐理由：** 本地优先的桌面工作区，集成DSH会话、项目与Office功能
...
- **标签：** `desktop-app`, `windows`, `electron`, `codex`, `ssh`
- **推荐理由：** 零配置Windows桌面客户端，内置Codex、SSH及插件扩展能力
...
- **标签：** `desktop-app`, `local-first`, `multi-model`, `privacy`, `tauri`
...
- **标签：** `office`, `spreadsheet`, `docs`, `desktop`
- **推荐理由：** 将表格/文档/演示/画布引入DSH，打造完整办公套件体验
...
- **标签：** `ios`, `mobile-client`, `websocket`, `native-app`
- **推荐理由：** DSH原生iOS客户端，WebSocket实时同步会话、工作区与Agent轨迹
...
- **标签：** `electron`, `desktop-app`, `codexpilot`, `cross-platform`
- **推荐理由：** CodePilot风格Electron桌面客户端，跨平台支持macOS/Win/Linux
...
- **推荐理由：** 安全P2P多端远程访问DSH，支持PC/Android/Web
...
- **推荐理由：** 开源锤子便签风格，支持Docker私有化与DSH插件
...
- **标签：** `marketplace`, `webui`, `plugins`
- **推荐理由：** 社区插件市场集成到DSH Web界面，一键安装
...
- **标签：** `menu`, `mcp`, `capability-management`, `ui`
...
- **推荐理由：** Electron桌面控制台，多Tab管理插件MCP和技能
...
- **标签：** `ui`, `info-bar`, `pricing`, `dashboard`
- **推荐理由：** 底部常驻栏显示模型余额和实时定价信息。
...
- **标签：** `bioinformatics`, `desktop`, `tauri`, `clinical`
- **推荐理由：** 面向临床的离线生物信息学桌面应用，含技能商店。
...
- **推荐理由：** 类似WorkBuddy的技能选择器，快速插入composer。
...
- **标签：** `kline`, `finance`, `visualization`, `sidebar`
- **推荐理由：** 侧栏交互式K线分析插件，支持技术指标与新闻资讯
...
- **推荐理由：** 一体化运维桌面客户端，集成数据库、SSH、Docker和AI助手。
...
- **推荐理由：** 基于Electron的DSH桌面化封装方案，集成多工具。
...
- **标签：** `ui-enhancement`, `webui`, `button`
- **推荐理由：** DSH Web上下文面板一键折叠按钮增强插件。
...
- **标签：** `agent-memory`, `rag`, `local-first`
- **推荐理由：** 本地优先的可移植记忆层，用户自主跨应用
...
- **标签：** `agent-memory`, `stateful`, `context-engineering`
- **推荐理由：** 构建有状态代理的专用记忆库
...
- **标签：** `memory`, `rag`, `context-management`
- **推荐理由：** Agent记忆管理套件，支持跨会话上下文记忆和知识精炼
...
- **推荐理由：** 持久统一Agent记忆层，基于Milvus向量搜索，支持所有AI代理
...
- **标签：** `memory`, `agent`, `RAG`, `long-term-memory`
- **推荐理由：** 本地记忆中心，让所有AI共享持久化上下文记忆。
...
- **标签：** `agent-memory`, `semantic-search`, `vector-search`, `skill-corpus`
- **推荐理由：** 将碎片化SKILL.md转化为可检索的Agent技能知识库
...
- **标签：** `multi-agent`, `agentic-workflow`, `mcp-server`, `skills`
- **推荐理由：** 多代理蜂群编排与自主工作流协调
...
- **标签：** `no-code`, `workflows`, `ai-agent`, `crm`
...
- **推荐理由：** 首个视觉插件，让纯文本编码Agent也��通过图像识别获取结构化证据
...
- **推荐理由：** 智能技能路由与工作流编排，+21.12pp奖励，-29.6%token消耗
...
- **标签：** `research`, `skill`, `web-search`, `agent`
- **推荐理由：** 自动搜索国内8大平台近30天内容，生成研究报告。
...
- **标签：** `writing`, `novel`, `agent-skill`, `ollama`
- **推荐理由：** 结构化小说创作流程，支持角色/世界观管理和Ollama本地推理。
...
- **推荐理由：** 免费无注册的通用Web搜索桥接插件，覆盖所有无原生搜索模型
...
- **标签：** `computer-vision`, `audio-understanding`, `ocr`, `multimodal`, `agent-observability`
- **推荐理由：** 赋予Agent视听能力，将视频音频屏幕活动转化为可验证证据
...
- **标签：** `creative-writing`, `workflow`, `novel`, `screenwriting`
- **推荐理由：** 内置网文/短剧/游戏/视频解说四种创作工作流，开箱即用
...
- **标签：** `task-management`, `sqlite`, `agent-workflow`, `native-ui`
- **推荐理由：** 原生本地任务板，SQLite持久化，Agent认领/评审，无iframe依赖
...
- **推荐理由：** 全栈数学建模竞赛Skill：AI自动驾驶+人类副驾，GitOps流水线支持
...
- **标签：** `vision`, `multimodal`, `ui-automation`
...
- **推荐理由：** 88个开源Agent技能，覆盖研究营销与自动化工作流
...
- **标签：** `ai-infra`, `skills`, `devops`
- **推荐理由：** 针对AI基础设施开发的专业编码优化调试技能
...
- **标签：** `agent-skills`, `automation`, `skill-generator`
- **推荐理由：** 自动将成功会话转化为可复用技能，提升效率
...
- **标签：** `superpowers`, `tdd`, `debugging`, `skills`
- **推荐理由：** TDD、调试与规划技能，源自obra/superpowers
...
- **标签：** `agent-skill`, `coding-agent`, `deepseek-harness`
- **推荐理由：** AI编程导师，阶梯式教学+自动判题+代码审查
...
- **标签：** `skills`, `agent`, `deepseek-harness`
- **推荐理由：** 25个工程技能开箱即用，侧边栏一键调用
...
- **⭐ 星标：** 51
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全��包含，克隆即用。
- **标签：** `gongwen`, `docx`, `agent-skill`
- **推荐理由：** 国标公文全流程处理：格式检查、模板生成、修订
...
- **标签：** `spec-driven`, `agent-skill`, `coding`
- **推荐理由：** 仓库原生规范驱动开发技能，适配DSH工程模式
...
- **标签：** `roleplay`, `narrative`, `npc`, `ai-agent`
- **推荐理由：** 沉浸式NPC角色扮演技能，含记忆与动态剧情存档。
...
- **标签：** `quality-gate`, `engineering`, `delivery`, `skill`
- **推荐理由：** 工程纪律技能包：审查需求测试实现确保交付。
...
- **推荐理由：** 支持跨Agent会话的上下文恢复，建立仓库级记忆连续性
...
- **标签：** `math-modeling`, `agent`, `skill`, `cumcm`, `mcm`
- **推荐理由：** 多Agent数学建模竞赛插件，含5角色+33模型库+论文模板
...
- **标签：** `agent-orchestration`, `multi-agent`, `session-memory`
- **推荐理由：** 持久化多Agent团队，支持私有记忆与跨会话身份
...
- **推荐理由：** 指导AI从零开发、构建、测试并安装DSH插件的Skill
...
- **推荐理由：** Godot 4.x全栈游戏开发Skill，支持GDScript
...
- **标签：** `video`, `remotion`, `motion-design`, `agent-skills`
- **推荐理由：** 电影感产品视频Agent，含152镜头配方卡与Remotion模板
...
- **标签：** `plugin-development`, `agent-skill`, `standards`
- **推荐理由：** 按官方标准开发DSH插件的Agent Skill，支持Claude Code/Codex
...
- **推荐理由：** 在DSH设置页管理技能与MCP服务器的Web GUI插件
...
- **标签：** `pets`, `ui-decoration`, `mascot`
- **推荐理由：** 为代理界面增添个性的动画宠物画廊
...
- **推荐理由：** AI生成设计系统，支持可视化编辑、模板市场与PPT制作。
...
- **推荐理由：** 在对话回复中直接渲染交互式UI组件：图表、表单、测验等
...
- **标签：** `video`, `ai-image`, `directorx`, `multimedia`
- **推荐理由：** AI视频图像音频创作套件，集成DirectorX。
...
- **标签：** `chatgpt`, `oauth`
- **推荐理由：** OAuth绑定ChatGPT，在DSH中使用订阅模型
...
- **标签：** `bootstrap`, `preset`, `standard-tools`
- **推荐理由：** 两阶段预配置：轻量对齐启动后切换全量标准工具，适合稳定运行
...
- **标签：** `marketplace`, `plugin-store`, `installation`
- **推荐理由：** DSH官方插件市场，支持搜索浏览和一键安装，生态核心入口
...
- **推荐理由：** 全���首个AI代理虚拟终端，支持沙箱化操作和虚拟文件系统
...
- **标签：** `installer`, `model-switch`, `multi-model`
- **推荐理由：** 一键安装和多模型切换，支持Claude/Codex/DeepSeek/Kimi/Qwen等
...
- **标签：** `documentation`, `code-to-doc`, `developer-tools`
- **推荐理由：** 几分钟内将代码转为清晰的技术文档和AI可用上下文
...
- **标签：** `plugin-registry`, `ecosystem`, `radar`
- **推荐理由：** 自部署插件雷达，自动发现并实测15900+候选插件。
...
- **标签：** `oauth`, `llm-provider`, `subscription`, `infrastructure`
- **推荐理由：** OAuth免密钥接入ChatGPT/Claude/Grok订阅，零配置即用
...
- **标签：** `cli`, `plugin-manager`, `schema`, `infrastructure`
- **推荐理由：** DSH插件市场开源CLI与Schema解析器，基础设施工具链
...
- **标签：** `cost-tracking`, `token-usage`, `budget`, `billing`
- **推荐理由：** 会话/每日成本追踪、预算预警、多来源Token用量统一管理
...
- **标签：** `config`, `backup`, `restore`, `sync`, `webdav`
- **推荐理由：** 完整配置备份恢复与WebDAV同步迁移工具
...
- **标签：** `knowledge-base`, `plugin-development`, `skill`
- **推荐理由：** 插件开发知识库，按需加载为Agent技能
...
- **标签：** `containers`, `local-management`, `devops`
- **推荐理由：** 容器隔离运行多个DSH版本，内嵌WebView界面。
...
- **推荐理由：** 一键同步MCP和技能到Claude/Codex/DSH等工具。
...
- **标签：** `launcher`, `infrastructure`, `cordis`, `orchestration`
- **推荐理由：** 统一编排Runtime、Provider、Plugin、Skill和MCP的本地AI平台
...
- **标签：** `cost-tracking`, `token-usage`, `data-visualization`
- **推荐理由：** 实时Token用量追踪与API费用估算工具。
...
- **标签：** `debugging`, `plugin-compatibility`, `diagnostics`
- **推荐理由：** 诊断插件间冲突并提供解决方案的实用工具。
...
- **标签：** `tts`, `xiaomi-mimo`, `audio`, `text-to-speech`
- **推荐理由：** 免费小米MiMo TTS语音合成插件，支持自定义声音。
...
- **标签：** `dsh-plugin`, `themes`, `desktop`
- **推荐理由：** 鲸鱼娘系列皮肤，为DSH提供个性化主题美化。
...
- **标签：** `theme`, `ocean`, `animated-background`, `glassmorphism`, `threejs`
- **推荐理由：** 实时海洋动态皮肤，波浪、日落与磨砂玻璃透明度可调
...
- **推荐理由：** DSH Web界面背景个性化插件，支持预设和自定义图片。
...
- **推荐理由：** 本地私有开源的跨平台AI内容发现Agent，自动从多平台搜集信息
...
- **推荐理由：** DSH生态 curated 插件、工具和基础设施精选列表。
...
- **标签：** `research`, `latex`, `gpu-ssh`, `experiment-tracking`
- **推荐理由：** 科研全周期工作台：LaTeX编译、arXiv管理、实验追踪、GPU远程
...
- **标签：** `finance`, `tushare`, `multi-agent`
- **推荐理由：** 基于Tushare的金融数据库，多Agent分析与IM盯盘
...
- **标签：** `research`, `industry`, `company-analysis`
- **推荐理由：** 产业链图谱与企业研究方法技能包
...
- **推荐理由：** 全面覆盖插件、工具、技能和MCP资源的Awesome列表
...
- **标签：** `marketplace`, `dsh-plugins`
- **推荐理由：** 类CodeX插件市场，便于探索DSH生态插件
...
- **标签：** `ai-curated`, `recommender`
- **推荐理由：** AI精选190+插件，12分类每小时更新
...
- **标签：** `verified`, `directory`
- **推荐理由：** 80+验证插件按功能分类，方便浏览
...
- **标签：** `ecosystem`, `analytics`
- **推荐理由：** DSH生态插件合集，含Bundle与分析标签
...
- **标签：** `awesome-list`, `deepseek-harness`, `auto-update`, `bilingual`
- **推荐理由：** 自动更新+双语README，结构化程度最佳。
...
- **标签：** `agent`, `dsh-plugin`, `marketplace`
- **推荐理由：** 每日更新的深度综合插件市场，一站式发现优质DSH插件
...
- **标签：** `awesome-list`, `curated`, `bilingual`, `developer-tools`
- **推荐理由：** 中英双语精选DSH插件与工具参考合集，开发者导航必备
...
- **标签：** `cybersecurity`, `redteam`, `vulnerability`, `ai-security`
- **推荐理由：** AI安全攻防平台，检测幻觉包与代码漏洞，保障AI生成代码安全
...
- **标签：** `ai-pedia`, `knowledge-base`, `awesome-list`, `r&d-resources`
- **推荐理由：** 一站式AI百科大全，涵盖技能、RAG、多模态工具与学习路线
...
- **标签：** `awesome-list`, `discovery`, `curation`, `plugin-marketplace`
- **推荐理由：** 自动抓取GitHub插件、剔除蹭标签项目、按热度精选推荐
...
- **标签：** `awesome-list`, `guide`, `quickstart`, `resource-aggregation`
- **推荐理由：** DSH终极指南：快速入门、资源聚合、精选插件与实用工具合集
...
- **标签：** `learning`, `marketplace`, `best-practices`, `documentation`
- **推荐理由：** DSH原理学习、插件市场探索与最佳实践文档一站式入口
...
- **标签：** `awesome-list`, `mcp`, `plugin-curation`, `patch-layer`
- **推荐理由：** 精选插件/技能/MCP/补丁层/编排器/UI集合，可视化PPT友好展示
...
- **标签：** `awesome-list`, `marketplace`, `catalog`
- **推荐理由：** 包含11,000+插件的DSH市场，支持搜索和免费API
...
- **标签：** `handbook`, `runtime`, `documentation`
- **推荐理由：** 全面的DSH手册，覆盖运行时、插件、MCP和评估
...
- **推荐理由：** 双语DSH插件目录，1958个仓库支持分类筛选排序
...
- **标签：** `awesome-list`, `official`, `catalog`
...
- **标签：** `awesome-list`, `tools`, `resources`
- **推荐理由：** 精选DSH工具与扩展资源索引列表
...
- **标签：** `awesome-list`, `plugin-marketplace`, `directory`
...
- **标签：** `awesome-list`, `dsh-plugins`, `plugin-registry`
- **推荐理由：** 官方反垃圾插件注册表，社区维护的精选插件库。
...
- **标签：** `awesome-list`, `curated-list`, `dsh-plugins`
- **推荐理由：** 2026年度精选DSH插件列表，含质量审核。
...
- **标签：** `awesome-list`, `dsh-plugin`, `tool-directory`
- **推荐理由：** 收录1000+ DSH工具的全面清单，按星标排序。
