...
- **标签：** `dsh-plugin`, `coding-agent`, `tui`
- **推荐理由：** DeepSeek原生终端编程代理，前缀缓存稳定
...
- **标签：** `router`, `routing`, `harness-tools`
- **推荐理由：** 任务感知推理模式路由器，增强DSH调度能力
...
- **标签：** `code-review`, `static-analysis`, `agent-skills`, `deepseek-harness`
- **推荐理由：** 基于12本经典工程著作的AI代码审查，带严重性诊断
...
- **标签：** `reasonix`, `superpowers`, `skills`
- **推荐理由：** Superpowers技能移植到Reasonix编码编排器
...
- **标签：** `plugin-dev`, `tooling`, `audit`
- **推荐理由：** 便携式DSH插件开发审计技能，含安装适配器
...
- **推荐理由：** DSH插件开发踩坑文档，涵盖cordis/tsconfig/Windows注意事项
...
- **标签：** `agent-tool`, `pre-flight`, `plugin-injection`
- **推荐理由：** 任务预检自动注入适用插件，提升Agent能力。
...
- **标签：** `dsh-plugin`, `browser-extension`, `prompt-manager`
- **推荐理由：** 面向主流AI聊天平台的浏览器增强套件
...
- **标签：** `sidebar`, `deepseek-harness`, `ui-extension`
- **推荐理由：** 可扩展侧边栏底座，内置文件/终端/对话/Git等页面
...
- **标签：** `agent-runtime`, `desktop`, `rust`, `computer-use-agent`
- **推荐理由：** Rust高性能Agent运行时+精美桌面应用，一体化体验
...
- **标签：** `desktop`, `client`, `dsh-plugin`
- **推荐理由：** 跨平台桌面客户端，仿CodePilot体验。
...
- **标签：** `electron`, `desktop-app`, `gui`
- **推荐理由：** 功能完善的Windows桌面客户端，支持SSH和移动端远程。
...
- **标签：** `console`, `plugin-management`
- **推荐理由：** 浏览器面板插件管理控制台及开发引导
...
- **推荐理由：** Electron桌面端Harness/插件/MCP管理控制台
...
- **推荐理由：** Tauri 2桌面应用整合数据库、SSH、Docker与AI
...
- **推荐理由：** Electron封装DSH Web并提供侧边栏集成
...
- **标签：** `background`, `webui`, `customization`
- **推荐理由：** 支持自定义图片上传与氛围预设的DSH背景插件。
...
- **标签：** `memory`, `token-savings`, `hybrid-retrieval`
...
- **推荐理由：** 专注Agent记忆管理的RAG知识库套件，功能明确
...
- **标签：** `agent-memory`, `milvus`, `embeddings`, `rag`, `long-term-memory`
- **推荐理由：** 基于Milvus向量库的持久化统一Agent记忆层
...
- **标签：** `knowledge`, `r&d`, `dsh-plugin`
- **推荐理由：** AI技能百科大全，覆盖Agent/RAG/MLOps。
...
- **标签：** `agent-memory`, `skill-rag`, `semantic-search`
- **推荐理由：** 将零散SKILL.md文件整理为可检索的智能体技能库。
...
- **标签：** `dsh-plugin`, `skills`, `workflow-automation`
- **推荐理由：** 530+ GPT图像提示词案例提炼为技能
...
- **标签：** `workspace`, `orchestration`, `cross-device`
- **推荐理由：** 统一代理工作区，跨设备任务看板追踪
...
- **推荐理由：** 基于Anthropic J-space全局工作空间研究的AI认知增强技能
...
- **标签：** `prompt`, `skill`, `dsh-plugin`
- **推荐理由：** DeepSeek破甲提示词稳定化插件。
...
- **标签：** `agent`, `skills`, `execution`
- **推荐理由：** 将AI经验转化为可重复执行的技能工作流。
...
- **标签：** `task-management`, `taskboard`, `deepseek-harness`
...
- **标签：** `agent-skills`, `research-tools`
- **推荐理由：** 88个研究、社交情报与营销技能
...
- **标签：** `spec-programming`, `dsh-plugin`
- **推荐理由：** 仓库级规范驱动开发便携技能
...
- **推荐理由：** 官方DSH插件开发知识库，按需加载为agent技能
...
- **标签：** `multi-agent`, `team`, `dsh-plugins`
...
- **标签：** `skills-manager`, `dsh-plugin`, `deepseek`
- **推荐理由：** DSH技能管理专用插件，集中管理agent技能
...
- **标签：** `swift-cycle`, `agent-skills`, `governance`
...
- **标签：** `vision`, `image-generation`, `skill`
- **推荐理由：** 为DSH增加图像读取与生成能力，开箱即用
...
- **标签：** `document`, `gongwen`, `word`
- **推荐理由：** 中文公文全流程处理，支持格式检查、修订对比与版头注入
...
- **标签：** `package-manager`, `agent-skills`, `cli`
- **推荐理由：** AI工程资产包管理器，42项技能+5个CLI，覆盖多种开发场景
...
- **标签：** `gamification`, `skills`, `mission`
- **推荐理由：** DSH技能游戏化任务系统，迷雾地图+任务栏推进
...
- **标签：** `godot`, `game-development`
- **推荐理由：** 为DSH代理注册Godot 4.x全栈游戏开发技能
...
- **标签：** `search`, `web`, `dsh-plugin`
- **推荐理由：** 最强免费网页搜索插件，无需API注册。
...
- **推荐理由：** 配置页粘贴JSON即连MCP，热加载免重启，技能即点即用
...
- **标签：** `bilibili`, `xiaohongshu`, `douyin`, `content-discovery`, `agent`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站/小红书/抖音等8大平台
...
- **标签：** `dingtalk`, `chat-bot`, `ai-agent`, `dsh-plugin`
- **推荐理由：** 钉钉IM通道，聊天即调用工具的智能体。
...
- **标签：** `deepseek-harness`, `claude-code`, `opencode`, `installation`
- **推荐理由：** 一键安装+模型切换，支持Claude/Codex/Grok/DeepSeek等多工具
...
- **推荐理由：** 开源锤子便签，支持Docker部署和公众号一键导出
...
- **标签：** `marketplace`, `plugin-market`, `deepseek-harness`
- **推荐理由：** 在DSH Web GUI中一键浏览安装GitHub插件市场
...
- **标签：** `context-migration`, `session-migration`, `deepseek-harness`
- **推荐理由：** 跨预设会话迁移，保留状态和上下文信息
...
- **标签：** `awesome-list`, `plugin-curation`, `deepseek-harness`
- **推荐理由：** 精心整理的DSH插件 awesome 列表目录
...
- **标签：** `market`, `plugin-market`, `webui`
- **推荐理由：** 社区插件市场，一键安装awesome-dsh-plugin插件
...
- **标签：** `config-manager`, `backup`, `restore`, `mcp`
- **推荐理由：** DSH完整配置备份还原，支持迁移同步
...
- **标签：** `dsh-plugin`, `mcp`, `capabilities`
...
- **标签：** `migration`, `agent-migration`
- **推荐理由：** 无缝迁移Claude Code等配置到DSH环境
...
- **标签：** `token-usage`, `cost-tracking`
- **推荐理由：** 实时追踪token用量、费用图表和API余额
...
- **标签：** `plugin-diagnostic`, `conflict-resolution`
- **推荐理由：** 诊断并解决插件与版本冲突，保障系统稳定。
...
- **标签：** `pets`, `mascot`, `theming`
- **推荐理由：** 动画宠物画廊，为编码代理增添个性化体验
...
- **标签：** `chrome-extension`, `theme`, `webgpu`
- **推荐理由：** 基于WebGPU的深海主题视觉皮肤，炫酷美观。
...
- **标签：** `research`, `web-search`, `agent-skill`, `chinese-platforms`
- **推荐理由：** 自动搜索8大中文平台近30天内容生成有据可查研究报告
...
- **标签：** `ai-agents`, `chinese-api`, `dsh-plugin`
- **推荐理由：** 跨工具搜索6.5万篇中国平台API文档
...
- **标签：** `awesome-deepseek-harness`
- **推荐理由：** cordis.run官方自动维护的精选插件目录
...
- **标签：** `kline`, `finance`, `analysis`, `deepseek-harness`
- **推荐理由：** 交互式K线分析，支持多市场指标和基本面
...
- **推荐理由：** 面向提示词/RAG/Agent的证据驱动评估框架，支持多裁判集成
...
- **标签：** `tutorial`, `education`, `beginner`
- **推荐理由：** DSH入门教程，从零开始学习引擎工程
...
- **标签：** `curated-list`, `discovery`, `directory`
- **推荐理由：** 精选插件目录，方便探索DSH生态
...
- **标签：** `toy`, `simulation`, `single-file`, `web`
- **推荐理由：** 零依赖单文件，真实录音采样的传统竹知了Web模拟版
...
- **推荐理由：** 精选插件雷达，覆盖9000+候选和Top 50优质插件
...
- **标签：** `awesome-list`, `curated`, `infrastructure`
- **推荐理由：** DSH插件和基础设施全面精��列表
...
- **推荐理由：** 含3100+插件的DSH插件市场，支持搜索和API。
...
- **标签：** `android`, `emulator`, `device-stream`
- **推荐理由：** 在对话中与Android模拟器或真机交互操作
...
- **标签：** `community`, `oauth`, `plugin`
- **推荐理由：** DSH内测合影站，OAuth零权限登录，趣味社区插件
...
- **标签：** `stock`, `quant`, `awesome-list`
- **推荐理由：** 精选股票/金融/量化插件专题清单，双语支持。
...
- **标签：** `guide`, `plugin-navigator`
- **推荐理由：** 插件使用指南与导航，涵盖安装与安全说明。
...
- **标签：** `plugin-radar`, `2origin`
- **推荐理由：** 2Origin实验室出品的插件雷达，有据可查。
...
- **推荐理由：** 已迁移至awesome-dsh-plugin/awesome-dsh-plugin。
...
- **标签：** `curated-list`, `dsh-plugin`, `ui`, `skills`
- **推荐理由：** 覆盖模型、工具、UI等全维度DSH插件。
