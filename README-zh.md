...
- **标签：** `ai-agent`, `deepseek`, `dsh-plugin`, `skills`
- **推荐理由：** 多模型多通道超级AI助手，具备自进化记忆能力
...
- **标签：** `coding-agent`, `deepseek`, `dsh-plugin`, `tui`
- **推荐理由：** DeepSeek原生终端编码Agent，支持前缀缓存优化
...
- **标签：** `loop-engineering`, `agent-control`, `codex`
- **推荐理由：** 长周期Agent控制平面，持久化多Agent工作流
...
- **标签：** `cybersecurity`, `redteam`, `vulnerability`
- **推荐理由：** AI安全攻防平台，审计AI生成代码中的漏洞和幻觉包。
...
- **标签：** `ctf`, `reverse-engineering`, `pentest`, `security`
- **推荐理由：** 87个逆向工程技能，用于CTF与渗透测试。
...
- **标签：** `plugin-dev`, `agent-skills`, `tools`
- **推荐理由：** 开发DSH插件的Agent Skill，提供插件/事件/工具标准指南
...
- **标签：** `plugin-dev`, `agent-skills`, `utilities`
- **推荐理由：** 管理已安装DSH插件、槽位、远程凭据的Agent Skill
...
- **标签：** `agent-tools`, `plugin-selection`
- **推荐理由：** Agent插件库起飞前检查，让插件选择可预期
...
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内直接使用Plus模型
...
- **标签：** `dsh-plugin`, `desktop`, `deepseek-harness`
...
- **标签：** `web-ui`, `plugin-aggregation`, `dsh`
- **推荐理由：** DSH插件生态Web UI，一切皆插件
...
- **标签：** `taskboard`, `ui-extension`, `codex`
- **推荐理由：** 现代化可嵌入任务面板，灵活集成Codex/DSH
...
- **推荐理由：** 本地优先的Electron桌面工作区，集成文件与研究
...
- **标签：** `desktop-app`, `tauri`, `multi-model`, `mcp`
- **推荐理由：** 本地优先的多模型AI Agent桌面应用，开源Claude Cowork替代。
...
- **推荐理由：** 跨平台桌面客户端+插件套件，灵感来自CodePilot。
...
- **标签：** `desktop-app`, `windows`, `ssh-client`, `gui`
- **推荐理由：** 零安装Windows桌面客户端，内置Codex、SSH、远程访问。
...
- **标签：** `office`, `dsh-plugin`, `desktop`
- **推荐理由：** 将完整办公套件引入DSH，功能全面
...
- **推荐理由：** Electron桌面控制台，多标签管理Harness/插件/MCP/Skills
...
- **推荐理由：** 集成数据库、SSH、Docker和AI的一体化桌面。
...
- **推荐理由：** Electron桌面包装器，集成ModLens与sidebar
...
- **标签：** `agent-memory`, `dsh-plugin`, `self-evolving`, `agentic-rag`
...
- **标签：** `dsh-plugin`, `rag`, `vector-search`, `knowledge-base`
- **推荐理由：** 将文���转化为可查询RAG和自维护知识库
...
- **标签：** `agent-memory`, `rag`, `markdown-native`
- **推荐理由：** 本地优先的跨应用记忆层，可移植自进化
...
- **标签：** `memory`, `RAG`, `agent`
- **推荐理由：** 个人AI Agent和本地记忆中枢，跨Agent持久共享
...
- **标签：** `agent-memory`, `skill-md`, `vector-search`, `reranking`
...
- **标签：** `notes`, `smartisan`, `docker`, `plugin`
- **推荐理由：** 锤子便签开源版，支持Skill调用与导出。
...
- **标签：** `agentic-ai`, `multi-agent`, `dsh-plugin`, `mcp-server`
- **推荐理由：** 部署智能多Agent群，协调自主工作流
...
- **标签：** `agent-workspace`, `multi-device`, `orchestration`
- **推荐理由：** 全端统一Agent工作台，看板任务管理
...
- **标签：** `routing`, `agent-ops`, `cordis`
- **推荐理由：** 任务感知路由套件，显著提升推理准确率
...
- **标签：** `agent-skills`, `workflow`, `skill-routing`
- **推荐理由：** 智能技能路由与工作流编排，基准测试显著提升
...
- **标签：** `cognitive-enhancement`, `agent-skills`, `j-space`
- **推荐理由：** 基于J-space研究的AI认知增强技能套件
...
- **标签：** `video-generation`, `tts`, `agent-skill`
- **推荐理由：** 从主题自动生成Vox风格解说视频的端到端技能
...
- **推荐理由：** SQLite驱动的原生任务看板，支持Agent认领审查工作流。
...
- **标签：** `creative-writing`, `dsh-plugin`, `skills`
- **推荐理由：** 小说与短剧创作技能，AI写作利器
...
- **标签：** `agent-review`, `self-evolution`, `dsh-plugin`
- **推荐理由：** Agent可组合可审查的自我进化模式
...
- **标签：** `skills`, `multi-agent`, `cross-framework`
- **推荐理由：** 跨框架Agent技能平台，经验复用
...
- **推荐理由：** 移植Superpowers的TDD/调试/协作技能。
...
- **标签：** `reasonix`, `skills`, `agent-skills`
- **推荐理由：** 将 Superpowers 技能移植到 Reasonix 编码Harness，扩展AI能力
...
- **推荐理由：** 88个开源智能体技能，覆盖研究、营销与业务自动化工作流
...
- **标签：** `agent-skills`, `coding-agent`, `education`
- **推荐理由：** AI编程导师技能，扫描架构、生成章节、自动代码审查
...
- **标签：** `chinese-traditional-wisdom`, `skill`, `dsh-plugin`
- **推荐理由：** 融合八字/风水/易经/中医的传统智慧技能包
...
- **标签：** `agent-skills`, `deepseek-harness`, `swift-cycle`
- **推荐理由：** 支持离线验证的DSH治理技能适配器。
...
- **标签：** `agent-preset`, `workflow`, `product-management`
- **推荐理由：** 模拟虚拟产品团队对话流程，涵盖PM到QA
...
- **推荐理由：** MCP服务器，集成20个中国开放平台65600+篇API文档，零配置
...
- **标签：** `agent-skills`, `ai-design`, `dsh-plugin`, `figma-alternative`
- **推荐理由：** 开源类Claude设计工具，本地优先桌面端
...
- **标签：** `agent-skills`, `architecture-diagram`, `dsh-plugin`, `diagrams-as-code`
- **推荐理由：** 生成美观的架构图、时序图和数据流图
...
- **标签：** `ai-image-generation`, `dsh-plugin`, `prompt-engineering`, `skills`
- **推荐理由：** 工业级GPT-Image2提示词引擎，530+逆向案例
...
- **标签：** `design`, `vector`, `fimga`, `vibedesign`
- **推荐理由：** 首个开源AI矢量设计工具，支持Agent团队协作
...
- **推荐理由：** Gemini/Claude/ChatGPT浏览器增强套件
...
- **标签：** `web-automation`, `content-discovery`, `bilibili`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、抖音、YouTube等
...
- **推荐理由：** 让AI Agent无缝控制已登录浏览器，不中断工作流
...
- **推荐理由：** 官方 Tabbit 浏览器自动化插件，基于 Playwright，适合 Web 任务
...
- **标签：** `communication`, `dingtalk`
- **推荐理由：** 钉钉IM集成，将聊天转为工具型智能体对话。
...
- **标签：** `dsh-plugin`, `mcp-server`, `self-hosted`, `typescript`
- **推荐理由：** 隐私优先的自托管简历构建器，完全可定制
...
- **标签：** `dsh-plugin`, `low-code`, `self-hosted`, `workflows`
...
- **标签：** `virtual-filesystem`, `agent-tools`, `vfs`
- **推荐理由：** 统一虚拟文件系统，跨Agent无缝文件操作
...
- **标签：** `model-switch`, `installer`, `multi-agent`
- **推荐理由：** 一键安装并切换Claude Code、Codex等主流工具
...
- **标签：** `marketplace`, `plugin-store`, `dsh`
- **推荐理由：** 官方插件市场，支持浏览搜索和一键安装
...
- **标签：** `armor-breaking`, `jailbreak`, `utility`
- **推荐理由：** DeepSeek专用稳定化破甲插件
...
- **推荐理由：** 通过OAuth将订阅账号转为DSH LLM提供商，免API密钥。
...
- **标签：** `cost-tracking`, `dsh-plugin`, `utility`
- **推荐理由：** 会话成本追踪与预算管理，控制API开销必备
...
- **标签：** `marketplace`, `plugin-marketplace`, `javascript`, `nodejs`
...
- **标签：** `backup`, `rollback`, `snapshot`, `windows`, `crash-recovery`
- **推荐理由：** 崩溃恢复工具，支持回滚与快照。
...
- **标签：** `awesome`, `cordis`, `dsh-plugin`
- **推荐理由：** cordis.run官方自动生成的DSH插件清单。
...
- **标签：** `mcp`, `dsh-plugin`, `utilities`
- **推荐理由：** 统一管理MCP工具暴露级别和执行模式，简化DSH配置
...
- **推荐理由：** 支持WebDAV同步的DSH配置备份与恢复工具，含插件与MCP
...
- **推荐理由：** 插件生态基建控制台，浏览器面板管理官方仓库，零补丁
...
- **标签：** `dsh-plugins`, `container`, `management`
- **推荐理由：** 隔离容器中运行多版本DSH，内嵌UI管理
...
- **标签：** `migration`, `configuration-migration`, `mcp`
...
- **标签：** `marketplace`, `distribution`
- **推荐理由：** 类CodeX插件市场，DSH生态分发基础设施
...
- **⭐ 星标：** 396
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂���、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一行源码。
- **标签：** `theme`, `ui`, `dsh-plugin`
- **推荐理由：** 磨砂玻璃主题，高度可自定义，美化DSH网页界面。
...
- **标签：** `theme`, `skin`, `chrome-extension`
- **推荐理由：** 惊艳的WebGPU海洋动态主题，颜值高
...
- **标签：** `theme`, `ui-customization`, `webui`
- **推荐理由：** Web界面背景个性化，支持自定义与预设氛围
...
- **标签：** `ai-resources`, `tutorials`, `prompt-library`
- **推荐理由：** AI资源大全，含教程和提示词，全面实用
...
- **标签：** `awesome-list`, `resources`, `research`
- **推荐理由：** 深度挖掘的DSH生态资源精选清单
...
- **标签：** `awesome-list`, `bilingual`, `research`
- **推荐理由：** 中英双语DSH插件精选目录与资源索引
...
- **标签：** `knowledge-base`, `ai-resources`, `learning`
- **推荐理由：** 一站式AI资源百科，涵盖Agent、RAG、MLOps等全维度内容。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `learning`
- **推荐理由：** DSH学习与最佳实践指南，新手必备
...
- **标签：** `marketplace`, `dsh-plugin`, `directory`
- **推荐理由：** 万款插件市场含搜索API，发现工具
...
- **标签：** `catalog`, `awesome-list`, `directory`
- **推荐理由：** 万级插件目录含中英搜索与分类筛选
...
- **标签：** `academic-search`, `mcp`, `arxiv`, `pubmed`, `citation`
- **推荐理由：** 跨库学术检索，支持引文核验与图谱生成。
...
- **标签：** `arxiv`, `latex`, `paper-writing`, `gpu-cluster`, `ssh`
- **推荐理由：** 一站式科研工作台：LaTeX/arXiv/GPU实验编排。
...
- **标签：** `patent`, `rag`, `legal-tech`, `research`
- **推荐理由：** 专利侵权分析系统，输入专利号生成竞品侵权分析报告
...
- **标签：** `industry-research`, `research`, `dsh-plugin`
- **推荐理由：** 行业与公司研究领域专业工具包
...
- **标签：** `awesome-list`, `plugin-discovery`, `dsh`
- **推荐理由：** 精选插件列表，发现优秀插件的最佳入口
...
- **标签：** `anime`, `bangumi`, `magnet`, `typescript`
...
- **推荐理由：** 面向 DeepSeek Harness Web GUI 的正式插件包：在设置页的「Web UI 插件」分组中新增一张「技能与 MCP」卡片，用于在浏览器里管理技
...
- **推荐理由：** Free image reading & generation for DeepSeek Harness (rc.7 / rc.8 / v0.1.1-rc.1 
...
- **推荐理由：** OMK — Evidence-backed evaluation and observability for prompts, RAG, skills, age
...
- **推荐理由：** GitHub dsh-plugin生态1000+仓库按星排序大全
...
- **推荐理由：** GitHub Actions每日自动爬取DSH插件。
...
- **标签：** `awesome-list`, `auto-update`
- **推荐理由：** 数据驱动自动更新，双语README+Vercel部署。
