...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 191,783 | The core DSH framework — everything is a plugin. |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 91,075 | Open-source design tool with local-first desktop a |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,289 | Multi-agent swarm orchestration with autonomous wo |
...
- **标签：** `dsh-plugin`, `ai-agents`, `harness`
- **推荐理由：** DSH核心框架，万物皆插件。
...
- **标签：** `multi-agent-workspace`, `enterprise`, `agent-collaboration`
- **推荐理由：** 企业级多Agent工作台，统一Codex/DSH/Hermes/OpenCode。
...
- **推荐理由：** 两阶段预设配置，最小化引导后达到Project2高分基准。
...
- **标签：** `claude-code`, `deepseek-harness`, `model-switch`, `dsh-plugin`
- **推荐理由：** 一站式安装和模型切换工具，支持所有主流编码Agent。
...
- **标签：** `marketplace`, `dsh-plugin`, `installation`
- **推荐理由：** DSH官方插件市场，可视化浏览和一键安装。
...
- **标签：** `agent-runtime`, `rust`, `coding-agent`, `desktop`
...
- **标签：** `code-review`, `static-analysis`, `clean-architecture`, `refactoring`
...
- **标签：** `agent-skills`, `dsh-plugin`, `plugin-development`, `skills`
- **推荐理由：** 按官方标准开发DSH插件的Agent技能指南
...
- **标签：** `agent-skill`, `plugin-development`, `knowledge-base`, `dsh-plugin`
- **推荐理由：** 即装即用的DSH插件开发知识库技能包
...
- **推荐理由：** 通过OAuth在DSH代理中使用ChatGPT模型。
...
- **标签：** `ui-generator`, `desktop-app`, `dsh-plugin`
- **推荐理由：** 开源设计工具，本地优先桌面应用，支持UI生成。
...
- **标签：** `dsh-web`, `dsh-plugin`, `marketplace`
- **推荐理由：** Web端DSH插件聚合商城，一站式发现所有创意插件。
...
- **标签：** `sidebar`, `ui`, `dsh-plugin`, `terminal`
- **推荐理由：** 开放的侧边栏底座，内置文件编辑、终端、对话和Git视图。
...
- **推荐理由：** Claude Code风格终端UI，含鲸鱼顶栏、流式思考和回滚功能。
...
- **标签：** `widget`, `floating`, `balance`, `desktop`
- **推荐理由：** 漂浮鲸鱼小部件，实时显示账户余额
...
- **标签：** `desktop-app`, `agent`, `tauri`
- **推荐理由：** 本地优先的多模型桌面Agent，支持自进化技能。
...
- **推荐理由：** 功能完整的Windows桌面GUI，支持SSH和移动端访问
...
- **标签：** `ui`, `plugin-management`, `console`, `dsh-plugin`
- **推荐理由：** DSH插件管理控制台与开发引导工具
...
- **推荐理由：** Electron desktop console for multi-tab DSH management
...
- **标签：** `dsh-plugin`, `canvas`, `mcp-tools`, `skills`
- **推荐理由：** 无限画布配合创意技能，增强DSH交互体验。
...
- **标签：** `deepseek-harness`, `desktop`, `ssh`, `docker`, `tauri`
...
- **标签：** `deepseek-harness`, `webui`, `plugin`
- **推荐理由：** 为DSH Web提供自定义背景与预设主题。
...
- **推荐理由：** 本地优先的持久记忆层，Markdown原生，跨工具自演进。
...
- **推荐理由：** 自演进记忆系统，混合检索，节省35%以上token消耗。
...
- **推荐理由：** 专为Agent设计的记忆管理套件，支持RAG持久化上下文。
...
- **推荐理由：** 基于Milvus向量数据库的持久统一Agent记忆层。
...
- **标签：** `rag`, `embeddings`, `skill-retrieval`
- **推荐理由：** 将SKILL.md转化为可检索的agent技能知识库
...
- **标签：** `memory`, `rag`, `long-term`
- **推荐理由：** 持久可 inspected 长期记忆，支持回忆工具
...
- **标签：** `deepseek-harness`, `memory`, `recall`, `handoff`
- **推荐理由：** 连接PowerContext实现记忆召回与知识传递。
...
- **推荐理由：** 统一多端Agent工作台，桌面/手机/浏览器全平台覆盖。
...
- **推荐理由：** 任务感知推理路由套件，支持注入器优化Agent调度。
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `j-space`, `ai-agents`
- **推荐理由：** 基于J-space全局工作空间研究的认知增强Agent技能套件。
...
- **标签：** `agent-skills`, `workflow-automation`, `context-engineering`, `multi-agent`
- **推荐理由：** 智能技能路由和流程编排工具，实现Harness自动化。
...
- **推荐理由：** 从Claude Code/Codex调度DSH Agent，支持子Agent追踪
...
- **推荐理由：** TDD、调试、规划与协作技能移植自obra/superpowers
...
- **推荐理由：** AI编程导师，带阶梯式学习与自动Code Review
...
- **标签：** `agent-skills`, `legal`, `visualization`, `timeline`, `svg`
- **推荐理由：** 诉讼可视化时间轴工具，法律人必备
...
- **标签：** `agent-skills`, `spec-driven-development`, `dsh-plugin`, `vibe-coding`
- **推荐理由：** 仓库原生规格驱动开发的便携Agent技能
...
- **推荐理由：** 版本锁定Swift Cycle治理技能，可离线验证
...
- **标签：** `skills`, `gamification`, `tasks`
- **推荐理由：** 游戏化任务系统，迷雾探索加进度追踪
...
- **标签：** `practices`, `engineering`, `skills`
- **推荐理由：** 工程纪律技能：事实导向与可靠决策
...
- **标签：** `deepseek-harness`, `mcp-server`, `skill-management`, `hot-reload`
- **推荐理由：** 粘贴JSON即连MCP，热加载免重启，即点即用。
...
- **标签：** `deepseek-harness`, `godot`, `game-development`, `gdscript`
- **推荐理由：** Godot 4.x全栈游戏开发技能插件，支持DSH。
...
- **标签：** `deepseek-harness`, `agent-skills`, `skill-generator`, `local-first`
...
- **标签：** `agent-tools`, `plugin-library`, `deepseek-harness`
- **推荐理由：** 插件预检清单，提升代理能力匹配。
...
- **推荐理由：** 首个统一虚拟文件系统，为AI Agent提供持久共享存储。
...
- **标签：** `cost-tracking`, `token-usage`, `utility`
- **推荐理由：** 追踪DSH会话/日成本、预算和Token用量
...
- **推荐理由：** 崩溃恢复与配置撤销，密钥安全快照一键SAFE MODE
...
- **标签：** `taskboard`, `sqlite`, `ui`
- **推荐理由：** 原生SQLite任务看板，支持Agent协作与审查
...
- **标签：** `api-documentation`, `chinese-api`, `dsh-plugin`, `ai-agents`
- **推荐理由：** 零配置搜索65,600+中国开放平台API文档
...
- **标签：** `dsh-plugin`, `ocr`, `vision`, `image-reading`
- **推荐理由：** 纯文本模型的图像OCR与像素读取工具
...
- **标签：** `dsh-plugin`, `plugin-manager`, `mcp`, `hot-reload`
- **推荐理由：** 内嵌GUI管理MCP/Skills/插件，热加载免重启。
...
- **标签：** `deepseek-harness`, `token-usage`, `cost`, `data-visualization`
- **推荐理由：** 实时Token用量、费用图表及API余额监控。
...
- **标签：** `plugin`, `conflict-resolution`, `deepseek-harness`
- **推荐理由：** 诊断并解决DSH插件冲突问题。
...
- **推荐理由：** 提供动效宠物吉祥物，为编码Agent增添个性化视觉体验。
...
- **标签：** `theme`, `skin`, `whale`, `dsh-plugin`
- **推荐理由：** 鲸鱼娘主题皮肤系列，为DSH提供可爱视觉定制。
...
- **⭐ 星标：** 14
- **语言：** JavaScript
- **描述：** 🎨 看腻了 DSH 默认皮肤？34 款 opencode 经典配色一键���上——tokyonight、dracula、gruvbox、matrix、rose-pine……即点即换，重启不丢。34 opencode themes for DeepSeek Harness, one click, persisted. More by @FeatherHunter: ⚡ dsh-prompt · 🧠 dsh-mattpocock-skills-deck
...
- **标签：** `bilibili`, `content-discovery`, `ai-agent`, `cross-platform`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、抖音、小红书等中外平台。
...
- **推荐理由：** 自动搜索中国8大主流平台近30天内容，生成有据可查报告。
...
- **标签：** `awesome-list`, `deepseek-harness`, `plugin-catalog`
- **推荐理由：** DSH生态插件、技能、MCP服务器精选索引目录
...
- **推荐理由：** cordis.run精选维护的自动生成DSH插件清单
...
- **标签：** `evaluation`, `rag-evaluation`, `llm-evaluation`
- **推荐理由：** 证据驱动的提示词、RAG和智能体评估框架
...
- **标签：** `awesome-list`, `finance`, `stock`, `quant`
- **推荐理由：** 精选24个股票/金融/量化插件，双语收录。
...
- **推荐理由：** 传统竹蜻蜓玩具的Web模拟版，零依赖单文件，真实录音采样。
...
- **推荐理由：** 9000+候选插件雷达，精选Top50，定期自动更新
...
- **标签：** `education`, `marketplace`
- **推荐理由：** 学习DSH原理及插件市场最佳实践。
...
- **标签：** `marketplace`, `community`, `webui`
- **推荐理由：** 社区插件市场，一键安装卸载DSH插件
...
- **标签：** `ai-agent`, `chinese-traditional-wisdom`
- **推荐理由：** 中华传统智慧AI技能包
...
- **标签：** `ai-agent`, `awesome`, `curated-list`, `deepseek-harness`
- **推荐理由：** 2026年DSH插件精选列表，经质量检查。
...
- **标签：** `verified`, `community-plugins`, `deepseek-harness`
- **推荐理由：** 按星标排序的已验证社区插件。
...
- **标签：** `plugin-directory`, `guide`, `deepseek-harness`
- **推荐理由：** 插件导航器，含安装指南与兼容性说明。
...
- **标签：** `plugin-radar`, `2Origin`, `deepseek-harness`
- **推荐理由：** 基于证据的DSH插件雷达与实验室。
...
- **标签：** `plugin-portal`, `static-site`, `deepseek-harness`
- **推荐理由：** 静态网页门户，浏览筛选DSH插件。
...
- **标签：** `redirect`, `meta`
- **推荐理由：** awesome-dsh-plugins仓库重定向通知。
...
- **标签：** `ai-curated`, `plugin-list`, `deepseek-harness`
- **推荐理由：** AI策展的综合插件列表，每小时更新。
