...
- **推荐理由：** 原生DeepSeek终端编码代理，前缀缓存稳定性强
...
- **标签：** `harness`, `dsh-plugin`, `agent`
- **推荐理由：** 从零到一掌握Harness工程的最佳入门教程
...
- **推荐理由：** 运行时注入器与任务感知路由套件，优化Agent推理
...
- **推荐理由：** 企业级多引擎Agent工作台，支持可视化编辑与团队协��
...
- **标签：** `preset`, `harness`, `standard`
- **推荐理由：** 两阶段预设：极简启动加全面Standard工具配置
...
- **标签：** `model-switch`, `dsh`, `coding-agent`
- **推荐理由：** 一键安装并切换多款 AI 编码代理和 CLI 工具
...
- **推荐理由：** Rust 高性能 Agent 运行时与精美桌面应用结合
...
- **标签：** `code-review`, `agent-skills`, `static-analysis`
- **推荐理由：** 基于12本经典工程的AI代码审查，带引用和严重度标签。
...
- **推荐理由：** Web端插件聚合生态，Creative Workshop分发平台
...
- **标签：** `task`, `ui`, `dsh`
- **推荐理由：** 现代化灵活任务面板，支持嵌入 Codex 和 DSH
...
- **推荐理由：** 本地优先的多模型AI Agent桌面应用，注重隐私
...
- **标签：** `desktop`, `electron`, `multi-platform`, `codepilot`
- **推荐理由：** CodePilot风格的DSH桌面客户端和插件套件
...
- **标签：** `windows`, `desktop`, `ssh`, `remote-access`, `gui`
- **推荐理由：** 开源Windows桌面客户端，零设置安装，支持SSH和远程访问
...
- **标签：** `desktop`, `devops`
- **推荐理由：** 一站式DevOps桌面，集成数据库、SSH、Docker和AI沙箱。
...
- **推荐理由：** Electron桌面壳集成DSH Web与侧边栏插件
...
- **标签：** `dsh-plugin-market`, `ui`
- **推荐理由：** DSH Web面板一键紧凑上下文按钮。
...
- **推荐理由：** 可移植的Markdown原生记忆层，支持跨应用自我进化
...
- **标签：** `memory`, `agent`, `RAG`, `personal-ai`
- **推荐理由：** 所有AI Agent共享持久化记忆中心，全平台通用。
...
- **标签：** `agent-orchestration`, `workspaces`, `skills`
- **推荐理由：** 跨平台统一Agent工作空间，支持任务看板管理
...
- **标签：** `agent-os`, `loop-engineering`, `skills`
- **推荐理由：** 自愈式Agent OS，自动评分与技能循环工程
...
- **推荐理由：** 智能技能路由与工作流编排，Benchmark 效果显著提升
...
- **推荐理由：** 基于 J-space 全局工作空间的 AI 认知增强技能套件
...
- **推荐理由：** 支持DSH多Agent团队协作，增强工作流编排能力。
...
- **标签：** `skill-corpus`, `retrieval`, `embeddings`, `semantic-search`
- **推荐理由：** 将分散的SKILL.md整理为可检索的Agent技能库
...
- **标签：** `web-search`, `free`, `no-signup`, `bridge`
- **推荐理由：** 最强免费网络搜索插件，为无原生搜索的模型提供桥接
...
- **标签：** `task-management`, `sqlite`, `native`, `project-board`
- **推荐理由：** 原生任务管理，SQLite后端，无需iframe运行时
...
- **推荐理由：** 在DSH内直接搜索、安装和验证GitHub插件的Skill。
...
- **标签：** `skills`, `workflow`, `experience`
- **推荐理由：** 将经验转化为可复用的跨Agent执行Skill。
...
- **标签：** `skills-manager`, `security`, `local`
- **推荐理由：** 集中管理和安全加载本地Agent技能
...
- **标签：** `agent-skills`, `skill-learning`, `automation`
...
- **推荐理由：** 源自DeepSeek Harness的代码简化技能，生成更简洁代码
...
- **推荐理由：** 42个AI Agent技能包管理器，含CLIs和插件资产
...
- **标签：** `media`, `image-generation`, `vision`, `skills`
- **推荐理由：** 提供图片读取和生成技能，支持视觉转录功能
...
- **推荐理由：** 10个QA技能+共享知识库，让AI像资深测试工程师工作
...
- **标签：** `agent-skills`, `governance`
- **推荐理由：** 版本固定、可离线验证的DSH治理技能，适合项目管理。
...
- **推荐理由：** 运行时注册Godot 4.x全栈开发技能，覆盖GDScript开发。
...
- **标签：** `cordis`, `canvas`
- **推荐理由：** 原生WeShop Cordis插件，支持无限画布创意技能。
...
- **推荐理由：** 首个视觉桥接插件，支持OCR与图像转结构化JSON
...
- **推荐理由：** 在Agent中直接搜索65,600+中国平台API文档
...
- **标签：** `codex`, `mcp`, `sync`, `deepseek-harness`
- **推荐理由：** Codex与DSH双向同步，支持MCP镜像和技能自动挂载
...
- **推荐理由：** 在浏览器设置页管理技能与MCP服务器的Web UI插件
...
- **推荐理由：** 通过OAuth在DSH内集成ChatGPT订阅服务
...
- **标签：** `video`, `design`, `skill`
- **推荐理由：** 从单主题自动生成 Vox 风格拼贴解说视频
...
- **标签：** `ui-components`, `charts`, `forms`, `inline-rendering`
- **推荐理由：** 通过dsh-ui栅栏渲染交互UI组件，支持图表和表单
...
- **标签：** `image-generation`, `chat`, `multi-model`, `openai`
- **推荐理由：** 在DSH对话中直接生成图片，支持多种模型
...
- **标签：** `community`, `photo`, `oauth`, `social`
- **推荐理由：** DSH内测合影墙，GitHub OAuth零权限登录拍立得风格
...
- **标签：** `marketplace`, `dsh`, `utility`
- **推荐理由：** DSH 可视化插件市场，支持浏览搜索和一键安装
...
- **标签：** `ecosystem`, `plugin-registry`, `radar`
- **推荐理由：** 自动发现15900+插件，K8s级实测验证，生态雷达。
...
- **推荐理由：** 在对话内控制iOS模拟器和本地iPhone，22个Agent工具
...
- **标签：** `session-migration`, `dsh-plugin`, `state-preservation`
- **推荐理由：** 支持跨预设会话迁移，保留状态与意图。
...
- **标签：** `marketplace`, `plugin-manager`, `infrastructure`
- **推荐理由：** Web界面一键浏览安装GitHub上所有插件。
...
- **标签：** `remote-access`, `security`, `p2p`
- **推荐理由：** 端到端加密多端远程访问Harness。
...
- **标签：** `marketplace`, `webui`, `plugin-manager`
- **推荐理由：** 社区插件市场，Web GUI内一键安装。
...
- **标签：** `config-manager`, `dsh-plugin`, `webdav`
...
- **标签：** `docker`, `container`, `management`
- **推荐理由：** 本地容器化多版本DSH管理工具
...
- **推荐理由：** 一键同步MCP和Skills配置至多Agent工具
...
- **标签：** `migration`, `mcp`
- **推荐理由：** 支持迁移Claude Code等工具的技能和配置到DSH。
...
- **推荐理由：** 实时Token用量、成本和API余额可视化追踪插件。
...
- **推荐理由：** AI资源大全，覆盖大模型、MCP、RAG及Prompt指南
...
- **标签：** `research`, `skills`, `china`
- **推荐理由：** 自动检索中文 8 大平台近 30 天内容生成研究报告
...
- **推荐理由：** 覆盖PubMed、arXiv等学术检索，支持引用核验。
...
- **标签：** `agent-skills`, `deep-research`, `research-automation`, `deepseek`
- **推荐理由：** 88个开源研究及商业工作流Agent技能
...
- **标签：** `kline`, `analysis`, `trading`, `visualization`
- **推荐理由：** 交互式K线分析，支持多市场行情、技术指标和基本面分析
...
- **标签：** `evaluation`, `observability`, `rag`, `benchmark`
- **推荐理由：** 为提示词、RAG、技能和Agent提供可验证评估与可观测性
...
- **标签：** `armor-breaking`, `enhancement`
- **推荐理由：** DeepSeek专用破甲插件，稳定增强生��能力。
...
- **标签：** `encyclopedia`, `navigation`, `resources`, `knowledge-base`
- **推荐理由：** 全面AI技能百科和DSH插件导航库，一站式资源大全
...
- **标签：** `discovery`, `review`, `filter`, `recommendation`
- **推荐理由：** 自动发现和复核DSH插件，过滤蹭标签的虚假项目
...
- **标签：** `tutorial`, `learning`, `marketplace`, `best-practices`
- **推荐理由：** 学习DSH原理、插件市场和最佳实践的指南
...
- **标签：** `awesome-list`, `cordis`, `reference`
- **推荐理由：** cordis.run官方精选自动生成的插件列表。
...
- **标签：** `dsh-plugin`, `dsh-bundle`, `dsh-repository-plugin`
- **推荐理由：** DSH插件精选目录列表。
...
- **标签：** `ai-agents`, `awesome-list`, `plugin`
- **推荐理由：** AI策展的190+插件12类推荐列表。
...
- **标签：** `coding-agents`, `plugin-directory`, `registry`
- **推荐理由：** 涵盖模型、工具、沙盒、UI的精选插件。
