...
- **标签：** `coding-agent`, `tui`, `tool-use`, `prompt-caching`
- **推荐理由：** 终端原生DeepSeek编码智能体，优化前缀缓存稳定性。
...
- **标签：** `routing`, `dsh-plugin`, `agent-orchestration`
- **推荐理由：** 任务感知推理路由，提升编码Agent准确率。
...
- **标签：** `preset`, `standards`, `dsh-plugin`
- **推荐理由：** 生产级DSH预设，对齐标准工具链保障编码可靠性。
...
- **标签：** `claude-code`, `deepseek-harness`, `coding-agent`
- **推荐理由：** 一键切换多个主流编程AI工具与harness。
...
- **标签：** `agent-runtime`, `rust`, `vibe-coding`
...
- **标签：** `agent`, `desktop`, `multi-model`, `dsh-plugin`
- **推荐理由：** 本地优先多模型Agent，具备自进化技能
...
- **推荐理由：** 打通Pi与DSH生态，运行Pi插件作为原生DSH插件
...
- **标签：** `deepseek-harness`, `plugin-development`, `codex`
- **推荐理由：** 用可复用 Codex 技能构建和验证 DSH/Cordis 插件
...
- **标签：** `agent`, `package-manager`, `skills`, `cli`
- **推荐理由：** 42技能+5CLI+1插件，AI Agent工程一站式包管理器
...
- **标签：** `plugin-development`, `audit`, `portable`
- **推荐理由：** 便携式技能，用于开发审核DSH插件及bundle适配
...
- **标签：** `best-practices`, `documentation`, `dsh-plugin`, `skill`
- **推荐理由：** DSH插件开发实战指南，涵盖踩坑记录与解决方案
...
- **标签：** `dsh-plugin`, `scaffolding`, `testing`, `skills`
- **推荐理由：** DSH插件开发与测试的完整技能套件
...
- **标签：** `dsh-web`, `dsh-plugin`, `community`
- **推荐理由：** Web端DSH插件聚合中心，丰富创意生态。
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git/子代理。
...
- **标签：** `desktop`, `electron`, `multi-platform`
- **推荐理由：** 跨平台Electron桌面客户端
...
- **推荐理由：** 开源Windows桌面GUI客户端，支持SSH和移动端
...
- **推荐理由：** Electron桌面控制台，多标签管理Harness/Plugin/MCP
...
- **标签：** `design`, `ui`, `vibe-coding`, `theme`
- **推荐理由：** 6种设计风格，让Vibe Coding网站摆脱AI通用外观
...
- **标签：** `data-visualization`, `token-usage`, `ai-cost`, `web-plugin`
- **推荐理由：** 实时Token用量可视化面板，含费用估算与余额
...
- **标签：** `desktop`, `tauri`, `docker`, `ssh`, `database`
- **推荐理由：** Tauri桌面整合数据库、SSH、Docker与AI助手
...
- **标签：** `desktop`, `electron`, `integration`, `modlens`
- **推荐理由：** Electron desktop wrapper integrating DSH web with useful plugins
...
- **标签：** `webui`, `dsh-plugin`, `background`
- **推荐理由：** 支持自定义背景和预设主题，个性化DSH界面
...
- **标签：** `agent-skills`, `knowledge-distillation`, `ai-persona`
- **推荐理由：** 提炼专家思维为可复用AI智能体技能与人格。
...
- **标签：** `agent-orchestration`, `workspaces`, `dsh-plugin`
- **推荐理由：** 统一Agent工作台，跨端任务管理，体验极佳。
...
- **标签：** `agent-skills`, `skill-md`, `vector-search`
- **推荐理由：** 将分散的SKILL.md组织为可检索的agent技能库
...
- **标签：** `agent-skills`, `math-modeling`, `coding`
- **推荐理由：** 全栈数学建模竞赛自动化Skill，支持GitOps流水线
...
- **标签：** `skills`, `agent`, `codex`
- **推荐理由：** 将经验转化为可重复执行的跨AI技能
...
- **标签：** `agent-skills`, `deep-research`, `market-research`
- **推荐理由：** 88个用于研究、营销和业务工作的agent技能
...
- **标签：** `chinese-wisdom`, `agent-skill`
- **推荐理由：** 八字/风水/中医 AI 智能体，本地确定性引擎。
...
- **标签：** `agent-handoff`, `context-memory`
...
- **标签：** `swift-cycle`, `governance`
- **推荐理由：** 版本固定的 Swift Cycle 治理技能，支持离线验证。
...
- **标签：** `agent`, `skills`, `gamification`, `mission`
...
- **⭐ 星标：** 18
- **语言：** Python
- **描述：** 中文公文全流程处理工具——��于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `template`, `china`
- **推荐理由：** 中文公文全流程：格式检查、内容优化、模板生成
...
- **标签：** `agent-skills`, `canvas`, `mcp`, `skills`
- **推荐理由：** 原生WeShop插件，支持无限画布与创意技能
...
- **推荐理由：** 可移植Agent技能，浓缩工程方法论与事实核查流程
...
- **标签：** `godot`, `game-development`, `gdscript`, `skill`
...
- **标签：** `deepseek-harness`, `api-documentation`, `chinese-api`
- **推荐理由：** 零配置搜索65600+中国平台API文档
...
- **标签：** `mcp`, `skills`, `gui`, `management`
- **推荐理由：** DSH设置页内置卡片，可视化管技能与MCP服务器
...
- **推荐理由：** 粘贴JSON热加载MCP，自动识别管理.skill文件
...
- **推荐理由：** 工业级GPT-Image2提示词引擎，含530+逆向模板。
...
- **标签：** `design`, `vibedesign`, `dsh-plugin`
- **推荐理由：** AI原生矢量设计工具，支持并行Agent团队协作设计。
...
- **标签：** `image-generation`, `dsh-plugin`, `ai-agent`
- **推荐理由：** 在DSH聊天中直接生成图片，便捷高效
...
- **标签：** `image`, `vision`, `media`, `generation`
...
- **标签：** `video`, `image`, `ai-director`, `multimodal`
- **推荐理由：** AI视频/图片/音频技能，支持故事板与可配置模型
...
- **推荐理由：** 浏览器��展套件，增强Gemini/Claude/ChatGPT使用体验。
...
- **标签：** `bilibili`, `xiaohongshu`, `content-discovery`, `local-first`
- **推荐理由：** 本地优先的跨平台内容发现Agent，覆盖8+平台。
...
- **标签：** `zhihu`, `search`, `mcp`, `knowledge`
- **推荐理由：** 知乎搜索、知答问答及官方开放平台API集成
...
- **标签：** `social`, `community`, `oauth`, `photo`
...
- **标签：** `chat-bot`, `dingtalk`, `communication`
- **推荐理由：** 钉钉IM通道，聊天即Agent，支持长连接
...
- **标签：** `vfs`, `infrastructure`, `dsh-plugin`
- **推荐理由：** 统一虚拟文件系统，抽象存储实现多Agent无缝操作。
...
- **标签：** `deepseek-harness`, `ocr`, `vision`
- **推荐理由：** OCR插件：为纯文本模型提供图像读取功能
...
- **标签：** `plugin-management`, `credentials`
- **推荐理由：** 轻松管理 DSH 插件、槽位和凭证。
...
- **标签：** `backup`, `config-management`
- **推荐理由：** 通过 WebDAV 备份、同步和迁移完整配置。
...
- **标签：** `container`, `multi-version`
- **推荐理由：** 在隔离容器中运行多版本 DSH，内嵌 WebView。
...
- **标签：** `codex`, `sync`, `mcp`
- **推荐理由：** Codex 与 DSH 双向同步，自动镜像 MCP。
...
- **标签：** `context-audit`, `diagnostics`
- **推荐理由：** 审计上下文 token，检测配置冲突和重复。
...
- **标签：** `utility`, `info-bar`, `pricing`, `monitoring`
- **推荐理由：** 实时显示提供商/模型信息、余额及峰谷定价倒计时
...
- **标签：** `migration`, `import`, `mcp`, `skills`
- **推荐理由：** 一键将Claude Code配置迁移至DSH，含技能与MCP
...
- **标签：** `skin`, `theme`, `deepseek-harness`
...
- **标签：** `theme`, `skin`, `color-scheme`, `customization`
...
- **标签：** `ai-resources`, `prompt-engineering`, `vibe-coding`, `rag`
- **推荐理由：** 全面AI资源库，涵盖大模型、提示词与技能教程。
...
- **标签：** `agent`, `search`, `discoverability`
- **推荐理由：** Agent内直接搜索浏览DSH插件，发现便捷
...
- **推荐理由：** Industry-specific DSH plugins for IT, finance, legal and more
...
- **标签：** `awesome-list`, `dsh-plugin`, `resources`
- **推荐理由：** 精选插件目录，探索DSH生态的最佳入口。
...
- **标签：** `toy`, `web-simulation`, `single-file`
- **推荐理由：** 零依赖的传统中国儿童玩具「竹知了」网页模拟版。
...
- **标签：** `awesome-list`, `plugins`, `bilingual`
- **推荐理由：** 中英双语精选插件目录与导航
...
- **标签：** `learning`, `documentation`, `dsh-plugin`
...
- **推荐理由：** DeepSeek Harness插件市场与精选集合
...
- **推荐理由：** DeepSeek Harness 搜番插件：对话内多源搜索番剧，卡片展示 Ban
...
- **推荐理由：** The DeepSeek Harness plugin for OpenPenc
...
- **推荐理由：** 让 Agent 的工作方式可组合、可审查、可持续改进，最终实现 Agent Se
...
- **标签：** `awesome-list`, `directory`
- **推荐理由：** 精选 280+ DSH 插件目录，覆盖 14 大分类。
