...
- [UI 与桌面扩展](ui-desktop-extensions-zh) (26)
...
- **标签：** `coding-agent`, `prompt-caching`, `terminal`
- **推荐理由：** 终端原生AI编码智能体，提示词缓存稳定
...
- **标签：** `agent-workspace`, `enterprise`, `dsh-plugin`
- **推荐理由：** 企业级多引擎Agent工作台，含可视化编辑器
...
- **标签：** `agent-os`, `evaluation`, `dsh-plugin`
- **推荐理由：** 自进化Agent OS，含门控评估与预算循环
...
- **标签：** `virtual-terminal`, `vfs`, `agent-sandbox`, `dsh-plugin`
- **推荐理由：** 虚拟终端与文件系统，安全沙箱隔离代理
...
- **标签：** `agent-runtime`, `rust`, `coding-agent`, `openclaw`
- **推荐理由：** Rust高性能agent运行时，融合桌面应用与代码智能体能力
...
- **标签：** `coding-agent`, `rpc`, `sub-agents`, `mcp`, `terminal`, `tui`
- **推荐理由：** 含RPC、子代理、hashline编辑和MCP的编码代理，完整终端TUI控制
...
- **标签：** `cybersecurity`, `vulnerability`, `security-audit`, `redteam`
...
- **推荐理由：** DSH插件中心的CLI工具，提供插件解析、管理和Agent工具链
...
- **标签：** `agent-review`, `self-evolution`, `prompt-engineering`, `dsh-plugin`
- **推荐理由：** 可组合、可审查的Agent工作流，支持Agent自我进化
...
- **推荐理由：** 为纯文本LLM增加视觉能力，支持截图/UI/图表分析
...
- **推荐理由：** 87个模块的逆向工程与渗透测试Skill，适用于安全研究
...
- **推荐理由：** Superpowers技能移植至Reasonix编码工具
...
- **标签：** `plugin-development`, `codex`, `validator`
- **推荐理由：** 可复用的插件构建与验证工具，加速DSH开发
...
- **标签：** `cli`, `skills`, `dsh-plugin`, `codex`
- **推荐理由：** 现代化灵活任务面板，可嵌入编码代理
...
- **标签：** `deepseek-harness`, `floating-widget`, `dsh-plugin`
- **推荐理由：** 可爱鲸鱼浮动组件，实时显示余额，支持拖拽吸附和滚动动画
...
- **标签：** `dsh-desktop`, `tauri`, `desktop-app`
- **推荐理由：** Tauri轻量桌面版，零环境配置，预装插件，支持三平台
...
- **标签：** `desktop-app`, `electron`, `gui`, `ssh-client`, `remote-access`
- **推荐理由：** 零配置Windows桌面客户端，集成Codex、插件、SSH和移动端远程访问
...
- **标签：** `desktop-app`, `local-first`, `mcp`, `office-automation`
- **推荐理由：** 本地优先AI桌面工作台，支持会话、项目、网页研究和Office文档
...
- **标签：** `canvas`, `non-linear`, `session-explorer`, `branching-workspace`
- **推荐理由：** 基于画布的非线性对话工作区，支持会话分支和可视化探索
...
- **标签：** `desktop-app`, `local-first`, `multi-model`, `self-evolving`, `privacy`
- **推荐理由：** 本地优先多模型AI桌面应用，支持自进化技能和多Harness路由
...
- **标签：** `office`, `ui-extension`, `productivity`
- **推荐理由：** 将表格、文档、幻灯片等办公套件融入Harness。
...
- **标签：** `mobile`, `ios`, `ui-extension`
- **推荐理由：** 原生iOS客户端，随时掌控工作区与Agent执行。
...
- **标签：** `desktop`, `ui-extension`, `client`
- **推荐理由：** CodePilot风格桌面客户端，全平台插件套件支持。
...
- **⭐ 星标：** 165
- **语言：** TypeScript
- **描述：** 开源版锤子便签，复刻锤科美学，一���Docker私有化部署，支持skill调用，支持dsh plugin，支持多租户，一键生成公众号格式，支持导出便签为图片
- **标签：** `notes`, `docker`, `smartisan`, `multi-tenant`
- **推荐理由：** 开源锤子便签，支持Docker部署、Skill调用与公众号导出
...
- **推荐理由：** Electron桌面控制台，多标签管理插件与MCP
...
- **标签：** `ui`, `usage-tracking`, `pricing`
- **推荐理由：** 实时显示模型、余额和价格的底部信息栏
...
- **标签：** `context`, `audit`, `ui`
- **推荐理由：** 上下文注入审计工具，含圆环面板和token成本分析
...
- **推荐理由：** 类WorkBuddy技能选择器，快速插入Agent技能
...
- **⭐ 星标：** 12
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。��开发运维每天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `devops`, `docker`, `ssh`, `sftp`
- **推荐理由：** 集成数据库、SSH、Docker、AI沙箱的一体化开发运维桌面
...
- **推荐理由：** Electron桌面版DSH，集成Web UI和ModLens等插件
...
- **推荐理由：** LLM知识库平台，支持RAG、智能体与自维护Wiki
...
- **标签：** `memory`, `rag`, `long-term-memory`, `dsh-plugin`
- **推荐理由：** 本地优先的Markdown记忆层，跨应用自进化
...
- **标签：** `memory`, `rag`, `token-savings`, `dsh-plugin`
...
- **标签：** `agent-memory`, `stateful-agents`, `dsh-plugin`
- **推荐理由：** 专为状态化Agent应用设计的记忆库
...
- **标签：** `agent-memory`, `milvus`, `rag`, `hybrid-search`
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持混合搜索，多AI代理共享
...
- **标签：** `long-term-memory`, `mcp`, `personal-ai`, `memory-hub`
- **推荐理由：** 个人AI记忆中枢，为所有AI代理提供共享持久化上下文
...
- **标签：** `context-management`, `dashboard`, `cordis-plugin`
- **推荐理由：** 全功能上下文管理面板，实时监控和精细控制对话上下文
...
- **标签：** `knowledge-base`, `awesome-list`, `ai-encyclopedia`, `multimodal`, `mlops`
- **推荐理由：** 全方位AI资源百科，涵盖Agent、RAG、多模态、MLOps及学习路线
...
- **标签：** `agent-orchestration`, `workspace`, `dsh-plugin`
- **推荐理由：** 多设备统一Agent与工作区，支持任务看板
...
- **标签：** `skills`, `marketplace`, `dsh-plugin`
- **推荐理由：** 最全技能商店，自动抓取GitHub技能目录
...
- **标签：** `agent-control-plane`, `long-horizon`, `dsh-plugin`
- **推荐理由：** 长周期Agent控制面，支持跨框架持久工作
...
- **标签：** `agent-skills`, `cognitive-enhancement`, `dsh-plugin`, `deepseek`
- **推荐理由：** 模型无关推理控制，支持深度推理与恢复
...
- **标签：** `agent-skill`, `video-generation`, `ffmpeg`, `tts`
- **推荐理由：** 端到端视频生成技能，一键将主题转化为Vox风格解说视频
...
- **标签：** `agent-teams`, `multi-agent`, `workflow`, `dsh-plugin`
- **推荐理由：** 支持多智能体协作与协调工作流，扩展DSH团队能力
...
- **标签：** `novel-writing`, `rag`, `creative-writing`, `long-form`
- **推荐理由：** 结构化AI小说创作流程，支持世界观、章节管理与审稿修稿
...
- **标签：** `agent-skills`, `skill-routing`, `vector-search`, `reranking`
- **推荐理由：** 将分散技能文件整合为可检索的代理技能库，支持评估与重排序
...
- **标签：** `task-management`, `workflow`, `agent`
...
- **标签：** `agent-skills`, `math-modeling`, `competition`, `gitops`
- **推荐理由：** 全栈数学建模竞赛Skill，含GitOps流水线与代码自证功能
...
- **标签：** `skills`, `agent`, `architecture`, `codex`
- **推荐理由：** 将经验转化为可复用执行技能，兼容多Agent平台
...
- **标签：** `agent-skills`, `research`, `marketing-automation`, `multi-platform`
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销自动化与商业流程
...
- **推荐理由：** 面向DSH的Superpowers测试调试规划技能
...
- **标签：** `agent-skill`, `mentoring`, `code-review`
...
- **标签：** `skills-deck`, `engineering`, `productivity`
- **推荐理由：** 预装25个工程效率技能，开箱即用无需配置
...
- **标签：** `spec-programming`, `agent-skills`, `workflow`
- **推荐理由：** 可移植的规格驱动开发技能，支持仓库原生模式
...
- **标签：** `chinese-wisdom`, `agent-skill`, `cultural`
- **推荐理由：** 独特中华传统智慧技能，含八字风水等确定性引擎
...
- **标签：** `engineering`, `quality-gate`, `delivery`
- **推荐理由：** 工程纪律技能包，需求审核与交付质量门控
...
- **标签：** `multi-agent`, `memory`, `continuity`
- **推荐理由：** 持久化多智能体系统，支持跨会话记忆延续
...
- **标签：** `handoff`, `session-memory`, `continuity`
- **推荐理由：** 跨平台会话延续记忆，支持智能体交接
...
- **标签：** `skill`, `code-simplification`, `deepseek-harness`
- **推荐理由：** 代码简化技能，适配DSH生成更整洁代码
...
- **标签：** `skill`, `tencent`, `api`, `deepseek-harness`
- **推荐理由：** 腾讯SkillHub官方API文档与DSH插件
...
- **标签：** `math-modeling`, `agent`, `skill`, `mcm`, `cumcm`
- **推荐理由：** 多Agent数学建模，5角色+33模型+论文模板
...
- **标签：** `math-modeling`, `skill`, `data-preprocessing`, `deepseek-harness`
- **推荐理由：** 完整数学建模技能：数据处理、编程、写作、可视化
...
- **标签：** `video`, `skill`, `remotion`, `motion-design`, `deepseek-harness`
- **推荐理由：** 电影感产品视频Agent技能，152张镜头配方卡
...
- **标签：** `godot`, `game-development`, `gdscript`, `skill`, `deepseek-harness`
...
- **标签：** `skill`, `workflow`, `planning`, `deepseek-harness`
- **推荐理由：** 议行分离AI协作任务机制，先规划后执行
...
- **标签：** `agent-skill`, `plugin-dev`, `claude-code`, `codex`
- **推荐理由：** 用Agent技能开发DSH插件，支持Claude Code和Codex
...
- **标签：** `routing`, `mcp`, `dsh-plugin`
- **推荐理由：** 注入器+路由套件，支持任务感知推理模式
...
- **⭐ 星标：** 67
- **语言：** TypeScript
- **描述：** 在 DeepSeek Harness / Claude Code / Cursor / Codex / Gemini CLI 里直接搜索 20 ���中国开放平台的 65,600+ 篇 API 文档；零配置，支持 Skill 与 DSH 原生插件。
- **标签：** `api-docs`, `mcp`, `chinese-api`
- **推荐理由：** 零配置搜索6.5万篇中国开放平台API文档
...
- **标签：** `cloud`, `mcp`, `huawei-cloud`
- **推荐理由：** 华为云官方插件，含MCP工具与云沙箱能力
...
- **推荐理由：** 像素转文字OCR工具，为纯文本模型提供图像读取能力
...
- **标签：** `mcp`, `skills`, `agent-config`, `deepseek-harness`
- **推荐理由：** 一键配置多AI工具MCP与Skills
...
- **标签：** `agent-skills`, `ui-generator`, `design-systems`
- **推荐理由：** 开源AI设计系统插件，支持代码可视化设计
...
- **标签：** `design-studio`, `ppt`, `prototyping`, `visual-editor`
- **推荐理由：** AI驱动的本地化设计与PPT工作室，含可视化编辑和模板市场
...
- **标签：** `ui-components`, `charts`, `forms`, `interactive`
- **推荐理由：** 在助手回复中内联渲染交互UI组件：图表、表单、测验等
...
- **标签：** `video`, `image`, `multimedia`
- **推荐理由：** AI音视频生成技能，支持分镜和知识语料库
...
- **标签：** `computer-vision`, `ocr`, `gui-automation`, `vision-tools`
- **推荐理由：** 纯文本模型视觉增强套件，支持OCR、截图分析和UI还原
...
- **⭐ 星标：** 508
- **语言：** TypeScript
- **描述：** 🥇 The strongest free web search plugin for DeepSeek Harness, and the search bridge for every model without native web access. Free, no signup, no API key. Ask the web or X, get structured JSON evidence. | 🥇 全网最强的 DeepSeek Harness 免费联网搜索插件，免费免注册免 API key。为不能联网的模型补���搜索，问网页或 X，拿回结构化 JSON 证据（搜索、抓取、引用）。
- **标签：** `web-search`, `web-fetch`, `agent-tools`, `free`
- **推荐理由：** 最强免费网页搜索桥接插件，无需注册和API密钥即可为任何模型提供搜索
...
- **标签：** `browser-automation`, `fingerprint-spoofing`, `playwright`, `android-emulation`
- **推荐理由：** 带指纹伪装和无限本地配置的持久化浏览器，支持Android模拟
...
- **推荐理由：** 内置iOS模拟器与USB真机自动化，22个Agent工具。
...
- **标签：** `low-code`, `ai-assistant`, `self-hosted`
...
- **标签：** `deepwiki`, `documentation`, `dsh-plugin`, `deepseek`
- **推荐理由：** 快速将代码库转为准确技术文档和AI上下文
...
- **推荐理由：** 复用ChatGPT/Claude/Grok订阅作为LLM提供商。
...
- **标签：** `provider`, `llm`, `model-catalog`
- **推荐理由：** 集成Command Code模型，支持智能选模与计划感知。
...
- **标签：** `remote-access`, `p2p`, `encryption`, `multi-device`
- **推荐理由：** P2P加密多端远程访问方案，支持PC/Android/Web
...
- **标签：** `migration`, `compatibility-layer`, `pi-agent`, `dsh-plugin`
- **推荐理由：** 打通Pi与DSH生态，让Pi扩展原样运行于DSH
...
- **标签：** `plugin-discovery`, `automation`, `dsh-plugin`, `verification`
...
- **标签：** `infrastructure`, `plugin-management`, `utilities`
- **推荐理由：** DSH生态基建插件，提供控制台与开发引导
...
- **标签：** `skills`, `management`, `security`
- **推荐理由：** 统一加载并安全管理本地Agent Skills的工具
...
- **⭐ 星标：** 42
- **语言：** TypeScript
- **描述：** A plugin-based Web work platform built on DeepSeek Harness, with global Skill management and native workspace integration. 基于 DeepSeek Harness 的插件化 Web 工作平台。
- **标签：** `workbench`, `platform`, `infrastructure`
- **推荐理由：** 基于DSH的插件化Web工作台，全局技能管理
...
- **标签：** `dsh-plugin`, `container`, `launcher`
- **推荐理由：** 多版本DSH隔离运行，一键导入插件
...
- **标签：** `migration`, `tool`, `utility`, `deepseek-harness`
- **推荐理由：** 从Claude/Codex迁移会话、记忆和技能的向导
...
- **⭐ 星标：** 24
- **语言：** Rust
- **描述：** Unlock DeepSeek Harness plugin-native core capabilities, unifying Runtime, Provider, Plugin, Skill and MCP orchestration to build an extensible, composable and manageable local AI runtime platform.
- **标签：** `dsh-plugin`, `launcher`, `runtime`, `cordis`
- **推荐理由：** 统一运行时、MCP、插件编排的核心能力扩展器
...
- **标签：** `token-usage`, `ai-cost`, `data-visualization`
- **推荐理由：** 实时Token用量、费用估算和API余额可视化面板
...
- **标签：** `tts`, `audio`, `xiaomi-mimo`
- **推荐理由：** 免费小米MiMo TTS，支持预设/自定义/浏览器语音朗读
...
- **标签：** `diagnostics`, `plugin-conflict`, `troubleshooting`
- **推荐理由：** 诊断并解决插件间及版本冲突问题
...
- **标签：** `mascot`, `sprites`, `dsh-plugin`
- **推荐理由：** 动画宠物头像，为编码代理增添趣味个性
...
- **标签：** `theme`, `skin`, `whale-girl`, `dsh-plugin`
- **推荐理由：** 鲸鱼娘系列主题皮肤，个性化DSH界面，可爱美观
...
- **标签：** `theme`, `glass-morphism`, `transparency`, `ui-customization`
- **推荐理由：** 高自由度磨砂玻璃主题，可自由调节模糊度、磨砂度和背景效果
...
- **⭐ 星标：** 378
- **语言：** JavaScript
- **描述：** 🌊 DeepSeek Harness 海洋���肤与动态主题 | Real-time ocean theme with adjustable waves, sunset & glass opacity. DSH plugin + Chrome/Edge extension; keeps your new-tab homepage.
- **标签：** `theme`, `skin`, `animated-background`
- **推荐理由：** 精美海洋动态主题，支持波浪调节与玻璃效果。
...
- **标签：** `content-discovery`, `ai-agent`, `cross-platform`, `dsh-plugin`
- **推荐理由：** 跨平台自进化内容发现代理，覆盖主流平台
...
- **标签：** `plugin-registry`, `ecosystem-radar`, `continuous-validation`
- **推荐理由：** 2.1万+插件发现引擎，配合k8s运行时验证，保障插件质量
...
- **标签：** `awesome-list`, `curated`, `plugin-index`
- **推荐理由：** 精心整理的DSH生态插件与工具大全，一站式资源索引
...
- **标签：** `awesome-list`, `curated-list`, `bilingual`, `developer-tools`
- **推荐理由：** 双语精选DSH插件目录，涵盖扩展、技能、客户端和集成资源
...
- **标签：** `arxiv`, `latex`, `paper-writing`, `experiment-tracking`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX写作、arXiv文献管理、实验追踪和GPU服务器SSH编排
...
- **标签：** `awesome-list`, `resources`, `plugins`
- **推荐理由：** 全面收录插件、MCP、技能、补丁的精选资源库。
...
- **标签：** `academic-search`, `mcp`, `citation-management`, `research-workflow`
- **推荐理由：** 跨多个学术数据库搜索，支持引文核验和图谱可视化
...
- **标签：** `finance`, `stock`, `multi-agent`, `tushare`
- **推荐理由：** 基于Tushare的多Agent金融分析系统，支持IM盯盘通知
...
- **标签：** `industry-research`, `skills`, `research`
- **推荐理由：** 含方法论技能的行业研究与产业链分析工具
...
- **标签：** `kline`, `trading`, `analysis`, `deepseek-harness`
...
- **标签：** `ai-guide`, `llm`, `vibe-coding`
- **推荐理由：** 全面AI资源库，涵盖主流大模型教程
...
- **标签：** `plugin-marketplace`, `curation`, `daily-update`
- **推荐理由：** 每日更新的全方位插件市场，所有上架内容经人工审核
...
- **标签：** `marketplace`, `catalog`, `plugin-directory`, `dsh`
- **推荐理由：** 官方DSH插件市场，收录超11000个插件并提供公共API
...
- **标签：** `handbook`, `documentation`, `agent-runtime`, `mcp`
- **推荐理由：** Agent-first DSH手册，覆盖运行时、插件、MCP和安全沙箱
...
- **标签：** `catalog`, `directory`, `plugin-marketplace`, `dsh`
- **推荐理由：** DSH插件总目录，1958个仓库，中英文搜索与分类筛选
...
- **标签：** `awesome-list`, `plugin-marketplace`, `community`
...
- **标签：** `awesome-list`, `community`, `plugin-catalog`
...
- **标签：** `awesome-list`, `resources`, `mcp`, `skills`, `deepseek-harness`
- **推荐理由：** 全面的DSH插件、工具、技能资源索引
...
- **推荐理由：** 24个经过验证的股票/金融/量化DSH插件双语列表
...
- **标签：** `awesome-list`, `curated-list`, `verified`
- **推荐理由：** 基于证据的DSH插件验证目录和权限安装指南
...
- **⭐ 星标：** 6
- **语言：** Unknown
- **描述：** 🐳 DeepSeek Harness（DSH）工具全量清单——GitHub topic:dsh-plugin 生态 1000+ ��库按 star 排序。Awesome list for DeepSeek Harness tools.
- **标签：** `awesome-list`, `inventory`, `github-topic`
- **推荐理由：** 按Star排序的DSH工具全量清单（1000+仓库）
...
- **标签：** `agent-harness`, `ai-agent`, `ai-agents`, `autonomous-agent`, `awesome`, `awesome-list`, `cli`, `coding-agent`, `cordis`, `deepseek`, `deepseek-harness`, `developer-tools`, `dsh`, `dsh-plugin`, `llm`, `llm-agent`, `open-source`, `plugin-ecosystem`, `plugins`, `web-ui`
- **推荐理由：** A curated list of plugins for DeepSeek Harness (dsh) - DeepSeek Harness plugin e
...
- **推荐理由：** The most comprehensive AI-curated list of DeepSeek Harness (DSH) plugins - 190+ 
...
- **推荐理由：** Evidence-backed DeepSeek Harness plugin radar and 2Origin plugin lab
...
- **推荐理由：** Plugin discoverability for DeepSeek Harness: search npm + awesome-dsh-plugin, lo
...
- **推荐理由：** A plugin of Deepseek Harness(DSH),transform your recent chat history into beauti
...
- **推荐理由：** 给 DSH agent 的插件库"起飞前检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用率可预期、不靠运气。Pre-flig
...
- **推荐理由：** ChatGPT Subscription - a DeepSeek Harness plugin: bind your ChatGPT account via 
...
- **推荐理由：** 收录80+经认证的DSH插件目录，适合快速发现插件。
...
- **标签：** `dsh-plugins`, `plugin-verify`, `dsh-ecosystem`
- **推荐理由：** 提供插件验证与平台分析功能的DSH插件索引库。
...
- **标签：** `awesome-list`, `dsh-plugins`, `migrated`
- **推荐理由：** 原始awesome列表已迁移，建议访问新版本仓库。
...
- **标签：** `dsh-plugins`, `awesome-list`, `auto-updating`
- **推荐理由：** 自动更新的双语插件列表，YAML驱动，数据准确。
...
- **标签：** `dsh-plugin`, `registry`, `curated-list`, `ui`, `agent-harness`
- **推荐理由：** 覆盖模型、工具、沙盒和UI的全面插件注册表。
