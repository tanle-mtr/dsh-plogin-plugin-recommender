...
- **标签：** `dsh-routing`, `mcp`, `agent-harness`
...
- **标签：** `agent-collaboration`, `multi-agent`, `enterprise-agent`
- **推荐理由：** 企业级多代理工作台，支持多引擎。
...
- **标签：** `virtual-terminal`, `agent-sandbox`, `vfs`, `dsh-plugin`
- **推荐理由：** AI代理虚拟终端和文件系统沙盒
...
- **推荐理由：** Rust高性能Agent运行时，集成代码Agent能���与桌面体验
...
- **标签：** `vision`, `ocr`, `agent-tool`
- **推荐理由：** 纯文本模型视觉工具箱：一行实现图像问答、OCR和UI还原
...
- **标签：** `cli`, `plugin-manager`, `deepseek-harness`, `typescript`, `developer-tools`
- **推荐理由：** 开源CLI和解析工具包，用于管理DSH插件中心生态。
...
- **标签：** `provider`, `model-access`, `llm`
- **推荐理由：** Command Code模型接入，动态目录与智能选型
...
- **标签：** `coding`, `superpowers`, `reasonix`
- **推荐理由：** 适配Reasonix编码 harness 的超级能力技能
...
- **推荐理由：** 来自superpowers的TDD、调试和规划技能
...
- **标签：** `plugin-creator`, `codex`, `harness-tools`
- **推荐理由：** 复用Codex技能构建和验证DSH插件，高效开发工具。
...
- **标签：** `code-simplification`, `codex`, `skill`, `refactor`
- **推荐理由：** Codex代码简化技能，自动发现并应用优化方案。
...
- **标签：** `quality-gate`, `engineering`, `requirements`, `guard`
- **推荐理由：** 工程纪律Bundle：检验需求、测试实现、验证交付。
...
- **标签：** `qa`, `testing`, `playwright`, `pytest`, `skill`
- **推荐理由：** 11个测试技能+共享知识库，让AI像资深测试工程师。
...
- **标签：** `plugin-dev`, `skill`, `deepseek-harness`
- **推荐理由：** 从零到一指导AI开发、构建和安装DSH插件
...
- **标签：** `plugin-dev`, `agent-skill`, `deepseek-harness`
- **推荐理由：** 开发DSH插件、服务与LLM适配器的Agent技能
...
- **标签：** `taskboard`, `ui-extension`, `skills`, `dsh-plugin`
- **推荐理由：** 现代化可嵌入任务面板，支持多平台
...
- **标签：** `tui`, `terminal`, `ui-extension`, `dsh-plugin`
- **推荐理由：** 鲸鱼顶栏流式思考TUI，支持双击回滚
...
- **标签：** `widget`, `floating-ui`, `balance-tracker`, `dsh-plugin`
- **推荐理由：** 萌系浮动鲸鱼组件实时显示账户余额
...
- **标签：** `desktop`, `electron`, `local-first`, `workspace`
- **推荐理由：** 本地优先的DSH桌面工作区，支持会话、项目与办公文件。
...
- **标签：** `desktop`, `windows`, `gui`, `codex`, `ssh`
- **推荐理由：** 零安装Windows桌面客户端，内置Codex、SSH与远程访问。
...
- **标签：** `canvas`, `branching`, `visualization`, `workspace`
...
- **标签：** `desktop`, `local-first`, `multi-model`, `tauri`
- **推荐理由：** 本地优先多模型AI桌面应用，支持自进化技能与多Harness。
...
- **标签：** `office`, `spreadsheet`, `docs`, `slides`, `deepseek-harness-plugin`
- **推荐理由：** 在Harness中集成电子表格、文档、演示文稿等完整办公套件。
...
- **⭐ 星标：** 289
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将��作区、会话、实时回复和 Agent 执行轨迹带到 iPhone，同时延续 DeepSeek WebUI 克制、清晰的视觉语言
- **标签：** `ios`, `ios-app`, `kotlin-multiplatform`, `mobile`, `deepseek-harness`
- **推荐理由：** 原生iOS客户端，将工作区、会话和Agent执行轨迹带到iPhone。
...
- **标签：** `desktop-app`, `electron`, `codepilot`, `macos`, `linux`
- **推荐理由：** CodePilot风格跨平台桌面客户端，支持macOS/Windows/Linux。
...
- **推荐理由：** 锤子便签复刻版，支持Docker私有化与DSH插件
...
- **标签：** `marketplace`, `ui`, `community`
- **推荐理由：** DSH Web GUI社区插件市场
...
- **标签：** `ui`, `status-bar`, `billing`, `provider-info`
- **推荐理由：** 底部状态栏实时显示用量、余额和定价，一目了然。
...
- **推荐理由：** 类似WorkBuddy的技能选择器，一键插入技能命令。
...
- **标签：** `kline`, `visualization`, `sidebar`
- **推荐理由：** 在原生侧栏展示交互式K线分析与技术指标
...
- **推荐理由：** 一站式DevOps桌面：SSH、Docker、数据库和AI工具集成
...
- **标签：** `desktop`, `electron`, `dsh-integration`
- **推荐理由：** Electron桌面壳集成DSH Web UI、侧边栏和ModLens
...
- **标签：** `ui`, `theme`, `background`, `webui`
- **推荐理由：** 支持自定义图片与预设主题的Web背景个性化插件
...
- **标签：** `ui`, `compact`, `context-meter`, `dsh-plugin`
- **推荐理由：** DSH上下文面板一键折叠按钮，节省空间
...
- **标签：** `knowledge-cards`, `ui`, `dsh-plugin`
...
- **标签：** `long-term-memory`, `rag`, `memory-management`, `token-savings`
...
- **标签：** `memory`, `rag`, `milvus`, `semantic-search`, `dsh-plugin`
- **推荐理由：** 基于Milvus的持久统一智能体记忆层
...
- **推荐理由：** 本地共享记忆中枢，让所有AI Agent持久记住上下文
...
- **标签：** `multi-agent`, `self-evolving`, `memory`
- **推荐理由：** 自进化多智能体，带记忆和知识管理。
...
- **标签：** `knowledge-distillation`, `meta-skill`, `ai-persona`
- **推荐理由：** 将专家思维模式提炼为可复用AI技能。
...
- **标签：** `agent-orchestration`, `agent-workspaces`, `no-code`, `tool-use`
- **推荐理由：** 跨设备统一代理工作空间与任务看板。
...
- **标签：** `agent-skills`, `skill-marketplace`, `dsh-plugin`
- **推荐理由：** 最大技能商店，自动整理GitHub技能库。
...
- **标签：** `agent-control-plane`, `long-horizon`, `workflow-automation`
- **推荐理由：** 长周期代理控制面，支持持久化流程治理。
...
- **标签：** `agent-skills`, `workflow`, `orchestration`, `dsh-plugin`
...
- **标签：** `cognitive-enhancement`, `agent-skills`, `j-space`, `dsh-plugin`
...
- **推荐理由：** 单主题自动生成Vox风格拼贴解说视频，全链路自动化
...
- **推荐理由：** 在DeepSeek Harness中启用多Agent团队编排与协作
...
- **标签：** `writing`, `creative`, `rag`
- **推荐理由：** 带RAG的小说创作工作流，整合角色管理与审稿流程
...
- **标签：** `agent-skills`, `skill-corpus`, `retrieval`, `evaluation`
- **推荐理由：** 将分散技能文件转为可检索智能体技能库，含评估工具。
...
- **标签：** `multimodal`, `ocr`, `video-analysis`, `computer-vision`
- **推荐理由：** 赋予智能体视听能力，将音视频转为可检索证据。
...
- **标签：** `novel-writing`, `creative-writing`, `agent-skills`, `drama-skills`, `fiction-writing`
- **推荐理由：** 专为网文、短剧、游戏和视频解说设计的AI写作工作流插件。
...
- **标签：** `task-management`, `taskboard`, `agent`, `sqlite`, `deepseek-harness`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领/审核，无iframe的整洁Web UI。
...
- **标签：** `agent-skills`, `math-modeling`, `competition`, `gitops`, `claude-code`
- **推荐理由：** 全栈式自动数学建模竞赛Skill，支持GitOps流水线和代码自证。
...
- **标签：** `skills`, `architecture`, `multi-agent`
- **推荐理由：** 将经验转化为可复用执行，支持多AI平台
...
- **标签：** `finance`, `multi-agent`, `stock-analysis`
- **推荐理由：** 基于Tushare的金融数据源，多Agent分析与IM盯盘
...
- **标签：** `agent-skills`, `research-automation`, `business`
- **推荐理由：** 88个开源技能，覆盖研究营销商业工作流
...
- **标签：** `skill-generation`, `automation`, `learning`
- **推荐理由：** 自动将成功会话转换为可复用技能
...
- **标签：** `ai-coding-agent`, `mentor`, `code-review`
- **推荐理由：** AI编程导师，生成阶梯式教程并自动判题和Code Review
...
- **标签：** `skills`, `engineering`, `deck`
- **推荐理由：** 内置25个工程技能，开箱即用的效率面板
...
- **标签：** `gongwen`, `docx`, `legal-docs`, `wps`
- **推荐理由：** 符合国标的全流程公文格式检查与修复工具
...
- **标签：** `spec-driven`, `agent-skill`, `vibe-coding`
- **推荐理由：** 可移植的规格驱动开发技能，参考DSH工程模式
...
- **标签：** `chinese-wisdom`, `bazi`, `fengshui`, `tcm`
- **推荐理由：** 八字风水中医融合技能，本地确定性引擎一键安装。
...
- **标签：** `ocr`, `image-reading`, `vision`, `skill`
- **推荐理由：** 让文本模型具备图像读取能力，支持OCR和像素扫描。
...
- **标签：** `multi-agent`, `collaboration`, `team`, `mcp`
- **推荐理由：** 多智能体协作框架，独立模型与共享工作空间。
...
- **标签：** `roleplay`, `narrative`, `18+`, `skill`
- **推荐理由：** 成人叙事互动技能，NPC有记忆立场，全维YAML存档。
...
- **标签：** `agent-handoff`, `session-memory`, `deepseek-harness`
- **推荐理由：** 跨平台Agent交接的仓库本地连续性记忆方案
...
- **标签：** `math-modeling`, `multi-agent`, `deepseek-harness`
- **推荐理由：** 多Agent数学建模，5角色33模型库含论文模板
...
- **标签：** `multi-agent`, `agent-team`, `deepseek-harness`
- **推荐理由：** 跨会话持久化多Agent团队，含私有记忆
...
- **推荐理由：** 电影感视频Agent，含152张镜头配方与Remotion模板
...
- **标签：** `huawei-cloud`, `mcp`, `sdk`, `terraform`
...
- **推荐理由：** 一键配置多种AI Agent工具的MCP、Skills和规则。
...
- **推荐理由：** 腾讯官方SkillHub Open API文档与DSH集成示例
...
- **标签：** `discoverability`, `search`, `agent`
- **推荐理由：** 在DSH代理内搜索npm和精选列表的插件发现工具。
...
- **标签：** `vibedesign`, `ui`, `flutter`, `react`
- **推荐理由：** AI原生矢量设计工具，支持Agent协作生成UI。
...
- **推荐理由：** AI生成可编辑设计系统，含模板市场与原生PPT工作室
...
- **标签：** `gui`, `ui-generation`, `charts`, `interactive`
- **推荐理由：** 在回复中内联生成交互式UI组件，支持图表和表单。
...
- **标签：** `video`, `image`, `audio`, `directorx`, `ai-creative`
- **推荐理由：** AI视频/图像/音频生成技能，支持分镜和知识语料库。
...
- **标签：** `content-discovery`, `cross-platform`, `web-automation`, `dsh-plugin`
- **推荐理由：** 跨平台AI内容发现智能体，支持多站点
...
- **推荐理由：** AI Agent通过CLI和扩展无缝操控已登录浏览器
...
- **标签：** `web-search`, `free`, `no-api-key`, `browser`
...
- **标签：** `ios`, `ios-simulator`, `webdriveragent`, `mobile-automation`, `mcp`
- **推荐理由：** 对话内嵌iOS模拟器及真机控制，提供22个自动化Agent工具。
...
- **标签：** `browser-automation`, `anti-detect`, `mcp-server`
- **推荐理由：** 防检测浏览器，支持指纹伪装与Android模拟
...
- **标签：** `plugin-portal`, `github-pages`, `static-site`
- **推荐理由：** 零依赖静态网页门户，浏览安装227+插件。
...
- **标签：** `pixel-art`, `sprites`, `cli`, `mascot`
- **推荐理由：** 公开像素宠物画廊，增强CLI体验。
...
- **标签：** `plugin`, `discovery`, `ecosystem`
- **推荐理由：** 自部署插件雷达，发现15900+候选并自动索引可用列表
...
- **推荐理由：** 精选DSH插件生态列表，整合公开源的工具与基础设施
...
- **推荐理由：** 全面精选插件目录，覆盖DeepSeek Harness完整生态
...
- **标签：** `security`, `audit`, `cybersecurity`, `redteam`
- **推荐理由：** AI安全攻防平台，审计代码幻觉与缺失防护。
...
- **标签：** `oauth`, `chatgpt`, `claude`, `llm`, `deepseek-harness`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅接入DSH，免API密钥。
...
- **标签：** `cost-tracking`, `token-usage`, `deepseek-api`, `llm`, `deepseek-harness`
- **推荐理由：** 实时追踪会话/日成本、预算、历史记录和多供应商余额。
...
- **标签：** `backup`, `sync`, `migration`
- **推荐理由：** 备份、恢复和同步完整DSH配置
...
- **标签：** `capabilities`, `menu`, `management`
- **推荐理由：** 统一管理MCP工具暴露和执行能力的面板
...
- **推荐理由：** 统一管理加载本机Agent Skills的安全工具
...
- **标签：** `container`, `management`, `isolated`, `infra`
- **推荐理由：** 多版本DSH容器隔离运行，内嵌WebView管理界面。
...
- **推荐理由：** DeepSeek API实时Token用量图表和成本估算
...
- **标签：** `diagnostic`, `plugin-management`, `dsh-plugin`
- **推荐理由：** 诊断并解决插件版本冲突问题，提供实用方案
...
- **推荐理由：** 通过OAuth将ChatGPT Plus订阅接入DSH。
...
- **推荐理由：** 鲸鱼娘系列主题皮肤，为DeepSeek Harness提供个性化界面
...
- **标签：** `theme`, `glassmorphism`, `customizable`, `ui`
- **推荐理由：** 高度可定制毛玻璃主题，支持模糊度与壁纸调节。
...
- **标签：** `theme`, `animated`, `ocean`, `threejs`
- **推荐理由：** 实时海洋动画主题，支持波浪、日落与玻璃效果调节。
...
- **标签：** `ai`, `rag`, `mcp`, `agent-skills`
- **推荐理由：** 全面的AI资源库，含教程、提示词和知识库。
...
- **标签：** `harness-engineering`, `dsh-plugin`, `agent`
...
- **标签：** `documentation`, `deepwiki`, `code-analysis`, `dsh-plugin`
- **推荐理由：** 快速生成代码技术文档和AI可用上下文
...
- **标签：** `arxiv`, `latex`, `research`, `experiment-tracking`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台，��持LaTeX、arXiv文献管理和GPU任务编排。
...
- **标签：** `marketplace`, `awesome-list`, `registry`
- **推荐理由：** 万款插件目录，支持搜索排行，插件生态研究必备
...
- **标签：** `handbook`, `awesome-list`, `documentation`
- **推荐理由：** 173个源码支撑的手册，涵盖运行时与MCP知识
...
- **标签：** `awesome-list`, `resources`, `development`
- **推荐理由：** 精选插件、工具和开发资源列表
...
- **标签：** `api-documentation`, `chinese-api`, `multi-platform`
- **推荐理由：** 零配置搜索20个中国开放平台6万+API文档
...
- **标签：** `patent`, `legal-tech`, `rag`, `analysis`
- **推荐理由：** 输入专利号自动生成竞品侵权分析报告
...
- **标签：** `2origin`, `plugin-registry`, `ai-agent`
- **推荐理由：** 基于证据的插件雷达图与插件实验室分析。
...
- **标签：** `awesome-list`, `bilingual`, `plugin-curation`
- **推荐理由：** 双语插件目录，证据驱动的评审列表，适合研究选型。
...
- **推荐理由：** 自动更新双语插件索引，YAML驱动生成，适合插件调研。
...
- **标签：** `awesome-list`, `curated`, `bilingual`, `reference`
- **推荐理由：** 中英文双语DSH插件、工具、技能精选导航列表。
...
- **标签：** `encyclopedia`, `knowledge-base`, `awesome-list`, `learning`
- **推荐理由：** 一站式AI资源百科，涵盖Agent、RAG、MLOps与学习路线。
...
- **标签：** `awesome-list`, `plugin-discovery`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 每日自动精选插件列表，按场景分类和热度排行快速筛选。
...
- **标签：** `awesome-list`, `deepseek-harness`, `developer-tools`, `guide`
- **推荐理由：** DSH终极指南，涵盖快速入门、资源推荐和精选插件工具。
...
- **标签：** `deepseek-harness`, `dsh`, `plugin-marketplace`, `best-practices`
- **推荐理由：** 学习DSH原理、浏览插件市场并探索最佳实践的实用工具。
...
- **标签：** `awesome-list`, `mcp`, `coding-agent`, `llm-agent`, `deepseek-harness`
- **推荐理由：** 精选插件、技能、MCP服务器、补丁和UI的集合，支持可视化。
...
- **推荐理由：** 专注股票/金融/量化的插件精选列表，24个验证条目
...
- **标签：** `awesome-list`, `inventory`, `dsh-plugin`
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件产品说明书与导航：按场景选���件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `plugin-directory`, `dsh-plugin`
- **推荐理由：** 双语插件指南，涵盖安装、权限与风险说明。
...
- **标签：** `catalog`, `dsh-plugin-directory`, `awesome-list`
- **推荐理由：** 通过GitHub Actions每日爬取的双语插件目录。
...
- **语言：** Unknown
- **描述：** DSHA精选插件合集
- **标签：** `dsh-plugins`, `plugin-collection`
- **推荐理由：** DSHA精选插件合集，整理规范。
...
- **标签：** `dsh-bundle`, `dsh-ecosystem`, `dsh-plugins-net`
- **推荐理由：** DSH插件仓库与生态分析平台。
...
- **标签：** `redirect`, `moved`
- **推荐理由：** 仓库已迁移至awesome-dsh-plugin/awesome-dsh-plugin。
...
- **语言：** Unknown
- **描述：** DSH插件
- **标签：** `awesome-list`, `minimal`
- **推荐理由：** DSH插件列表，元数据较少，用途不够明确。
