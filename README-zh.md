...
- **标签：** `ai`, `ai-agent`, `ai-agents`, `claude`, `claude-code`, `codex`, `deepseek`, `harness`, `llm`, `mcp`, `multi-agent`, `openai`, `openclaw`, `skills`
- **推荐理由：** 超级AI助手框架，自进化记忆与多模型支持。
...
- **推荐理由：** DeepSeek原生终端编程Agent，前缀缓存稳定性优异。
...
- **标签：** `agent-orchestration`, `mcp`, `workspace`, `coding-agent`
...
- **标签：** `agent-os`, `llm-orchestration`, `mcp`, `skills`
- **推荐理由：** 自进化Agent系统，带评估门禁与预算演化循环
...
- **标签：** `preset`, `dsh-plugin`
- **推荐理由：** 两阶段预设，从精简引导到完整工具配置
...
- **标签：** `terminal`, `vfs`, `dsh-plugin`
- **推荐理由：** AI代理专用虚拟终端，支持虚拟文件系统
...
- **标签：** `runtime`, `desktop`, `dsh-plugin`
- **推荐理由：** 高性能Rust代理运行时与精美桌面应用
...
- **推荐理由：** 打通Pi与DSH生态，一行ABI兼容层迁移原有扩展。
...
- **标签：** `plugin-dev`, `build`, `harness`
- **推荐理由：** 构建DSH插件的Agent Skill，含slots和凭证管理
...
- **标签：** `spec-driven`, `coding`, `development`
- **推荐理由：** 仓库原生规格驱动开发Skill，适配DSH模式
...
- **标签：** `plugin-creator`, `codex`, `dsh`
- **推荐理由：** 复用型 Codex 技能，快速构建和验证 DSH 插件
...
- **标签：** `agent-skills`, `ai-design`, `byok`, `design-systems`, `desktop-app`, `dsh-plugin`, `figma-alternative`, `ui-generator`, `vibe-coding`
- **推荐理由：** 本地优先的设计工具，DSH版Claude Design替代方案。
...
- **标签：** `dsh-web`, `ui`, `plugin-aggregation`
- **推荐理由：** DSH Web插件聚合生态，Creative Workshop分发
...
- **推荐理由：** Claude Code风格终端UI，含实时状态与回滚
...
- **⭐ 星标：** 1,824
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支��拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `widget`, `balance`, `ui`
- **推荐理由：** 可爱鲸鱼小部件，实时追踪账户余额
...
- **标签：** `genui`, `ui-components`, `interactive`
- **推荐理由：** 在回复中内联渲染图表、表单等交互UI组件。
...
- **标签：** `desktop`, `electron`, `ssh-client`
- **推荐理由：** 零配置Windows桌面客户端，支持SSH和插件。
...
- **标签：** `desktop`, `local-first`, `privacy`
- **推荐理由：** 本地优先多模型桌面应用，自进化技能。
...
- **标签：** `office`, `desktop`, `ui`
- **推荐理由：** 在DSH中集成电子表格、文档、演示文稿
...
- **标签：** `ios`, `mobile`, `websocket`
- **推荐理由：** 原生iOS客户端，WebSocket实时同步，移动便捷使用Harness。
...
- **⭐ 星标：** 168
- **语言：** TypeScript
- **描述：** 一个基于 DeepSeek Harness 插件机制构建的多端远程访问方案，通过安全、低延迟、端到端加密的 P2P 优先网络，支持从 PC、Android 和 Web ���时访问并操作远程 Harness 和 CodeX。 (A multi-device remote access solution built on the DeepSeek Harness plugin system, enabling PC, Android, and Web clients to securely access and operate remote Harness and CodeX over a low-latency, end-to-end encrypted, P2P-first network.)
- **标签：** `remote`, `p2p`, `encryption`, `multi-device`
- **推荐理由：** 端到端加密P2P远程访问，支持多端操作Harness。
...
- **标签：** `notes`, `smartisan`, `docker`, `social`
- **推荐理由：** 开源锤子便签复刻，Docker部署支持skill调用与多平台推送。
...
- **标签：** `market`, `plugin`, `webui`, `ui`
- **推荐理由：** 一键浏览安装社区插件，丰富生态体验。
...
- **标签：** `desktop`, `electron`, `ui`
- **推荐理由：** Electron桌面控制台，多Tab管理DSH/MCP/Skill
...
- **推荐理由：** 底部信息栏，实时展示 provider、余额、价格与倒计时
...
- **推荐理由：** WorkBuddy 风格技能选择器，Composer 内一键插入
...
- **标签：** `bioinformatics`, `desktop-app`, `tauri`, `llm-agent`, `single-cell`
- **推荐理由：** 生物信息学桌面代理，含技能商店和离线模式
...
- **标签：** `desktop`, `docker`, `ssh`, `sftp`, `devops`, `dsh-plugin`
- **推荐理由：** DevOps一体化桌面：SSH/SFTP/Docker/数据库/AI助手
...
- **标签：** `knowledge-cards`, `ui-extension`
...
- **标签：** `agent-memory`, `rag`, `markdown`, `long-term-memory`
- **推荐理由：** 本地优先的Markdown持久化记忆层，跨应用共享
...
- **标签：** `agent-memory`, `rag`, `token-savings`, `skills`
- **推荐理由：** 自演化记忆系统，混合检索节省35%令牌消耗
...
- **标签：** `agent-memory`, `agentic-rag`, `context-engineering`
- **推荐理由：** 专为状态Agent设计的记忆库，支持持续学习
...
- **标签：** `memory`, `consolidation`, `proactive`
- **推荐理由：** 三层自动记忆系统，零提示注入回忆
...
- **标签：** `agentic-ai`, `agentic-framework`, `agentic-workflow`, `agents`, `ai-assistant`, `ai-skills`, `autonomous-agents`, `dsh-plugin`, `harness`, `mcp-server`, `multi-agent`, `skills`, `swarm`, `swarm-intelligence`
- **推荐理由：** 多智能体蜂群编排器，实现自主工作流协调。
...
- **标签：** `agent-skills`, `workflow`, `mcp`, `router`
- **推荐理由：** 运行时注入器+任务感知路由器，智能模式切换
...
- **标签：** `agent-skills`, `skill-store`, `finance`, `office`
- **推荐理由：** 最大技能商店，自动分类整理上万Skills项目
...
- **标签：** `agent-control-plane`, `loop-engineering`, `workflow`
- **推荐理由：** 长期任务控制面，支持持久治理的多Agent工作流
...
- **标签：** `skills`, `workflow`, `dsh-plugin`
- **推荐理由：** 智能技能路由与工作流编排，性能显著提升
...
- **标签：** `cognitive`, `skills`, `dsh-plugin`
...
- **标签：** `video`, `skill`, `generation`
- **推荐理由：** AI一键生成Vox风格视频解说
...
- **标签：** `research`, `skill`, `chinese-web`
- **推荐理由：** 自动搜索中国八大平台近30天内容
...
- **推荐理由：** 原生SQLite任务看板，支持Agent任务认领审核。
...
- **标签：** `creative-writing`, `novel`, `drama-skills`
- **推荐理由：** 小说写作与短剧制作专用技能，基于Oh Story引擎。
...
- **标签：** `workflow`, `math-modeling`, `gitops`
- **推荐理由：** 全栈自动数学建模Skill，含GitOps流水线
...
- **标签：** `agent`, `self-evolution`, `workflows`
- **推荐理由：** 可组合任务控制和人工审核的自进化Agent模式
...
- **推荐理由：** 为纯文本LLM增添视觉能力，支持截图/UI/图表分析。
...
- **标签：** `skills`, `research`, `marketing`, `multi-platform`
- **推荐理由：** 88个开源技能覆盖研究、营销自动化与社会情报分析。
...
- **标签：** `skill`, `agent`, `automation`, `learning`
- **推荐理由：** 自动将成功会话转为可复用Skill，提升效率。
...
- **推荐理由：** TDD、调试、规划等Superpowers技能适配DSH。
...
- **推荐理由：** AI编程导师，扫描架构、生成阶梯教程、Code Review。
...
- **标签：** `traditional-wisdom`, `agent-skill`, `bazi`
- **推荐理由：** 八字风水易经融合 AI 技能，本地确定性引擎驱动
...
- **标签：** `video`, `ai-media`, `directorx`
- **推荐理由：** AI 视频/图片/音频技能，可配置视觉媒体模型
...
- **标签：** `qa`, `testing`, `skills`
- **推荐理由：** 11 个 QA 技能加共享知识库，AI 测试自动化框架
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`, `skills`, `mcp`
- **推荐理由：** 在DSH Web UI中直接管理技能与MCP服务器
...
- **标签：** `godot`, `godot-engine`, `gdscript`, `game-development`, `dsh-plugin`
- **推荐理由：** 为DSH代理提供Godot 4.x全栈游戏开发技能
...
- **标签：** `skillhub`, `deepseek-harness`, `dsh-plugin`, `api`
- **推荐理由：** 官方SkillHub含Open API文档与DSH插件集成
...
- **标签：** `image-generation`, `vision`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 图像读取与生成技能，支持自动视觉转写
...
- **标签：** `engineering-discipline`, `quality-gate`, `delivery-gate`, `dsh-plugin`, `skill`
- **推荐理由：** 工程纪律技能包：检验需求并验证交付质量
...
- **标签：** `finance`, `mcp`, `database`, `tushare`
- **推荐理由：** 基于Tushare本地财经数据库，自动生成MCP接口支持多Agent分析。
...
- **标签：** `mcp`, `cloud`, `huawei-cloud`
- **推荐理由：** 华为云官方MCP工具包，含沙箱和合规防护
...
- **⭐ 星标：** 28
- **语言：** TypeScript
- **描述：** 一键配置各类 AI Agent 工具��� MCP、Skills 和规则；支持 Claude Code、Codex、DeepSeek Harness、OpenCode、Cursor、Gemini CLI、Qwen Code 等。 One-click local setup for MCP servers, skills, and rules across AI coding agents.
- **标签：** `mcp`, `config-sync`, `multi-platform`
- **推荐理由：** 一键同步 MCP/Skills 配置至多平台 AI Agent
...
- **标签：** `agent-skills`, `architecture-as-code`, `architecture-diagram`, `code-visualization`, `data-flow-diagram`, `deepseek-harness`, `diagram-as-code`, `diagrams`, `mermaid-alternative`, `sequence-diagram`, `software-architecture`, `text-to-diagram`
- **推荐理由：** 代码生成高质量架构图、序列图和数据流图。
...
- **推荐理由：** AI原生矢量���计工具，支持Agent团队协作生成UI
...
- **标签：** `design-skills`, `design-system`, `ui`, `vibe-coding`, `web-design`
...
- **标签：** `background`, `customization`, `webui`
- **推荐理由：** 支持自定义背景图片及预设氛围主题，实时预览
...
- **标签：** `content-discovery`, `web`, `dsh-plugin`
- **推荐理由：** 跨平台AI内容发现代理，支持多社交媒体
...
- **推荐理由：** 内置22个Agent工具实现iOS模拟器和设备自动化
...
- **推荐理由：** OAuth绑定ChatGPT账号，DSH内直接使用ChatGPT模型
...
- **标签：** `theme`, `glass-effect`, `customizable`
- **推荐理由：** 毛玻璃质感主题，自由调节模糊度与背景。
...
- **标签：** `theme`, `skin`, `webgpu`, `visual`
- **推荐理由：** 基于WebGPU和Three.js的海洋主题视觉皮肤
...
- **标签：** `ai-agent`, `chatgpt`, `deepseek`, `prompt`, `tutorial`
...
- **标签：** `harness`, `tutorial`, `dsh-plugin`, `beginner`
- **推荐理由：** 从零开始的DSH插件工程入门最佳指南
...
- **⭐ 星标：** 230
- **语言：** TypeScript
- **描述：** Mimir — 一站式科研工作台插件：LaTeX 论文边写边���译、arXiv 文献管理、实验追踪、指标图表、GPU 服务器 SSH 任务编排，管理科研全周期。An open-source research workbench plugin for the whole research cycle.
- **标签：** `research`, `latex`, `arxiv`, `gpu`
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU远程管理
...
- **⭐ 星标：** 198
- **语言：** Python
- **描述：** Academic Paper Search：中文科研用户的 Codex / Claude Code / DeepSeek Harness Skill + MCP；跨 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC 检索去重，��持 MeSH、引用核验、引文图谱、试验检索与审计导出。
- **标签：** `academic`, `mcp`, `research`, `citation`
- **推荐理由：** 多源学术检索支持引用核验与图谱，中文科研首选。
...
- **标签：** `api`, `documentation`, `research`, `chinese-platforms`
...
- **标签：** `evaluation`, `rag-evaluation`, `prompt-engineering`, `agent-evaluation`, `dsh-plugin`
- **推荐理由：** 基于证据的提示词、RAG和代理评估框架
...
- **标签：** `dsh-bundle`, `dsh-plugin-verify`, `dsh-repository-plugin`
- **推荐理由：** 带验证功能的DSH插件精选目录
...
- **标签：** `2origin`, `ai-agent`, `plugin-registry`
- **推荐理由：** 插件雷达结合2Origin实验室，支持DSH生态
...
- **标签：** `ai-agents`, `coding-agents`, `mcp`
- **推荐理由：** AI托管小时更新的190+插件推荐列表
...
- **标签：** `ai-agents`, `awesome-list`, `plugins`
- **推荐理由：** 80+个经过验证的功能分类插件目录
...
- **推荐理由：** YAML驱动自动更新的双语插件仓库，Vercel部署
...
- **标签：** `mascot`, `pixel-art`, `claude-code`, `codex`, `dsh-plugin`
- **推荐理由：** 可爱的动画宠物画廊，支持主流Agent工具链集成
...
- **标签：** `radar`, `registry`, `ecosystem`
- **推荐理由：** 自部署插件生态雷达，自动发现上万个插件
...
- **标签：** `encyclopedia`, `knowledge-base`, `resources`
- **推荐理由：** 全维度AI资源百科，覆盖技能、RAG和学习路线。
...
- **标签：** `marketplace`, `plugins`, `directory`
- **推荐理由：** 每日更新的全方位插件市场，含审核机制
...
- **标签：** `marketplace`, `directory`, `api`
- **推荐理由：** 万级插件商店，含搜索、排名和公共API
...
- **推荐理由：** 精选DSH插件awesome-list，便于快速发现和参考。
...
- **标签：** `awesome`, `plugin`, `directory`
- **推荐理由：** cordis.run官方维护的插件精选目录。
...
- **标签：** `community`, `catalog`, `marketplace`, `awesome`
...
- **标签：** `awesome-list`, `curated-list`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 精选DSH插件列表，含交互式网页目录
...
- **标签：** `awesome-list`, `deepseek-harness`, `dsh-plugin`, `mcp`, `skills`
- **推荐理由：** 全面的DSH插件、工具、技能和资源精选列表
...
- **标签：** `awesome-list`, `plugin-directory`, `typescript`, `dsh-plugin`
- **推荐理由：** 公开GitHub目录，提供安装命令和插件发现功能
...
- **标签：** `plugin-registry`, `curated-list`, `dsh-plugins`, `skills`, `open-data`
- **推荐理由：** 经垃圾过滤的DSH开源插件注册表和技能目录
...
- **标签：** `awesome-list`, `documentation`, `navigation`
...
- **标签：** `awesome-list`, `portal`, `static-site`
- **推荐理由：** 无依赖静态插件发现与安装门户
