...
- **标签：** `cli`, `tui`, `terminal`, `r1`
- **推荐理由：** DeepSeek原生终端编码代理，前缀缓存稳定高效
...
- **推荐理由：** 两阶段预设引导，快速为DSH提供完整Standard工具链。
...
- **推荐理由：** AI agent的虚拟终端沙箱，隔离并管理agent运行环境。
...
- **标签：** `model-switch`, `multi-agent`, `utility`
- **推荐理由：** 一键切换Claude Code/Codex/DSH/Kimi Code等多agent模型。
...
- **标签：** `radar`, `plugin`, `discovery`
- **推荐理由：** 自动发现15000+插件，生产级实测管线
...
- **推荐理由：** SQLite任务管理，原生UI无需iframe，Agent认领审核功能实用
...
- **标签：** `cost-tracking`, `budget`, `token-usage`
- **推荐理由：** 会话/日成本追踪，预算告警与余额监控，防超支必备
...
- **标签：** `cli`, `plugin-manager`, `schema`
- **推荐理由：** 开源CLI插件管理器，Schema解析器助力Hub插件运维
...
- **标签：** `llm-provider`, `typescript`, `plugin`
- **推荐理由：** Command Code的LLM提供者插件，支持模型选择与图像输入
...
- **标签：** `ai-infra`, `skills`, `system-optimization`
- **推荐理由：** AI基础设施开发专用技能，支持系统优化调试
...
- **标签：** `api-docs`, `chinese-platforms`, `zero-config`
- **推荐理由：** 在编码工具内直搜20平台6万+ API文档，零配置开箱即用
...
- **标签：** `plugin-builder`, `validator`, `coding-agent`
- **推荐理由：** 构建验证DSH/Cordis插件，自动管理插槽和凭证
...
- **标签：** `plugin-creator`, `codex`, `validator`
...
- **标签：** `codex`, `mcp`, `deepseek-harness`
- **推荐理由：** Codex与DSH双向同步桥接，支持MCP镜像
...
- **推荐理由：** 开放式侧边栏底座，内置文件编辑/终端/Git等页面。
...
- **标签：** `taskboard`, `ui`, `panel`
- **推荐理由：** 现代化灵活任务面板，无缝集成Codex和DSH。
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** DSH鲸鱼顶栏TUI，支持流式思考、双击Esc回滚等功能。
...
- **标签：** `widget`, `ui`, `balance`
- **推荐理由：** DSH右下角小鲸鱼挂件，实时追踪账户余额。
...
- **推荐理由：** 功能完整的Electron桌面客户端，支持SSH和移动端。
...
- **标签：** `gui`, `interactive`, `inline-ui`
- **推荐理由：** 对话内嵌交互UI，图表表单即时呈现。
...
- **标签：** `desktop`, `tauri`, `local-first`
- **推荐理由：** 本地优先Tauri桌面代理，隐私安全多模型。
...
- **标签：** `office`, `spreadsheet`, `documents`
- **推荐理由：** 完整办公套件：表格/文档/幻灯片内嵌，提升工作区生产力
...
- **推荐理由：** 对话内嵌入iOS模拟器，22个Agent工具驱动真机自动化
...
- **推荐理由：** 类CodePilot桌面客户端，Electron多端支持macOS/Windows/Linux
...
- **标签：** `ios`, `mobile`, `native`, `websocket`
- **推荐理由：** 原生iOS客户端，WebSocket连接工作区与会话，移动协作便捷
...
- **标签：** `ui`, `marketplace`, `plugin-market`
- **推荐理由：** Web界面一键浏览安装社区插件
...
- **标签：** `desktop`, `electron`, `ts`, `ui`
- **推荐理由：** Electron桌面控制台，多Tab管理DSH插件/MCP/Skill
...
- **标签：** `desktop`, `devops`, `docker`
- **推荐理由：** 集成数据库/SSH/SFTP/Docker的DevOps桌面中心
...
- **推荐理由：** Electron桌面壳整合DSH，含侧边栏和ModLens。
...
- **推荐理由：** 专为agent设计的记忆管理工具包，实现知识记忆与精炼。
...
- **推荐理由：** 持久化统一agent记忆层，由Markdown和Milvus驱动。
...
- **标签：** `memory`, `agent`, `persistent`
- **推荐理由：** 所有AI代理共享持久记忆中心，全可控
...
- **标签：** `skill`, `corpus`, `retrieval`
- **推荐理由：** 将散乱SKILL.md转为可检索语料库
...
- **标签：** `academic-search`, `citation`, `crossref`, `arxiv`
- **推荐理由：** 跨平台学术检索：CrossRef/PubMed/arXiv去重，引用核验与图谱
...
- **标签：** `memory`, `rag`, `auto-consolidation`
- **推荐理由：** 三层自动记忆，模型响应前零提示主动召回
...
- **标签：** `cognitive-enhancement`, `skills`, `research`
...
- **标签：** `video`, `skill`, `workflow`
- **推荐理由：** 端到端视频生成技能，一键完成Vox风格视频
...
- **推荐理由：** 支持多代理协作，扩展DeepSeek Harness能力
...
- **标签：** `vision`, `ocr`, `toolkit`
- **推荐理由：** 视觉工具箱：OCR识别、截图问答全能
...
- **标签：** `multimodal`, `ocr`, `skill`
- **推荐理由：** 多模态感知技能，视频音频屏幕转证据。
...
- **推荐理由：** 自动数学建模竞赛Skill，GitOps流水线+代码自证国赛美赛通用
...
- **⭐ 星标：** 206
- **语言：** TypeScript
- **描述：** 让 Agent 的工作方式可组合、可审查、可持��改进，最终实现 Agent Self Evoling。 DeepSeek Harness Web plugin with composable task controls and isolated, human-reviewed self-evolution.
- **标签：** `agent-review`, `plan-mode`, `prompt-engineering`
- **推荐理由：** 支持组合式可审查的Agent自进化工作流
...
- **⭐ 星标：** 173
- **语言：** JavaScript
- **描述：** 帮 DSH 搜索、安装并��证插件的 Skill｜A DSH skill that finds, installs, and verifies GitHub plugins
- **标签：** `plugin-discovery`, `agent-skills`, `automation`
- **推荐理由：** 通过Skill自动搜索安装验证DSH插件
...
- **标签：** `skill-learning`, `automation`
- **推荐理由：** 将成功会话自动转化为可复用技能
...
- **标签：** `skills`, `coding`, `mentor`
- **推荐理由：** AI编程导师：架构分析、阶梯教学、代码审查
...
- **标签：** `patent-analysis`, `legal-tech`, `rag`, `multi-agent`
- **推荐理由：** 任意Agent可调用的专利侵权分析技能，RAG+多智能体
...
- **标签：** `skills`, `matt-pocock`, `workflow`
- **推荐理由：** 25个内置Matt Pocock工程效率技能，安装即调用
...
- **标签：** `gongwen`, `gb-t-9704`, `doc-processing`
- **推荐理由：** 国标公���全流程处理，格式检查修复/内容优化/模板生成
...
- **标签：** `spec-driven`, `development`, `agent-skill`
- **推荐理由：** 仓库级Spec驱动开发技能，内联DSH工程模式
...
- **标签：** `skillhub`, `api`, `deepseek-harness`
- **推荐理由：** 腾讯官方SkillHub API文档与DSH示例
...
- **⭐ 星标：** 21
- **语言：** TypeScript
- **描述：** dsh-plugin-developer — DeepSeek Harness 插件开发 Skill  > 指导 AI Agent 从 0 到 1 开发、构建、安装、测试 DeepSeek Harness（dsh）��件。基于 **dsh 0.1.1-rc.2** 与 dsh-openmaic 项目的完整实战经验，并内置一个**已通过 web 界面实测**的天气插件作为整包参考案例。  ## 这是什么  dsh 采用"无特权内核、万物皆插件"的设计：模型适配器、工具注册表、Agent 循环、网页界面都是插件
- **标签：** `plugin-dev`, `skill`, `dsh`
- **推荐理由：** 指导AI从0到1开发测试DSH插件的技能
...
- **标签：** `plugin-dev`, `agent-skills`, `standard`
- **推荐理由：** DSH插件/服务/工具开发标准Agent技能
...
- **标签：** `tts`, `xiaomi-mimo`, `voice`
- **推荐理由：** 接入免费MiMo TTS，支持预设/自定义/浏览器声音。
...
- **推荐理由：** 基于Tushare构建本地财经数据库，支持MCP自动调用
...
- **标签：** `huawei-cloud`, `mcp`, `cloud`, `official`
- **推荐理由：** 华为云官方插件，含Skills/MCP工具/云端沙箱安全护栏
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`, `code-visualization`
- **推荐理由：** 生成精美架构与序列图，自包含HTML输出
...
- **推荐理由：** DSH首个视觉插件，为纯文本agent添加OCR和图片理解能力。
...
- **标签：** `design`, `template`, `visual`
- **推荐理由：** 可编辑设计系统，含模���市场和PPT功能
...
- **标签：** `multimodal`, `vision`, `ui-analysis`
- **推荐理由：** 为纯文本LLM添加强大视觉分析能力
...
- **标签：** `visualization`, `legal`, `svg`, `timeline`
- **推荐理由：** 将杂乱案件材料绘制为精确时间轴/诉讼图(SVG)
...
- **推荐理由：** 像素到文字OCR工具，支持image_scan/image_ocr工具链
...
- **推荐理由：** 钉钉IM通道，流式模式将聊天转为可工具化agent
...
- **标签：** `discovery`, `verification`, `ranking`
- **推荐理由：** 每日抓取GitHub插件并复核，分类收录与热度排行帮快速选型
...
- **标签：** `guide`, `quickstart`, `resources`
- **推荐理由：** 终极入门指南：快速上手、资源推荐与插件精选目录
...
- **标签：** `learning`, `best-practices`, `exploration`
...
- **标签：** `catalog`, `mcp`, `visualization`
- **推荐理由：** 精选插件/Skill/MCP服务器列表，含可视化与PPT导览
...
- **标签：** `marketplace`, `11k-plugins`, `api`
- **推荐理由：** 1.1万+插件市场，搜索排行与免费API，插件生态聚合平台
...
- **标签：** `remote-access`, `security`, `multi-device`
- **推荐理由：** 端到端加密P2P远程访问多端Harness
...
- **标签：** `plugin-registry`, `infrastructure`, `cli`
- **推荐理由：** 官方插件注册台+开发脚手架引导，生态基建工具
...
- **⭐ 星标：** 42
- **语言：** JavaScript
- **描述：** DSH Skills Manager — 在 DeepSeek Harness 中统一加载并安全管理���机 Agent Skills · Load and safely manage local Agent Skills in DSH
- **标签：** `skills-manager`, `security`, `utility`
- **推荐理由：** 统一安全管理本地Agent Skills加载器，含访问控制
...
- **标签：** `awesome-list`, `plugin-index`, `community`
- **推荐理由：** 14类280+精选插件目录，一键直达仓库
...
- **推荐理由：** 实时Token用量追踪，含成本估算和API余额监控。
...
- **标签：** `conflict-resolution`, `debugging`, `utility`
...
- **标签：** `skin`, `theme`, `desktop`
- **推荐理由：** 鲸鱼娘系列可爱皮肤，美化DeepSeek Harness桌面
...
- **⭐ 星标：** 406
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH 任何一��源码。
- **标签：** `theme`, `glass`, `visual`
- **推荐理由：** 磨砂玻璃主题，高度自定义模糊和背景。
...
- **标签：** `webui`, `background`, `customization`
- **推荐理由：** 自定义背景图，支持实时预览和预设氛围主题。
...
- **推荐理由：** 跨平台内容发现agent，支持B站/YouTube/Reddit等，隐私优先。
...
- **标签：** `research`, `search`, `china`
- **推荐理由：** 搜索8大中文平台，自动生成30天研究报告
...
- **标签：** `marketplace`, `plugin-shop`, `curated`
- **推荐理由：** 每日更新的全面插件市场，发现神器。
...
- **标签：** `encyclopedia`, `knowledge`, `resource`
- **推荐理由：** 一站式AI百科全书，技能MCP教程齐全。
...
- **标签：** `research`, `latex`, `arxiv`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX边写边编译、arXiv管理、GPU调度编排
...
- **推荐理由：** DSH全面手册，覆盖运行时、插件、MCP及故障排查
...
- **标签：** `fintech`, `backtesting`, `research`
- **推荐理由：** 多策略基金回测与合规投资研究工作台
...
- **标签：** `awesome-list`, `plugin-directory`, `resources`
- **推荐理由：** 全面的DSH插件、工具和技能精选列表。
...
- **标签：** `awesome-list`, `curated-directory`, `verified`
- **推荐理由：** 基于证据的插件目录，含验证扩展和安装指南。
...
- **标签：** `finance`, `stock`, `quant`, `trading`
- **推荐理由：** 股票/金融/量化垂直插件列表，24个验证条目。
...
- **标签：** `guide`, `documentation`, `plugin-directory`
- **推荐理由：** 插件产品说明书，含场景选择和风险说明。
...
- **标签：** `awesome-list`, `inventory`, `catalog`
- **推荐理由：** 1000+工具全量清单，按Star排序。
...
- **推荐理由：** 有据可依的DSH插件雷达与2Origin插件实验室
...
- **推荐理由：** GitHub Actions自动爬取的双语插件目录，每日更新
...
- **推荐理由：** 在DSH agent内搜索npm和精选插件目录，提升发现效率
...
- **标签：** `ai-agents`, `coding-agents`, `deepseek-harness`, `mcp`
- **推荐理由：** AI每小时更新的190+插件推荐，覆盖12个分类
...
- **标签：** `deepseek-harness`, `dsh-bundle`, `dsh-plugins`
- **推荐理由：** 通用DSH插件注册表，涵盖多种插件类型
...
- **标签：** `dsh-plugins`
- **推荐理由：** 已迁移至awesome-dsh-plugin仓库，仅供参考
...
- **标签：** `ai`, `ai-agent`, `artificial-intelligence`, `chatgpt`, `claude`, `claude-code`, `codex`, `cursor`, `deep-learning`, `deepseek`, `gemini`, `generative-ai`, `gpt`, `llm`, `mcp`, `openai`, `rag`, `vibe-coding`, `vibecoding`, `vuepress`
...
- **标签：** `agent`, `agentic-ai`, `ai`, `ai-agents`, `chatgpt`, `claude`, `deepseek-harness`, `dsh-plugin`, `hermes`, `llm`, `long-term-memory`, `mcp`, `memory`, `memory-management`, `openclaw`, `rag`, `self-evolving`, `skills`, `token-savings`
...
- **推荐理由：** DeepSeek Harness (DSH) Web Plugin Aggreg
...
- **标签：** `agent-memory`, `agentic-rag`, `ai`, `ai-agents`, `ai-memory`, `context-engineering`, `continual-learning`, `dsh-plugin`, `llm`, `memory`
- **推荐理由：**  Memory library for building stateful ag
...
- **标签：** `agent-skills`, `claude-skills`, `codex`, `deepseek`, `deepseek-harness`, `developer-tools`, `dsh-plugin`, `finance-skill`, `gemini-skills`, `hermes-skill`, `office-skill`, `openclaw-skills`, `opencode`, `skill`, `skill-md`, `skills`
- **推荐理由：** 收录最全、更新最快的技能Skills商店：精选原创技能包（涵盖文档处理、内容创作
...
- **标签：** `agent-os`, `agentic-ai`, `ai-agent`, `ai-coding-agent`, `claude-code`, `cli`, `codex`, `coding-agent`, `deepseek`, `developer-tools`, `dsh`, `dsh-plugin`, `llm-evaluation`, `llm-orchestration`, `loop-engineering`, `mcp`, `opencode`, `skill`, `skills`
...
- **标签：** `agent-control-plane`, `agent-harness`, `agent-ops`, `ai-agents`, `codex`, `dsh-plugin`, `long-horizon`, `long-horizon-agents`, `loop-engineering`, `loopx`, `workflow-automation`
- **推荐理由：** Long-horizon agent control plane for dur
...
- **标签：** `marketplace`, `awesome-list`, `community`
- **推荐理由：** 社区插件市场，每4小时自动同步目录
...
- **⭐ 星标：** 26
- **语言：** Python
- **描述：** DeepSeek自带破甲18+互动叙事 Skill：NPC 活人感（有记忆/立场/底线）、数百项素材库、随机开局可预锁、时间推进、全维 YAML 存档。
- **标签：** `agent-skills`, `ai-agent`, `ai-roleplay`, `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** DeepSeek自带破甲18+互动叙事 Skill：NPC 活人感（有记忆/立场
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin-bundle`, `dsh-plugin-desktop`
- **推荐理由：** 类CodeX插件市场，方便DSH用户发现和管理插件
