...
- **标签：** `ai-agents`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 自进化AI助手，支持多模型与持久记忆。
...
- **推荐理由：** 终端原生DeepSeek编码Agent，前缀缓存稳定。
...
- **标签：** `agent-orchestration`, `mcp`, `coding-agent`
...
- **推荐理由：** 自进化Agent操作系统，带面试门禁评估和预算进化循环
...
- **标签：** `agent-harness`, `loop-engineering`, `workflow-automation`
- **推荐理由：** 跨Harness的长周期Agent控制平面，支持持久化治理
...
- **标签：** `agent-workspace`, `multi-agent`, `coding-agent`
...
- **标签：** `code-review`, `code-quality`, `dsh-plugin`
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，附引用来源。
...
- **标签：** `security`, `vulnerability`, `redteam`, `dsh-plugin`
- **推荐理由：** AI安全攻防平台，审计幻觉包、缺失防护与漏洞
...
- **推荐理由：** 全能编码Agent，支持子Agent、RPC及MCP
...
- **推荐理由：** 打通Pi与DSH生态，让Pi插件在DSH中原生运行
...
- **⭐ 星标：** 168
- **语言：** TypeScript
- **描述：** Unofficial DeepSeek Harness LLM provider plugin for Command Code: live model catalog, plan-aware model picker, reasoning-effort + image input support, Command Code web search for dsh, multi-account rotation with browser sign-in. Ported from pi-commandcode-provider.
- **标签：** `llm-provider`, `typescript`, `provider`
- **推荐理由：** 非官方LLM提供程序，支持实时模型目录和智能体选择
...
- **标签：** `plugin-dev`, `agent-skills`, `adapter`
- **推荐理由：** 开发DSH插件与LLM适配器的标准化Agent Skill
...
- **标签：** `plugin-build`, `credentials`, `slots`
- **推荐理由：** 管理已安装插件、插槽与凭证的Agent Skill
...
- **标签：** `spec-driven`, `vibe-coding`, `developer-tools`
- **推荐理由：** 仓库原生规范驱动开发的便携式Agent Skill
...
- **标签：** `dsh-plugin`, `floating-widget`, `deepseek`
- **推荐理由：** 可爱鲸鱼浮动控件，实时监控DeepSeek余额，支持拖拽吸附。
...
- **推荐理由：** 本地优先Electron桌面工作区，支持DSH会话和Office。
...
- **标签：** `dsh-plugin`, `ui-components`, `charts`, `forms`
- **推荐理由：** 通过dsh-ui围栏在回复中渲染图表、表单、测验等交互式UI组件
...
- **推荐理由：** 开源Electron桌面客户端，集成Codex、SSH、技能与手机远程访问
...
- **标签：** `desktop-app`, `local-first`, `privacy`, `multi-model`
- **推荐理由：** 本地优先的多模型桌面AI代理，支持自进化技能与隐私保护
...
- **标签：** `canvas`, `workspace`, `conversation`, `non-linear`
- **推荐理由：** 基于画布的非线性对话工作区，支持会话分支与可视化探索
...
- **推荐理由：** 将表格、文档、幻灯片引入DeepSeek Harness
...
- **推荐理由：** 原生iOS客户端，WebSocket实时同步DeepSeek Harness工作区
...
- **推荐理由：** 开源版锤子便签，支持Docker私有化部署和Skill调用
...
- **标签：** `marketplace`, `ui`, `webui`
- **推荐理由：** Web界面一键安装卸载社区插件
...
- **标签：** `ui`, `menu`, `capabilities`
- **推荐理由：** 统一管理MCP和技能曝光级别与执行模式
...
- **标签：** `ui`, `picker`, `skill`
- **推荐理由：** 类似WorkBuddy的Skill选择器，快速插入技能指令
...
- **标签：** `bioinformatics`, `desktop`, `tauri`, `llm-agent`, `dsh-plugin`
- **推荐理由：** 含技能商店与离线模式的生信桌面应用，面向科研人员。
...
- **标签：** `desktop`, `devops`, `ssh`, `docker`, `dsh-plugin`
- **推荐理由：** 一体化DevOps桌面：数据库、SSH、SFTP、Docker和AI沙箱合一。
...
- **标签：** `webui`, `background`, `personalization`
- **推荐理由：** 支持自定义背景图片和多种预设氛围。
...
- **标签：** `desktop-app`, `electron`, `integration`
- **推荐理由：** Electron桌面封装，集成Dsh Web服务。
...
- **推荐理由：** 本地优先、Markdown原生的跨应用Agent记忆层
...
- **标签：** `agent-memory`, `agentic-rag`
- **推荐理由：** 构建有状态Agent的记忆库，支持智能RAG
...
- **标签：** `context`, `dsh-plugin`, `cordis-plugin`
- **推荐理由：** 最佳上下文管理插件，含面板、统计和浏览器功能。
...
- **标签：** `agent-memory`, `embeddings`, `vector-search`, `skill-md`
- **推荐理由：** 将SKILL.md转为检索就绪的技能库，支持向量搜索与重排序
...
- **标签：** `memory`, `auto-consolidation`, `skill-crystallization`
- **推荐理由：** 三层自动整合记忆，无需提示即可主动召回
...
- **标签：** `low-code`, `ai-agents`, `dsh-plugin`
- **推荐理由：** AI赋能的无代码平台，快速构建业务系统。
...
- **标签：** `agent-skills`, `skill-marketplace`
- **推荐理由：** 最全技能商店，自动抓取GitHub上万技能项目分类整理
...
- **标签：** `task-management`, `sqlite`, `workflow`, `agent`
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核流程与纯净Web UI
...
- **标签：** `creative-writing`, `novel`, `drama`, `fiction`
- **推荐理由：** 基于Oh Story技能的长篇小说与短剧创作专用插件
...
- **标签：** `agent`, `self-evolve`, `workflow`
- **推荐理由：** 可组合Agent工作流，支持人工审查自进化
...
- **推荐理由：** 为纯文本LLM赋能多模态视觉能力，支持截图和UI分析
...
- **标签：** `infra`, `skills`, `ai-infrastructure`
- **推荐理由：** AI基础设施开发专用技能，支持Claude Code高性能系统调试
...
- **推荐理由：** 将Superpowers技能框架移植至Reasonix
...
- **推荐理由：** TDD、调试、规划等Superpowers核心技能
...
- **推荐理由：** AI编程导师：扫描架构、生成章节、Code Review
...
- **⭐ 星标：** 53
- **语言：** JavaScript
- **描述：** Matt Pocock 技能面板（DSH Deck）：安装即自带 25 个工程与效率技能（mattpocock/skills v1.2.3：triage、grilling、wayfinder、handoff 等），右侧面板直接调用，无需手动装技能。当前主力支持 GitHub issue；Markdown 本地文件为预览版；GitLab 暂不在支持范围。希望它能帮你把想法更快变成现实。
- **标签：** `skills`, `productivity`, `github-issues`
- **推荐理由：** 25个工程技能预装，侧边栏一键调用
...
- **标签：** `knowledge-base`, `agent-skill`, `guide`
- **推荐理由：** 官方文档知识库，按需调用的Agent技能
...
- **标签：** `simplification`, `codex`, `skill`
- **推荐理由：** 自动发现和简化复杂代码模式的Codex技能
...
- **标签：** `video`, `motion-design`, `remotion`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 152张电影感镜头配方卡，搭配Remotion模板制作产品视频。
...
- **⭐ 星标：** 3
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起���前检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用率可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `ai-agents`, `dsh-plugin`
- **推荐理由：** 任务前预检插件库，提升agent插件利用率
...
- **标签：** `finance`, `mcp`, `stock`, `tushare`
- **推荐理由：** 基于tushare的财经数据库，支持MCP调用和多Agent金融分析
...
- **推荐理由：** 从DSH Web UI直接管理技能与MCP服务器，开箱即用。
...
- **标签：** `agent-skills`, `design-systems`, `dsh-plugin`
- **推荐理由：** 本地优先的设计插件，媲美Figma的AI生成体验。
...
- **标签：** `visual-design`, `mcp`, `vibecoding`
...
- **标签：** `deepseek`, `armor-breaking`, `dsh-plugin`
- **推荐理由：** DeepSeek图像生成破甲插件，输出更稳定。
...
- **标签：** `legal-tech`, `visualization`, `svg`
- **推荐理由：** 法律时间轴与诉讼图谱可视化工具
...
- **标签：** `design`, `ui`, `vibe-coding`, `dsh-plugin`
- **推荐理由：** 6种专业设计风格，避免AI生成网站的同质化外观。
...
- **标签：** `dsh-plugin`, `knowledge-cards`
- **推荐理由：** 将聊天记录转化为精美知识卡片，支持下载分享
...
- **⭐ 星标：** 19,984
- **语言：** TypeScript
- **描述：** Enhancement suite for Gemini, AI Studio, Claude & ChatGPT — plus a prompt manager for any web UI, DeepSeek Harness included. / 面向 Gemini、AI Studio、Claude 与 ChatGPT 的增强套件；其中的提示词管理器可用于任意 Web UI，含 DeepSeek Harness。
- **标签：** `browser-extension`, `dsh-plugin`, `chat-management`
- **推荐理由：** 多平台AI浏览器增强套件，统一提示词管理。
...
- **推荐理由：** 让AI Agent无缝自动化操作你已登录的真实浏览器。
...
- **推荐理由：** 最强免费网页搜索插件，无需注册即可为无网络访问能力的模型提供搜索能力
...
- **⭐ 星标：** 338
- **语言：** Python
- **描述：** Give AI agents eyes, ears, and verifiable results. Watch Skill provides video, audio and screen understanding with timestamped evidence; DeepWatch brings that evidence and independent verification into an agent workspace built on the official DeepSeek Harness. MCP, CLI, REST and Web.
- **标签：** `computer-vision`, `ocr`, `multimodal`, `ffmpeg`
- **推荐理由：** 赋予AI视觉听觉能力，支持视频音频屏幕理解及时间戳证据
...
- **推荐理由：** 基于Tabbit/Playwright的强大浏览器自动化
...
- **标签：** `ai-agent`, `chat-bot`, `dingtalk`, `dsh-plugin`
- **推荐理由：** 钉钉IM渠道，将聊天转化为带工具能力的agent对话
...
- **标签：** `dsh-plugin`, `mcp-server`, `resume-builder`
- **推荐理由：** 隐私优先的自托管简历构建工具，支持MCP。
...
- **标签：** `dsh-plugin`, `agent-orchestration`
- **推荐理由：** DSH运行时注入器和任务感知路由器套件
...
- **⭐ 星标：** 1,456
- **语言：** Python
- **描述：** DSH Plugin Radar — 开源可自部署的 DSH 插件生态雷达：自动发现 15900+ 候选、k8s 运行级实测管线；自动索引可用Plugin List
- **标签：** `ecosystem-radar`, `plugin-registry`, `dsh-plugin`
- **推荐理由：** 自部署生态雷达，发现并实测验证15900+插件。
...
- **标签：** `awesome-list`, `dsh-plugin`, `plugins`
- **推荐理由：** DeepSeek Harness生态全面精选插件目录。
...
- **标签：** `oauth`, `subscriptions`, `llm-provider`, `utility`
- **推荐理由：** 通过OAuth将ChatGPT/Claude/Grok订阅转为DSH LLM提供商，无需API密钥
...
- **标签：** `cost-tracking`, `utility`, `budget`
...
- **标签：** `remote`, `p2p`, `infrastructure`
- **推荐理由：** 端到端加密P2P远程访问，支持PC/Android/Web多端操作
...
- **标签：** `utility`, `upgrade`, `maintenance`
- **推荐理由：** 插件随DSH版本自动升级，省心省力
...
- **标签：** `backup`, `config`, `migration`
- **推荐理由：** 完整备份恢复与同步迁移DSH配置
...
- **⭐ 星标：** 59
- **语言：** TypeScript
- **描述：** DSH 插件生态基建：薄控制台（浏览器面板管理官方 repository 插件，0 patch）+ make-dsh-plugin skill 官方插件开���引导
- **标签：** `plugin-management`, `infrastructure`
- **推荐理由：** 浏览器面板管理官方插件库，零补丁安装引导
...
- **标签：** `skills-manager`, `security`, `plugin-management`
- **推荐理由：** DSH中统一加载与管理本地Agent Skills
...
- **标签：** `ui`, `balance`, `info-bar`
- **推荐理由：** 底部状态栏显示模型、余额和实时定价信息
...
- **标签：** `container`, `manager`, `utils`
- **推荐理由：** 容器化运行多版本DSH，内置WebView管理界面
...
- **标签：** `config`, `mcp`, `multi-agent`
- **推荐理由：** 一键配置多种AI Agent工具的MCP和Skills
...
- **标签：** `sync`, `codex`, `bridge`
- **推荐理由：** Codex与DSH双向同步聊天、技能和MCP的桥梁
...
- **标签：** `evaluation`, `rag`, `prompt-engineering`, `dsh-plugin`
- **推荐理由：** 基于证据的提示词、RAG与智能体评估框架，功能全面。
...
- **标签：** `quality-gate`, `engineering`, `delivery`, `dsh-plugin`
- **推荐理由：** 工程纪律套件：验证需求、测试实现、确保交付质量。
...
- **标签：** `neovim`, `lua`, `plugin-manager`, `nvim`
- **推荐理由：** 用Lua编写的轻量级Neovim插件管理器，快速简洁。
...
- **标签：** `ai-cost`, `data-visualization`, `deepseek-api`
- **推荐理由：** 实时追踪Token用量和API费用，可视化图表清晰。
...
- **标签：** `diagnostic`, `conflict-resolution`, `plugin-health`
- **推荐理由：** 检测和修复插件版本冲突问题。
...
- **标签：** `ai-agents`, `chatgpt`, `dsh-plugin`, `oauth`
...
- **标签：** `mascot`, `themes`, `developer-tools`
- **推荐理由：** 多平台AI编码Agent的动画宠物吉祥物画廊
...
- **标签：** `theme`, `glass-ui`, `desktop`, `dsh-plugin`
- **推荐理由：** 高度可定制的磨砂玻璃主题，可调模糊度与背景壁纸
...
- **推荐理由：** 基于WebGPU的海洋主题视觉皮肤，提升Harness界面体验
...
- **标签：** `ai-agent`, `rag`, `prompt`
- **推荐理由：** AI资源大全+教程，适合初学者和进阶者查阅
...
- **标签：** `dsh`, `harness`, `tutorial`
- **推荐理由：** 零基础上手Harness工程开发的最佳入门教程
...
- **推荐理由：** 一站式科研工作台：LaTeX、arXiv、GPU编排
...
- **⭐ 星标：** 205
- **语言：** Python
- **描述：** Academic Paper Search：中文科���用户的 Codex / Claude Code / DeepSeek Harness Skill + MCP；跨 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC 检索去重��支持 MeSH、引用核验、引文图谱、试验检索与审计导出。
- **标签：** `academic`, `research`, `mcp`
- **推荐理由：** 跨库学术检索，支持引文图谱与MeSH
...
- **标签：** `research`, `social-listening`, `marketing`
- **推荐理由：** 88个研究、社交情报和营销自动化技能，跨平台兼容
...
- **标签：** `api-docs`, `chinese-platforms`, `research`
...
- **⭐ 星标：** 48
- **语言：** Python
- **描述：** Alpha Desk — a deepseek-harness (dsh) skill pack that turns an agent session into a compliance-first AI investment desk: multi-strategy fund backtesting via ai-hedge-fund, a tools/pre-execute risk gate, cron monitoring and thesis memory. 把 dsh 会话变成合规、可复现、可追责的 AI 投研工作台。
- **标签：** `fintech`, `backtesting`, `investment-research`
- **推荐理由：** AI投资工作台，支持多策略回测与合规风控
...
- **标签：** `awesome-list`, `plugin-curation`, `directory`
- **推荐理由：** 精选280+社区插件目录，覆盖14大类别
...
- **标签：** `agent`, `discoverability`, `dsh-plugin`, `search`
- **推荐理由：** 在DSH agent内搜索npm和插件列表，提升可发现性
...
- **标签：** `ai-agents`, `coding-agents`, `dsh-plugin`, `mcp`
- **推荐理由：** AI精选190+插件，12个分类，每小时自动更新
...
- **推荐理由：** OpenBitFun combines a high-performance a
...
- **标签：** `awesome-list`, `bilingual`, `curated-list`, `dsh-plugin`
- **推荐理由：** 中英双语DSH插件精选合集，涵盖工具、技能与集成参考
...
- **标签：** `knowledge-base`, `encyclopedia`, `ai-skills`, `awesome-list`
- **推荐理由：** 全维度AI知识库百科，涵盖Agent、RAG、多模态与学习路线
...
- **标签：** `awesome-list`, `plugin-discovery`, `curated`, `guide`
- **推荐理由：** 每日自动抓取精选插件指南，支持场景分类与热度排行
...
- **标签：** `guide`, `awesome-list`, `resource`
...
- **标签：** `learning`, `guide`, `community`
- **推荐理由：** 学习DSH原理与最佳实践，探索插件生态
...
- **标签：** `marketplace`, `directory`, `catalog`
- **推荐理由：** 万款插件目录，支持搜索与公共API
...
- **标签：** `catalog`, `plugin-marketplace`, `awesome-list`
- **推荐理由：** 1958个仓库的中英文插件目录，支持搜索和分类筛选
...
- **标签：** `awesome-list`, `catalog`, `plugins`
- **推荐理由：** 精选DeepSeek Harness插件合集清单
...
- **标签：** `skillhub`, `api`, `documentation`, `dsh-plugin`
- **推荐理由：** 官方技能API文档与示例，作为参考资源很有价值。
...
- **⭐ 星标：** 12
- **语言：** Python
- **描述：** A curated list of plugins for DeepSeek Harness (dsh) — 精选 DeepSeek Harness 插件列表https://billliao.github.io/awesome-dsh-plugin
- **标签：** `awesome-list`, `curated`, `dsh-plugin`
- **推荐理由：** 精选插件列表配合网站展示，DSH用户探索体验良好。
...
- **标签：** `awesome-list`, `directory`, `plugin-discovery`, `dsh-plugin`
- **推荐理由：** 公开GitHub目录，含安装命令，便于插件发现。
...
- **标签：** `registry`, `awesome-list`, `plugin-registry`, `dsh-plugin`
- **推荐理由：** 去垃圾插件注册表，开放数据，社区资源可信。
...
- **标签：** `awesome-list`, `directory`, `installation-guide`
- **推荐理由：** 基于实证的插件目录，附安装指南。
...
- **标签：** `awesome-list`, `chinese`, `plugin-collection`
- **推荐理由：** 中文社区精选插件集合。
...
- **标签：** `awesome-list`, `verified`, `star-ranked`
- **推荐理由：** 按星标排序的社区插件验证目录。
...
- **标签：** `guide`, `navigation`, `plugin-directory`
- **推荐理由：** 按场景推荐插件，附权限和风险说明。
...
- **标签：** `plugin-portal`, `static-site`, `discovery`
- **推荐理由：** 静态Web门户，便捷发现和筛选插件。
...
- **标签：** `awesome-dsh-plugin`, `dsh-plugin`, `dsh-plugin-market`
- **推荐理由：** 类CodeX插件市场，方便DSH插件发现与浏览
...
- **标签：** `dsh-plugin`, `dsh-plugins`, `dsh-ecosystem`
- **推荐理由：** 精选DSH插件合集，支持插件验证功能
...
- **推荐理由：** 已迁移至新仓库，请前往github.com/awesome-dsh-plugin
...
- **推荐理由：** 自动更新插件列表，双语README和Vercel部署
...
- **标签：** `plugin-directory`, `curated-list`, `dsh`, `awesome-list`
- **推荐理由：** DSH生态插件目录，方便发现与安装各类扩展
