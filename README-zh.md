...
- **推荐理由：** DeepSeek原生终端编码Agent，前缀缓存优化。
...
- **标签：** `model-switch`, `multi-agent`, `installation`
- **推荐理由：** 一键安装切换8大编程Agent模型平台
...
- **标签：** `code-review`, `ai-code`, `dsh-plugin`
- **推荐理由：** 基于经典软件工程书籍的智能代码审查工具
...
- **标签：** `cybersecurity`, `code-audit`, `dsh-plugin`
- **推荐理由：** 审计AI生成代码中的幻觉包、缺失防护和漏洞模式
...
- **推荐理由：** 在DSH内集成iOS模拟器和真机控制，提供22个agent工具
...
- **标签：** `coding-agent`, `sub-agents`, `mcp`
- **推荐理由：** 全功能编码Agent，支持子Agent与MCP集成
...
- **标签：** `dsh-plugin`, `developer-tools`, `skill`
- **推荐理由：** 开发 DSH 插件的标准 Agent Skill，支持打包安装
...
- **推荐理由：** 可复用插件创建技能，支持 DSH/Cordis 插件构建与校验
...
- **标签：** `skills-manager`, `dsh-plugin`, `local-skills`
- **推荐理由：** 在 DSH 中统一加载并安全管理本机 Agent 技能
...
- **标签：** `web-ui`, `plugin-marketplace`, `ecosystem`
- **推荐理由：** DSH Web插件聚合生态中心，创意工坊分发。
...
- **标签：** `sidebar`, `ui-extension`, `codex-desktop`
- **推荐理由：** 可扩展侧边栏底座，内置文件/终端/Git页面
...
- **标签：** `bitfun`, `desktop`, `rust`
- **推荐理由：** Rust高性能代理运行时配合精致桌面应用
...
- **推荐理由：** 本地优先的Electron桌面工作区，集成项目管理
...
- **标签：** `desktop-app`, `local-first`, `dsh-plugin`
- **推荐理由：** 本地优先的多模型桌面agent应用，支持自进化技能
...
- **推荐理由：** 开源Windows桌面GUI客户端，支持SSH、移动端远程访问
...
- **标签：** `dsh-plugin`, `ios`, `mobile`
- **推荐理由：** 原生iOS客户端，随时随地使用DSH工作区。
...
- **标签：** `ui`, `web`, `plugin-market`
- **推荐理由：** 在DSH网页界面直接浏览安装社区插件
...
- **标签：** `ui`, `plugin-management`, `console`
- **推荐理由：** 插件注册表控制台与开发引导技能
...
- **推荐理由：** Electron 多标签桌面控制台，一键管理 Harness/插件/MCP/技能
...
- **推荐理由：** 一站式DevOps桌面：SSH/数据库/Docker/AI集成
...
- **推荐理由：** Electron桌面壳，集成DSH Web UI与侧边栏插件
...
- **标签：** `ui`, `theme`, `webui`
- **推荐理由：** 支持上传图片与预设主题的DSH Web背景个性化插件
...
- **标签：** `memory`, `rag`, `local-first`, `portable`
...
- **标签：** `memory`, `token-optimization`, `rag`, `hybrid-retrieval`
- **推荐理由：** 自演进记忆OS，混合检索节省35%令牌成本。
...
- **标签：** `agent-memory`, `skill-rag`, `vector-search`, `embeddings`
- **推荐理由：** 将SKILL.md转化为可检索的agent技能知识库，支持向量搜索
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`, `agent-skills`
- **推荐理由：** 优雅的架构图与流程图即代码生成。
...
- **标签：** `meta-skill`, `agent-skills`, `knowledge-distillation`
- **推荐理由：** 提炼专家思维为可复用AI技能。
...
- **标签：** `workspace`, `task-management`, `multi-platform`
- **推荐理由：** 全平台统一Agent工作空间，支持任务看板管理。
...
- **标签：** `long-horizon`, `workflow`, `control-plane`
- **推荐理由：** 长程Agent控制平面，跨Harness工作流自动化。
...
- **标签：** `cognitive`, `j-space`, `skills`
- **推荐理由：** 基于J-space全局工作区研究的认知增强技能
...
- **标签：** `task-management`, `workflow`, `dsh-plugin`
- **推荐理由：** 原生SQLite任务管理，支持agent认领与审核流程
...
- **标签：** `creative-writing`, `drama`, `dsh-plugin`
- **推荐理由：** 专为小说创作和短剧生产设计的插件，内置Oh Story技能
...
- **标签：** `math-modeling`, `competition`, `workflow`
- **推荐理由：** 自动化数学建模竞赛技能，支持GitOps流水线
...
- **标签：** `self-evolving`, `agent-review`, `modes`
- **推荐理由：** 支持可组合、可审查的Agent自进化模式
...
- **推荐理由：** 为DSH适配Superpowers的TDD与调试技能
...
- **标签：** `ai-director`, `video-editing`, `dsh-plugin`
...
- **标签：** `skillhub`, `api`, `dsh-plugin`
- **推荐理由：** 腾讯官方SkillHub API文档与DSH插件
...
- **推荐理由：** Swift Cycle治理技能适配器，支持离线验证
...
- **推荐理由：** 虚拟产品团队智能体预设，模拟PM/工程师/QA协作流程
...
- **推荐理由：** 华为云官方 MCP 工具集，含云端沙箱支持 AI 代理部署运维
...
- **标签：** `design`, `vector`, `figma`, `agent-teams`
- **推荐理由：** AI原生矢量设计工具，支持Agent团队协作与Figma。
...
- **标签：** `genui`, `ui-components`, `dsh-plugin`
...
- **标签：** `visual`, `workspace`, `dsh-plugin`
- **推荐理由：** 基于Canvas的非线性对话工作区，支持分支会话探索
...
- **标签：** `design`, `ui-design`, `dsh-plugin`
...
- **推荐理由：** 跨平台开源内容发现Agent，支持B站/小红书/抖音等
...
- **推荐理由：** 支持登录态浏览器自动化，Agent可安全操控真实浏览器
...
- **标签：** `web-search`, `browser`, `dsh-plugin`
- **推荐理由：** 免费无需注册的网页搜索插件，填补无原生联网模型的空白
...
- **标签：** `dingtalk`, `im`, `communication`
- **推荐理由：** 钉钉IM渠道，流式Agent支持工具调用
...
- **标签：** `routing`, `infrastructure`, `router`
- **推荐理由：** 任务感知路由注入器，智能推理模式分配。
...
- **标签：** `marketplace`, `installation`, `utility`
- **推荐理由：** DSH可视化插件市场，支持一键安装
...
- **标签：** `cost`, `billing`, `monitoring`
- **推荐理由：** 追踪会话与日成本，支持预算提醒
...
- **推荐理由：** 精选DeepSeek Harness插件列表，资源聚合
...
- **标签：** `awesome-list`, `resources`, `dsh-plugin`
- **推荐理由：** DSH插件工具技能资源精选合集大全
...
- **标签：** `utilities`, `troubleshooting`
- **推荐理由：** 诊断并解决DSH插件间及版本间的兼容冲突
...
- **标签：** `pets`, `mascot`, `cli`, `themes`
- **推荐理由：** 热门AI编程工具的动态宠物吉祥物主题库。
...
- **推荐理由：** 最全AI资源大全，覆盖大模型、提示词与Agent教程。
...
- **标签：** `encyclopedia`, `ai-knowledge`, `dsh-plugin`
- **推荐理由：** 一站式AI百科，覆盖技能、MCP、RAG、多模态等全维度资源
...
- **标签：** `awesome-list`, `plugin-discovery`, `dsh-plugin`
- **推荐理由：** 每日自动抓取并复核插件目录，按场景分类推荐
...
- **推荐理由：** 海量插件市场，11000+条目 searchable
...
- **推荐理由：** 产业分析方法论、产业链图谱与公开政策资讯追踪一体化研究包
...
- **标签：** `portal`, `web`, `awesome-list`
- **推荐理由：** 零依赖静态网站，便于发现和筛选227+个插件
...
- **标签：** `multi-agent`, `mcp`, `ai-agent`
- **推荐理由：** 多模型智能体平台，具备自演进记忆。
...
- **推荐理由：** 14 类 280+ DSH 插件精选目录，快速发现社区资源
...
- **推荐理由：** 八字风水易经中医一体化本地确定性引擎，附可视化看板
...
- **标签：** `awesome-list`, `plugin-directory`, `curated-list`, `registry`
