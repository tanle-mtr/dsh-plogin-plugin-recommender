...
- **标签：** `agent-orchestration`, `mcp`, `skills`
- **推荐理由：** 统一多设备Agent工作空间
...
- **标签：** `agent-collaboration`, `workspace`
- **推荐理由：** 企业级多Agent跨���台工作台
...
- **标签：** `agent-ops`, `workflow-automation`
- **推荐理由：** 长时间 horizon Agent控制平面
...
- **标签：** `preset`, `dsh-plugin`, `agent-skills`
...
- **标签：** `security`, `cybersecurity`, `audit`
- **推荐理由：** AI安全攻防平台，检测幻觉包、缺失防护与漏洞
...
- **推荐理由：** 编码代理含RPC插件、子代理与MCP支持，适配终端工作流
...
- **标签：** `oauth`, `llm-provider`
- **推荐理由：** OAuth登录，将ChatGPT Claude Grok订阅转为DSH Provider。
...
- **推荐理由：** DSH插件管理安装CLI工具，提供schema和解析器。
...
- **推荐理由：** 让纯文本模型具备图像识别能力的OCR插件，突破模态限制。
...
- **推荐理由：** 类似WorkBuddy的技能选择器，提升DSH编排效率。
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 类Claude Code TUI，鲸鱼顶栏/流式思考/双击回滚
...
- **标签：** `deepseek`, `desktop`, `dsh-desktop`, `dsh-plugin`, `tauri`
- **推荐理由：** Tauri轻量桌面版，预置插件，零配置一键安装。
...
- **推荐理由：** 本地优先的AI桌面工作空间，集成会话、文件、网页研究与插件
...
- **推荐理由：** 零配置Windows桌面客户端，内置SSH与移动端远程访问
...
- **标签：** `gui`, `charts`, `interactive`
- **推荐理由：** 在回复中直接生成图表、表单、测验等交互式UI组件
...
- **标签：** `canvas`, `visualization`, `conversation`
...
- **推荐理由：** 本地优先多模型桌面代理，具备自演进技能与隐私优先设计
...
- **标签：** `office`, `ui-extension`
- **推荐理由：** 将电子表格文档幻灯片引入DeepSeek Harness办公环境。
...
- **标签：** `mobile`, `ui-extension`
- **推荐理由：** 原生iOS客户端，通过WebSocket连接DSH随时使用。
...
- **推荐理由：** 开源锤子便签复刻，支持Docker私有部署和DSH插件
...
- **标签：** `desktop`, `ui`, `electron`, `management`
...
- **标签：** `ui`, `information-bar`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 实时显示模型、余额和峰谷定价的信息栏，实用必备。
...
- **推荐理由：** 面向生物信息学科研人员的桌面Agent应用，离线可用。
...
- **推荐理由：** 一站式DevOps桌面客户端，集成数据库、SSH、Docker和AI助手。
...
- **标签：** `ui`, `webui`, `dsh-plugin`
- **推荐理由：** DSH网页上下文面板的一键折叠按钮增强。
...
- **标签：** `agent`, `memory`, `mcp`, `dsh-plugin`
- **推荐理由：** 持久记忆中枢，为所有AI代理提供共享长期记忆。
...
- **标签：** `skill-rag`, `embeddings`, `reranking`
- **推荐理由：** 将分散技能文件转为可检索的知识库，支持嵌入与重排序工具
...
- **标签：** `knowledge-base`, `awesome-list`, `ai-encyclopedia`
- **推荐理由：** 覆盖LLM、Agent、RAG、多模态工具与学习路线的全维度AI百科
...
- **标签：** `multi-agent`, `agentic-workflow`, `mcp-server`
- **推荐理由：** 多智能体群体编排，支持自主工作流。
...
- **标签：** `skills`, `workflow`, `marketplace`
- **推荐理由：** 最大技能商店，自动抓取GitHub技能
...
- **标签：** `ai-video`, `ffmpeg`, `dsh-plugin`, `tts`
...
- **标签：** `vision`, `ocr`, `dsh-plugin`, `computer-vision`
- **推荐理由：** 为纯文本模型打造强大视觉工具包，支持OCR和截图还原。
...
- **标签：** `ai-writing`, `novel`, `rag`, `dsh-plugin`
- **推荐理由：** 从灵感到完稿的全流程小说创作助手，支持RAG管理。
...
- **标签：** `multimodal`, `ocr`, `video-analysis`
- **推荐理由：** 赋予AI视听能力，将视频音频截图转为可检索时间戳证据
...
- **推荐理由：** 一站式科研工作台：LaTeX边写��编、arXiv管理、实验追踪与GPU SSH
...
- **标签：** `creative-writing`, `agent-skills`
- **推荐理由：** 小说、短剧、影视解说一站式创作工作台。
...
- **推荐理由：** SQLite本地任务板，支持Agent认领审核与原生Web UI。
...
- **标签：** `agent-skill`, `coding-agent`, `code-review`
- **推荐理由：** AI编程导师，自动扫描项目、判题与代码审查
...
- **标签：** `document`, `gongwen`, `workflow`, `skill`
- **推荐理由：** 公文全流程处理：格式检查、优化与模板生成
...
- **推荐理由：** 仓库原生Spec驱动开发技能，支持多Agent平台
...
- **标签：** `ai-agent`, `chinese-traditional-wisdom`, `fengshui`, `dsh-plugin`
- **推荐理由：** 融合八字/风水/易经/中医的本地确定性AI技能包，特色鲜明。
...
- **标签：** `ai-roleplay`, `interactive-narrative`, `agent-skills`, `dsh-plugin`
- **推荐理由：** NPC有记忆/立场的深度互动叙事技能，YAML存档系统。
...
- **标签：** `code-simplification`, `skill`, `dsh-plugin`
- **推荐理由：** 源自Codex的代码简化技能，可直接用于DSH工作流。
...
- **标签：** `qa`, `testing`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 11个测试技能的专业QA框架，支持AI自动化测试。
...
- **标签：** `agent-handoff`, `session-memory`, `cross-platform`, `dsh-plugin`
- **推荐理由：** 跨平台会话记忆技能，实现Agent间任务延续交接。
...
- **推荐理由：** DSH首个视觉插件，支持OCR和图片转结构化JSON
...
- **推荐理由：** 集成DirectorX的AI视频/图像/音频创作套件，功能全面。
...
- **推荐理由：** 自进化跨平台内容发现智能体，覆盖B站/YouTube/Reddit等
...
- **推荐理由：** 免费免注册的网页搜索桥接插件，适用于所有无原生搜索的模型
...
- **标签：** `ios`, `automation`
- **推荐理由：** 对话内集成iOS模拟器与真机自动化，含22个Agent工具。
...
- **标签：** `tts`, `xiaomi-mimo`, `communication`
- **推荐理由：** 接入小米MiMo免费TTS，朗读DSH正文内容。
...
- **标签：** `routing`, `agent`, `infrastructure`
- **推荐理由：** 运行时注入器与任务感知推理路由
...
- **推荐理由：** 一键安装切换Claude Code/Codex/Grok/Kimi等多模型
...
- **标签：** `dsh-plugin`, `dsh-external`
- **推荐理由：** 提供上下文仪表盘、浏览器和命令工具的智能管理。
...
- **推荐理由：** 社区精选插件生态列表，涵盖Agent、CLI和Web工具。
...
- **推荐理由：** 追踪会话日费用预算和多Provider Token使用量。
...
- **标签：** `api-docs`, `chinese-api`, `utility`
...
- **标签：** `plugin`, `registry`, `infrastructure`
- **推荐理由：** 插件注册表控制台与官方开发引导工具
...
- **推荐理由：** DSH中统一加载与管理本机Agent Skills的安全工具
...
- **标签：** `directory`, `awesome-list`, `plugin`
- **推荐理由：** 14类280+精选插件目录，快速发现优质DSH插件
...
- **标签：** `plugin-creator`, `scaffold`, `codex`
- **推荐理由：** Codex可复用技能，辅助创建和验证DSH插件
...
- **标签：** `context`, `audit`, `tool`, `ui`, `dsh-plugin`
- **推荐理由：** 审计上下文指令链的实用工具，检测重复与冲突tokens。
...
- **标签：** `container`, `management`, `dsh-plugin`
- **推荐理由：** 基于容器的DSH版本管理工具，支持隔离运行环境。
...
- **标签：** `dsh-plugin`, `troubleshooting`, `utilities`
...
- **推荐理由：** 高自由度磨砂玻璃主题，支持调节模糊度、透明度与动态背景
...
- **标签：** `theme`, `animated`, `threejs`
- **推荐理由：** 动态海洋主题，可调波浪、日落效果与磨砂透明度
...
- **标签：** `awesome-list`, `research`
- **推荐理由：** 每日自动抓取GitHub，筛选分类真实DSH插件目录。
...
- **标签：** `learn`, `marketplace`
- **推荐理由：** 学习DSH原理浏览插件市场并了解最佳实践。
...
- **推荐理由：** 11000+插件商店，含搜索排行安装命令及开放API。
...
- **⭐ 星标：** 186
- **语言：** Python
- **描述：** 基于tushare构建本地财经数据库。AI原生的多Agent金融分析系统，支��skill拓展Agent能力并自适应生成http接口与mcp调用。支持本地数据库访问的skill对接deepseek-harness/openclaw等通用智能体与微信/QQ/飞书等IM通道盯盘
- **标签：** `finance`, `stock`, `mcp`
- **推荐理由：** 基于Tushare的本地财经数据库，支持多Agent分析和IM盯盘
...
- **标签：** `finance`, `kline`, `analysis`, `dsh-plugin`
...
- **标签：** `awesome-list`, `dsh-plugin`, `coding-agent`
- **推荐理由：** DSH插件、工具和技能的精选清单，方便发现资源。
...
- **标签：** `awesome-list`, `dsh-plugins`, `registry`
- **推荐理由：** 去垃圾的DSH插件与技能开放数据注册表。
...
- **推荐理由：** DSH股票/金融/量化插件精选，24个双语验证条目。
...
- **标签：** `awesome-list`, `registry`, `dsh-plugin`
- **推荐理由：** 按star排序的DSH工具全量清单，覆盖千余仓库。
...
- **标签：** `awesome-dsh-plugin`, `static-site`
...
- **标签：** `plugin-registry`, `plugin-radar`
...
- **标签：** `no-code`, `low-code`, `crm`, `erp`
- **推荐理由：** AI增强型低代码业务系统构建平台。
...
- **标签：** `awesome-list`, `catalog`, `discovery`
- **推荐理由：** 精选DSH插件清单，便于发现和参考
...
- **标签：** `plugin-marketplace`, `community`
- **推荐理由：** 社区维护的插件目录，4小时自动同步
...
- **推荐理由：** UltraMath 数学建模竞赛多 Agent 求解 DSH 插件: 5 角色预
...
- **推荐理由：** Double-check before you ship: grill the 
...
- **推荐理由：** dsh-plugin-developer — DeepSeek Harness 
...
- **⭐ 星标：** 22
- **语言：** TypeScript
- **描述：** 面向 DeepSeek Harness 的电���感产品视频 Agent Skill，包含 152 张镜头配方卡、Remotion 模板、代码组件和音频资产。
- **标签：** `agent-skills`, `deepseek-harness`, `motion-design`, `remotion`, `typescript`, `video`
- **推荐理由：** 面向 DeepSeek Harness 的电���感产品视频 Agent Ski
...
- **推荐理由：** 释放 DeepSeek Harness 插件化内核能力，将 Runtime、Pr
...
- **标签：** `awesome-list`, `deepseek-harness`, `automation`
- **推荐理由：** 自动更新的插件列表，YAML驱动+Vercel部署
