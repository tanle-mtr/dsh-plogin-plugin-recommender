...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 190,393 | The core DSH platform — everything is a plugin. |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 90,941 | Open-source Claude Design alternative for UI proto |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,224 | Original multi-agent swarm meta-harness with skill |
...
- **标签：** `agent-runtime`, `coding`, `ide`
- **推荐理由：** Rust高性能Agent运行时，搭配��面编程IDE
...
- **标签：** `coding-agents`, `agent-skills`, `architecture-driven-development`
- **推荐理由：** 架构感知编程代理，基线优先、证据验证、漂移检测
...
- **标签：** `coding-agent`, `deepseek`, `macos`, `dsh-plugin`
- **推荐理由：** DeepSeek团队开发的Swift原生macOS编程代理
...
- **标签：** `agent-teams`, `dsh-plugin`, `coding-agents`
- **推荐理由：** 多代理团队协作插件，增强编程任务协作能力
...
- **推荐理由：** 从Claude Code/Codex调度DSH Agent子任务
...
- **推荐理由：** 将Superpowers技能适配至Reasonix编码工具
...
- **推荐理由：** DSH上下文注入审计工具，检测token成本及指令冲突，带Web UI
...
- **标签：** `plugin-development`, `cordis`
- **推荐理由：** 便携式插件开发与审计技能，含可选DSh Bundle适配器。
...
- **标签：** `documentation`, `best-practices`, `dsh`
- **推荐理由：** DSH插件开发踩坑档案，含实测方案与文档。
...
- **标签：** `engineering`, `best-practices`
- **推荐理由：** 从DSH提炼的工程纪律：事实驱动、持久决策。
...
- **标签：** `plugin-development`, `testing`, `scaffolding`
- **推荐理由：** DSH插件开发的脚手架与测试技能集合。
...
- **推荐理由：** Agent任务预检插件库，自动注入Top-K适用插件。
...
- **标签：** `coding-agents`, `skills`, `ui`, `typescript`
- **推荐理由：** 覆盖模型、工具、沙箱、UI与智能体循环的插件集
...
- **标签：** `dsh-web`, `dsh-plugin`, `cordis`
- **推荐理由：** DSH插件聚合生态，一切皆插件的Web分发平台
...
- **标签：** `mascot`, `react`, `tailwindcss`, `nextjs`
- **推荐理由：** 代码Agent个性化动态宠物画廊，多引擎支持
...
- **⭐ 星标：** 2,422
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录的 TUI 补位插件：Claude Code 风，鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上下文��度+TPS。npm 一键装。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 鲸鱼主题TUI界面，实时状态与流式思考
...
- **标签：** `desktop-companion`, `electron`, `dsh-plugin`, `mcp`
...
- **推荐理由：** 可爱的小鲸鱼浮动组件，监控DeepSeek账户余额
...
- **标签：** `desktop-app`, `electron`, `dsh-plugin`, `mcp`
- **推荐理由：** 本地优先桌面工作台，支持会话、项目和办公自动化
...
- **标签：** `UI`, `components`, `charts`
- **推荐理由：** 在对话中内联渲染图表表单等交互式UI组件
...
- **推荐理由：** 跨平台桌面客户端，CodePilot风格并带插件套件
...
- **推荐理由：** 开源Electron桌面GUI，支持SSH和远程访问。
...
- **标签：** `webui`, `marketplace`, `plugin-market`
...
- **推荐理由：** Electron桌面管理端，多标签页管理插件与MCP
...
- **推荐理由：** DSH编辑器技能选择器，类似WorkBuddy的快捷指令插入功能
...
- **标签：** `skills`, `mcp`, `webgui`, `manager`
- **推荐理由：** Web GUI技能与MCP管理插件，浏览器卡片式管理界面
...
- **标签：** `hud`, `ui`, `status-panel`
- **推荐理由：** 浮动HUD面板，显示git状态、MCP服务、技能及Token用量。
...
- **标签：** `desktop`, `tauri`, `all-in-one`
- **推荐理由：** 跨平台桌面应用，整合数据库、SSH、Docker面板与AI助手。
...
- **推荐理由：** Electron桌面壳集成多插件，增强本地使用体验。
...
- **推荐理由：** 本地优先的Markdown记忆层，自进化跨应用RAG
...
- **标签：** `long-term-memory`, `rag`, `skills`, `mcp`
...
- **标签：** `long-term-memory`, `mcp`, `memory`
- **推荐理由：** 提供持久化记忆与召回工具的长期记忆插件
...
- **标签：** `knowledge-base`, `agent-skill`, `plugin-development`
- **推荐理由：** 官方DSH文档知识库，支持中英双语，按需调用的智能技能
...
- **标签：** `memory`, `context`, `recall`
- **推荐理由：** 连接PowerContext服务端，实现记忆召回与技能交接。
...
- **推荐理由：** 原始多智能体Swarm元框架，支持skills编排。
...
- **标签：** `agents`, `skills`, `workflow-automation`, `prompt-engineering`
- **推荐理由：** 20+工业级模板，提炼Skills提升提示词工程效率
...
- **标签：** `agent-orchestration`, `agent-skills`, `coding-agent`
- **推荐理由：** 全设备统一的Agent编排与看板任务追踪
...
- **标签：** `dsh-plugin`, `cordis`, `agent-skills`
- **推荐理由：** 任务感知推理模式路由套件，提升Agent决策质量
...
- **标签：** `agent-os`, `mcp`, `loop-engineering`, `llm-orchestration`
- **推荐理由：** 自进化Agent OS，通过循环工程自主提升能力
...
- **标签：** `multi-agent-workspace`, `agent-skills`, `visual-editor`
- **推荐理由：** 企业级多引擎工作站，可视化编辑器支持团队协作
...
- **标签：** `agent-skills`, `cognitive`, `workflow`
- **推荐理由：** 基于J-space认知增强技能，前沿AI研究应用
...
- **推荐理由：** 智能编排Agent工作流，路由本地Skills高效
...
- **标签：** `agent-workflows`, `productivity`, `browser-extension`
...
- **标签：** `task-management`, `taskboard`, `webui`
- **推荐理由：** 本地SQLite任务管理，原生Web UI无iframe
...
- **推荐理由：** 将Superpowers技能适配到DSH的测试调试插件
...
- **标签：** `agent-skills`, `spec-driven-development`, `dsh-plugin`
- **推荐理由：** 便携式规格驱动开发技能，支持DSH项目
...
- **推荐理由：** 跨平台代理交接技能，支持Codex/Claude Code的上下文记忆恢复
...
- **推荐理由：** 从DSH移植的代码简化技能，适用于Codex自动化重构
...
- **推荐理由：** 中华传统智慧AI技能包，集成八字风水六爻等本地确定性引擎
...
- **推荐理由：** 多Agent团队协作插件，独立模型/技能/上下文共享工作空间
...
- **标签：** `media`, `image`, `vision`, `generation`
- **推荐理由：** 免费图片识别生成技能，支持粘贴图片自动视觉转录
...
- **标签：** `agent-skills`, `mcp`, `database`
- **推荐理由：** 42项技能+5个CLI的AI代理资产包管理器，支持数据库和MCP。
...
- **标签：** `agent-skills`, `project-governance`
- **推荐理由：** Swift Cycle治理技能适配器，用户调用且可离线验证。
...
- **推荐理由：** 中文公文全流程处理工具，符合GB/T 9704国家标准。
...
- **标签：** `agent-skills`, `canvas`, `creative`, `mcp`
- **推荐理由：** 原生WeShop无限画布插件，含丰富创意技能。
...
- **推荐理由：** Godot 4.x全栈开发技能插件，覆盖GDScript与引擎工具。
...
- **推荐理由：** 游戏化任务系统，迷雾地图+任务栏推进，技能管理有趣。
...
- **推荐理由：** 虚拟产品团队Agent预设，模拟PM/开发/QA协作流程。
...
- **推荐理由：** 开源Claude Design替代方案，UI原型神器。
...
- **标签：** `architecture-diagram`, `mermaid-alternative`, `agent-skills`
- **推荐理由：** 生成精美架构图、工作流图的Agent技能。
...
- **标签：** `mcp`, `ui`, `vibedesign`, `flutter`, `react`
- **推荐理由：** 首个开源AI原生矢量设计工具，并发Agent团队协作
...
- **标签：** `video`, `design`, `automation`
- **推荐理由：** 自动化生成Vox风格拼贴解说视频，全流程
...
- **标签：** `vision-tools`, `computer-vision`, `ocr`, `dsh-plugin`
- **推荐理由：** 纯文本模型视觉工具箱：图像问答、OCR、截图识别
...
- **推荐理由：** 在DSH中预览、检查和编辑OpenPencil文档。
...
- **标签：** `design`, `vibe-coding`, `ui`, `styles`
- **推荐理由：** 6种设计风格预设，让AI生成网页告别千篇一律的AI风格
...
- **推荐理由：** DirectorX AI视频/图像/音频技能，支持可配置视觉模型。
...
- **标签：** `visualization`, `cost`, `token`
- **推荐理由：** 实时Token用量图表、成本估算与API余额可视化展示。
...
- **标签：** `social`, `photo`, `oauth`, `community`
- **推荐理由：** DSH内测收官合影墙，GitHub OAuth零权限登录的拍立得站点
...
- **标签：** `chatgpt`, `oauth`
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内与GPT模型对话。
...
- **标签：** `mcp-server`, `self-hosted`, `dsh-plugin`
- **推荐理由：** 隐私优先的开源简历生成器，支持自托管。
...
- **标签：** `dsh-plugin`, `deepseek`
- **推荐理由：** 两阶段标准预设，覆盖Project2工具链98/99分
...
- **标签：** `vfs`, `agent-tools`, `bash`, `python`, `typescript`
- **推荐理由：** AI Agent统一虚拟文件系统，抽象存储层
...
- **标签：** `deepseek-harness`, `model-switch`, `utility`
- **推荐理由：** 一键安装切换多款AI编程工具，省时省力
...
- **标签：** `subscription`, `provider`, `oauth`
- **推荐理由：** 将现有订阅作为DSH的LLM提供商使���
...
- **标签：** `compatibility`, `migration`, `abi`
- **推荐理由：** 兼容层，使Pi扩展可作为DSH原生插件运行。
...
- **标签：** `marketplace`, `plugin-management`
- **推荐理由：** 一键浏览安装更新所有DSH插件的实用工具
...
- **标签：** `crash-recovery`, `backup`, `snapshot`
...
- **标签：** `dsh-plugin`, `plugin-management`, `ui`
- **推荐理由：** 官方插件管理控制台，附带开发引导
...
- **推荐理由：** 收录280+社区插件的精选目录，覆盖14大类，方便查找和发现
...
- **推荐理由：** OpenAI Codex与DSH双向同步桥接，支持聊天/技能/MCP自动镜像
...
- **推荐理由：** DSH配置备份恢复迁移工具，支持WebDAV同步完整配置
...
- **推荐理由：** DSH技能管理插件，直接在harness中管理和组织所有技能
...
- **标签：** `utility`, `balance-tracking`, `web`
- **推荐理由：** 状态栏显示供应商、模型余额和实时价格倒计时。
...
- **推荐理由：** Lua编写的快速优雅Neovim插件管理器，简洁可靠。
...
- **推荐理由：** 内置GUI插件管理器，热生效无需重启，管理MCP和技能。
...
- **标签：** `awesome-dsh-plugin`, `utilities`
- **推荐理由：** 诊断并解决插件间及版本间冲突问题。
...
- **标签：** `cli`, `plugin-manager`, `utilities`
- **推荐理由：** CLI管理DSH插件：搜索安装更新审计
...
- **推荐理由：** 程序员鱼皮AI资源大全与Vibe Coding教程。
...
- **标签：** `research`, `analysis`, `china`
- **推荐理由：** 搜索8大中文平台近30天内容生成研究报告
...
- **推荐理由：** 精选插件榜单，自动发现9000+候选，Top50人工策展
...
- **标签：** `encyclopedia`, `RAG`, `resource`
- **推荐理由：** 覆盖模型、代理、RAG和MLOps的全面AI百科
...
- **标签：** `registry`, `open-data`, `filtered`
- **推荐理由：** 垃圾过滤的开放数据插件注册表，内容可信。
...
- **标签：** `awesome-list`, `dsh-plugin`, `analytics`
- **推荐理由：** DSH插件目录，含验证与分析工具
...
- **标签：** `ai-agents`, `awesome-list`, `bilingual`
...
- **推荐理由：** GitHub Actions每日自动爬取的插件目录
...
- **标签：** `industry`, `awesome-list`, `categorized`
- **推荐理由：** 金融法律HR制造医疗等行业精选插件
...
- **标签：** `cordis`, `awesome-list`, `official`
...
- **标签：** `harness-engineering`, `tutorial`, `agent`
- **推荐理由：** 从零到一学习Harness工程的入门教程
...
- **标签：** `toy`, `simulation`, `novelty`
- **推荐理由：** 传统玩具竹知了网页模拟版，零依赖有趣
...
- **⭐ 星标：** 270
- **语言：** JavaScript
- **描述：** 用 30 秒为你的 DeepSeek Harness找到合适的插件。 仓库由脚本每天自动抓取，再经��工逐个核实——真插件进目录，蹭热度的进黑名单，每条剔除理由公开可查。并告诉你每个插件适合谁、从哪里开始。
- **标签：** `awesome-list`, `plugin-finder`, `verification`
- **推荐理由：** 每日自动验证的插件目录，含黑名单和推荐
...
- **标签：** `awesome-list`, `quickstart`, `guide`
- **推荐理由：** DSH终极入门指南，含精选插件和工具资源
...
- **推荐理由：** 3100+插件的DSH插件市场中心，提供公共API。
...
- **标签：** `marketplace`, `plugins`, `plugin-collection`
- **推荐理由：** DeepSeek Harness插件市场与精选合集。
...
- **标签：** `notes`, `productivity`, `docker`
- **推荐理由：** 开源智能笔记，支持Docker部署和DSH集成。
...
- **推荐理由：** DeepSeek Harness精选插件列表，便于生态探索。
...
- **标签：** `awesome-list`, `typescript`, `nextjs`
- **推荐理由：** 公开GitHub插件目录，含安装命令与生态发现。
