...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 217,123 | The foundational plugin harness framework enabling |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 95,069 | Open-source Claude Design alternative for AI-power |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 71,749 | Deploy multi-agent swarms and coordinate autonomou |
...
- [Agent 技能与工作流](#agent-技能与工作流) — 49 个插件
...
- [Agent 技能与工作流](agent-skills-workflows-zh) (49)
...
- **标签：** `multi-agent`, `agent-memory`, `skills`
- **推荐理由：** 支持记忆与自进化的多智能体插件调度框架
...
- **推荐理由：** DeepSeek原生终端编码Agent，支持提示词缓存与工具调用
...
- **标签：** `agent-os`, `evaluation`, `evolution`
- **推荐理由：** 带评估门的自演进代理操作系统
...
- **推荐理由：** 提供最小化引导与完整工具集预设，适合DSH工作流起步。
...
- **推荐理由：** 基于12本经典工程著作的AI代码审查，带引用和严重性标注。
...
- **推荐理由：** 为纯文本模型打造的强大视觉工具箱，支持图片问答和长截图OCR。
...
- **标签：** `security`, `audit`, `redteam`, `vulnerability`
- **推荐理由：** 审计AI生成代码的安全缺陷和幻觉包
...
- **标签：** `superpowers`, `coding`, `skills`
- **推荐理由：** Superpowers技能移植至Reasonix编码框架
...
- **标签：** `codex`, `sync`, `mcp`, `bridge`, `coding-agent`
- **推荐理由：** 双向聊天同步桥接OpenAI Codex与DSH，支持MCP镜像
...
- **标签：** `web-ui`, `aggregation`, `ecosystem`
- **推荐理由：** DSH插件聚合的Web生态系统
...
- **标签：** `terminal`, `sandbox`, `virtual`
- **推荐理由：** 首款AI Agent虚拟终端，支持沙盒化终端体验。
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git等多功能页面。
...
- **标签：** `tui`, `theme`, `terminal`
- **推荐理由：** 鲸鱼主题TUI，支持流式思考、回滚与TPS显示。
...
- **标签：** `widget`, `balance`, `dsh-plugin`
- **推荐理由：** 萌系浮动小鲸鱼，实时查看���额，支持拖拽吸附动画。
...
- **推荐理由：** 仅5MB的轻量桌面版，预置插件，支持Windows/macOS/Linux。
...
- **标签：** `desktop`, `workspace`, `electron`
- **推荐理由：** 本地优先的桌面工作区，支持会话、项目、网页研究和办公文档。
...
- **标签：** `desktop`, `local-first`, `privacy`, `multi-model`
- **推荐理由：** 本地优先多模型桌面应用，隐私保护强
...
- **标签：** `canvas`, `branching`, `workspace`, `visual`
- **推荐理由：** 基于画布的可视化分支对话工作空间
...
- **标签：** `office`, `ui`, `spreadsheet`, `docs`
- **推荐理由：** 在DSH中集成表格、文档、幻灯片，办公能力大幅增强。
...
- **标签：** `desktop-app`, `electron`, `typescript`, `multi-platform`
- **推荐理由：** 跨平台桌面客户端，CodePilot风格，Mac/Win/Linux全覆盖。
...
- **标签：** `ios`, `swift`, `mobile`, `websocket`, `native-app`
- **推荐理由：** 原生iOS客户端，WebSocket连接DSH，随时查看会话与Agent轨迹。
...
- **标签：** `status-bar`, `usage-tracking`, `pricing`
- **推荐理由：** 底部状态栏：实时余额、价格、峰谷倒计时一目了然
...
- **⭐ 星标：** 12
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运维每天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体��操作。
- **标签：** `desktop`, `devops`, `ssh`, `docker`
- **推荐理由：** 一体化DevOps桌面：数据库、SSH、Docker合一
...
- **推荐理由：** Electron桌面版DSH，集成侧边栏与WebUI。
...
- **标签：** `ui`, `web-extension`, `compact`
- **推荐理由：** 一键收起DSH Web上下文面板，简洁实用。
...
- **标签：** `ui`, `knowledge-cards`, `share`
- **推荐理由：** 将对话历史转为精美知识卡片，可下载分享。
...
- **推荐理由：** 自进化上下文数据库，统一智能体记忆与RAG知识存储
...
- **推荐理由：** 开源LLM知识库平台，将文档转化为可查询RAG和Wiki
...
- **标签：** `memory`, `local-first`, `rag`
- **推荐理由：** 可移植的本地优先AI代理记忆层
...
- **标签：** `memory`, `hybrid-retrieval`, `token-savings`
- **推荐理由：** 自演进记忆系统，节省35% token
...
- **推荐理由：** 基于Milvus的持久统一记忆层，支持向量语义搜索。
...
- **标签：** `skill-retrieval`, `embeddings`, `rag`
- **推荐理由：** 将分散的技能文件转化为可检索的agent技能语料库。
...
- **标签：** `financial`, `multi-agent`, `mcp`
- **推荐理由：** 本地财经数据库，支持多Agent分析与MCP调用
...
- **标签：** `skills`, `marketplace`, `collection`
- **推荐理由：** 全面的技能商店，自动更新
...
- **推荐理由：** 基于Anthropic J-space研究的AI认知增强技能套件。
...
- **推荐理由：** 自动搜索国内8大平台近30天内容，生成有据可查的报告。
...
- **标签：** `agent-teams`, `collaboration`, `dsh-plugin`
...
- **标签：** `multimodal`, `ocr`, `video-analysis`, `speech-to-text`
- **推荐理由：** 多模态技能，将音视频屏转为可验证证据
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核流
...
- **标签：** `math-modeling`, `agent-skill`, `gitops`, `competition`
- **推荐理由：** 全栈数学建模竞赛Skill，GitOps流水线+强制代码自证，国赛美赛通用。
...
- **⭐ 星标：** 206
- **语言：** TypeScript
- **描述：** 让 Agent 的工作方式可组合、可审查、可持续改��，最终实现 Agent Self Evoling。 DeepSeek Harness Web plugin with composable task controls and isolated, human-reviewed self-evolution.
- **标签：** `acceptance-review`, `plan-mode`, `agent-review`
- **推荐理由：** 支持Agent可组合、可审查的自演化工作流
...
- **标签：** `agent-skills`, `research`, `marketplace`
- **推荐理由：** 88个开源Agent技能，覆盖研究与业务流程
...
- **标签：** `skill-generator`, `automation`
- **推荐理由：** 将成功会话转为可复用Agent Skill
...
- **推荐理由：** 来自superpowers的TDD/调试/规划技能
...
- **⭐ 星标：** 72
- **语言：** TypeScript
- **描述：** ProMentor 是一�� AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `coding`, `mentor`, `skills`
- **推荐理由：** AI编程导师技能，扫描项目并Code Review
...
- **标签：** `patent`, `rag`, `multi-agent`
- **推荐理由：** 基于RAG的专利侵权分析，多Agent可调用
...
- **⭐ 星标：** 46
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制��景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `docx`, `template`
...
- **标签：** `plugin`, `skill`, `development`
- **推荐理由：** 构建和验证DSH插件的Agent技能工具
...
- **推荐理由：** 统一加载和管理本机Agent Skills的DSH插件
...
- **标签：** `spec`, `coding`, `agent-skills`
- **推荐理由：** 仓库原生规范驱动开发的便携式Agent技能
...
- **推荐理由：** 多Agent协作，独立模型+共享工作区，团队开发利器
...
- **标签：** `chinese-traditional-wisdom`, `bazi`, `fengshui`, `skill`
- **推荐理由：** 独家中华传统智慧技能：八字/风水/易经全融合
...
- **推荐理由：** WorkBuddy风格技能选择器，快速插入技能到对话
...
- **标签：** `ai-video`, `ai-image`, `director`, `creative`
- **推荐理由：** AI视频/图片/音频技能插件，支持脚本编排和视觉生成
...
- **推荐理由：** QA测试工程技能框架，11技能+共享知识库，测试自动化
...
- **标签：** `agent-handoff`, `cross-platform`, `memory`
- **推荐理由：** 跨平台Agent交接技能，本地持续记忆，无缝切换
...
- **标签：** `agent-skills`, `ai-roleplay`
- **推荐理由：** NPC角色扮演技能，支持记忆和YAML存档系统
...
- **⭐ 星标：** 21
- **语言：** TypeScript
- **描述：** dsh-plugin-developer — DeepSeek Harness 插件开发 Skill  > 指导 AI Agent 从 0 到 1 开发、构建、��装、测试 DeepSeek Harness（dsh）插件。基于 **dsh 0.1.1-rc.2** 与 dsh-openmaic 项目的完整实战经验，并内置一个**已通过 web 界面实测**的天气插件作为整包参考案例。  ## 这是什么  dsh 采用"无特权内核、万物皆插件"的设计：模型适配器、工具注册表、Agent 循环、网页界面都是插件
- **标签：** `plugin-development`, `skill`
- **推荐理由：** 从零到一开发构建DSH插件的分步指南
...
- **推荐理由：** 152张镜头配方卡+Remotion模板制作电影感视频
...
- **标签：** `plugin-development`, `agent-skill`
- **推荐理由：** 涵盖插件/服务/适配器开发标准的多功能Agent Skill
...
- **标签：** `marketplace`, `skills`, `dsh-bundle`
- **推荐理由：** 类CodeX插件市场，便于发现安装技能。
...
- **⭐ 星标：** 22
- **语言：** TypeScript
- **描述：** 面向 DeepSeek Harness Web GUI 的正式插件包：在设置页的「Web UI ���件」分组中新增一张「技能与 MCP」卡片，用于在浏览器里管理技能（skills）与 MCP 服务器。
- **标签：** `mcp`, `web-ui`, `manager`
- **推荐理由：** 在Web GUI中直接管理技能与MCP服务器
...
- **推荐理由：** Agent技能，可将代码自动生成可验证的架构图和流程图
...
- **推荐理由：** DSH首个视觉插件，让纯文本Agent具备图像识别能力���
...
- **推荐理由：** AI生成可编辑设计系统，内置模板市场和PPT工作室。
...
- **标签：** `gui`, `charts`, `interactive`, `inline`
- **推荐理由：** 生成交互内联UI组件：图表、表单、测验
...
- **推荐理由：** DeepSeek API实时Token用量图表与成本追踪。
...
- **标签：** `browser`, `content-discovery`, `automation`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、小红书、YouTube等。
...
- **推荐理由：** 让AI代理无缝操控真实登录浏览器，跨任意shell自动化。
...
- **标签：** `ios`, `mobile-automation`, `webdriveragent`, `mcp`
- **推荐理由：** 22个iOS自动化工具，支持模拟器与真机，移动端测试利器。
...
- **标签：** `dingtalk`, `im`, `channel`
- **推荐理由：** 钉钉IM渠道，每个对话变工具代理。
...
- **标签：** `router`, `injector`, `infrastructure`
- **推荐理由：** 任务感知的推理模式路由套件
...
- **标签：** `model-switch`, `install`, `utility`
- **推荐理由：** 一键安装与多模型切换，支持Claude/Codex/DSH等。
...
- **推荐理由：** 可自部署的插件雷达，自动发现15900+候选并做K8s级实测。
...
- **标签：** `cli`, `plugin-manager`, `typescript`, `developer-tools`
- **推荐理由：** DSH插件管理器CLI，支持插件安装与解析，开发必备。
...
- **标签：** `cost-tracking`, `token-usage`, `budget`, `deepseek-api`
- **推荐理由：** 实时追踪会话/日花费、预算和Token用量，支持多渠道余额监控。
...
- **标签：** `remote-access`, `dsh-plugin`, `multi-device`
...
- **标签：** `api`, `documentation`, `utility`
- **推荐理由：** 零配置搜索65K+中国开放平台API文档
...
- **标签：** `context-audit`, `quality`, `utility`
- **推荐理由：** 上下文审计工具，检测重复/冲突指令，提升质量
...
- **标签：** `version-manager`, `container`, `desktop`
- **推荐理由：** 容器化管理多版本DSH，内置WebView，轻量高效
...
- **推荐理由：** 一键同步MCP/Skills配置，支持10+ AI工具跨平台管理
...
- **标签：** `awesome-list`, `directory`, `plugin-directory`
- **推荐理由：** 公共GitHub目录，提供插件发现与安装命令
...
- **标签：** `troubleshooting`, `utilities`
- **推荐理由：** 诊断并解决插件间冲突与版本兼容问题。
...
- **标签：** `theme`, `glass`, `customizable`, `ui`
- **推荐理由：** 高度可定制磨砂玻璃主题，零源码修改
...
- **标签：** `chrome-extension`, `theme`, `threejs`, `webgpu`
- **推荐理由：** WebGPU海洋主题，Three.js渲染，视觉体验独特。
...
- **推荐理由：** 精选DSH插件合集，覆盖代理、编码和Web UI场景。
...
- **标签：** `encyclopedia`, `research`, `rag`, `mcp`
- **推荐理由：** 全维度AI资源百科，覆盖Agent/RAG/多模态
...
- **标签：** `academic`, `latex`, `arxiv`, `research`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、实验追踪、GPU SSH编排。
...
- **⭐ 星标：** 217
- **语言：** Python
- **描述：** Academic Paper Search：中��科研用户的 Codex / Claude Code / DeepSeek Harness Skill + MCP；跨 CrossRef、PubMed、arXiv、OpenAlex、Europe PMC 检索去重，支持 MeSH、引用核验、引文图谱、试验检索与审计导出。
- **标签：** `academic`, `arxiv`, `mcp`, `citation`, `research`
- **推荐理由：** 跨源学术检索，支持PubMed/arXiv等，含引文图谱与引用核验功能。
...
- **标签：** `research`, `industry`, `skills`
- **推荐理由：** 行业研究技能包，含方法论与产业链映射
...
- **标签：** `plugin-development`, `knowledge-base`, `official-docs`
- **推荐理由：** 官方插件开发知识库，中英双语，入门必备
...
- **标签：** `kline`, `finance`, `trading`, `analysis`
- **推荐理由：** 交互式K线分析插件，多市场行情+技术指标+基本面
...
- **标签：** `awesome-list`, `plugin-directory`
- **推荐理由：** 实用的DeepSeek Harness插件集合，便于发现。
...
- **标签：** `awesome-list`, `automation`, `catalog`
- **推荐理由：** 每日自动更新的插件目录，双语支持。
...
- **标签：** `plugin-radar`, `2origin`, `discovery`
...
- **标签：** `recommender`, `ai-curated`, `hourly-update`
- **推荐理由：** AI策展的190+插件列表，每小时更新，12个分类。
...
- **标签：** `verified`, `plugin-directory`, `catalog`
- **推荐理由：** 80+经认证的插件按功能分类整理。
...
- **标签：** `plugin-marketplace`, `curated`
- **推荐理由：** 每日更新的DSH插件商城，资源丰富
...
- **标签：** `awesome-list`, `guide`, `plugin-directory`, `quickstart`
- **推荐理由：** DSH终极指南，涵盖入门、精选插件与实用工具推荐。
...
- **标签：** `awesome-list`, `mcp`, `coding-agent`, `plugins`, `patch`
- **推荐理由：** 精选DSH插件、Skill、MCP服务器与UI扩展的 curated 合集。
...
- **标签：** `marketplace`, `plugin-store`, `awesome-list`, `directory`
- **推荐理由：** 11,000+插件市场，支持搜索、排行、安装命令及免费公共API。
...
- **标签：** `handbook`, `awesome-list`, `developer-tools`
- **推荐理由：** 全面的Agent开发手册，涵盖运行时与插件
...
- **推荐理由：** 精选DeepSeek Harness插件Awesome列表
...
- **标签：** `bioinformatics`, `desktop`, `medical`, `niche`
- **推荐理由：** 生信桌面Agent，面向临床医生和湿实验科学家
...
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH中聊天订阅模型。
...
- **语言：** Unknown
- **描述：** DSH插件
- **标签：** `awesome-list`, `curation`
- **推荐理由：** DSH插件精选合集，便于发现有用插件
...
- **标签：** `awesome-list`, `curation`, `evidence-aware`
- **推荐理由：** 基于证据的DSH插件精选清单，质量可控
