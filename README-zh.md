...
- **推荐理由：** DeepSeek原生终端编程Agent，支持提示词缓存
...
- **标签：** `workspace`, `orchestration`, `multi-device`, `mcp`
- **推荐理由：** 多端统一的Agent工作区，支持任务看板管理
...
- **标签：** `routing`, `agent-control`, `injector`, `mcp`
- **推荐理由：** 任务感知路由套件，支持推理模式动态切换
...
- **标签：** `control-plane`, `long-horizon`, `workflow`, `ops`
- **推荐理由：** 长周期Agent控制面，支持跨框架持久工作流
...
- **标签：** `enterprise`, `workspace`, `multi-agent`, `editor`
- **推荐理由：** 企业级多Agent工作台，支持可视化编辑和多引擎
...
- **推荐理由：** 基于12本经典工程书籍的AI代码审查，含严重程度标签。
...
- **推荐理由：** 为纯文本模型打造的视觉工具箱，支持OCR和GUI自动化。
...
- **标签：** `ios-simulator`, `mobile-automation`, `mcp`
- **推荐理由：** 对话内直接控制iOS模拟器与真机，22个工具
...
- **推荐理由：** 将Superpowers技能移植到Reasonix编程框架
...
- **推荐理由：** 按标准开发DSH插件的Agent Skill，规范完整
...
- **标签：** `mascot`, `ui`, `animated`, `community`
- **推荐理由：** 多编码Agent的动画宠物公开画廊，增添乐趣
...
- **推荐理由：** 轻量级Tauri桌面应用，零环境配置，预置插件开箱即用。
...
- **推荐理由：** 本地优先的桌面工作空间，集成DSH会话、文件和办公工具。
...
- **标签：** `ui`, `interactive`, `dsh-ui`, `charts`
...
- **标签：** `desktop-app`, `multi-model`, `privacy`, `mcp`
- **推荐理由：** 本地优先多模型桌面Agent，隐私保护支持自进化技能
...
- **标签：** `office`, `spreadsheets`, `documents`
- **推荐理由：** 电子表格文档幻灯片集成到DSH办公环境
...
- **标签：** `desktop-app`, `electron`, `codepilot`
- **推荐理由：** 跨平台桌面客户端，CodePilot风格体验
...
- **⭐ 星标：** 28
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/技能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工���。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `ui`, `tool`
- **推荐理由：** 检测上下文Token成本，Web UI面板可视化
...
- **推荐理由：** 集成数据库、SSH、Docker和AI的一体化DevOps桌面
...
- **标签：** `ui`, `customization`, `webui`
- **推荐理由：** 支持自定义背景图与预设主题的UI美化插件
...
- **标签：** `memory`, `rag`, `stateful`, `continual-learning`
...
- **标签：** `memory`, `rag`, `management`
- **推荐理由：** 专为代理设计的记忆管理套件，支持记忆精炼
...
- **标签：** `agent-memory`, `semantic-search`, `vector-search`, `embeddings`
- **推荐理由：** 将零散技能文件转化为带检索评估的知识库
...
- **标签：** `ai-agent`, `mcp`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 自进化的多模型智能助手，具备记忆与知识能力
...
- **标签：** `skills`, `store`, `automation`, `workflow`
- **推荐理由：** 最全技能商店，自动抓取上万Skill项目分类整理
...
- **推荐理由：** SQLite原生任务看板，支持Agent认领与协作审查
...
- **标签：** `creative-writing`, `novel`, `screenwriting`, `skills`
- **推荐理由：** 专为小说创作和短剧剧本设计的工作流技能插件
...
- **⭐ 星标：** 69
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。���上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `skill`, `mentor`, `coding`
- **推荐理由：** AI编程导师技能：生成教程、代码审查、自动判题
...
- **标签：** `agent-skills`, `legal`, `visualization`, `timeline`
- **推荐理由：** 诉讼时间轴和图表可视化，法律人必备工具
...
- **推荐理由：** 将Matt Pocock技能集引入DSH，含25个工程效率技能
...
- **标签：** `agent-skills`, `gongwen`, `docx`, `wps-office`
- **推荐理由：** 中文公文全流程处理，格式检查与优化
...
- **标签：** `agent-skill`, `chinese-traditional-wisdom`, `fengshui`
- **推荐理由：** 融合八字风水中医等传统智慧，带可视化面板
...
- **标签：** `dsh-plugin`, `multi-agent`, `collaboration`
- **推荐理由：** 多智能体团队协作，独立模型共享工作区
...
- **标签：** `deepseek`, `dsh`, `skills`
- **推荐理由：** 统一管理系统本地技能，安全便捷
...
- **标签：** `agent-handoff`, `session-memory`, `dsh`
- **推荐理由：** 跨会话保持Agent上下文记忆连续性
...
- **标签：** `agent-evaluation`, `rag-evaluation`, `skill-evaluation`
- **推荐理由：** 支持提示词和RAG评估的开源框架
...
- **标签：** `fintech`, `investment-research`, `backtesting`, `mcp`
- **推荐理由：** 将DSH转化为合规驱动的AI投资分析台
...
- **推荐理由：** 虚拟产品团队Agent预设，覆盖PM/QA/研发角色
...
- **标签：** `api`, `chinese-platforms`, `documentation`
...
- **标签：** `agent-skills`, `diagram-as-code`, `dsh-plugin`
- **推荐理由：** 从文本生成精美的架构和工作流 diagrams
...
- **标签：** `design`, `ui`, `vector`, `vibedesign`
...
- **推荐理由：** 端到端自动生成Vox风格拼贴解说视频，全流程自动化。
...
- **推荐理由：** AI驱动的设计系统，含可视化编辑、模板市场和PPT生成。
...
- **推荐理由：** 跨平台内容发现代理，覆盖B站、YouTube、Reddit等
...
- **推荐理由：** 无缝浏览器自动化，支持真实登录状态，不影响日常使用。
...
- **标签：** `low-code`, `crm`, `erp`, `dsh-plugin`
...
- **推荐理由：** 自动化插件生态雷达，发现并验证超15900个候选插件。
...
- **标签：** `awesome-list`, `ecosystem`, `catalog`
- **推荐理由：** 精心策划的DSH插件生态完整列表，方便查找。
...
- **标签：** `cybersecurity`, `redteam`, `vulnerabilities`, `agent`
...
- **标签：** `oauth`, `subscription`, `llm-provider`, `utility`
- **推荐理由：** 通过OAuth订阅ChatGPT/Claude/Grok作为DSH可用模型源
...
- **标签：** `learning`, `plugin-marketplace`, `guide`
...
- **标签：** `plugin-management`, `registry`, `console`
- **推荐理由：** 插件注册表控制台，浏览器面板管理+开发引导
...
- **标签：** `dsh-plugin`, `container`, `utility`
- **推荐理由：** 多版本DSH容器隔离运行，内嵌WebView界面
...
- **标签：** `agent-config`, `mcp`, `dsh`
- **推荐理由：** 一键配置多Agent的MCP和技能
...
- **标签：** `agent-framework`, `mcp`, `dsh`
- **推荐理由：** 42项技能包，Agent工程资产管家
...
- **推荐理由：** 全面AI资源教程，覆盖主流大模型和Prompt技巧
...
- **标签：** `awesome-list`, `plugins`, `discovery`
- **推荐理由：** 精选DSH插件列表，帮助你发现优质扩展
...
- **标签：** `research`, `web-search`, `report`
- **推荐理由：** 自动搜索国内8大平台内容，生成有据可查的研究报告。
...
- **标签：** `knowledge-base`, `ai-encyclopedia`, `mcp`, `rag`
- **推荐理由：** 一站式AI百科，涵盖Agent/RAG/MCP及学习路线
...
- **⭐ 星标：** 311
- **语言：** JavaScript
- **描述：** 30 秒找到真正适合你的 DeepSeek Harness插件。每天自动抓取 GitHub 上的 `dsh-plugin` 项目并逐个复核：真实插件分类收录，蹭标签项目剔除。通过场景化分类、精选推荐、热度排行和图文导览，帮你快速看懂每个插件能做什么、适合谁，以及如何开始使用。欢迎 Star ，让好用的插件更���被发现。
- **标签：** `awesome-list`, `discovery`, `plugin-market`, `curated`
- **推荐理由：** 每日自动爬取并筛选真实插件，告别虚假标签
...
- **标签：** `academic-search`, `mcp`, `skills`
- **推荐理由：** 多源学术检索与引文管理，提升科研效率。
...
- **标签：** `research`, `industry`, `reporting`
- **推荐理由：** 行业研究领域包，含产业链映射与政策追踪
...
- **标签：** `kline`, `analysis`, `dsh-plugin`
- **推荐理由：** 交���式K线分析，支持多市场行情
...
- **标签：** `web`, `plugin-ecosystem`, `aggregation`
- **推荐理由：** DSH插件Web聚合生态，一切皆插件
...
- **标签：** `enhancement`, `specialized`
- **推荐理由：** 专为增强DeepSeek模型能力设计的插件。
...
- **标签：** `skillhub`, `documentation`, `api`
- **推荐理由：** 腾讯官方SkillHub文档与插件示例
...
- **标签：** `finance`, `stock`, `awesome-list`
- **推荐理由：** 双语精选股票/金融/量化插件合集
...
- **⭐ 星标：** 3
- **语言：** Unknown
- **描述：** 面向 DeepSeek Harness 用户的插件���品说明书与导航：按场景选插件，了解安装、权限、兼容性和风险
- **标签：** `awesome-list`, `documentation`
- **推荐理由：** 插件使用手册，含安装、权限与兼容性指南
...
- **推荐理由：** 通过OAuth在DSH中使用ChatGPT订阅聊天。
...
- **推荐理由：** 插件雷达与2Origin实验室，证据支持 curated。
...
- **标签：** `awesome-list`, `archive`
- **推荐理由：** 原awesome-dsh-plugins，已迁移至新仓库。
