...
- **推荐理由：** 自进化多智能体平台，支持记忆、任务规划与多模型协同。
...
- **推荐理由：** 终端原生DeepSeek编码智能体，前缀缓存稳定适合长会话。
...
- **标签：** `agent`, `dsh-plugin`, `harness-engineering`, `tutorial`
- **推荐理由：** 从零到一掌握DSH插件开发的全面入门教程
...
- **推荐理由：** 运行时注入器和任务感知推理模式路由器的高级DSH控制工具
...
- **标签：** `agent-os`, `llm-orchestration`, `mcp`, `loop-engineering`
- **推荐理由：** 具备评估门控进化循环的自改进Agent OS，集成MCP服务器
...
- **推荐理由：** AI Agent首个虚拟终端，支持虚拟文件系统和沙箱执行。
...
- **标签：** `deepwiki`, `dsh-plugin`, `developer-tools`
- **推荐理由：** 自动生成代码技术文档，构建AI可用上下文。
...
- **标签：** `jailbreak`, `dsh-plugin`, `deepseek`
- **推荐理由：** 破甲插件，支持多模型灵活突破限制。
...
- **标签：** `context`, `dsh-plugin`, `developer-tools`
- **推荐理由：** DSH最佳上下文管理工具，带仪表盘和浏览器。
...
- **标签：** `coding-agent`, `cli`, `mcp`
- **推荐理由：** 带RPC插件、子代理和MCP集成的编码Agent工具。
...
- **推荐理由：** DSH插件市场管理CLI和解析工具，插件开发者必备。
...
- **推荐理由：** 本地优先的多模型桌面 Agent，支持自进化技能与隐私保护
...
- **⭐ 星标：** 198
- **语言：** JavaScript
- **描述：** Import conversation history from 25+ AI coding agents into DeepSeek Harness as resumable sessions — tool calls, reasoning and results kept intact, with reverse export and sync. | 把 25+ AI 编程 Agent 的聊天记录导入 DeepSeek Harness 继续对话，保留工具调用、推理与结果，支持反向导出与同步。
- **标签：** `migration`, `import`, `sessions`
- **推荐理由：** 从25+ AI编程工具导入对话为可恢复会话
...
- **标签：** `agent-skills`, `vision`, `claude-code`
- **推荐理由：** 为Claude Code增添多模态视觉能力，支持截图和UI分析
...
- **推荐理由：** 开源设计系统生成器，本地优先桌面应用与UI生成工具。
...
- **标签：** `desktop`, `dsh`, `plugin-first`
- **推荐理由：** DSH生态现代化桌面客户端，插件优先架构与本地优先设计。
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑、终端、对话和Git等页面。
...
- **标签：** `taskboard`, `skills`, `ui-extension`
- **推荐理由：** 现代化可嵌入任务面板，支持Codex和DSH界面集成。
...
- **推荐理由：** 功能丰富的TUI伴侣：鲸鱼顶栏、流式思考、回滚等功能。
...
- **标签：** `floating-widget`, `dsh-plugin`, `deepseek`
- **推荐理由：** 可爱鲸鱼挂件，实时监控DeepSeek账户余额。
...
- **标签：** `desktop-app`, `gui`, `ssh-client`
- **推荐理由：** 功能丰富的桌面客户端，支持SSH、远程访问和插件系统。
...
- **标签：** `desktop-app`, `local-first`, `mcp`
- **推荐理由：** 本地优先的AI桌面工作台，支持MCP和项目管理和网页研究。
...
- **推荐理由：** 在回复中内联生成交互式UI组件：图表、表单、测验等。
...
- **标签：** `office`, `productivity`, `deepseek-harness-plugin`
- **推荐理由：** 将电子表格、文档、演示等办公套件深度集成进 Harness
...
- **标签：** `mobile`, `ios`, `desktop-app`
- **推荐理由：** 原生 iOS 客户端，通过 WebSocket 同步会话与 Agent 执行轨迹
...
- **推荐理由：** 在对话中运行实时 iOS 模拟器，提供 22 个自动化控制工具
...
- **推荐理由：** 跨平台桌面客户端，灵感源自 CodePilot，附带插件套件
...
- **标签：** `mcp`, `ui`, `skills`
- **推荐理由：** DSH Web界面技能与MCP管理工具，提升操作效率
...
- **标签：** `market`, `ui`, `plugin`
- **推荐理由：** DSH Web GUI集成插件市场，一键安装社区插件
...
- **标签：** `platform`, `workspace`, `plugins`
- **推荐理由：** 基于DSH的插件化Web工作平台，支持全局技能管理
...
- **标签：** `ui`, `skills`, `menu`, `context-management`
- **推荐理由：** 统一能力菜单，三档暴露策略减少上下文占用
...
- **推荐理由：** 一体化DevOps桌面，集成数据库、SSH、Docker和AI沙箱
...
- **标签：** `ui`, `compact`, `button`
- **推荐理由：** 一键折叠DSH上下文面板的实用按钮
...
- **推荐理由：** 自进化上下文数据库，统一智能体记忆、RAG知识与技能管理。
...
- **推荐理由：** 将文档转化为可查询RAG知识库，支持自主推理与维基维护。
...
- **推荐理由：** 便携式本地优先记忆层，支持跨应用Markdown持久化
...
- **标签：** `agent-memory`, `rag`, `memory-management`, `token-savings`
- **推荐理由：** 自演进记忆OS，混合检索大幅节省token的Agent记忆方案
...
- **推荐理由：** 专为构建有状态Agent设计的记忆库，支持持续学习
...
- **标签：** `memory`, `rag`, `context`, `consolidation`
- **推荐理由：** 三层自动整合的关联记忆，支持技能结晶化
...
- **标签：** `awesome-list`, `plugins`, `dsh`
- **推荐理由：** 按功能分类的80+插件目录，便于快速发现。
...
- **标签：** `dsh-plugins`, `ecosystem`, `dsh`
- **推荐理由：** DSH插件仓库和生态系统分析资源。
...
- **推荐理由：** 提炼人类思维模式为可复用智能体技能，适用于各类AI助手。
...
- **标签：** `agent-workspaces`, `coding-agent`, `no-code`, `self-hosted`
- **推荐理由：** 跨设备统一Agent工作区，支持任务追踪和API访问
...
- **标签：** `agent-skills`, `skill-marketplace`, `coding`, `automation`
- **推荐理由：** 全面的技能商店，覆盖文档、编程、自动化和机器学习
...
- **推荐理由：** 企业级多Agent工作台，支持Codex和DSH的可视化编辑器
...
- **推荐理由：** 持久状态内核支持长周期多Agent协作，大幅减少人工干预。
...
- **标签：** `agent-teams`, `dsh-plugin`, `deepseek`
...
- **标签：** `vision-toolkit`, `ocr`, `dsh-plugin`, `agent-skills`
- **推荐理由：** 含OCR和图片问答的视觉工具箱，增强Agent能力。
...
- **推荐理由：** 为DSH引入认知循环方法论，支持结构化科学发现工作流。
...
- **推荐理由：** 赋予 AI 可验证的视听感知能力，生成带时间戳的证据链
...
- **推荐理由：** 原生 SQLite 任务看板，支持 Agent 认领与审查流程
...
- **标签：** `evolution`, `self-improvement`, `review`
- **推荐理由：** 可组合Agent自进化模式，支持人工审查改进
...
- **推荐理由：** 基于Tushare的金融分析系统，支持MCP和技能拓展
...
- **标签：** `skill-generator`, `automation`, `agent`
...
- **标签：** `skills`, `tdd`, `debugging`, `planning`
- **推荐理由：** TDD、调试、规划技能，源自成熟的superpowers框架
...
- **标签：** `skills`, `engineering`, `productivity`, `github-issues`
- **推荐理由：** 25个预装工程技能，300B token训练，开箱即用
...
- **标签：** `web-search`, `research`, `agent-skills`, `integration`
- **推荐理由：** You.com提供的搜索与研究技能，适用于AI代理
...
- **标签：** `mentor`, `coding`, `education`, `review`
- **推荐理由：** AI编程导师，扫描项目生成章节并做代码审查
...
- **标签：** `docx`, `gongwen`, `templates`, `word`
- **推荐理由：** 全流程公文处理，符合国标GB/T 9704
...
- **标签：** `agent`, `skill`, `math-modeling`, `cumcm`
- **推荐理由：** 数学建模多智能体套件，含模型库和论文模板，参赛利器
...
- **标签：** `skill`, `agent`, `workflow`
- **推荐理由：** 议行分离机制，提升AI协作任务质量，跨平台通用
...
- **标签：** `skill`, `api`, `deepseek-harness`
- **推荐理由：** 腾讯官方SkillHub插件，含API文档与示例
...
- **标签：** `skill`, `godot`, `game-development`
...
- **推荐理由：** 免费小米MiMo语音合成插件，支持预设和自定义声音
...
- **推荐理由：** 工程工作流的 structured 任务阶段和验证记录。
...
- **推荐理由：** 文本Agent的视觉桥接插件，粘贴图片即可获取结构化JSON证据。
...
- **标签：** `api-docs`, `mcp`, `chinese-platforms`, `search`
- **推荐理由：** 零配置搜索6万+中国平台API文档
...
- **标签：** `cloud`, `mcp`, `huawei`, `terraform`, `sdk`
- **推荐理由：** 华为云官方工具包，含MCP工具与云端沙箱
...
- **标签：** `mcp`, `config`, `skills`, `deepseek-harness`
- **推荐理由：** 一键配置多平台MCP和Skills，支持7种AI工具
...
- **标签：** `search`, `discovery`, `mcp`
- **推荐理由：** 在DSH agent内搜索插件的发现工具
...
- **标签：** `browser-extension`, `chatgpt`, `deepseek`, `chrome-extension`
- **推荐理由：** 多功能浏览器扩展，增强多个AI平台并自带提示词管理
...
- **标签：** `browser-use`, `agent`, `cli`
- **推荐理由：** CLI+扩展让Agent无需中断即可使用真实登录浏览器
...
- **推荐理由：** 免费网页搜索插件，无需注册即可为任何模型添加联网能力。
...
- **推荐理由：** 持久指纹伪装浏览器，支持无限本���配置文件的Agent工具。
...
- **推荐理由：** 基于Playwright的浏览器自动化插件，支持自动化任务
...
- **标签：** `content-discovery`, `personal-ai`, `cross-platform`
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、YouTube、小红书等平台。
...
- **推荐理由：** OAuth集成ChatGPT，直接在DSH Agent中对话。
...
- **标签：** `resume-builder`, `privacy`, `self-hosted`
- **推荐理由：** 隐私优先的开源简历构建器，支持完全自定义与自托管。
...
- **标签：** `mcp`, `registry`, `secrets`
- **推荐理由：** 类OpenRouter的Agent工具注册中心，支持MCP和密钥管理。
...
- **标签：** `cli`, `multi-model`, `credentials`
- **推荐理由：** 统一多模型账户管理，支持智能路由和自动故障切换。
...
- **标签：** `ecosystem`, `plugin-registry`, `dsh-plugin`
- **推荐理由：** 生态雷达扫描2万+插件并做K8s验证。
...
- **标签：** `plugin-shop`, `dsh-plugin`, `marketplace`
- **推荐理由：** 每日更新的综合插件商店，内容预审核。
...
- **标签：** `cybersecurity`, `vulnerability`, `redteam`
- **推荐理由：** AI安全审计平台，检测幻觉包、漏洞和安全缺口。
...
- **推荐理由：** 通过 OAuth 将各平台订阅转为 DSH LLM 提供商，免 API Key
...
- **推荐理由：** 集成 Command Code 模型提供商，支持实时目录与智能选择
...
- **推荐理由：** 安全P2P多端远程访问方案，支持PC/手机/Web
...
- **推荐理由：** 一键Docker私有化便签应用，支持skill调用和多租户
...
- **推荐理由：** 完整的DSH配置备份恢复工具，支持WebDAV同步
...
- **标签：** `community`, `catalog`, `marketplace`
...
- **标签：** `utility`, `container`, `dsh-plugin`, `management`
- **推荐理由：** 隔离容器管理多DSH实例，内嵌WebView界面
...
- **推荐理由：** 从Claude Code/Codex等迁移会话和技能的迁移工具
...
- **标签：** `utilities`, `keybindings`, `terminal`
- **推荐理由：** 实用插件集合：任务通知、LLM追踪、快捷键和终端
...
- **⭐ 星标：** 7
- **语言：** TypeScript
- **描述：** 致力于解决各插件之间也与版本之间可能存在的��突问题并提供解决方案
- **标签：** `conflict-resolution`, `utility`, `debugging`
- **推荐理由：** 检测并解决插件冲突和版本兼容问题
...
- **推荐理由：** 动画宠物画廊��为你的编码Agent添加可爱吉祥物视觉。
...
- **⭐ 星标：** 405
- **语言：** JavaScript
- **描述：** 是一层高自由度的玻璃质感主题，套在 DeepSeek Harness 网页端。顶栏、侧边栏、输入框、统计行、轨迹视图都成了磨砂玻璃片。玻璃模糊度、磨砂度、背景（流体或自定义壁纸，壁纸还能单独调模糊和磨砂）全都能在设置卡片里自由调节。关掉开关就回到原生界面，不改 DSH ���何一行源码。
- **标签：** `theme`, `glassmorphism`, `deepseek-harness-plugin`
- **推荐理由：** 高自由度磨砂玻璃主题，无需修改源码即可自由调节
...
- **标签：** `theme`, `animated`, `glassmorphism`
- **推荐理由：** 动态海洋主题，支持波浪、日落和磨砂玻璃效果可调
...
- **标签：** `ai-resources`, `tutorials`, `prompt-engineering`
- **推荐理由：** 全面的AI资源库，涵盖DeepSeek、GPT、Claude及RAG教程。
...
- **标签：** `latex`, `arxiv`, `research`
- **推荐理由：** 一站式科研平台：LaTeX、arXiv、GPU SSH和实验追踪。
...
- **推荐理由：** 每日自动抓取复核，按场景分类精选，帮你快速找到合适插件
...
- **标签：** `awesome-list`, `resources`, `curation`
- **推荐理由：** 全面收录插件、技能、MCP、补丁和 UI 的精选资源清单
...
- **推荐理由：** DSH 终极指南：快速入门、精选插件与实用工具合集
...
- **推荐理由：** 跨库学术检索，支持引文图谱、MeSH 标注与临床试验核验
...
- **标签：** `research`, `company-research`, `industry`
- **推荐理由：** 完整的行业/公司研究包，含方法论和公开源政策追踪
...
- **标签：** `patent`, `analysis`, `legal-tech`, `rag`
- **推荐理由：** 专利侵权分析系统，生成竞品比对报告
...
- **标签：** `stock-analysis`, `quantitative`, `backtesting`, `research`
- **推荐理由：** 39个工具和20种策略，覆盖全球多市场量化分析
...
- **推荐理由：** 精选股票/金融/量化插件，24条经过��证的双语条目
...
- **标签：** `dsh-web`, `dsh-plugin`, `plugin-aggregation`
- **推荐理由：** Web插件聚合生态，通过创意工坊分发DSH插件
...
- **标签：** `ai-writing`, `novel-writing`, `dsh-plugin`
- **推荐理由：** 全流程AI小说创作工具，含角色与世界构建。
...
- **标签：** `knowledge-base`, `mcp`, `mlops`
- **推荐理由：** 全面的AI知识百科，涵盖RAG、Agent、MLOps和学习路线。
...
- **标签：** `discovery`, `plugin-manager`, `verification`
- **推荐理由：** 自动搜索、安装和验证DSH插件的Skill
...
- **推荐理由：** 精选的DSH插件awesome列表，便于发现和探索插件
...
- **推荐理由：** The bioinformatics agent desktop for clinicians and wet-lab scientists — built o
...
- **⭐ 星标：** 44
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇���/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
- **标签：** `ai-agent`, `bazi`, `chinese-traditional-wisdom`, `dashboard`, `deepseek-harness`, `dsh-plugin`, `fengshui`, `iching`, `tcm`, `ziwei-doushu`
- **推荐理由：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫微/六爻/梅花/奇���/风水/五运六气/
...
- **标签：** `deepseek-harness`, `delivery-gate`, `dsh`, `dsh-plugin`, `engineering-discipline`, `guard`, `quality-gate`, `requirements`, `skill`
- **推荐理由：** Double-check before you ship: grill the requirements, test the implementation, p
...
- **⭐ 星标：** 41
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据��处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **推荐理由：** 数学建模资源，包含辅助建模、数据��处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harne
...
- **标签：** `agent-orchestration`, `agent-team`, `ai-agents`, `deepseek`, `deepseek-harness`, `dsh`, `dsh-plugin`, `multi-agent`, `multi-agent-systems`
- **推荐理由：** dsh-agent-team gives DeepSeek Harness agents that don't reset: durable Members w
...
- **推荐理由：** DeepSeek Harness (DSH) plugin: replaces the stats row under the composer with pr
...
- **标签：** `agent-skills`, `deepseek-harness`, `motion-design`, `remotion`, `typescript`, `video`
- **推荐理由：** 面向 DeepSeek Harness 的电影感产品视频 Agent Skill，包含 152 张镜头配方卡、Remotion 模板、代码组件和音频资产。
...
- **推荐理由：** 让 AI 像资深测试工程师一样工作：面向 AI Agent 的测试工程 Skill 框架——11 Skills + 共享知识库 + 类型决策矩阵（Claude 
...
- **标签：** `catalog`, `community`, `reference`
- **推荐理由：** 社区精选DSH插件目录，便于发现和参考
...
- **推荐理由：** 全面插件生态目录，涵盖CLI、Web UI和智能体
...
- **标签：** `awesome-list`, `catalog`, `directory`
- **推荐理由：** 收录1000+插件的全量清单，按star排序
...
- **标签：** `documentation`, `guide`, `awesome-list`
- **推荐理由：** 面向用户的插件说明书与导航指南
...
- **标签：** `marketplace`, `skills`, `directory`
...
- **标签：** `radar`, `evidence-backed`, `awesome-list`
...
- **语言：** Unknown
- **描述：** DSHA精选插件合集
- **标签：** `awesome-list`, `curated`, `collection`
- **推荐理由：** DSH精选插件合集
