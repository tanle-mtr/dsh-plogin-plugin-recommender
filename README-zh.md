...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 236,771 | Core framework powering the entire DSH plugin ecos |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 98,183 | Open-source Claude Design alternative with local-f |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 73,323 | Deploy multi-agent swarms with autonomous workflow |
...
- **标签：** `coding-agent`, `terminal`, `prompt-caching`, `r1`
...
- **标签：** `agent-workspace`, `coding-agent`
- **推荐理由：** 统一智能体工作空间，多设备跨平台任务看板。
...
- **标签：** `router`, `dsh-plugin`
- **推荐理由：** 任务感知推理路由，优化DSH智能体执行流程。
...
- **标签：** `vision`, `ocr`, `agent`, `multimodal`
- **推荐理由：** DSH首个视觉插件，支持OCR识别和结构化JSON输出。
...
- **标签：** `terminal`, `vfs`, `sandbox`, `agent`
- **推荐理由：** 首个AI虚拟终端，支持VFS隔离执行环境。
...
- **标签：** `desktop`, `electron`, `gui`, `deepseek-harness`
- **推荐理由：** 零安装Windows桌面客户端，完整插件技能生态
...
- **标签：** `desktop`, `local-first`, `react`
- **推荐理由：** 本地优先AI桌面工作区，支持项目和文件管理
...
- **标签：** `coding-agent`, `rpc`, `go`
- **推荐理由：** 强大的编码代理，支持rpc、子代理和哈希行编辑
...
- **标签：** `free-model`, `llm`, `plugin`
- **推荐理由：** 免费无限制使用Muse Spark等前沿模型
...
- **推荐理由：** 为Claude Code添加多模态视觉能力，支持截图和图表分析
...
- **标签：** `guard`, `quality-gate`, `requirements`
- **推荐理由：** 工程纪律quality gate，交付前严苛审查需求与实现
...
- **推荐理由：** DSH插件开发官方文档存档，按需调用为agent技能
...
- **⭐ 星标：** 2
- **语言：** JavaScript
- **描述：** 给 DSH agent 的插件库"起飞前检查单"——任务型请求时自动预检插件库并注入 Top-K 适用插件提示，让插件库利用���可预期、不靠运气。Pre-flight plugin-library check for DSH agents — task-type requests trigger a Top-K hint of suitable plugins injected into the runtime context, making plugin usage predictable instead of opportunistic.
- **标签：** `agent-tools`, `ai-agents`, `dsh-plugin`
- **推荐理由：** 插件库预检，让DSH agent按需自动匹配适用插件
...
- **标签：** `desktop-app`, `dsh-plugin-desktop`, `cordis`
...
- **标签：** `browser-extension`, `ai-studio`, `deepseek`
- **推荐理由：** 多平台AI浏览器增强套件，功能全面易用。
...
- **标签：** `desktop`, `deepseek-harness`
- **推荐理由：** 官方桌面客户端，将DSH拓展至桌面应用。
...
- **标签：** `tui`, `terminal`, `ui`, `desktop`
- **推荐理由：** 功能丰富的TUI插件，支持实时状态和流式思考显示。
...
- **标签：** `task`, `panel`, `ui`, `desktop`
- **推荐理由：** 现代化任务面板，可灵活嵌入Codex和DSH桌面。
...
- **标签：** `floating-widget`, `plugin`
- **推荐理由：** 可爱的鲸鱼浮动小部件，实时显示DeepSeek账户余额。
...
- **⭐ 星标：** 358
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将工作��、会话、实时回复和 Agent 执行轨迹带到 iPhone，同时延续 DeepSeek WebUI 克制、清晰��视觉语言
- **标签：** `mobile`, `ios`, `client`
- **推荐理由：** 原生iOS客户端，WebSocket连接
...
- **推荐理由：** 一键Docker部署的锤子便签风格笔记，支持公众号导出
...
- **标签：** `market`, `plugin`, `webui`
- **推荐理由：** 嵌入DSH Web GUI的社区插件市场，一键安装管理
...
- **标签：** `desktop`, `workspace`, `workbench`
- **推荐理由：** 基于DSH生态的工作台桌面应用，项目管理一体化
...
- **推荐理由：** 面向临床与湿实验科学家的生物信息学桌面应用，支持离线运行
...
- **标签：** `ui`, `info-bar`, `stats`
- **推荐理由：** 替换底部状态栏，显示模型、余额与峰谷定价
...
- **推荐理由：** Electron封装的DSH桌面版，集成侧边栏和Web UI
...
- **标签：** `ui`, `theme`, `background`, `personalization`
- **推荐理由：** 支持上传自定义图片或切换预设背景，美化Web界面。
...
- **标签：** `rag`, `memory`, `milvus`
- **推荐理由：** 基于Milvus的持久化AI代理记忆层，支持语义搜索。
...
- **推荐理由：** 将分散的SKILL.md文件转化为可检索的智能体语料库
...
- **标签：** `meta-skill`, `knowledge-distillation`, `agent-skills`
...
- **标签：** `low-code`, `ai-agent`, `internal-tools`, `workflows`
...
- **标签：** `agent`, `workflow`, `multi-agent`, `automation`
- **推荐理由：** 持久状态内核，支持长周期多Agent工作流自动化。
...
- **标签：** `skills`, `workflow`, `agent`, `routing`
- **推荐理由：** 智能技能路由和流程编排，显著提升Agent性能。
...
- **标签：** `creative-writing`, `novel-writing`
- **推荐理由：** 端到端AI小说创作工作流，支持角色与世界管理。
...
- **推荐理由：** 为纯文本模型提供视觉工具包：图像问答、OCR、UI还原。
...
- **标签：** `epistemic-loop`, `agent`, `ai-agents`
- **推荐理由：** 为DeepSeek智能体引入认知循环推理能力
...
- **标签：** `skill`, `multimodal`, `mcp`
- **推荐理由：** 赋予AI多模态感知与可验证输出
...
- **标签：** `task`, `productivity`, `agent`
- **推荐理由：** 原生SQLite任务管理，支持Agent工作流
...
- **标签：** `skills`, `research`, `workflows`
- **推荐理由：** 88个开源技能，覆盖研究与营销智能工作流
...
- **⭐ 星标：** 196
- **语言：** JavaScript
- **描述：** DSH 技能调用审计 + 应用��� skill 管理器
- **标签：** `skill-panel`, `audit`, `cordis`
- **推荐理由：** DSH技能调用审计与管理面板，Cordis集成
...
- **标签：** `skills`, `architecture`, `portable`
- **推荐理由：** 将经验转化为可复用执行模板，跨多平台通用
...
- **标签：** `finance`, `skill`, `datasource`
- **推荐理由：** 本地财经数据库，自适应接口与多Agent支持
...
- **推荐理由：** AI基础设施开发专用agent技能，支持编写和调试
...
- **标签：** `agent-skills`, `automation`, `skill-learning`
...
- **推荐理由：** Superpowers工程技能移植，TDD调试规划全覆盖
...
- **标签：** `skills`, `dsh-plugin`, `agent`
- **推荐理由：** 内置25个工程技能，开发效率提升十倍
...
- **标签：** `agent-skills`, `mcp`, `dsh-plugin`
- **推荐理由：** 网页搜索与研究插件，AI Agent必备技能
...
- **标签：** `dsh-plugin`, `ai-docs`, `gongwen`
- **推荐理由：** 公文全流程处理，格式校验优化一键生成
...
- **推荐理由：** 融合八字、紫微、风水、中医等中华传统智慧的AI技能包
...
- **⭐ 星标：** 42
- **语言：** Python
- **描述：** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其���要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **标签：** `math-modeling`, `skill`, `research`
- **推荐理由：** 数学建模全流程辅助技能，含数据处理、编程与论文写作
...
- **推荐理由：** 多智能体协作，独立模型/技能/MCP工具与共享工作区
...
- **推荐理由：** 5角色多智能体数学建模求解器，含33模型库与论文模板
...
- **推荐理由：** 像素级图像识别与OCR工具，弥补纯文本模型视觉能力
...
- **标签：** `video`, `motion-design`, `skill`
- **推荐理由：** 电影感产品视频制作技能，含152张镜头配方与Remotion模板
...
- **标签：** `skill`, `planning`, `workflow`
- **推荐理由：** 议行分离机制，让AI协作变更任务更有序
...
- **推荐理由：** DSH插件开发Agent技能，兼容Claude Code和Codex
...
- **推荐理由：** 开源Claude Design替代方案，本地优先UI生成。
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`, `architecture-diagram`
- **推荐理由：** 精美架构图和时序图，自包含HTML输出。
...
- **标签：** `design`, `agent`, `ui`, `vibedesign`
- **推荐理由：** 首个开源AI原生矢量设计工具，支持Agent协作和代码驱动设计。
...
- **推荐理由：** AI生成设计系统，含可视化编辑器、模板和PPT工作室。
...
- **标签：** `canvas`, `session`, `visual`
- **推荐理由：** 基于画布的可视化会话探索器与分支工作区
...
- **标签：** `visual`, `knowledge-card`, `sharing`
- **推荐理由：** 将聊天记录转为精美知识卡片，便于分享下载。
...
- **推荐理由：** 本地优先跨平台内容发现Agent，覆盖B站、YouTube等多平台。
...
- **标签：** `browser`, `playwright`, `proxy`
- **推荐理由：** 持久化浏览器身份与指纹伪装引擎
...
- **推荐理由：** Tabbit浏览器集成，支持Playwright自动化操作
...
- **推荐理由：** GPT Image 2提示词大全，530+工业案例。
...
- **标签：** `preset`, `config`, `dsh`, `harness`
- **推荐理由：** 两阶段预设配置，最小引导加完整工具链。
...
- **标签：** `utility`, `account`, `routing`, `cli`
- **推荐理由：** 统一管理多模型账户，支持智能路由和配额追踪。
...
- **标签：** `research`, `latex`, `gpu`
- **推荐理由：** 一站式科研工作台，支持LaTeX、arXiv、GPU管理
...
- **推荐理由：** 完���的DSH配置备份恢复同步工具，支持WebDAV
...
- **推荐理由：** 官方插件注册表控制台，零侵入安装，自带插件开发引导技能
...
- **标签：** `container`, `management`, `tool`
- **推荐理由：** 容器化管理多版本DSH，内嵌Web UI
...
- **标签：** `migration`, `tool`, `utility`
- **推荐理由：** 从Claude Code迁移会话、技能和配置到DSH
...
- **推荐理由：** 接入免费小米MiMo TTS，支持自定义和浏览器语音
...
- **标签：** `conflict-resolution`, `compatibility`, `utility`
- **推荐理由：** 解决插件冲突与版本兼容问题，保障系统稳定。
...
- **标签：** `mascot`, `pixel-art`, `ui`, `theme`
- **推荐理由：** 可定���动画宠物插件，美化终端体验。
...
- **标签：** `theme`, `ocean`, `animated`
- **推荐理由：** 动态海洋主题，可调波浪与磨砂效果
...
- **标签：** `ai-guide`, `vibe-coding`, `rag`, `prompt-engineering`
- **推荐理由：** 全面AI资源库，涵盖DeepSeek/GPT等教程。
...
- **标签：** `deepwiki`, `dsh-plugin`
- **推荐理由：** 快速生成结构化技术文档和AI可用的上下文。
...
- **标签：** `encyclopedia`, `resources`, `comprehensive`
- **推荐理由：** 覆盖全维度AI资源百科全书
...
- **标签：** `api-documentation`, `dsh-plugin`, `research`
- **推荐理由：** 零配置检索20个中国平台6万+API文档
...
- **标签：** `stock`, `analysis`, `backtest`
- **推荐理由：** 39个工具+20策略覆盖多市场股票回测分析
...
- **推荐理由：** 全面的DSH插件生态列表，涵盖CLI和Web UI
...
- **标签：** `catalog`, `awesome-list`, `discovery`
- **推荐理由：** 收录1000+插件的全量目录，按star排序便于检索。
...
- **标签：** `guide`, `documentation`, `directory`
- **推荐理由：** 提供插件安装、权限与兼容性指南，帮助用户选型。
...
- **标签：** `ai-curated`, `recommender`, `awesome-list`
...
- **标签：** `curated`, `directory`, `comprehensive`
- **推荐理由：** 涵盖模型、工具、沙箱、UI等全方位插件目录。
...
- **标签：** `radar`, `evidence-backed`, `plugin-lab`
...
- **标签：** `automated`, `crawler`, `bilingual`
- **推荐理由：** 每日自动爬取GitHub标签，生成双语文档目录。
...
- **标签：** `security`, `audit`, `offense-defense`
- **推荐理由：** AI安全攻防平台，含幻觉代码审计功能
...
- **标签：** `writing`, `creative`, `fiction`
- **推荐理由：** 创作型写作工作流，支持网文、短剧、游戏视频平台
...
- **推荐理由：** 精选DSH插件awesome列表，便于发现和探索插件生态
...
- **标签：** `catalog`, `plugins`, `awesome-list`
- **推荐理由：** 非官方DSH插件社区目录汇总
