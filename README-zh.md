...
- [Agent 技能与工作流](#agent-技能与工作流) — 53 个插件
...
- [Agent 技能与工作流](agent-skills-workflows-zh) (53)
...
- **标签：** `dsh`, `harness`, `ai-agents`, `plugin-system`
- **推荐理由：** DSH核心平台，万物皆插件
...
- **标签：** `coding-agent`, `cli`, `prompt-caching`, `r1`, `tui`
- **推荐理由：** 终端原生AI编码代理，支持提示缓存
...
- **标签：** `no-code`, `ai-agent`, `crm`, `low-code`, `workflows`
- **推荐理由：** AI增��型零代码平台，快速构建业务系统
...
- **标签：** `routing`, `harness-tool`
- **推荐理由：** DSH工作流中任务感知推理模式的路由注入套件
...
- **标签：** `terminal`, `vfs`, `sandbox`, `agent-tools`
- **推荐理由：** 首个虚拟终端，为Agent提供隔离的Bash/Python环境。
...
- **标签：** `inference`, `reasoning`, `agent-tools`
- **推荐理由：** 跨模型推理控制套件，支持深度推理和错误恢复
...
- **推荐理由：** Rust高性能Agent运行时，搭配精美桌面IDE体验
...
- **推荐理由：** 编码代理，支持RPC插件、子代理和MCP，高效代码操作。
...
- **推荐理由：** 为纯文本LLM增加多模态视觉能力，支持截图UI分析
...
- **标签：** `workbench`, `skills`, `plugin-system`
- **推荐理由：** 插件化Web工作平台，内置全局技能管理
...
- **标签：** `desktop`, `dsh-plugin`, `cordis`, `ui-extension`
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git等多页面。
...
- **标签：** `taskboard`, `ui`, `codex`, `dsh`
- **推荐理由：** 现代灵活任务面板，可嵌入Codex和DSH使用。
...
- **标签：** `tui`, `terminal`, `status`, `dsh`
- **推荐理由：** 功能丰富的TUI：鲸鱼顶栏/实时状态/流式思考/回滚。
...
- **标签：** `widget`, `deepseek`, `balance`
- **推荐理由：** 可爱鲸鱼浮窗监控余额，支持拖拽吸附和动画效果
...
- **推荐理由：** 功能完整的桌面GUI客户端，支持SSH和移动端远程访问，开箱即用。
...
- **标签：** `canvas`, `visualization`, `branching`
...
- **标签：** `office`, `dsh-plugin`, `deepseek-harness`
- **推荐理由：** 将表格/文档/幻灯片等办公能力嵌入Harness
...
- **标签：** `ios`, `mobile`, `dsh-plugin`
- **推荐理由：** 原生iOS客户端，WebSocket连接Harness，移动办公
...
- **推荐理由：** 跨平台桌面客户端，CodePilot风格，插件套件丰富
...
- **标签：** `mcp`, `skills`, `ui`
- **推荐理由：** Web界面Skill/MCP管理工具，提升使用效率。
...
- **推荐理由：** Electron桌面控制台，多标签管理Harness/MCP/技能
...
- **⭐ 星标：** 12
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运维���天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `devops`, `ssh`, `docker`
- **推荐理由：** 集数据库、SSH、Docker、AI沙箱于一体的DevOps桌面
...
- **标签：** `ui`, `knowledge-cards`, `visualization`
...
- **标签：** `mcp`, `registry`, `api-keys`, `proxy`
- **推荐理由：** 类OpenRouter的Agent工具注册中心，支持MCP。
...
- **推荐理由：** 基于tushare的本地财经DB，自动MCP接口与IM盯盘
...
- **标签：** `design`, `ui-generator`, `figma-alternative`, `desktop`
- **推荐理由：** 开源设计替代，内置UI生成能力
...
- **标签：** `diagrams`, `mermaid-alternative`, `architecture`, `visualization`
- **推荐理由：** 优雅的架构/序列图，代码即图表
...
- **推荐理由：** 开源AI原生矢量设计工具，支持多Agent协同设计。
...
- **标签：** `3d`, `blender`, `automation`
- **推荐理由：** 通过TCP直连实现AI实时驱动Blender 3D建模
...
- **推荐理由：** 无需注册的免费网页搜索插件，为无网络能力的模型提供搜索桥接。
...
- **标签：** `image-uploader`, `s3`, `cloudflare-r2`, `electron`
...
- **标签：** `account`, `routing`, `utility`, `multi-model`
- **推荐理由：** 统一多模型账户管理，支持智能路由与故障转移。
...
- **标签：** `curated-list`, `resources`, `awesome`
- **推荐理由：** 精选DSH插件与工具资源清单，一站式发现
...
- **推荐理由：** 用于管理和发现DSH插件的CLI工具，支持插件中心。
...
- **推荐理由：** OAuth登录使用现有订阅作为模型提供商，免去API Key
...
- **标签：** `llm-provider`, `commandcode`, `dsh-plugin`
- **推荐理由：** 新增Command Code模型提供商，支持智能选型
...
- **标签：** `container`, `manager`, `utility`
- **推荐理由：** 容器隔离多版本DSH，嵌入式UI管理
...
- **标签：** `conflict-resolution`, `compatibility`
- **推荐理由：** 解决插件间冲突与版本兼容问题
...
- **标签：** `mascot`, `pixel-art`, `cli`, `fun`
- **推荐理由：** 为CLI编码Agent提供动画吉祥物宠物画廊。
...
- **标签：** `theme`, `glass`, `customization`
- **推荐理由：** 磨砂玻璃风格主题，支持完全自定义DSH网页界面。
...
- **标签：** `theme`, `animated-background`, `skin`
- **推荐理由：** 动态海洋主题，支持波浪/日落/玻璃效果调节
...
- **标签：** `background`, `customization`, `theme`
- **推荐理由：** 支持上传自定义图片和预设氛围背景
...
- **标签：** `ai-guide`, `tutorials`, `prompt-library`, `knowledge-base`
- **推荐理由：** 全面AI资源大全，含教程与知识库
...
- **标签：** `documentation`, `code`, `technical`
- **推荐理由：** 从代码生成精准技术文档，适合团队和AI代理使用
...
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、实验追踪和GPU编排。
...
- **标签：** `research`, `mcp`, `academic-search`
- **推荐理由：** 跨库学术检索，支持引文图谱、核验与MeSH标签
...
- **标签：** `plugin-discovery`, `search`, `verify`
- **推荐理由：** 搜索安装验证DSH GitHub插件的一站式Skill
...
- **推荐理由：** 精选280+插件目录，覆盖14类DSH插件发现指南
...
- **标签：** `stock`, `quant`, `backtest`
- **推荐理由：** 39工具+20策略，覆盖亚太美股分析
...
- **标签：** `image-prompts`, `prompt-engineering`, `gpt-image-2`, `template-library`
- **推荐理由：** GPT Image2提示词库，530+案例模板
...
- **推荐理由：** 收录插件/技能/MCP服务器/补丁/UI的全面指南
...
- **标签：** `plugin-registry`, `dsh-plugins`, `skills`
- **推荐理由：** 去垃圾的官方插件注册表，开放数据
...
- **标签：** `plugin-directory`, `dsh-plugin`, `typescript`
...
- **标签：** `awesome-list`, `dsh-plugin`, `ai-agent`
- **推荐理由：** 全面的DSH插件生态列表，覆盖CLI和Web界面
...
- **标签：** `ai-curated`, `recommendation`, `awesome-list`
- **推荐理由：** AI 筛选的 190+ 插件分类推荐列表
