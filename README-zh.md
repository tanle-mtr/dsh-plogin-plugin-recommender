...
- [记忆与知识（RAG）](memory-knowledge-rag-zh) (14)
...
- **标签：** `routing`, `orchestration`, `agent`
- **推荐理由：** 任务感知推理模式路由器，支持高级代理编排。
...
- **标签：** `vision`, `ocr`, `multimodal`, `agent-tools`
- **推荐理由：** DSH首个视觉插件，将图片转为结构化JSON证据
...
- **标签：** `preset`, `harness-tools`, `deepseek-harness`
- **推荐理由：** 两阶段DSH预设：最小化引导后加载完整标准工具
...
- **标签：** `virtual-terminal`, `sandbox`, `agent-tools`, `vfs`
- **推荐理由：** 全球首个AI Agent虚拟终端，支持VFS与沙箱环境
...
- **标签：** `agent-tools`, `routing`, `multi-model`, `harness`
- **推荐理由：** 多模型账号统一管理，支持智能路由与自动故障转移
...
- **标签：** `agent-runtime`, `coding-assistant`, `dsh-plugin`
- **推荐理由：** Rust高性能Agent运行时，搭配精美桌面界面。
...
- **推荐理由：** 功能丰富的编码Agent，支持RPC插件、子Agent和MCP。
...
- **标签：** `security`, `auditing`, `vulnerability`, `redteam`
...
- **标签：** `ai-agents`, `migration`, `dsh-plugin`
- **推荐理由：** 打通Pi生态与DSH插件无缝迁移
...
- **推荐理由：** 可复用插件构建验证技能，加速DSH插件开发生命周期
...
- **标签：** `context`, `audit`, `ui`, `tooling`
- **推荐理由：** 上下文注入审计工具，Web UI检测重复与冲突tokens
...
- **推荐理由：** 开��侧边栏底座，内置文件编辑/终端/Git/子代理页面
...
- **标签：** `taskboard`, `ui`, `panel`, `desktop`
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex和DSH中使用
...
- **标签：** `tui`, `ui`, `terminal`, `status`
- **推荐理由：** TUI增强插件：鲸鱼顶栏/实时状态/流式思考/TPS显示
...
- **标签：** `desktop`, `gui`, `ssh`, `electron`
- **推荐理由：** DSH官方桌面客户端，支持SSH、插件和移动端远程访问。
...
- **标签：** `desktop`, `local-first`, `mcp`, `office`
- **推荐理由：** 本地优先桌面工作空间，集成会话、文件与办公自动化。
...
- **标签：** `ui`, `charts`, `interactive`, `components`
- **推荐理由：** 在对话中渲染图表、表单、测试等交互式UI组件。
...
- **标签：** `canvas`, `conversation`, `visual`, `branching`
- **推荐理由：** 画布式非线性对话工作区，支持分支探索与可视化导航。
...
- **标签：** `desktop-app`, `agent-framework`, `local-first`
- **推荐理由：** 本地优先的多模型桌面代理应用，支持技能自进化。
...
- **标签：** `ios`, `mobile`, `native-client`
- **推荐理由：** 原生iOS客户端，通过WebSocket连接DSH随时访问。
...
- **推荐理由：** 跨平台桌面客户端插件套件，灵感来自CodePilot。
...
- **推荐理由：** Electron桌面控制台，多标签管理DSH/MCP/Skills
...
- **标签：** `ui`, `stats-bar`, `usage-tracking`
- **推荐理由：** 替换底部状态栏，实时显示模型用量余额与峰谷定价
...
- **推荐理由：** 集成数据库、SSH、Docker 和 AI 沙箱的一体化桌面终端
...
- **推荐理由：** Electron 桌面集成 DSH Web 服务，含侧边栏和 ModLens
...
- **标签：** `ui`, `knowledge-card`, `chat`
- **推荐理由：** 将聊天记录转化为精美可分享的卡片。
...
- **推荐理由：** 可移植本地优先的记忆层，Markdown原生长期存储。
...
- **标签：** `agent-memory`, `stateful-agents`, `memory`
- **推荐理由：** 构建有状态自学习代理的核心记忆库。
...
- **标签：** `documentation`, `wiki`, `knowledge`, `rag`
- **推荐理由：** 分钟级生成精准技术文档与AI可读上下文
...
- **标签：** `agent-memory`, `rag`, `milvus`, `long-term-memory`
- **推荐理由：** 基于Milvus的持久化记忆层，支持多Agent统一检索。
...
- **标签：** `rag`, `embeddings`, `skill-management`, `vector-search`
- **推荐理由：** 将SKILL.md文件转为可语义检索的Agent知识库。
...
- **标签：** `awesome-list`, `catalog`, `marketplace`
- **推荐理由：** 11000+插件目录，支持搜索和免费API
...
- **标签：** `agentic-workflow`, `mcp-server`, `dsh-plugin`
- **推荐理由：** 多智能体蜂群编排框架
...
- **标签：** `writing`, `webnovel`, `skill`
- **推荐理由：** 专为200万字网文创作设计的技能，解决遗忘问题。
...
- **标签：** `ai-writing`, `novel-writing`, `rag`, `worldbuilding`
- **推荐理由：** 结构化小说创作流程，支持世界观构建与RAG检索。
...
- **标签：** `agent-vision-toolkit`, `ocr`, `computer-vision`
- **推荐理由：** 纯文本模型视觉扩展，支持OCR与图片问答。
...
- **标签：** `creative-writing`, `novel`, `workflow`, `drama`
- **推荐理由：** 网文/短剧/游戏/视频解说四大创作工作台，专属写作流。
...
- **标签：** `agent-skills`, `computer-vision`, `audio-understanding`
- **推荐理由：** 赋予AI多模态感知能力，支持视频音频屏幕分析。
...
- **标签：** `ai-agents`, `compatibility-layer`, `dsh`
- **推荐理由：** 数学建模AI自动领航+GitOps流水线
...
- **标签：** `cordis`, `dsh-plugin`, `skills`
- **推荐理由：** DSH技能调用审计+应用内管理器
...
- **推荐理由：** AI编程导师，生成阶梯式教程并自动Code Review
...
- **标签：** `agent-skills`, `web-search`
- **推荐理由：** You.com提供网络搜索与研究技能，助力AI代理
...
- **标签：** `skill`, `gongwen`, `docs`
- **推荐理由：** 党政机关公文全流程处理，支持格式校验与优化
...
- **标签：** `ai-agent`, `bazi`, `fengshui`, `tcm`
- **推荐理由：** 八字紫微风水中医融合，本地确定性引擎，文化特色鲜明
...
- **标签：** `math-modeling`, `data-processing`, `paper-writing`
- **推荐理由：** 数学建模全流程覆盖，数据处理到论文写作一步到位
...
- **标签：** `multi-agent`, `mcp`, `collaboration`
- **推荐理由：** 独立模型与MCP工具的协作Agent团队，共享工作区
...
- **推荐理由：** 五角色数学建模多Agent系统，含33篇模型库与论文模板
...
- **标签：** `skillhub`, `documentation`, `deepseek-harness`
- **推荐理由：** 官方SkillHub文档示例，DSH生态学习必备
...
- **推荐理由：** 改编自官方DSH的Code Simplification技能
...
- **推荐理由：** 开发 DSH 插件的标准 Agent Skill，支持完整适配器
...
- **标签：** `agent-tool`, `workflow`, `automation`
...
- **标签：** `mcp`, `api-keys`, `credentials`, `secrets`
- **推荐理由：** Agent工具版OpenRouter，支持MCP代理与密钥管理。
...
- **标签：** `agent`, `ui`, `vibedesign`, `design`
- **推荐理由：** 开源AI原生矢量设计工具，支持Agent协同与代码化设计
...
- **标签：** `ai-design`, `ppt`, `prototyping`, `visual-editor`
- **推荐理由：** AI原生设计工作室，支持PPT生成与原型设计。
...
- **⭐ 星标：** 61
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（���事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **标签：** `visualization`, `legal`, `diagram`
- **推荐理由：** 法律诉讼可视化工具，生成时间轴与关系图
...
- **标签：** `video`, `remotion`, `motion-design`, `shot-craft`
...
- **标签：** `browser-extension`, `ai-studio`, `prompt-manager`
- **推荐理由：** 多功能浏览器扩展，增强多个AI平台并支持提示词管理。
...
- **标签：** `browser`, `content-discovery`, `multi-platform`, `web-automation`
...
- **标签：** `browser-automation`, `fingerprint-spoofing`, `playwright`, `mcp`
- **推荐理由：** 持久指纹伪造浏览器，无限本地Profile及安卓模拟。
...
- **标签：** `ios-automation`, `mobile-automation`, `webdriveragent`
- **推荐理由：** 22个代理工具控制iOS模拟器和真实设备。
...
- **标签：** `cli`, `plugin-manager`, `hub`, `infrastructure`
- **推荐理由：** DSH插件中心必备CLI，支持插件发现与安装管理。
...
- **推荐理由：** 通过OAuth将订阅账号作为DSH的LLM提供商使用。
...
- **标签：** `llm-provider`, `command-code`, `integration`
- **推荐理由：** 添加Command Code模型提供商，支持智能模型选择。
...
- **标签：** `deepseek`, `dsh-plugin`, `remote`
- **推荐理由：** 安全P2P多端远程访问Harness方案
...
- **标签：** `agent-skills`, `finance`, `mcp`
- **推荐理由：** tushare本地金融数据库+多Agent支持
...
- **标签：** `skill-picker`, `ui-enhancement`, `shortcuts`
...
- **标签：** `container`, `multi-version`, `local-management`
- **推荐理由：** 容器隔离运行多版本DSH，嵌入式WebView管理
...
- **推荐理由：** 一键同步MCP和Skills配置到所有主流AI工具
...
- **推荐理由：** 四源迁移助手，支持Claude/Codex/OpenCode数据迁移
...
- **标签：** `cordis`, `orchestration`, `launcher`, `extensible`
- **推荐理由：** 统一编排运行时、插件、技能和MCP的核心能力
...
- **推荐理由：** 精选 DSH 插件，含任务通知、trace 检测和快捷键
...
- **推荐理由：** 实时 Token 用量图表与费用估算，方便 API 成本管理
...
- **标签：** `oauth`, `chatgpt`, `subscription`, `utility`
- **推荐理由：** OAuth绑定ChatGPT账户，在DSH中使用模型。
...
- **标签：** `mascot`, `pixel-art`, `theme`, `ui`
- **推荐理由：** 为各Agent提供动效宠物装饰，趣味UI个性化
...
- **标签：** `theme`, `glass-morphism`, `ui-customization`, `skin`
- **推荐理由：** 高自由度磨砂玻璃主题，支持毛玻璃模糊与自定义壁纸。
...
- **标签：** `ai-guide`, `dsh-plugin`, `rag`
- **推荐理由：** 全面的AI资源和Vibe Coding教程
...
- **标签：** `marketplace`, `plugin-shop`, `dsh-plugin`
- **推荐理由：** 每日更新的插件市场，全网搜集审核上架。
...
- **推荐理由：** 一站式科研工作台：LaTeX实时编译、arXiv管理、GPU调度。
...
- **标签：** `patent`, `analysis`, `rag`
- **推荐理由：** 专利侵权分析系统，一键生成竞品侵权报告
...
- **标签：** `kline`, `technical-analysis`, `trading`, `sidebar`
...
- **标签：** `awesome-list`, `curated`, `chinese`
- **推荐理由：** 精选中文DSH插件列表，适合国内开发者
...
- **推荐理由：** 24 个经过验证的股票/金融/量化插件双语精选列表
...
- **标签：** `awesome-list`, `verified`, `catalog`
- **推荐理由：** 经认证按star排序的社区插件目录。
...
- **标签：** `awesome-list`, `documentation`, `directory`
- **推荐理由：** 按场景选插件，附安装与兼容性说明。
...
- **标签：** `awesome-list`, `ai`, `recommendation`
...
- **标签：** `radar`, `research`, `awesome-list`
- **推荐理由：** 实证驱动的插件雷达与插件实验室。
...
- **标签：** `automation`, `directory`, `bilingual`
- **推荐理由：** GitHub Actions每日自动爬取双语插件目录。
...
- **标签：** `encyclopedia`, `resource`, `awesome-list`, `ai-knowledge`
- **推荐理由：** 全维度AI资源百科：Agent、RAG、MCP与学习路线一站收录。
...
- **推荐理由：** Complete reverse-skill (87 SKILL.md) as a DeepSeek Harness (dsh) Cordis plugin —
...
- **推荐理由：** 一个面向 DeepSeek Harness 的统一能力管理插件，为 Tools/Skills 提供常驻、按需、禁用三档暴露策略，以减少上下文占用并支持运行时动态
...
- **推荐理由：** 收录280+插件的分类索引，是探索DSH生态的最佳起点
...
- **推荐理由：** 覆盖 CLI、Web UI 和智能体工具的精选插件生态列表
...
- **标签：** `dsh-repository-plugin`, `plugin-verify`, `analytics`
- **推荐理由：** 带验证和分析功能的插件仓库。
...
- **标签：** `deprecated`, `redirect`, `awesome-list`
- **推荐理由：** 已迁移，请访问新版仓库获取最新插件。
