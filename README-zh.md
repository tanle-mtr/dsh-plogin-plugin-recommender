...
- **推荐理由：** Rust高性能智能体运行时结合精美桌面应用，开箱即用。
...
- **推荐理由：** 为纯文本模型提供强大视觉能力：图像问答、OCR、UI还原。
...
- **标签：** `coding-agent`, `rpc`, `mcp`, `go`
- **推荐理由：** Go语言编程Agent，支持RPC、子Agent、hashline编辑和MCP。
...
- **推荐理由：** 保护AI生成代码免受幻觉包和安全缺陷侵害的安全审计平台。
...
- **标签：** `provider`, `oauth`, `utility`
- **推荐理由：** 用订阅账号替代API Key，OAuth登录即可调用ChatGPT/Claude/Grok。
...
- **标签：** `provider`, `model-access`
- **推荐理由：** 集成Command Code模型提供商，支持实时编目和智能选模。
...
- **标签：** `multimodal`, `vision`, `claude-code`, `agent-skills`
- **推荐理由：** 为纯文本LLM增添多模态视觉能力，用于截图和UI分析。
...
- **标签：** `agent-platform`, `skills`, `typescript`
...
- **标签：** `plugin-creator`, `developer-tools`, `harness`
- **推荐理由：** 可复用的DSH/Cordis插件构建与验证工具
...
- **推荐理由：** 全栈Godot 4.x游戏开发技能，支持GDScript
...
- **标签：** `agent-skill`, `plugin-dev`, `toolkit`
- **推荐理由：** 开发DSH插件的Agent Skill，提供标准化工具链
...
- **标签：** `search`, `agent`, `discoverability`
- **推荐理由：** 在DSH agent内直接搜索npm和精选插件列表
...
- **标签：** `agent-tools`, `preflight`, `capability`
- **推荐理由：** 任务前自动预检Top-K适用插件，提升插件库利用率
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内与AI模型对话
...
- **标签：** `browser-extension`, `chatgpt`, `deepseek`
- **推荐理由：** 跨平台AI聊天界面增强与提示词管理器。
...
- **推荐理由：** 开放侧边栏底座，支持文件编辑、终端、Git和子代理页面。
...
- **标签：** `taskboard`, `cli`, `skills`
- **推荐理由：** 现代化灵活任务面板，可嵌入Codex和DSH工作流。
...
- **标签：** `tui`, `terminal`, `dsh-plugin`, `react`
- **推荐理由：** TUI插件，支持实时状态、流式思考和上下文进度显示。
...
- **推荐理由：** 仅5MB的Tauri桌面版，免环境配置，预装插件，跨平台支持。
...
- **推荐理由：** 零安装Windows桌面GUI，支持SSH、Codex和移动端远程访问。
...
- **标签：** `desktop-app`, `mcp`, `office-automation`
- **推荐理由：** 本地优先的DSH桌面工作区，支持项目、网页研究和Office输出。
...
- **推荐理由：** 在对话中内联渲染交互式UI组件，图表、表单、测验一步到位。
...
- **标签：** `office`, `desktop-extension`
- **推荐理由：** 将表格/文档/演示等办公能力深度集成进 Harness 桌面端。
...
- **推荐理由：** 本地优先的AI Agent桌面客户端，多模型、隐私安全、技能自进化。
...
- **推荐理由：** 原生iOS客户端，支持工作区同步和Agent执行轨迹实时查看。
...
- **推荐理由：** 跨平台桌面客户端，CodePilot风格，附带完整插件生态。
...
- **标签：** `cordis`, `deepseek-harness`, `dsh-plugin`, `skill-manager`
...
- **标签：** `mcp`, `skills`, `webui`
- **推荐理由：** Web界面skill/MCP管理工具，提升使用体验
...
- **推荐理由：** 生物信息学桌面应用，离线模式+技能商店，适合科研工作者
...
- **推荐理由：** 集成数据库、SSH、Docker和AI的一体化工具桌面
...
- **推荐理由：** Electron桌面壳集成DSH Web及侧边栏和ModLens
...
- **标签：** `ui`, `background`, `webui`
- **推荐理由：** 上传自定义背景，实时预览，支持细调效果
...
- **推荐理由：** 便携式本地优先Markdown记忆层，全Agent通用。
...
- **标签：** `memory`, `rag`, `reme`, `agent`
- **推荐理由：** 专为Agent设计的记忆管理工具包，支持RAG持久化上下文。
...
- **推荐理由：** 基于Milvus的持久化统一记忆层，支持多智能体共享记忆。
...
- **推荐理由：** 将SKILL.md文件转化为可检索的agent技能语料库，支持向量检索。
...
- **推荐理由：** 全维度AI知识库百科，涵盖Agent、RAG、MLOps和学习路线。
...
- **标签：** `memory`, `handoff`, `session`, `cross-platform`
- **推荐理由：** 跨平台会话记忆延续，支持Agent恢复
...
- **标签：** `agent-workspaces`, `multi-device`
...
- **标签：** `skills`, `skill-store`
- **推荐理由：** 最全技能商店，自动更新分类整理。
...
- **推荐理由：** 智能技能路由与工作流编排，SkillsBench上Token减少29.6%。
...
- **推荐理由：** 在DSH中启用多智能体团队协作，提升复杂任务处理效率。
...
- **推荐理由：** 结构化AI小说创作工具，整合灵感、角色、世界观管理。
...
- **标签：** `agent-skills`, `epistemic-loop`, `scientific-discovery`
- **推荐理由：** 将认识论循环框架引入DSH，适合严谨的科学发现工作流。
...
- **标签：** `agent-skills`, `creative-writing`
- **推荐理由：** 网文/短剧/游戏/视频解说全流程工作台，创作效率高。
...
- **推荐理由：** 视频/音频分析工具，给AI Agent提供可验证的多模态感知能力。
...
- **标签：** `task-management`, `agent-skills`
- **推荐理由：** 原生任务板，SQLite驱动，Agent认领/审查流程完整。
...
- **标签：** `mobile-automation`, `ios`, `agent-skills`
- **推荐理由：** 对话内运行iOS模拟器，22个工具实现iPhone应用自动化测试。
...
- **标签：** `agent-skills`, `deepseek-harness`, `dsh-plugin`, `math-modeling`
- **推荐理由：** 支持GitOps流水线的全栈数学建模竞赛Skill，国赛美赛通用。
...
- **标签：** `agent-review`, `ai-agents`, `deepseek-harness`, `prompt-engineering`
- **推荐理由：** 让Agent工作方式可组合、可审查、可持续改进。
...
- **标签：** `agent-skills`, `deep-research`, `marketing-automation`, `workflow`
- **推荐理由：** 88个跨平台Agent技能，覆盖研究、营销和商业流程。
...
- **标签：** `skills`, `coding`, `agent`, `multi-tool`
- **推荐理由：** 将经验转化为可在多个AI工具中复用的可执行技能。
...
- **标签：** `agent-skills`, `deepseek-harness`, `plugin-discovery`, `dsh-plugin`
- **推荐理由：** 自动搜索、安装和验证GitHub插件的DSH Skill。
...
- **推荐理由：** TDD、调试和规划技能，源自Superpowers
...
- **标签：** `dsh-ecosystem`, `plugin-marketplace`
- **推荐理由：** 社区维护的DSH插件目录，每4小时自动同步维护
...
- **标签：** `skills`, `github-issues`, `agent`
- **推荐理由：** 内置25个工程效率技能，预训练30B token，开箱即用
...
- **标签：** `gongwen`, `dsh-plugin`, `ai-docs`
- **推荐理由：** 国标公文全流程处理：格式检查、内容优化、模板生成
...
- **标签：** `legal-tech`, `agent-skills`, `diagram`
- **推荐理由：** 法律诉讼可视化工具：时间轴、对抗图、法律关系图
...
- **标签：** `math-modeling`, `competition`, `academic`
- **推荐理由：** 数学建模全能资源包，含数据预处理到论文写作
...
- **推荐理由：** 资深测试工程师技能框架，11项Skill+共享知识库
...
- **推荐理由：** 5角色多Agent数学建模，33篇模型库+进度可视化
...
- **推荐理由：** 152张镜头配方+Remotion模板，电影感视频生产
...
- **推荐理由：** 跨平台内容发现Agent，覆盖B站、抖音、YouTube等平台。
...
- **推荐理由：** 免费免注册的网页搜索桥接插件，让所有模型获得Web搜索能力。
...
- **推荐理由：** 持久浏览器身份+指纹伪装，支持无限本地Profile，适合自动化。
...
- **标签：** `llm-agent`, `deepseek-harness`, `preset`
...
- **标签：** `multi-model`, `quota-tracking`, `smart-routing`
...
- **推荐理由：** 代理工具的OpenRouter式注册表，支持API密钥管理。
...
- **推荐理由：** 带Schema和解析器的CLI插件管理器，DSH Hub必备工具。
...
- **标签：** `compatibility-layer`, `deepseek-harness`, `dsh-plugin`, `migration`
- **推荐理由：** 打通Pi与DSH生态，让Pi插件直接作为DSH插件运行。
...
- **标签：** `migration`, `deepseek-harness`, `dsh-plugin`, `import`
- **推荐理由：** 从25+AI编程工具导入对话历史到DSH，保持完整会话。
...
- **标签：** `backup`, `config-manager`, `migrate`
- **推荐理由：** 完整配置备份恢复迁移工具，支持WebDAV同步
...
- **标签：** `mcp`, `config`, `setup`, `multi-tool`
- **推荐理由：** 一键配置同步MCP服务器，支持6+ AI工具
...
- **标签：** `container`, `version`, `management`, `ui`
- **推荐理由：** 隔离容器中多版本DSH管理，含WebView UI
...
- **标签：** `migration`, `wizard`, `claude-code`, `import`
- **推荐理由：** 四源迁移向导，支持会话/记忆/技能/配置
...
- **标签：** `marketplace`, `utility`, `discovery`
- **推荐理由：** 类CodeX的插件市场，便于发现和管理插件
...
- **标签：** `theme`, `skin`, `decoration`
- **推荐理由：** 鲸鱼娘系列皮肤，为DSH界面增添可爱视觉定制。
...
- **推荐理由：** 实时海洋动态主题，可调节波浪与玻璃透明度，视觉效果出众。
...
- **标签：** `deepwiki`, `documentation`, `code-analysis`
- **推荐理由：** 将代码转化为结构化技术文档，适合人类团队和AI智能体。
...
- **标签：** `plugin-discovery`, `radar`, `ecosystem`
- **推荐理由：** 开源生态雷达，发现2万+插件并经k8s运行时验证。
...
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、GPU SSH和实验追踪。
...
- **推荐理由：** 每日自动筛选优质插件，场景化分类+热度排行，快速选型。
...
- **推荐理由：** 全面收录D SH生态资源，可视化导航，适合快速了解全貌。
...
- **推荐理由：** DeepSeek Harness终极入门指南，资源精选+实用工具一站收录。
...
- **推荐理由：** 跨库学术搜索技能+MCP，支持引文核验与知识图谱构建。
...
- **标签：** `finance`, `agent`, `mcp`, `tushare`, `research`
...
- **⭐ 星标：** 30
- **语言：** HTML
- **描述：** 面向 DeepSeek Harness 的交互式 K 线分析插件，支持多市场行情、技术指标、支撑压力位、新闻与���本面分析，并在原生侧栏中直接展示。
- **标签：** `kline`, `finance`, `analysis`, `sidebar`
- **推荐理由：** 侧栏交互式K线分析，含技术指标支撑压力
...
- **标签：** `radar`, `awesome-list`, `evidence`
- **推荐理由：** 基于证据的插件雷达，含2Origin实验室分析
...
- **推荐理由：** GitHub Actions每日自动爬取，最新插件目录
...
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin-market`, `marketplace`
- **推荐理由：** 最大DSH插件商店，11000+插件，支持搜索和API。
...
- **标签：** `deepseek-harness`, `awesome-list`, `mcp`, `developer-tools`
- **推荐理由：** 全面的手册，涵盖运行时、插件、MCP等173个主题。
...
- **标签：** `catalog`, `deepseek-harness`, `dsh-plugin`, `directory`
- **推荐理由：** 收录1819个真实插件的DSH插件目录，支持搜索筛选排序。
...
- **推荐理由：** 官方自动生成的插件目录，由cordis.run维护
...
- **推荐理由：** 精选280+插件目录，覆盖14大类，快速发现有用工具
...
- **标签：** `documentation`, `api`, `tencent`
- **推荐理由：** 腾讯官方SkillHub API文档和插件示例
...
- **标签：** `registry`, `plugins`, `skills`
- **推荐理由：** 开源插件注册表，带垃圾过滤和技能目录
...
- **推荐理由：** 覆盖CLI、Web UI和Agent的DSH生态精选
...
- **标签：** `repository`, `plugins`, `dsh-bundle`
- **推荐理由：** DSH插件仓库，支持发现与浏览功能
