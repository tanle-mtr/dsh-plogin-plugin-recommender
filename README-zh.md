...
- **标签：** `agent-orchestration`, `multi-platform`, `coding-agent`
- **推荐理由：** 跨设备统一Agent工作空间，支持桌面、移动端和API
...
- **标签：** `terminal`, `sandbox`, `vfs`
- **推荐理由：** 全球首款虚拟终端，为Agent提供隔离计算环境
...
- **标签：** `coding-agent`, `rpc`, `sub-agents`, `mcp`, `tui`
- **推荐理由：** 支持RPC、子Agent与MCP集成的编码智能体插件
...
- **标签：** `plugin-creator`, `coding`, `developer-tools`
- **推荐理由：** 构建和验证DSH插件的可复用工具
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `codex`
- **推荐理由：** Codex技能适配DSH简化功能，实用重构。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `migration`
- **推荐理由：** 四源迁移向导，支持Claude/Codex等。
...
- **标签：** `desktop`, `dsh`, `ui`
- **推荐理由：** DeepSeek Harness官方桌面客户端，原生用户体验
...
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** 功���丰富的TUI，含鲸鱼顶栏/实时状态/流式思考显示
...
- **标签：** `floating-widget`, `deepseek-harness`, `ui`
- **推荐理由：** 可爱的浮动鲸鱼小部件，实时显示账户余额。
...
- **推荐理由：** 零配置Windows桌面端，集成GUI、SSH与插件支持
...
- **推荐理由：** 本地优先桌面工作区，支持DSH会话与Office自动化
...
- **标签：** `canvas`, `session-explorer`, `non-linear`, `visual`
...
- **推荐理由：** 隐私优先的本地AI桌面应用，优秀的Claude Cowork替代方案
...
- **推荐理由：** 开源锤子便签复刻版，支持Docker私有化与DSH插件。
...
- **标签：** `desktop-app`, `electron`, `typescript`, `ui`
...
- **标签：** `desktop-app`, `bioinformatics`, `tauri`, `single-cell`
- **推荐理由：** 面向临床和实验科学家的生物信息学桌面应用，支持离线模式
...
- **⭐ 星标：** 32
- **语言：** TypeScript
- **描述：** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/��能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **标签：** `context`, `ui`, `audit`, `tool`
- **推荐理由：** 上下文注入审计与token成本可视化面板
...
- **推荐理由：** 集成数据库、SSH、SFTP、Docker和AI助手的一体化桌面中心。
...
- **标签：** `desktop`, `electron`, `dsh-web`
- **推荐理由：** Electron桌面版DSH，集成侧边栏、ModLens和市场。
...
- **⭐ 星标：** 5
- **语言：** JavaScript
- **描述：** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可��全移除；内置中英文双语界面。
- **标签：** `webui`, `dsh-plugin-market`
- **推荐理由：** 支持上传图片或预设切换Web背景
...
- **推荐理由：** 本地优先的Markdown记忆层，跨应用持久化Agent记忆
...
- **标签：** `agent-memory`, `continual-learning`, `stateful`
...
- **标签：** `memory`, `rag`, `agent`
- **推荐理由：** Agent记忆管理套件，支持持久化和精准上下文记忆
...
- **推荐理由：** 基于Milvus的统一持久化记忆层，适用于各类AI代理。
...
- **标签：** `vector-search`, `embeddings`, `skill-reranking`, `information-retrieval`
- **推荐理由：** 将零散技能文档转为可检索的Agent知识库
...
- **标签：** `knowledge-base`, `mcp`, `rag`, `ai-skills`
- **推荐理由：** AI全维度知识库百科，涵盖Agent、RAG与MLOps
...
- **标签：** `ai-writing`, `webnovel`, `creative-workflow`
- **推荐理由：** 长篇网文写作助手，解决AI遗忘与幻觉问题
...
- **标签：** `skills`, `marketplace`, `coding-agent`
- **推荐理由：** 综合技能商店，涵盖多领域精选技能包
...
- **标签：** `agent`, `workflow`, `harness`
- **推荐理由：** 长程Agent控制面，支持多工具体系的可治理工作流
...
- **⭐ 星标：** 396
- **语言：** Python
- **描述：** DeepSeek 写网文/小说的工作流插件：DeepSeek Harness 社区插件，内置小说、短剧、���戏、视频解说四个工作台 | Community DeepSeek Harness plugin with novel, short-drama, game and video-recap workbenches.
- **标签：** `creative-writing`, `novel-writing`, `screenwriting`, `workflow`
- **推荐理由：** 网文/短剧/游戏创作工作流，内置四个工作台
...
- **标签：** `agent-observability`, `computer-vision`, `video-understanding`
- **推荐理由：** 赋予AI视觉听觉感知能力，可验证的视频音频分析
...
- **标签：** `office`, `productivity`, `agent-skills`
- **推荐理由：** 将完整办公套件带入DSH，支持表格文档演示文稿操作
...
- **标签：** `epistemic-loop`, `scientific-discovery`, `agent-skill`
- **推荐理由：** 为智能体实现认知循环，支持结构化科学发现流程
...
- **标签：** `task-management`, `native-ui`, `productivity`
...
- **推荐理由：** 22个智能体工具，支持iOS模拟器与真机自动化操作
...
- **推荐理由：** AI编程导师，扫描项目架构并生成阶梯式章节指导编码
...
- **推荐理由：** 内置25个工程技能，300亿token训练，开发效率提升10倍
...
- **推荐理由：** You.com提供的网页搜索、内容提取和研究报告技能集
...
- **推荐理由：** DSH中统一加载和安全管理局部Agent技能的管家
...
- **标签：** `skill`, `docx`, `gongwen`, `wps-office`
...
- **⭐ 星标：** 43
- **语言：** TypeScript
- **描述：** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫��/六爻/梅花/奇门/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
...
- **推荐理由：** 官方SkillHub API文档与DSH示例，实用。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `agent-handoff`
- **推荐理由：** 跨平台Agent交接记忆，保持连续性。
...
- **标签：** `agent-skill`, `dsh-plugin`, `claude-code`
- **推荐理由：** 开发DSH插件的Agent Skill，提供插件/服务/工具标准。
...
- **推荐理由：** 免费小米MiMo TTS接入，支持多种声音朗读正文。
...
- **标签：** `mcp`, `finance`, `tushare`, `data-source`
- **推荐理由：** 基于tushare的本地财经数据库，自动暴露MCP接口。
...
- **标签：** `deepseek-harness`, `mcp`, `plexus`
- **推荐理由：** 一键本地MCP配置，支持多AI工具。
...
- **标签：** `diagrams`, `architecture`, `dsh-plugin`
- **推荐理由：** 自动生成精美架构与序列图，HTML可交互展示。
...
- **标签：** `image-prompts`, `gpt-image`, `dsh-plugin`
- **推荐理由：** 530+提示词案例与20套工业模板，含对比专区。
...
- **标签：** `design`, `prototyping`, `ppt`, `visual`
- **推荐理由：** AI驱动设计工作室，支持模板、原型和PPT。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `ai-video`, `directorx`
- **推荐理由：** AI视频/图像生成，支持分镜和文生视频。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `remotion`, `video`
- **推荐理由：** 电影感产品视频，152张镜头配方。
...
- **标签：** `api-keys`, `mcp`, `proxy`, `credentials`
- **推荐理由：** 代理工具的API密钥集中注册与代理服务。
...
- **标签：** `cli`, `plugin-manager`, `schemas`, `developer-tools`
- **推荐理由：** DSH插件市场管理CLI与解析工���
...
- **推荐理由：** 通过OAuth将订阅服务转为DSH模型提供商，无需API密钥
...
- **推荐理由：** 安全P2P多端远程访问方案，支持PC/安卓/Web。
...
- **标签：** `console`, `plugin-management`, `infrastructure`
- **推荐理由：** DSH插件生态基建：浏览器面板控制台与开发引导技能
...
- **标签：** `workbench`, `plugin-system`, `skills`, `typescript`
- **推荐理由：** 基于DSH的插件化Web工作平台，全局技能管理与工作区集成
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `dshbox`
- **推荐理由：** 容器管理多DSH版本，内嵌WebView UI。
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `launcher`
- **推荐理由：** 解锁DSH原生核心能力，统一编排。
...
- **推荐理由：** 实用DSH增强：任务通知、LLM追踪、快捷键和内置终端。
...
- **标签：** `debug`, `conflict-resolution`, `dsh-plugin`
- **推荐理由：** 检测并解决DSH插件版本冲突，保障生态稳定。
...
- **标签：** `skin`, `theme`, `deep-whale`
- **推荐理由：** 鲸鱼娘系列皮肤，美化DSH界面外观。
...
- **标签：** `theme`, `glass-effect`, `customizable`, `desktop`
- **推荐理由：** 高度可定制的磨砂玻璃主题，支持壁纸与模糊调节
...
- **标签：** `theme`, `animated`, `ocean-skin`
- **推荐理由：** 精美的动态海洋主题，支持波浪和玻璃效果调节
...
- **标签：** `ai-tutorials`, `resources`, `dsh-plugin`
- **推荐理由：** 程序员鱼皮的AI资源大全，涵盖主流大模型教程。
...
- **标签：** `arxiv`, `latex`, `research-tools`, `gpu-cluster`, `experiment-tracking`
- **推荐理由：** 全流程科研工具：LaTeX编译、arXiv管理与GPU调度
...
- **标签：** `awesome-list`, `curated`, `resource-guide`
- **推荐理由：** 全面的DSH资源精选列表，涵盖插件MCP和技能
...
- **标签：** `awesome-list`, `guide`, `chinese`
- **推荐理由：** 中文DSH终极指南，含快速入门和精选资源
...
- **标签：** `academic`, `mcp`, `citation`
- **推荐理由：** 跨数据库学术搜索，支持引文图谱和中文科研
...
- **标签：** `agent-runtime`, `mcp`, `handbook`
- **推荐理由：** 覆盖运行时、插件、MCP的完整Agent手册。
...
- **⭐ 星标：** 61
- **语言：** Python
- **描述：** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：��凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（大事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **标签：** `visualization`, `legal`, `diagram`, `svg`
- **推荐理由：** 诉讼可视化工具集：时间轴、大事记表和法律文书图表
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `kline`, `ftshare`
...
- **标签：** `no-code`, `crm`, `dsh-plugin`
- **推荐理由：** 开源AI+低代码平台，快速搭建业务系统的利器。
...
- **标签：** `dsh-web`, `plugin-ecosystem`, `aggregation`
- **推荐理由：** 插件聚合生态，通过创意工坊分发万物皆插件
...
- **标签：** `marketplace`, `plugin-shop`, `review`
- **推荐理由：** 每日刷新、预审核的综合插件商城。
...
- **标签：** `awesome-list`, `plugins`, `resource`
- **推荐理由：** 收录280+社区插件的精选目录索引
...
- **标签：** `deepseek-harness`, `awesome-list`
- **推荐理由：** 精选DSH插件/工具/技能清单，万物皆插件。
...
- **推荐理由：** 带垃圾过滤的开放插件注册表，方便发现DSH生态工具。
...
- **推荐理由：** 中文实用的DeepSeek Harness插件合集。
...
- **标签：** `awesome-list`, `plugin-ecosystem`, `cli`
- **推荐理由：** 涵盖CLI、Web UI和自主智能体的DSH插件生态列表。
...
- **推荐理由：** 收录80+个经过验证的DSH插件目录，便于快速发现
...
- **标签：** `dsh-plugins`, `dsh-repository-plugin`, `dsh-plugin-verify`
- **推荐理由：** 含bundle、verify及分析功能的DSH插件仓库
...
- **标签：** `awesome-list`, `moved`, `dsh-plugins`
- **推荐理由：** 已归档，已重定向到新仓库，建议查看更新
...
- **标签：** `awesome-list`, `auto-updating`, `dsh-plugins`
- **推荐理由：** 自动更新的双语插件列表，README自动生成
...
- **推荐理由：** 覆盖模型、工具、沙箱和UI扩展的 curated 插件注册表
