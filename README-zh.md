...
- **推荐理由：** DeepSeek原生终端编码代理，前缀缓存稳定支持长时运行。
...
- **标签：** `routing`, `agent-orchestration`
...
- **标签：** `multi-agent`, `enterprise`
- **推荐理由：** 企业级多Agent工作台，支持多引擎统一协作
...
- **标签：** `jailbreak`, `harness-tools`
- **推荐理由：** 全模型破甲提示词，支持按模型自定义切换
...
- **标签：** `deepseek-harness`, `dsh-plugin`, `security`
- **推荐理由：** 逆向工程与安全授权的87项专业Skill
...
- **⭐ 星标：** 78
- **语言：** TypeScript
- **描述：** ProMentor ��一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描项目架构、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `mentor`, `coding`, `education`
- **推荐理由：** AI编程导师，扫描架构、引导手写、自动评测
...
- **标签：** `plugin-development`, `codex`, `tooling`
- **推荐理由：** 构建和验证DSH/Cordis插件的可复用Codex技能。
...
- **推荐理由：** OAuth绑定ChatGPT账户，DSH内直接使用。
...
- **标签：** `web-ui`, `plugin-marketplace`
- **推荐理由：** Web端插件聚合���态，支持创意工坊分发
...
- **标签：** `tui`, `ui`, `status-bar`
- **推荐理由：** TUI状态栏插件，实时显示思考流与上下文进度
...
- **标签：** `floating-widget`, `deepseek-harness`
- **推荐理由：** 可爱鲸鱼挂件，实时显示余额，支持拖拽动画
...
- **标签：** `desktop`, `gui`, `ssh`, `codex`
- **推荐理由：** 零配置桌面客户端，集成SSH、Codex、技能与移动端远程访问。
...
- **标签：** `desktop`, `local-first`, `mcp`, `ui`
- **推荐理由：** 本地��先的AI桌面工作区，支持会话、项目与网页研究。
...
- **标签：** `ui-components`, `charts`, `interactive`, `genui`
- **推荐理由：** 在回复中直接渲染图表、表单等交互式UI组件。
...
- **标签：** `canvas`, `visual`, `conversation`, `workspace`
- **推荐理由：** 可视化画布式会话探索器，支持非线性分支对话。
...
- **推荐理由：** 本地优先的多模型桌面AI助手，隐私优先且技能可自进化
...
- **推荐理由：** 原生iOS客户端，将DSH工作区和Agent轨迹带到iPhone
...
- **推荐理由：** 原生SQLite任务看板，Agent认领与审核纯Web UI
...
- **标签：** `desktop`, `workspace`, `plugin-system`
- **推荐理由：** 基于DSH生态的WorkBuddy风格桌面工作空间
...
- **标签：** `desktop`, `electron`, `hub`
- **推荐理由：** Electron桌面控制台，统一管理各类插件
...
- **推荐理由：** 基于Tauri的生物信息学桌面应用，面向临床和科研。
...
- **标签：** `ui`, `stats`, `usage-tracking`
- **推荐理由：** 在作曲家下方显示提供商、模型和价格信息的UI扩展。
...
- **推荐理由：** WorkBuddy风格技能选择器，内嵌DSH编辑器
...
- **推荐理由：** 本地优先的可移植记忆层，Markdown原生跨Agent持久化
...
- **标签：** `docs`, `rag`, `deepwiki`
- **推荐理由：** 自动生成技术文档与AI就绪上下文，结构化清晰
...
- **推荐理由：** 持久统一记忆层，Milvus向量搜索驱动多Agent
...
- **标签：** `embedding`, `vector-search`, `skill-md`, `retrieval`
- **推荐理由：** 将SKILL.md文件转化为可检索的Agent技能知识库。
...
- **标签：** `deepseek-harness`, `agent`
- **推荐理由：** 基于tushare的本地金融数据库与多Agent分析
...
- **标签：** `skill-distillation`, `meta-skill`, `agent-skills`
...
- **标签：** `ai-writing`, `webnovel`
- **推荐理由：** 网文长篇小说写作助手，解决AI遗忘与幻觉问题
...
- **标签：** `novel-writing`, `creative`, `workflows`, `drama`
- **推荐理由：** 内置网文、短剧、游戏脚本、视频解说四大工作台。
...
- **标签：** `deepseek-harness-plugin`, `office`, `dsh`
- **推荐理由：** 为DSH添加完整办公套件功能，支持电子表格、文档和幻灯片
...
- **⭐ 星标：** 259
- **语言：** Python
- **描述：** 全栈式自动数学建模竞赛Skill，支持 Claude Code / Codex / opencode / DeepSeek Harness。AI 担任 Autopilot，人类担任 Copilot；国赛/美赛通用，GitOps 流水线 + 强制代码自��，可选 addons 模块。
...
- **标签：** `agent-skills`, `deepseek-harness`, `marketplace`
- **推荐理由：** 88个开源Agent技能，覆盖研究与商务
...
- **推荐理由：** 为Claude Code添加多模态视觉能力，支持截图与UI分析
...
- **标签：** `skills`, `github-issues`, `productivity`
- **推荐理由：** 25个预装技能，10倍开发效率，GitHub支持
...
- **标签：** `document`, `government`, `workflow`
- **推荐理由：** 国标公文处理全流程工具
...
- **标签：** `bazi`, `fengshui`, `tcm`, `skill`
- **推荐理由：** 独特的八字/风水/中医知识技能包，本地确定性引擎。
...
- **标签：** `math-modeling`, `datascience`, `skill`
- **推荐理由：** 数学建模全流程技能包：数据处理、绘图、论文写作。
...
- **标签：** `spec-driven`, `ai-coding`, `skill`
- **推荐理由：** 受DSH工程模式启发的规格驱动开发技能。
...
- **⭐ 星标：** 36
- **语言：** Python
- **描述：** UltraMath 数学建模竞赛多 Agent 求解 DSH 插件: 5 角色预设 + 33 篇模型库 + 论文模板/审稿脚��随包 + 进度可视化
- **标签：** `math-modeling`, `multi-agent`, `competition`
- **推荐理由：** 5角色多Agent数学建模，含33个模型模板和论文。
...
- **推荐理由：** 11个QA技能+共享知识库，专为AI Agent设计
...
- **标签：** `ai-video`, `ai-image`, `skills`
- **推荐理由：** AI视频/图像/音频生成技能与视觉工具集成
...
- **标签：** `engineering-workflow`, `task-management`, `workflow`
- **推荐理由：** 工程工作流插件，支持任务阶段管理与验证。
...
- **标签：** `mcp`, `skills`, `webui`
- **推荐理由：** Web界面Skill与MCP管理工具，提升配置效率
...
- **推荐理由：** 530+ GPT图像提示词，含工业级模板和可复用技能库。
...
- **标签：** `agent`, `design`, `vibedesign`, `mcp`
- **推荐理由：** AI原生矢量设计工具，支持Agent协作与代码化设计
...
- **标签：** `3d-modeling`, `blender`, `design`
- **推荐理由：** AI直连Blender，驱动视口与渲染流程
...
- **标签：** `ocr`, `vision`, `image-processing`
- **推荐理由：** 为纯文本模型提供像素级图像识别和OCR能力。
...
- **推荐理由：** 支持多平台AI的浏览器扩展，含跨站点提示词管理功能
...
- **标签：** `content-discovery`, `browser`, `multi-platform`
...
- **标签：** `web-search`, `free`, `browser`, `bridge`
...
- **标签：** `anti-detect`, `playwright`, `browser-automation`, `mcp`
- **推荐理由：** 指纹欺骗浏览器，支持持久身份与安卓模拟。
...
- **推荐理由：** 支持60+平台的终极图床工具，兼容Obsidian和VS Code。
...
- **标签：** `no-code`, `ai-agent`, `low-code`
- **推荐理由：** AI驱动的无代码平台，快速构建业务系统，高度可扩展。
...
- **标签：** `proxy`, `api-keys`, `mcp`, `registry`
- **推荐理由：** 代理工具与API密钥的统一注册与代理管理平台
...
- **标签：** `security`, `vulnerability`, `redteam`, `code-audit`
- **推荐理由：** 审计AI生成代码的安全缺陷与幻觉包问题。
...
- **推荐理由：** 通过OAuth将ChatGPT、Claude、Grok订阅用作DSH LLM提供商
...
- **标签：** `llm-provider`, `command-code`
- **推荐理由：** 添加Command Code模型访问，支持实时目录和智能选择
...
- **标签：** `ai-agents`, `dsh-plugin`, `free-model`
- **推荐理由：** 免费使用前沿模型，无需API Key注册
...
- **标签：** `migration`, `automation`, `dsh-plugin`
- **推荐理由：** 从25+AI工具导入对话历史为可恢复会话
...
- **标签：** `upgrade`, `utility`, `maintenance`
- **推荐理由：** 帮助插件随DSH版本自动升级的实用工具
...
- **推荐理由：** 一键同步MCP/Skills配置，支持7+ AI工具
...
- **标签：** `containers`, `management`, `ui`
- **推荐理由：** 容器化DSH多版本管理，内嵌WebView界面
...
- **标签：** `migration`, `tool`, `session-import`
- **推荐理由：** 从4款AI编码工具迁移会话、技能和配置
...
- **推荐理由：** AI资源大全，涵盖Agent、RAG、提示词和Vibe Coding教程。
...
- **标签：** `academic`, `latex`, `arxiv`, `gpu-cluster`
- **推荐理由：** 一站式科研工作台：LaTeX编译、arXiv管理、GPU追踪。
...
- **推荐理由：** 跨库学术搜索，支持引文图谱和MeSH，适合科研人员
...
- **标签：** `awesome-list`, `deepseek-harness`, `agent-runtime`
- **推荐理由：** 173资源覆盖运行时、插件、MCP的权威手册
...
- **标签：** `stock`, `quantitative`, `analysis`
- **推荐理由：** 39个工具+20策略技能，覆盖多市场行情分析
...
- **标签：** `awesome-list`, `meta-resource`
- **推荐理由：** DSH插件精选聚合列表，是发现和参考的最佳资源
...
- **标签：** `plugin-shop`, `marketplace`
- **推荐理由：** 每日更新的全方位插件市场，发布前预审核
...
- **标签：** `awesome-list`, `curated`, `bilingual`, `extensions`
- **推荐理由：** 双语精选DSH插件大全，适合探索与导航。
...
- **标签：** `encyclopedia`, `knowledge-base`, `awesome-list`, `ai`
- **推荐理由：** 全维度AI百科，涵盖LLM、Agent、RAG及学习路线。
...
- **标签：** `awesome-list`, `official`, `cordis`
...
- **标签：** `marketplace`, `ecosystem`, `community`
- **推荐理由：** 社区插件聚合市场，4小时自动同步维护
...
- **标签：** `awesome-dsh-plugin`, `dsh`, `dsh-plugin`, `dsh-plugin-market`, `xiaomi-mimo`, `xiaomi-mimo-tts`
- **推荐理由：** 给DSH接入免费的 Xiaomi MiMo TTS API，支持使用预置/自定义
...
- **标签：** `deepseek`, `deepseek-harness`, `dsh-plugin`, `dsh-plugins`, `dsh-web`, `enhancement`, `keybindings`, `terminal`, `trace`
- **推荐理由：** A curated collection of awesome plugins 
...
- **标签：** `dsh-plugins`, `dsh-ecosystem`, `dsh-bundle`
