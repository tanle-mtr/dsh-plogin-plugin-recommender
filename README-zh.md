...
| 插件 | 星标 | 简介 |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 201,576 | Core framework enabling everything-as-plugin archi |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 92,294 | Open-source design system with local-first desktop |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 69,609 | Multi-agent swarm orchestrator with autonomous wor |
...
- [Agent 技能与工作流](#agent-技能与工作流) — 31 个插件
...
- [Agent 技能与工作流](agent-skills-workflows-zh) (31)
...
- **标签：** `agent-runtime`, `ai-ide`, `vibe-coding`
- **推荐理由：** 高性能Rust Agent运行时配合精美桌面IDE。
...
- **推荐理由：** 基于经典工程书籍的AI代码审查，提供可操作的修复建议
...
- **标签：** `agent`, `desktop`, `mcp`, `privacy`, `typescript`
- **推荐理由：** 本地优先AI Agent桌面应用，多模型自进化技能
...
- **标签：** `dsh-plugin`, `ai-agent`, `coding`
- **推荐理由：** AI编程导师，扫描架构、生成章节、代码审查一气呵成。
...
- **标签：** `dsh-plugin`, `agent-skill`, `development`
...
- **标签：** `agent-config`, `mcp`, `multi-agent`, `dsh-plugin`
- **推荐理由：** 一键配置Claude/Codex/DSH等多Agent工具。
...
- **标签：** `codex`, `sync`, `mcp`, `dsh-plugin`
- **推荐理由：** OpenAI Codex与DSH双向同步桥梁工具。
...
- **标签：** `coding`, `codex`, `simplification`
- **推荐理由：** 适配DeepSeek Harness的代码简化技能，适合Codex工作流
...
- **标签：** `plugin-creator`, `tooling`, `codex`
...
- **标签：** `package-manager`, `agent-framework`, `multi-tool`
- **推荐理由：** 包管理器含42个技能和5个CLI，覆盖评估驱动开发、数据库、K8s等
...
- **推荐理由：** 一键迁移Claude Code/Codex/OpenCode配置至DSH。
...
- **标签：** `plugin-development`, `cordis`, `agent-skills`
- **推荐理由：** 便携技能，用于开发审计DSH插件。
...
- **标签：** `agent-tools`, `plugin-discovery`, `pre-flight`
- **推荐理由：** Agent任务前自动预检并推荐适用插件。
...
- **标签：** `dsh-plugin`, `sidebar`, `deepseek`
- **推荐理由：** 可扩展侧边栏底座，内置文件/终端/Git面板。
...
- **推荐理由：** Claude Code风格TUI，鲸鱼顶栏实时状态显示。
...
- **推荐理由：** 可爱的浮动鲸鱼组件，实时监控DeepSeek账户余额
...
- **标签：** `ui`, `interactive`, `dsh-plugin`, `charts`
- **推荐理由：** 在对话中内联渲染交互式UI组件，图表表单均可
...
- **标签：** `desktop`, `electron`, `codepilot`, `typescript`
- **推荐理由：** 跨平台代码飞行员桌面客户端，Electron多系统支持
...
- **推荐理由：** 零配置Windows桌面客户端，集成SSH、插件和移动远程访问。
...
- **推荐理由：** 将完整办公套件（表格/文档/演示）引入DSH桌面体验。
...
- **推荐理由：** 原生SQLite任务看板，支持Agent认领审核与纯净Web UI。
...
- **推荐理由：** Electron桌面控制台，多Tab统一管理Harness和插件。
...
- **标签：** `context`, `ui`, `audit`
- **推荐理由：** 审计上下文token成本，检测指令冲突，带Web UI面板
...
- **标签：** `rag`, `knowledge-base`, `wiki`, `llm`
- **推荐理由：** 开源知识平台，文档转RAG与自维护Wiki。
...
- **标签：** `memory`, `rag`, `persistent-context`
- **推荐理由：** 为AI助手提供无限持久化记忆存储能力
...
- **标签：** `memory`, `agent`, `local-first`
- **推荐理由：** 本地记忆中枢，为所有AI代理提供共享持久上下文
...
- **标签：** `skill-corpus`, `rag`, `embeddings`, `semantic-search`
- **推荐理由：** 将SKILL.md文件转为可检索的Agent技能语料库
...
- **标签：** `workflow`, `canvas`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 可视化非线性对话画布，支持分支工作区探索
...
- **推荐理由：** 将AI经验转化为可重复执行的技能，跨编码Agent通用。
...
- **推荐理由：** TDD、调试和规划等超级技能适配DSH Agent。
...
- **推荐理由：** Superpowers编码技能移植至Reasonix框架。
...
- **标签：** `agent-skills`, `research`, `marketing-automation`, `deep-research`
- **推荐理由：** 88个开源Agent技能，覆盖研究、营销与业务流程。
...
- **标签：** `agent-skills`, `spec-programming`, `dsh-plugin`
- **推荐理由：** 支持规格驱动开发的便携Agent技能。
...
- **标签：** `agent-skill`, `chinese-traditional-wisdom`, `dsh-plugin`
- **推荐理由：** 融合八字风水易经的中华传统智慧技能包。
...
- **标签：** `agent`, `skills`, `task-board`, `dsh-plugin`
- **推荐理由：** 将skills化为可视任务板，派单式管理。
...
- **标签：** `agent-handoff`, `session-memory`, `dsh-plugin`
- **推荐理由：** 跨平台Agent会话状态记忆与交接技能。
...
- **推荐理由：** DSH Composer中类似WorkBuddy的技能选择器。
...
- **标签：** `skills-manager`, `dsh-plugin`, `agent`
- **推荐理由：** 统一加载管理DSH本地Agent技能。
...
- **标签：** `multi-agent`, `team`, `dsh-plugin`
- **推荐理由：** 多Agent团队协作，共享工作区与工具。
...
- **标签：** `docx`, `gongwen`, `chinese`
- **推荐理由：** 基于国标的中文公文全流程处理工具，支持格式检查和模板生成
...
- **标签：** `governance`, `swift-cycle`, `workflow`
- **推荐理由：** Swift Cycle治理技能，版本锁定且可离线验证
...
- **推荐理由：** AI视频/图像/音频全流程，含故事板、知识语料和可配置模型工具
...
- **标签：** `agent-skills`, `engineering`, `workflows`
- **推荐理由：** 从DSH实践提炼的工程纪律技能。
...
- **标签：** `diagram-as-code`, `architecture`, `mermaid`
- **推荐理由：** Agent技能生成精美动画架构图与流程图。
...
- **标签：** `image-generation`, `prompt-engineering`, `workflow`
- **推荐理由：** 530+逆向工程提示词，20+工业级模板。
...
- **标签：** `video-generation`, `ffmpeg`, `ai-video`
- **推荐理由：** 一键生成Vox风格拼贴解说视频。
...
- **推荐理由：** AI生成的设计系统，含可视化编辑器与PPT制作工具
...
- **标签：** `image-generation`, `visual`, `dsh-plugin`
- **推荐理由：** 直接在DSH对话中生成图片，支持多模型
...
- **标签：** `photo-editing`, `image-processing`, `editorial-design`
- **推荐理由：** 严格保真度的照片转杂志风格合成，创意控制丰富。
...
- **标签：** `design`, `aesthetics`, `vibe-coding`
- **推荐理由：** 6种设计风格预设，让AI生成的网站远离通用AI审美
...
- **标签：** `ios`, `mobile-automation`, `webdriveragent`, `typescript`
- **推荐理由：** 在对话内嵌入iOS模拟器，22个移动自动化Agent工具
...
- **标签：** `chat-bot`, `dingtalk`
- **推荐理由：** 钉钉IM通道，聊天即Agent工具调用
...
- **标签：** `no-code`, `crm`, `erp`, `internal-tools`
...
- **标签：** `dsh-plugin`, `deepseek`, `claude-code`
- **推荐理由：** 一键安装并切换主流编码CLI工具。
...
- **标签：** `dsh-plugin`, `marketplace`, `deepseek-harness`
- **推荐理由：** 内置可视化插件市场，支持浏览和一键安装。
...
- **标签：** `vision`, `ocr`, `utility`
- **推荐理由：** 纯文本模型的视觉工具箱：OCR、图片问答与截图分析
...
- **标签：** `oauth`, `subscription`, `infrastructure`, `typescript`
- **推荐理由：** 用订阅账号作DSH模型提供商，免API Key配置
...
- **标签：** `marketplace`, `plugin-marketplace`, `nodejs`
- **推荐理由：** 一键浏览、安装和更新所有DSH社区插件。
...
- **标签：** `market`, `webui`, `plugin-market`
- **推荐理由：** 社区插件市场浏览和一键安装到Web GUI。
...
- **标签：** `dsh-plugin`, `plugin-management`, `build`
- **推荐理由：** 构建和安装DSH插件的Agent技能，支持槽位配置。
...
- **标签：** `backup`, `config-manager`, `dsh-plugin`
- **推荐理由：** DSH配置完整备份恢复，支持同步迁移。
...
- **标签：** `dsh-plugin`, `container`, `plugin-management`
- **推荐理由：** 隔离容器中运行多个DSH版本，管理便捷。
...
- **⭐ 星标：** 6
- **语言：** TypeScript
- **描述：** 致力于��决各插件之间也与版本之间可能存在的冲突问题并提供解决方案
...
- **标签：** `theme`, `ui`, `glass-effect`, `dsh-plugin`
- **推荐理由：** 磨砂玻璃主题，支持高度自定义，无需修改源码
...
- **标签：** `awesome-list`, `dsh-plugin`, `bilingual`
- **推荐理由：** 全面的DSH插件精选目录，中英双语资源一览
...
- **标签：** `security`, `cybersecurity`, `redteam`, `dsh-plugin`
- **推荐理由：** AI安全攻防平台，审计幻觉包和漏洞防护
...
- **标签：** `ai-pedia`, `knowledge-base`, `dsh-plugin`
- **推荐理由：** 一站式AI百科，覆盖技能、Agent、RAG等全维度资源
...
- **标签：** `tutorial`, `dsh-plugin`, `marketplace`
...
- **推荐理由：** DSH终极指南，含入门教程、精选插件和实用工具资源。
...
- **标签：** `awesome-list`, `mcp`, `curated`
- **推荐理由：** DSH插件、MCP服务器、补丁和编排器的精选合集。
...
- **推荐理由：** 插件市场含11000+插件，支持搜索、排名和免费API。
...
- **标签：** `kline`, `financial`, `analysis`, `dsh-plugin`
- **推荐理由：** 交互式K线分析，支持技术指标与基本面。
...
- **推荐理由：** 基于证据的提示词、RAG和技能评估框架，支持原生DSH集成
...
- **标签：** `research`, `industry`, `company`
- **推荐理由：** 行业研究技能包，含方法论、产业链映射和新闻追踪能力
...
- **标签：** `awesome-list`, `resources`, `mcp`
- **推荐理由：** 全面的DSH生态插件/工具/技能综合目录。
...
- **标签：** `awesome-list`, `bilingual`, `verified`
- **推荐理由：** 证据导向的双语插件目录，不可变提交审核。
...
- **标签：** `awesome-list`, `auto-update`
- **推荐理由：** YAML数据驱动的自动更新插件列表
...
- **标签：** `curated-list`, `plugin-directory`
- **推荐理由：** 覆盖模型、工具、沙箱的插件目录
...
- **⭐ 星标：** 1,424
- **语言：** Python
- **描述：** DSH 插件雷达与精选榜：多路���动发现 14900+ 候选，容器真实安装路径运行级实测（四档判定），精选 Top 50 · 11 类人工策展，全量索引 PLUGINS-ALL.md，自动更新。
- **标签：** `awesome-list`, `plugin-registry`, `curated`
- **推荐理由：** 精选插件雷达，经真实运行测试并收录Top 50优质插件
...
- **标签：** `awesome-list`, `plugin-registry`, `curated`
- **推荐理由：** 全面的DeepSeek Harness生态插件精选清单
...
- **标签：** `awesome-list`, `plugin-registry`, `index`
- **推荐理由：** 来自DSH生态枢纽的精选插件与工具索引目录
...
- **标签：** `prompt`, `jailbreak`, `specialized`
- **推荐理由：** DeepSeek专用破甲提示词插件，稳定输出优化
...
- **标签：** `plugins`, `marketplace`, `collection`
- **推荐理由：** DeepSeek Harness插件集合与市场中心。
...
- **⭐ 星标：** 75
- **语言：** Python
- **描述：** 🐳 DeepSeek Harness 插件聚合��区 — 自动同步 dsh-plugin 生态 · 精选目录 · 每 4 小时自动维护 | Oh-My-DSH: a community-maintained catalog of DeepSeek Harness plugins, auto-synced from the dsh-plugin topic
- **标签：** `awesome-list`, `plugin-marketplace`, `dsh-ecosystem`
- **推荐理由：** 社区维护的DSH插件聚合商城，每4小时自动同步。
...
- **推荐理由：** DSH内测收官合影墙，GitHub OAuth登录+拍立得风格
...
- **标签：** `awesome-list`, `curated-list`, `directory`
- **推荐理由：** 精选的DSH插件中文/英文列表。
...
- **标签：** `registry`, `open-data`, `plugin-registry`
- **推荐理由：** 去垃圾的插件/技能开放数据注册表。
...
- **标签：** `marketplace`, `plugin-market`, `skills`
...
- **推荐理由：** OAuth绑定ChatGPT账号，在DSH内使用Plus模型。
