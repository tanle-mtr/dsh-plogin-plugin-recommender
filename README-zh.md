...
- **标签：** `coding-agent`, `r1`, `tui`, `dsh-plugin`
...
- **标签：** `no-code`, `ai-agent`, `self-hosted`, `dsh-plugin`
...
- **标签：** `routing`, `harness-tool`, `injector`
- **推荐理由：** DSH任务感知路由器和运行时注入器。
...
- **推荐理由：** 在DeepSeek Harness中实现多智能体团队协作。
...
- **推荐理由：** 支持RPC插件、子代理和MCP的代码Agent CLI，适合终端工作流
...
- **推荐理由：** DSH插件市场CLI工具，含schema解析器和Agent工具。
...
- **推荐理由：** Command Code模型接入，智能选型推理优化
...
- **标签：** `infrastructure`, `devops`, `skills`
- **推荐理由：** AI基础设施开发专用技能，性能优化调试
...
- **推荐理由：** Superpowers技能适配Reasonix编码工
...
- **标签：** `skills-manager`, `harness-tool`
...
- **推荐理由：** 复用型插件创建工具，支持DSH和Cordis插件开发与验证。
...
- **标签：** `plugin-dev`, `skill`, `dsh-plugins`
- **推荐理由：** 指导AI Agent从零开发测试安装DSH插件。
...
- **推荐理由：** 通过OAuth绑定ChatGPT账号，在DSH内与模型对话
...
- **标签：** `desktop`, `dsh`, `dsh-desktop`, `tauri`
- **推荐理由：** 5MB零配置桌面版，跨平台预装插件开箱即用。
...
- **推荐理由：** 本地优先AI桌面工作区，整合DSH会话、文件、网页研究和Office工具
...
- **推荐理由：** Windows桌面GUI客户端，内置Codex、插件、技能、SSH和移动远程访问
...
- **标签：** `dsh-plugin`, `canvas`, `conversation`, `workspace`
- **推荐理由：** 基于画布的非线性对话工作区，支持会话探索和多分支导航
...
- **推荐理由：** 本地优先多模型Tauri桌面应用，支持自进化技能和多Harness路由
...
- **标签：** `office`, `desktop`, `plugin`
- **推荐理由：** 将完整办公套件（表格、文档、幻灯片）集成到DSH桌面。
...
- **标签：** `desktop`, `electron`, `macos`, `windows`, `linux`
- **推荐理由：** 跨平台桌面客户端，仿CodePilot设计，插件丰富。
...
- **标签：** `ios`, `mobile`, `kotlin-multiplatform`, `native`
- **推荐理由：** 原生iOS客户端，WebSocket连接DSH工作区与Agent轨迹。
...
- **标签：** `notes`, `productivity`, `ui`
- **推荐理由：** 开源锤子便签，Docker一键私有化部署
...
- **标签：** `ui`, `mcp`, `skills`, `web`
- **推荐理由：** Web界面可视化Skill和MCP管理工具
...
- **推荐理由：** Electron桌面管理控制台，多Tab管理插件、MCP和技能。
...
- **推荐理由：** 集成SSH/Docker/DB的DevOps桌面客户端，AI沙箱一体。
...
- **推荐理由：** Electron打包的DSH桌面应用，集成侧边栏工具。
...
- **标签：** `theming`, `customization`, `webui`
- **推荐理由：** 可自定义Web背景，支持预设和实时预览。
...
- **标签：** `memory`, `stateful-agent`, `continual-learning`
...
- **推荐理由：** 持久化统一记忆层，由Milvus和Markdown驱动。
...
- **推荐理由：** 将散乱的SKILL.md文件转化���检索就绪的agent技能库，附带评估工具
...
- **标签：** `workspace`, `task-tracking`, `multi-device`
...
- **标签：** `skill-store`, `agent-skills`, `automation`
...
- **标签：** `ai-video`, `agent-skill`, `dsh-plugin`, `ffmpeg`
- **推荐理由：** 全自动生成Vox风格纸拼贴解说视频，一站式体验。
...
- **标签：** `ai-writing`, `novel`, `dsh-plugin`, `rag`
- **推荐理由：** 有组��地AI小说创作，整合角色、世界观、写作与审稿流程。
...
- **标签：** `multimodal`, `ocr`, `video-analysis`, `agent-skills`
- **推荐理由：** 将视频音频屏幕活动转化为可搜索的时间戳证据，支持OCR和语音转文字
...
- **标签：** `creative-writing`, `novel-writing`, `drama-skills`, `agent-skills`
- **推荐理由：** 基于Oh Story和Drama Skills的小说创作与短剧生成专用插件
...
- **推荐理由：** 原生SQLite任务管理，支持Agent认领与审核工作流。
...
- **推荐理由：** 全栈式自动数学建模竞赛Skill，GitOps流水线支持。
...
- **⭐ 星标：** 206
- **语言：** TypeScript
- **描述：** 让 Agent 的工作方式可组合、可审查、可持续��进，最终实现 Agent Self Evoling。 DeepSeek Harness Web plugin with composable task controls and isolated, human-reviewed self-evolution.
- **标签：** `agent`, `evolution`, `workflows`
- **推荐理由：** Agent自我进化工作流，可组合可审查
...
- **标签：** `skills`, `agent`, `experience`
- **推荐理由：** 经验转化为可复用执行模式，技能标准化
...
- **标签：** `skills`, `research`, `business`
- **推荐理由：** 88个开源Agent技能，研究营销业务全覆盖
...
- **推荐理由：** 完整逆向工程和渗透测试技能包，含87个SKILL.md
...
- **标签：** `skills`, `deck`, `agent-skills`
- **推荐理由：** 25个工程技能开箱即用，侧边栏直接调用无需手动安装。
...
- **推荐理由：** 党政公文全流程处理：格式检查、模板生成、Markdown转换。
...
- **标签：** `spec-driven`, `vibe-coding`, `agent-skill`
- **推荐理由：** 可移植的规格驱动开发技能，支持仓库原生编程工作流。
...
- **标签：** `bazi`, `fengshui`, `agent`
- **推荐理由：** 中华传统智慧AI技能包，本地确定性引擎
...
- **⭐ 星标：** 29
- **语言：** Python
- **描述：** DeepSeek自带破��18+互动叙事 Skill：NPC 活人感（有记忆/立场/底线）、数百项素材库、随机开局可预锁、时间推进、全维 YAML 存档。
- **标签：** `narrative`, `18+`
- **推荐理由：** 18+互动叙事NPC记忆系统
...
- **标签：** `diagrams-as-code`, `mermaid-alternative`
- **推荐理由：** 从代码生成可验证的架构图与序列图，支持动效。
...
- **推荐理由：** 原生设计工作室，AI生成+可视化编辑+模板市场+PPT。
...
- **推荐理由：** 在回复中内联渲染交互UI组件，支持图表、表单、测验等
...
- **标签：** `installer`, `model-switch`
- **推荐理由：** 一键安装切换Claude Code、Codex、Kimi Code等。
...
- **推荐理由：** DeepSeek Harness最佳上下文仪表盘，可视化洞察管理。
...
- **标签：** `cybersecurity`, `vulnerabilities`, `plugin-verify`, `redteam`
- **推荐理由：** AI安全攻防平台，审计幻觉包、缺失防护和AI模式错误
...
- **标签：** `oauth`, `llm`, `deepseek-harness`
- **推荐理由：** OAuth接入多LLM供应商，免API密钥，开箱即用。
...
- **推荐理由：** 追踪会话/日成本、预算和token用量，多供应商支持。
...
- **标签：** `marketplace`, `registry`, `catalog`
- **推荐理由：** DSH生态核心插件市场，11000+插件搜索引擎
...
- **标签：** `remote`, `access`, `encryption`
- **推荐理由：** 端到端加密P2P远程访问，多端无缝操作
...
- **标签：** `config`, `backup`, `restore`, `sync`
- **推荐理由：** 支持备份恢复迁移同步的完整配置管理
...
- **标签：** `migration`, `session-import`, `resume`
- **推荐理由：** Claude Code/Codex会话一键迁移到DSH。
...
- **标签：** `debugging`, `conflict-resolution`, `utility`
- **推荐理由：** 自动检测并修复插件与版本冲突问题。
...
- **标签：** `theme`, `pet`, `customization`
- **推荐理由：** 面向编程Agent的动效宠物画廊。
...
- **标签：** `dsh`, `dsh-plugin`
- **推荐理由：** 鲸鱼娘皮肤系列，丰富DeepSeek Harness视觉体验。
...
- **推荐理由：** DSH网页端磨砂玻璃主题，可调模糊度、磨砂度和背景壁纸效果
...
- **标签：** `theme`, `animated-background`, `ocean-skin`, `threejs`
- **推荐理由：** 实时动态海洋主题，可调海浪、日落效果和玻璃透明度，支持Chrome/Edge扩展
...
- **标签：** `ai-guide`, `tutorial`, `resource`
- **推荐理由：** 最全AI资源库，教程和提示词一应俱全。
...
- **推荐理由：** DeepSeek Harness生态插件、工具和基础设施精选列表。
...
- **推荐理由：** 涵盖Agent和CLI工具的DeepSeek Harness插件精选目录。
...
- **推荐理由：** 一站式科研工作台：LaTeX写作、arXiv管理、GPU SSH编排。
...
- **标签：** `finance`, `research`, `database`
- **推荐理由：** Tushare金融数据库，多Agent自适应分析
...
- **标签：** `api-cost`, `data-visualization`, `token-usage`
- **推荐理由：** 实时Token用量追踪与API成本估算面板。
...
- **推荐理由：** 中英双语DSH插件精品导航，收录插件、工具、技能和客户端
...
- **标签：** `dsh-bundle`, `knowledge-base`, `awesome-list`, `mcp`
- **推荐理由：** 全维度AI资源百科，涵盖模型���Agent、RAG、多模态和学习路线
...
- **标签：** `awesome-list`, `mcp`, `plugin`, `skill`
- **推荐理由：** DSH插件、技能、MCP服务器与UI的精选资源列表。
...
- **标签：** `awesome-list`, `plugin-list`, `curated`
- **推荐理由：** cordis.run官方自动精选插件目录
...
- **⭐ 星标：** 74
- **语言：** TypeScript
- **描述：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描��目架构、生成阶梯式 Chapter、带你手写核心逻辑、自动判题、AI Code Review。
- **标签：** `deepseek-harness`, `dsh`, `dsh-plugin`
- **推荐理由：** ProMentor 是一个 AI Coding Agent Skill。装上它，你的 AI 编程助手立刻化身为导师——扫描��目架构、生成阶梯式 Chapter
...
- **推荐理由：** 诉讼可视化工具，生成时间轴和案件材料图，适合法律人。
...
- **推荐理由：** 精选280+社区插件目录，14类覆盖全面，快速链接直达仓库。
...
- **标签：** `awesome-list`, `plugin`, `resources`
- **推荐理由：** DSH插件技能资源的全面精选列表。
...
- **标签：** `awesome-list`, `plugins`, `ai-agents`
- **推荐理由：** 80+经验证插件按功能分类整理
...
- **标签：** `dsh-plugins`, `dsh-bundle`, `dsh-repository-plugin`
...
- **标签：** `plugin-directory`, `curated-list`, `dsh`
- **推荐理由：** 精选DSH插件目录，便于发现和安装有用插件
...
- **标签：** `plugin-directory`, `curated-list`, `dsh`, `coding-agents`
- **推荐理由：** 全面的DSH插件与工具精选注册表
