...
- **Tags:** `agent-skills`, `knowledge-distillation`, `digital-human`
...
- **Why use it:** Unified workspace with task boards across devices
...
- **Tags:** `agent-skills`, `skill-store`, `skills`
- **Why use it:** Largest skill store with auto-collected GitHub skills
...
- **Tags:** `loop-engineering`, `workflow-automation`, `agent-ops`
- **Why use it:** Long-horizon control plane for durable agent workflows
...
- **Tags:** `video`, `agent-skill`, `dsh-plugin`
- **Why use it:** Automated Vox-style video generation.
...
- **Why use it:** Enables agent teams collaboration.
...
- **Why use it:** Vision toolkit for text-only models.
...
- **Tags:** `agent-skills`, `skills-routing`, `evaluation`
- **Why use it:** Curates scattered skills into retrieval-ready corpora with built-in evaluation
...
- **Tags:** `agent-observability`, `multimodal`, `video-analysis`
- **Why use it:** Gives AI agents visual/audio observation and verifiable evidence.
...
- **Tags:** `novel-writing`, `creative-writing`, `drama-skills`
- **Why use it:** Built-in novel, drama, game, and video-recap workbenches.
...
- **Tags:** `agent-skills`, `math-modeling`
- **Why use it:** Full-stack math modeling competition skill with GitOps pipeline and forced code self-verification.
...
- **Why use it:** Composable, human-reviewed agent workflows enabling continuous self-evolution and plan mode.
...
- **Tags:** `skills`, `architecture`, `cross-platform`
- **Why use it:** Turn experience into repeatable execution across Claude, Codex, Grok, GPT, and DSH agents.
...
- **Why use it:** 88 production agent skills for research, social intelligence, marketing, and business automation.
...
- **Why use it:** Auto-finds, installs, and verifies DSH GitHub plugins — plug-and-play plugin discovery skill.
...
- **Why use it:** Adds multimodal vision to pure-text LLMs for screenshot/UI/chart analysis with browser automation.
...
- **Why use it:** Complete reverse engineering and CTF skill pack for authorized security research
...
- **Why use it:** Superpowers skills ported for Reasonix coding harness workflows
...
- **Why use it:** Comprehensive web search and content extraction skills for AI agents
...
- **Why use it:** Transforms your AI coding assistant into a mentor with graded chapters and code review
...
- **Why use it:** 25 ready-to-use engineering and efficiency skills in a right sidebar deck
...
- **Why use it:** Complete government document processing skill following GB/T 9704 national standard
...
- **⭐ Stars:** 56
- **Language:** Python
- **Description:** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视��工具集：把凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（大事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **Tags:** `legal`, `visualization`, `timeline`, `svg`
- **Why use it:** Makes law visible with timelines, confrontation charts, and case visualizations
...
- **Why use it:** Portable spec-driven development skill with engineering patterns
...
- **Why use it:** Fusion of Chinese traditional wisdom skills with local engine
...
- **Tags:** `multi-agent`, `collaboration`, `team-workflow`
...
- **Tags:** `qa`, `testing`, `skill-framework`, `pytest`
- **Why use it:** 11 AI testing skills with shared knowledge base and decision matrix
...
- **Tags:** `multi-agent`, `memory`, `durable-agents`
...
- **Tags:** `agent-handoff`, `agent-skills`, `deepseek-harness`
- **Why use it:** Enables cross-agent session continuity and context memory recovery.
...
- **Tags:** `agent-skills`, `deepseek-harness`, `codex`
- **Why use it:** Adapts DSH simplification skill for Codex to refactor and clean code.
...
- **Tags:** `math-modeling`, `agent`, `deepseek-harness`
- **Why use it:** 5-role multi-agent math modeling assistant with 33 models and paper templates.
...
- **⭐ Stars:** 28
- **Language:** Python
- **Description:** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非���有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用各种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **Tags:** `math-modeling`, `skill`, `deepseek-harness`
- **Why use it:** Complete math modeling skill suite: data preprocessing, coding, paper writing, visualization.
...
- **Tags:** `godot`, `game-development`, `deepseek-harness`
- **Why use it:** Full-stack Godot 4.x game dev skill with GDScript support registered at runtime.
...
- **Tags:** `video`, `motion-design`, `deepseek-harness`
- **Why use it:** 152 camera shot recipes and Remotion templates for cinematic product video generation.
...
- **Why use it:** Discusses before acting—structured agent collaboration workflow for task execution.
...
- **Tags:** `agent-tools`, `dsh-plugin`, `pre-flight`
- **Why use it:** Pre-flight plugin check for DSH agents, boosts utilization
