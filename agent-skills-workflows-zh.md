...
- **标签：** `agent-workspace`, `no-code`, `orchestration`
- **推荐理由：** 全设备统一Agent工作空间，支持看板与API
...
- **推荐理由：** 智能编排并自动路由本地Skills的工作流自动化插件。
...
- **标签：** `agent-skills`, `architecture`, `spec-driven`
- **推荐理由：** 架构感知编码，带漂移检测的安全代理
...
- **标签：** `task-manager`, `workflow`, `agentic`
- **推荐理由：** 人机协作的自托管AI任务管理器
...
- **标签：** `agent-teams`, `orchestration`, `multi-agent`
- **推荐理由：** DSH多代理团队协作编排
...
- **推荐理由：** 一键搜索、安装和验证GitHub DSH插件的技能
...
- **推荐理由：** SQLite原生任务看板，支持��理认领审查，无iframe。
...
- **标签：** `skills`, `reasonix`
- **推荐理由：** Superpowers技能移植到Reasonix，增强开发工作流。
...
- **标签：** `spec-driven-development`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 基于规范的开发技能，支持可移植的代码驱动工作流
...
- **标签：** `agent-skills`, `session-memory`, `dsh-plugin`
- **推荐理由：** 跨Agent会话记忆传递，保持任务连续性
...
- **推荐理由：** Composer内快速选择Skill，提升使用效率
...
- **标签：** `agent-skills`, `code-simplification`, `dsh-plugin`
- **推荐理由：** 代码简化技能，提升代码质量
...
- **标签：** `agent-skills`, `chinese-wisdom`, `fengshui`, `dsh-plugin`
- **推荐理由：** 融合八字/风水/易经等中华传统智慧技能包
...
- **标签：** `agent-skills`, `package-manager`, `mcp`, `dsh-plugin`
- **推荐理由：** 42技能+5工具包，一站式Agent工程资源
...
- **标签：** `agent-skills`, `deepseek-harness`, `dsh-plugin`, `project-governance`, `swift-cycle`
- **推荐理由：** 版本固定的治理技能，保障 DSH 工作流可靠运行。
...
- **⭐ 星标：** 13
- **语言：** Python
- **描述：** 中文公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、事实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆���用。
- **标签：** `ai-docs`, `deepseek-harness`, `docx`, `dsh-plugin`, `gongwen`, `wps-office`
...
- **标签：** `agent`, `agent-skills`, `agentic-ai`, `canvas`, `dsh-plugin`, `mcp`
- **推荐理由：** 原生 Cordis 插件，支持无限画布与创意技能。
...
- **标签：** `game-development`, `godot`, `skill`
- **推荐理由：** 为DSH增加Godot 4.x游戏开发技能
...
- **⭐ 星标：** 10
- **语言：** Python
- **描述：** DeepSeek Harness 0.1.0-rc.6 数学建模增强套件：外置插件 + Preset + 17 个手动 Skill 卡片 + Windows ���携版脚本
- **标签：** `math`, `modeling`, `skill-cards`
- **推荐理由：** 数学建模增强套件，含17张预设技能卡
