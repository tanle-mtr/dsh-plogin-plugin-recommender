...
- **标签：** `agent-workspace`, `multi-device`, `workflow`
...
- **标签：** `skills`, `store`, `agent-skills`
- **推荐理由：** 最全技能商店，自动分类GitHub上万技能
...
- **推荐理由：** 智能技能路由与工作流编排，显著提升奖励并降低Token消耗。
...
- **推荐理由：** 基于J-space全局工作空间研究的认知增强技能，提升推理能力。
...
- **标签：** `multimodal`, `ocr`, `video`, `audio`, `agent-skill`
- **推荐理由：** 通过视频音频OCR赋予AI视觉听觉能力。
...
- **标签：** `agent-reviews`, `self-evolving`, `workflow`
...
- **标签：** `vision`, `multimodal`, `automation`
...
- **标签：** `skill-generator`, `automation`, `agent-skills`
...
- **标签：** `patent`, `multi-agent`, `rag`
- **推荐理由：** 专利侵权分析技能，可被任意agent调用，支持竞品报告生成
...
- **⭐ 星标：** 55
- **语言：** JavaScript
- **描述：** Matt Pocock 技能面板（DSH Deck）：安装即自带 25 个工程与效率技能（mattpocock/skills v1.2.3：triage、grilling、wayfinder、handoff 等），右侧面板直接调用，无需手动装技能。当前主力支持 GitHub issue；Markdown 本地文件为预览版；GitLab 暂不在支持范围。希望它��帮你把想法更快变成现实。
- **标签：** `skills`, `agent`, `github-issues`
- **推荐理由：** 内置25个工程技能面板，安装即用，支持GitHub Issue处理
...
- **推荐理由：** 全流程公文处理，符合国标格式，支持格式检查与内容优化
...
- **标签：** `skills`, `dsh-plugin`, `dsh-plugins`
- **推荐理由：** 统一加载并安全管理本地Agent技能的DSH管理器
...
- **标签：** `agent-skill`, `knowledge-base`, `docs`
- **推荐理由：** 官方文档作为即插即用的Agent技能，中英双语
...
- **推荐理由：** 类似WorkBuddy的技能选择器， composer内直接插入
...
- **标签：** `handoff`, `memory`, `session`, `cross-platform`
- **推荐理由：** 本地仓库连续性记忆，跨Agent会话恢复
...
- **标签：** `plugin-dev`, `dsh-plugin`, `skills`
- **推荐理由：** 指导AI从零开发DSH插件的完整技能
...
- **标签：** `video`, `motion-design`, `remotion`, `dsh-plugin`
- **推荐理由：** 152张镜头配方卡的电影感视频生成技能
...
- **标签：** `plugin-dev`, `agent-skills`, `dsh-plugin`
- **推荐理由：** 开发DSH插件的标准Agent Skill技能包
