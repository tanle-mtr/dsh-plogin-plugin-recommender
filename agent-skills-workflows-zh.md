...
- **标签：** `agent-orchestration`, `skills`, `workspaces`, `mcp`
- **推荐理由：** 跨设备多引擎智能体编排与工作区管理
...
- **标签：** `writing`, `novel`, `agent-skills`
- **推荐理由：** 解决遗忘与幻觉的网文长篇创作助手
...
- **标签：** `skills`, `marketplace`, `coding`, `writing`
- **推荐理由：** 涵盖全领域的精选技能商店与自动抓取
...
- **标签：** `skills`, `workflow-automation`, `agent-skills`, `orchestration`
...
- **推荐理由：** 适配DSH的TDD、调试、规划等Superpowers技能。
...
- **推荐理由：** 自带25个工程技能，300亿token训练，无需手动安装
...
- **标签：** `mentor`, `skills`, `coding-agent`
- **推荐理由：** AI编程导师，扫描项目架构生成阶梯式教程，自动判题和Code Review
...
- **推荐理由：** You.com技能插件，支持网络搜索、内容提取和AI集成发现
...
- **⭐ 星标：** 64
- **语言：** Python
- **描述：** 公文全流程处理工具——基于 GB/T 9704《党政机关公文格式》 国家标准，面向公文写作、企事业单位材料编制场景，支持 格式检查与修复、内容优化（Word 原生修订+批注/差异对比版）、模板生成、Markdown 转公文、版头版记页码注入、��实核验、风格增强 等完整能力。原生支持 DeepSeek Harness (DSH) 技能系统，打包为可被 AI Agent 直接调用的 Skill，完全自包含，克隆即用。
- **标签：** `gongwen`, `office`, `chinese-docs`
- **推荐理由：** 基于国标GB/T 9704的公文全流程处理，支持格式检查和模板生成
...
- **标签：** `agent-skill`, `chinese-traditional-wisdom`, `dashboard`
- **推荐理由：** 八字/风水/易经/中医一站式技能包，本地确定性引擎可离线运行
...
- **标签：** `math-modeling`, `paper-writing`, `data-preprocessing`
- **推荐理由：** 覆盖建模全流程：数据处理到论文写作，支持降AIGC检测
...
- **推荐理由：** 可移植的规格驱动开发技能，遵循DeepSeek Harness工程模式
...
- **推荐理由：** 多Agent协作框架，独立模型/技能/上下文+共享工作空间
...
- **标签：** `multi-agent`, `persistent-memory`, `agent-team`
- **推荐理由：** 跨会话持久化Agent团队，保留记忆与技能不重置
...
- **推荐理由：** 11个测试技能+共享知识库，AI驱动软件测试全流程
...
- **推荐理由：** 5角色多Agent数学建模，含33篇模型库+论文模板+进度可视化
...
- **标签：** `collaboration`, `planning`, `agent-skill`
- **推荐理由：** 议行分离协作机制，先充分讨论再动手执行
...
- **推荐理由：** 用于开发DSH插件的Agent Skill，遵循标准规范。
...
- **标签：** `agent-tools`, `awesome-dsh-plugin`, `cordis`
- **推荐理由：** 任务前自动预检插件库并注入Top-K推荐。
...
- **标签：** `awesome-dsh-plugin`, `workflow`, `task-management`
- **推荐理由：** 工程工作流插件，支持任务阶段与技能管理。
