...
- **标签：** `taskboard`, `dsh-plugin`, `skills`
- **推荐理由：** 现代化任务面板，灵活嵌入Codex/DSH
...
- **⭐ 星标：** 2,251
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你盯着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动��，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `widget`, `floating`, `dsh-plugin`
- **推荐理由：** 可爱鲸鱼浮窗，实时监控DeepSeek账户余额
...
- **推荐理由：** 本地优先的DSH桌面工作区，支持会话、项目和Office工件管理
...
- **推荐理由：** 零配置Windows桌面GUI，集成Codex、SSH、技能和移动端远程访问
...
- **标签：** `UI`, `components`, `charts`, `inline-rendering`
- **推荐理由：** 在回复中内联生成UI组件：布局、图表、表单和测验
...
- **标签：** `canvas`, `workspace`, `non-linear`, `conversation`
- **推荐理由：** 基于画布的非线性对话工作空间，支持分支会话探索
...
- **推荐理由：** 本地优先多模型AI代理桌面应用，支持自进化技能和隐私保护
...
- **标签：** `office`, `deepseek-harness-plugin`, `desktop`
- **推荐理由：** 将电子表格、文档、幻灯片等完整办公软件套件带入DeepSeek Harness。
...
- **推荐理由：** 原生iOS客户端，WebSocket同步工作区、会话和Agent执行轨迹。
...
- **推荐理由：** 类CodePilot桌面客户端及插件套件，支持macOS/Windows/Linux。
...
- **标签：** `notes`, `ui`, `docker`, `smartisan`
- **推荐理由：** 开源锤子便签复刻版，支持Docker部署和Skill调用
...
- **标签：** `web`, `usage-tracking`, `pricing`
- **推荐理由：** 底部状态栏实时显示模型、余额、峰谷定价，持久化。
...
- **标签：** `bioinformatics`, `tauri`, `desktop`
- **推荐理由：** Tauri生物信息学桌面端，一键安装，支持离线使用。
...
- **标签：** `skill-picker`, `ui`
- **推荐理由：** WorkBuddy同款技能选择器，composer一键插入技能。
...
- **标签：** `kline`, `sidebar`, `finance`
- **推荐理由：** 交互式K线分析侧栏，多市场技术指标+新闻基本面。
...
- **标签：** `ui`, `mcp-manager`, `web-gui`
- **推荐理由：** Web GUI新增技能与MCP管理卡片，浏览器内直接管理
...
- **⭐ 星标：** 11
- **语言：** Rust
- **描述：** StarHub — All-in-One DevOps Desktop Command Center。把开发运维每天要用���的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体机操作。
- **标签：** `desktop`, `devops`, `docker`
- **推荐理由：** 集成数据库、SSH、Docker、AI助手的桌面运维工具。
...
- **标签：** `desktop`, `electron`
- **推荐理由：** Electron桌面版DSH，集成Web UI与侧边栏。
