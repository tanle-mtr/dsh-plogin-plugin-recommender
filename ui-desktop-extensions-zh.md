...
- **标签：** `design-systems`, `dsh-plugin`, `ui-generator`
- **推荐理由：** 本地优先的设计插件，将编码智能体转化为设计师。
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 鲸鱼顶栏风格TUI，支持流式思考、回滚和TPS显示
...
- **标签：** `floating-widget`, `balance-monitor`, `ui`
- **推荐理由：** 右下角小鲸鱼浮动窗口��实时监控余额，支持拖拽吸附
...
- **推荐理由：** 仅5MB的Tauri桌面版，零环境配置，预装插件跨平台运行
...
- **标签：** `office`, `ui`, `productivity`
- **推荐理由：** 在DSH中提供电子表格、文档、演示等完整办公套件。
...
- **⭐ 星标：** 191
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将工作区、会话、实���回复和 Agent 执行轨迹带到 iPhone，同时延续 DeepSeek WebUI 克制、清晰的视觉语言
- **标签：** `ios`, `mobile`, `harness`
- **推荐理由：** DeepSeek Harness原生iOS客户端，支持移动端管理
...
- **标签：** `desktop`, `electron`, `ui`
- **推荐理由：** Electron桌面控制台，多Tab管理DSH插件、MCP和Skills
...
- **推荐理由：** WorkBuddy 风格技能选择器，composer 一键插入。
...
- **推荐理由：** 集成SSH、Docker、数据库客户端的一站式DevOps桌面
...
- **推荐理由：** Electron封装版DSH桌面应用，集成侧边栏等组件
