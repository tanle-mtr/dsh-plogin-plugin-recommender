...
- **推荐理由：** DeepSeek Harness原生桌面客户端，提升用户体验。
...
- **推荐理由：** 开放侧边栏底座，内置文件编辑、终端、Git和子代理页面
...
- **推荐理由：** 现代化可嵌入任务面板，支持灵活集成到编码Agent中
...
- **⭐ 星标：** 3,165
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录的 TUI 补位插件：鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上���文进度+TPS。npm 一键装。  DSH official WeChat featured TUI plugin — whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `terminal`, `deepseek-harness`
- **推荐理由：** TUI插件，提供鲸鱼顶栏、流式思考与上下文进度显示
...
- **推荐理由：** 可爱的鲸鱼浮动组件，实时追踪DeepSeek余额，支持拖拽吸附。
...
- **推荐理由：** 轻量Tauri桌面版，预置插件，零环境配置即可使用。
...
- **推荐理由：** 零配置桌面GUI，支持SSH、插件和远程访问，开箱即用
...
- **标签：** `desktop`, `mcp`, `local-first`, `office-automation`
- **推荐理由：** 本地优先的桌面工作空间，支持会话、MCP和办公自动化
...
- **标签：** `ui`, `interactive`, `charts`, `forms`
- **推荐理由：** 在回复中内联渲染图表、表单和互动组件的UI插件
...
- **标签：** `ui`, `canvas`, `conversation`, `visual`
- **推荐理由：** 可视化非线性对话画布，支持分支会话和多线程探索
...
- **标签：** `deepseek-harness-plugin`, `dsh-plugin`, `office`
- **推荐理由：** 为DSH添加完整办公套件，支持表格、文档、幻灯片等。
...
- **标签：** `desktop-app`, `ai-agent`, `dsh-plugin`, `local-first`
- **推荐理由：** 本地优先的AI桌面应用，支持多模型和自我进化技能。
...
- **标签：** `ios-app`, `dsh-plugin`, `mobile`, `kotlin-multiplatform`
- **推荐理由：** DSH原生iOS客户端，支持工作区、会话和实时Agent跟踪。
...
- **标签：** `desktop-app`, `electron`, `dsh-plugin`, `codepilot`
- **推荐理由：** CodePilot风格跨平台桌面客户端，丰富插件套件支持DSH。
...
- **标签：** `mcp`, `plugin`, `skills`
- **推荐理由：** Web界面技能与MCP管理工具，浏览器内直接操作
...
- **⭐ 星标：** 60
- **语言：** TypeScript
- **描述：** DSH Desktop Hub — DeepSeek Harness 桌面管理控制台（Electron + TypeScript）。多 Tab 管理 Harness / Plugin / MCP / Skills，双��即用。
- **标签：** `dsh-plugin`, `electron`, `deepseek-harness`
- **推荐理由：** Electron桌面管理控制台，多标签管理DSH各模块
...
- **推荐理由：** 全能DevOps桌面客户端：数据库、SSH、Docker、AI一体化。
...
- **推荐理由：** Electron桌面封装，集成DSH网页、侧边栏和ModLens。
...
- **标签：** `dsh-plugin`, `webui`, `background`
- **推荐理由：** 支持上传图片与预设的DSH Web背景插件。
