...
- **标签：** `web-ui`, `plugin-store`, `ecosystem`
- **推荐理由：** Web端插件聚合平台，支持Creative Workshop分发
...
- **推荐理由：** 可扩展侧边栏，集成文件编辑/终端/对话/Git/子代理
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 鲸鱼顶栏TUI，支持流式思考、双击回滚和上下文进度显示
...
- **⭐ 星标：** 1,915
- **语言：** JavaScript
- **描述：** DeepSeek Harness（DSH）一只住在 DSH 界面右下角的小鲸鱼娘，帮你��着DeepSeek账户余额。QQ弹弹，支持拖拽吸附、左吸附翻转、数字滚动动画，随界面自动启用，建议直接喊来你的dsh安装
- **标签：** `floating-widget`, `deepseek-harness`, `dsh-plugin`
- **推荐理由：** 可爱鲸鱼浮动控件，实时显示账户余额，支持拖拽和动画效果。
...
- **推荐理由：** 仅5MB的Tauri桌面版，零配置开箱即用，支持跨平台。
...
- **推荐理由：** 本地优先的Electron桌面工作空间，集成DSH会话和办公工具。
...
- **标签：** `desktop`, `windows`, `electron`
- **推荐理由：** 功能完整的开源Windows桌面客户端，支持SSH和移动端
...
- **推荐理由：** 将电子表格、文档、幻灯片等办公软件引入DeepSeek Harness。
...
- **推荐理由：** 原生iOS客户端，将工作区与Agent执行轨迹带到iPhone。
...
- **推荐理由：** Electron桌面控制台，多Tab统一管理DSH插件与MCP技能
...
- **标签：** `skills-manager`, `ui`, `safety`
- **推荐理由：** 本地技能统一加载与管理器，带安全控制的DSH面板
...
- **推荐理由：** DSH Composer内集成WorkBuddy风格技能选择器
...
- **推荐理由：** 生物信息学桌面应用，支持离线模式，适合临床与湿实验科研人员。
...
- **标签：** `devops`, `desktop`, `docker`, `ssh`
- **推荐理由：** DevOps一体化桌面：数据库、SSH、SFTP、Docker与AI沙箱合一。
...
- **标签：** `electron`, `desktop-app`, `deepseek-harness`, `dsh`
