...
- **标签：** `task-panel`, `ui`, `coding-agent`
- **推荐理由：** 现代化灵活任务面板，嵌入Codex/DSH界面
...
- **⭐ 星标：** 2,950
- **语言：** TypeScript
- **描述：** DSH 官方公众号收录��� TUI 补位插件：Claude Code 风，鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上下文进度+TPS。npm 一键装。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **标签：** `tui`, `terminal`, `ui`
- **推荐理由：** 鲸鱼顶栏TUI，支持流式思考与回滚操作
...
- **推荐理由：** 本地优先Electron桌面工作区，集成DSH会话与MCP工具。
...
- **推荐理由：** 开源Windows桌面客户端，含SSH、Codex和远程访问功能。
...
- **推荐理由：** 在DSH回复中内联渲染图表、表单和测验等交互式UI组件。
...
- **标签：** `canvas`, `visual`, `workspace`, `conversation`
- **推荐理由：** 基于画布的可视化对话工作区，支持非线性分支会话。
...
- **推荐理由：** 本地优先Tauri桌面代理，支持多模型和自我进化技能。
...
- **推荐理由：** 类CodePilot桌面客户端，全平台原生插件套件
...
- **标签：** `mcp`, `ui`, `skills`
- **推荐理由：** 通过Web界面可视化管理skill和MCP，操作便捷
...
- **推荐理由：** Web GUI一键安装卸载社区插件，插件管理更方便
...
- **推荐理由：** WorkBuddy风格技能选择器大幅提升 Composer 体验
...
- **标签：** `web-ui`, `mcp`, `plugin-manager`
- **推荐理由：** 在Web UI中可视化管理局部技能与MCP服务器
...
- **推荐理由：** 集成数据库、SSH、Docker的DevOps桌面控制台
...
- **推荐理由：** Electron桌面版DSH，集成ModLens和侧边栏。
