...
- **推荐理由：** 开放侧边栏底座，内置文件编辑、终端、对话、Git等页面。
...
- **标签：** `taskboard`, `tui`, `ui`
- **推荐理由：** 现代可嵌入任务面板，支持Codex和DSH，设计简洁美观。
...
- **标签：** `tui`, `ui`, `terminal`
- **推荐理由：** 鲸鱼顶栏TUI插件，支持实时状态、流式思考，获官方推荐。
...
- **推荐理由：** 可爱漂浮鲸鱼挂件，实时显示DS账户余额，支持拖拽和动画。
...
- **标签：** `desktop`, `local-first`, `agent`, `multi-model`
- **推荐理由：** 本地优先多模型桌面��用，技能自进化。
...
- **⭐ 星标：** 234
- **语言：** Kotlin
- **描述：** DeepSeek Harness Mobile 是一个面向 DeepSeek Harness 的原生 iOS 客户端。它通过 dsh-plugin-mobile-gateway 与 Harness 建立 WebSocket 连接，将工作区、会话、实时回复和 Agent ��行轨迹带到 iPhone，同时延续 DeepSeek WebUI 克制、清晰的视觉语言
- **标签：** `ios`, `mobile`, `client`
- **推荐理由：** 原生iOS客户端，实时查看Agent执行轨迹。
...
- **推荐理由：** 一站式DevOps桌面：数据库/SSH/Docker/AI沙箱
...
- **标签：** `ui`, `compact`, `web-extension`
- **推荐理由：** 一键折叠DSH Web上下文面板，节省显示空间
