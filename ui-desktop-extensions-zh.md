...
- **推荐理由：** 开放侧边栏底座，内置文件编辑/终端/Git/对话页面
...
- **推荐理由：** Claude Code风格终端界面，实时状态与流式思考显示
...
- **推荐理由：** 轻量Tauri桌面版，仅5MB安装器，预置插件，开箱即用
...
- **推荐理由：** 零配置Windows桌面客户端，内置SSH和Codex支持。
...
- **标签：** `desktop-app`, `local-first`, `typescript`
- **推荐理由：** 本地优先桌面工作区，集成会话、文件和Office组件。
...
- **标签：** `canvas`, `conversation`, `visual`
- **推荐理由：** 可视化非线性会话画布，支持分支对话流探索。
...
- **标签：** `office`, `desktop`, `spreadsheets`
- **推荐理由：** 将表格、文档和幻灯片直接引入DSH桌面。
...
- **推荐理由：** 开源锤子便签复刻版，支持Docker部署、DSH插件与微信/QQ对接
...
- **推荐理由：** Electron桌面管理控制台，多Tab统一管理DSH组件
...
- **标签：** `ui`, `balance-tracking`, `info-bar`
- **推荐理由：** 实时显示模型余额与峰值定价信息栏，实用便捷
...
- **推荐理由：** WorkBuddy风格技能选择器，嵌入DSH编辑器使用
...
- **推荐理由：** 集成数据库、SSH、Docker和AI沙箱的一体化DevOps桌面。
...
- **推荐理由：** 精选DSH插件：任务通知、LLM追踪检查和快捷键绑定。
...
- **标签：** `desktop`, `electron`
- **推荐理由：** Electron壳封装DSH Web服务，集成侧边栏等组件。
