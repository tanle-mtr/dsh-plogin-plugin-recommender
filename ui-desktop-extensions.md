...
- **Why use it:** World's first virtual terminal for AI agents, enabling sandboxed terminal experiences.
...
- **Tags:** `tui`, `theme`, `terminal`
- **Why use it:** Claude Code-style TUI with whale theme, streaming thought, rollback, and TPS display.
...
- **Why use it:** Rust-based high-performance agent runtime paired with a polished desktop app.
...
- **Why use it:** Cute floating whale widget tracks DeepSeek balance with drag-and-drop adsorption.
...
- **Why use it:** Lightweight 5MB Tauri desktop app with preset plugins, cross-platform support.
...
- **Why use it:** Local-first desktop workspace for DSH sessions, projects, web research, and office artifacts.
...
- **Why use it:** Open-source Windows desktop GUI with SSH and mobile remote access
...
- **Tags:** `canvas`, `branching`, `workspace`, `visual`
- **Why use it:** Canvas-based visual branching conversation workspace for DSH
...
- **Tags:** `office`, `ui`, `spreadsheet`, `docs`
- **Why use it:** Brings spreadsheets, docs, slides and relational tables directly into DSH conversations.
...
- **Tags:** `desktop-app`, `electron`, `typescript`, `multi-platform`
- **Why use it:** CodePilot-inspired native desktop client for DSH across macOS, Windows, and Linux platforms.
...
- **Tags:** `ios`, `swift`, `mobile`, `websocket`, `native-app`
- **Why use it:** Native iOS client connecting to DSH via WebSocket, bringing workspace and agent traces to iPhone.
...
- **Why use it:** Open-source Smartisan Notes with DSH plugin and Docker support
...
- **Tags:** `mcp`, `skills`, `ui`
- **Why use it:** Unified menu to manage MCP tool and skill exposure
...
- **Why use it:** Electron-based DSH desktop control center with multi-tab management
...
- **Tags:** `status-bar`, `usage-tracking`, `pricing`
- **Why use it:** Status bar for DSH: live balance, pricing, peak/off-peak tracking
...
- **⭐ Stars:** 12
- **Language:** Rust
- **Description:** StarHub — All-in-One DevOps Desktop Command Center。把开发运维每天要用到的工具收进同一个窗口:数据库客户端 · SSH 终端 · SFTP · Docker · AI 助手,以及 AI 驱动的沙箱桌面与 Android 实体��操作。
...
- **Why use it:** Electron desktop wrapper for DSH with integrated sidebar and UI.
...
- **Tags:** `ui`, `web-extension`, `compact`
- **Why use it:** One-click compact context button for the DSH Web panel.
...
- **Tags:** `ui`, `knowledge-cards`, `share`
- **Why use it:** Transform chat history into downloadable shareable knowledge cards.
