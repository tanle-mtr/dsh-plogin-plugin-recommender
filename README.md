...
|--------|-------|
| **Plugins tracked** | 196 |
| **Total community stars** | 970,686 ⭐ |
| **Categories** | 12 |
| **Last updated** | 2026-09-05 18:55:57 UTC |
...
| Plugin | Stars | Why |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 213,052 | Core platform enabling everything as a plugin for  |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 94,264 | Local-first open-source design tool powered by AI  |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 70,636 | Deploy multi-agent swarms and autonomous workflows |

---

## Categories

- [Coding Agents & Harness Tools](#coding-agents-&-harness-tools) — 33 plugins
...
- [Coding Agents & Harness Tools](coding-agents-harness-tools) (33)
...
- **Why use it:** Core platform enabling everything as a plugin for the DSH ecosystem
...
- **Tags:** `agent-control`, `long-horizon`
- **Why use it:** Long-horizon agent control plane for durable multi-engine work
...
- **Why use it:** First vision plugin enabling image-to-text for text-only coding agents via structured JSON OCR output.
...
- **Tags:** `deepseek-harness`, `preset`, `tools`
- **Why use it:** Two-phase preset: minimal bootstrap then full Standard tools, ideal for Project2 workflows.
...
- **Why use it:** World's first virtual terminal with VFS, giving AI agents sandboxed persistent shell environments.
...
- **Tags:** `model-switch`, `coding-agent`, `multi-tool`
- **Why use it:** One-click model switch between Claude Code, Codex, DeepSeek, Kimi, and other coding agents.
...
- **Why use it:** High-performance Rust agent runtime paired with a polished desktop app for deep coding agent workflows.
...
- **Why use it:** AI code reviews grounded in 12 classic engineering books with risk diagnostics and auto-fix.
...
- **Tags:** `dsh-plugin`, `security`, `cybersecurity`
- **Why use it:** Security audit tool that shields AI-generated code from hallucinations and vulnerabilities.
...
- **Tags:** `dsh-plugin`, `coding-agent`, `rpc`
...
- **Why use it:** CLI and resolver for DSH Plugin Hub — install and manage plugins easily
...
- **Why use it:** Run unmodified Pi extensions natively in DSH via one ABI
...
- **Tags:** `plugin-management`, `ui`, `deepseek-harness`
- **Why use it:** Official plugin management console for DSH ecosystem
...
- **Why use it:** Agent skill that guides DSH plugin development from scratch
...
- **Tags:** `plugin-development`, `codex`, `skill`
...
- **Why use it:** Plugin dev knowledge base as on-demand agent skill with official docs
...
- **Tags:** `skills-manager`, `plugin-management`, `dsh`
...
- **Tags:** `deepseek-harness`, `containers`, `dsh-plugin`
...
- **Why use it:** Audit AGENTS.md tokens, detect duplicates and conflicts via ring panel
...
- **Tags:** `agent-framework`, `agent-skills`, `mcp`, `dsh-plugin`
- **Why use it:** All-in-one package manager for DSH agent skills, CLIs, and plugins
...
- **Why use it:** Pre-flight plugin check improves agent tool selection accuracy.
...
- **Why use it:** Search and browse DSH plugins directly from within agents.
...
- **Why use it:** Modern flexible task board embedding into Codex and DeepSeek Harness with rich customization.
...
- **Why use it:** Claude Code-style TUI with whale toolbar, streaming thoughts, rollback, and real-time TPS display.
...
- **Why use it:** Cute draggable whale widget displaying DeepSeek account balance in DSH interface.
...
- **Why use it:** Lightweight 5MB Tauri desktop app with preset plugins, zero environment setup, cross-platform.
...
- **Tags:** `dsh-plugin`, `ui-components`, `genui`
- **Why use it:** Interactive UI components rendered inline via dsh-ui fence for rich responses.
...
- **Tags:** `dsh-plugin`, `desktop-app`, `local-first`
...
- **Tags:** `dsh-plugin`, `desktop-app`, `windows`
...
- **Why use it:** Brings spreadsheets, docs, slides into DSH — real office environment
...
- **Tags:** `desktop-app`, `electron`, `codepilot`, `ui`
...
- **Tags:** `ios`, `mobile`, `ui`
- **Why use it:** Official iOS client with WebSocket workspace and agent tracking
...
- **Why use it:** WorkBuddy-style skill picker with /skill-name command insertion
...
- **Why use it:** Desktop bioinformatics agent for clinicians with offline support
...
- **Tags:** `desktop`, `devops`, `docker`, `ssh`, `dsh-plugin`
...
- **⭐ Stars:** 6
- **Language:** JavaScript
- **Description:** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即���完全移除；内置中英文双语界面。
- **Tags:** `webui`, `customization`, `background`
- **Why use it:** Web background personalization with presets and upload support
...
- **Why use it:** Electron desktop wrapper integrating multiple DSH features
...
- **Tags:** `rag`, `wiki`, `vector-search`
- **Why use it:** Turn raw documents into queryable RAG knowledge base and wiki
...
- **Why use it:** Dedicated memory management kit enabling agents to remember and refine their own knowledge over time.
...
- **Why use it:** Persistent unified memory layer for all AI agents backed by Markdown and Milvus hybrid search.
...
- **Why use it:** Best context insight and management plugin with dashboard, browser, statistics, and commands.
...
- **Why use it:** Largest skill store covering docs, coding, automation workflows
...
- **Tags:** `skills`, `workflow`, `agent-routes`
- **Why use it:** Intelligent skill routing and workflow orchestration improving rewards by 21pp while cutting tokens 30%.
...
- **Tags:** `dsh-plugin`, `task-management`, `workflow`
...
- **Tags:** `dsh-plugin`, `creative-writing`, `drama`
...
- **Tags:** `math-modeling`, `competition`, `agent-skill`, `gitops`
...
- **Tags:** `agent-review`, `self-evolving`, `workflow`, `prompt-engineering`
...
- **Tags:** `skills`, `multi-agent`, `workflow`
...
- **Tags:** `discovery`, `plugin`, `automation`
- **Why use it:** Auto-finds, installs and verifies GitHub plugins via skill
...
- **Why use it:** Specialized skills for AI infra dev, optimize and debug systems
...
- **Tags:** `security`, `ctf`, `pentest`
- **Why use it:** 87 reverse engineering and pentesting SKILL.md plugins for CTF
...
- **Tags:** `chinese-traditional-wisdom`, `fengshui`, `divination`
- **Why use it:** Bazi, Fengshui, I-Ching and TCM fusion in one deterministic engine
...
- **Tags:** `agent-skills`, `fintech`, `backtesting`
...
- **Why use it:** Codex-style skill adapted for DSH to simplify code automatically
...
- **Why use it:** Cross-platform session memory for smooth agent handoffs
...
- **Tags:** `ai-video`, `skills`, `directorx`
...
- **Why use it:** Adds Godot 4.x game dev skill to DSH agents at runtime
...
- **Tags:** `image-generation`, `vision`, `skill`, `dsh-plugin`
- **Why use it:** Enables image reading and generation for DSH vision-capable models
...
- **Tags:** `swift-cycle`, `project-governance`, `dsh-plugin`
- **Why use it:** Version-pinned Swift Cycle governance skill for DSH workflows
...
- **Why use it:** Native multi-agent financial analysis with MCP and HTTP API support
...
- **Tags:** `mcp`, `cloud`, `huaweicloud`, `api`
- **Why use it:** Official Huawei Cloud MCP tools and sandbox for AI agent deployment
...
- **Why use it:** OAuth-based ChatGPT integration, use GPT models inside DSH.
...
- **Why use it:** Local-first open-source design tool powered by AI coding agents
...
- **Why use it:** 530+ GPT-Image2 prompt templates with industrial-grade precision
...
- **Tags:** `pixel-art`, `mascot`, `ui`
- **Why use it:** Animated pet gallery adding personality to your AI agents
...
- **Tags:** `video`, `generative-ai`, `explainer`
...
- **Why use it:** Native AI design studio with editable templates, visual editor, and PPT generation in DSH.
...
- **Why use it:** Transform chaotic case materials into professional litigation diagrams
...
- **Why use it:** OCR and image-to-text for pixel art with vision-trained methodology
...
- **Why use it:** 6 design styles (dark-saas, apple-minimal, neo-neu) for vibe-coded sites
...
- **Tags:** `knowledge-cards`, `ui`, `deepseek-harness`
- **Why use it:** Transforms chat history into shareable beautiful knowledge cards.
...
- **Why use it:** Enhancement suite and prompt manager for AI web chat interfaces
...
- **Why use it:** Local-first cross-platform content discovery agent scraping Bilibili, YouTube, Xiaohongshu, and more.
...
- **Why use it:** Seamless logged-in browser automation for AI agents without interrupting your workflow.
...
- **Tags:** `dsh-plugin`, `web-search`, `free`
- **Why use it:** Strongest free web search plugin with no signup or API key required.
...
- **Why use it:** DingTalk IM bot with stream mode and tool-capable agent support.
...
- **Why use it:** Runtime injector with task-aware reasoning-mode routing
...
- **Tags:** `marketplace`, `plugin-store`, `utility`
- **Why use it:** Visual plugin marketplace inside DSH for browsing, searching, and one-click installing plugins.
...
- **Why use it:** Self-hostable plugin ecosystem radar auto-discovering 15900+ candidates with k8s pipeline testing.
...
- **Why use it:** Curated awesome list covering the full DSH plugin ecosystem and ecosystem breadth.
...
- **Why use it:** Community-curated list of DSH plugins, tools, and infrastructure from official sources.
...
- **Tags:** `dsh-plugin`, `oauth`, `utility`
- **Why use it:** OAuth integration to use ChatGPT/Claude/Grok subscriptions as DSH providers.
...
- **Why use it:** Track session/daily costs, budgets, token usage across multiple providers
...
- **Why use it:** Secure P2P encrypted remote access across PC, Android and Web
...
- **Tags:** `ai-cost`, `data-visualization`, `token-usage`, `dsh-plugin`
- **Why use it:** Real-time token usage, cost charts, and API balance for DeepSeek
...
- **Tags:** `plugin-directory`, `registry`, `curated-list`
- **Why use it:** Curated plugin directory for discovering and installing DSH plugins.
...
- **Why use it:** Whale Girl themed skin series customizing the visual appearance of DeepSeek Harness.
...
- **Why use it:** Highly customizable glass-morphism theme with frosted glass effects.
...
- **Why use it:** Curated list of all DSH plugins for easy discovery
...
- **Why use it:** Auto-searches 8 major Chinese platforms for 30-day content, generates cited research reports.
...
- **Tags:** `dsh-plugin`, `research`, `ai-pedia`
- **Why use it:** Comprehensive AI encyclopedia covering agents, RAG, MCP, and coding resources.
...
- **Why use it:** Daily automated plugin curation with scenario classification and popularity ranking.
...
- **⭐ Stars:** 202
- **Language:** TypeScript
- **Description:** Mimir — 一站式科研工作台插件：LaTeX 论文边写边编译、arXiv 文献管理、实验追踪、指标图��、GPU 服务器 SSH 任务编排，管理科研全周期。An open-source research workbench plugin for the whole research cycle.
- **Tags:** `research`, `latex`, `arxiv`, `experiment-tracking`
- **Why use it:** One-stop research workbench: LaTeX, arXiv, experiment tracking, SSH
...
- **Tags:** `academic-search`, `arxiv`, `crossref`, `citation`
- **Why use it:** Cross-database academic search with citation graph, MeSH, verification
...
- **Why use it:** Patent infringement analysis with RAG-powered prior art search
...
- **Tags:** `kline`, `analysis`, `fintech`
- **Why use it:** Interactive K-line analysis with multi-market technical indicators
...
- **Tags:** `evaluation-as-code`, `rag-evaluation`, `llm-evaluation`, `dsh-plugin`
...
- **Why use it:** Vertical curated list of 24 verified stock/finance plugins
...
- **Why use it:** Verified star-ranked community DSH plugins
...
- **Why use it:** Complete 1000+ DSH tool inventory sorted by stars
...
- **Why use it:** Plugin product manual and scenario-based navigation guide
...
- **Why use it:** Auto-updated daily plugin directory via GitHub Actions crawl
...
- **Why use it:** Zero-dependency static portal for discovering and installing plugins
...
- **Why use it:** Evidence-led bilingual plugin directory, immutable review commits.
...
- **Why use it:** Auto-updating YAML-driven plugin list with bilingual README.
...
- **Why use it:** General DSH plugin listing, basic reference collection.
...
- **Tags:** `awesome-dsh-plugin`
- **Why use it:** Curated evidence-aware plugin shortlist for DSH.
...
- **Why use it:** Privacy-first open-source resume builder, fully self-hosted
...
- **Tags:** `armor-breaking`, `deepseek`, `utility`
- **Why use it:** Stable armor-breaking plugin specifically optimized for DeepSeek usage limits.
...
- **Why use it:** Essential bilingual curated list for discovering DSH plugins and tools.
...
- **Tags:** `awesome-list`, `guide`, `resources`, `starter`
- **Why use it:** Ultimate DSH guide with quickstart, resources, plugins and toolkits
...
- **Tags:** `marketplace`, `catalog`, `plugin-store`, `directory`
- **Why use it:** 11,000+ DSH plugins marketplace with search, rankings and public API
...
- **Why use it:** Curated list of DSH plugins, skills, MCP servers, patches and UIs
...
- **Why use it:** Catalog of 1958 repos / 1819 real DSH plugins with search and filters
...
- **⭐ Stars:** 163
- **Language:** TypeScript
- **Description:** DeepSeek Harness 搜番插件：对话内多��搜索番剧，卡片展示 Bangumi 评分与详情，支持复制磁力。
- **Tags:** `anime`, `entertainment`, `search`
- **Why use it:** Search anime with Bangumi ratings and magnet support in chat
...
- **Why use it:** Open-source Smartisan notes with Docker, skill call and export
...
- **Tags:** `skillhub`, `open-api`, `dsh-plugin`
- **Why use it:** Official SkillHub Open API docs and DSH plugin examples
...
- **Why use it:** Curated list of DSH plugins with web directory and GitHub source
...
- **Tags:** `awesome-list`, `dsh`, `plugin`, `skills`
...
- **Why use it:** DSH plugin marketplace similar to CodeX, easy plugin discovery.
...
- **Why use it:** Redirects to new repo; check github.com/awesome-dsh-plugin/awesome-dsh-plugin.
