...
|--------|-------|
| **Plugins tracked** | 201 |
| **Total community stars** | 1,066,584 ⭐ |
| **Categories** | 11 |
...
| Plugin | Stars | Why |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 231,140 | Core DSH platform, enables everything-as-plugin ar |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,283 | Local-first design tool that turns coding agents i |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 72,925 | Deploy multi-agent swarms and coordinate autonomou |
...
- **Why use it:** Core DSH platform, enables everything-as-plugin architecture
...
- **Why use it:** Self-evolving multi-agent assistant with memory and tool support
...
- **Why use it:** Virtual terminal and filesystem giving agents isolated sandboxed execution.
...
- **Why use it:** High-performance Rust agent runtime with polished desktop and open coding capabilities.
...
- **Tags:** `context-management`, `dashboard`, `sidebar`
- **Why use it:** Dashboard and sidebar for deep context insight, management, and status monitoring.
...
- **Why use it:** CLI and schema tools for DSH Plugin Hub — install, resolve, manage plugins
...
- **Tags:** `auto-pilot`, `gitops`, `math-modeling`
- **Why use it:** Full-stack auto math modeling with GitOps pipeline and code self-verification
...
- **Why use it:** Official DSH plugin dev knowledge base as an on-demand agent skill bundle.
...
- **Tags:** `api`, `documentation`, `skillhub`
- **Why use it:** Official SkillHub Open API docs and examples for DSH plugin development.
...
- **Why use it:** Integrate ChatGPT into DSH via OAuth — extend your AI toolkit.
...
- **Why use it:** Local-first design tool that turns coding agents into designers
...
- **Why use it:** Modern desktop client treating the app itself as a plugin
...
- **Tags:** `ai-studio`, `browser-extension`, `chrome-extension`, `deepseek-harness`, `chatgpt`, `claude-ai`
- **Why use it:** Powerful browser extension enhancing multiple AI platforms with prompt management
...
- **Tags:** `deepseek-harness-desktop`, `dsh-plugin-desktop`, `desktop`
- **Why use it:** Official desktop application bringing DSH beyond browser — full-featured and native
...
- **Tags:** `dsh-web`, `dsh-web-ui`, `deepseek-harness`
- **Why use it:** Web plugin aggregation hub with creative workshop distribution model
...
- **⭐ Stars:** 3,688
- **Language:** TypeScript
- **Description:** 开放的侧边栏底座，支持三方拓展注册新侧边栏页面。内置文件渲染编辑/终端/侧边对话/Git/子���理页面 ｜ Open sidebar foundation, supports third-party extensions to register new sidebar pages. Built-in file rendering/editing, terminal, side chat, Git, and sub-agent pages.
- **Tags:** `sidebar`, `ui-extension`, `third-party`
...
- **Tags:** `taskboard`, `ui-extension`, `codex-plugin`
- **Why use it:** Modern flexible task board that embeds into Codex and DSH interfaces.
...
- **⭐ Stars:** 3,115
- **Language:** TypeScript
- **Description:** DSH 官方公众号收录的 TUI 补位插件：Claude Code 风，鲸鱼顶栏/实时状态/流式思考/双击 Esc 回滚/上下文进度+TPS。npm 一键��。  DSH official WeChat featured TUI plugin — Claude Code style: whale bar, live status, streaming thoughts, double-Esc rollback, context bar + TPS. npm one-click.
- **Tags:** `tui`, `terminal`, `claude-code-style`
- **Why use it:** Claude Code-style TUI with whale theme, real-time status and streaming thinking display.
...
- **Tags:** `floating-widget`, `balance-tracker`
- **Why use it:** Adorable floating widget tracking DeepSeek balance with drag-and-drop and animations.
...
- **Why use it:** Lightweight Tauri desktop app, 5MB installer, zero setup, cross-platform.
...
- **Tags:** `desktop-app`, `electron`, `gui`, `windows-desktop`
- **Why use it:** Zero-setup Windows desktop GUI for DSH with SSH, plugins, mobile access
...
- **Why use it:** Local-first AI desktop workspace for DSH sessions, projects, MCP, office
...
- **Why use it:** Local-first AI desktop app with self-evolving skills and privacy focus
...
- **Tags:** `office`, `productivity`, `ui-extension`
- **Why use it:** Brings spreadsheets, docs, and slides into the Harness environment
...
- **Why use it:** Native iOS client connecting to Harness via WebSocket
...
- **Tags:** `desktop-app`, `electron`, `coding-agent`
- **Why use it:** CodePilot-inspired desktop client for macOS Windows and Linux
...
- **Tags:** `dsh-plugin`, `workbench`, `web`, `platform`
- **Why use it:** Plugin-based web workbench with global skill management and workspace.
...
- **Why use it:** Desktop bioinformatics agent with one-click installers and offline mode.
...
- **Why use it:** Persistent info bar showing model, balance and peak/off-peak pricing.
...
- **Tags:** `desktop`, `devops`, `electron`
- **Why use it:** All-in-one DevOps desktop combining DB, SSH, SFTP, Docker in one window.
...
- **Why use it:** Electron desktop wrapping DSH web with sidebar, ModLens, and more.
...
- **Why use it:** Open-source RAG platform turning documents into queryable knowledge
...
- **Tags:** `agent-memory`, `deepseek-harness`, `mcp`, `rag`, `long-term-memory`
- **Why use it:** Portable, local-first memory layer with Markdown-native cross-app knowledge storage
...
- **Tags:** `agent-memory`, `mcp`, `rag`, `long-term-memory`, `token-savings`
...
- **Why use it:** Dedicated memory library enabling stateful, persistent agent conversations
...
- **Tags:** `memory`, `rag`, `agent-framework`
- **Why use it:** Durable memory management kit enabling agents to remember and refine over time.
...
- **Tags:** `rag`, `long-term-memory`, `milvus`
- **Why use it:** Persistent unified memory layer for all AI agents backed by Milvus hybrid search.
...
- **Tags:** `memory-hub`, `mcp`, `local-first`
- **Why use it:** Personal AI agent and local memory hub giving all agents shared persistent context.
...
- **Tags:** `dsh-plugin`, `memory`, `context-management`
- **Why use it:** Proactive associative memory with zero-prompt recall before model speaks.
...
- **Tags:** `agent-orchestration`, `agent-skills`, `mcp`, `local-first`, `no-code`
- **Why use it:** Unified multi-platform agent workspace with board-based task tracking
...
- **Why use it:** Task-aware routing with injector and reasoning-mode router for optimized workflows
...
- **Tags:** `agent-skills`, `claude-skills`, `gemini-skills`, `skill-md`
- **Why use it:** Comprehensive skills marketplace with auto-curation from thousands of GitHub projects
...
- **Tags:** `agent-collaboration`, `multi-agent-workspace`, `visual-editor`, `enterprise-agent`
- **Why use it:** Enterprise multi-agent workspace with visual editor and team collaboration support
...
- **Tags:** `agent-os`, `loop-engineering`, `mcp`, `llm-orchestration`, `skills`
- **Why use it:** Self-evolving agent OS with staged evaluation and budgeted iterative improvement
...
- **Why use it:** Long-horizon agent control plane with durable governance for multi-harness workflows.
...
- **Tags:** `cognitive-enhancement`, `inference-control`, `agent-skills`
...
- **Why use it:** Enable multi-agent collaboration and team workflows directly within DSH.
...
- **Why use it:** Structured AI novel writing workflow with character/world management and revision.
...
- **Why use it:** Powerful vision toolkit for text-only models: OCR, multi-image Q&A, UI restoration.
...
- **Tags:** `agent-skills`, `skill-routing`, `vector-search`, `embeddings`
- **Why use it:** Curates scattered skills into retrieval-ready corpora with eval tooling
...
- **Tags:** `creative-writing`, `novel-writing`, `agent-skills`, `workflow`
- **Why use it:** Creative writing workflows for novels, short dramas, games, and video recaps
...
- **Why use it:** Gives agents vision and hearing via video/audio/screen analysis
...
- **Tags:** `task-management`, `workflow`, `sqlite`
- **Why use it:** Native local taskboard with SQLite and agent claim/review workflow
...
- **Tags:** `ios-simulator`, `automation`, `mobile`
- **Why use it:** Live iOS Simulator inside conversation with 22 automation tools
...
- **Why use it:** Brings Epistemic Loop to DSH for scientific discovery workflows
...
- **Tags:** `skill-audit`, `management`, `dsh-plugin`
- **Why use it:** Skill audit and in-app manager for DSH with call logging and control
...
- **Why use it:** Composable, reviewable agent modes enabling self-evolution workflows
...
- **Why use it:** Turns experience into repeatable agent skills across multiple AI platforms
...
- **Why use it:** Skill that discovers, installs and verifies DSH plugins from GitHub
...
- **Why use it:** AI coding mentor that scans architecture and guides step-by-step.
...
- **Tags:** `agent-skills`, `web-search`, `multi-agent`
- **Why use it:** Web search, research and finance skills for AI agents across platforms.
...
- **Tags:** `dsh-plugin`, `skills-deck`, `engineering`
- **Why use it:** 25 built-in engineering skills available via right sidebar panel.
...
- **Why use it:** Patent infringement analysis skill generating competitor reports.
...
- **Tags:** `dsh-plugin`, `gongwen`, `docx`, `wps`
- **Why use it:** Official Chinese document processing skill with formatting, review and templates.
...
- **Tags:** `dsh-plugin`, `legal`, `visualization`, `svg`
- **Why use it:** Legal visualization skills: timelines, case diagrams, trial battle maps.
...
- **Why use it:** Fusion of Bazi, Fengshui, IChing and TCM into a local deterministic agent skill.
...
- **Why use it:** Engineering discipline bundle for requirements check and quality gating.
...
- **Why use it:** Durable multi-agent system with persistent memory and cross-session skills.
...
- **Why use it:** 11 QA testing skills framework with shared knowledge base and decision matrix.
...
- **Why use it:** Enables cross-agent memory continuity for seamless task handoff
...
- **Why use it:** Adapts DSH code simplification patterns for Codex workflows
...
- **Why use it:** Separates discussion from execution for structured AI collaboration
...
- **Tags:** `math-modeling`, `cumcm`, `dsh-plugin`
- **Why use it:** Multi-agent math modeling suite with 5 roles, 33 models and templates
...
- **⭐ Stars:** 28
- **Language:** Python
- **Description:** 数学建模资源，包含辅助建模、数据预处理、编程、论文写作、降aigc、查重、科研绘图等关键skill。创建者使用skill的大模型为deepseek harness v4.1 pro，但并非仅有dsh适配，skill内涵skill.md文件、README.md文件，reference文件夹及所需依赖，可以利用��种大模型agent进行安装，此外，库内还上传了数学建模workbench的自定义agent，涵盖着数学建模建模手，编程手，论文手三者分工流程及其他要点，使用该agent便于减少数学建模相关提示词的输入，配合各类skill能完成较为高质量的数学建模论文
- **Tags:** `math-modeling`, `skill`, `chinese`
- **Why use it:** Comprehensive CN math modeling resource covering data, coding and writing
...
- **Why use it:** Standards-based agent skill for building DSH plugins efficiently.
...
- **Why use it:** Pre-flight plugin check for task-type agent requests
...
- **Tags:** `dsh-plugin`, `api-documentation`, `mcp`
- **Why use it:** Search 65,600+ API docs from 20 Chinese open platforms zero-config.
...
- **Tags:** `dsh-plugin`, `huaweicloud`, `mcp`, `cloud`
- **Why use it:** Official Huawei Cloud SDK with MCP tools, skills and cloud sandbox.
...
- **Tags:** `vibedesign`, `ui`, `react-native`, `flutter`, `mcp`, `skill`
- **Why use it:** Open-source AI-native vector design tool with concurrent agent teams for Design-as-Code
...
- **Why use it:** Native AI design studio with visual editor, template marketplace, and PPT generation.
...
- **Tags:** `dsh-plugin`, `visual`, `ui-components`, `interactive`
- **Why use it:** Generates interactive UI components inline in assistant replies — charts, forms, quizzes
...
- **Why use it:** Pixel-to-text OCR and image scanning tools for text-only LLM models.
...
- **Why use it:** AI video/image/audio skills with configurable vision and media model tools.
...
- **Tags:** `kline`, `finance`, `deepseek-harness-plugin`
- **Why use it:** Interactive K-line analysis with indicators, support/resistance levels
...
- **Tags:** `video`, `motion-design`, `dsh-plugin`
...
- **Why use it:** Real logged-in browser automation for AI agents without disrupting user workflow
...
- **Why use it:** Self-evolving cross-platform content discovery agent from major Chinese and global platforms.
...
- **Tags:** `web-search`, `web-fetch`, `agent-skills`, `agentic-workflow`
...
- **Why use it:** Anti-detect browser with persistent identity, fingerprint spoofing, Playwright
...
- **Tags:** `low-code`, `crm`, `internal-tools`
- **Why use it:** AI-powered low-code platform for rapid business system building
...
- **Tags:** `marketplace`, `dsh-plugin`, `plugin-browser`
- **Why use it:** Built-in DSH plugin marketplace with one-click install and search.
...
- **Tags:** `account-management`, `multi-model`, `smart-routing`
- **Why use it:** Unified account management and smart multi-model routing with automatic failover.
...
- **Why use it:** Ecosystem radar discovering and k8s-validating 21k+ DSH plugin candidates.
...
- **Why use it:** Curated comprehensive list of DSH ecosystem plugins, tools, and community resources.
...
- **Tags:** `marketplace`, `plugin-store`, `discovery`
- **Why use it:** Comprehensive daily-refreshed plugin marketplace with pre-publishing review process.
...
- **Why use it:** Uses ChatGPT Claude Grok subscriptions as DSH LLM providers
...
- **Tags:** `llm-provider`, `model-access`, `utility`
...
- **Why use it:** Secure P2P remote access for DSH across PC, Android and Web devices
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `skill-management`
- **Why use it:** Dynamic skill/tool exposure management reduces context bloat.
...
- **Tags:** `dsh-plugin`, `plugin-management`, `infrastructure`
- **Why use it:** Lightweight plugin console and official dev guide for DSH ecosystem.
...
- **Why use it:** Context injection audit tool with token cost analysis and Web UI dashboard.
...
- **Tags:** `mcp`, `config-sync`, `dsh`, `claude-code`
- **Why use it:** One-click MCP, Skills and config sync across 7+ AI coding tools
...
- **Tags:** `dsh-plugin`, `cordis`, `infrastructure`
...
- **Why use it:** Migration wizard for Claude Code, Codex sessions into DSH
...
- **Tags:** `terminal`, `keybindings`, `trace`
- **Why use it:** Task notifications, LLM trace inspection, and keybindings with built-in terminal.
...
- **Tags:** `cost-tracking`, `visualization`, `token`
...
- **Tags:** `troubleshooting`, `conflict-resolution`
...
- **Why use it:** Search npm and curated list inside DSH agents
...
- **Tags:** `mascot`, `pixel-art`, `ui-enhancement`
- **Why use it:** Animated pet gallery that personalizes your coding agent interface.
...
- **Tags:** `skin`, `theme`, `whale-girl`
- **Why use it:** Charming whale girl skin series for DeepSeek Harness personalization.
...
- **Tags:** `theme`, `ui`, `glass-morphism`, `customizable`
- **Why use it:** Highly customizable glass-morphism theme for DSH web with frosted blur effects
...
- **Tags:** `theme`, `animated`, `chrome-extension`
- **Why use it:** Dynamic ocean theme with adjustable waves and glassmorphism
...
- **Tags:** `webui`, `background`, `themes`
- **Why use it:** Customizable web UI backgrounds with real-time preview
...
- **Why use it:** 530+ GPT Image prompts and 20+ industrial templates for reference
...
- **Tags:** `documentation`, `deepwiki`, `code-analysis`
...
- **Tags:** `arxiv`, `latex`, `research`, `experiment-tracking`
...
- **Why use it:** Learn DSH principles and browse plugin marketplace best practices
...
- **Why use it:** Cross-platform academic search with citation graph and verification via MCP
...
- **Why use it:** Curated plugin directory for DSH ecosystem exploration
...
- **Why use it:** Plugin manual with installation and compatibility guides
...
- **Tags:** `ai-curated`, `recommendation`
- **Why use it:** AI-curated 190+ plugins across 12 categories hourly
...
- **Tags:** `radar`, `analysis`
- **Why use it:** Evidence-backed plugin radar and analysis tool
...
- **Why use it:** Curated plugin showcase list — best starting point for exploring DSH ecosystem
...
- **Tags:** `agent`, `dsh-plugin`, `harness-engineering`, `llm`
- **Why use it:** Beginner-friendly tutorial from zero to harness engineering fundamentals
...
- **Tags:** `cybersecurity`, `security`, `vulnerabilities`, `redteam`
...
- **Tags:** `knowledge-base`, `awesome-list`, `mcp`, `agent`
- **Why use it:** Comprehensive AI resource encyclopedia: skills, MCP, RAG, agents, learning paths
...
- **Why use it:** Curated plugin discovery with real reviews and scenario classification
...
- **Tags:** `awesome-list`, `resources`, `reference`
- **Why use it:** Comprehensive curated list of DSH plugins skills MCP servers and tools
...
- **Why use it:** Plugin marketplace with 11,000+ plugins, search, rankings and free API
...
- **Why use it:** Comprehensive handbook covering runtime, plugins, MCP and 74 resource guides
...
- **Why use it:** Bridges Pi ecosystem to DSH, running Pi extensions as native plugins
...
- **Why use it:** Plugin directory cataloging 1,958 repos with bilingual search and filtering
...
- **Why use it:** 基于tushare构建本地财经数据库。AI原生的多Agent金融分析系统，支持skill拓展Agent能力并自适应生成http接口与mcp调用。支持本地数据库访
...
- **Why use it:** Complete reverse-skill (87 SKILL.md) as 
...
- **Tags:** `dsh-ecosystem`, `plugin-marketplace`, `awesome-list`
...
- **Why use it:** Curated plugin list to help discover quality DSH extensions quickly.
...
- **Why use it:** Spam-filtered open-data registry with verified plugins and skills.
...
- **Tags:** `awesome-list`, `verified`, `curation`
- **Why use it:** Evidence-led directory with verified loadable extensions and installation guidance.
...
- **Why use it:** 2026 curated plugin list with quality checks for reliability.
...
- **Why use it:** Collection of useful DSH plugins in Chinese, easy to browse.
...
- **Tags:** `collection`
- **Why use it:** Curated DSHA selected plugin collection
...
- **Tags:** `awesome-list`, `plugin-directory`
- **Why use it:** Curated DSH plugin directory — useful discovery resource.
...
- **Why use it:** DSH plugins listing with ecosystem analytics tags.
...
- **Why use it:** Deprecated — redirected to another repo.
...
- **Why use it:** Auto-updating bilingual plugin list with YAML data.
...
- **Tags:** `plugin-directory`, `curated-list`
- **Why use it:** Curated plugin registry covering models, tools, and UI.
