...
|--------|-------|
| **Plugins tracked** | 200 |
| **Total community stars** | 1,081,777 ⭐ |
| **Categories** | 11 |
...
| Plugin | Stars | Why |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 233,381 | Core plugin system — everything is a plugin, essen |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,654 | Open-source design tool with UI generator and loca |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 73,074 | Original agent harness for deploying multi-agent s |
...
- **Why use it:** Core plugin system — everything is a plugin, essential for DSH users.
...
- **Why use it:** Original agent harness for deploying multi-agent swarms and workflows.
...
- **Tags:** `ai-agent`, `multi-model`, `dsh-plugin`
...
- **Tags:** `agent-orchestration`, `multi-platform`, `coding-agent`
- **Why use it:** Unified agent workspace across desktop, mobile, browser and API
...
- **Tags:** `routing`, `injector`, `reasoning-mode`
...
- **Why use it:** Enterprise multi-agent workbench with unified workspace across engines
...
- **Tags:** `terminal`, `sandbox`, `vfs`
- **Why use it:** World's first virtual terminal providing isolated agent computing environments
...
- **Why use it:** Model-agnostic inference control for deep reasoning and long-horizon tasks.
...
- **Tags:** `agent-runtime`, `rust`, `coding-agent`
...
- **Tags:** `vision`, `ocr`, `computer-vision`, `gui-automation`
...
- **Tags:** `coding-agent`, `rpc`, `sub-agents`, `mcp`, `tui`
- **Why use it:** Coding agent with RPC, sub-agents and MCP integration for DSH
...
- **Why use it:** Skill call audit and in-app skill manager for DSH transparency.
...
- **Tags:** `plugin-creator`, `coding`, `developer-tools`
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `codex`
- **Why use it:** Codex skill adapted from DeepSeek Harness simplifications, practical refactoring.
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `godot`
- **Why use it:** Godot Engine 4.x game dev skill, registers at runtime, GDScript native support.
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `migration`
- **Why use it:** Four-source migration wizard for Claude Code/Codex/OpenCode/Hermes sessions.
...
- **Why use it:** Fast, simple Neovim plugin manager written in Lua — essential for DSH coding workflows.
...
- **Why use it:** Modern desktop solution for DSH where even the desktop is a plugin.
...
- **Tags:** `desktop`, `dsh`, `ui`
- **Why use it:** Official desktop client for DeepSeek Harness with native UX
...
- **Why use it:** Feature-rich TUI with whale bar, live status, and streaming thought display
...
- **Tags:** `floating-widget`, `deepseek-harness`, `ui`
...
- **Why use it:** Lightweight Tauri desktop app with preset plugins, zero setup needed.
...
- **Tags:** `desktop-app`, `gui`, `electron`, `ssh-client`
- **Why use it:** Zero-setup Windows desktop with GUI, SSH, and plugin support
...
- **Why use it:** Local-first desktop workspace for DSH sessions and Office automation
...
- **Tags:** `canvas`, `session-explorer`, `non-linear`, `visual`
- **Why use it:** Visual non-linear canvas for branching conversation and session exploration
...
- **Why use it:** Privacy-first local AI desktop app, great Claude Cowork alternative
...
- **Tags:** `ios`, `mobile`, `kotlin-multiplatform`
- **Why use it:** Native iOS client for DSH with real-time agent execution tracking
...
- **Why use it:** Open-source Smartisan-style notes app with Docker, DSH plugin and WeChat support.
...
- **Tags:** `mcp`, `ui`, `skills`
- **Why use it:** Web UI panel for managing skills and MCP servers directly in DSH interface
...
- **Tags:** `desktop-app`, `electron`, `typescript`, `ui`
- **Why use it:** Electron desktop console for multi-tab management of Harness, Plugins, MCP and Skills
...
- **Tags:** `desktop-app`, `bioinformatics`, `tauri`, `single-cell`
- **Why use it:** Bioinformatics agent desktop with one-click installers, skill store and offline mode
...
- **Why use it:** Replaces composer stats row with model, balance, and pricing info
...
- **⭐ Stars:** 32
- **Language:** TypeScript
- **Description:** DSH 上下文注入审计插件：统计 AGENTS.md 指令链/��能目录/工具 schema 的 token 成本，检测重复与冲突；Web UI 圆环面板 + context_audit 工具。Context Doctor for DeepSeek Harness: audit instruction-chain / skill catalog / tool schemas token cost.
- **Tags:** `context`, `ui`, `audit`, `tool`
- **Why use it:** Web UI audit panel for context injection, token costs, duplicates and conflicts
...
- **Tags:** `desktop`, `electron`, `dsh-web`
- **Why use it:** Electron desktop shell wrapping DSH web with sidebar, UI, ModLens, and market integrations.
...
- **⭐ Stars:** 5
- **Language:** JavaScript
- **Description:** DSH Web 背景个性化插件：上传自己的图片（JPG / PNG / WEBP / GIF，浏览器端自动压缩到 1600px 以内）或一键切换极光、余烬、宣纸三种预设氛围；实时预览所见即所得，支持细调图像存在感、暗色遮罩、柔焦、适配方式与焦点位置；上传即自动保存到 DSH 设置，重启后原样恢复，浅色 / 深色主题均正常；侧栏、消息气泡、输入框保持原样不遮挡，浮层菜单不受影响；全程本地处理不上传任何服务器，关闭开关或一键恢复默认即可��全移除；内置中英文双语界面。
- **Tags:** `webui`, `dsh-plugin-market`
- **Why use it:** Customize DSH Web background with uploads or presets, real-time preview
...
- **Tags:** `dsh-plugin`, `awesome-dsh-plugin`
...
- **Why use it:** LLM knowledge platform turning documents into RAG, agent, and Wiki.
...
- **Why use it:** Local-first Markdown memory layer enabling cross-app agent persistence
...
- **Why use it:** Self-evolving memory with hybrid retrieval, cuts tokens by 35%
...
- **Tags:** `agent-memory`, `continual-learning`, `stateful`
- **Why use it:** Memory library for building stateful, continually learning agents
...
- **Why use it:** Agent memory management kit for persistent and refined context recall
...
- **Tags:** `memory`, `rag`, `milvus`, `agent-memory`
...
- **Tags:** `vector-search`, `embeddings`, `skill-reranking`, `information-retrieval`
- **Why use it:** Turns scattered skill docs into searchable agent knowledge corpus
...
- **Tags:** `knowledge-base`, `mcp`, `rag`, `ai-skills`
...
- **Tags:** `ai-writing`, `webnovel`, `creative-workflow`
- **Why use it:** Long-form web novel writing assistant solving AI遗忘 and幻觉
...
- **Tags:** `skills`, `marketplace`, `coding-agent`
- **Why use it:** Comprehensive skills marketplace with curated packages across domains
...
- **Tags:** `vision`, `ocr`, `agent`
- **Why use it:** Vision bridge enabling image-to-JSON for text-only coding agents
...
- **Tags:** `novel-writing`, `creative-writing`, `rag`
- **Why use it:** End-to-end AI novel writing workflow with worldbuilding and chapter management.
...
- **⭐ Stars:** 396
- **Language:** Python
- **Description:** DeepSeek 写网文/小说的工作流插件：DeepSeek Harness 社区插件，内置小说、短剧、���戏、视频解说四个工作台 | Community DeepSeek Harness plugin with novel, short-drama, game and video-recap workbenches.
- **Tags:** `creative-writing`, `novel-writing`, `screenwriting`, `workflow`
- **Why use it:** Creative writing workflow: novels, short dramas, games and video scripts
...
- **Tags:** `agent-observability`, `computer-vision`, `video-understanding`
- **Why use it:** Give agents real sensory perception with verifiable video/audio results
...
- **Tags:** `office`, `productivity`, `agent-skills`
- **Why use it:** Brings full office suite (sheets, docs, slides) into DSH agents
...
- **Tags:** `epistemic-loop`, `scientific-discovery`, `agent-skill`
- **Why use it:** Implements Epistemic Loop for structured scientific discovery in agents
...
- **Tags:** `task-management`, `native-ui`, `productivity`
...
- **Why use it:** Enables composable, reviewable agent workflows with self-evolution support.
...
- **Why use it:** Turns experience into repeatable execution across multiple AI agents.
...
- **Why use it:** Comprehensive reverse engineering and pentesting skill pack with 87 SKILL.md
...
- **Why use it:** Specialized AI infra dev skills for writing, optimizing and debugging systems
...
- **Why use it:** AI coding mentor that scans projects and guides hand-written code with auto-grading
...
- **Why use it:** 25 pre-installed engineering skills backed by 30B token corpus, 10x dev efficiency
...
- **Tags:** `agent-skills`, `web-search`, `mcp`
- **Why use it:** Web search, content extraction, research and finance skills for AI agents
...
- **Why use it:** Unified local Agent Skills loader and manager for DSH with safe management
...
- **Tags:** `skill`, `docx`, `gongwen`, `wps-office`
- **Why use it:** Full-process Chinese official document tool following GB/T 9704 standard
...
- **⭐ Stars:** 43
- **Language:** TypeScript
- **Description:** 中华传统智慧（玄枢）AI Agent 技能包的 DeepSeek Harness（dsh）Cordis 插件：八字/紫��/六爻/梅花/奇门/风水/五运六气/体质全融合，本地确定性引擎 + 可视化 Dashboard，一行 dsh plugin add 安装。
- **Tags:** `ai-agent`, `chinese-traditional-wisdom`, `skill`
- **Why use it:** Fusion Bazi/Fengshui/IChing agent skill with local deterministic engine
...
- **Why use it:** Official docs archive and plugin dev knowledge base as on-demand skill
...
- **Why use it:** Engineering quality gate: grill requirements, test implementation before ship
...
- **Tags:** `math-modeling`, `research`, `paper-writing`
- **Why use it:** Math modeling suite: preprocessing, coding, paper writing, visualization
...
- **Why use it:** Durable multi-agent teams with cross-session memory and skills
...
- **Why use it:** OCR and pixel-to-text image reading skill for text-only models
...
- **Why use it:** Multi-agent team with independent models, skills, MCP tools, shared workspace
...
- **Why use it:** 11 QA skills framework with test automation and case writing for agents
...
- **Why use it:** 5-role multi-agent math modeling suite with 33 model templates for competitions
...
- **Tags:** `planning`, `agent-skills`, `workflow`
- **Why use it:** Separates discussion from action for AI collaborative task management
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `skillhub`
- **Why use it:** Official SkillHub API docs with DSH examples, practical for plugin dev.
...
- **Why use it:** WorkBuddy-style skill picker for composer, one-click /skill-name insert.
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `agent-handoff`
- **Why use it:** Cross-platform continuity memory for agent handoff between Codex/Claude.
...
- **Tags:** `agent-skill`, `dsh-plugin`, `claude-code`
- **Why use it:** Agent skill for DSH plugin development with standards for plugins/services/events/tools.
...
- **Tags:** `tts`, `xiaomi-mimo`, `dsh-plugin`
- **Why use it:** Free Xiaomi MiMo TTS integration with preset/custom/browser voices for DSH agent readout.
...
- **Why use it:** Pre-flight plugin library check for DSH agents, boosts utilization predictability
...
- **Why use it:** Open-source design tool with UI generator and local-first desktop.
...
- **Tags:** `diagrams`, `architecture`, `dsh-plugin`
- **Why use it:** Generate beautiful architecture and sequence diagrams as self-contained HTML.
...
- **Tags:** `image-prompts`, `gpt-image`, `dsh-plugin`
- **Why use it:** 530+ image prompt cases and 20+ industrial templates with comparison tool.
...
- **Tags:** `agent`, `ui`, `design`, `vibecoding`
...
- **Tags:** `design`, `prototyping`, `ppt`, `visual`
- **Why use it:** AI-powered design studio with templates, prototyping, and PPT support.
...
- **Why use it:** Interactive UI components: charts, forms, quizzes rendered in-chat
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `ai-video`, `directorx`
- **Why use it:** AI video/image generation with storyboard and text-to-video pipeline.
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `remotion`, `video`
- **Why use it:** Film-style product video with 152 shot recipes, Remotion templates, audio assets.
...
- **Why use it:** Real-time token usage, cost estimates, and API balance visualization for DSH Web.
...
- **Why use it:** Multi-platform browser extension enhancing AI chat with prompt management
...
- **Tags:** `browser`, `content`, `ai-agent`
- **Why use it:** Local-first cross-platform AI content discovery agent across major platforms
...
- **Tags:** `web-search`, `web-fetch`, `free`, `no-signup`
- **Why use it:** Free web search bridge for any model without native web access
...
- **Why use it:** Anti-detect browser with fingerprint spoofing and unlimited profiles
...
- **Why use it:** Tabbit browser automation plugins with Playwright integration
...
- **Why use it:** Zero-dependency static web portal to discover and filter 227+ plugins
...
- **Why use it:** Ultimate image uploader supporting 60+ hosting services, works with Obsidian, VS Code etc.
...
- **Tags:** `marketplace`, `dsh`
- **Why use it:** Built-in DSH plugin marketplace for browsing and one-click installation
...
- **Tags:** `preset`, `dsh`
- **Why use it:** Two-phase preset for minimal bootstrap then full standard tooling
...
- **Tags:** `account`, `routing`, `multi-model`
...
- **Tags:** `api-keys`, `mcp`, `proxy`, `credentials`
- **Why use it:** Centralized API key registry and proxy for agent tools, like OpenRouter.
...
- **Why use it:** Comprehensive context insight and management with dashboard and sidebar.
...
- **Tags:** `cli`, `plugin-manager`, `schemas`, `developer-tools`
- **Why use it:** CLI and resolver tools for managing DSH Plugin Hub ecosystem
...
- **Tags:** `cybersecurity`, `aisecurity`, `vulnerability`, `redteam`
- **Why use it:** AI security audit: detects hallucinated packages and vulnerable patterns
...
- **Why use it:** Secure P2P multi-device remote access for DSH across PC, Android, Web.
...
- **Why use it:** Bridges Pi and DSH ecosystems via unified ABI for seamless plugin migration.
...
- **Why use it:** Auto-upgrades plugins and skills alongside DSH version releases
...
- **Tags:** `cordis`, `mcp`, `skills`
- **Why use it:** Unified capability management with persistent/on-demand/disable exposure strategies
...
- **Tags:** `console`, `plugin-management`, `infrastructure`
- **Why use it:** Plugin ecosystem infrastructure: browser-panel console and official dev guide skill
...
- **Tags:** `workbench`, `plugin-system`, `skills`, `typescript`
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `dshbox`
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `launcher`
- **Why use it:** Unlock DSH plugin-native core capabilities, unified Runtime/Provider/MCP orchestration.
...
- **Tags:** `terminal`, `keybindings`, `trace`, `notifications`
- **Why use it:** Curated DSH enhancements: task notifications, LLM trace, keybindings, and built-in terminal.
...
- **Tags:** `debug`, `conflict-resolution`, `dsh-plugin`
- **Why use it:** Detects and resolves plugin version conflicts in DSH — essential for stable plugin ecosystems.
...
- **Why use it:** Search npm + awesome list for plugin discoverability inside DSH agents
...
- **Tags:** `ai-tutorials`, `resources`, `dsh-plugin`
- **Why use it:** Comprehensive AI resource hub with tutorials for DeepSeek, GPT, Claude and more.
...
- **Why use it:** Generate accurate technical docs and AI-ready context from your codebase
...
- **Tags:** `arxiv`, `latex`, `research-tools`, `gpu-cluster`, `experiment-tracking`
- **Why use it:** Full-cycle academic research tool: LaTeX, arXiv, GPU SSH orchestration
...
- **Why use it:** Auto-curated daily plugin discovery list with real filtering and rankings
...
- **Tags:** `awesome-list`, `curated`, `resource-guide`
- **Why use it:** Comprehensive curated list covering plugins, MCPs, skills and patches
...
- **Tags:** `awesome-list`, `guide`, `chinese`
- **Why use it:** Ultimate Chinese DSH guide with quickstart, plugins and toolkit resources
...
- **Tags:** `academic`, `mcp`, `citation`
- **Why use it:** Cross-database academic search with citation graphs and Chinese support
...
- **Why use it:** Essential DSH learning resource with marketplace and best practices guide.
...
- **Tags:** `agent-runtime`, `mcp`, `handbook`
- **Why use it:** Comprehensive agent-first handbook covering runtime, plugins, MCP and more.
...
- **Why use it:** Methodology-driven industry research skills with policy and news tracking
...
- **⭐ Stars:** 61
- **Language:** Python
- **Description:** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：��凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（大事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **Tags:** `visualization`, `legal`, `diagram`, `svg`
- **Why use it:** Visual litigation tools: timeline, event charts and legal relationship diagrams for lawyers
...
- **Tags:** `deepseek-harness`, `dsh-plugin`, `kline`, `ftshare`
- **Why use it:** Interactive financial K-line analysis with multi-market and technical indicators.
...
- **Why use it:** 24 verified stock/finance/quant plugins for DSH — bilingual, vertically focused.
...
- **Why use it:** Comprehensive star-ranked DSH plugin directory with 1000+ repos
...
- **Why use it:** Plugin directory with install, permission and risk guidance
...
- **Why use it:** Daily auto-crawled bilingual DSH plugin directory via GitHub Actions
...
- **Tags:** `awesome-list`, `dsha`
- **Why use it:** Curated DSHA精选插件合集集合
...
- **Why use it:** Privacy-focused resume builder, fully open-source and self-hostable.
...
- **Tags:** `no-code`, `crm`, `dsh-plugin`
- **Why use it:** Open-source AI + no-code platform for rapidly building business systems.
...
- **Why use it:** Curated catalog of all DSH plugins — essential discovery resource
...
- **Why use it:** Plugin aggregation ecosystem enabling creative workshop distribution
...
- **Tags:** `marketplace`, `plugin-shop`, `review`
- **Why use it:** Daily-refreshed comprehensive plugin marketplace with pre-publishing reviews.
...
- **Why use it:** Bilingual curated plugin directory for DSH ecosystem discovery
...
- **Why use it:** Massive plugin directory with search, rankings, and public API access.
...
- **Why use it:** Large curated plugin catalog with search, filters, and STAR sorting.
...
- **Tags:** `plugin-marketplace`, `awesome-list`, `dsh-ecosystem`
...
- **Tags:** `awesome-list`, `plugins`, `resource`
...
- **Tags:** `deepseek-harness`, `dsh-plugin`
- **Why use it:** Unofficial community catalog for DSH plugins, curated resource aggregation.
...
- **Tags:** `deepseek-harness`, `awesome-list`
- **Why use it:** Curated awesome list of DSH plugins, tools, skills & resources — everything is a plugin.
...
- **Tags:** `deepseek-harness`, `awesome-list`
- **Why use it:** Curated list of DSH plugins — 精选DeepSeek Harness插件列表。
...
- **Why use it:** Curated open-data plugin registry with spam filtering for DSH ecosystem discovery.
...
- **Why use it:** Chinese-curated collection of useful DeepSeek Harness plugins for quick reference.
...
- **Why use it:** 2026 curated DSH plugin list with quality checks — stay updated with the latest.
...
- **Tags:** `awesome-list`, `plugin-ecosystem`, `cli`
- **Why use it:** Curated DSH plugin ecosystem list covering CLI, web UI, and autonomous agent tools.
...
- **Tags:** `dsh-plugin-market`, `dsh-plugin-bundle`
...
- **Why use it:** Curated directory of 80+ verified DSH plugins for quick discovery
...
- **Why use it:** DSH plugin repository with bundle, verify, and analytics topics
...
- **Tags:** `awesome-list`, `moved`, `dsh-plugins`
- **Why use it:** Archived list; redirected to new repo — check for updates
...
- **Tags:** `awesome-list`, `auto-updating`, `dsh-plugins`
