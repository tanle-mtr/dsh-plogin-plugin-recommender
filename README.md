...
|--------|-------|
| **Plugins tracked** | 200 |
| **Total community stars** | 1,081,984 ⭐ |
| **Categories** | 12 |
...
| Plugin | Stars | Why |
|--------|-------|-----|
| [deepseek-ai/deepseek-harness](https://github.com/deepseek-ai/deepseek-harness) | ⭐ 233,403 | Core DSH platform, everything is a plugin |
| [nexu-io/open-design](https://github.com/nexu-io/open-design) | ⭐ 97,663 | Open-source design alternative, local-first deskto |
| [ruvnet/ruflo](https://github.com/ruvnet/ruflo) | ⭐ 73,078 | Multi-agent swarm orchestration framework |
...
- **Why use it:** Multi-agent super assistant with self-evolving memory
...
- **Tags:** `coding-agent`, `dsh-plugin`, `tui`
...
- **Why use it:** Unified agent workspace with task boards across desktop, mobile, and web.
...
- **Tags:** `routing`, `orchestration`, `agent`
- **Why use it:** Task-aware reasoning-mode router with injector for advanced agent orchestration.
...
- **Tags:** `vision`, `ocr`, `multimodal`, `agent-tools`
- **Why use it:** First vision plugin for DSH — paste images to get structured JSON evidence
...
- **Tags:** `preset`, `harness-tools`, `deepseek-harness`
- **Why use it:** Two-phase DSH preset: minimal bootstrap then full Standard tools setup
...
- **Why use it:** World's first virtual terminal for AI agents with VFS and sandboxing
...
- **Tags:** `agent-tools`, `routing`, `multi-model`, `harness`
- **Why use it:** Multi-model account manager with smart routing, priority, and failover support
...
- **Tags:** `agent-runtime`, `coding-assistant`, `dsh-plugin`
...
- **Why use it:** Feature-rich coding agent with RPC plugin, sub-agents, hashline edits, and MCP integration.
...
- **Tags:** `security`, `auditing`, `vulnerability`, `redteam`
...
- **Tags:** `ai-agents`, `migration`, `dsh-plugin`
- **Why use it:** Bridge Pi ABI to native DSH plugins seamlessly
...
- **Why use it:** Official docs archive with plugin dev scaffold, essential for creators
...
- **Tags:** `context`, `audit`, `ui`, `tooling`
- **Why use it:** Context injection auditor with Web UI ring panel, detects duplicate tokens and conflicts
...
- **Why use it:** Fast, simple Neovim plugin manager written in Lua for efficient code workflows
...
- **Why use it:** Modern desktop solution for DSH plugin ecosystem
...
- **Why use it:** Official desktop client bringing DSH to your native OS environment.
...
- **Tags:** `taskboard`, `ui`, `panel`, `desktop`
...
- **Tags:** `tui`, `ui`, `terminal`, `status`
- **Why use it:** TUI enhancement with whale bar, live status, streaming thought, and TPS display
...
- **Why use it:** Floating whale widget tracking DeepSeek account balance with animations.
...
- **Tags:** `desktop`, `gui`, `ssh`, `electron`
- **Why use it:** Official-style desktop GUI for DSH with SSH, plugins, and mobile remote access built in.
...
- **Tags:** `desktop`, `local-first`, `mcp`, `office`
- **Why use it:** Local-first desktop workspace integrating DSH sessions, files, web research, and Office artifacts.
...
- **Tags:** `ui`, `charts`, `interactive`, `components`
- **Why use it:** Renders interactive inline UI—charts, forms, quizzes—directly inside assistant chat responses.
...
- **Tags:** `canvas`, `conversation`, `visual`, `branching`
- **Why use it:** Canvas-based non-linear conversation explorer enabling branching and visual session navigation.
...
- **Tags:** `desktop-app`, `agent-framework`, `local-first`
- **Why use it:** Local-first multi-model desktop agent app with self-evolving skills.
...
- **Tags:** `ios`, `mobile`, `native-client`
- **Why use it:** Native iOS client connecting to DSH via WebSocket for on-the-go access.
...
- **Why use it:** Smartisan-style notes with Docker deploy and skill calls
...
- **Why use it:** Electron desktop console for multi-tab DSH/MCP/Skills management
...
- **Why use it:** Bioinformatics agent desktop with offline mode and skill store
...
- **Tags:** `ui`, `stats-bar`, `usage-tracking`
...
- **Why use it:** Electron desktop shell integrating DSH web service with sidebar and ModLens
...
- **Tags:** `ui`, `theme`, `webui`, `background`
...
- **Why use it:** One-click compact mode for DSH context meter panel.
...
- **Tags:** `ui`, `knowledge-card`, `chat`
- **Why use it:** Transform chat history into beautifully designed shareable knowledge cards.
...
- **Why use it:** LLM knowledge platform with RAG and wiki capabilities
...
- **Why use it:** Portable local-first memory layer with Markdown-native long-term storage.
...
- **Tags:** `agent-memory`, `stateful-agents`, `memory`
- **Why use it:** Essential memory library for building stateful, continual-learning agents.
...
- **Why use it:** Agent memory management kit — remember and refine context across sessions
...
- **Tags:** `documentation`, `wiki`, `knowledge`, `rag`
- **Why use it:** Generate accurate technical docs and AI-ready context from code in minutes
...
- **Tags:** `agent-memory`, `rag`, `milvus`, `long-term-memory`
- **Why use it:** Persistent memory layer backed by Markdown and Milvus for all agents.
...
- **Tags:** `rag`, `embeddings`, `skill-management`, `vector-search`
- **Why use it:** Turns scattered SKILL.md files into indexed, semantically searchable agent corpora with evaluation.
...
- **Why use it:** Official DSH learning hub with plugin marketplace
...
- **Tags:** `awesome-list`, `catalog`, `marketplace`
- **Why use it:** 11000+ plugin registry with search and free API
...
- **Tags:** `agent-runtime`, `awesome-list`, `mcp`
- **Why use it:** 173-source handbook: runtime, MCP, sandbox, eval
...
- **Why use it:** 1958-repo plugin catalog with STAR ranking and filters
...
- **Tags:** `agentic-workflow`, `mcp-server`, `dsh-plugin`
- **Why use it:** Multi-agent swarm orchestration framework
...
- **Why use it:** Distill human thinking into reusable agent skills
...
- **Tags:** `writing`, `webnovel`, `skill`
- **Why use it:** Specialized skill for 2M-word web novel writing with forgetting mitigation.
...
- **Tags:** `skills`, `store`, `community`
- **Why use it:** Largest skill store with auto-crawled GitHub skills across all categories.
...
- **Why use it:** Long-horizon agent control plane with durable workflow orchestration
...
- **Why use it:** Intelligent skill routing and workflow orchestration — 21pp reward gain
...
- **Tags:** `ai-writing`, `novel-writing`, `rag`, `worldbuilding`
- **Why use it:** Organized workflow for novel writing with worldbuilding and RAG support.
...
- **Tags:** `creative-writing`, `novel`, `workflow`, `drama`
- **Why use it:** Dedicated creative writing workflows for novels, short dramas, games, and video scripts with four workbenches.
...
- **Tags:** `agent-skills`, `computer-vision`, `audio-understanding`
- **Why use it:** Gives agents multimodal perception with video, audio, and screen analysis.
...
- **Tags:** `ai-agents`, `compatibility-layer`, `dsh`
- **Why use it:** Full-stack math modeling auto-pilot with GitOps pipeline
...
- **Tags:** `cordis`, `dsh-plugin`, `skills`
- **Why use it:** Skill audit and in-app manager for DSH
...
- **Why use it:** Composable, reviewable self-evolving agent modes
...
- **Why use it:** Turn experience into repeatable AI execution flows
...
- **Why use it:** 88 open-source skills for research/marketing workflows
...
- **Why use it:** Auto-find/install/verify GitHub DSH plugins via skill
...
- **Why use it:** Multimodal vision for text-only LLM backends
...
- **Tags:** `agent-skill`, `coding`
- **Why use it:** AI coding mentor that generates step-by-step chapters and code reviews
...
- **Tags:** `skills`, `agent`
- **Why use it:** 25 pre-installed engineering skills with 10x dev efficiency boost
...
- **Tags:** `agent-skills`, `web-search`
- **Why use it:** Web search, research, finance skills from You.com for agents
...
- **Tags:** `skill`, `gongwen`, `docs`
- **Why use it:** Full workflow tool for Chinese official document formatting per GB/T 9704
...
- **Tags:** `ai-agent`, `bazi`, `fengshui`, `tcm`
- **Why use it:** Unique fusion of Bazi, IChing, Fengshui, TCM into one deterministic engine
...
- **Why use it:** Engineering discipline guard: validates requirements, tests, and delivery quality
...
- **Why use it:** Full pipeline for math modeling: data, code, paper writing, AIGC detection
...
- **Why use it:** Durable multi-agent team with persistent memory and cross-session skills
...
- **Why use it:** Pixel-to-text OCR skill for text-only models, enables vision via tooling
...
- **Tags:** `multi-agent`, `mcp`, `collaboration`
- **Why use it:** Multi-agent team with independent models, MCP tools, and shared workspace
...
- **Why use it:** 11 QA skills with shared knowledge base for AI-assisted testing and automation
...
- **Why use it:** 5-role multi-agent math modeling system with 33 model library and paper templates
...
- **Why use it:** Separation-of-planning-and-execution mechanism for collaborative AI task workflows
...
- **Tags:** `skillhub`, `documentation`, `deepseek-harness`
...
- **Why use it:** Cross-platform session continuity memory for agent handoff workflows
...
- **Why use it:** Adapted code simplification skill inspired by official DSH skill
...
- **Why use it:** Standardized agent skill for developing DSH plugins with full adapter support
...
- **Tags:** `agent-tool`, `workflow`, `automation`
...
- **Tags:** `mcp`, `api-keys`, `credentials`, `secrets`
- **Why use it:** OpenRouter for agent tools — MCP proxy with secret registry management.
...
- **Tags:** `api`, `mcp`, `chinese`
- **Why use it:** Search 65,600+ Chinese API docs across 20 open platforms
...
- **Tags:** `agent-skills`, `ai-design`, `dsh-plugin`
- **Why use it:** Open-source design alternative, local-first desktop
...
- **Why use it:** Self-contained architecture diagram generation
...
- **Tags:** `ai-image-generation`, `dsh-plugin`, `skills`
- **Why use it:** 530+ image prompts with industrial templates
...
- **Tags:** `agent`, `ui`, `vibedesign`, `design`
- **Why use it:** Open-source AI-native vector design tool with Agent Teams and Design-as-Code
...
- **Why use it:** AI-native design studio with PPT generation, prototyping and templates.
...
- **⭐ Stars:** 61
- **Language:** Python
- **Description:** 把法律画出来 · Make the Law Visible —— 给法律人的诉讼可视化工具集：把凌乱的诉讼图重画成能进材料的图，直接读案件材料画准一张时间轴，并提供庭前三大法宝（���事记表、庭审对抗图、法律关系图）。Claude Skill / DeepSeek Harness 通用。
- **Tags:** `visualization`, `legal`, `diagram`
- **Why use it:** Legal litigation visualization: timelines, relationship diagrams, trial maps
...
- **Why use it:** AI video/image/audio generation skills with model tool integration
...
- **Tags:** `video`, `remotion`, `motion-design`, `shot-craft`
- **Why use it:** Cinematic product video Agent Skill with 152 shot recipes and Remotion
...
- **Tags:** `browser-extension`, `ai-studio`, `prompt-manager`
...
- **Why use it:** Let AI agents control your real logged-in browser without interrupting work.
...
- **Tags:** `browser`, `content-discovery`, `multi-platform`, `web-automation`
- **Why use it:** Self-evolving cross-platform content discovery agent for Bilibili, YouTube, X, etc.
...
- **Why use it:** Free, no-signup web search bridge enabling any model to access live web results natively.
...
- **Tags:** `browser-automation`, `fingerprint-spoofing`, `playwright`, `mcp`
- **Why use it:** Persistent fingerprint-spoofed browser with unlimited local profiles and Android emulation.
...
- **Tags:** `ios-automation`, `mobile-automation`, `webdriveragent`
- **Why use it:** 22 agent tools for live iOS Simulator and device control inside conversations.
...
- **Why use it:** Free Xiaomi MiMo TTS integration with prebuilt/custom/browser voice support
...
- **Tags:** `context-management`, `dsh-plugin`, `dashboard`
- **Why use it:** Context dashboard with browser integration and sidebar statistics.
...
- **Tags:** `cli`, `plugin-manager`, `hub`, `infrastructure`
- **Why use it:** Essential CLI and schema resolver for discovering, installing, and managing DSH plugins from the hub.
...
- **Tags:** `llm-provider`, `oauth`, `infrastructure`
...
- **Tags:** `llm-provider`, `command-code`, `integration`
- **Why use it:** Adds Command Code model provider with plan-aware selection and reasoning.
...
- **Tags:** `deepseek`, `dsh-plugin`, `remote`
- **Why use it:** Secure P2P multi-device remote access to Harness
...
- **Tags:** `agent-skills`, `finance`, `mcp`
- **Why use it:** tushare-based local financial DB with multi-agent support
...
- **Tags:** `skill-picker`, `ui-enhancement`, `shortcuts`
- **Why use it:** WorkBuddy-style skill picker integrating /skill-name shortcuts
...
- **Tags:** `container`, `multi-version`, `local-management`
...
- **Why use it:** One-click MCP/Skills/rules config sync across all major AI agents
...
- **Why use it:** Four-source migration wizard for sessions, memories, skills and slash cmds
...
- **Tags:** `cordis`, `orchestration`, `launcher`, `extensible`
- **Why use it:** Unifies Runtime, Provider, Plugin, Skill and MCP orchestration capabilities
...
- **Why use it:** Curated DSH plugins for task notifications, trace inspection and keybindings
...
- **Why use it:** Real-time token usage charts and cost estimates for DeepSeek API management
...
- **Why use it:** Diagnoses and resolves conflicts between DSH plugins and their versions
...
- **Tags:** `oauth`, `chatgpt`, `subscription`, `utility`
...
- **Why use it:** Comprehensive tutorial from zero to hero for harness engineering.
...
- **Tags:** `marketplace`, `plugin-shop`, `dsh-plugin`
- **Why use it:** Daily-refreshed plugin marketplace sourced and reviewed across the web.
...
- **Why use it:** One-stop research workbench: LaTeX live compile, arXiv management, experiment tracking, GPU SSH.
...
- **Tags:** `epistemic-loop`, `scientific-discovery`, `research`
- **Why use it:** Introduces epistemic loop methodology for systematic scientific discovery.
...
- **Why use it:** Cross-source academic search with citation graphs and verification for researchers.
...
- **Tags:** `kline`, `technical-analysis`, `trading`, `sidebar`
- **Why use it:** Interactive K-line chart analysis with technical indicators in sidebar
...
- **Why use it:** Unofficial community catalog covering all DSH plugin types
...
- **Tags:** `awesome-list`, `curated`, `chinese`
...
- **Why use it:** Comprehensive 1000+ plugin ecosystem catalog sorted by stars.
...
- **Tags:** `awesome-list`, `verified`, `catalog`
...
- **Tags:** `awesome-list`, `documentation`, `directory`
- **Why use it:** Plugin product manuals and scenario-based navigation guide.
...
- **Tags:** `awesome-list`, `ai`, `recommendation`
...
- **Tags:** `radar`, `research`, `awesome-list`
...
- **Tags:** `automation`, `directory`, `bilingual`
...
- **Why use it:** Officially curated DSHA plugin collection.
...
- **Why use it:** Search npm + awesome-dsh-plugin directly inside DSH agents.
...
- **Tags:** `low-code`, `dsh-plugin`, `workflows`
...
- **Why use it:** Curated directory of DSH plugins, ideal for discovering new tools.
...
- **Why use it:** Web-based plugin aggregation ecosystem for distributed DSH plugin discovery.
...
- **Why use it:** Bilingual curated directory of all known DSH plugins, tools, skills, and verified references.
...
- **Tags:** `encyclopedia`, `resource`, `awesome-list`, `ai-knowledge`
- **Why use it:** Comprehensive AI resource encyclopedia covering agents, RAG, MCP, multimodal, and learning paths.
...
- **Tags:** `awesome-list`, `plugin-discovery`, `directory`
- **Why use it:** Daily-curated DSH plugin directory with reviews and scenario-based filtering.
...
- **Why use it:** Ultimate DSH guide with quickstart resources, plugins, and toolkit links.
...
- **Why use it:** 一个面向 DeepSeek Harness 的统一能力管理插件，为 Tools/Skills 提供常驻、按需、禁用三档暴露策略，以减少上下文占用并支持运行时动态
...
- **Why use it:** Comprehensive curated directory of 280+ plugins across 14 categories
...
- **Why use it:** Comprehensive open-data plugin registry for DSH ecosystem discovery
...
- **Why use it:** Public plugin directory with install commands for DSH ecosystem discovery
...
- **Why use it:** Curated collection of useful DSH plugins for the community
...
- **Why use it:** Evidence-led curated directory with verified loadable plugins and guidance
...
- **Tags:** `awesome-list`, `curated-list`
- **Why use it:** 2026 quality-checked curated DSH plugin list for staying current
...
- **Why use it:** Curated plugin ecosystem list covering CLI, web UI and autonomous agent tools
...
- **Tags:** `dsh-repository-plugin`, `plugin-verify`, `analytics`
- **Why use it:** Plugin repository with verification and analytics tracking.
...
- **Tags:** `deprecated`, `redirect`, `awesome-list`
- **Why use it:** Deprecated — redirected to updated repo for latest plugins.
